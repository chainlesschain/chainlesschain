...
>
> **源码 HEAD 0.163.1 候选（尚未发布）**：当前 HEAD [`a29eb4203d`](https://github.com/chainlesschain/chainlesschain/commit/a29eb4203d333756fc258b493b0c43af4fc36759) 进一步修复 live session tail 在路径检查后遇到 transcript 删除/恢复时的竞态，改由 durable session witness 重新分类并失败闭合，不再泄漏平台相关的原始 `ENOENT`。候选还包含有界长会话压缩、Windows MCP 原子启动、真实跨平台可靠性探针，以及原生签名/notarization/updater 凭据 preflight。该 SHA 的 [CLI CI](https://github.com/chainlesschain/chainlesschain/actions/runs/31219886076) 与 [CLI Strict Sandbox](https://github.com/chainlesschain/chainlesschain/actions/runs/31219890201) 已成功，但[发布工作流](https://github.com/chainlesschain/chainlesschain/actions/runs/31221549835)仅执行 dry-run，exact-SHA、制品和 publish job 均跳过；`v-npm-0-163-1` 与 registry `0.163.1` 均不存在，因此生产安装契约仍固定 `0.163.0`。
>
