...
>
> **CLI 0.163.0**：完整承接 `0.162.200` 的 canonical session authority、固定 MCP npm 执行 capsule、原生更新恢复与可移植可靠性门，并修复 macOS `node-pty` helper 权限；六个原生目标改由匹配架构的真实宿主验证。该版本开启第二个命令生命周期观察周期，但 25 个兼容别名仍全部保留。
>
