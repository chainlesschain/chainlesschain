...
### 落地范围（201 测试全绿，含 unit / integration / e2e 三层）
...
测试分层：core-mtc 140 + CLI mtc/audit-mtc 单元 30 + 集成 25（mtc-cli 7 + batch-skills 3 + batch-dids 4 + serve 3 + audit-mtc 5 + publish-skills 3）+ e2e 6 = **201 全绿**。
...
**当前签名**：Ed25519 stopgap；待 `@noble/post-quantum` 上线后切 SLH-DSA-128f（接口已为此预留，单点替换）。**Audit 产线启用**仍待 Q-COMP-1（等保三级最终性窗口）+ Q-COMP-2（T/ZGCMCA 023—2025 条款摘要）法务出函；脚手架 off-by-default 已就位，出函后单一 `cc audit mtc enable --interval <60|3600>` 即可上线，无代码改动。
