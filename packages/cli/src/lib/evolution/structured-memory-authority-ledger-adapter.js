...
function normalizeDescriptor(input) {
  ...
  return Object.freeze({ tenantId: requiredString(input.tenantId, "tenantId"),
    ...
    streamId: requiredString(input.streamId, "streamId"), audience: requiredString(input.audience, "audience"),
    purpose: requiredString(input.purpose, "purpose"), authorityId: requiredString(input.authorityId, "authorityId"),
    authorityRevision: input.authorityRevision, handlerDigest: input.handlerDigest });
}

function validateCanonicalReceipt(receipt, tenantId) {
  if (receipt?.schema !== STRUCTURED_MEMORY_RECEIPT_SCHEMA || receipt.tenantId !== tenantId ||
      !DIGEST.test(receipt.receiptDigest || "")) throw new TypeError("structured memory authority receipt is invalid");
...
function parseResolution(resolution, descriptor) {
  if (resolution?.schema !== EVOLUTION_ARTIFACT_RESOLUTION_SCHEMA || resolution.authenticated !== true ||
    ...
    corrupt("structured memory receipt artifact resolution is unauthenticated or incomplete");
  }
  let record;
  try { record = JSON.parse(resolution.bytes.toString("utf8")); } catch { corrupt("structured memory receipt artifact is not JSON"); }
  if (record?.schema !== EVOLUTION_DURABLE_ARTIFACT_RECORD_SCHEMA || record.tenantId !== descriptor.artifactTenantId ||
      ...
      record.type !== ARTIFACT_TYPE) corrupt("structured memory receipt durable record binding is invalid");
  return validateCanonicalReceipt(record.value, descriptor.tenantId);
}

export class StructuredMemoryAuthorityLedgerAdapter {
  constructor({ descriptor: input, artifactPorts, ledger, ledgerArtifactResolver, receiptVerifier } = {}) {
  ...
  _events() {
    ...
    if (!Array.isArray(events)) corrupt("EvolutionLedger did not return an event array");
    return events.filter((event) => event.schema === EVOLUTION_LEDGER_DOMAIN_EVENT_SCHEMA &&
      event.type === STRUCTURED_MEMORY_AUTHORITY_RECEIPT_EVENT_TYPE && event.tenantId === this.descriptor.tenantId &&
      event.correlationId === this.descriptor.streamId);
  }

  _resolveEvent(event) {
    const identity = this._verifyLedger();
    const resolution = this._resolveArtifact({ epoch: identity.epoch, ledgerId: identity.ledgerId,
  ...
  async retainReceipt(receipt) {
    ...
    if (await this._verifyReceipt({ receipt, source: "retain", ledgerEvent: null }) !== true) {
      throw new Error("structured memory authority receipt authentication failed");
    }
    const eventId = `${STRUCTURED_MEMORY_AUTHORITY_RECEIPT_EVENT_TYPE}.${receipt.receiptDigest.slice(7)}`;
    const matching = this._events().filter((event) => event.eventId === eventId);
    if (matching.length > 1) corrupt("structured memory receipt has duplicate ledger events");
    if (matching.length === 1) {
      ...
      return Object.freeze({ persisted: true, recovered: true, receiptDigest: receipt.receiptDigest });
    }
    const head = this._verifyLedger();
    const published = this._put(ARTIFACT_TYPE, receipt, { audience: this.descriptor.audience,
    ...
    if (published?.receipt?.persisted !== true || published.receipt.readbackVerified !== true ||
      ...
      corrupt("structured memory receipt persistence was not durably confirmed");
    }
    const previous = this._events().at(-1);
    const ledgerReceipt = this._append({ artifactTenantId: this.descriptor.artifactTenantId,
      correlationId: this.descriptor.streamId, decision: "committed", eventId,
      reason: `${receipt.kind} structured memory authority receipt committed`, skillName: null,
      sourceRefs: previous ? [previous.subjectRef] : [], subjectRef: published.ref,
      tenantId: this.descriptor.tenantId, timestamp: receipt.issuedAt,
      type: STRUCTURED_MEMORY_AUTHORITY_RECEIPT_EVENT_TYPE },
    { expectedHeadDigest: head.headDigest, expectedSequence: head.sequence });
    if (ledgerReceipt?.authenticated !== true || ledgerReceipt.committed !== true || ledgerReceipt.durable !== true ||
        ledgerReceipt.eventId !== eventId || !DIGEST.test(ledgerReceipt.receiptDigest || "")) {
      corrupt("structured memory receipt ledger append was not durably confirmed");
    }
    return Object.freeze({ persisted: true, receiptDigest: receipt.receiptDigest,
      ledgerReceiptDigest: ledgerReceipt.receiptDigest });
  }

  createReceiptProvider() {
    const providerDescriptor = { tenantId: this.descriptor.tenantId, authorityId: this.descriptor.authorityId,
      authorityRevision: this.descriptor.authorityRevision, handlerDigest: this.descriptor.handlerDigest };
    return createStructuredMemoryReceiptProvider({ descriptor: providerDescriptor,
      resolver: { resolve: async (request) => {
        ...
        if (matches.length !== 1) corrupt("structured memory receipt ledger resolution is missing or ambiguous");
        const receipt = this._resolveEvent(matches[0]);
        return Object.freeze({ schema: STRUCTURED_MEMORY_RECEIPT_RESOLUTION_SCHEMA, authenticated: true,
          ...providerDescriptor, kind: request.kind, receiptDigest: request.receiptDigest, receipt,
          ledgerEventDigest: matches[0].eventDigest,
          resolutionReceiptDigest: hash({ eventDigest: matches[0].eventDigest,
            receiptDigest: request.receiptDigest, subjectRef: matches[0].subjectRef }) });
      } },
      verifier: { verify: async ({ request, resolution }) => {
        const eventId = `${STRUCTURED_MEMORY_AUTHORITY_RECEIPT_EVENT_TYPE}.${request.receiptDigest.slice(7)}`;
        const matches = this._events().filter((event) => event.eventId === eventId);
        if (matches.length !== 1 || resolution.ledgerEventDigest !== matches[0].eventDigest) return false;
        return await this._verifyReceipt({ receipt: resolution.receipt, source: "resolve",
