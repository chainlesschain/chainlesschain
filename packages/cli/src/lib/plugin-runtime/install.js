...
function preflightManagedSource(source, opts, action, options = {}) {
  ...
  if (registrySource) {
    ...
    if (
      opts.sourceMetadata.resolvedSource !== registryAuthority.declaredSource ||
      (opts.sourceMetadata.ref || null) !== (registryAuthority.ref || null)
    ) {
      throw new Error(
    ...
    if (registryAuthority.sourceType === "archive") {
      const archive = opts.sourceMetadata.catalogAuthority?.archiveSource;
      if (
        !archive ||
        archive.archiveSha256 !== registryAuthority.archiveSha256 ||
        archive.payloadSha256 !== registryAuthority.payloadSha256
      ) {
        throw new Error(
          "registry source authority does not match archive digest authority",
        );
      }
    }
  ...
  const sourceAuthority = {
    ...
    resolvedSourceDigest: registrySource
      ? registryResolvedSourceAuthorityDigest(registryAuthority)
      : null,
...
function normalizeSourceMetadata(value) {
  ...
  const catalogAuthority =
    value.catalogAuthority != null
      ? normalizeCatalogAuthority(value.catalogAuthority)
      : null;
...
function archiveSourceAuthorityDigest(source, archive) {
  return crypto
    .createHash("sha256")
    .update(
      canonicalJson({
