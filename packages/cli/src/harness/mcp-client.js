...
// Long-running MCP tools are allowed to exceed the ordinary request deadline,
// but may not retain an agent turn forever without any protocol progress. A
// server-specific value can only tighten this host ceiling; zero/invalid values
// never disable it.
const MCP_TOOL_IDLE_TIMEOUT_MS = 5 * 60 * 1000;
const MCP_DISCOVERY_CACHE_TTL_MS = 60 * 1000;
const MCP_DISCOVERY_CACHE_MAX_STALE_MS = 10 * 60 * 1000;
const MCP_DISCOVERY_CACHE_MAX_STRIKES = 2;
...
function mcpToolIdleTimeoutMs(config, options = {}) {
  const candidate = Number.isFinite(options.idleTimeoutMs)
    ? options.idleTimeoutMs
    : config?.toolIdleTimeoutMs;
  if (Number.isFinite(candidate) && candidate > 0) {
    return Math.min(MCP_TOOL_IDLE_TIMEOUT_MS, Math.floor(candidate));
  }
  return MCP_TOOL_IDLE_TIMEOUT_MS;
}

function mcpToolIdleTimeoutError(serverName, toolName, timeoutMs) {
  const error = new Error(
    `MCP tool "${toolName}" on server "${serverName}" made no progress for ${timeoutMs}ms`,
  );
  error.code = "CC_MCP_TOOL_IDLE_TIMEOUT";
...
function discoveryCachePolicy(config = {}) {
  const configured = config?.discoveryCache;
  if (configured !== true && (!configured || typeof configured !== "object")) {
    return { enabled: false };
  }
  if (configured?.enabled === false) return { enabled: false };
  const fields = configured === true ? {} : configured;
  const ttlMs = boundedPositiveInt(
    fields.ttlMs,
    MCP_DISCOVERY_CACHE_TTL_MS,
    MCP_DISCOVERY_CACHE_MAX_STALE_MS,
  );
  const maxStaleMs = boundedPositiveInt(
    fields.maxStaleMs,
    MCP_DISCOVERY_CACHE_MAX_STALE_MS,
    24 * 60 * 60 * 1000,
  );
  return {
    enabled: true,
    ttlMs,
    maxStaleMs: Math.max(ttlMs, maxStaleMs),
    maxStrikes: boundedPositiveInt(
      fields.maxStrikes,
      MCP_DISCOVERY_CACHE_MAX_STRIKES,
      10,
    ),
  };
}

function discoveryCapabilitiesSignature(capabilities) {
  const value =
    capabilities && typeof capabilities === "object" ? capabilities : {};
  return JSON.stringify({
    tools: value.tools !== undefined,
    resources: value.resources !== undefined,
    prompts: value.prompts !== undefined,
  });
}

function discoveryCacheKey(name, entry) {
  const config = entry?.config || {};
  // This key never leaves process memory. Bind to an endpoint/launcher so a
  // same-named server pointed at a different target cannot inherit tools.
  return JSON.stringify([
    String(name),
    entry?.transportKind || inferTransport(config),
    entry?.httpUrl || entry?.socketUrl || config.url || null,
    config.command || null,
    Array.isArray(config.args) ? config.args : [],
  ]);
}

function cloneDiscoveryList(value) {
...
export class MCPClient extends EventEmitter {
  ...
  constructor(options = {}) {
    ...
    // progressToken → reset-idle callback for in-flight tools/call requests.
    // Tokens are generated by this client and are removed on every terminal
    // path, so a peer notification cannot retain timers or request state.
    this._toolProgress = new Map();
    // Capability metadata stays process-local by default. A server must opt in
    // through `discoveryCache`; each reconnect still performs initialize and
    // all policy/transport admission before a cache entry is considered.
    this._discoveryCache =
      options?.discoveryCache instanceof Map
  ...
  async connect(name, config, internalOptions = false) {
    ...
    try {
      ...
      const cachePolicy = discoveryCachePolicy(entry.config);
      const cacheKey = discoveryCacheKey(name, entry);
      const cached = cachePolicy.enabled
        ? this._discoveryCache.get(cacheKey) || null
        : null;
      const now = Date.now();
      const capabilitiesSignature = discoveryCapabilitiesSignature(
        entry.capabilities,
      );
      const cacheCompatible =
        cached?.capabilitiesSignature === capabilitiesSignature;
      const cacheAgeMs = cacheCompatible
        ? Math.max(0, now - cached.savedAt)
        : 0;
      ...
      if (cacheCompatible && cacheAgeMs <= cachePolicy.ttlMs) {
        ...
        // Fetch available tools. Per MCP a server advertises a `tools`
        // capability in its initialize response; if it does and tools/list
        // fails, that is a genuine fetch failure we must surface. A server that
        // did not advertise tools simply has none, so a failure stays quiet.
        entry.tools = [];
        entry.toolsError = null;
        const advertisesTools =
          entry.capabilities && entry.capabilities.tools !== undefined;
        let toolsDiscoveryFailed = false;
        try {
        ...
        const staleUsable =
          cacheCompatible &&
          cacheAgeMs <= cachePolicy.maxStaleMs &&
          cached.strikes < cachePolicy.maxStrikes;
        ...
        } else if (cachePolicy.enabled && !toolsDiscoveryFailed) {
          this._discoveryCache.set(cacheKey, {
            savedAt: now,
            strikes: 0,
            capabilitiesSignature,
            tools: cloneDiscoveryList(entry.tools),
            resources: cloneDiscoveryList(entry.resources),
            resourceTemplates: cloneDiscoveryList(entry.resourceTemplates),
            prompts: cloneDiscoveryList(entry.prompts),
          });
          entry.discoveryCache = { state: "miss", ageMs: null };
        }
      }
  ...
  async _callToolOnce(serverName, toolName, args, options = {}) {
    ...
    const timeoutMs = mcpToolIdleTimeoutMs(entry.config, options);
    const progressToken = `cc:${randomUUID()}`;
    ...
    const resetIdleTimer = () => {
      ...
      idleTimer = setTimeout(() => {
        idleError = mcpToolIdleTimeoutError(serverName, toolName, timeoutMs);
        controller.abort();
      }, timeoutMs);
      idleTimer.unref?.();
    };
    ...
    this._toolProgress.set(progressToken, resetIdleTimer);
    resetIdleTimer();

    let result;
    try {
      result = await this._sendRequest(
        serverName,
        "tools/call",
        {
          name: toolName,
      ...
      // The inner transport supplies cancellation and JSON-RPC
      // notifications/cancelled. Preserve the idle classification at the host
      // boundary instead of leaking its generic AbortError/cancelled detail.
      ...
      this._toolProgress.delete(progressToken);
    }
  ...
  _handleMessage(serverName, msg, sourceEntry = null) {
    ...
    if (typeof wire.method === "string" && wire.method) {
      ...
      if (wire.method === "notifications/tools/list_changed") {
        if (entry._discoveryCacheKey) {
          this._discoveryCache.delete(entry._discoveryCacheKey);
        }
      ...
      } else if (wire.method === "notifications/progress") {
        const progressToken = safeRpcProperty(wire.params, "progressToken");
        const resetIdle =
          typeof progressToken === "string"
            ? this._toolProgress.get(progressToken)
            : null;
        if (resetIdle) resetIdle();
        // Project only numeric progress. Server-controlled status text belongs
        // to the tool result, not host diagnostics/events.
        this.emit("tool-progress", {
          server: serverName,
          ...(typeof progressToken === "string" ? { progressToken } : {}),
          ...(Number.isFinite(safeRpcProperty(wire.params, "progress"))
            ? { progress: safeRpcProperty(wire.params, "progress") }
            : {}),
          ...(Number.isFinite(safeRpcProperty(wire.params, "total"))
            ? { total: safeRpcProperty(wire.params, "total") }
            : {}),
        });
