...
import {
  EVOLUTION_RAW_DELETION_LEDGER_CORRUPT_CODE,
...
const TENANT_ID = "tenant:raw-deletion";
const ARTIFACT_TENANT_ID = "artifact-tenant-raw-deletion";
const roots = [];
...
function signingAuthority(label) {
  const trust = Object.freeze({
    ...
    keyId: `key://tests/raw-deletion-${label}`,
    trustPolicyDigest: digest(`${label}-policy`),
  });
  const secret = `test-only-raw-deletion-${label}-secret`;
  const sign = (message) =>
...
const RECEIPT_SECRET = "test-only-privacy-governor-secret";
...
function deletionReceipt() {
  const unsigned = {
    ...
    tenantId: TENANT_ID,
    decision: "delete",
    evidenceRef: "evidence:private-one",
    sourceDigest: digest("private-source"),
    artifactRef: "artifact://trusted/private-one",
    rawArtifactRef: `artifact://${TENANT_ID}/raw/private-one`,
    rawCipherDigest: digest("private-ciphertext"),
    keyRef: `kms://${TENANT_ID}/private-one`,
    issuedAt: NOW,
    attestation: {
      algorithm: "hmac-sha256",
      issuer: "authority:privacy-governor",
      keyId: "key://tests/privacy-governor",
      trustPolicyDigest: digest("privacy-governor-policy"),
      value: "pending",
    },
  };
...
function resources() {
  const root = fs.mkdtempSync(
    path.join(fs.realpathSync.native(os.tmpdir()), "cc-raw-deletion-ledger-"),
  );
  roots.push(root);
  const now = Date.parse(NOW);
  const artifactSecret = "test-only-raw-deletion-artifact-secret";
  const algorithm = "hmac-sha256";
  const keyId = "test:key/raw-deletion-artifacts";
  const policyDigest = digest("raw-deletion-artifact-policy");
  const sign = (message) =>
  ...
  const backendOptions = {
    ...
    witnessId: "raw-deletion-ledger-witness",
  ...
  const deletionReceiptVerifier = {
    verify: vi.fn(({ receipt }) => {
...
function adapter(input, ledger) {
  return new EvolutionRawDeletionLedgerAdapter({
    descriptor: {
      tenantId: TENANT_ID,
      artifactTenantId: ARTIFACT_TENANT_ID,
      streamId: "raw-deletion:privacy",
      audience: "evolution-runtime",
      purpose: "evolution-ledger",
      authorityId: "authority:privacy-deletion-ledger",
      revision: 1,
      handlerArtifactDigest: digest("raw-deletion-handler"),
    },
    artifactPorts: input.artifactPorts,
    ledger,
    ledgerArtifactResolver: input.resolver,
    deletionReceiptVerifier: input.deletionReceiptVerifier,
    now: () => Date.parse(NOW),
  });
}
...
function pruningCall(receipt) {
  const request = {
    tenantId: TENANT_ID,
    planDigest: digest("pruning-plan"),
    wikiStateDigest: digest("wiki-state"),
...
describe("EvolutionRawDeletionLedgerAdapter", () => {
  it("persists a verified receipt and crypto-shred tombstone across Ledger reopen", async () => {
    const storage = resources();
    const firstBackend = createEvolutionLedgerFileBackend(
      storage.backendOptions,
    );
    const first = adapter(storage, firstBackend.ledger);
    const receipt = deletionReceipt();
    await expect(
      first.retainDeletionReceipt({ receipt }),
    ...
    const shredder = new EvolutionRawCryptoShred({
      ...
      ports: first.cryptoShredPorts({
        destroyKey: async (request) => ({
          ...
          receiptDigest: digest("kms-destruction"),
        }),
        confirmKeyDestroyed: async ({ destructionReceiptDigest }) => ({
          authenticated: true,
          destroyed: true,
          keyRef: receipt.keyRef,
          destructionReceiptDigest,
          receiptDigest: digest("kms-confirmation"),
        }),
      }),
    });
    const shredded = await shredder.shred(pruningCall(receipt));
    expect(firstBackend.ledger.verify()).toMatchObject({ sequence: 2 });

    const reopenedBackend = createEvolutionLedgerFileBackend(
      storage.backendOptions,
    );
    const reopened = adapter(storage, reopenedBackend.ledger);
    await expect(
      reopened.resolveDeletionReceipt({
        tenantId: TENANT_ID,
        receiptDigest: receipt.receiptDigest,
      }),
    ).resolves.toMatchObject({
    ...
    await expect(
      reopened.resolveTombstone({
        tenantId: TENANT_ID,
        tombstoneDigest: shredded.tombstoneDigest,
      }),
    ).resolves.toMatchObject({
      authenticated: true,
      durable: true,
      tombstone: {
        deletionReceiptDigest: receipt.receiptDigest,
        rawArtifactRef: receipt.rawArtifactRef,
      },
    });
  ...
  it("recovers identical receipt and tombstone retries without duplicate events", async () => {
    const storage = resources();
    const backend = createEvolutionLedgerFileBackend(storage.backendOptions);
    const subject = adapter(storage, backend.ledger);
    const receipt = deletionReceipt();
    await subject.retainDeletionReceipt({ receipt });
    await expect(
      subject.retainDeletionReceipt({ receipt }),
    ).resolves.toMatchObject({ recovered: true });
    const shredder = new EvolutionRawCryptoShred({
      tenantId: TENANT_ID,
      ports: subject.cryptoShredPorts({
        destroyKey: async (request) => ({
          authenticated: true,
          durable: true,
          destroyed: true,
    ...
    const first = await shredder.shred(pruningCall(receipt));
    const second = await shredder.shred(pruningCall(receipt));
    expect(second.tombstoneDigest).toBe(first.tombstoneDigest);
    expect(backend.ledger.verify()).toMatchObject({ sequence: 2 });
  });

  it("rejects forged receipts and tombstones with substituted receipt lineage", async () => {
    const storage = resources();
    const backend = createEvolutionLedgerFileBackend(storage.backendOptions);
    const subject = adapter(storage, backend.ledger);
    const receipt = deletionReceipt();
    const forged = structuredClone(receipt);
    forged.attestation.value = "forged-attestation-value-00000000000000000000";
    forged.receiptDigest = digestEvolutionRawDeletionReceipt(forged);
    await expect(
      subject.retainDeletionReceipt({ receipt: forged }),
    ).rejects.toMatchObject({
      code: EVOLUTION_RAW_DELETION_LEDGER_CORRUPT_CODE,
    });
    expect(backend.ledger.verify()).toMatchObject({ sequence: 0 });

    await subject.retainDeletionReceipt({ receipt });
    const other = deletionReceipt();
    const tombstoneCore = {
      schema: "chainlesschain.evolution-raw-deletion-tombstone/v1",
      tenantId: TENANT_ID,
      pruningRequestDigest: digest("pruning-request"),
      destructionRequestDigest: digest("destruction-request"),
      destructionReceiptDigest: digest("destruction-receipt"),
      confirmationReceiptDigest: digest("confirmation-receipt"),
      evidenceRef: other.evidenceRef,
      rawArtifactRef: `${other.rawArtifactRef}-substituted`,
      rawCipherDigest: other.rawCipherDigest,
      deletionReceiptDigest: receipt.receiptDigest,
    };
    await expect(
      subject.retainTombstone({
        tombstone: {
          ...tombstoneCore,
