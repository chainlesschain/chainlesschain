...
function ports({ state, evidenceByRef, operations = [], overrides = {} } = {}) {
  ...
  return {
    ...
    resolveEvidence: vi.fn(async (ref) => evidenceByRef?.[ref] ?? evidence(ref)),
...
async function maintain(options = {}) {
  ...
  const maintainer = new EvidenceBackedWikiMaintainer({ descriptor: descriptor(), policy, ports: p });
...
describe("EvidenceBackedWikiMaintainer", () => {
  it("creates a corroborated actionable pattern and atomically binds its revision to EvolutionRun", async () => {
    ...
    const { result, ports: p } = await maintain({ evidenceByRef, operations: [{ type: "upsert", pattern: pattern() }] });
    ...
    expect(result.state.evidenceDependents["ev-1"]).toEqual(["pat-safe-refactor"]);
    expect(p.commitRevision).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it("keeps a single-source model summary as a non-actionable hypothesis", async () => {
    const modelEvidence = evidence("ev-1", { kind: "model-inference", trustDomain: "model-a" });
    const { result } = await maintain({
      evidenceRefs: ["ev-1"],
      evidenceByRef: { "ev-1": modelEvidence },
      operations: [{ type: "upsert", pattern: pattern({ positiveEvidence: ["ev-1"] }) }],
    });
    expect(result.state.patterns["pat-safe-refactor"]).toMatchObject({ status: "hypothesis", actionable: false });
  });

  it("allows a real grader receipt to corroborate a procedure without pretending it is multi-source", async () => {
    const grader = evidence("ev-1", { kind: "grader-receipt", trustDomain: "grader-suite" });
    const { result } = await maintain({
      evidenceRefs: ["ev-1"], evidenceByRef: { "ev-1": grader },
      operations: [{ type: "upsert", pattern: pattern({ positiveEvidence: ["ev-1"] }) }],
    });
    expect(result.state.patterns["pat-safe-refactor"]).toMatchObject({ status: "corroborated", actionable: true });
  });

  it("deduplicates an equivalent cluster and retains all evidence lineage", async () => {
    const first = await maintain({
      evidenceByRef: { "ev-1": evidence("ev-1", { trustDomain: "a" }), "ev-2": evidence("ev-2", { trustDomain: "b" }) },
      operations: [{ type: "upsert", pattern: pattern() }],
    });
    const duplicate = pattern({ patternId: "pat-duplicate-refactor", positiveEvidence: ["ev-3"] });
    ...
    expect(Object.keys(second.result.state.patterns)).toEqual(["pat-safe-refactor"]);
    expect(second.result.state.patterns["pat-safe-refactor"].positiveEvidence).toEqual(["ev-1", "ev-2", "ev-3"]);
    expect(second.result.state.evolutionLog.at(-1).type).toBe("pattern-deduplicated");
  });
  ...
  it("maintains counterevidence and contradiction as a fail-closed pattern state", async () => {
    const { result } = await maintain({
      ...
      evidenceByRef: {
        ...
        "ev-negative": evidence("ev-negative", { trustDomain: "grader-negative", kind: "grader-receipt" }),
      },
      operations: [{ type: "upsert", pattern: pattern({ negativeEvidence: ["ev-negative"] }) }],
    });
    expect(result.state.patterns["pat-safe-refactor"]).toMatchObject({
      status: "contradicted", actionable: false,
  ...
  it("records a rejected proposal as impact without rolling back Wiki history", async () => {
    ...
    const decisionCore = {
      candidateId: "candidate-1", skillName: "safe-refactor", outcome: "rejected",
      patternRefs: ["pat-safe-refactor"], reason: "holdout regression",
    };
    const decision = evidence("decision-1", {
      kind: "proposal-decision", trustDomain: "review-board", data: { decisionDigest: hash(decisionCore) },
    });
    const second = await maintain({
      state: first.result.state,
      evidenceRefs: ["decision-1"], evidenceByRef: { "decision-1": decision },
      operations: [{ type: "proposal-impact", decision: { ...decisionCore, receiptRef: "decision-1" } }],
    });
    expect(second.result.state.patterns["pat-safe-refactor"]).toMatchObject({
      status: "corroborated", rejectionCount: 1, actionable: false, operationalConfidence: 0.4,
    });
    expect(second.result.state.skillImpact["safe-refactor"]).toMatchObject({ accepted: 0, rejected: 1 });
  ...
  it("rejects proposal-impact field substitution against its authenticated receipt", async () => {
    ...
    const receipt = evidence("decision-1", {
      kind: "proposal-decision", trustDomain: "review-board", data: { decisionDigest: hash(signedDecision) },
    });
    const p = ports({
      state: first.result.state,
      evidenceByRef: { "decision-1": receipt },
      operations: [{ type: "proposal-impact", decision: { ...signedDecision, receiptRef: "decision-1", reason: "different reason" } }],
    });
    await expect(new EvidenceBackedWikiMaintainer({ descriptor: descriptor(), policy, ports: p })
      .maintain({ evidenceRefs: ["decision-1"], effectiveAt: "2026-09-03T00:00:00.000Z" }))
  ...
  it("propagates evidence revocation and expiry to dependents and prunes the default index", async () => {
    ...
    const second = await maintain({
      ...
      evidenceByRef: {
        ...
        "ev-2": evidence("ev-2", { trustDomain: "b", expiresAt: "2026-09-02T00:00:00.000Z" }),
      },
      operations: [],
      effectiveAt: "2026-09-03T00:00:00.000Z",
    });
    expect(second.result.state.patterns["pat-safe-refactor"].status).toBe("stale");
    expect(second.result.state.index).toEqual([]);
    expect(second.result.state.evidenceDependents["ev-2"]).toEqual(["pat-safe-refactor"]);
  });
  ...
  it("deterministically decays old knowledge to stale without deleting its lineage", async () => {
    ...
    expect(result.state.patterns["pat-safe-refactor"]).toMatchObject({ status: "stale", actionable: false });
    expect(result.state.patterns["pat-safe-refactor"].operationalConfidence).toBeLessThan(0.2);
    expect(result.state.patterns["pat-safe-refactor"].positiveEvidence).toEqual(["ev-1", "ev-2"]);
  ...
  it("supports explicit merge and tombstone lifecycle operations", async () => {
    ...
    const created = await maintain({
      ...
      operations: [
        ...
        { type: "upsert", pattern: pattern({ patternId: "pat-other-success", summary: "A second bounded refactor pattern.", rootCause: "A separate small-change cluster.", positiveEvidence: ["ev-3"] }) },
      ],
    });
    const merged = await maintain({
      state: created.result.state, evidenceRefs: ["ev-3"],
      evidenceByRef: { "ev-3": evidence("ev-3", { trustDomain: "c" }) },
      operations: [{ type: "merge", targetPatternId: "pat-safe-refactor", sourcePatternIds: ["pat-other-success"] }],
    });
    expect(merged.result.state.patterns["pat-other-success"].status).toBe("revoked");
    const tombstoned = await maintain({
      state: merged.result.state, evidenceRefs: ["ev-1"],
      evidenceByRef: { "ev-1": evidence("ev-1", { trustDomain: "a" }) },
      operations: [{ type: "tombstone", patternId: "pat-safe-refactor", reason: "privacy deletion" }],
    });
    expect(tombstoned.result.state.patterns["pat-safe-refactor"].status).toBe("tombstoned");
  ...
  it("rejects raw/secret material, cross-tenant evidence, and excessive authority", async () => {
    expect(() => new EvidenceBackedWikiMaintainer({
      descriptor: descriptor(), policy: { ...policy, activeSkillWrite: true }, ports: ports(),
    ...
    for (const bad of [
      ...
      bad.envelopeDigest = hash(Object.fromEntries(Object.entries(bad).filter(([key]) => key !== "envelopeDigest")));
      const p = ports({ evidenceByRef: { "ev-1": bad } });
      await expect(new EvidenceBackedWikiMaintainer({ descriptor: descriptor(), policy, ports: p })
        .maintain({ evidenceRefs: ["ev-1"], effectiveAt: "2026-09-02T00:00:00.000Z" })).rejects.toThrow();
  ...
  it("fails closed when persistence cannot confirm exact revision identity", async () => {
    const p = ports({
      ...
      operations: [],
      overrides: { commitRevision: vi.fn(async () => ({ committed: true, revisionId: "wiki:substituted" })) },
    });
    await expect(new EvidenceBackedWikiMaintainer({ descriptor: descriptor(), policy, ports: p })
      .maintain({ evidenceRefs: ["ev-1"], effectiveAt: "2026-09-02T00:00:00.000Z" }))
