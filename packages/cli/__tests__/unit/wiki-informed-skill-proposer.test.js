...
function validProposal(refs) {
  return {
    ...
    machineDiff: [
      { op: "replace", path: "SKILL.md", beforeDigest: "old", afterDigest: "new" },
      { op: "add", path: "PURPOSE.md", beforeDigest: null, afterDigest: "purpose" },
    ],
  };
}
...
function ports(overrides = {}) {
  const initial = {
    ...
    "skill-impact": envelope("skill-impact", { affectedSkills: ["safe-refactor"] }),
    "active-skill": envelope("active-skill", { skillName: "safe-refactor", digest: "active" }),
    "training-summary": envelope("training-summary", { sampleCount: 5, passed: 4 }),
  };
  const refs = Object.values(initial).map((item) => item.ref);
  return {
    readInitial: vi.fn(async (kind) => initial[kind]),
    readSelective: vi.fn(async (kind, ref) => envelope(kind, { selected: true }, ref)),
...
describe("WikiInformedSkillProposer", () => {
  it("creates one candidate with PURPOSE, boundaries, lineage and machine diff", async () => {
    const p = ports();
    const result = await new WikiInformedSkillProposer({ descriptor: descriptor(), policy, ports: p }).propose();
    ...
    expect(p.readInitial.mock.calls.map((call) => call[0])).toEqual([
      "wiki-index", "skill-impact", "active-skill", "training-summary",
    ]);
  ...
  it("abstains on insufficient samples instead of forcing a change", async () => {
    ...
    p.readInitial = vi.fn(async (kind) =>
      kind === "training-summary" ? envelope(kind, { sampleCount: 2 }) : original(kind),
    );
    const result = await new WikiInformedSkillProposer({ descriptor: descriptor(), policy, ports: p }).propose();
    expect(result).toMatchObject({ status: "needs-evidence", reason: expect.stringContaining("insufficient") });
    expect(p.createCandidate).not.toHaveBeenCalled();
  });
  ...
  it("selectively reads only requested pattern/raw evidence and retries once", async () => {
    ...
    const initialRefs = ["evidence://wiki-index/1", "evidence://skill-impact/1", "evidence://active-skill/1", "evidence://training-summary/1"];
    p.generate = vi.fn()
      .mockResolvedValueOnce({ status: "needs-evidence", requests: [{ kind: "pattern", ref: patternRef }] })
  ...
  it("returns no-proposal without writing a candidate", async () => {
    const p = ports({ generate: vi.fn(async () => ({ status: "no-proposal", reason: "no measurable benefit" })) });
    const result = await new WikiInformedSkillProposer({ descriptor: descriptor(), policy, ports: p }).propose();
  ...
  it("rejects a second Skill, unsafe active path, and unknown PURPOSE lineage", async () => {
    for (const mutate of [
      (o) => { o.skillName = "other-skill"; },
      (o) => { o.machineDiff[0].path = "../active/SKILL.md"; },
      (o) => { o.purpose.sourceEvidenceRefs.push("evidence://unknown/1"); },
    ]) {
      const p = ports();
      const refs = ["evidence://wiki-index/1", "evidence://skill-impact/1", "evidence://active-skill/1", "evidence://training-summary/1"];
      const output = validProposal(refs);
      mutate(output);
      p.generate = vi.fn(async () => output);
      await expect(new WikiInformedSkillProposer({ descriptor: descriptor(), policy, ports: p }).propose()).rejects.toThrow();
      expect(p.createCandidate).not.toHaveBeenCalled();
    }
  });
  ...
  it("rejects tampered evidence before invoking the generator", async () => {
    ...
    p.readInitial = vi.fn(async (kind) => {
      ...
      return kind === "wiki-index" ? { ...value, data: { contradictionRefs: [], tampered: true } } : value;
    });
    await expect(new WikiInformedSkillProposer({ descriptor: descriptor(), policy, ports: p }).propose()).rejects.toMatchObject({ code: "WIKI_PROPOSAL_UNTRUSTED_EVIDENCE" });
    expect(p.generate).not.toHaveBeenCalled();
  });

  it("requires the Wiki capability policy to exclude execution agents", () => {
    expect(() => new WikiInformedSkillProposer({ descriptor: descriptor(), policy: { proposerWikiRead: true, executionAgentWikiRead: true }, ports: ports() })).toThrow(/isolate Wiki reads/);
  });

  it("fails closed when the candidate sink cannot confirm immutable content", async () => {
    const p = ports({ createCandidate: vi.fn(async () => ({ created: true })) });
    await expect(new WikiInformedSkillProposer({ descriptor: descriptor(), policy, ports: p }).propose()).rejects.toMatchObject({ code: "WIKI_PROPOSAL_CANDIDATE_UNCONFIRMED" });
  });
  ...
  it("rejects a sink that substitutes the target runtime after proposal review", async () => {
    ...
    await expect(
      new WikiInformedSkillProposer({ descriptor: descriptor(), policy, ports: p }).propose(),
