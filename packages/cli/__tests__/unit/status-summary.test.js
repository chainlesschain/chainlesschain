...
describe("formatStatus", () => {
  ...
  it("renders version, model, session, cwd and connection counts", () => {
    ...
    expect(out).toMatch(
      /cc 0\.162\.130 {2}· {2}node v22\.22\.2 {2}· {2}win32-x64/,
    );
    expect(out).toMatch(
      /provider\/model: volcengine \/ doubao-seed-1-6-251015/,
    );
    expect(out).toMatch(/session: sess_abc {2}· {2}messages: 12/);
    expect(out).toMatch(/cwd: \/proj/);
    expect(out).toMatch(
      /IDE: none {2}· {2}MCP: 0 server\(s\) {2}· {2}hooks: 0 event\(s\)/,
    );
  ...
  it("flags a disk-newer version (restart to apply)", () => {
    ...
    expect(out).toMatch(
      /cc 0\.162\.130 \(disk 0\.162\.131 — restart to apply\)/,
    );
  });
  ...
  it("shows extra roots and live connection counts", () => {
    ...
    expect(out).toMatch(
      /IDE: connected {2}· {2}MCP: 3 server\(s\) {2}· {2}hooks: 2 event\(s\)/,
    );
  });
