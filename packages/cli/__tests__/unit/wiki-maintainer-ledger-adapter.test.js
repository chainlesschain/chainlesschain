...
const LEDGER_TRUST = {
  ...
  keyId: "key://tests/wiki-ledger",
  trustPolicyDigest: hash("wiki-ledger-trust"),
};
const WITNESS_TRUST = {
  algorithm: "hmac-sha256",
  keyId: "key://tests/wiki-witness",
...
function durableWitness(witnessId) {
  ...
  return {
    ...
    proveAncestry: () => {
      throw new Error("unexpected ancestry request in linear Wiki test");
    },
  };
}
...
function durableBackends() {
  ...
  const ledger = {
    ...
    appendDomainEvent: vi.fn((input, options) => {
      ...
      if (
        options.expectedSequence !== ledgerState.events.length ||
  ...
  return {
    artifactPorts: artifactWriter,
    rawArtifactPorts: artifactPorts,
    putSpy,
    ledger,
    ledgerState,
    resolver,
...
function operation() {
  return {
    type: "upsert",
    pattern: {
      patternId: "pat-durable-wiki",
      kind: "success",
      summary: "Durable Wiki revisions recover across adapter instances.",
      rootCause: "Artifact and ledger identities are bound.",
      procedure: "Commit immutable state then append its ledger event.",
      appliesWhen: ["durable authorities are available"],
      doesNotApplyWhen: ["the ledger is unavailable"],
...
function maintainer(adapter, overrides = {}) {
  ...
  return new EvidenceBackedWikiMaintainer({
    descriptor: {
      tenantId: "tenant-a",
      evolutionRunId: "run-1",
      maintainerModel: "provider:model-v1",
...
function adapter(backends) {
  return new WikiMaintainerLedgerAdapter({
    descriptor,
    artifactPorts: backends.artifactPorts,
    ledger: backends.ledger,
    ledgerArtifactResolver: backends.resolver,
  });
}
...
describe("WikiMaintainerLedgerAdapter", () => {
  it("round-trips through real EvolutionLedger files and a reopened ledger instance", async () => {
    ...
    const firstAdapter = new WikiMaintainerLedgerAdapter({
      descriptor,
      artifactPorts: backends.artifactPorts,
      ledger: firstLedger,
      ledgerArtifactResolver: backends.resolver,
    });
    const result = await maintainer(firstAdapter).maintain({
      evidenceRefs: ["ev-1", "ev-2"],
      effectiveAt: "2026-09-02T00:00:00.000Z",
    });

    const reopenedLedger = openRealLedger(backends, witness);
    const reopenedAdapter = new WikiMaintainerLedgerAdapter({
      descriptor,
      artifactPorts: backends.artifactPorts,
      ledger: reopenedLedger,
      ledgerArtifactResolver: backends.resolver,
    });
    expect(reopenedAdapter.loadWiki()).toMatchObject({
  ...
  it("persists a Wiki revision in immutable artifacts and recovers it through a new adapter instance", async () => {
    const backends = durableBackends();
    const first = await maintainer(adapter(backends)).maintain({
      evidenceRefs: ["ev-1", "ev-2"],
      effectiveAt: "2026-09-02T00:00:00.000Z",
    });
    ...
    expect(backends.putSpy).toHaveBeenCalledWith(
      "wiki-revision",
      expect.any(Object),
      {
        audience: "evolution-runtime",
  ...
  it("allows exactly one concurrent writer from the same Wiki baseline", async () => {
    ...
    const derive = async () => {
      ...
      if (arrival === 2) {
        ...
        next.pattern.summary =
          "A competing Wiki revision from the same baseline.";
        next.pattern.rootCause =
          "A concurrent maintainer derived a different cluster.";
      }
      return { operations: [next] };
    };
    const results = await Promise.allSettled([
      maintainer(adapter(backends), { derive }).maintain({
        evidenceRefs: ["ev-1", "ev-2"],
        effectiveAt: "2026-09-02T00:00:00.000Z",
      }),
      maintainer(adapter(backends), { derive }).maintain({
    ...
    ).toBe(WIKI_LEDGER_CONFLICT_CODE);
    expect(backends.ledgerState.events).toHaveLength(1);
  });
  ...
  it("recovers an idempotent commit after ledger response loss", async () => {
    ...
    let request;
    const wrapped = {
      ...persisted.maintainerPorts({
        resolveEvidence: async (ref) => evidence(ref, ref),
        derive: async () => ({ operations: [operation()] }),
      }),
      commitRevision: (input) => {
        request = input;
        return persisted.commitRevision(input);
      },
    };
    const control = new EvidenceBackedWikiMaintainer({
      descriptor: {
        tenantId: "tenant-a",
        evolutionRunId: "run-1",
        maintainerModel: "provider:model-v1",
    ...
    expect(persisted.commitRevision(request)).toMatchObject({
      committed: true,
      recovered: true,
    });
    expect(backends.ledgerState.events).toHaveLength(1);
  ...
  it("rejects an unbranded resolver before it can substitute stored bytes", async () => {
    const backends = durableBackends();
    await maintainer(adapter(backends)).maintain({
      evidenceRefs: ["ev-1", "ev-2"],
      effectiveAt: "2026-09-02T00:00:00.000Z",
    });
    const tamperedResolver = (request) => ({
      ...backends.resolver(request),
      bytes: Buffer.from("{}"),
    });
    expect(
      () =>
        new WikiMaintainerLedgerAdapter({
          descriptor,
          artifactPorts: backends.artifactPorts,
          ledger: backends.ledger,
          ledgerArtifactResolver: tamperedResolver,
        }),
    ).toThrow(/branded/);
  });
  ...
  it("uses the EvolutionLedger head CAS so unrelated concurrent appends cannot be hidden", async () => {
    ...
    backends.ledgerState.beforeAppend = () => {
      ...
      backends.ledgerState.events.push({
        schema: EVOLUTION_LEDGER_DOMAIN_EVENT_SCHEMA,
        sequence: 1,
        eventDigest: hash("other"),
        type: "other.event",
        tenantId: "tenant-a",
        correlationId: null,
      });
    ...
    await expect(
      maintainer(adapter(backends)).maintain({
        evidenceRefs: ["ev-1", "ev-2"],
        effectiveAt: "2026-09-02T00:00:00.000Z",
      }),
    ).rejects.toMatchObject({ code: "CC_EVOLUTION_LEDGER_HEAD_CONFLICT" });
    expect(
      backends.ledgerState.events.filter(
