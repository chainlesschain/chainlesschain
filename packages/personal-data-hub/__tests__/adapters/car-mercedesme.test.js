...
describe("car-mercedesme", () => {
  it("name + tripToRecord (km direct + meters fallback; no id → null)", () => {
    ...
    // meters fallback
    expect(
      mb.tripToRecord({ tripId: "T2", distanceMeters: 5230 }).extras.distanceKm,
    ).toBeCloseTo(5.23, 2);
    ...
    const a = new mb.MercedesMeAdapter({
      ...
      fetchFn: async ({ query }) =>
        query.page > 1
          ...
          : {
              trips: [
                { tripId: "T9", startTime: 1716383000, distanceMeters: 8000 },
              ],
            },
    });
  ...
  it("does not complete a capped scan on a non-empty short page", async () => {
    ...
    const a = new mb.MercedesMeAdapter({
      ...
      listUrl: "https://captured.example/trips",
      fetchFn: async () => ({
        trips: [{ tripId: "T-short", startTime: 1716383000 }],
      }),
    });
  ...
  it("medium sensitivity; default fetch throws", async () => {
    expect(new mb.MercedesMeAdapter().dataDisclosure.sensitivity).toBe(
      "medium",
    );
    const unverified = new mb.MercedesMeAdapter({
      account: { cookies: COOKIES },
    });
