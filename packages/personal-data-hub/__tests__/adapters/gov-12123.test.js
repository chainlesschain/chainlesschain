...
describe("gov-12123", () => {
  ...
  it("snapshot → violation + license OTHER events", async () => {
    ...
    try {
      ...
      expect(items[1].capturedAt).toBe(1716383000000);
    } finally {
      fs.unlinkSync(p);
    }
  });
  ...
  it("cookie-api: paginated violations + single license + unverified", async () => {
    ...
    const license = items.find((item) => item.kind === "license");
    expect(license.capturedAt).toBeLessThan(
      license.payload.record.validUntilMs,
    );
  ...
  it("province base host (verified .122.gov.cn/app); override + default", () => {
    ...
    expect(
      new g.Tmri12123Adapter({ violationUrl: "https://x/y" })._urls.violation,
    ).toBe("https://x/y");
  });
  ...
  it("high sensitivity + legalGate; default fetch / no input throw", async () => {
    ...
    const unverified = new g.Tmri12123Adapter({
    ...
    await expect(collect(unverified.sync({}))).rejects.toThrow(
      /explicit violationUrl/,
    );
    await expect(collect(new g.Tmri12123Adapter().sync({}))).rejects.toThrow(
