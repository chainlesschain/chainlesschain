...
describe("orderToRecord", () => {
  ...
  it("maps refund / cancelled status", () => {
    expect(
      orderToRecord({ orderId: "R", shopName: "x", statusText: "已退款" })
        .status,
    ).toBe("refunded");
    expect(
      orderToRecord({ orderId: "C", shopName: "x", statusText: "已取消" })
        .status,
    ).toBe("cancelled");
  });
...
describe("DianpingAdapter snapshot mode", () => {
  ...
  it("authenticate validates inputPath readability", async () => {
    ...
    try {
      ...
      const bad = await a.authenticate({
        inputPath: path.join(os.tmpdir(), "nope-dp.json"),
      });
  ...
  it("sync yields order + normalize end-to-end", async () => {
    ...
    try {
      ...
      expect(batch.events[0].content.amount).toMatchObject({
        value: 176,
        direction: "out",
      });
...
describe("DianpingAdapter cookie-api mode", () => {
  ...
  it("stops at sinceWatermark", async () => {
    ...
    const items = await collect(
      a.sync({ sinceWatermark: 1_500_000_000_000, pageSize: 2 }),
    );
  ...
  it("respects opts.limit", async () => {
    ...
    const items = await collect(
      a.sync({ sinceWatermark: 0, limit: 1, pageSize: 2 }),
    );
    expect(items).toHaveLength(1);
  });
