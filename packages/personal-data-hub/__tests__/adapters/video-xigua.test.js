...
const {
  ...
  NAME,
  VERSION,
} = require("../../lib/adapters/video-xigua");
...
describe("video-xigua mappers", () => {
  ...
  it("extractItems tolerant", () => {
    expect(extractItems({ data: { history: [{ group_id: 1 }] } })).toHaveLength(
      1,
    );
    expect(
      extractItems({ data: { favorites: [{ group_id: 1 }] } }),
    ).toHaveLength(1);
    expect(() => extractItems({})).toThrow(/recognized list/u);
  });
});
  ...
  it("cookie-api fetch + normalize", async () => {
    ...
    const data = {
      watch: [
        {
          behot_time: 1716300000,
          article: { group_id: "C1", title: "汽车评测", video_duration: 500 },
        },
      ],
