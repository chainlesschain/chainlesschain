...
const CURSOR_PREFIX = "alipay-bill:v1:";
...
function cursorError(code, message) {
  const error = new Error(`alipay-bill scan cursor: ${message}`);
  error.code = code;
...
function invalid(message) {
  throw cursorError("ALIPAY_BILL_CURSOR_INVALID", message);
}
...
function canonicalizeCursor(value) {
  if (!hasExactKeys(value, ["v", "source", "after", "upper"])) {
    invalid("cursor must contain exactly v, source, after, and upper");
  }
  if (value.v !== CURSOR_VERSION) {
    throw cursorError(
      "ALIPAY_BILL_CURSOR_UNSUPPORTED",
      `cursor version ${String(value.v)} is unsupported`,
    );
  }
  if (value.upper === null) {
    if (value.source !== null || value.after !== null) {
      invalid("a reset cursor must clear source, after, and upper");
    }
    return initCursor();
  }
...
function beginScan(value, { source, upper }) {
...
function assertScanIdentity(value, { source, upper }) {
  const cursor = checkedCursor(value);
  if (cursor.upper === null) return cursor;
  if (cursor.source !== source || cursor.upper !== upper) {
    throw cursorError(
      "ALIPAY_BILL_CURSOR_SOURCE_CHANGED",
      "the selected bill export differs from the active scan source",
    );
  }
  return cursor;
}
...
function parseCursor(value) {
  ...
  if (!value.startsWith(CURSOR_PREFIX)) {
    const versionMatch = /^alipay-bill:v([^:]*):/u.exec(value);
    if (versionMatch) {
      throw cursorError(
        "ALIPAY_BILL_CURSOR_UNSUPPORTED",
    ...
    invalid("stored cursor must use the alipay-bill:v1 prefix");
  }
  let parsed;
  try {
