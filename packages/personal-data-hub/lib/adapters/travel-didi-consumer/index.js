...
class DidiConsumerAdapter {
  constructor(opts = {}) {
    ...
    this._liveConfigured = Boolean(
      this._ordersUrl && typeof opts.fetchFn === "function",
    );
  ...
  async authenticate(ctx = {}) {
    ...
    if (this._cookieAuth && !this._liveConfigured) {
      return {
        ...
        message:
          "travel-didi-consumer: live collection requires a captured ordersUrl and fetchFn; JSON import is ready",
      };
    }
    if (this._cookieAuth) {
  ...
  async *sync(opts = {}) {
    ...
    if (this._cookieAuth) {
      throw new Error(
        "travel-didi-consumer.sync: explicit ordersUrl and fetchFn required for custom live collection",
      );
    }
    throw new Error(
      "travel-didi-consumer.sync: inputPath or dataPath is required",
    );
  }
