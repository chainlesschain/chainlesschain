...
const CURSOR_PREFIX = "qqandroid:v1:";
const CURSOR_VERSION = 1;
const MAX_CURSOR_BYTES = 4096;
const POSITION_KINDS = Object.freeze(["contact", "group", "message"]);
...
function cursorError(code, message) {
  const error = new Error(`messaging-qq scan cursor: ${message}`);
  error.code = code;
...
function invalid(message) {
  throw cursorError("QQ_ANDROID_CURSOR_INVALID", message);
}
...
function normalizePosition(value, label = "position") {
  if (!isPlainObject(value) || !POSITION_KINDS.includes(value.kind)) {
    invalid(`${label}.kind must be contact, group, or message`);
  }
  if (value.kind === "message") {
    ...
    return {
      kind: value.kind,
      table: normalizeText(value.table, `${label}.table`, 512),
...
function parseCursor(value) {
  ...
  if (!value.startsWith(CURSOR_PREFIX)) {
    const versionMatch = /^qqandroid:v([^:]*):/u.exec(value);
    if (versionMatch) {
      throw cursorError(
        "QQ_ANDROID_CURSOR_UNSUPPORTED",
    ...
    invalid("stored cursor must use the qqandroid:v1 prefix");
  }
  let parsed;
  try {
