...
| approval verdict | `{"type":"approval","id":str,"decision":ApprovalDecision,"approve":bool,"binding":str}`                                                       | answers an `approval_request`; `decision` is canonical, `binding` MUST echo the request for approval, and `approve` is an N-1 compatibility bit that must agree  |
| legacy approval  | `{"type":"approval","id":str,"approve":bool}`                                                                                                 | N-1 compatibility for direct boolean responders; grants only the current call and cannot create a reusable permission                                            |
...
| session command  | `{"type":"slash_command","request_id":str,"command":str,"args":str}`                                                                          | read-only live-session query; one correlated `slash_command_result`, no model turn                                                                               |

### 1.2 CLI → client (stdout events)

| `type`                                                          | Key fields                                                                                                                                                                                                                                                                                                                                                                    |
| --------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `system` (`subtype:"init"`)                                     | `session_id` (resume id — persist this), `model`, `provider`, `permission_mode`, `tools[]`, `resumed_messages`, `slash_commands[]`                                                                                                                                                                                                                                            |
...
| `approval_request`                                              | `id`, `session_id`, `tool`, `command`, `risk`, `rule`, `reason`, `binding` — tool is BLOCKED until answered; structured approval MUST echo `binding`; CLI fails closed after `CC_APPROVAL_TIMEOUT_MS` (default 120 s)                                                                                                                                                         |
| `approval_resolved`                                             | `id`, `approved`, `decision`, `via` — canonical decision and final fail-closed outcome used to settle UI cards                                                                                                                                                                                                                                                                |
| `question_request` / `question_resolved`                        | `id`, `question`, `options?`, `multiSelect?`, `mode:"blocking"\|"deferred"?`, `blocking?`, `purpose?`, `context_revision?`, `binding?:{backgroundAgentId,sessionId,turnId,toolUseId,sequence}` (needs env `CC_INTERACTIVE_QUESTIONS=1`); deferred is restricted to preference/information and never grants authority; a bound answer missing or changing any field is ignored |
...
| `compaction-degraded` / `compaction_usage_unknown`              | explicit degraded or unknown-accounting outcome; consumers must not infer successful semantic compaction                                                                                                                                                                                                                                                                      |
| `stream_retry`                                                  | provider retry notice                                                                                                                                                                                                                                                                                                                                                         |
| `iteration_warning` / `iteration_budget_exhausted`              | `message` / `budget`                                                                                                                                                                                                                                                                                                                                                          |
| `cost_budget_exhausted`                                         | session cost ceiling and accounted spend at the stopping boundary                                                                                                                                                                                                                                                                                                             |
| `checkpoint`                                                    | workspace checkpoint identity and related tool metadata                                                                                                                                                                                                                                                                                                                       |
| `hook_started` / `hook_progress` / `hook_response`              | redacted Hook v2 lifecycle projection when explicitly enabled                                                                                                                                                                                                                                                                                                                 |
| `subagent_started` / `subagent_progress` / `subagent_completed` | bounded child-agent lifecycle projection when Hook events are enabled                                                                                                                                                                                                                                                                                                         |
| `remote_control`                                                | remote-approval pairing or unavailable status                                                                                                                                                                                                                                                                                                                                 |
| `recovery_degraded`                                             | a durable recovery authority could not be fully restored; the companion terminal result remains authoritative                                                                                                                                                                                                                                                                 |
| `question_response_rejected`                                    | an answer failed its exact interaction binding                                                                                                                                                                                                                                                                                                                                |
| `structured_result`                                             | per-turn JSON Schema validation verdict and structured value/errors                                                                                                                                                                                                                                                                                                           |
| `session_error`                                                 | SDK-side process/session transport failure projected as an event                                                                                                                                                                                                                                                                                                              |
...
| `slash_command_result`                                          | `request_id`, `command`, `ok`; success has `text`, failure has `error:{code,message}`; does not replace or terminate a turn                                                                                                                                                                                                                                                   |
...
- **Approval callback**: supplying an approval handler implies
  `--interactive-approvals`; each `approval_request` is answered with a
  canonical `ApprovalDecision` plus the unchanged request `binding`. Boolean
  callback results remain source-compatible and normalize to `acceptOnce` or
  `decline`; invalid/throwing handlers produce a structured decline (fail
  closed, matching the CLI's own timeout behavior).
- **Question callback**: `AgentSession` automatically echoes the opaque
  ...
  resolver. `blocking` remains the default. A `deferred` question returns a
  pending receipt immediately, may only collect preference/information, and its
  later answer is user context rather than an approval or permission grant.
