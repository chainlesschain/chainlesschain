...
const { verifyTaskCheckpoint } = require("./task-checkpoint.js");
...
function validateNode(value, node, path, errors, root) {
  ...
  if (node.not) {
    ...
    if (branchErrors.length === 0)
      errors.push({ path, message: "must not match forbidden schema" });
  }
...
function validateRuntimeInvariants(name, value, errors) {
  const normalizer =
    name === "ContextItem"
      ...
      : name === "MemoryRecord"
        ? normalizeMemoryRecord
        : name === "TaskCheckpoint"
          ? (input) => verifyTaskCheckpoint(input, input.sessionId)
          : null;
  ...
  try {
    ...
    errors.push({
      path: "#",
      message: `runtime invariant failed: ${error?.code || error?.message || "invalid"}`,
    });
  }
}
...
function validateContextMemoryDefinition(name, value) {
  ...
  if (!definition) {
    return validationResult([
      {
        path: "#",
        message: `unknown context/memory definition ${String(name)}`,
      },
    ]);
  }
  const errors = [];
