...
describe('Router registration', () => {
  it('has exactly 47 child routes under root', async () => {
    const routerModule = await import('../../src/router/index.js')
    const router = routerModule.default
    const rootRoute = router.options.routes[0]
    // 1 redirect + 46 named pages = 47 children
    expect(rootRoute.children.length).toBe(47)
  })
  ...
  it('contains all expected route names', async () => {
    ...
    const expected = [
      ...
      'NLProgramming', 'Tenant', 'Pipeline', 'Governance', 'Audit', 'Reputation', 'Recommend', 'Sla', 'Codegen', 'Search', 'VideoEditing',
    ]
