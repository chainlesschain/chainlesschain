...
describe('Router registration', () => {
  it('has exactly 65 child routes under root', async () => {
    ...
    // 1 redirect + 64 named pages = 65 children
    //  FamilyGuardDashboard added 2026-06-01 for FAMILY-26 家长端家庭守护仪表板 (cross-shell port);
  ...
  it('contains all expected route names', async () => {
    ...
    const expected = [
      ...
      'Providers', 'McpTools', 'ProjectSettings', 'SpeechSettings',
      'Notes', 'Memory', 'Knowledge', 'Cron',
  ...
  it('maps FamilyGuardDashboard to /family-guard path', async () => {
    ...
    const router = routerModule.default
    const rootRoute = router.options.routes[0]
    const fgRoute = rootRoute.children.find(r => r.name === 'FamilyGuardDashboard')
    expect(fgRoute).toBeDefined()
    expect(fgRoute.path).toBe('family-guard')
  })
  ...
  it('maps SpeechSettings to /speech-settings path', async () => {
    ...
    const router = routerModule.default
    const rootRoute = router.options.routes[0]
    const ssRoute = rootRoute.children.find(r => r.name === 'SpeechSettings')
    expect(ssRoute).toBeDefined()
    expect(ssRoute.path).toBe('speech-settings')
  })
  ...
  it('maps Pipeline to /pipeline path', async () => {
    const routerModule = await import('../../src/router/index.js')
    const router = routerModule.default
    const rootRoute = router.options.routes[0]
    const pRoute = rootRoute.children.find(r => r.name === 'Pipeline')
    expect(pRoute).toBeDefined()
    expect(pRoute.path).toBe('pipeline')
  })

  it('maps Governance to /governance path', async () => {
    const routerModule = await import('../../src/router/index.js')
    const router = routerModule.default
    const rootRoute = router.options.routes[0]
    const gRoute = rootRoute.children.find(r => r.name === 'Governance')
    expect(gRoute).toBeDefined()
    expect(gRoute.path).toBe('governance')
  })

  it('maps Audit to /audit path', async () => {
    const routerModule = await import('../../src/router/index.js')
    const router = routerModule.default
    const rootRoute = router.options.routes[0]
    const aRoute = rootRoute.children.find(r => r.name === 'Audit')
    expect(aRoute).toBeDefined()
    expect(aRoute.path).toBe('audit')
  })

  it('maps Reputation to /reputation path', async () => {
    const routerModule = await import('../../src/router/index.js')
    const router = routerModule.default
    const rootRoute = router.options.routes[0]
    const rRoute = rootRoute.children.find(r => r.name === 'Reputation')
    expect(rRoute).toBeDefined()
    expect(rRoute.path).toBe('reputation')
  })

  it('maps Recommend to /recommend path', async () => {
    const routerModule = await import('../../src/router/index.js')
    const router = routerModule.default
    const rootRoute = router.options.routes[0]
    const rRoute = rootRoute.children.find(r => r.name === 'Recommend')
    expect(rRoute).toBeDefined()
    expect(rRoute.path).toBe('recommend')
  })

  it('maps Sla to /sla path', async () => {
    const routerModule = await import('../../src/router/index.js')
    const router = routerModule.default
    const rootRoute = router.options.routes[0]
    const sRoute = rootRoute.children.find(r => r.name === 'Sla')
    expect(sRoute).toBeDefined()
    expect(sRoute.path).toBe('sla')
  })

  it('maps Codegen to /codegen path', async () => {
    const routerModule = await import('../../src/router/index.js')
    const router = routerModule.default
    const rootRoute = router.options.routes[0]
    const cRoute = rootRoute.children.find(r => r.name === 'Codegen')
    expect(cRoute).toBeDefined()
    expect(cRoute.path).toBe('codegen')
  })

  it('maps Search to /search path', async () => {
    const routerModule = await import('../../src/router/index.js')
    const router = routerModule.default
    const rootRoute = router.options.routes[0]
    const sRoute = rootRoute.children.find(r => r.name === 'Search')
    expect(sRoute).toBeDefined()
    expect(sRoute.path).toBe('search')
  })

  it('maps Tokens to /tokens path', async () => {
    const routerModule = await import('../../src/router/index.js')
    const router = routerModule.default
    const rootRoute = router.options.routes[0]
    const tRoute = rootRoute.children.find(r => r.name === 'Tokens')
    expect(tRoute).toBeDefined()
    expect(tRoute.path).toBe('tokens')
  })

  it('maps Trust to /trust path', async () => {
    const routerModule = await import('../../src/router/index.js')
    const router = routerModule.default
    const rootRoute = router.options.routes[0]
