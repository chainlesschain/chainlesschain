...
class MockWebSocket {
  ...
  send(data) {
    this._sent = (this._sent || []).concat(JSON.parse(data));
  }
  close() {
...
vi.stubGlobal("window", {
  __CC_CONFIG__: { wsHost: "127.0.0.1", wsPort: 9999 },
});
...
describe("ws store — message routing", () => {
  ...
  it("rejects an in-flight execute when the connection never comes back", async () => {
    const store = useWsStore();
    store.connect();
    MockWebSocket._last._open();
    await Promise.resolve();
    ...
    // execute() retries ONCE with the same commandId after a drop; when the
    // reconnect never completes, the retry's connection wait times out and
    // the failure propagates instead of hanging.
    const assertion = expect(p).rejects.toThrow(/WS not ready/);
  ...
  it("maps worktree protocol messages into runtime events with normalized record payloads", async () => {
    ...
    MockWebSocket._last._message({
      ...
      record: {
        ...
        summary: { changedFiles: 3 },
        previewEntrypoints: [
          { type: "worktree-diff", branch: "agent/task-42" },
        ],
      },
    });
    MockWebSocket._last._message({
  ...
  it("resolves pending request using requestId from v1.0 envelope", async () => {
    ...
    const p = store.sendRaw(
      { type: "session-create", sessionType: "chat" },
      5000,
    );
    await flushMicrotasks();
    const req = MockWebSocket._last._sent.at(-1);
...
describe("ws store — execute() idempotency keys + reconnect retry", () => {
  beforeEach(() => {
    setActivePinia(createPinia());
    vi.clearAllTimers();
    MockWebSocket._last = null;
  ...
  it("execute() carries commandId + monotonic seq (and deviceId when storage exists)", async () => {
    ...
    try {
      ...
      const p1 = store.execute("status", 5000);
      await Promise.resolve();
      const req1 = MockWebSocket._last._sent.at(-1);
      expect(typeof req1.commandId).toBe("string");
      expect(req1.commandId.length).toBeGreaterThan(8);
      expect(typeof req1.seq).toBe("number");
      expect(req1.deviceId).toMatch(/^web-panel-/);
      MockWebSocket._last._message({ id: req1.id, stdout: "ok", exitCode: 0 });
      await p1;

      const p2 = store.execute("status", 5000);
      await Promise.resolve();
      const req2 = MockWebSocket._last._sent.at(-1);
      // Distinct command → distinct id; seq strictly increases; deviceId stable.
      expect(req2.commandId).not.toBe(req1.commandId);
      expect(req2.seq).toBeGreaterThan(req1.seq);
      expect(req2.deviceId).toBe(req1.deviceId);
      MockWebSocket._last._message({ id: req2.id, stdout: "ok", exitCode: 0 });
      await p2;
    } finally {
      vi.unstubAllGlobals();
      vi.stubGlobal("WebSocket", MockWebSocket);
  ...
  it("re-sends the SAME commandId once after a mid-request connection drop", async () => {
    const store = useWsStore();
    await connectAndReady(store);

    const first = MockWebSocket._last;
    const p = store.execute("long job", 30000);
    await Promise.resolve();
    const req1 = first._sent.at(-1);
    expect(req1.commandId).toBeTruthy();

    // Connection drops before the response arrives → pending rejects, execute()
    // waits for the reconnect and re-sends the SAME logical command.
    first.close();
    await Promise.resolve();
    await vi.advanceTimersByTimeAsync(1000); // reconnect timer fires connect()
    const second = MockWebSocket._last;
    expect(second).not.toBe(first);
    second._open();
    await vi.advanceTimersByTimeAsync(200); // waitConnected() poll tick
    await Promise.resolve();

    const req2 = second._sent.at(-1);
    expect(req2.type).toBe("execute");
    expect(req2.commandId).toBe(req1.commandId); // SAME idempotency key
    expect(req2.id).not.toBe(req1.id); // fresh transport correlation id

    // The server had already executed the first delivery → replies `replayed`.
    second._message({
      id: req2.id,
      type: "replayed",
      commandId: req2.commandId,
      applyIndex: 0,
    });
    const res = await p;
  ...
  it("does not retry more than once (second drop surfaces the error)", async () => {
    const store = useWsStore();
    await connectAndReady(store);

    const p = store.execute("job", 30000);
    await Promise.resolve();
    MockWebSocket._last.close();
    await Promise.resolve();
    ...
    await Promise.resolve();
    // Second attempt also dies mid-flight → no third attempt, error propagates.
    second.close();
