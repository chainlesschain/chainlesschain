...
class FakeRelay {
  ...
  emit(type, event = {}) {
    ;(this.listeners[type] || []).forEach((cb) => cb({ ...event, target: this }))
  }
...
describe('remoteSession store', () => {
  beforeEach(() => {
    ...
    FakeRelay.host = new RemoteSessionCryptoContext({ sessionId: SESSION_ID, localPeerId: 'host' })
  ...
  it('pairs, connects, mirrors events and sends scoped controls', () => {
    ...
    expect(store.events.at(-1)).toMatchObject({ type: 'assistant.delta', content: 'building' })

    store.sendPrompt('continue')
    expect(active.controls.at(-1)).toEqual({ type: 'prompt', content: 'continue' })
    ...
    store.interrupt()
    expect(active.controls.at(-1)).toEqual({ type: 'interrupt' })
  })
