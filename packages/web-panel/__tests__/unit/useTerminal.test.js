...
describe("WS stdout fan-out", () => {
  it("decodes base64 stdout and delivers to the session subscriber", () => {
    ...
    emit({
      ...
      payload: { sessionId: "s1", data: b64, seq: 3 },
    });
  ...
  it("delivers terminal.exit to exit subscribers", () => {
    ...
    emit({
      type: "terminal.exit",
      payload: { sessionId: "s1", exitCode: 0, signal: null },
    });
...
describe("command methods", () => {
  it("create returns result and throws on ok:false", async () => {
    sendRaw.mockResolvedValueOnce({
      ok: true,
      result: { sessionId: "s9", pid: 1 },
    });
    expect(await useTerminal().create({ shell: "bash" })).toEqual({
      sessionId: "s9",
      pid: 1,
    });
  ...
  it("stdin base64-encodes the data", async () => {
    const term = useTerminal();
    await term.stdin("project-1", "s1", "echo 你好");
  ...
  it("history base64-decodes each chunk", async () => {
    ...
    const h = await term.history("project-1", "s1", 2);
  ...
  it("close/resize throw on ok:false", async () => {
    ...
    await expect(useTerminal().close("project-1", "s1")).rejects.toThrow(
      "boom",
    );
    await expect(
      useTerminal().resize("project-1", "s1", 80, 24),
