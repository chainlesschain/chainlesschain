...
// ── Idempotency identity (Phase 5 remote control) ─────────────────────────
// `commandId` makes a re-send of the same command safe: the server's
// RemoteCommandLedger executes at most once per id and answers a re-delivery
// with a `replayed` ack instead of running it again. `deviceId` + a monotonic
// `seq` add per-device ordering + revocation. The deviceId is per-TAB
// (sessionStorage): it survives reloads (so revocation sticks) but two tabs
// never share one seq space — a shared id would let same-millisecond sends
// from two tabs trip the server's stale-seq rejection.
const newCommandId = () =>
...
function panelDeviceId() {
  try {
    ...
    let id = store.getItem("cc-panel-device-id");
    if (!id) {
      id = `web-panel-${newCommandId()}`;
      store.setItem("cc-panel-device-id", id);
    }
    return id;
  } catch {
    return null; // storage unavailable (privacy mode) — commandId dedup still works
  }
}

// Monotonic even across a page reload (the server's per-device watermark
// outlives the tab): a plain counter restarting at 1 would make every command
// after a reload look stale. max(now, last+1) also stays strict within a burst.
let lastSeq = 0;
function nextSeq() {
        ...
        "worktree:merge:completed",
        {
          ...
          record: msg.record ||
            payload.record || {
              ...
              previewEntrypoints:
                msg.previewEntrypoints || payload.previewEntrypoints || [],
            },
        },
        { kind: "server" },
      );
...
export const useWsStore = defineStore("ws", () => {
      ...
      ws.onopen = () => {
        ...
        if (cfg.wsToken) {
          sendRaw({ type: "auth", id: genId(), token: cfg.wsToken })
            ...
            .catch(() => {
              status.value = "connected";
            }); // still set connected even if auth fails
        } else {
          status.value = "connected";
        }
      };
  ...
  function handleMessage(msg) {
    ...
    if (
      correlationId &&
      pendingStreams.has(correlationId) &&
      typeof type === "string"
    ) {
      ...
      if (type.endsWith(".chunk")) {
        try {
          ...
          /* consumer error must not break the stream wire */
        }
        stream.rearm();
        return true;
      }
    ...
    const sessionId =
      msg.sessionId || (msg.payload && msg.payload.sessionId) || null;
    ...
    return new Promise((resolve, reject) => {
      ...
      const onAbort = () => {
        ...
        try {
          ...
          /* swallow — abort path must not throw */
        }
        const reason =
  ...
  //
  // Carries an idempotency key: on a connection drop mid-request we re-send the
  // SAME frame (same commandId) once after reconnect — the server's ledger
  // guarantees at-most-once execution, so the retry can't double-run the
  // command. A `replayed` ack means the first delivery DID execute but its
  // response was lost; we surface that instead of failing.
  async function execute(command, timeoutMs = 30000) {
    ...
    const deviceId = panelDeviceId();
    if (deviceId) frame.deviceId = deviceId;
    let result;
    for (let attempt = 0; ; attempt++) {
      try {
        result = await sendRaw({ ...frame, id: genId() }, timeoutMs);
        break;
      } catch (err) {
        const connectionLost =
          err?.message === "WebSocket closed" ||
          err?.message === "WebSocket not connected";
        if (!connectionLost || attempt >= 1) throw err;
        await waitConnected(8000); // reconnect, then re-send the SAME commandId
      }
    }
    if (result?.type === "replayed") {
      // The command already executed server-side; the original response was
      // lost with the dropped connection. Not an error — but the output is gone.
      return {
        output: "(command already executed — output lost on reconnect)",
        exitCode: 0,
        replayed: true,
      };
    }
  ...
  async function createSession(
    ...
    if (
      Array.isArray(options.shellPolicyOverrides) &&
      options.shellPolicyOverrides.length
    ) {
