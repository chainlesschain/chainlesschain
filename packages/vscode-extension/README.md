...
> Extension release `0.37.100` pairs with `chainlesschain@0.166.50`. It reports
> the exact active request context window, distinguishes a provider-reported
> measurement from a transcript estimate, and retains cache totals after turns.

Release `0.37.100` recommends CLI `0.166.50`. These runtime capabilities remain
owned by the CLI host; this release does not add new editor controls for every
CLI option.

| Component                 | Current status                                               |
| ------------------------- | ------------------------------------------------------------ |
| VS Code extension         | **0.37.100**; CLI 0.166.50 pairing release                   |
| Recommended CLI           | **`chainlesschain@0.166.50`**                                |
...
### What's new in 0.37.100

- **Show accurate context usage.** The panel uses the active provider request's
  capacity and token totals, excluding sub-agent and compaction calls.
- **Make estimates explicit.** A session-transcript fallback is labeled as an
  estimate until the provider reports request usage.
- **Keep cache accounting visible.** Completed turns retain cache-read and
  cache-write totals alongside input and output tokens.

### Retained capabilities from earlier releases

- **Pair with the verified CLI 0.166.50 release.** Consume governed
...
- **Keep governance boundaries visible.** The extension delegates signature and
  digest verification to CLI `0.166.50`, stores no private key, and displays that
...
- CLI `0.166.50` preserves protected context while compacting an oversized turn
...
- Older CLI installations receive the normal upgrade prompt for `0.166.50`.
...
- On startup, an installed CLI older than `0.166.50` receives an explicit
...
> [0.37.100 VSIX directly (available only after extension publication)](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.100/file/chainlesschain.chainlesschain-ide-0.37.100.vsix)
> and run **Extensions: Install from VSIX...** instead. VSCodium and other Open
> VSX editors can continue to install by extension ID.

Paired CLI `0.166.50` is the recommended install. It routes Graph, Team,
...
and shared permission/budget enforcement. Version `0.37.100` accepts only the
...
Version `0.37.100` also consumes only strict, CLI-issued multi-agent merge-review
...
`0.37.100`. The extension continues to fail closed when an older CLI cannot
...
[`ide-vscode-v0.37.100`](https://github.com/chainlesschain/chainlesschain/releases/tag/ide-vscode-v0.37.100).
...
```bash
npm i -g chainlesschain@0.166.50
cc --version
```

Using `@0.166.50` reproduces the preferred, fully gated public CLI pairing,
...
- In stock Microsoft VS Code, do not click Open VSX's generic **Install** link.
  Download the [versioned `.vsix`](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.100/file/chainlesschain.chainlesschain-ide-0.37.100.vsix)
...
node scripts/verify-vsix.mjs chainlesschain-ide-0.37.100.vsix
```
