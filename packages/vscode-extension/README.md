...
> Extension candidate `0.37.92` pairs with `chainlesschain@0.166.37` for verified model saves
...
Source candidate `0.37.92` verifies saved model settings by fresh CLI readback
...
| ------------------------- | ------------------------------------------------------------ |
| VS Code extension         | **0.37.92**; model-save confirmation candidate               |
| Recommended CLI           | **`chainlesschain@0.166.37`**                                |
...
- CLI `0.166.37` gives reasoning-model connection probes enough output budget
...
- Older CLI installations receive the normal upgrade prompt for `0.166.37`.
...
- On startup, an installed CLI older than `0.166.37` receives an explicit
...
> [0.37.92 VSIX directly (available only after extension publication)](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.92/file/chainlesschain.chainlesschain-ide-0.37.92.vsix)
> and run **Extensions: Install from VSIX...** instead. VSCodium and other Open
> VSX editors can continue to install by extension ID.

Paired CLI `0.166.37` is the recommended install. It routes Graph, Team,
...
and shared permission/budget enforcement. Version `0.37.92` accepts only the
...
Version `0.37.92` also consumes only strict, CLI-issued multi-agent merge-review
...
`0.37.92`. The extension continues to fail closed when an older CLI cannot
...
[`ide-vscode-v0.37.92`](https://github.com/chainlesschain/chainlesschain/releases/tag/ide-vscode-v0.37.92).
...
```bash
npm i -g chainlesschain@0.166.37
cc --version
```

Using `@0.166.37` reproduces the preferred, fully gated public CLI pairing,
...
- In stock Microsoft VS Code, do not click Open VSX's generic **Install** link.
  Download the [versioned `.vsix`](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.92/file/chainlesschain.chainlesschain-ide-0.37.92.vsix)
...
node scripts/verify-vsix.mjs chainlesschain-ide-0.37.92.vsix
```
