...
| ------------------------- | ------------------------------------------------------------ |
| VS Code extension         | **0.37.65**; immutable tag-gated Open VSX release            |
| Recommended CLI           | **`chainlesschain@0.165.10`** public npm release             |
...
The recommended CLI `0.165.10` includes the governed Automation/Routine commands,
the released Automation Center v3 projection, scoped permission and side-effect
authority, and shared permission/budget enforcement. Version `0.37.65` accepts
...
Version `0.37.65` also consumes only strict, CLI-issued multi-agent merge-review
...
history-rewriting rollback commands. CLI `0.165.10` supplies the corresponding
governed `team merge-review` command and exact evidence contract.

The paired CLI `0.165.10` contains the audited Artifact access, managed-copy
deletion settlement, orphan recovery, and durable workflow authorities used by
`0.37.65`. The extension continues to fail closed when an older CLI cannot
provide the exact projection or refreshed action evidence.

CLI `0.165.10` also bounds durable-session event backlogs and sidecars, routes
...
require an available Docker sandbox. `0.165.10` restores Docker-optional normal
...
and Strict Sandbox matrices, npm publication, and public-registry readback. For
that reason, `0.165.10` is the preferred CLI for this extension.

The immutable publication tag for this package is
[`ide-vscode-v0.37.65`](https://github.com/chainlesschain/chainlesschain/releases/tag/ide-vscode-v0.37.65).
...
- **Permission and Side-effect Center** - review CLI-authoritative filesystem,
  ...
  Public CLI `0.165.10` provides the exact `permissions activity`, `scoped`, and
...
```bash
npm i -g chainlesschain@0.165.10
cc --version
```

Using `@0.165.10` reproduces the preferred, fully gated public CLI pairing,
...
node scripts/verify-vsix.mjs chainlesschain-ide-0.37.65.vsix
```
