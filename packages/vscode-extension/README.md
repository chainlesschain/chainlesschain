...
> Extension release `0.37.113` pairs with `chainlesschain@0.166.71`. It adds
> CLI-owned local Skill decision provider selection without moving provider
> credentials, routing authority, or Skill execution into the IDE.

Release `0.37.113` recommends CLI `0.166.71`. These runtime capabilities remain
owned by the CLI host; this release does not add new editor controls for every
CLI option.

| Component                 | Current status                                            |
| ------------------------- | --------------------------------------------------------- |
| VS Code extension         | **0.37.113**; CLI 0.166.71 pairing release                |
| Recommended CLI           | **`chainlesschain@0.166.71`**                             |
...
### What's new in 0.37.113

- **Pair with the verified CLI 0.166.71 release.** Recommend the publicly
  verified `chainlesschain@0.166.71` release in runtime upgrade guidance,
...
- **Keep local decision providers in the CLI.** Loopback Laya and compatible
  ...
  default stays `off` and invalid endpoints fail closed.
...
- Older CLI installations receive the normal upgrade prompt for `0.166.71`.
...
- On startup, an installed CLI older than `0.166.71` receives an explicit
...
> the [0.37.113 VSIX directly](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.113/file/chainlesschain.chainlesschain-ide-0.37.113.vsix)
> and run **Extensions: Install from VSIX...** instead. VSCodium and other Open
> VSX editors can install by extension ID.

Paired CLI `0.166.71` is the recommended install. It routes Graph, Team,
...
and shared permission/budget enforcement. Version `0.37.113` accepts only the
...
Version `0.37.113` also consumes only strict, CLI-issued multi-agent merge-review
...
`0.37.113`. The extension continues to fail closed when an older CLI cannot
...
retained compatibility floor. The publicly verified `0.166.71` release is the
...
[`ide-vscode-v0.37.113`](https://github.com/chainlesschain/chainlesschain/releases/tag/ide-vscode-v0.37.113).
...
```bash
npm i -g chainlesschain@0.166.71
cc --version
```

Using `@0.166.71` reproduces the preferred, fully gated public CLI pairing,
...
- In stock Microsoft VS Code, download the [versioned `.vsix`](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.113/file/chainlesschain.chainlesschain-ide-0.37.113.vsix)
...
node scripts/verify-vsix.mjs chainlesschain-ide-0.37.113.vsix
```
