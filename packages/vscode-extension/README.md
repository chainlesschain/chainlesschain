...
> Extension release `0.37.111` pairs with `chainlesschain@0.166.69`. It carries
> durable Volcengine function authority, replay and revocation checks, and
> supervised execution without moving authority into the IDE.

Release `0.37.111` recommends CLI `0.166.69`. These runtime capabilities remain
owned by the CLI host; this release does not add new editor controls for every
CLI option.

| Component                 | Current status                                            |
| ------------------------- | --------------------------------------------------------- |
| VS Code extension         | **0.37.111**; CLI 0.166.69 pairing release                |
| Recommended CLI           | **`chainlesschain@0.166.69`**                             |
...
### What's new in 0.37.111

- **Pair with the verified CLI 0.166.69 release.** Recommend the publicly
  verified `chainlesschain@0.166.69` release in runtime upgrade guidance,
  marketplace metadata, and installation instructions.
- **Carry durable Volcengine function authority.** Requests bind exact policy,
  bounded arguments and deadlines to cross-process replay reservations;
  revocation evidence, issuer chains and signer-status floors are verified
  before execution.
- **Retain supervised execution boundaries.** Approved functions run in child
  processes with cooperative cancellation, hard termination and fixed receipts.
...
- Older CLI installations receive the normal upgrade prompt for `0.166.69`.
...
- On startup, an installed CLI older than `0.166.69` receives an explicit
...
> the [0.37.111 VSIX directly](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.111/file/chainlesschain.chainlesschain-ide-0.37.111.vsix)
> and run **Extensions: Install from VSIX...** instead. VSCodium and other Open
> VSX editors can install by extension ID.

Paired CLI `0.166.69` is the recommended install. It routes Graph, Team,
...
and shared permission/budget enforcement. Version `0.37.111` accepts only the
...
Version `0.37.111` also consumes only strict, CLI-issued multi-agent merge-review
...
`0.37.111`. The extension continues to fail closed when an older CLI cannot
...
retained compatibility floor. The publicly verified `0.166.69` release is the
...
[`ide-vscode-v0.37.111`](https://github.com/chainlesschain/chainlesschain/releases/tag/ide-vscode-v0.37.111).
...
```bash
npm i -g chainlesschain@0.166.69
cc --version
```

Using `@0.166.69` reproduces the preferred, fully gated public CLI pairing,
...
- In stock Microsoft VS Code, download the [versioned `.vsix`](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.111/file/chainlesschain.chainlesschain-ide-0.37.111.vsix)
...
node scripts/verify-vsix.mjs chainlesschain-ide-0.37.111.vsix
```
