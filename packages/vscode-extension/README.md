...
| ------------------------- | ------------------------------------------------------------ |
| VS Code extension         | **0.37.77**; immutable tag-gated Open VSX release            |
| Recommended CLI           | **`chainlesschain@0.166.15`** public npm release             |
...
> [0.37.77 VSIX directly](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.77/file/chainlesschain.chainlesschain-ide-0.37.77.vsix)
> and run **Extensions: Install from VSIX...** instead. VSCodium and other Open
> VSX editors can continue to install by extension ID.

Public CLI `0.166.15` is the recommended install. It routes Graph, Team,
...
CLI `0.166.15` includes canonical scoped approval decisions, a
...
CLI `0.166.15` includes the governed Automation/Routine commands, the
Automation Center v3 projection, scoped permission and side-effect authority,
and shared permission/budget enforcement. Version `0.37.77` accepts only the
...
Version `0.37.77` also consumes only strict, CLI-issued multi-agent merge-review
...
history-rewriting rollback commands. CLI `0.166.15` supplies the
corresponding governed `team merge-review` command and exact evidence contract.

CLI `0.166.15` contains the audited Artifact access, managed-copy
deletion settlement, orphan recovery, and durable workflow authorities used by
`0.37.77`. The extension continues to fail closed when an older CLI cannot
provide the exact projection or refreshed action evidence.

CLI `0.166.15` also bounds durable-session event backlogs and sidecars, routes
...
extensionless POSIX shim before `docker.exe`. Public CLI `0.166.15` restores
...
credentials never enter the Webview. For those reasons, `0.166.15` is the
...
[`ide-vscode-v0.37.77`](https://github.com/chainlesschain/chainlesschain/releases/tag/ide-vscode-v0.37.77).
...
- **Permission and Side-effect Center** - review CLI-authoritative filesystem,
  ...
  CLI `0.166.15` provides the exact `permissions activity`, `scoped`,
...
Using `@0.166.15` reproduces the preferred, fully gated public CLI pairing,
...
- In stock Microsoft VS Code, do not click Open VSX's generic **Install** link.
  Download the [versioned `.vsix`](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.77/file/chainlesschain.chainlesschain-ide-0.37.77.vsix)
...
node scripts/verify-vsix.mjs chainlesschain-ide-0.37.77.vsix
```
