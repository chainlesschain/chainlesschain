...
> Extension release `0.37.101` pairs with `chainlesschain@0.166.51`. It reports
...
Release `0.37.101` recommends CLI `0.166.51`. These runtime capabilities remain
owned by the CLI host; this release does not add new editor controls for every
CLI option.

| Component                 | Current status                                               |
| ------------------------- | ------------------------------------------------------------ |
| VS Code extension         | **0.37.101**; CLI 0.166.51 pairing release                   |
| Recommended CLI           | **`chainlesschain@0.166.51`**                                |
...
- **Pair with the verified CLI 0.166.51 release.** Consume governed
...
- **Keep governance boundaries visible.** The extension delegates signature and
  digest verification to CLI `0.166.51`, stores no private key, and displays that
...
- CLI `0.166.51` preserves protected context while compacting an oversized turn
...
- Older CLI installations receive the normal upgrade prompt for `0.166.51`.
...
- On startup, an installed CLI older than `0.166.51` receives an explicit
...
> [0.37.101 VSIX directly (available only after extension publication)](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.101/file/chainlesschain.chainlesschain-ide-0.37.101.vsix)
> and run **Extensions: Install from VSIX...** instead. VSCodium and other Open
> VSX editors can continue to install by extension ID.

Paired CLI `0.166.51` is the recommended install. It routes Graph, Team,
...
and shared permission/budget enforcement. Version `0.37.101` accepts only the
...
Version `0.37.101` also consumes only strict, CLI-issued multi-agent merge-review
...
`0.37.101`. The extension continues to fail closed when an older CLI cannot
...
[`ide-vscode-v0.37.101`](https://github.com/chainlesschain/chainlesschain/releases/tag/ide-vscode-v0.37.101).
...
```bash
npm i -g chainlesschain@0.166.51
cc --version
```

Using `@0.166.51` reproduces the preferred, fully gated public CLI pairing,
...
- In stock Microsoft VS Code, do not click Open VSX's generic **Install** link.
  Download the [versioned `.vsix`](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.101/file/chainlesschain.chainlesschain-ide-0.37.101.vsix)
...
node scripts/verify-vsix.mjs chainlesschain-ide-0.37.101.vsix
```
