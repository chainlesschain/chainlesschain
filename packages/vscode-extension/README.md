...
> Extension `0.37.85` is an **unreleased source candidate**. CLI
> `chainlesschain@0.166.25` is publicly available, but this CLI-only release did
> not publish an extension. Versioned VSIX links below are reserved for the
> future extension release; use the Open VSX listing for the currently
> published extension.

| Component                 | Current status                                               |
| ------------------------- | ------------------------------------------------------------ |
| VS Code extension         | **0.37.85**; unreleased source candidate                    |
| Recommended CLI           | **`chainlesschain@0.166.25`** public npm release             |
...
- File-read traces include requested line ranges and long-line columns. CLI
  `0.166.25` keeps forward read progress through compaction and stops a run
...
- **Interactive Agent turns now run to completion without a hidden 50-call
  stop.** CLI `0.166.25` keeps explicit turn, cost, environment, and session
...
- On startup, an installed CLI older than `0.166.25` receives an explicit
...
Public CLI `0.166.25` is the recommended install. It routes Graph, Team,
...
CLI `0.166.25` includes canonical scoped approval decisions, a
...
CLI `0.166.25` includes the governed Automation/Routine commands, the
...
history-rewriting rollback commands. CLI `0.166.25` supplies the
corresponding governed `team merge-review` command and exact evidence contract.

CLI `0.166.25` contains the audited Artifact access, managed-copy
...
CLI `0.166.25` also bounds durable-session event backlogs and sidecars, routes
...
extensionless POSIX shim before `docker.exe`. Public CLI `0.166.25` restores
...
credentials never enter the Webview. For those reasons, `0.166.25` is the
...
- **Permission and Side-effect Center** - review CLI-authoritative filesystem,
  ...
  CLI `0.166.25` provides the exact `permissions activity`, `scoped`,
...
```bash
npm i -g chainlesschain@0.166.25
cc --version
```

Using `@0.166.25` reproduces the preferred, fully gated public CLI pairing,
