...
> Extension candidate `0.37.93` pairs with `chainlesschain@0.166.43` and adds
> visual configuration for governed Skill evolution. Publication follows the
> complete CLI and IDE release gates; the Open VSX listing shows the currently
> available extension.

Source candidate `0.37.93` lets an administrator select a signed deployment
descriptor and Ed25519 trust root, verify them through the CLI, and persistently
enable or disable the deployment. It awaits the release gates; the versioned
download links below become available only after publication.

| Component                 | Current status                                               |
| ------------------------- | ------------------------------------------------------------ |
| VS Code extension         | **0.37.93**; governed-evolution configuration candidate      |
...
> [0.37.93 VSIX directly (available only after extension publication)](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.93/file/chainlesschain.chainlesschain-ide-0.37.93.vsix)
...
and shared permission/budget enforcement. Version `0.37.93` accepts only the
...
Version `0.37.93` also consumes only strict, CLI-issued multi-agent merge-review
...
`0.37.93`. The extension continues to fail closed when an older CLI cannot
...
[`ide-vscode-v0.37.93`](https://github.com/chainlesschain/chainlesschain/releases/tag/ide-vscode-v0.37.93).
...
- In stock Microsoft VS Code, do not click Open VSX's generic **Install** link.
  Download the [versioned `.vsix`](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.93/file/chainlesschain.chainlesschain-ide-0.37.93.vsix)
...
node scripts/verify-vsix.mjs chainlesschain-ide-0.37.93.vsix
```
