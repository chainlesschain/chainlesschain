...
> Extension release `0.37.105` pairs with `chainlesschain@0.166.59`. It saves
> bounded task evidence during long conversations and can continue verified
> notes in an independent new chat after a completed tool boundary.

Release `0.37.105` recommends CLI `0.166.59`. These runtime capabilities remain
owned by the CLI host; this release does not add new editor controls for every
CLI option.

| Component                 | Current status                                               |
| ------------------------- | ------------------------------------------------------------ |
| VS Code extension         | **0.37.105**; CLI 0.166.59 pairing release                   |
| Recommended CLI           | **`chainlesschain@0.166.59`**                                |
...
### What's new in 0.37.105

- **Continue with durable task notes.** The new `↗` action saves a bounded,
...
- **Pair with safer evaluation and sessions.** CLI `0.166.59` adds normalized
...
- **Pair with the verified CLI 0.166.59 release.** Consume governed
...
- **Keep governance boundaries visible.** The extension delegates signature and
  digest verification and generated TEST-key custody to CLI `0.166.59`; the
...
- CLI `0.166.59` preserves protected context while compacting an oversized turn
...
- Older CLI installations receive the normal upgrade prompt for `0.166.59`.
...
- On startup, an installed CLI older than `0.166.59` receives an explicit
...
> [0.37.105 VSIX directly (available only after extension publication)](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.105/file/chainlesschain.chainlesschain-ide-0.37.105.vsix)
> and run **Extensions: Install from VSIX...** instead. VSCodium and other Open
> VSX editors can continue to install by extension ID.

Paired CLI `0.166.59` is the recommended install. It routes Graph, Team,
...
and shared permission/budget enforcement. Version `0.37.105` accepts only the
...
Version `0.37.105` also consumes only strict, CLI-issued multi-agent merge-review
...
`0.37.105`. The extension continues to fail closed when an older CLI cannot
...
[`ide-vscode-v0.37.105`](https://github.com/chainlesschain/chainlesschain/releases/tag/ide-vscode-v0.37.105).
...
```bash
npm i -g chainlesschain@0.166.59
cc --version
```

Using `@0.166.59` reproduces the preferred, fully gated public CLI pairing,
...
- In stock Microsoft VS Code, do not click Open VSX's generic **Install** link.
  Download the [versioned `.vsix`](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.105/file/chainlesschain.chainlesschain-ide-0.37.105.vsix)
...
node scripts/verify-vsix.mjs chainlesschain-ide-0.37.105.vsix
```
