...
| ------------------------- | ------------------------------------------------------------ |
| VS Code extension         | **0.37.71**; immutable tag-gated Open VSX release            |
| Recommended CLI           | **`chainlesschain@0.166.6`** public npm release              |
...
> [0.37.71 VSIX directly](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.71/file/chainlesschain.chainlesschain-ide-0.37.71.vsix)
> and run **Extensions: Install from VSIX...** instead. VSCodium and other Open
> VSX editors can continue to install by extension ID.

CLI `0.166.6` bounds Agent IPC child-process admission, pending interactions,
outbound requests, JSONL frames, stderr diagnostics, and stdin backpressure.
Overload now returns a structured retry hint, while initialization, heartbeat,
disconnect, and late-response paths have finite cleanup fences.

CLI `0.166.6` adds canonical scoped approval decisions, a schema-owned Agent
stream-event inventory, and durable Team
collaboration authority. The chat approval card keeps one-shot approval as the
fast path and opens a native, explicitly reviewed choice for exact turn/session
grants, denial, or cancellation. Team Monitor consumes the canonical
Message/Handoff projection as bounded status counters only: message payloads,
attempt/agent identities, artifacts, and authority digests never enter the
Webview.

The recommended CLI `0.166.6` includes the governed Automation/Routine commands,
the released Automation Center v3 projection, scoped permission and side-effect
authority, and shared permission/budget enforcement. Version `0.37.71` accepts
...
Version `0.37.71` also consumes only strict, CLI-issued multi-agent merge-review
...
history-rewriting rollback commands. CLI `0.166.6` supplies the corresponding
governed `team merge-review` command and exact evidence contract.

The paired CLI `0.166.6` contains the audited Artifact access, managed-copy
deletion settlement, orphan recovery, and durable workflow authorities used by
`0.37.71`. The extension continues to fail closed when an older CLI cannot
provide the exact projection or refreshed action evidence.

CLI `0.166.6` also bounds durable-session event backlogs and sidecars, routes
...
publishes `@chainlesschain/session-core@0.3.6` with the required
`./runtime-claims` entry point.

CLI `0.165.9` is superseded because normal Windows startup could incorrectly
require an available Docker sandbox. `0.166.6` preserves Docker-optional normal
startup while keeping explicit sandbox and managed-policy requests fail closed.
Its exact release commit passed the complete Linux, Windows, and macOS CLI CI
and Strict Sandbox matrices, npm publication, and public-registry readback. It
also adds lease-bound real-time teammate send/receive/ack/follow-up tools
...
credentials never enter the Webview. For those reasons, `0.166.6` is the
...
[`ide-vscode-v0.37.71`](https://github.com/chainlesschain/chainlesschain/releases/tag/ide-vscode-v0.37.71).
...
- **Permission and Side-effect Center** - review CLI-authoritative filesystem,
  ...
  Public CLI `0.166.6` provides the exact `permissions activity`, `scoped`, and
...
```bash
npm i -g chainlesschain@0.166.6
cc --version
```

Using `@0.166.6` reproduces the preferred, fully gated public CLI pairing,
...
- In stock Microsoft VS Code, do not click Open VSX's generic **Install** link.
  Download the [versioned `.vsix`](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.71/file/chainlesschain.chainlesschain-ide-0.37.71.vsix)
...
node scripts/verify-vsix.mjs chainlesschain-ide-0.37.71.vsix
```
...
- VS Code commands: **56** registered entries
