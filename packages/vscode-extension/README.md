...
> Extension release `0.37.112` pairs with `chainlesschain@0.166.70`. It carries
> the CLI's governed Jev Skill decision pilot without moving provider
> credentials, routing authority, or Skill execution into the IDE.

Release `0.37.112` recommends CLI `0.166.70`. These runtime capabilities remain
owned by the CLI host; this release does not add new editor controls for every
CLI option.

| Component                 | Current status                                            |
| ------------------------- | --------------------------------------------------------- |
| VS Code extension         | **0.37.112**; CLI 0.166.70 pairing release                |
| Recommended CLI           | **`chainlesschain@0.166.70`**                             |
...
### What's new in 0.37.112

- **Pair with the verified CLI 0.166.70 release.** Recommend the publicly
  verified `chainlesschain@0.166.70` release in runtime upgrade guidance,
  marketplace metadata, and installation instructions.
- **Retain the Jev pilot boundary.** Typed Skill decisions remain opt-in for
  durable, single-prompt headless CLI sessions; the default stays `off` and
  `shadow` never changes Agent-visible routing.
- **Do not widen the IDE surface.** Interactive IDE sessions do not expose
  `shadow` or `suggest`, and the extension stores no TypeSafe credential.
...
- Older CLI installations receive the normal upgrade prompt for `0.166.70`.
...
- On startup, an installed CLI older than `0.166.70` receives an explicit
...
> the [0.37.112 VSIX directly](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.112/file/chainlesschain.chainlesschain-ide-0.37.112.vsix)
> and run **Extensions: Install from VSIX...** instead. VSCodium and other Open
> VSX editors can install by extension ID.

Paired CLI `0.166.70` is the recommended install. It routes Graph, Team,
...
and shared permission/budget enforcement. Version `0.37.112` accepts only the
...
Version `0.37.112` also consumes only strict, CLI-issued multi-agent merge-review
...
`0.37.112`. The extension continues to fail closed when an older CLI cannot
...
retained compatibility floor. The publicly verified `0.166.70` release is the
...
[`ide-vscode-v0.37.112`](https://github.com/chainlesschain/chainlesschain/releases/tag/ide-vscode-v0.37.112).
...
```bash
npm i -g chainlesschain@0.166.70
cc --version
```

Using `@0.166.70` reproduces the preferred, fully gated public CLI pairing,
...
- In stock Microsoft VS Code, download the [versioned `.vsix`](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.112/file/chainlesschain.chainlesschain-ide-0.37.112.vsix)
...
node scripts/verify-vsix.mjs chainlesschain-ide-0.37.112.vsix
```
