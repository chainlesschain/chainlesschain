...
> Extension `0.37.87` pairs with `chainlesschain@0.166.30` for page-first
> workspaces and custom LLM connections. Publication follows the complete CLI and IDE release
> gates; the Open VSX listing shows the currently available extension.

| Component                 | Current status                                               |
| ------------------------- | ------------------------------------------------------------ |
| VS Code extension         | **0.37.87**; page-first workspace release             |
| Recommended CLI           | **`chainlesschain@0.166.30`**                               |
...
- **Configure custom LLM relays in one form.** Choose a compatible protocol,
  ...
  CLI `0.166.30` saves the connection atomically through stdin and tests the
...
- **Recover work that keeps investigating without delivering.** CLI `0.166.30`
...
- File-read traces include requested line ranges and long-line columns. CLI
  `0.166.30` keeps forward read progress through compaction and stops a run
...
- **Interactive Agent turns now run to completion without a hidden 50-call
  stop.** CLI `0.166.30` keeps explicit turn, cost, environment, and session
...
- On startup, an installed CLI older than `0.166.30` receives an explicit
...
> [0.37.87 VSIX directly (available only after extension publication)](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.87/file/chainlesschain.chainlesschain-ide-0.37.87.vsix)
> and run **Extensions: Install from VSIX...** instead. VSCodium and other Open
> VSX editors can continue to install by extension ID.

Public CLI `0.166.30` is the recommended install. It routes Graph, Team,
...
CLI `0.166.30` includes canonical scoped approval decisions, a
...
CLI `0.166.30` includes the governed Automation/Routine commands, the
Automation Center v3 projection, scoped permission and side-effect authority,
and shared permission/budget enforcement. Version `0.37.87` accepts only the
...
Version `0.37.87` also consumes only strict, CLI-issued multi-agent merge-review
...
history-rewriting rollback commands. CLI `0.166.30` supplies the
corresponding governed `team merge-review` command and exact evidence contract.

CLI `0.166.30` contains the audited Artifact access, managed-copy
deletion settlement, orphan recovery, and durable workflow authorities used by
`0.37.87`. The extension continues to fail closed when an older CLI cannot
provide the exact projection or refreshed action evidence.

CLI `0.166.30` also bounds durable-session event backlogs and sidecars, routes
...
extensionless POSIX shim before `docker.exe`. Public CLI `0.166.30` restores
...
credentials never enter the Webview. For those reasons, `0.166.30` is the
...
[`ide-vscode-v0.37.87`](https://github.com/chainlesschain/chainlesschain/releases/tag/ide-vscode-v0.37.87).
...
- **Permission and Side-effect Center** - review CLI-authoritative filesystem,
  ...
  CLI `0.166.30` provides the exact `permissions activity`, `scoped`,
...
Using `@0.166.30` reproduces the preferred, fully gated public CLI pairing,
...
- In stock Microsoft VS Code, do not click Open VSX's generic **Install** link.
  Download the [versioned `.vsix`](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.87/file/chainlesschain.chainlesschain-ide-0.37.87.vsix)
...
node scripts/verify-vsix.mjs chainlesschain-ide-0.37.87.vsix
```
