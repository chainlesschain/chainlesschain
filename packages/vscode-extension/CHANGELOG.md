...
- Carry the CLI's 256 KiB request/response limits for optional local Skill
  ...
  session resume. The extension does not invoke decision modes in IDE chat.
- Keep approval, sandbox, and editor bridge authority in the CLI host.

## [0.37.117] - Pair with CLI 0.166.76 state lock and evaluation fixes (2026-09-26)
...
- Acknowledge every accepted chat send immediately, including follow-ups queued
  in a persistent session. An acknowledgement timeout no longer diagnoses a
  missing CLI or marks an active request as finished.
- Retain the CLI's offline PM effect evidence and existing execution approval
  and sandbox boundaries.

## [0.37.115] - Pair with CLI 0.166.73 PM evidence workflow (2026-09-24)

- Recommend `chainlesschain@0.166.73` in runtime upgrade guidance and installation instructions.
...
- Keep Eval authority and provider credentials in the CLI host; the extension gains no new execution authority.
...
- Keep the extension a bounded CLI client; it receives fixed projections and
  does not gain function execution, revocation, or publication authority.

## [0.37.110] - Pair with governed CLI 0.166.68 (2026-09-20)
...
## [0.37.109] - Recover bounded Agent loops with CLI 0.166.65 (2026-09-19)
...
- Keep the extension a bounded CLI client; execution and recovery authority
  remain in the exact-gated CLI release.
...
- Add **Continue in New Conversation with Task Notes** so a completed tool
  boundary can hand verified notes to a fresh chat without copying the full
  prior transcript or inheriting temporary permissions.
...
- Bind approval grants and terminal decisions to the exact operation,
  ...
  fail closed.
- Vendor Agent SDK `0.2.5` and pair the candidate with CLI `0.166.9`; require
...
## [0.37.51] - Governed automatic ghost text (2026-08-12)
...
- Consume only the canonical `chainlesschain.automation-center/v3` projection,
  validate and render bounded sanitized run incidents, and keep them read-only
...
- Publish a maintenance build with no intentional user-facing IDE behavior
  changes from 0.37.47.
- Re-certify the immutable VSIX and its paired JetBrains artifact from the same
  latest-main commit under the fail-closed cross-platform and native ARM64
  release gates.
- Refresh the packaged Marketplace overview and recommended stable CLI baseline
  to `chainlesschain@0.163.2`.
