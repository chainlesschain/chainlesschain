...
- Keep `shadow` and `suggest` unavailable in interactive IDE sessions until
  ...
  not add decision-model credentials or execution authority to the plugin.
...
- Recommend the publicly verified `chainlesschain@0.166.62` release in runtime
  upgrade guidance and release metadata.
- Keep the bridge a bounded CLI client; this correction changes neither
  execution authority nor the plugin's compatibility floor.
...
- Route open, browser-open, reveal, and copy-path actions through the CLI's
  audited `artifacts access` authority before exposing stored bytes. Missing,
  stale, malformed, or out-of-store paths now fail closed.
- Give every artifact removal a stable JetBrains-scoped deletion identifier and
...
## [0.4.92] - Secure LLM setup repair (2026-08-18)
...
- Reuse the plugin's validated CLI resolver and managed-CLI fallback when the
  global `cc` shim is stale or broken, while preserving explicit user paths.
...
- Persist bounded conflict evidence for explanation and recovery, and expose
  ...
  recheck; the plugin never derives merge or reset commands itself.
...
- Publish a maintenance build with no intentional user-facing IDE behavior
  changes from 0.4.84.
- Recommend companion CLI `0.163.4`, which adds immutable MCP capsule build
  inputs, platform-bound live capsule evidence, durable revision-CAS scheduler
  storage, and delivery/recovery hardening while carrying forward `0.163.3`'s
  background-agent worktree isolation and fenced ownership.
...
- Re-certify the immutable plugin and its paired VSIX from the same latest-main
  commit under the fail-closed cross-platform and native ARM64 release gates.
- Preserve the 2024.2/2025.2 six-host compatibility matrix and exact 11-cell
  aggregate as release-blocking evidence.
...
- Show canonical session ownership, worktree, pending input, artifact, and PR
  detail in the native Sessions Workbench table selection.
- Route background replies through the CLI-authored action preview and
  authenticated background transport with revision rechecks.
- Drive the five canonical session kinds through Dispatch, `needs_input`,
  Reply, artifact/PR rendering, and a second independent IntelliJ process that
  verifies restart recovery in the Remote Robot host gate.
