...
Source candidate `0.4.129` pairs with CLI candidate `0.166.63`. It carries the
CLI's governed RSIAgent evolution and the immutable CLI pairing without
...
- Recommend `chainlesschain@0.166.63` for older installations, including when the
...
- If npm lookup is unavailable or stale, the plugin still treats `0.166.63` as
...
Plugin **0.4.129** is the current release candidate that re-certifies the read-only
...
The recommended CLI pairing is `chainlesschain@0.166.63`. Candidate CLI
...
permission/budget enforcement. Version `0.4.129` accepts only the exact
...
Version `0.4.129` also consumes only strict, CLI-issued multi-agent merge-review
...
`0.4.129`. The plugin continues to fail closed when an older CLI cannot provide
