...
Release candidate `0.4.132` pairs with public CLI `0.166.69`. It carries the
CLI's durable Volcengine function authority, replay and revocation checks,
supervised child execution, and signer-status rollback protection without
moving authority into the IDE. It also retains verified task-note handoff,
...
- Recommend `chainlesschain@0.166.69` for older installations, including when the
...
- If npm lookup is unavailable or stale, the plugin still treats `0.166.69` as
...
Plugin **0.4.132** is the current release candidate that re-certifies the read-only
...
The recommended CLI pairing is `chainlesschain@0.166.69`. Candidate CLI
...
permission/budget enforcement. Version `0.4.132` accepts only the exact
...
Version `0.4.132` also consumes only strict, CLI-issued multi-agent merge-review
...
`0.4.132`. The plugin continues to fail closed when an older CLI cannot provide
