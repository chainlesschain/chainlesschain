...
Release candidate `0.4.135` pairs with public CLI `0.166.72`. The CLI records
local Skill decision deadlines as `provider-timeout` with unknown usage and
uses one-sided 95% exact binomial bounds for offline quality checks. The pilot
remains limited to durable, single-prompt headless CLI sessions; interactive
IDE sessions do not expose `shadow` or `suggest`, and the plugin stores no
decision-model credentials. It retains verified task-note handoff, stale-edit
repair recovery, and bounded diagnostics for the next repair step.
...
- Recommend `chainlesschain@0.166.72` for older installations, including when the
...
- If npm lookup is unavailable or stale, the plugin still treats `0.166.72` as
...
Plugin **0.4.135** is the current release candidate that re-certifies the read-only
...
The recommended CLI pairing is `chainlesschain@0.166.72`. Candidate CLI
...
permission/budget enforcement. Version `0.4.135` accepts only the exact
...
Version `0.4.135` also consumes only strict, CLI-issued multi-agent merge-review
...
`0.4.135`. The plugin continues to fail closed when an older CLI cannot provide
