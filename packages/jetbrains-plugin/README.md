...
- **Restore web fetch and recover repeated CI-log requests.** CLI `0.166.33`
...
- Recommend `chainlesschain@0.166.33` for older installations, including when the
...
- **Long-running work recovers from non-delivery exploration.** Public CLI
  `0.166.33` counts source inspection separately from implementation progress,
...
- **Long-document tasks use focused reads and local counting.** CLI `0.166.33`
...
- **Slow foreground commands no longer make the persistent IDE Agent appear
  dead.** CLI `0.166.33` keeps the host lease heartbeat responsive and permits
  the unchanged live owner to recover safely after an event-loop stall.
- If npm lookup is unavailable or stale, the plugin still treats `0.166.33` as
...
- **Skill Retrieval** displays the canonical CLI search result with witnessed
  ...
  matches.
- The paired **CLI 0.166.33** adds encrypted tenant-scoped evolution knowledge,
...
Candidate CLI `0.166.33` routes Graph, Team, distributed-team, Cowork, Scheduler,
...
The recommended public CLI is `chainlesschain@0.166.33`. Candidate CLI
`0.166.33` contains the governed Automation/Routine commands, Automation Center
...
history-rewriting rollback commands. Candidate CLI `0.166.33` provides the
governed `team merge-review` command and exact evidence contract.

Candidate CLI `0.166.33` contains the audited Artifact access, managed-copy
...
Public CLI `0.166.33` supersedes `0.166.18`, whose public npm dependency graph
...
Candidate CLI `0.166.33` also adds lease-bound real-time teammate send/receive/ack and
