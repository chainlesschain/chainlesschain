...
Release candidate `0.4.136` pairs with public CLI `0.166.73`. The CLI supports
...
- Recommend `chainlesschain@0.166.73` for older installations, including when the
...
- If npm lookup is unavailable or stale, the plugin still treats `0.166.73` as
...
Plugin **0.4.136** is the current release candidate that re-certifies the read-only
...
The recommended CLI pairing is `chainlesschain@0.166.73`. Candidate CLI
...
permission/budget enforcement. Version `0.4.136` accepts only the exact
...
Version `0.4.136` also consumes only strict, CLI-issued multi-agent merge-review
...
`0.4.136`. The plugin continues to fail closed when an older CLI cannot provide
