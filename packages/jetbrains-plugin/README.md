...
Source candidate `0.4.124` pairs with the publicly verified CLI `0.166.56`.
...
- Recommend `chainlesschain@0.166.56` for older installations, including when the
...
- If npm lookup is unavailable or stale, the plugin still treats `0.166.56` as
...
Plugin **0.4.124** is the current release candidate that re-certifies the read-only
...
The recommended CLI pairing is `chainlesschain@0.166.56`. Candidate CLI
...
permission/budget enforcement. Version `0.4.124` accepts only the exact
...
Version `0.4.124` also consumes only strict, CLI-issued multi-agent merge-review
...
`0.4.124`. The plugin continues to fail closed when an older CLI cannot provide
