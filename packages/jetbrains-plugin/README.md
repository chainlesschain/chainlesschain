...
Source candidate `0.4.127` pairs with CLI candidate `0.166.60`. It carries the
...
- Recommend `chainlesschain@0.166.60` for older installations, including when the
...
- If npm lookup is unavailable or stale, the plugin still treats `0.166.60` as
...
Plugin **0.4.127** is the current release candidate that re-certifies the read-only
...
The recommended CLI pairing is `chainlesschain@0.166.60`. Candidate CLI
...
permission/budget enforcement. Version `0.4.127` accepts only the exact
...
Version `0.4.127` also consumes only strict, CLI-issued multi-agent merge-review
...
`0.4.127`. The plugin continues to fail closed when an older CLI cannot provide
