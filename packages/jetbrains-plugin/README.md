...
Release candidate `0.4.133` pairs with public CLI `0.166.70`. It carries the
CLI's governed Jev Skill decision pilot without moving provider credentials,
routing authority, or Skill execution into the IDE. The pilot remains limited
to durable, single-prompt headless CLI sessions; interactive IDE sessions do
not expose `shadow` or `suggest`. It also retains verified task-note handoff,
stale-edit repair recovery, and bounded diagnostics for the next repair step.
...
- Recommend `chainlesschain@0.166.70` for older installations, including when the
...
- If npm lookup is unavailable or stale, the plugin still treats `0.166.70` as
...
Plugin **0.4.133** is the current release candidate that re-certifies the read-only
...
The recommended CLI pairing is `chainlesschain@0.166.70`. Candidate CLI
...
permission/budget enforcement. Version `0.4.133` accepts only the exact
...
Version `0.4.133` also consumes only strict, CLI-issued multi-agent merge-review
...
`0.4.133`. The plugin continues to fail closed when an older CLI cannot provide
