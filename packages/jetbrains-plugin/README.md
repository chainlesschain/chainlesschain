...
Source candidate `0.4.128` pairs with CLI candidate `0.166.62`. It carries the
CLI's bounded, independently graded PM exploration and authenticated recovery
evidence without moving execution or promotion authority into the IDE. It also
...
- Recommend `chainlesschain@0.166.62` for older installations, including when the
...
- If npm lookup is unavailable or stale, the plugin still treats `0.166.62` as
...
Plugin **0.4.128** is the current release candidate that re-certifies the read-only
...
The recommended CLI pairing is `chainlesschain@0.166.62`. Candidate CLI
...
permission/budget enforcement. Version `0.4.128` accepts only the exact
...
Version `0.4.128` also consumes only strict, CLI-issued multi-agent merge-review
...
`0.4.128`. The plugin continues to fail closed when an older CLI cannot provide
