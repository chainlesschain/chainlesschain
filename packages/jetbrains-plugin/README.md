...
Plugin **0.4.104** is the current release candidate that re-certifies the read-only
...
Candidate CLI `0.166.9` routes Graph, Team, distributed-team, Cowork, Scheduler,
...
The recommended public CLI remains `chainlesschain@0.166.7`. Candidate CLI
`0.166.9` contains the governed Automation/Routine commands, Automation Center
v3 projection, scoped permission and side-effect authority, and shared
permission/budget enforcement. Version `0.4.104` accepts only the exact
...
history-rewriting rollback commands. Candidate CLI `0.166.9` provides the
...
Public CLI `0.166.7` supersedes `0.165.9`, whose normal Windows startup could
...
Candidate CLI `0.166.9` also adds lease-bound real-time teammate send/receive/ack and
