...
- **Long-document tasks use focused reads and local counting.** CLI `0.166.27`
...
- **Slow foreground commands no longer make the persistent IDE Agent appear
  dead.** CLI `0.166.27` keeps the host lease heartbeat responsive and permits
  the unchanged live owner to recover safely after an event-loop stall.
- If npm lookup is unavailable or stale, the plugin still treats `0.166.27` as
...
- **Skill Retrieval** displays the canonical CLI search result with witnessed
  ...
  matches.
- The paired **CLI 0.166.27** adds encrypted tenant-scoped evolution knowledge,
...
Candidate CLI `0.166.27` routes Graph, Team, distributed-team, Cowork, Scheduler,
...
The recommended public CLI is `chainlesschain@0.166.27`. Candidate CLI
`0.166.27` contains the governed Automation/Routine commands, Automation Center
...
history-rewriting rollback commands. Candidate CLI `0.166.27` provides the
governed `team merge-review` command and exact evidence contract.

Candidate CLI `0.166.27` contains the audited Artifact access, managed-copy
...
Public CLI `0.166.27` supersedes `0.166.18`, whose public npm dependency graph
...
Candidate CLI `0.166.27` also adds lease-bound real-time teammate send/receive/ack and
