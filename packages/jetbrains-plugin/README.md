...
- Recommend `chainlesschain@0.166.65` for older installations, including when the
...
- If npm lookup is unavailable or stale, the plugin still treats `0.166.65` as
...
The recommended CLI pairing is `chainlesschain@0.166.65`. Candidate CLI
...
permission/budget enforcement. Version `0.4.130` accepts only the exact
...
Version `0.4.130` also consumes only strict, CLI-issued multi-agent merge-review
...
`0.4.130`. The plugin continues to fail closed when an older CLI cannot provide
