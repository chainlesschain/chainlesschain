...
Release candidate `0.4.134` pairs with public CLI `0.166.71`. It includes the
CLI's local Skill decision provider selection without moving credentials,
...
- Recommend `chainlesschain@0.166.71` for older installations, including when the
...
- If npm lookup is unavailable or stale, the plugin still treats `0.166.71` as
...
Plugin **0.4.134** is the current release candidate that re-certifies the read-only
...
The recommended CLI pairing is `chainlesschain@0.166.71`. Candidate CLI
...
permission/budget enforcement. Version `0.4.134` accepts only the exact
...
Version `0.4.134` also consumes only strict, CLI-issued multi-agent merge-review
...
`0.4.134`. The plugin continues to fail closed when an older CLI cannot provide
