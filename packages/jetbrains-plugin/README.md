...
Release candidate `0.4.137` pairs with public CLI `0.166.74`. The CLI supports
...
covers Git arguments, explicit PowerShell selection, and saved JSON BOMs.
The release also retains
signed, offline PM effect evidence, frozen cohort slots, conservative unresolved
denominators, and signed usage totals. Target-environment PM/Pilot outcomes and
complete costs remain unverified. Local Skill decisions remain limited to
durable, single-prompt headless CLI sessions; interactive IDE sessions do not
expose `shadow` or `suggest`, and the plugin stores no decision-model credentials.
It retains verified task-note handoff, stale-edit repair recovery, and bounded
diagnostics for the next repair step.
The bridge remains a bounded CLI client: trust, approval, publication, and
command construction stay in the CLI host. Marketplace upload follows only
after this exact IDE commit passes its gates.
...
- **Search before reading.** The paired CLI supports keyword web discovery,
  ...
  matches with exact positions and focused read cursors.

- **Restore web fetch and recover repeated CI-log requests.** CLI `0.166.34`
...
- Recommend `chainlesschain@0.166.74` for older installations, including when the
...
- If npm lookup is unavailable or stale, the plugin still treats `0.166.74` as
...
- **Skill Retrieval** displays the canonical CLI search result with witnessed
  ...
  matches.
- The paired **CLI 0.166.34** adds encrypted tenant-scoped evolution knowledge,
...
Plugin **0.4.137** is the current release candidate that re-certifies the read-only
...
Candidate CLI `0.166.34` routes Graph, Team, distributed-team, Cowork, Scheduler,
...
The recommended CLI pairing is `chainlesschain@0.166.74`. Candidate CLI
...
permission/budget enforcement. Version `0.4.137` accepts only the exact
...
Version `0.4.137` also consumes only strict, CLI-issued multi-agent merge-review
...
history-rewriting rollback commands. Candidate CLI `0.166.34` provides the
...
`0.4.137`. The plugin continues to fail closed when an older CLI cannot provide
...
Candidate CLI `0.166.34` also adds lease-bound real-time teammate send/receive/ack and
...
- JetBrains actions: **38** registered entries
