...
Candidate CLI `0.166.11` routes Graph, Team, distributed-team, Cowork, Scheduler,
...
The recommended public CLI is `chainlesschain@0.166.11`. Candidate CLI
`0.166.11` contains the governed Automation/Routine commands, Automation Center
...
history-rewriting rollback commands. Candidate CLI `0.166.11` provides the
governed `team merge-review` command and exact evidence contract.

Candidate CLI `0.166.11` contains the audited Artifact access, managed-copy
...
Public CLI `0.166.11` supersedes `0.165.9`, whose normal Windows startup could
...
Candidate CLI `0.166.11` also adds lease-bound real-time teammate send/receive/ack and
