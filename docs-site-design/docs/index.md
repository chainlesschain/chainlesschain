...
hero:
  name: ChainlessChain
  text: 系统设计文档
  tagline: v5.0.3.45 | 95+ 模块设计 | 139 技能 | CLI 112 命令/0.161.7 | 14800+ 总测试 | V2 规范层 220+ 治理表面（iter16-iter28） | Web Panel 50 routes（Phase A+B+C+D 全量收官 + i18n M3 全覆盖）| Web Shell（Phase 1.6 默认 · `--no-web-shell` opt-out · MTC 视图 in-process 提速绕过 asar 冷启动 · Phase 3c.7 截图识别 + 通知设置 + 托盘路由收口）| MTC v0.11（Phase 0–4 联邦 + 跨联邦信任锚 + 离线审计 + 多跳路由 + Gas 感知 · 476 测试 / 6 层 · 30+ CLI 子命令 · **landmark.publisher_signature 验证 + M-of-N strip-all-sigs 对称化（v5.0.3.43）**）| **社区/频道跨机同步 + B4 audit-grade 闭环 §2.2.10–§2.2.24 全量设计**（Phase A 7 bug 修 + Phase B v1 MTC 双轨 + B4 DID 签名 + 自动 peer 桥接 + Merkle envelope finality + 跨机分发 + trust filter + viewer + 外部归档 + M-of-N 多签 + 跨联邦信任 + sign-as-self + WebDAV cred-persist + 周期定时归档 cron · 私钥 / 密码均不过线）| **chat-panel-v5 三壳对齐**（V5 / V6 / web-shell 聊天能力严格对等：流式 + 历史会话 + 上下文记忆 + 工具调用面板，Phase E commit `b33527d31` + commit `72b13388a`）| B4 post-pack ASAR surgery（Win 安装 20m → ~5m，issue #8）| **安全硬化级联（v5.0.3.43）**（HIGH 44→0 / MOD 4→0 / LOW 45→0 · drop hdkey + werift + speedtest-net · CVE-2024-29415 修补）| CI 三发解锁（macOS / 规则误报 / Win cold-start）| 面向开发者的架构与实现文档
  image:
    src: /logo.png
