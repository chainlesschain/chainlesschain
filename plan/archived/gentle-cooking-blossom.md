...
ipcMain.handle(
  ...
  async (_event, { mnemonic, password }) => {
...
ipcMain.handle(
  ...
  async (_event, { walletId, tx, useUKey }) => {
...
class AssetManager extends EventEmitter {
    ...
    if (onChain) {
      try {
        ...
        if (assetData.asset_type === AssetType.TOKEN) {
          // 部署 ERC-20
          const result = await this.blockchainAdapter.deployERC20Token(
            walletId,
            {
              name: assetData.name,
        ...
        await this._saveBlockchainAsset({
          ...
          chain_id: chainId,
          token_type:
            assetData.asset_type === AssetType.TOKEN ? "ERC20" : "ERC721",
  ...
  async transferAsset(options) {
    ...
    // 1. 执行本地转账（原有逻辑）
    const localTransfer = await this._transferLocal(
      assetId,
      fromDid,
      toDid,
      amount,
    ...
    if (onChain) {
      ...
      switch (contractData.contract_type) {
        case ContractType.SIMPLE_TRADE:
          // 部署托管合约
          deployedAddress = await this._deployEscrowContract(
            walletId,
            chainId,
            contractData,
          );
        ...
        case ContractType.SUBSCRIPTION:
          // 部署订阅合约
          deployedAddress = await this._deploySubscriptionContract(
            walletId,
            chainId,
            contractData,
          );
...
<template>
  <div class="wallet-page">
    ...
    <!-- 创建钱包对话框 -->
    <CreateWalletModal
      v-model:visible="showCreateWallet"
  ...
  actions: {
    ...
    async connectExternalWallet(provider) {
      if (provider === "metamask") {
        this.currentWallet = await window.ipcRenderer.invoke(
...
class BridgeManager extends EventEmitter {
  ...
  async bridgeAsset(options) {
    ...
    // 1. 在源链锁定资产
    const lockTx = await this._lockOnSourceChain(
      fromChainId,
      assetId,
      amount,
      walletId,
    ...
    // 3. 在目标链铸造资产
    const mintTx = await this._mintOnTargetChain(
      toChainId,
      assetId,
      amount,
      walletId,
