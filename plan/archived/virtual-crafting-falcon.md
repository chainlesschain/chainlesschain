...
>
  <a-form ref="formRef" :model="formData" layout="vertical">
    <!-- 动态生成的表单项 -->
    <template
      v-for="variable in template.variables_schema"
      :key="variable.name"
    >
      <!-- String 类型 -->
      <a-form-item
...
function initFormData() {
  ...
  template.value.variables_schema?.forEach((variable) => {
    data[variable.name] =
      variable.default || (variable.type === "array" ? [] : "");
  });
  formData.value = data;
}
...
async function handleSubmit() {
  try {
    ...
    const project = await projectStore.createProjectStream({
      name:
        formData.value[template.value.variables_schema[0]?.name] ||
...
<!-- 修改为 -->
<TemplateGallery :category="selectedType" :subcategory="activeCategory" />
