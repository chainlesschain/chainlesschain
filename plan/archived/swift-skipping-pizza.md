...
class DBSyncManager extends EventEmitter {
  ...
  async downloadRemoteChanges(tableName) {
    ...
    console.log(
      `[DBSyncManager] 下载表 ${tableName} 的增量数据，上次同步: ${new Date(lastSyncedAt)}`,
    );
    ...
    for (const deletedId of deletedIds || []) {
      this.database.db.run(`UPDATE ${tableName} SET deleted = 1 WHERE id = ?`, [
        deletedId,
      ]);
    }
...
ipcMain.handle("sync:get-status", async () => {
  try {
    return await this.syncManager.httpClient.getSyncStatus(
  ...
  <a-layout>
    <a-layout-header>
      ...
      <div class="sync-indicator" @click="showSyncStatus">
        ...
        <CloudSyncOutlined
          v-else
          :style="{ color: syncError ? 'red' : '#52c41a' }"
        />
      </div>
    </a-layout-header>
...
const api = {
  ...
  sync: {
    ...
    onShowConflicts: (callback) =>
      ipcRenderer.on("sync:show-conflicts", callback),
  },
};
```
