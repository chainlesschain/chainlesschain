...
ipcMain.handle("project:get-file-content", async (_event, fileId, filePath) => {
  ...
  const dbFile = this.database.db
    .prepare("SELECT content FROM project_files WHERE id = ? OR file_path = ?")
    .get(fileId, filePath);
...
const treeData = computed(() => {
  ...
  // 验证数据完整性
  const invalidFiles = props.files.filter(
    (f) => !f.file_path && !f.path && !f.file_name,
  );
...
1. **文件ID不一致** - 旧数据可能使用UUID，新数据使用fs\_前缀
   - 缓解措施：添加兼容层，支持两种ID格式
