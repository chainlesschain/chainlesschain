...
class PluginLoader {
  ...
  async installFromNpm(packageName, options) {
    ...
    await this.execCommand(
      `npm install ${fullPackage} --prefix ${installPath}`,
    );
...
const createPluginAPI = () => {
  return {
    ...
    console: {
      log: (...args) =>
        parentPort.postMessage({ type: "log", level: "info", args }),
      error: (...args) =>
...
class MyPlugin {
  ...
  async onEnable() {
    ...
    // 注册生命周期钩子
    await api.lifecycle.registerHook(
      "knowledge:before-add",
      async (context) => {
        const { item } = context;
...
export default {
  ...
  methods: {
    async getWeather() {
      try {
        // 调用插件API
        this.weather = await window.pluginAPI.call("getWeather", {
          city: this.city,
        });
      } catch (error) {
