...
| **加密 App IM 且 App 能跑起来+无强反 frida（头条✅/微信✅；抖音有 libmsaoaidsec 可能杀 frida）** | **方法 C：frida `sqlcipher_export` 在线解密**（见下，脚本 `pdh-frida-decrypt.sh`）。借 App **自己已用正确 key 打开的连接** `ATTACH '' KEY ''`+`sqlcipher_export` 导出明文副本——**不碰 cipher 参数**。⚠️**离线解密别走**：拿 key 后用 better-sqlite3 复现 WCDB 自研 cipher，2026-06-17 实测 72 组合全失败。 | **完整明文库副本**（最可靠的加密 IM 解法）|
...
## 方法 C 流程（frida `sqlcipher_export` 在线解密 — 加密 IM 最可靠解）

> **关键洞见（2026-06-17 突破）**：加密 IM 库（WCDB/SQLCipher，如头条/抖音 `encrypted_<uid>_im.db`）
...
> **72 组合全失败**。正解 = **借 App 进程里已用正确 key 打开的那条连接**，直接在它上面跑
> `ATTACH '<out>' AS ccpt KEY ''`（空 key=明文）+ `SELECT sqlcipher_export('ccpt')` → 完整明文副本，
> **完全绕过 cipher 参数复现**。脚本：`scripts/android/pdh-frida-decrypt.sh` + `pdh-frida-sqlcipher-export.js`。
...
- **反 frida**：头条无 `libmsaoaidsec`，frida-inject 正常；抖音带它，可能在 attach 后被杀（换头条同账号 IM，
  ByteDance 同 `com.ss.android.im` 框架、schema 一致；或先解抖音的非 IM 明文库）。
