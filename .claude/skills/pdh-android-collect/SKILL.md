...
| **✅ 微信 EnMicroMsg.db（已验证·首选解密路）** | **方法 W：`scripts/android/pdh-wechat-decrypt.mjs --serial <s> --out-dir ~/pdh-data/wx --ingest`**。一键 pull+候选 7 位口令`MD5(IMEI+uin)[:7]`暴力+Node crypto 逐页 AES（SQLCipher1：1024/无HMAC/kdf4000）+入库。口令存盘下次秒解。新账号取口令走 frida `pdh-frida-wechat-aeskey.mjs`(hook `aes_v8_set_encrypt_key`，**非** sqlite3_key)。详 `docs/internal/pdh-wechat-decrypt-runbook.md`。 | **实测 52129 消息/1926 联系人入库** |
