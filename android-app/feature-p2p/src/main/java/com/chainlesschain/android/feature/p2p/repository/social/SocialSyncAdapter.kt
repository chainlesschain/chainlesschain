...
class SocialSyncAdapter @Inject constructor(
    ...
    fun syncFriendAdded(friend: FriendEntity) {
        val syncItem = SyncItem(
            ...
            data = json.encodeToString(FriendSyncData.serializer(), friend.toSyncData()),
    ...
    fun syncFriendDeleted(did: String) {
        val syncItem = SyncItem(
            ...
            data = json.encodeToString(FriendSyncData.serializer(), FriendSyncData(did)),
            timestamp = System.currentTimeMillis()
        )
        syncManager.recordChange(syncItem)
    ...
    fun syncPostCreated(post: PostEntity) {
        val syncItem = SyncItem(
            ...
            operation = SyncOperation.CREATE,
            data = json.encodeToString(PostSyncData.serializer(), post.toSyncData()),
            timestamp = System.currentTimeMillis()
        )
        syncManager.recordChange(syncItem)
    ...
    fun syncPostDeleted(postId: String) {
        val syncItem = SyncItem(
            ...
            resourceType = ResourceType.POST,
            operation = SyncOperation.DELETE,
            data = json.encodeToString(PostSyncData.serializer(), PostSyncData(id = postId)),
            timestamp = System.currentTimeMillis()
        )
        syncManager.recordChange(syncItem)
    ...
    fun syncLikeAdded(like: PostLikeEntity) {
        val syncItem = SyncItem(
            ...
            operation = SyncOperation.CREATE,
            data = json.encodeToString(PostLikeSyncData.serializer(), like.toSyncData()),
            timestamp = System.currentTimeMillis()
        )
        syncManager.recordChange(syncItem)
    ...
    fun syncLikeRemoved(likeId: String) {
        val syncItem = SyncItem(
            ...
            resourceType = ResourceType.POST_LIKE,
            operation = SyncOperation.DELETE,
            data = json.encodeToString(PostLikeSyncData.serializer(), PostLikeSyncData(id = likeId)),
            timestamp = System.currentTimeMillis()
        )
        syncManager.recordChange(syncItem)
    ...
    fun syncCommentAdded(comment: PostCommentEntity) {
        val syncItem = SyncItem(
            ...
            operation = SyncOperation.CREATE,
            data = json.encodeToString(PostCommentSyncData.serializer(), comment.toSyncData()),
            timestamp = System.currentTimeMillis()
        )
        syncManager.recordChange(syncItem)
    ...
    fun syncCommentDeleted(commentId: String) {
        val syncItem = SyncItem(
            ...
            resourceType = ResourceType.POST_COMMENT,
            operation = SyncOperation.DELETE,
            data = json.encodeToString(PostCommentSyncData.serializer(), PostCommentSyncData(id = commentId)),
            timestamp = System.currentTimeMillis()
        )
        syncManager.recordChange(syncItem)
    ...
    fun syncNotificationCreated(notification: NotificationEntity) {
        val syncItem = SyncItem(
            ...
            operation = SyncOperation.CREATE,
            data = json.encodeToString(NotificationSyncData.serializer(), notification.toSyncData()),
            timestamp = System.currentTimeMillis()
        )
        syncManager.recordChange(syncItem)
    ...
    private suspend fun applyFriendSync(syncItem: SyncItem) {
        val data = json.decodeFromString(FriendSyncData.serializer(), syncItem.data)
    ...
    private suspend fun applyPostSync(syncItem: SyncItem) {
        val data = json.decodeFromString(PostSyncData.serializer(), syncItem.data)
    ...
    private suspend fun applyLikeSync(syncItem: SyncItem) {
        val data = json.decodeFromString(PostLikeSyncData.serializer(), syncItem.data)
    ...
    private suspend fun applyCommentSync(syncItem: SyncItem) {
        val data = json.decodeFromString(PostCommentSyncData.serializer(), syncItem.data)
    ...
    private suspend fun applyNotificationSync(syncItem: SyncItem) {
        val data = json.decodeFromString(NotificationSyncData.serializer(), syncItem.data)
