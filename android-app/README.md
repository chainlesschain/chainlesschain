...
| M3 (2/5) | L2 捕获：ShareReceiver + LocationTagger（D-voice/D-camera/D-push 待真机） | 31 |
| M4 | RemoteSkillRegistry (Android) + mobile-skill-whitelist + approval channel + command-router gate (Desktop) | 80 |
| M5 | 反向 SignAsService — 桌面调手机硬件签名（macOS/Linux 无 U-Key 的跨平台 USB key 路径） | 33 |
| **共** | | **212** ✅ |

**v1.0 GA 仍待**：M3 三件设备 API 集成 / M4/M5 mobile-bridge wire-up + ApprovalUI Compose 屏 / M6 性能压测 / E2E 真机走完。详见 [CHANGELOG.md](./CHANGELOG.md#0370---2026-05-11--v10-rfc-实施轨道)。
