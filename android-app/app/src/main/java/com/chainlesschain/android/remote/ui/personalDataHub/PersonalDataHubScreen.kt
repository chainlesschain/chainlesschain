...
/**
 * Phase 14.1 + A3.8 — 个人数据中台容器屏
 *
 * 五 Tab：提问 / Adapters / 审计 / 本机数据 / 本机提问。
 ...
 * 用户在两 tab 切换不丢 askCardState / systemDataState。
 *
 * Tab index 0-3 保留不变以兼容现有 NavGraph initialTab=0/3 路由（详见
 * navigation/NavGraph.kt createRoute(initialTab) caller）。新 tab 添加在尾部
 * index 4，老 caller 无需改动。
 *
 * ScrollableTabRow 替换 TabRow：5 tab + 中文 4 字标题在 360dp 窄屏上 TabRow
...
fun PersonalDataHubScreen(
    ...
    val titles = listOf("提问", "Adapter", "审计", "本机数据", "本机提问")
