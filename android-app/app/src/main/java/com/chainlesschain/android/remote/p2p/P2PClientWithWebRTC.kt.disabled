...
class P2PClientWithWebRTC @Inject constructor(
    ...
    init {
        ...
        // 设置消息回调
        webrtcClient.setOnMessageReceived { data ->
            scope.launch {
                handleMessage(data)
            }
        }
    ...
    suspend fun connect(pcPeerId: String, pcDID: String): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            if (result.isFailure) {
                ...
                return@withContext Result.failure(result.exceptionOrNull() ?: Exception("Connection failed"))
            }
