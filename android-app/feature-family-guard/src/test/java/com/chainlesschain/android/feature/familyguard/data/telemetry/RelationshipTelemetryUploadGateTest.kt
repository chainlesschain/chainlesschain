...
/**
 * FAMILY-25 v0.1 验收: RelationshipTelemetryUploadGate level gate —
 * L0 永远放行 / L1+ 逐 guardian 查 checker / 无 active 返空。
 */
class RelationshipTelemetryUploadGateTest {
    ...
    private fun event(level: TelemetryLevel) = TelemetryEvent(
    ...
    fun `no active relationship returns empty`() = runTest {
        val gate = RelationshipTelemetryUploadGate(
            relationshipRepository = FakeRelRepo(active = emptyList()),
            permissionChecker = FakeChecker(allowed = setOf(mom, dad)),
        )
    ...
    fun `L0 aggregate allows all active guardians regardless of checker`() = runTest {
        val gate = RelationshipTelemetryUploadGate(
            relationshipRepository = FakeRelRepo(active = listOf(rel(mom), rel(dad))),
            permissionChecker = FakeChecker(allowed = emptySet()), // checker 全 Deny
        )
    ...
    fun `L1 returns only guardians the checker allows`() = runTest {
        val gate = RelationshipTelemetryUploadGate(
            relationshipRepository = FakeRelRepo(active = listOf(rel(mom), rel(dad))),
            permissionChecker = FakeChecker(allowed = setOf(mom)), // 只 mom 同意 L1
        )
    ...
    fun `L1 maps to ReadTelemetryL1 action`() = runTest {
        ...
        val gate = RelationshipTelemetryUploadGate(FakeRelRepo(listOf(rel(mom))), checker)
    ...
    private fun rel(friendDid: String) = FamilyRelationshipEntity(
        familyGroupId = "grp",
        friendDid = friendDid,
        roleSelf = "child",
        roleOther = "guardian",
