 ...
 * 网络模拟器（共享版）
 *
 * Lifted from :core-network/androidTest into :core-test-helpers/main so any
 * feature module's androidTest source set can use it via
 *   androidTestImplementation(project(":core-test-helpers"))
 *
 * Behavior preserved from the original (commit 4bfc8f474).
 */
class NetworkSimulator : TestWatcher() {
    ...
    /** 入队 SSE 流响应（模拟 LLM 流式响应） */
    fun enqueueSSE(events: List<String>, delayMs: Long = 100) {
        val sseBody = buildString {
            events.forEach { event -> append("data: $event\n\n") }
            append("data: [DONE]\n\n")
        }
        mockWebServer.enqueue(
            MockResponse()
    ...
    /** 获取最后一个请求 */
    fun takeRequest(timeoutMs: Long = 1000): RecordedRequest? = try {
