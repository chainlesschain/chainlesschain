...
if command -v python3 &> /dev/null; then
    ...
    # macOS自动打开浏览�?    if [[ "$OSTYPE" == "darwin"* ]]; then
        open http://localhost:8000
    # Linux自动打开浏览�?    elif [[ "$OSTYPE" == "linux-gnu"* ]]; then
        xdg-open http://localhost:8000 2>/dev/null || echo "请手动打开浏览器访�?http://localhost:8000"
    fi
