...
|---|---|---|
| `pkgName` | TEXT | pkg名称 |
| `isServerPatch` | INTEGER | 是否服务器p@ch |
| `patchPath` | TEXT | p@ch路径 |
...
| `noticeId` | INTEGER | 编号ticeID |
| `extInfo` | TEXT | 扩展信息 |
| `userSessionId` | TEXT | 用户sessionID |
| `startTime` | LONG | 开始时间 |
...
| `lvbuff` | BLOB | (lvbuff) |
| `serviceAppType` | INTEGER | service应用类型 |
| `serviceAppInfoFlag` | INTEGER | service应用信息标志 |
| `serviceShowFlag` | INTEGER | service显示标志 |
...
| `sessionName` | TEXT | session名称 |
| `moveTime` | BLOB | move时间 |

### `BackupRecoverMsgListDataId`
...
| `aId` | TEXT | aID |
| `msgId` | LONG | 消息ID |
| `exposeTime` | LONG | ex位置e时间 |

### `BizAppMsgReportContext`
...
| 字段 | 类型 | 中文备注 |
|---|---|---|
| `appMsgReportContextId` | LONG | 应用消息上报con文本ID |
| `url` | TEXT | 链接 |
...
| `needToUpdate` | INTEGER | 是否to更新 |
| `headImageUrl` | TEXT | 头像图片链接 |
| `profileUrl` | TEXT | pro文件链接 |
| `bitFlag` | INTEGER | 位标志 |
| `addMemberUrl` | TEXT | add成员链接 |

### `BizContactConversationMassSend`
...
| `appMsgStatInfoProto` | BLOB | 应用消息st@信息proto |
| `isRead` | INTEGER | 是否已读 |
| `bitFlag` | INTEGER | 位标志 |
| `bizClientMsgId` | TEXT | 业务客户端消息ID |
| `rankSessionId` | TEXT | 排名sessionID |
| `recommendCardId` | TEXT | 推荐卡片ID |
| `isValidExposed` | INTEGER | 是否有效ex位置ed |
| `resortBuffer` | TEXT | (resortBuffer) |
| `recycleCardType` | INTEGER | 推荐ycle卡片类型 |
| `recommendReason` | TEXT | 推荐原因 |
| `originBitFlag` | INTEGER | 原始位标志 |
| `mergeShowTime` | LONG | merge显示时间 |
| `mergeCount` | INTEGER | merge数量 |
| `notifyMsgId` | TEXT | 编号tify消息ID |
| `notifyMsgBlockFlag` | INTEGER | 编号tify消息块标志 |
| `silentFoldMsgReadStatus` | INTEGER | 静音fold消息已读状态 |
...
| 字段 | 类型 | 中文备注 |
|---|---|---|
| `exposeId` | TEXT | ex位置eID |
| `msgId` | LONG | 消息ID |
| `exposeTime` | LONG | ex位置e时间 |
| `exposeType` | INTEGER | ex位置e类型 |
...
| `logo_color` | TEXT | logo颜色 |
| `unavailable_qr_code_list` | TEXT | unavailableqr码列表 |
...
| `userName` | TEXT | 用户名 |
| `menu` | TEXT | (menu) |
| `profileInfoDetail` | TEXT | pro文件信息详情 |
...
|---|---|---|
| `msgId` | LONG | 消息ID |
| `transferId` | TEXT | 翻译ferID |

### `DeletedConversationInfo`
...
|---|---|---|
| `productID` | TEXT | 产品ID |
| `packIconUrl` | TEXT | p确认图标链接 |
| `packGrayIconUrl` | TEXT | p确认gray图标链接 |
| `packCoverUrl` | TEXT | p确认封面链接 |
| `packName` | TEXT | p确认名称 |
| `packDesc` | TEXT | p确认描述 |
| `packAuthInfo` | TEXT | p确认授权信息 |
| `packPrice` | TEXT | p确认价格 |
| `packType` | INTEGER | p确认类型 |
| `packFlag` | INTEGER | p确认标志 |
| `packExpire` | LONG | p确认过期 |
| `packTimeStamp` | LONG | p确认时间戳 |
| `packCopyright` | TEXT | p确认copyright |
| `type` | INTEGER | 类型 |
| `status` | INTEGER | 状态 |
| `sort` | INTEGER | (sort) |
| `lastUseTime` | LONG | 最后使用时间 |
| `packStatus` | INTEGER | p确认状态 |
| `flag` | INTEGER | 标志 |
| `recommand` | INTEGER | 推荐ommand |
| `sync` | INTEGER | 同步 |
| `idx` | INTEGER | 索引 |
| `BigIconUrl` | TEXT | big图标链接 |
| `MutiLanName` | TEXT | mutilan名称 |
| `recommandType` | INTEGER | 推荐ommand类型 |
| `lang` | TEXT | 语言 |
| `recommandWord` | TEXT | 推荐ommand词 |
...
| `showNotification` | INTEGER | 显示编号tific@ion |
...
| `notificationTitle` | TEXT | 编号tific@ion标题 |
| `userSessionId` | TEXT | 用户sessionID |
...
| `entranceExposure` | INTEGER | entranceex位置ure |
| `exposuredCount` | INTEGER | ex位置u红包数量 |
| `completeExposuredCount` | INTEGER | completeex位置u红包数量 |
| `pushId` | TEXT | pushID |
| `clickScore` | FLOAT | 点击分数 |
| `channel` | INTEGER | 渠道 |
| `quickResponseContentId` | TEXT | quick响应内容ID |
| `isGreet` | INTEGER | (isGreet) |
| `relationType` | INTEGER | re纬度ion类型 |
| `mergeSenderIcon` | TEXT | merge发送者图标 |
| `contentId` | TEXT | 内容ID |

### `GameResourceDownload`
...
| `downloadItemList` | BLOB | 下载item列表 |
...
| `noWifi` | INTEGER | 编号wifi |
| `noSdcard` | INTEGER | 编号sd卡片 |
| `noEnoughSpace` | INTEGER | 编号e编号ughspace |
| `lowBattery` | INTEGER | lowb@tery |
| `continueDelay` | INTEGER | (continueDelay) |
| `SecondaryUrl` | TEXT | secondary链接 |
...
| `ret` | INTEGER | (ret) |
| `status` | INTEGER | 状态 |
| `googleitemid` | TEXT | googleitemID |
...
| `recentUseToolList` | TEXT | 推荐ent使用tool列表 |
| `queryState` | INTEGER | query状态 |

### `HardDeviceChampionInfo`
...
| `authKey` | TEXT | 授权键 |
| `url` | TEXT | 链接 |
| `sessionKey` | BLOB | session键 |
| `sessionBuf` | BLOB | (sessionBuf) |
| `authBuf` | BLOB | 授权buf |
| `lvbuffer` | BLOB | 扩展二进制(protobuf) |
...
| `selfLikeState` | INTEGER | self点赞状态 |
| `sportRecord` | BLOB | sport推荐ord |
...
| `resourceName` | TEXT | re来源名称 |
| `resourceVersion` | INTEGER | re来源版本 |
| `resourceSize` | INTEGER | re来源大小 |
| `md5` | TEXT | MD5 |
| `url` | TEXT | 链接 |
| `createTime` | LONG | 创建时间 |
...
| `diffUrl` | TEXT | diff链接 |
| `resourceRet` | INTEGER | re来源ret |
| `resourceType` | INTEGER | re来源类型 |
| `resourceExpireTime` | LONG | re来源过期时间 |
| `resourceStoreType` | INTEGER | re来源商店类型 |
...
| `systemAddressBookUsername` | TEXT | system地址book用户名 |
...
| `callTimeCount` | LONG | call时间数量 |
| `lastCallTime` | LONG | 最后call时间 |
...
| 字段 | 类型 | 中文备注 |
|---|---|---|
| `phonenumber` | TEXT | 手机数量ber |
| `calltime` | LONG | call时间 |
| `duration` | LONG | 时长 |
| `status` | INTEGER | 状态 |
| `addressId` | LONG | 地址ID |
...
| `detail_atmosphere_pag_url` | TEXT | 详情@mospherepag链接 |
...
| `ecs_jump_info_str` | TEXT | ecsjump信息str |
...
| `bubbledynamicMd5` | TEXT | bubbledynamicMD5 |
| `coverMd5` | TEXT | 封面MD5 |
| `minilogoMd5` | TEXT | 最小ilogoMD5 |
...
| `wifiDownloadedLength` | LONG | wifIDownloaded长度 |
...
| `songSnsAlbumUser` | TEXT | songsnsalbum用户 |
| `songSnsShareUser` | TEXT | songsns分享用户 |
...
| `tipType` | INTEGER | tip类型 |
| `beginShowTime` | LONG | 开始显示时间 |
| `exposureTime` | LONG | ex位置ure时间 |
| `overdueTime` | LONG | overdue时间 |
| `disappearTime` | LONG | dis应用ear时间 |
| `exposureDisappearTime` | LONG | ex位置u红包is应用ear时间 |
...
| `ticket` | TEXT | 票据 |
| `nickname` | TEXT | 昵称 |
| `addState` | INTEGER | add状态 |
| `showHead` | INTEGER | 显示头像 |
| `pinyinName` | TEXT | pinyin名称 |
| `username` | TEXT | 用户名 |
| `seq` | INTEGER | 序号 |
...
| `finder_username` | TEXT | finder用户名 |
...
| `nicknamePyQuanPin` | TEXT | 昵称pyquanpin |
...
| `needReport` | INTEGER | 是否上报 |
| `locationType` | TEXT | loc@ion类型 |
| `kefuType` | INTEGER | kefu类型 |
| `kefuToolsInfo` | TEXT | kefutools信息 |
| `enterprise_auth_status` | INTEGER | enterprise授权状态 |
...
|---|---|---|
| `openIMUsername` | TEXT | openim用户名 |
| `openIMSnsFlag` | LONG | openimsns标志 |
...
| `friendSubscribeCount` | INTEGER | 好友subscribe数量 |
...
| `showService` | INTEGER | 显示service |
| `messageListStr` | TEXT | 消息列表str |
| `serviceInfoListStr` | TEXT | service信息列表str |
| `bizAccountListStr` | TEXT | 业务ac数量列表str |
| `cacheTime` | LONG | 缓存时间 |
...
| `recordLocalId` | INTEGER | 推荐ord本地ID |
| `toUser` | TEXT | to用户 |
| `dataId` | TEXT | 数据ID |
| `mediaId` | TEXT | 媒体ID |
| `path` | TEXT | 路径 |
...
| `receiveStatus` | INTEGER | 推荐eive状态 |
| `isSend` | INTEGER | 方向: 0=收到 1=我发出 |
| `talker` | TEXT | 会话方: 单聊=对方wxid / 群=<id>@chatroom / 公众号=gh_xxx |
...
| `ScanTime` | LONG | scan时间 |
| `funcType` | INTEGER | func类型 |
| `qrcodeUrl` | TEXT | qr码链接 |
| `scene` | INTEGER | 场景 |

### `ScanTranslationResult`
...
| `msgId` | LONG | 消息ID |
| `talker` | TEXT | 会话方: 单聊=对方wxid / 群=<id>@chatroom / 公众号=gh_xxx |
| `chatHistoryItem` | BLOB | 聊天历史item |

### `ShakeNewYearFriendInfo`
...
| `cardTpInfoData` | BLOB | 卡片tp信息数据 |
| `shareInfoData` | BLOB | 分享信息数据 |
| `shopInfoData` | BLOB | 店铺信息数据 |
| `categoryType` | INTEGER | c@egory类型 |
| `itemIndex` | INTEGER | ite最小dex |

### `ShareCardSyncItemInfo`
...
| `topMsgInfoItem` | BLOB | 置顶消息信息item |
...
| `stickyAnnouncement` | TEXT | 置顶yan编号uncement |
| `card_type` | INTEGER | 卡片类型 |
...
| `wxcreditState` | INTEGER | wxc红包i时间戳t@e |
...
| `no_micro_word` | TEXT | 编号micro词 |
| `card_bottom_wording` | TEXT | 卡片bottom文案 |
| `support_lqt_turn_in` | INTEGER | (support_lqt_turn_in) |
| `support_lqt_turn_out` | INTEGER | supportlqttur编号ut |
...
| `minimch_system_state` | INTEGER | 最小imchsystem状态 |
...
| `is_show_notice` | INTEGER | 是否显示编号tice |
| `wording` | TEXT | 文案 |
| `left_icon` | TEXT | left图标 |
| `jump_url` | TEXT | jump链接 |
| `notice_id` | TEXT | 编号ticeID |
| `type` | INTEGER | 类型 |
...
| 字段 | 类型 | 中文备注 |
|---|---|---|
| `mNativeUrl` | TEXT | mn@ive链接 |
| `hbType` | INTEGER | hb类型 |
| `receiveAmount` | LONG | 推荐eive金额 |
| `receiveTime` | LONG | 推荐eive时间 |
| `receiveStatus` | INTEGER | 推荐eive状态 |
...
|---|---|---|
| `wallet_region` | INTEGER | wallet地区 |
| `wallet_grey_item_buf` | BLOB | walletgreyiteMBuf |
...
| `main_card_bind_serialno` | TEXT | main卡片bind序列编号 |
...
| `authen_account_type` | INTEGER | 授权enac数量类型 |
...
| `clearPkgTime` | LONG | clearpkg时间 |
| `checkIntervalTime` | LONG | checkinterval时间 |
| `packMethod` | INTEGER | p确认method |
| `domain` | TEXT | (domain) |
| `md5` | TEXT | MD5 |
| `downloadUrl` | TEXT | 下载链接 |
| `pkgSize` | INTEGER | pkg大小 |
...
| `preloadFilesReady` | INTEGER | preload文件s已读y |
| `preloadFilesAtomic` | INTEGER | preload文件s@omic |
| `autoDownloadCount` | INTEGER | auto下载数量 |
| `disable` | INTEGER | 禁用 |
...
| `realnamequanpin` | TEXT | real名称quanpin |
| `username` | TEXT | 用户名 |
| `nickname` | TEXT | 昵称 |
| `nicknamepyinitial` | TEXT | 昵称pyinitial |
| `nicknamequanpin` | TEXT | 昵称quanpin |
| `type` | INT | 类型 |
| `moblie` | TEXT | (moblie) |
| `email` | TEXT | 邮箱 |
| `status` | INT | 状态 |
...
| `forward_msg_talker` | TEXT | forward消息talker |

### `bizfansvideoinfoVideoHash`
...
### `bizphotofansvideoinfoVideoHash`
...
| `modifytime` | LONG | 修改时间 |
| `chatroomnotice` | TEXT | 聊天群编号tice |
| `xmlChatroomnotice` | TEXT | XML聊天群编号tice |
| `chatroomVersion` | INTEGER | 聊天群版本 |
| `chatroomnoticeEditor` | TEXT | 聊天群编号ticeeditor |
| `chatroomnoticePublishTime` | LONG | 聊天群编号ticepublish时间 |
| `chatroomNoticeNew` | INTEGER | 聊天群编号ticenew |
| `chatroomLocalVersion` | LONG | 聊天群本地版本 |
| `chatroomStatus` | INTEGER | 聊天群状态 |
| `memberCount` | INTEGER | 群成员数 |
| `chatroomfamilystatusmodifytime` | LONG | 聊天群family状态修改时间 |
| `associateOpenIMRoomName` | TEXT | associ@eopenim群名称 |
| `openIMRoomMigrateStatus` | INTEGER | openim群mig速率状态 |
...
| `localChatRoomWatchMembers` | BLOB | 本地聊天群w@ch成员s |
| `spamStatus` | INTEGER | spam状态 |
| `compactFlag` | LONG | compact标志 |
| `qrCodeAccessType` | INTEGER | qr码access类型 |
...
| `ConQQMBlog` | TEXT | conqqMBlog |
| `ConSMBlog` | TEXT | consMBlog |
| `DomainList` | TEXT | domain列表 |
| `reserved1` | INT | 保留1 |
| `reserved2` | INT | 保留2 |
| `reserved3` | INT | 保留3 |
...
| `fmsgIsHasShowSelfQRCode` | INTEGER | f消息ishas显示selfqr码 |
...
| `qqquanpin` | TEXT | (qqquanpin) |
| `qqremark` | TEXT | qq备注 |
| `qqremarkpyinitial` | TEXT | qq备注pyinitial |
| `qqremarkquanpin` | TEXT | qq备注quanpin |
| `reserved1` | TEXT | 保留1 |
| `reserved2` | TEXT | 保留2 |
| `reserved3` | INT | 保留3 |
| `reserved4` | INT | 保留4 |
...
| `content` | TEXT | 内容 |
| `msgType` | TEXT | 消息类型 |
| `customNotify` | TEXT | custom编号tify |
...
| 字段 | 类型 | 中文备注 |
|---|---|---|
| `shakeItemID` | INTEGER | shakeitemID |
| `username` | TEXT | 用户名 |
| `nickname` | TEXT | 昵称 |
| `province` | TEXT | 省 |
| `city` | TEXT | 城市 |
...
| `lvbuffer` | BLOB | 扩展二进制(protobuf) |
| `regionCode` | TEXT | 地区码 |
| `snsFlag` | INTEGER | sns标志 |
| `sns_bgurl` | TEXT | sns背景链接 |
...
### `public_account_profile_fts_config`
...
### `public_account_profile_fts_data`
...
### `public_account_profile_fts_docsize`
...
### `public_account_profile_fts_idx`
...
### `doubt_group_notify_lastest_update_seq`
...
### `group_detail_info_ver1_fts_config`
...
### `group_detail_info_ver1_fts_data`
...
### `group_detail_info_ver1_fts_docsize`
...
### `group_detail_info_ver1_fts_idx`
...
### `group_member_new_ext_info_table`
...
### `group_notify_lastest_update_seq`
...
| `silent` | INTEGER | 静音 |
| `silent_normal_only` | INTEGER | 静音编号rmalonly |
| `mode` | INTEGER | 模式 |
| `ext` | TEXT | 扩展 |
...
| `badge_count` | INTEGER | badge数量 |
| `read_badge_count` | INTEGER | 已读badge数量 |
