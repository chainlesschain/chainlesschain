...
## 31. e2e server-readiness timeout 在冷启动高负载下提前到期 —— 静默空输出级联 + per-test/child timeout 倒挂（隐性风险）

**Slug**: `cli_e2e_server_readiness_timeout`（实战触发 2026-06-15）

**陷阱本质**：

CLI e2e 用 `singleFork`（所有 e2e 串到一个进程里跑）。某测试 `beforeAll` 里 `spawn` 一个子服务器（`cc ui` / coding-agent WS bridge），再用一个 readiness 等待器轮询/解析 stdout，**等到一个固定 fallback 超时**为止。两个独立的坑：

1. **fallback 太短 + 静默兜底**：`startUiServer` 的 fallback 是 8s/10s，但 `cc ui` 冷启动要加载整个 CLI bundle + DB init——本机独立跑 ~3.3s，可一旦在 singleFork 满负载下（前面一长串 e2e 把 CPU 占满，子进程冷启动被拖慢），轻松 >10s。fallback 到期后 helper **`resolve` 出空 output**（而非 reject），于是 `expect(startOutput).toContain(url)` 报 `expected '' to contain 'http://…'`——**根因（启动慢）被翻译成一个看起来毫不相干的断言失败**，且会**级联**：同 describe 块后续所有依赖该 server 的测试一起红（本案一个 8s fallback 砸了 13 个测试）。
2. **timeout 倒挂**：`it(..., 20000)` 的测试预算 < 它内部 `execSync(cmd, {timeout: 30000})` 子命令自己的超时。vitest 在 20s 杀掉测试，**子命令自己的 30s timeout-kill 路径根本没机会跑**——测的"优雅退出"逻辑永远观测不到，只看到 `Test timed out in 20000ms`。

**为什么排查难**：

- 报错文本（`expected '' to contain URL` / `connect ECONNREFUSED` / `Test timed out`）指向断言/网络/超时，**都不是真因**（真因是冷启动慢 + 等待预算太紧）。容易误判成"server 启动 bug"去翻产品代码。
- 高度**负载相关**：单独重跑这几个文件全绿（机器空了，冷启动回到 3s），全量 e2e 跑就红。`--retry=2` 也救不了——负载是持续的，3 次重试都在同一个满负载窗口里超时。
- 判据：标准输出里**整条 run 异常慢**（本案连 `cc --version` 都要 ~10s，正常 <1s）= 机器在抖，几乎所有失败都是这一类，不是产品 bug。先单独重跑确认，再动手。

**SOP / checklist**：

1. 看失败 timing：2ms 的断言失败（空 output）+ 同块批量红 = readiness fallback 到期级联；xx000ms 整 = 撞 testTimeout。
2. **单独重跑该文件**确认产品没坏（`npx vitest run --config vitest.e2e.config.js <file>`）；绿 = 负载 flake。
...
- ❌ readiness 到期 `resolve(空 output)`（静默兜底）→ 把"没起来"伪装成"起来了但内容不对"，断言失败完全误导。应当 reject，或至少让失败信息携带"server 未在 N s 内就绪"。
- ❌ 看到 `ECONNREFUSED` 就去查端口/防火墙/server bind——满负载下它只是 server 还没起来。
- ❌ 把 per-test timeout 设得比内部子命令 timeout 还短（倒挂）。
- ❌ 给 flaky e2e 无脑加 `--retry`——持续负载下重试只是多烧几遍时间，治标不治本；先把预算调到能扛冷启动。

**关联 memory / trap**：

- trap #23（bs3mc/bs3 ABI dual-load，desktop vitest Electron-ABI gotcha，同属"e2e/测试环境而非产品 bug"家族）
- memory: `cli_subprocess_test_db_isolation`（e2e 共享 helper `__tests__/e2e/_helpers/cli-e2e.js`）、`cli_loop_command`（CI 治理 Layer1+3：默认 config exclude e2e + e2e shard `--retry=2`）
- 实战 commit：`26a811874`（ui-command/web-panel/coding-agent-envelope/mtc-audit/orchestrate 五处，89/89 验证）。注意区别于 README `2026-06-14` 的"24 个 e2e 文件**子进程** timeout 15s→30s"——那条调的是 `execSync` 子命令超时，本 trap 调的是**server-readiness 等待器 + per-test 预算**，两条正交。
