...
| Plugin / Marketplace      | A/B：多来源 catalog、单 registry 执行 authority、update impact 与显式来源切换已实现                               | 缺多 registry 统一选择、真实 private registry、组织签名撤销、代理/离线与故障注入矩阵                                                            | P1/P2           |
...
catalog 的冲突/优先级变成统一候选选择，也没有完成 license/SBOM/signature 实物回读、组织 key revocation、
真实 private registry、代理/离线及供应链故障注入矩阵。
...
| 17  | P1-5 / R5            | 部分完成（catalog、执行绑定与 update impact 子门已实现）       | `392ed39d24` 提供版本化多来源 catalog；`b93f354679` 让 registry add/upgrade 在 clone 前预检并持久 exact catalog authority；`07f4d41fa6` 又增加 immutable-install→candidate 的 version/source/integrity/license/capability/dependency diff、stable impact digest，以及来源切换/降级的显式批准和 exact-digest upgrade 绑定。仍需多 registry 统一候选选择、private registry、组织签名撤销、代理/离线及供应链故障矩阵。                                                                                                                                                                                                                                                                                                                                                                 |
...
| 11  | P1-5：Marketplace 发现与组织治理               | 部分完成                         | `392ed39d24` 新增多 registry catalog 与有界依赖/健康图；`b93f354679` 新增 install preflight、fetched identity 校验和 immutable catalog provenance；`07f4d41fa6` 新增 update-impact contract/命令、candidate/content/impact digest，upgrade 对来源切换、降级和 stale impact fail closed，并持久批准过的 diff authority；registry assertions 始终不冒充 install/load crypto verification                                                                                                                                                                                                                             | 完成多 registry 统一候选选择、license/SBOM/signature 实物回读、真实 private registry、组织 key revocation、代理/离线，以及依赖冲突和供应链故障注入矩阵                                                                                                                        |
