...
- ChainlessChain 候选版本基线：CLI `0.162.194` release candidate（`0.162.190` / `0.162.191` / `0.162.192` 未发布；`0.162.193` 被非权威通用 workflow 发布，npm `latest` 为 `0.162.193`）、VS Code `0.37.41`、JetBrains `0.4.79`
...
| P0-4 / Q4a / R3a         | 真实宿主矩阵已运行并产出 fail-closed 工件；macOS current stable DOM 门仍阻断      | `350ef5601e` 的 IDE run `30947671847` 已证明固定命令 driver、已安装 VSIX、17 个命令和 loopback bridge 实际运行。`61f8235105` 的 `30960237759` 首次让 macOS minimum `1.85.2` 完整通过 initial 的 stream/retry/plan/permission/interrupt 与 restart-resume。已结算候选 SHA `f72dc01c4f` 的 IDE Extensions `30960881338` 再次确认：capability、Windows VS Code、JetBrains 2024.2/2025.2 × 三 OS 六格与 JetBrains build 全部成功；macOS minimum `1.85.2` 完整通过，但 current stable `1.131.0` 仍 fail-closed。候选 `0.37.41` 本地扩展/协议/VSIX 矩阵通过                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | macOS job `92164590186` 的 stable 阶段仍因 45 秒内没有 Webview ready/protocol 消息失败；即使 `--disable-gpu` 软件渲染也未改变结果，minimum 成功不能替代 current stable。依赖 macOS job success 的最终 VSIX package/publish job `92165638115` 按设计跳过；Remote/SSH/WSL/devcontainer、多根/多窗口、网络抖动、fresh-profile 升降级、ARM64 与 8 小时 soak 仍待完成。restricted 二进制工件不宣称文本级脱敏             |
| Q4b / R3b                | 未完成，发布保持 fail-closed                                                      | 已结算候选 SHA `f72dc01c4f` 的 CLI CI `30960881488` 与 CLI Strict Sandbox `30960888570` 均成功；同 SHA IDE Extensions `30960881338` 中 Windows VS Code、macOS minimum、JetBrains required matrix 与 build 成功，但 macOS current stable 失败，最终 VSIX package/publish 跳过。CLI 双门成功不能越过失败的 required IDE gate                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | 最终 release SHA 仍须同时通过完整 `CLI CI`、`CLI Strict Sandbox`、IDE required journeys 与受影响组件门，并生成绑定该 SHA 的 immutable tarball/SBOM；还缺签名/notarization/Authenticode、x64/ARM64、安装/升级/回滚、Microsoft Marketplace/Open VSX 与发行资产公开回读。当前不得打 tag 或 publish                                                                                                                     |
| P1 / R4 与 P2 / R5       | 三批命令生命周期迁移与会话预算 foundation 已提交；完整 R4/R5 未完成               | `c50d2f8a53`、`1f2a9caf3d` 与 `56c87fa5d0` 完成三批长尾迁移：25 个旧顶层入口保留兼容别名并收敛到虚拟 `cc lab ...`，至少保留两个 release cycle；注册图仍为 175、净增长 0，推荐顶层命令从 166 降至 151，manifest、README、namespace help 与四种 shell completion 同源生成。核心 lifecycle/lazy/completion 39/39，扩大命令/文档矩阵 103/103。`008335171f` 至 `6b4570c80f` 的预算 foundation、后台与 TeamRunner 定向矩阵继续保持通过                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | 命令面迁移已完成计划中的三批，但仍需按弃用周期观测使用并决定后续移除；预算能力仍只是 foundation/local adapters，生产 root、全部 turn/token/tool 与 WS/REPL/headless 入口尚未统一 authority。Dynamic Workflow、Execution Location、Automation、Context/Permission Center、Marketplace 治理、真实长会话与 1 GiB 验收仍待推进；不得宣称 R4/R5 产品闭环或据此发布                                                       |

上述提交已推送到 GitHub 与 Gitee，部分 exact SHA 也已产生组件级 CI 结果；代码交付和验证证据都不等同于 Microsoft Marketplace 发布、真实 PR/merge 或产品 release。`f72dc01c4f` 已在同一 exact SHA 取得完整 `CLI CI` 与 `CLI Strict Sandbox` 成功，但 required IDE matrix 因 macOS current stable 失败而未关闭；任何状态升级仍须绑定同一精确提交、完整运行矩阵和可回读 artifact。
...
- `b86ea54468` 的 IDE run `30964554075` 证明固定 stable `1.130.0` 仍失败、minimum `1.85.2` 再次完整通过，因此不采用 host pin。下一候选恢复 current stable 门，删除会让宿主更早停滞的 `--disable-gpu`，并使用 VS Code 自动化自身采用的 `--use-inmemory-secretstorage`，避免 fresh macOS CI profile 在 Webview 启动前阻塞于无界面的 Keychain；新的 exact SHA 在 current stable 与 minimum 两条 journey 都通过前仍为 NO-GO。

最终判定保持 **release NO-GO**，直到上述候选在 current stable 与 minimum `1.85.2` 上都通过。P0-4/Q4a 不把激活级检查包装成 DOM 成功，也不以 minimum 成功替代 stable；最终 release SHA 仍须重跑两条 journey。Q4b 还要求该 SHA 同时通过完整 CLI CI、CLI Strict Sandbox、Windows/macOS/Linux VS Code、JetBrains required matrix 与受影响组件门，并生成 immutable tarball/SBOM、签名及 fresh-profile 安装/升级/回滚证据。仓库 secret 回读已确认存在 `OVSX_PAT`、但缺少 `VSCE_PAT`，Microsoft Marketplace/Open VSX 也没有 `0.37.41` 的公开 exact-version/digest 回读；npm `latest` 仍是非权威流程发布的 `0.162.193`。因此在门禁完成前不得创建 release tag、不得 publish，也不得宣称路线图产品范围已经全部交付。
