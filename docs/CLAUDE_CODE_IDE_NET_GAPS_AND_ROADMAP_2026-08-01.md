...
### 2026-08-02 canonical 宿主、流式恢复与原生事务增量

- `63a67cc676` 把 REPL canonical JSONL 校验提前到 feature/config 等宿主访问之前：已验证记录优先，存在但未验证的记录不得回退到 DB 或新会话；主工作区与隔离提交切片分别通过 REPL 87/87、宿主门 5/5、JSONL/MCP 相邻集 99/99。`9cbe020b08` 随后提交 forward verified projection、sidecar head/count anchor、checkpoint 后缀消息重建、增量 MCP reducer、WS turn lifecycle 投影和导出/index 兼容；核心矩阵 139/139。该路径移除了恢复时必须整体 materialize 全部 event 的堆占用，但普通 hash-chained JSONL 仍需 O(N) 前向认证，sidecar 仍不是独立 anti-rollback anchor。
- `213c3ae7c5` 将宿主恢复进一步收敛到同一 verified sample：Stream 的消息历史与 MCP recovery 不再分别读取，任何 transcript/sidecar/同步 capability 异常都会在 config/settings、Setup Hook、bootstrap、输入消费、MCP、模型、工具和 user append 之前返回 content-free terminal error；WS 对 canonical session 在模型/工具前持久化 request claim，以 opaque claim capability 结算 success/failure，相同 request id 跨进程只允许一个 owner。owner 崩溃后的 claim 保持 pending，不自动接管或重放；需要显式裁决或新 request id。恢复告警固定在 leading system prefix，canonical history refresh 不再把副作用/MCP “勿重放”提示丢掉。
- `213c3ae7c5` 提交前，本地完整 Stream 回归为 19 files / 228 tests，WS/恢复门相关回归为 3 files / 53 tests；`session-host-consistency-gate.mjs` 本体在 Windows/Node 22.22.2 通过 verified host agreement、WS atomic turn、双进程 claim race、claim-owner crash、model-period forged settlement 与 tamper refusal。tamper fixture 明确验证 REPL、单次 Headless、Stream、authenticated background attach 与 WS 均拒绝，且 Stream 的 input/bootstrap/Hook/MCP/model/append 调用数均为 0。该门仍只授权其声明范围：没有 general cross-process session lease、独立 anti-rollback anchor、1 GiB cold-process P95/RSS、fsync/断电或 remote-host durability 证明；WS claim fencing 也只覆盖 canonical JSONL resume session，不外推到 legacy/create path。
- `2a85acb901` 与 `be86097be2` 收紧持久会话预算 authority：sidecar 绑定 platform/parent、opaque authority id 与 async observer context；最终 close 改为 persist-first，失败时保留 durable dirty recovery、撤销本地能力并阻止旧 handle/lease 继续 mutation，成功时才归零引用并删除 registry。预算定向为 56 passed / 3 skipped，六文件 adapter 矩阵为 112 passed / 3 skipped。它仍不是 machine-wide scheduler，也没有覆盖所有生产 root/turn/token/tool 入口。
- `b95424dc02` 与 `8d11082f71` 以 fd-bound staging、无覆盖发布、tombstone re-bind、priorAlias identity 和 recovery retirement guard 加固 POSIX installer；`4145508010` 明确区分“公共路径已耐久恢复”与“私有 tombstone 清理降级”，不会把后者误报为语义 rollback 失败。bash/sh/dash 语法、Node 语法和 scoped diff 均通过；当前 Windows 可运行 native 集为 6 passed / 44 个 POSIX 动态用例按平台跳过。Linux/macOS 动态故障注入、真实 SIGKILL/断电、ARM64、签名/notarization/Authenticode 和公开资产回读仍必须由 exact-SHA CI/实机证据关闭。
- `9cbe020b08` 的 `CLI Strict Sandbox` run `30732462022` 已在 Ubuntu、macOS、Windows 三个 job 全绿；同 SHA 的 `CLI CI` run `30732462105` 最终为 cancelled，且取消前 Windows unit shard 2/4 已因 `session-budget-runtime.test.js` 的 `committed sidecar identity mismatch` 出现 32 项失败，因此不是双门成功。`213c3ae7c5` 的 exact-SHA `CLI CI` run `30733555516`、`CLI Strict Sandbox` run `30733555412` 与 `CLI Session Host Consistency` run `30733555422` 在本快照时仍 queued/running、尚无失败；未完成矩阵不构成发布授权。最终待发布 SHA 仍必须同时取得完整 `CLI CI` 与 `CLI Strict Sandbox` 三 OS 矩阵成功，任何 queued/running、失败、取消、部分 matrix 或旧 SHA 结果均不构成发布授权。
