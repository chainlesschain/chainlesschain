...
- `223c0f505c` 的 Strict Sandbox run `30715641925`、`b6a2c096ea` 的 run `30716039185` 和 `5df2e1bdac` 的 run `30724810078` 均在三 OS 全绿；`5df2e1bdac` 已覆盖完整 MCP recovery/adjudication 生产与测试矩阵。`925a49fb7b` 的 Strict Sandbox run `30725436007` 在 Ubuntu 24.04、macOS 15、Windows 全绿，但 `CLI CI` run `30725436119` 被取消；`963c6d46dd` 的 Strict Sandbox run `30725805331` 也在三 OS 全绿，但 `CLI CI` run `30725805407` 被取消。它们仍只能授权各自 SHA 的 Strict Sandbox，当前没有更晚待发布 exact SHA 上同时成功的完整 `CLI CI` + `CLI Strict Sandbox`。
...
- `008335171f` 提交 session resource budget primitive 与 SubAgent 本地 adapter；`f9c3a7d258` 提交后台 cleanup 基础；`65796e6ec6` 保持后台 lease 直到子进程实际退出；`9611afb8c8` 校验 usage aggregate/details 一致性；`6383e66201` 为 TeamRunner 增加 fence 与 scoped authority；`6b4570c80f` 把相关矩阵纳入 Strict Sandbox gate。预算基础相关测试 185/185、后台最终矩阵 44/44、TeamRunner 55/55、Strict 本地单 worker 8 文件 250/250 通过。
- 当前状态仍是 **foundation/local adapters**。生产 root 尚未创建统一 authority，root turn、token、tool 也未统一计量。工作树中未提交的 runtime 候选仍有 warm CAS 恢复后首动作可能漏过 admission/fence、sidecar 缺少独立 anti-rollback anchor、host snapshot 缺少 head lease 且仍执行 O(N) 全量读取等 NO-GO；不得标记“统一全会话预算”或“真实长会话完成”。`6b4570c80f` 的 Strict Sandbox run `30727179738` 已在 Ubuntu 24.04、macOS 15、Windows 全绿；同 SHA 的 `CLI CI` run `30727179832` 仍在运行，单独的 Strict 成功不构成发布授权。最终待发布 SHA 的 `CLI CI` 与 `CLI Strict Sandbox` 全矩阵同时成功前不得发布。
