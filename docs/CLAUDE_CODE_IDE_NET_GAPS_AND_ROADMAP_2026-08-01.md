...
| Plugin / Marketplace      | A/B：供应链和治理强；版本化多来源 catalog、依赖/license/健康投影已实现                                            | catalog 尚未绑定安装执行；缺真实 private registry、组织签名撤销、代理/离线与故障注入矩阵                                                        | P1/P2           |
...
该子切片只关闭“安装前多来源可审阅治理投影”，不关闭 P1-5 整项：`add/upgrade` 尚未强制消费 catalog authority，
也没有完成安装前/更新影响 diff、来源切换事务、license/SBOM artifact 回读、组织 key revocation、真实 private
registry、代理/离线及供应链故障注入矩阵。
...
| 17  | P1-5 / R5            | 部分完成（多来源治理 catalog 子门已实现）                      | `392ed39d24` 已提供版本化多来源 catalog、候选 digest/signature/SBOM/license/capability、依赖/兼容性/健康图和 strict fail-closed 投影；仍需把 authority 强制接入 add/upgrade，完成安装前/更新影响 diff、来源切换事务、private registry、组织签名撤销、代理/离线及供应链故障矩阵。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| 11  | P1-5：Marketplace 发现与组织治理               | 部分完成                         | 签名、SBOM、策略、升级恢复与供应链治理底座较强；`392ed39d24` 新增 `cc-plugin-marketplace-catalog/v1` 与多 registry catalog，统一展示候选 digest/signature/SBOM/license/capability、来源健康、宿主兼容性和有界依赖图，strict 模式对不可用来源、元数据缺失、依赖/兼容性问题及来源冲突 fail closed；registry assertions 不冒充 install/load 验证                                                                                                                                                                                                                                                                      | 将 catalog authority 强制接入 add/upgrade，完成安装前与更新影响 diff、来源切换/回滚 artifact、license/SBOM/signature 实物回读、真实 private registry、组织 key revocation、代理/离线，以及依赖冲突和供应链故障注入矩阵                                                        |
