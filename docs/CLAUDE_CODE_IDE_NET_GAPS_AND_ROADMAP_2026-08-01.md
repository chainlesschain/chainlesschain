...
| Plugin / Marketplace      | A/B：供应链和治理强；版本化多来源 catalog 及单 registry add/upgrade authority 已实现                              | 缺跨 registry 来源选择/切换、真实 private registry、组织签名撤销、代理/离线与故障注入矩阵                                                       | P1/P2           |
...
这两个提交关闭可读预检、版本化保存和单机 exact replay 子门，但 run history 写入仍为 best-effort；运行中
...
上述子切片仍不关闭 P1-5 整项：`add/upgrade` 当前绑定的是用户已选择的单一 registry 候选，尚未把多 registry
冲突/优先级变成统一来源选择和切换事务，也没有完成安装前/更新影响 diff、license/SBOM/signature 实物回读、
组织 key revocation、真实 private registry、代理/离线及供应链故障注入矩阵。
...
| 11  | P1-1 / R4            | 部分完成（definition/preflight/persistence/replay 子门已实现） | `58bfdceb84` 在现有 Cowork DAG 上增加版本化 definition/digest、raw plan 与 fail-closed capability/location/permission/sandbox/data-boundary/credential/规模/成本预检；`b6dc772384` 增加 current envelope、content-addressed immutable versions、legacy-unversioned 边界、CLI/WS exact-digest replay 与 run history digest。仍需生成式编排、运行中 pause/resume、恢复后阶段最多一次、耐久 history 与 artifact/ledger/checkpoint lineage、Workbench/双 IDE 消费、跨 host 矩阵和 plugin/marketplace 分发。                                                                                                                                                                                                                                                                    |
...
| 17  | P1-5 / R5            | 部分完成（catalog 与单 registry 执行绑定子门已实现）           | `392ed39d24` 已提供版本化多来源 catalog、候选 digest/signature/SBOM/license/capability、依赖/兼容性/健康图和 strict fail-closed 投影；`b93f354679` 又让 registry add/upgrade 在 clone 前消费候选预检、校验 fetched identity，并把 exact catalog authority 写入 immutable install provenance。仍需多 registry 来源选择/切换、安装前/更新影响 diff、private registry、组织签名撤销、代理/离线及供应链故障矩阵。                                                                                                                                                                                                                                                                                                                                                              |
...
| --- | ---------------------------------------------- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | S0-1：Plan、权限与运行时正确性                 | 部分完成；P0 release gate        | Plan/contract/Hook/子树指令、durable host revocation、MCP send-time admission、后台 interaction journal CAS、delegated/recovery lease、精确进程身份、独立 machine-local anti-rollback witness、covered-scope 磁盘故障恢复与 canonical session SIGKILL/fsync v1 均已进入 `main`；PR #182 exact-head 双门、PR #196 三平台 CLI CI、Session Scale、Host Consistency 及后续发布门已通过                                                                                                                                                                                                                 | 关闭已 dispatch 外部 effect 无法召回的分布式撤权/人工裁决边界、真实任意物理断电与跨设备 fsync、首次 state-root 逐级目录持久化、同时回滚 transcript 与机器外部 witness 的同 UID 强篡改者、跨宿主 authority 及长期安全矩阵；任何 authority-bearing 失败均不得扩大能力                    |
...
| 7   | Q4b：完整发布与用户旅程门                      | 部分完成；外部阻塞/本期延后      | CLI npm `0.163.8` exact-SHA 发布和公网回读、Open VSX `0.37.53` 与 JetBrains `0.4.89` 公开回读、unsigned CLI 六目标执行已完成                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | 完成 Microsoft Marketplace、JetBrains 作者签名、Desktop/native x64+ARM64 签名与公证、公开渠道 fresh-profile 升降级/回滚、网络抖动/重连/Bridge 与 CLI restart、8 小时 IDE soak 和 nightly live-provider trajectory                                                          |
| 8   | P1-1：Dynamic Workflow façade                  | 部分完成                         | 已有 Cowork DAG、run history、Team/Batch 和统一 scheduler/Automation 控制基础；`58bfdceb84` 增加版本化 definition/digest、raw plan、最坏任务调用与 token/USD/DAG 耗时投影，以及 capability/location/permission/sandbox/network/data-boundary/credential 的 fail-closed preflight；16 soft/64 hard 规模门不可静默放宽。`b6dc772384` 又增加 current envelope、content-addressed immutable versions、legacy-unversioned 边界、CLI/WS exact-digest replay 和 run history digest                                                                                                                                        | 完成自然语言生成与审阅、运行中 pause/resume/stop/restart、恢复后阶段最多一次、耐久 history 与 artifact/ledger/checkpoint lineage、Workbench/双 IDE 消费、跨 host 矩阵和 plugin/marketplace 分发；当前 manifest 明确不承诺 durable resume、exactly-once 或阶段间 `needs_input` |
...
| 11  | P1-5：Marketplace 发现与组织治理               | 部分完成                         | 签名、SBOM、策略、升级恢复与供应链治理底座较强；`392ed39d24` 新增 `cc-plugin-marketplace-catalog/v1` 与多 registry catalog，统一展示候选 digest/signature/SBOM/license/capability、来源健康、宿主兼容性和有界依赖图；`b93f354679` 新增 install preflight，registry add/upgrade 在 clone 前阻断硬问题、校验 fetched identity、持久 exact catalog authority，同版本无 authority 必须 `--force` 重验，hash-only 不冒充 signed lock                                                                                                                                                                                    | 完成多 registry 来源选择、安装前与更新影响 diff、来源切换/回滚 artifact、license/SBOM/signature 实物回读、真实 private registry、组织 key revocation、代理/离线，以及依赖冲突和供应链故障注入矩阵                                                                             |
