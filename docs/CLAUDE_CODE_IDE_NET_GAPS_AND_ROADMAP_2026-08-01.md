...
- `0ab774f1b7` 增加 operation-local restore intent，`daa4a63f58` 暴露 immutable restore target identity，`ec31d6410b` 提供 shared durable restore orchestrator foundation；`70d24d76e7` 才把传统 direct `cc checkpoint restore` 接入该 orchestrator/saga，`fb03baf466` 保留可验证的 durable session intent。`a3755f0505` 进一步把 timeline 的 `restore-code` / `restore-both` 迁入同一 `plan → locked revalidate → restore → withSessionAuthority` 编排，并保持 conversation-only action 不走 workspace restore。至此 direct 与 timeline code restore 已统一使用 saga；不能再把 timeline 统一错误归到 `ec31d6410b`，也不能把统一接线等同于恢复闭环完成。
- `87edbd5be2` 落地 verified session recovery read-model，其精确分类是 `no-session/direct`、`clean-abort`、`code-settlement-resumable`、`both-settlement-resumable`、`already-completed` 与 `conflict/unknown`；`b1fdc986c2` 只提供 pre-intent abort / terminal release 的保守 controller，`19cf060030` 已公开 `checkpoint recovery list/show/abort/release`。`7d79b0941c` 新增的则是 **already-completed reconciliation foundation**：只在 verified session 已完成且 workspace target/owner/fence 精确匹配时收口 saga，不重放 workspace 或 conversation mutation；它不是通用 resume，也尚未接入公开 `checkpoint recovery resume` 命令。
- `1c1df20cc7` 已加入真实 Git/copy 的 **terminal kill matrix**：子进程在 saga `completed` 且 workspace lock 尚未释放的窗口被 SIGKILL，重启后验证 terminal saga/target/retained owner，拒绝错误 owner digest，并以精确 fence 完成 release/archive。该矩阵只关闭“terminal completed → restart → release”窗口，不覆盖 mutation-phase kill。partial-mutation rollback、通用 rollback，以及 already-completed controller 的 public resume wiring 仍未完成（public wiring 当前进行中），因此仍不得宣称通用 resume/rollback 或端到端恢复已经完成。
- 截至 2026-08-03，本地 `packages/cli/package.json` 与 npm latest 均仍为 `0.162.189`，尚未修改版本号、changelog、tag 或 npm 发布状态。结论保持：**功能量值得准备 `0.162.190`，当前继续 release NO-GO，不要发布**。当前 HEAD 尚无最终精确 SHA 的 `CLI CI` 与 `CLI Strict Sandbox` 三 OS 全绿证据，本批 checkpoint 提交也不关闭本文其余 P0/P1/P2 blockers；只有补齐 public resume wiring、partial-mutation rollback/相应强杀矩阵，并让独立版本与 changelog 所在的**最终精确 SHA**通过上述双门，同时通过 Background Interaction、Session Host 与受影响的 checkpoint/scale 门，才允许打 immutable tag 和发布 npm。
