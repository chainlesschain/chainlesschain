...
| Plugin / Marketplace      | A/B：catalog、统一选择、执行/impact authority 与本地 artifact evidence 已实现                                     | 缺真实 private registry、组织签名撤销、远端 artifact 抓取/验证、代理/离线与故障注入矩阵                                                         | P1/P2           |
...
的 `cc-plugin-marketplace-payload-sbom/v1`。可比字段输出 exact match/mismatch，CycloneDX 等未抓取远端 SBOM 明确是
...
上述子切片仍不关闭 P1-5 整项：registry publisher identity 和远端 signature/SBOM artifact 均未由本地 readback
验证，尚未完成组织 key revocation、真实 private registry、代理/离线及供应链故障注入矩阵。
...
| 17  | P1-5 / R5            | 部分完成（catalog、选择、执行/impact 与本地 evidence 已实现）  | `392ed39d24` 提供多来源 catalog；`b93f354679` 持久 install authority；`07f4d41fa6` 增加 update impact；`6bd8c11271` 增加 exact registry-set selection；`3d171256fb` 又把 registry expectations 与 immutable manifest/license、实际 Ed25519/signature-bound SBOM 及有界 payload SBOM 做稳定回读，精确区分 matched/mismatch/partial。仍需 private registry、组织签名撤销、远端 artifact 抓取/验证、代理/离线及供应链故障矩阵。                                                                                                                                                                                                                                                                                                                                 |
...
| 11  | P1-5：Marketplace 发现与组织治理               | 部分完成                         | `392ed39d24` 新增 catalog；`b93f354679` 新增 preflight/provenance；`07f4d41fa6` 新增 update impact；`6bd8c11271` 新增 exact registry-set selection；`3d171256fb` 新增 artifact-readback contract/命令，把 persisted registry expectations 与 immutable manifest/license、实际签名与有界 payload SBOM 比较，tamper fail closed，未抓取远端 SBOM 保持 partial；registry assertions 始终不冒充 publisher/remote-artifact verification                                                                                                                               | 完成远端 signature/SBOM artifact 抓取与验证、真实 private registry、组织 key revocation、代理/离线，以及依赖冲突和供应链故障注入矩阵                                                                                                                                           |
