...
- Windows Server 2025 / Node 22.12 / libuv 1.49.1 的剩余误判已在独立分支提交为 `db2524fef0`：path↔path 与 fd↔fd 继续全字段精确，path↔fd 只通过受信 volume/share-root device 的兼容比较；55/55 定向回归、Prettier、语法与 diff 检查通过。该提交尚未集成主线；并行 checkpoint saga 新增的 `readBoundedPlainFile`、`inspectPlannedRegularFile` 与 `inspectTarget` 三个入口必须一并接线和复验，不能把独立分支结果外推为主线通过。
...
- 当前 package 与 npm latest 仍为 `0.162.189`，尚未修改版本号、changelog、tag 或 npm 发布状态。建议进入 `0.162.190` 发布准备，但只有在 checkpoint saga 与 Windows identity 修复收口提交后，创建独立版本/changelog release commit，并让该**最终精确 SHA**的 `CLI CI` 与 `CLI Strict Sandbox` 在 Linux/Windows/macOS 全绿；建议同时要求 Background Interaction、Session Host 与受影响的 checkpoint/scale 门通过，之后才允许打 immutable tag 和发布 npm。
- `4ddb5c9c98` 已把上述 Windows path↔fd identity 修复与 copy restore hard-kill safety 合入主线。`non-authoritative-trash/v1` 让 rename 后保留原 workspace ACL 的隔离文件退出恢复 authority；真实 Windows `Everyone:F` + rename 后强杀 + fresh-process 篡改/恢复用例通过，组合定向为 78 passed / 1 skip，独立终审 P0/P1/P2 均为 0。该提交的 `CLI CI` run `30760248218` 已触发但尚未形成完整终态；durable saga 仍未接入 restore 命令，因此发布判断继续是：**值得准备 `0.162.190`，现在不要发布**。
