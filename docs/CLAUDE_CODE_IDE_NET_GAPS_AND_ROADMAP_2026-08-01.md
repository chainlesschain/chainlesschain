...
配置放宽的 64 硬上限；`facade.requirements.credentials` 内出现 credential value/API key 时拒绝生成 manifest。该提交只关闭可读版本化投影和启动前有界预检
子门；`saveWorkflow()` 仍保存原始 JSON，run history 仍是 best-effort，运行中 pause/resume、阶段最多一次提交、
artifact/ledger/checkpoint lineage、Workbench/双 IDE 消费和 plugin/marketplace 分发仍未完成，因此 P1-1 整体保持
部分完成。
...
| 11  | P1-1 / R4            | 部分完成（definition/preflight 子门已实现）     | `58bfdceb84` 在现有 Cowork DAG 上增加版本化 definition/digest、raw plan 与 fail-closed capability/location/permission/sandbox/data-boundary/credential/规模/成本预检；仍需生成式编排、运行中 pause/resume、恢复后阶段最多一次、耐久 history 与 artifact/ledger/checkpoint lineage、Workbench/双 IDE 消费、版本化保存和 plugin/marketplace 分发。                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| 8   | P1-1：Dynamic Workflow façade                  | 部分完成                         | 已有 Cowork DAG、run history、Team/Batch 和统一 scheduler/Automation 控制基础；`58bfdceb84` 增加版本化 definition/digest、raw plan、最坏任务调用与 token/USD/DAG 耗时投影，以及 capability/location/permission/sandbox/network/data-boundary/credential 的 fail-closed preflight；16 soft/64 hard 规模门不可静默放宽                                                                                                                                                                                                                                                                                               | 完成自然语言生成与审阅、运行中 pause/resume/stop/restart、恢复后阶段最多一次、耐久 history 与 artifact/ledger/checkpoint lineage、Workbench/双 IDE 消费、版本化持久保存和 plugin/marketplace 分发；当前 manifest 明确不承诺 durable resume、exactly-once 或阶段间 `needs_input` |
