...
- **双 IDE 发布已执行，并分别保留真实渠道状态。** PR #191 的 exact merge SHA
  `66b26822c1a35fcf11afdb069cba187dbcb4d261` 通过 15/15 workflows；不可变 tag
  `ide-vscode-v0.37.53` 触发的
  [Open VSX 发布 run `31797960019`](https://github.com/chainlesschain/chainlesschain/actions/runs/31797960019)
  成功，官方公开回读确认 `0.37.53` 为 latest、listed 且可下载。不可变 tag `ide-jetbrains-v0.4.89`
  触发的 [JetBrains 发布 run `31797973047`](https://github.com/chainlesschain/chainlesschain/actions/runs/31797973047)
  也成功，六格真实宿主、build、Plugin Verifier 与 upload 均通过；随后官方 Marketplace API 已确认
  `0.4.89` 为 `approve=true`、`listed=true`、`hidden=false`，因此该版本现已公开发布。Microsoft Marketplace
  未配置发布 authority，本轮未发布。两端
  changelog/README 均说明 CLI-owned merge-review。公开 CLI `0.163.8` 已由不可变 tag `v-npm-0-163-8`
  发布，[npm publish run `31806101423`](https://github.com/chainlesschain/chainlesschain/actions/runs/31806101423)
  与[独立公开回读 run `31807574517`](https://github.com/chainlesschain/chainlesschain/actions/runs/31807574517)
  成功；该版本包含 governed Automation recovery、`cc team merge-review` 与只读 MCP resource templates。
  同一 release SHA 在发布前的 CLI CI `31804468633`、CLI Strict Sandbox `31804468464` 已成功；tag 后重复触发的
  CLI CI `31806101603` 因 Ubuntu scheduler-soak resource bound 失败，仍保留为红灯证据，不能反写为全绿。
...
[PR #191](https://github.com/chainlesschain/chainlesschain/pull/191) 已以 exact merge SHA
`66b26822c1a35fcf11afdb069cba187dbcb4d261` 进入 `main`；最终 checks 为 **114 success / 8 skipped / 0 failed**，
其中 15/15 workflows 与 6/6 required checks 全绿。因此本节的“完成”、下方
**14/19 尚未关闭、5/19 完成**以及 **14 个剩余工作包**已经生效；跳过项是按条件不适用的发布分支，
不用于替代 required checks。

### 2026-08-14 S0-1 canonical session authority crash-consistency v1 候选

- **持久化顺序已收敛到明确边界。** canonical transcript 在 file fsync（新文件于 POSIX 再做 parent-dir
  fsync）后才允许 sidecar 前进；meta 与 tombstone 改为同目录 temp→file fsync→atomic rename→POSIX
  parent-dir fsync，live successor 删除 tombstone 后也持久化目录项。activity journal 继续明确为可重建、
  非 authority 的排序提示。
- **repair、fork 与 delete 不再越过未刷盘状态。** 尾部 truncate/补换行在返回前 file fsync；fork 的 source
  copy、lineage append 与 pending→final publication 分别完成 file/file/POSIX parent-dir fsync；delete 在
  transcript unlink 的 POSIX 目录屏障之后才发布 tombstone 和外部 witness。任一失败均保持 exact-old、
  exact-new/outcome-unknown 或 fail-closed blocked，不允许后级 authority 静默超前。
- **机器本地 anti-rollback append 增加可判定失败语义。** file/directory fsync 前的故障可回滚到旧 revision；
  首条记录在 open 后失败不会遗留一个随后绕过 directory fsync 的空文件；回滚二次失败显式返回
  `commitState=unknown`，调用方必须 readback 裁决，不能盲目重放。首条完整 record 在 file/directory fsync
  前遗留时，exact replay 或直接推进下一 revision 都会先重新结算 file fsync 与 POSIX parent-dir fsync；sidecar
  发布还绑定已 fsync 描述符、路径身份与精确 payload，缩小同 UID 路径替换窗口。
- **本地候选验证只证明进程崩溃，不冒充物理断电。** Windows 已通过 session-list-index **12/12**、
  anti-rollback **10/10**、JSONL store **146/146**、session-scale gate **4/4** 以及 Session Host Consistency
  **11/11**；强杀矩阵实际完成 5 个 production append persistence cut 与 2 个 partial-record cut，两个 POSIX
  directory-fsync cut 在 Windows 明确 skipped。artifact 固定 `physicalPowerLossSimulated=false`、预先存在且已
  持久的 state roots 前置条件，Linux/macOS 仍须由 exact-head GitHub Actions 验证目录 fsync 分支。

该候选只关闭 canonical session authority 的**进程 SIGKILL + 已声明 fsync syscall 边界**内部子门，且只有在
承载 PR 的 exact-head CLI CI、CLI Session Scale 与 CLI Session Host Consistency 三平台矩阵全部成功后生效。
它不关闭真实任意物理断电、首次 recursive state-root 的逐级目录发布、同时回滚 transcript 与机器外部 witness
的强同 UID 攻击者、跨宿主即时撤权、已 dispatch effect 召回或长期安全矩阵。因此 S0-1 仍为部分完成，
当前 **14/19 尚未关闭、5/19 完成**及 14 个剩余工作包均不减少。
...
| --- | ---------------------------------------------- | ----------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | S0-1：Plan、权限与运行时正确性                 | 部分完成；P0 release gate                 | Plan/contract/Hook/子树指令等安全底座已加固；durable host revocation 与 MCP send-time admission 已进入 `main`；[PR #182](https://github.com/chainlesschain/chainlesschain/pull/182) 已以 merge commit `de6105566b54cdd26df0f551259009015c8e3059` 合入后台 interaction journal CAS、delegated/recovery lease、精确进程身份与故障恢复加固；canonical session SIGKILL/fsync v1 为当前候选 | 关闭已 dispatch 外部 effect 的撤权边界、真实任意物理断电与首次 state-root 逐级目录持久化、同时回滚 transcript 与机器外部 witness 的强篡改者、跨宿主即时 authority 和长期安全矩阵；当前候选仍须通过 exact-head 三平台 CLI CI、Session Scale 与 Host Consistency，任何 authority-bearing 失败均不得扩大能力 |
...
| 4   | Q0：可信入口与 Microsoft Marketplace           | 部分完成；外部阻塞                        | Desktop 单一 command registry、公开 capability manifest、不可变发布门已实现；Open VSX `0.37.53` 与 JetBrains `0.4.89` 均已公开发布并完成 listing 回读                                                                                                                                                                                                                                  | 配置 Microsoft Marketplace 发布 authority，发布同一已验证 VSIX，并完成 exact publisher/version/digest 回读及 stock VS Code 搜索、fresh-profile 安装、升级和回滚                                                                                                                                           |
...
| 7   | Q4b：完整发布与用户旅程门                      | 部分完成；外部阻塞/本期延后               | CLI npm exact-SHA 发布和公网回读、Open VSX `0.37.53` 与 JetBrains `0.4.89` 公开回读、unsigned CLI 六目标执行已完成                                                                                                                                                                                                                                                                     | 完成 Microsoft Marketplace、JetBrains 作者签名、Desktop/native x64+ARM64 签名与公证、公开渠道 fresh-profile 升降级/回滚、网络抖动/重连/Bridge 与 CLI restart、8 小时 IDE soak 和 nightly live-provider trajectory                                                                                         |
