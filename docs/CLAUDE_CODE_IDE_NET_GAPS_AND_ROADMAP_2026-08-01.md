...
| ------------------------ | ----------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P0-S / S0-1～S0-3        | 工作树本地实现完成，release gate 尚未全局关闭               | Plan hard ceiling、contract/permission/Hook fail-closed、子树 mutation 前置、Skill/MCP 边界、Plan/TODO 持久化与语义压缩等定向集共 581 项本地测试通过；`46688dd9ca` 补齐 owner-only `secure-fs` 运行时依赖                                                                                                         | 仍需把剩余 P0-S 文件按独立闭包提交，并在 exact release commit 上完成 Linux/Windows/macOS、strict sandbox、重启/恢复与安全矩阵；在此之前不扩大 Auto/无人值守声明                                                                                 |
| P0-0 / R0                | capability/发布门代码完成，公开发布仍阻断                   | `a1fa5e41e8` 提交不可覆盖、带 digest、缺诊断 fail-closed 的 host evidence；`af6cc890a4` 提交 Microsoft Marketplace exact publisher/name/version/stable/VSIX digest 回读器；`72429c1729` 提交单一公开 capability manifest、漂移检查和 required release workflow gate                                               | Desktop 单一 command registry 仍需按最终范围复核；缺少 `VSCE_PAT` 与可回读的 Microsoft Marketplace 公共版本，故没有实际发布，Open VSX 不能替代官方 Marketplace                                                                                  |
| P0-1 / Q1 / R1 Workbench | 第一垂直切片已实现并曾通过定向验证；当前后续增量为 WIP      | canonical session projection v1、`cc session projection --json`、VS Code/JetBrains 只读 projection，以及 local/background/remote 动作和 stale fail-closed；首切片 38 项测试与 JetBrains 定向验证通过                                                                                                              | 第二切片与首切片位于同一组文件，fixture/parser parity 尚未收口，当前故意不提交、不计为验收完成；完整 dispatch→needs-input→resume→artifact 与重启恢复矩阵仍待完成                                                                                |
...
| P0-3 / Q3 / R2           | 核心协议、策略与 VS Code 纯控制器完成，端到端交付闭环未验收 | `30bb5cd661` 提交 resumable delivery coordinator、impacted gate selector、不可变 evidence、exact-commit strict PR policy、CLI delivery 命令与 VS Code 纯 controller；在干净 `df78ee7060` worktree 上 11 个 CLI 文件集 92/92、VS controller 5/5 通过                                                               | 可视化 view 中仍混有 P0-1 WIP，JetBrains delivery UI 也未完成验证，均不纳入本次完成范围；没有创建真实 PR、没有执行真实 CI fix-rerun、merge 或 auto-archive，因此不能宣称 R2 退出条件已满足                                                      |
| P0-4 / Q4a / R3a         | 验收基础设施部分完成，门禁代码已提交                        | `72429c1729` 提交 VS Code stable/minimum × 三 OS gate、JetBrains 2024.2/2025.2 × 三 OS deterministic driver、capability drift gate 与证据上传；该提交的干净 worktree 通过 capability check、14 项相关 CLI 测试、VS Code 17 项单测、Marketplace self-test，以及 JetBrains 2024.2 Gradle `test`/`compileUiTestJava` | 现有 VS Code runner 仍主要证明 activation/bridge，真实 DOM/CDP journey 为 WIP 且未提交；JetBrains 六格 Remote Robot 矩阵、Remote/多根/多窗口/重启/网络抖动/8 小时 soak 尚未实际运行。二进制截图标记为 restricted artifact，不宣称经过文本级脱敏 |
| Q4b / R3b                | 未完成                                                      | 已有 required workflow 定义，但没有足以关闭发布门的 exact-release-commit 全矩阵运行证据                                                                                                                                                                                                                           | required stock/minimum/OS/remote journeys、fresh-profile 安装/升级/回滚、真实 Marketplace 回读和失败诊断 artifact 仍待完成                                                                                                                      |
...
当前唯一可声明的外部代码动作是上述提交已由仓库 post-commit hook 推送到 GitHub 与 Gitee；这不等同于
Microsoft Marketplace 发布，也不等同于真实 GitHub PR、CI、merge 或产品发布。任何后续状态升级都应绑定
exact commit、运行矩阵和可回读 artifact。
