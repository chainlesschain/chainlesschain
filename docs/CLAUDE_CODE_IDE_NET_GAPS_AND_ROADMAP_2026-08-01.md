...
| Plugin / Marketplace      | A/B：多来源 catalog、统一候选选择、执行 authority、update impact 与显式来源切换已实现                             | 缺真实 private registry、组织签名撤销、实物回读、代理/离线与故障注入矩阵                                                                        | P1/P2           |
...
上述子切片仍不关闭 P1-5 整项：registry metadata 仍明确是未验证 assertion，尚未完成 license/SBOM/signature
实物回读、组织 key revocation、真实 private registry、代理/离线及供应链故障注入矩阵。
...
| 17  | P1-5 / R5            | 部分完成（catalog、统一选择、执行绑定与 impact 子门已实现）    | `392ed39d24` 提供多来源 catalog；`b93f354679` 持久 exact install authority；`07f4d41fa6` 增加 update impact、来源切换/降级批准；`6bd8c11271` 又让 exact registry set 以 stable selection digest 统一选择最高候选，任一来源不可用或最高候选 blocked 时不回退，并让 add/impact/upgrade 在 clone 前绑定 reviewed digest、回读 immutable provenance。仍需 private registry、组织签名撤销、实物回读、代理/离线及供应链故障矩阵。                                                                                                                                                                                                                                                                                                                                                           |
...
| 11  | P1-5：Marketplace 发现与组织治理               | 部分完成                         | `392ed39d24` 新增多 registry catalog；`b93f354679` 新增 install preflight 与 immutable provenance；`07f4d41fa6` 新增 update impact、来源切换/降级批准和 impact digest；`6bd8c11271` 新增 exact registry-set selection contract/digest，按最高版本/来源优先级统一选择，来源不全或最高候选 blocked 时不回退，add/impact/upgrade 可绑定 reviewed selection 并持久 exact authority；registry assertions 始终不冒充 install/load crypto verification                                                                                                                                        | 完成 license/SBOM/signature 实物回读、真实 private registry、组织 key revocation、代理/离线，以及依赖冲突和供应链故障注入矩阵                                                                                                                                                  |
