...
| ------------------------ | --------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P0-S / S0-1～S0-3        | 仓库级安全/正确性闭包及审计加固已拆分提交；release gate 尚未全局关闭        | `70306bd8ee` 提交 structured semantic handoff；`ece470137d` 提交 Plan/TODO persistence；`7c97c11ebd`、`c841a58e2b` 提交 Skill/MCP safety 与受控 Skill 执行；`199b2cb7c6` 提交 WS recovery state；`9332a21ab3` 提交子树 mutation preflight；`6be06f4448` 提交 Plan hard ceiling、capability fence、contract/permission/Hook fail-closed 与 subagent authority inheritance；`46688dd9ca` 补齐 owner-only `secure-fs` 依赖；`a72d75d153` 收口 verified MCP authority read/write、严格 ledger 转换、Headless/Stream/Cowork 恢复及 semantic-compaction 计费；`b21ba58c2c`、`38ab06cfdd`、`d1bbabef62`、`77b3d8eda9`、`c208d0aace` 加固 ledger schema、malformed recovery、动态 latch、冻结对象和 ambiguous outcome；`54228b67d4`、`a83b6a9908`、`c9244db57d`、`ab32a57e4b` 完成 REPL 原子切换及 REPL/Headless/Stream/WS 已知辅助 `callTool` 的共享恢复 authority。最新相关本地矩阵首轮 363 项中 362 项通过，唯一失败为既有 15 秒用例在并行负载下超时，单文件复跑 12/12 通过；随后 WS 122/122、REPL 与 Host runtime 67/67 通过 | 仍需完成独立的 MCP recovery 人工 adjudication，且不得伪造成机器 `settled`；还需在 exact release commit 上完成 Linux/Windows/macOS、strict sandbox、kill/restart、恶意 MCP 与长期安全矩阵。sidecar anchor 只覆盖 crash/尾截断，不防同时改写 transcript 与 metadata 的更强攻击者；真实矩阵完成前不把仓库实现宣称为发布验收，也不扩大 Auto/无人值守保证 |
...
- `38ab06cfdd` 至 `ab32a57e4b` 的后续提交把 malformed recovery、运行时 latch、Host `callTool`、WS resume 与 REPL session switch 收敛到共享 fail-closed authority；模型发起的 MCP 调用仍使用 raw client + 同一 durable ledger，避免重复记账。
- 当前仍把“旧 started-only 调用究竟是否已经生效”视为人工核验问题。安全闭环必须使用独立 adjudication authority event、verified head/digest 与 CAS；不得写入伪造 `completed`/`failed` 记录，也不得允许 Agent 通过非交互参数给自己降权。
- 原生 installer/OTA 仍缺 durable intent/phase journal 或 generation pointer；即使当前锁、backup lineage、alias/sidecar 和 rollback 修复通过本地测试，也不能据此宣称断电/强杀事务已经 release-ready。
