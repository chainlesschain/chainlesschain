...
| 16  | P1-4 / R5            | 部分完成（Permission Center 子切片已实现）     | CLI authority 已提供 workspace 绑定、TTL、CAS revoke 和逐工具调用刷新，双 IDE 只通过 CLI 创建/撤销 scoped rule，managed deny 不可放宽；仍需完成统一 context chips、确定性 token 裁剪、实际资源、不可逆副作用与恢复覆盖解释。                                                                                                                                                                                                                          |
...
| 10  | P1-4：Context 与 Permission/Side-effect Center | 部分完成                                  | 已有 IDE context primitive、policy/approval/ledger；CLI authority 支持 workspace scoped rule、TTL、CAS revoke 与逐工具调用刷新，双 IDE Policy Viewer 通过 CLI 创建/撤销并展示来源、状态、有效期和 revision，managed deny 不可放宽                         | 统一可移除 context chips 及 source/freshness/range/token 解释；展示实际文件/网络/进程/credential、副作用与恢复覆盖；补齐长期并发/真实宿主矩阵                                                                                              |
