...
| P0-4 / Q4a / R3a         | required 真实宿主矩阵已在同一 exact SHA 通过                                      | `5860747f0a` 为隔离的 macOS journey 启用 VS Code 自带的 in-memory test secret storage，恢复 current stable 验收而不采用失败的旧版 host pin。已结算候选 `fb39e2cbe6` 的 IDE Extensions `30965289911` 整体成功：capability、Windows/macOS/Linux 的 stable + minimum `1.85.2`、JetBrains 2024.2/2025.2 × 三 OS 六格、VSIX package/artifact 与 JetBrains build/compatibility 全部通过。macOS job `92177910568` 的 `1.131.0` 和 `1.85.2` evidence 均为 `result=passed`、`evidenceComplete=true`、15 个 artifact；Workspace Publish Staleness Check `30965289905` 成功                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Q4a 的本轮 required local-host matrix 已关闭，但 Remote/SSH/WSL/devcontainer、多根/多窗口、网络抖动、fresh-profile 升降级、ARM64 与 8 小时 soak 仍待完成；这些范围不得从本轮本地宿主成功外推。restricted 二进制工件仍不宣称文本级脱敏                                                                                                                                                                               |
| Q4b / R3b                | required CI/IDE 门已通过，公开发布闭环仍未完成                                    | 已结算候选 `fb39e2cbe6` 的 CLI CI `30965290031`、CLI Strict Sandbox `30965296663` 与 IDE Extensions `30965289911` 均成功；IDE workflow 上传了 VSIX、Linux/Windows/macOS host evidence 和 JetBrains plugin artifact，PR 场景的 Marketplace publish/readback 按条件跳过。该结果关闭本轮 exact-SHA required CI/host gate，不构成公开发布授权                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | GitHub tag `v-npm-0-162-194` 随后在未形成发布授权时被创建并指向 `fb39e2cbe6`；正式 workflow `30966796114` 的 exact-SHA gate 成功，但 Agent SDK build 因缺失 `node_modules/.bin/tsc` 失败，package/publish 全部跳过，npm 仍为 `0.162.193`。该 tag 不移动、不删除、不重用；修复后只能前进版本。还缺 immutable CLI tarball/SBOM、签名、ARM64、安装/升级/回滚和公开渠道回读，仓库也缺 `VSCE_PAT`                        |
...
上述提交已推送到 GitHub 与 Gitee，部分 exact SHA 也已产生组件级 CI 结果；代码交付和验证证据都不等同于 Microsoft Marketplace 发布、真实 PR/merge 或产品 release。`fb39e2cbe6` 已在同一 exact SHA 取得完整 `CLI CI`、`CLI Strict Sandbox` 与 required IDE matrix 成功；公开发布状态仍须绑定后续 release SHA、不可变发布工件和渠道可回读结果，不能由这些 PR 门自动升级。
...
P0-4/Q4a 的本轮 required local-host gate 已由 `fb39e2cbe6` 关闭，但最终判定仍保持 **release NO-GO**。`0.162.194` 的 tag 身份已经被失败 workflow 消耗且 npm 未发布；不得移动、删除或复用该 tag，也不得直接重跑 publish。修复后的新版本仍须在 final exact SHA 重新通过 required gates，并生成 immutable CLI tarball/SBOM、签名及 fresh-profile 安装/升级/回滚证据。仓库存在 `OVSX_PAT`、但缺少 `VSCE_PAT`，Microsoft Marketplace/Open VSX 也没有 `0.37.41` 的公开 exact-version/digest 回读。因此不得继续 publish，也不得把 host/CI 成功外推为路线图所有产品范围已经交付。
