...
P1-4 仍未关闭：legacy/`/v2`/`/v6-preview` 对同一 contract 的消费，以及实际 resource/credential、side effect 与
恢复覆盖解释仍待完成。
