...
| P0-4 / Q4a / R3a         | 真实宿主矩阵已运行并产出 fail-closed 工件；macOS current stable DOM 门仍阻断      | `9327ea0ad2` 的 IDE Extensions `30948438452` 中 capability、Windows VS Code、JetBrains 六格与 JetBrains build 已通过。`350ef5601e` 的 `30947671847` 已证明固定命令 driver、已安装 VSIX、17 个命令和 loopback bridge 实际运行。`61f8235105` 的 IDE run `30960237759` 首次让 macOS minimum `1.85.2` 完整通过 initial 的 stream/retry/plan/permission/interrupt 与 restart-resume，evidence complete；current stable `1.131.0` 仍 fail-closed。候选 `0.37.41` 本地扩展/协议/VSIX 矩阵通过，Workspace Publish Staleness Check `30960237792` 成功                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | macOS stable job `92162462820` 仍因 45 秒内没有 Webview ready/protocol 消息失败；minimum 成功不能替代 current stable。依赖 macOS job success 的最终 Linux/package job仍不可达；Remote/SSH/WSL/devcontainer、多根/多窗口、网络抖动、fresh-profile 升降级、ARM64 与 8 小时 soak 仍待完成。restricted 二进制工件不宣称文本级脱敏                                                                                       |
| Q4b / R3b                | 未完成，发布保持 fail-closed                                                      | `9327ea0ad2` 的 CLI CI `30948439064` 成功；`61f8235105` 的 CLI Strict Sandbox `30960275491` 三 OS 成功、Windows VS Code host 与 macOS minimum 通过，但同 SHA CLI CI 尚未结算且 macOS current stable 失败。排队、运行中、取消或不同 SHA 的结果均不拼接放行                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | 最终 release SHA 仍须同时通过完整 `CLI CI`、`CLI Strict Sandbox`、IDE required journeys 与受影响组件门，并生成绑定该 SHA 的 immutable tarball/SBOM；还缺签名/notarization/Authenticode、x64/ARM64、安装/升级/回滚、Microsoft Marketplace/Open VSX 与发行资产公开回读。当前不得打 tag 或 publish                                                                                                                     |
...
- 截至本节快照，`54db9c8ff7` 的 IDE workflow `30959722716` 中 macOS stable job `92160841699` 已失败、minimum 已跳过，其余 job 尚未全部结算；CLI CI `30959722986` 与同 SHA 手动触发的 Strict Sandbox 也必须等待最终结果。queued/in-progress 状态不计通过，后续文档提交会形成新的 SHA，仍不能沿用实现提交的门禁结果授权发布。
...
- 下一候选保持 current stable 门不变，只对隔离的 macOS real-DOM host 强制 `--disable-gpu` 软件渲染，并启用 `--verbose` 保留 renderer/service-worker 诊断；该 journey 不验收 GPU 行为。仓库扩展单测 **59/59**、runner 专项 **32/32**、语法/格式检查通过，但在 exact candidate SHA 的真实 macOS stable + minimum 结果完成前，这仍只是待验证修复，不是放行依据。

最终判定保持 **release NO-GO**，直到上述候选在真实 current stable 上通过。P0-4/Q4a 不把激活级检查包装成 DOM 成功，也不以 minimum 成功替代 stable；最终 release SHA 仍须重跑 stable + minimum。Q4b 还要求该 SHA 同时通过完整 CLI CI、CLI Strict Sandbox、Windows/macOS/Linux VS Code、JetBrains required matrix 与受影响组件门，并生成 immutable tarball/SBOM、签名及 fresh-profile 安装/升级/回滚证据。仓库 secret 回读已确认存在 `OVSX_PAT`、但缺少 `VSCE_PAT`，Microsoft Marketplace/Open VSX 也没有 `0.37.41` 的公开 exact-version/digest 回读；npm `latest` 仍是非权威流程发布的 `0.162.193`。因此在门禁完成前不得创建 release tag、不得 publish，也不得宣称路线图产品范围已经全部交付。
