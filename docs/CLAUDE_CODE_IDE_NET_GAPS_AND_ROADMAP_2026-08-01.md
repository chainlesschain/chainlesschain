...
- `5ac697dfa2` 加入 crash-safe rollback v2 saga 边界，`5654e1ac1f` 以 `checkpoint_restore_recovery_resolution` 建立 workspace settlement event hash → session resolution hash → saga session commit digest 的双向权威链，`db90af42c5` 同步 recovery read-model/CLI phase 与 rollback request 投影。两组独立审计均为 P0=0、P1=0；但 production rollback controller 和真实跨存储 kill-restart 尚未接线，公开 rollback 继续 read-only。
- `db90af42c5` 冻结后的合并恢复矩阵为 8 files、**209 passed / 2 个平台条件 skip**，帮助索引、Prettier、ESLint、Node `--check` 与 `git diff --check` 通过。此前 a375 Windows 局部 20 秒预算假红已由 `9a99ab1091` 去除，真实 verifier 的跨平台 temp-root 假红已由 `8facbdc3de` 修正；旧失败/取消 run 不得当作后续提交的证明。
- 截至 2026-08-03，本地 `packages/cli/package.json` 与 npm latest 均仍为 `0.162.189`；从 registry gitHead 到 `db90af42c5` 的 CLI 范围为 156 commits、388 files、127,181 additions / 3,803 deletions。结论是：**应准备 `0.162.190` release candidate，但当前仍是 release NO-GO，不要发布**。版本/changelog 所在的最终精确 SHA 必须手动或自动取得 `CLI CI` 与 `CLI Strict Sandbox` 的 Linux/Windows/macOS 全绿，并通过受影响的 Session Host/Background/checkpoint 门和 immutable tarball 校验；任何旧 SHA、queued、cancelled、failure 或局部成功都不能授权发布。
