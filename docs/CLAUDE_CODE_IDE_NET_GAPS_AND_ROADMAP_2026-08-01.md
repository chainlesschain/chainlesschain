...
- 下一候选删除会让 `1.131.0` 更早停滞的 `--disable-gpu`，保留有界诊断，并把本次 macOS release-authoritative stable 明确固定为 `1.130.0`；minimum 继续为 `1.85.2`，Windows/Linux 继续 current stable。该 pin 只适用于 `0.37.41` / CLI `0.162.194`，下个 VS Code extension release 前必须删除或重新评审；新的 exact SHA 在两条 macOS journey 都通过前仍为 NO-GO。

最终判定保持 **release NO-GO**，直到上述候选在固定 stable `1.130.0` 与 minimum `1.85.2` 上都通过。P0-4/Q4a 不把激活级检查包装成 DOM 成功，也不以 minimum 成功替代 validated stable；最终 release SHA 仍须重跑两条 journey。Q4b 还要求该 SHA 同时通过完整 CLI CI、CLI Strict Sandbox、Windows/macOS/Linux VS Code、JetBrains required matrix 与受影响组件门，并生成 immutable tarball/SBOM、签名及 fresh-profile 安装/升级/回滚证据。仓库 secret 回读已确认存在 `OVSX_PAT`、但缺少 `VSCE_PAT`，Microsoft Marketplace/Open VSX 也没有 `0.37.41` 的公开 exact-version/digest 回读；npm `latest` 仍是非权威流程发布的 `0.162.193`。因此在门禁完成前不得创建 release tag、不得 publish，也不得宣称路线图产品范围已经全部交付。
