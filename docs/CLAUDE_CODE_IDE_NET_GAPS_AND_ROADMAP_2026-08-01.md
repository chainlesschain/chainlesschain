...
| ------------------------ | ------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P0-S / S0-1～S0-3        | 语义交接、持久化、Skill/MCP 与 WS 恢复闭包已拆分提交；release gate 尚未全局关闭 | `70306bd8ee` 提交 structured semantic handoff（37 项测试）；`ece470137d` 提交 Plan/TODO persistence（155 项测试）；`7c97c11ebd` 提交 Skill/MCP safety controls（48 项测试）；`199b2cb7c6` 提交 WS recovery state（201 项测试）；`46688dd9ca` 补齐 owner-only `secure-fs` 运行时依赖                                                                                                      | Plan hard ceiling、contract/permission/Hook fail-closed、子树 mutation 前置等剩余闭包仍需独立提交并复核；还需在 exact release commit 上完成 Linux/Windows/macOS、strict sandbox、重启/恢复与安全矩阵；在此之前不扩大 Auto/无人值守声明                                |
| P0-0 / R0                | Desktop command、capability 与发布门代码已提交，公开发布仍阻断                  | `66f8a7e467` 提交 Desktop 单一 command registry，覆盖 4 个测试文件、17 项测试，并通过 `vue-tsc` 与 lint（0 errors）；`a1fa5e41e8` 提交不可覆盖、带 digest、缺诊断 fail-closed 的 host evidence；`af6cc890a4` 提交 Microsoft Marketplace exact publisher/name/version/stable/VSIX digest 回读器；`72429c1729` 提交单一公开 capability manifest、漂移检查和 required release workflow gate | 缺少 `VSCE_PAT` 与可回读的 Microsoft Marketplace 公共版本，故没有实际发布；Open VSX 不能替代官方 Marketplace，本地 lint/typecheck 也不能替代 stock IDE 安装、升级、回滚与公开 exact readback                                                                          |
