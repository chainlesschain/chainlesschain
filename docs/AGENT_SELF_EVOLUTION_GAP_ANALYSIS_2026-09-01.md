...
| P1 | EVO-P1-1、P1-3、P1-8、P1-9 已完成，P1-2 仓库闭环，P1-7 持久控制协议部分完成；其余主能力仍待验收 | shared EvolutionRun 已统一 CLI/Desktop/Graph 的 Raw/Wiki/Registry/Eval 投影；Wiki Maintainer 已闭环认证 evidence、确定性 pattern/index/log/impact、生命周期、ArtifactPorts 不可变 revision 与真实 Ledger CAS/event/reopen；结构化 Memory 已具备四层写入规则、多角色 receipt gate、可回退 compaction，以及 ArtifactStore+Ledger event/snapshot 持久 adapter 和校验式恢复；SkillInvocationReceipt 与有界评分循环已闭环 | 生产目标矩阵 Eval、统一 Registry/Agent ingress、Memory receipt resolver/PostCompact production composition root 仍未完成；真实流量接入归 P2 |
...
| EVO-P1-7 Memory 与多 Agent 权力分离 | 持久控制协议部分完成 | shared canonical event/projection/snapshot 已强制四层；child/proposer→critic+evaluator→governor、promotion-controller、human policy governor 权限路径与 receipt/content/lineage 绑定已落地；runtime actor 使用 tenant-bound branded authority；增量 state 绑定上一 projection digest；compaction 强制八类保真字段并失败回退；有限 event/snapshot ArtifactPorts 类型、Ledger CAS/lineage、branded resolver、响应丢失幂等、并发冲突和校验式 hydration 已接通；实际 Ledger 文件/签名/witness 已完成新实例 reopen；控制协议 14/14、session-core 533/533、adapter 5/5 通过 | 接入真实 PostCompact hook；为 critic/evaluator/promotion/policy receipt 接入 branded resolver 并由 production composition root 固定 |
...
| EVO-P1-7 结构化 Memory 持久控制协议 | session-core 控制协议 14/14；Memory adapter 5/5，连同 ArtifactPorts 合计 30 项通过、1 项按平台条件跳过；覆盖四层 reducer、authority/compaction、hydration digest 校验、真实 ArtifactStore event/snapshot、Ledger head CAS、并发冲突、branded resolve、响应丢失幂等，以及实际 Ledger 文件/签名/witness 新实例恢复 | 持久 adapter 与实际 Ledger reopen 已闭环；真实 PostCompact hook 与四类 receipt resolver/production composition 仍需关闭 |
...
总计 20 项：**6 项已完成、1 项仓库闭环、6 项部分完成、7 项待完成**。计入本次 EVO-P1-7 实际 Ledger reopen 验收后共有 **30 个基础批次**。这两个计数维度不能混用：基础提交数量不代表路线项完成数量。
...
| P1 | EVO-P1-7 | 结构化 Memory、Compaction 与多 Agent 权力分离 | 🟡 部分完成 | shared canonical event/projection/snapshot、四层规则、semantic 多角色 gate、promotion-only procedural、human-only policy、tenant-bound branded actor、digest-bound 增量 state、八类 compaction 保真及失败 rollback 已落地；ArtifactStore+Ledger event/snapshot adapter 已补有限类型、CAS/lineage、branded resolve、响应丢失幂等、并发冲突和校验式 hydration，并通过实际 Ledger 文件/签名/witness 新实例恢复；控制协议 14/14、session-core 533/533、adapter 5/5 通过；详见 §1.1、§6.7 | 接入真实 PostCompact hook 和 critic/evaluator/promotion/policy receipt branded resolver，并由 production composition root 固定 |
