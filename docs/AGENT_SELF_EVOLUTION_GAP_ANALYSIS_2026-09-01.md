...
| P1 | EVO-P1-1、P1-3、P1-8、P1-9 已完成，P1-2 仓库闭环，P1-7 持久/权力分离控制协议部分完成；其余主能力仍待验收 | shared EvolutionRun 已统一 CLI/Desktop/Graph 的 Raw/Wiki/Registry/Eval 投影；Wiki Maintainer 已闭环认证 evidence、确定性 pattern/index/log/impact、生命周期、ArtifactPorts 不可变 revision 与真实 Ledger CAS/event/reopen；结构化 Memory 已具备四层写入规则、branded actor/receipt/PostCompact gate、可回退 compaction，以及 ArtifactStore+Ledger event/snapshot 持久 adapter 和校验式恢复；SkillInvocationReceipt 与有界评分循环已闭环 | 生产目标矩阵 Eval、统一 Registry/Agent ingress、Memory 真实 receipt/PostCompact adapters 与最终 Agent composition 仍未完成；真实流量接入归 P2 |
...
| EVO-P1-7 Memory 与多 Agent 权力分离 | 持久/权力分离控制协议部分完成 | shared canonical event/projection/snapshot 已强制四层；runtime actor、四类 receipt resolver 与 PostCompact 均使用 tenant-bound branded provider；receipt 必须精确绑定 transition，PostCompact 必须绑定 candidate/projection/previous snapshot；Ledger adapter 构造期固定全部 provider，不暴露逐调用替换；有限 event/snapshot ArtifactPorts、Ledger CAS/lineage、响应丢失幂等、并发冲突、校验式 hydration 和实际 Ledger 文件/签名/witness reopen 已接通；控制协议 19/19、session-core 538/538、adapter 6/6 通过 | 接入真实 receipt resolver 后端、CLI/Desktop PostCompact hook adapter 与最终 Agent production composition |
...
| EVO-P1-7 结构化 Memory 持久与权力分离协议 | session-core 控制协议 19/19、全量 538/538；Memory adapter 6/6，连同 ArtifactPorts 合计 31 项通过、1 项按平台条件跳过；覆盖四层 reducer、branded actor/receipt/PostCompact、四类 receipt、transition/result substitution、hydration、真实 ArtifactStore、Ledger CAS/并发/响应丢失，以及实际 Ledger 文件/签名/witness 新实例恢复 | 持久与组合原语已形成；真实 receipt resolver、CLI/Desktop PostCompact hook adapters 和最终 Agent production composition 仍需关闭 |
...
总计 20 项：**6 项已完成、1 项仓库闭环、6 项部分完成、7 项待完成**。计入本次 EVO-P1-7 receipt/PostCompact 权力分离后共有 **31 个基础批次**。这两个计数维度不能混用：基础提交数量不代表路线项完成数量。
...
| P1 | EVO-P1-7 | 结构化 Memory、Compaction 与多 Agent 权力分离 | 🟡 部分完成 | shared canonical event/projection/snapshot、四层规则、semantic/procedural/policy gate、tenant-bound branded actor、构造期固定且独立认证的 receipt/PostCompact providers、digest-bound 增量 state、八类 compaction 保真及失败 rollback 已落地；ArtifactStore+Ledger adapter 已补有限类型、CAS/lineage、响应丢失幂等、并发冲突、校验式 hydration 和实际 Ledger 文件/签名/witness reopen；控制协议 19/19、session-core 538/538、adapter 6/6 通过；详见 §1.1、§6.7 | 接入真实 receipt resolver 后端、CLI/Desktop PostCompact hook adapters，并由最终 Agent production composition 使用 |
