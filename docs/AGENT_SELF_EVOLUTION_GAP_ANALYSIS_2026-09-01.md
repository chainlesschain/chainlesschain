...
总计 20 项：**6 项已完成、1 项仓库闭环、6 项部分完成、7 项待完成**。计入本次 EVO-P1-7 Agent semantic/procedural use surfaces 后共有 **41 个基础批次**。这两个计数维度不能混用：基础提交数量不代表路线项完成数量。
...
| P1 | EVO-P1-7 | 结构化 Memory、Compaction 与多 Agent 权力分离 | 🟡 部分完成 | shared canonical 四层 gate、真实 ArtifactStore+Ledger resolver、CLI/Desktop PostCompact、四类 producer、同 stream branded Agent root、新实例恢复、AgentRuntime semantic lifecycle、root-only evaluated promotion facade 与 remote bridge 已落地；相关联合回归均通过；详见 §1.1、§6.7 | root 下完整 evaluated promotion 执行、生产密钥/witness 后端、进程重启与 reconciliation 验收 |
