...
| EVO-P1-7 Memory 与多 Agent 权力分离 | 持久/权力分离纵切部分完成 | shared canonical event/projection/snapshot 已强制四层；runtime actor、四类 receipt resolver 与 PostCompact 均使用 tenant-bound branded provider；receipt canonical digest/issuer、真实 ArtifactStore+Ledger resolver、独立 verifier、响应丢失恢复已接通；CLI Hooks V2 与 Desktop HookSystem PostCompact 均要求非空完整成功结果及 attestor；Ledger adapter 构造期固定全部 provider；event/snapshot CAS/lineage、并发冲突、hydration 和实际 Ledger 文件/签名/witness reopen 已通过；控制协议 19/19、session-core 538/538、Memory/authority adapter 8/8、CLI PostCompact 5/5、Desktop PostCompact 5/5 通过 | 接入实际 critic/evaluator/promotion/policy producer 写入与最终 Agent production composition |
...
总计 20 项：**6 项已完成、1 项仓库闭环、6 项部分完成、7 项待完成**。计入本次 EVO-P1-7 Desktop PostCompact adapter 后共有 **34 个基础批次**。这两个计数维度不能混用：基础提交数量不代表路线项完成数量。
...
| P1 | EVO-P1-7 | 结构化 Memory、Compaction 与多 Agent 权力分离 | 🟡 部分完成 | shared canonical event/projection/snapshot、四层规则、semantic/procedural/policy gate、tenant-bound branded actor、canonical issuer-bound receipt、真实 ArtifactStore+Ledger receipt resolver、CLI Hooks V2 与 Desktop HookSystem PostCompact+attestor、digest-bound 增量 state、八类 compaction 保真及失败 rollback 已落地；event/snapshot adapter 具备 CAS/lineage、响应丢失幂等、并发冲突、hydration 和实际 Ledger 文件/签名/witness reopen；控制协议 19/19、session-core 538/538、Memory/authority adapter 8/8、CLI PostCompact 5/5、Desktop PostCompact 5/5、ArtifactPorts 25/25 通过；详见 §1.1、§6.7 | 接入实际 critic/evaluator/promotion/policy producer 写入，并由最终 Agent production composition 使用 |
