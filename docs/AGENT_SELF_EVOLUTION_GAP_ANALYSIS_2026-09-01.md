...
| EVO-P1-7 结构化 Memory 持久控制协议 | session-core 控制协议 14/14；Memory adapter 与 ArtifactPorts 合计 29 项通过、1 项按平台条件跳过；覆盖四层 reducer、authority/compaction、hydration digest 校验、真实 ArtifactStore event/snapshot、Ledger head CAS、并发冲突、branded resolve 与响应丢失幂等 | 持久 adapter 基础已形成；实际 Ledger 文件/witness 专项 reopen、真实 PostCompact hook 与四类 receipt resolver/production composition 仍需关闭 |
...
总计 20 项：**6 项已完成、1 项仓库闭环、6 项部分完成、7 项待完成**。计入本次 EVO-P1-7 持久 adapter 后共有 **29 个基础批次**。这两个计数维度不能混用：基础提交数量不代表路线项完成数量。
