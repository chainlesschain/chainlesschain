...
本节记录本报告转入实施后的当前状态，实施基线截至提交 `face4e9250`。状态采用两层口径：**“已提交”只表示一个可独立审查、已验证的基础批次完成，不等于对应路线项已经达到第 9 节的生产验收标准**；只有剩余项全部关闭后，路线项才可标记为“完成”。
...
当前工作区进一步补上 authority attestation 撤销基线：精确描述符必须提供正整数 revision 与至多 64 个唯一 revoked keyId；当前 active key 已被撤销时 adapter 拒绝构造，grace key 即使仍在 `notAfter` 窗口内也会以独立 `REVOKED` 结果失败关闭。revocation revision 会传入构造期捕获的 verifier，避免 verifier 在不知道撤销快照版本的情况下给出认证结论。该批次只建立 fail-closed 数据模型与组合契约，不宣称已经具备在线 CRL/OCSP、撤销发布认证、跨进程刷新、应急轮换或生产演练。

| 优先级 | 当前判断 | 已完成并写入仓库 | 当前仍需完成 |
| --- | --- | --- | --- |
| P0 | 已完成 19 个基础提交并有 1 个工作区续作批次；整体未关闭 | 能力真值与 mutation freeze、可信 mutation authority、tenant-scoped candidate/release/promotion、独立监督 Eval Gate、signed target-matrix 全 cell 合取门、accepted matrix receipt→promotion/release intent 窄绑定、typed/digest-bound receipt-resolution 端口、evaluated-only controller 组合、attested durable receipt lifecycle adapter 及撤销快照基线、证据投影与制品端口、tamper-evident ledger、类型化领域事件、规范执行清单、持久账本端口契约 | 缺真实跨进程 durability authority/PKI 后端、真实进程级 Eval supervisor、现有生产 adapter 的实际构造接线与持久 child resolver、跨 cell 统计校准/多重比较、人工 quorum、端到端控制面和生产崩溃/攻击测试 |
| P1 | 部分底座提前落地；主能力未开始验收 | target matrix 数据模型与全 cell 合取证据基础、content-addressed registry、ledger/event/port 等依赖已具备基础实现 | Canonical Raw/Wiki/Skill、Maintainer、Proposer、生产目标矩阵 Eval、统一 Registry 接线、结构化 Memory、InvocationReceipt 和有界评分循环均未形成生产纵切 |
...
| EVO-P0-3 独立真实 Eval Gate | 单 cell 监督、matrix 合取证据、evaluated-promotion 窄绑定、receipt resolver、evaluated-only 组合及 attested/bounded/fresh/rotatable/revocation-aware durable lifecycle adapter 基础完成，生产接线 HOLD | 隔离 target、角色/信任分权、签名 receipt v3、调用/撤销独立证据、hard-termination 收敛、全 run 单调 deadline、descriptor-bound authority root、后验防 TOCTOU；signed matrix plan、tenant durable reserve/finalize 端口契约、verified full child receipt（含 attestation）的有序摘要根、因果时间线与全 cell 一致通过判定；accepted receipt 在 authority consume/release prepare 前绑定 candidate/runtime/target/active CAS，并由 release intent 固定 eval envelope digest；按 tenant/digest 解析完整 receipt 的 fail-closed 端口；构造期 branded provider 与拒绝直接 `promote()` 的策略；要求 authenticated/durable acknowledgement、验证规范 payload attestation、证据新鲜度、全局唯一 keyId/有界 grace trust、版本化撤销快照，并对 authority 调用施加根时限/AbortSignal 的 retain/resolve 生命周期适配器 | `52427b742c`、`5c2de980d3`、`881abf6090`、`1a70a880fa`、`0feb536d82`、`face4e9250`；2026-09-02 revocation policy 工作区续作 | train/validation/test 隔离、真实 grader、anti-gaming、跨 cell 统计校准/多重比较；现有生产 adapter 实际构造 evaluated-only controller、真实跨进程 durability authority/PKI 后端、持久 child resolver 与恢复 E2E；生产 attested loader 绑定 descriptor↔callable/真实 settlement 上限；进程级 kill/资源回收；跨进程 plan resolver/reservation/finalization 持久性；认证撤销分发/刷新及生产密钥轮换/撤销演练与目标平台 grace 校准 |
...
| EVO-P1-5 Registry 与单写者治理 | tenant 存储与 evaluated-only controller 原语基础完成 | content-addressed tenant candidate/release、promotion controller、lease/CAS/journal/recovery 原语、可拒绝直接 promotion 的构造策略 | 唯一 production writer 的实际构造接线、kill switch、active/last-known-good/canary 生产接线与跨进程持久权威 |
...
| Evaluated-only controller 组合 | matrix eval 3/3、promotion controller 14/14 定向回归通过，包含真实 provider 验证成功及 direct promote 在 authority consume 前被拒绝 | 已提交 `0feb536d82`；resolver/verifier 已收敛为构造期 branded provider，控制器可强制禁用直接 promotion；真实生产构造点接线仍 HOLD |
| Attested/bounded/fresh/rotatable/revocation-aware durable matrix receipt lifecycle adapter | 13/13 定向测试通过，覆盖跨 adapter 实例恢复、非 durable acknowledgement、receipt substitution、无效 attestation、永久悬挂/AbortSignal、过期签名重放、grace key 窗口、重复 keyId、超长 grace policy、grace key 撤销、active key 已撤销及重复撤销项 | 基础 adapter 已提交 `face4e9250`，2026-09-02 工作区续作补上版本化撤销快照；已建立外部 authority retain/resolve、规范 payload attestation、根操作时限、证据新鲜度、唯一 keyId/有界 grace 及 fail-closed revocation 的组合契约；真实跨进程后端、生产 PKI/撤销分发运营、进程级 hard kill、崩溃恢复和生产接线仍 HOLD |
...
总计 20 项：**0 项已完成、0 项仓库闭环、10 项部分完成、10 项待完成**。另有 **19 个已提交基础批次和 1 个尚未提交的工作区批次**。这两个计数维度不能混用：基础提交数量不代表路线项完成数量。

| 优先级 | 编号 | 任务 | 状态 | 已完成与验证证据 | 剩余工作 |
| --- | --- | --- | --- | --- | --- |
| P0 | EVO-P0-1 | 能力真实性与 mutation freeze | 🟡 部分完成 | canonical capability/runtime status、candidate-only writer 边界、mutation 路径盘点与 Desktop Skill 生命周期证据已落地；基础提交为 `3fdff6c1ee`、`0da1f36a8b`、`c16e1a3912`；详见 §1.1、§5.1 | 对全部真实生产入口完成 cutover/E2E；确认旧 self-evolving 壳不能绕过门禁或继续报告 phantom success |
| P0 | EVO-P0-2 | Candidate、Promotion 与 Rollback 事务 | 🟡 部分完成 | 不可变 tenant candidate、可信 mutation authority、execution manifest、lease/CAS/journal/recovery、subject binding 与持久账本端口原语已落地；详见 §1.1、§5.2 | 统一两阶段事务与真实 durable authority；commit-unknown/concurrency E2E；active/last-known-good/canary、session digest pinning、cache invalidation、生产 ACL/只读 CAS 与持续审计 |
| P0 | EVO-P0-3 | 独立真实 Eval Gate | 🟡 部分完成 | 独立监督 Gate、signed target matrix 全 cell 合取、accepted receipt→promotion 窄绑定、typed resolver、evaluated-only controller、attested/bounded/fresh/rotatable durability adapter 已提交；当前工作区新增版本化 revocation snapshot。最新相关回归 30/30 通过；详见 §1.1、§5.3 | 提交当前撤销批次；接入真实 grader 与 train/validation/test 隔离；完成统计校准/多重比较、生产 evaluated-only 构造接线、跨进程 authority/PKI/child resolver、进程级 kill、崩溃恢复及密钥撤销演练 |
...
| P1 | EVO-P1-5 | Registry、生命周期与单写者治理 | 🟡 部分完成 | content-addressed tenant candidate/release、promotion controller、lease/CAS/journal/recovery 与拒绝 direct promotion 的策略已落地；详见 §1.1、§6.5 | 建立唯一 production writer 的实际构造接线、kill switch、active/last-known-good/canary 与跨进程持久权威 |
...
| P1 | EVO-P1-8 | SkillInvocationReceipt 与因果归因 | 🟡 部分完成 | Desktop lifecycle metrics/evidence 与 ledger/artifact ports 可复用；详见 §1.1、§6.8 | 每次调用固定 skill digest、router reason、模型段、环境/权限、outcome/cost，并可反向 join Eval/Wiki |
| P1 | EVO-P1-9 | 有界评分改进循环 | ⏳ 待完成 | GoalConditionEngine、预算、Eval、worktree/checkpoint 可复用；尚未组成 candidate-only 改进闭环；详见 §1.1、§6.9 | 串成一次一候选、独立评分、best 保留、失败分类和根预算停止的离线纵切，再做生产准入 |
