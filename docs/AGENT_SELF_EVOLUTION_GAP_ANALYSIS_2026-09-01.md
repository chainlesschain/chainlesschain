...
| P1 | EVO-P1-1、P1-3、P1-8、P1-9 已完成；P1-2 控制协议部分完成；其余主能力仍待验收 | shared EvolutionRun 已统一 CLI/Desktop/Graph 的 Raw/Wiki/Registry/Eval 投影；Wiki Maintainer 已具备认证 evidence、确定性 pattern/index/log/impact、生命周期与 CAS revision 端口；Wiki-informed proposer 已具备证据 abstention、单 Skill PURPOSE/diff/candidate-only 纵切；SkillInvocationReceipt 与有界评分循环已闭环 | Maintainer 真实跨进程 authority/持久 adapter、生产目标矩阵 Eval、统一 Registry 生产接线和结构化 Memory 仍未完成；真实流量接入归 P2 |
...
| EVO-P1-2 Evidence-backed Wiki Maintainer | 部分完成 | trusted/digest-bound evidence ingress、最小权限 Maintainer、确定性 pattern/index/evolution-log/skill-impact/reverse-dependency、去重/merge/conflict/decay/expiry/revoke/tombstone、digest-bound proposal rejection impact 与 CAS revision 端口；1 文件 13/13 通过 | 接入真实 ArtifactStore+EvolutionLedger Wiki authority；完成跨进程并发 writer、crash/restart、旧数据迁移、真实 evidence ingress 与生产 retention/crypto-shredding 演练 |
...
| EVO-P1-2 Wiki Maintainer 控制协议 | Maintainer 1 文件 13/13 通过；覆盖多源/grader corroboration、单模型 hypothesis、cluster 去重、反证、proposal rejection impact/字段替换拒绝、confidence decay、expiry/revocation reverse propagation、merge/tombstone、跨 tenant/raw secret 拒绝、CAS acknowledgement substitution 与确定性重放 | pattern/index/log/impact 与生命周期 reducer 已形成候选纵切；真实跨进程持久 authority、并发/restart/迁移及生产 ingress 继续 HOLD |
...
总计 20 项：**6 项已完成、0 项仓库闭环、7 项部分完成、7 项待完成**。计入本次 EVO-P1-2 控制协议后共有 **26 个基础批次**。这两个计数维度不能混用：基础提交数量不代表路线项完成数量。
...
| P1 | EVO-P1-2 | Evidence-backed Wiki Maintainer | 🟡 部分完成 | trusted/digest-bound evidence、最小权限构造、确定性 pattern/index/log/impact/reverse-dependency、去重/merge/conflict/decay/expiry/revoke/tombstone、digest-bound proposal rejection impact 与 CAS revision 端口已落地；13/13 通过；详见 §1.1、§6.2 | 接入真实 ArtifactStore+EvolutionLedger authority，完成跨进程并发/restart/迁移、真实 ingress 和生产 retention/crypto-shredding 演练 |
