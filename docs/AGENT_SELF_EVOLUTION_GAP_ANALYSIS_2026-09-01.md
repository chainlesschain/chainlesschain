...
| P1 | EVO-P1-1、P1-3、P1-8、P1-9 已完成；P1-2 持久组合部分完成；其余主能力仍待验收 | shared EvolutionRun 已统一 CLI/Desktop/Graph 的 Raw/Wiki/Registry/Eval 投影；Wiki Maintainer 已具备认证 evidence、确定性 pattern/index/log/impact、生命周期、ArtifactPorts 不可变 revision 与 Ledger CAS/event lineage；Wiki-informed proposer 已具备证据 abstention、单 Skill PURPOSE/diff/candidate-only 纵切；SkillInvocationReceipt 与有界评分循环已闭环 | Maintainer 真实 EvolutionLedger 跨进程 kill/restart、迁移与生产 ingress，生产目标矩阵 Eval、统一 Registry 生产接线和结构化 Memory 仍未完成；真实流量接入归 P2 |
...
| EVO-P1-2 Evidence-backed Wiki Maintainer | 部分完成 | trusted/digest-bound evidence ingress、最小权限 Maintainer、确定性 pattern/index/evolution-log/skill-impact/reverse-dependency、去重/merge/conflict/decay/expiry/revoke/tombstone、digest-bound proposal impact；`wiki-revision` ArtifactPorts ledger-retention 类型、branded resolver、不可变 revision→Ledger domain event、head/sequence CAS、跨 adapter 恢复与响应丢失幂等已接通；Maintainer 13/13、adapter 5/5、ArtifactPorts 25/25 通过 | 完成真实 EvolutionLedger 跨进程并发/kill/restart、旧数据迁移、真实 evidence ingress 与生产 retention/crypto-shredding 演练 |
...
| EVO-P1-2 Wiki Maintainer 控制协议与持久组合 | Maintainer 13/13、adapter 5/5、ArtifactPorts 25/25 通过；覆盖多源/grader corroboration、单模型 hypothesis、去重/反证/decay/revoke、proposal/commit substitution、真实不可变 artifact 写入与 branded resolve、Ledger head CAS、跨 adapter 恢复及响应丢失幂等 | pattern/index/log/impact 生命周期已绑定现有 ArtifactPorts+Ledger 接口；真实 EvolutionLedger 进程级 kill/restart、迁移、生产 ingress 与 retention/crypto-shredding 演练继续 HOLD |
...
实施结果（2026-09-02，工作区）：**该路线项部分完成。** `EvidenceBackedWikiMaintainer` 已建立认证 trusted-projection ingress、tenant/digest 绑定、最小权限构造策略和确定性 Wiki revision reducer。canonical state 同时维护 pattern、默认检索 index、append-only evolution log、Skill proposal impact、evidence metadata 与反向依赖；upsert 会按规范 fingerprint 去重，显式 merge 仅允许同 kind pattern，counterevidence/contradiction、TTL、半衰期 confidence decay、evidence revoke/delete、pattern revoke/tombstone 会使 pattern 进入 `contradicted/stale/revoked/tombstoned` 并从默认 index 剪枝。单源模型总结保持不可操作 `hypothesis`；多 trust-domain evidence 或真实 grader receipt 才能使 procedure 进入 `corroborated`，proposal rejection 只追加 impact/rejection count 并降低 operational confidence，不删除或回滚历史；decision candidate/outcome/pattern/reason 必须与认证 receipt digest 精确绑定。revision commit 必须以 prior state digest 做 CAS，并精确确认 revision ID、state digest 和 EvolutionRun ID；替换确认失败关闭。新增持久组合直接复用 `EvolutionArtifactPorts + EvolutionLedger`：有限 `wiki-revision` 类型仅允许 `evolution-ledger` retention，branded resolver、防 artifact substitution、head/sequence CAS、previous-revision lineage、跨 adapter 恢复和 append 响应丢失幂等均已落地。Maintainer 13/13、adapter 5/5、真实 ArtifactPorts 25/25 通过，另 1 项按平台条件跳过。真实 EvolutionLedger 进程级 kill/restart、旧数据迁移、真实 ingress 和生产 retention/crypto-shredding 演练尚未完成，故仍不能标记为仓库闭环或已完成。
...
总计 20 项：**6 项已完成、0 项仓库闭环、7 项部分完成、7 项待完成**。计入 EVO-P1-2 持久组合后共有 **27 个基础批次**。这两个计数维度不能混用：基础提交数量不代表路线项完成数量。
...
| P1 | EVO-P1-2 | Evidence-backed Wiki Maintainer | 🟡 部分完成 | trusted/digest-bound Maintainer 与生命周期 reducer 已落地；`wiki-revision` ArtifactPorts ledger retention、branded resolver、不可变 revision→Ledger event、head CAS、跨 adapter 恢复及响应丢失幂等已接通；13/13 + 5/5 + 25/25 通过；详见 §1.1、§6.2 | 完成真实 EvolutionLedger 跨进程并发/kill/restart、旧数据迁移、真实 ingress 和生产 retention/crypto-shredding 演练 |
