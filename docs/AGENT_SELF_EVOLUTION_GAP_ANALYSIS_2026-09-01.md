...
| EVO-P1-7 Memory 与多 Agent 权力分离 | 持久/权力分离纵切部分完成 | shared canonical 四层 gate、真实 ArtifactStore+Ledger resolver、独立 verifier、响应丢失恢复与四类 producer 已接通；branded Agent control root 已一次性固定同 stream adapters/authorities/writers、完成新实例恢复并进入 AgentRuntime→REPL→remote bridge | root 的 semantic/procedural Agent 使用点、生产密钥/witness、进程重启与 reconciliation 验收 |
...
总计 20 项：**6 项已完成、1 项仓库闭环、6 项部分完成、7 项待完成**。计入本次 EVO-P1-7 Agent root runtime wiring 后共有 **40 个基础批次**。这两个计数维度不能混用：基础提交数量不代表路线项完成数量。
...
| P1 | EVO-P1-7 | 结构化 Memory、Compaction 与多 Agent 权力分离 | 🟡 部分完成 | shared canonical 四层 gate、真实 ArtifactStore+Ledger receipt resolver、CLI/Desktop PostCompact、四类 producer、同 stream branded Agent control root、新实例恢复及 AgentRuntime→REPL→remote bridge 接线已落地；相关联合回归均通过；详见 §1.1、§6.7 | root 的 semantic/procedural Agent 使用点、生产密钥/witness 后端、进程重启与 reconciliation 验收 |
