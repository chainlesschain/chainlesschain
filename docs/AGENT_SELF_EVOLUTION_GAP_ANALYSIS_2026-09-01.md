...
| --- | --- | --- | --- |
| P0 | 已完成 23 个基础提交；EVO-P0-1、EVO-P0-2 已完成，P0 整体未关闭 | 能力真值与 mutation freeze 已完成产品表面切换；Candidate/Promotion/Rollback 已完成不可变制品、CAS、journal/recovery、100 并发单赢家和字节级 LKG 回滚验收；可信 mutation authority、独立监督 Eval Gate、signed target-matrix 全 cell 合取门、accepted matrix receipt→promotion/release intent 窄绑定、typed/digest-bound receipt-resolution 端口、evaluated-only controller 与受限 control-plane facade 组合、attested durable receipt lifecycle adapter 及撤销快照基线、密文 Raw→model/trusted projection 的 ArtifactStore 持久组合与 derivation manifest、统一 Candidate secret/PII plaintext guard、tamper-evident ledger、类型化领域事件、规范执行清单、持久账本端口契约 | P0-3～P0-5 仍缺真实跨进程 durability authority/PKI 后端、真实进程级 Eval supervisor 与持久 child resolver、跨 cell 统计校准/多重比较、真实 Agent pre-model ingress、删除传播、Candidate 恶意指令/capability diff、人工 quorum 和生产攻击测试；统一最终用户控制面构造接线归 EVO-P1-5 |
...
| EVO-P0-4 Raw、入模投影与 Skill 编译安全边界 | 持久组合与 Candidate plaintext 基础完成，生产接线 HOLD | attested source→Raw/model/trusted projection；外部 encryptor + tenant KMS ref 的 ciphertext-only Raw ArtifactStore；secret/PII 脱敏、injection 标记与 trust/quarantine；四分片认证写入及最后提交的完整 derivation manifest；Candidate strict schema/name/size/realpath/symlink/single-link 门与复用同一 Unicode-aware policy 的 pre-write secret/PII 拒绝；联合回归 72 通过、1 跳过，Candidate+Projector 74/74 通过 | `b8490faa94`、`b4dca1ee05`、`4cffc53054`、`bc24db7a0b`、`f668598a42` | 真实 Agent/tool pre-model ingress 与旧直通路径退役、生产 KMS/HSM、删除/撤销依赖传播、Candidate 语义恶意指令审查和相对 parent 的 capability/permission diff、人工 quorum、1,000 条生产攻击轨迹验收 |
...
实施结果（2026-09-02，工作区）：**该路线项的持久组合与 Candidate plaintext 基础已完成，整体仍为部分完成。** 既有 `EvolutionEvidenceProjector` 已执行 source verification、tenant-keyed commitment、storage/ACL/retention policy、secret/PII redaction、prompt-injection 标记、trust/quarantine、model/trusted 双投影和签名 attestation；本批 `ArtifactStoreEncryptedRawStore` 将其 Raw 端口接到真实 ArtifactStore，plaintext 只交给捕获的外部 encryptor，持久层只得到 AES-256-GCM sealed bytes、cipher digest、tenant KMS ref 与无敏感 lineage。`EvolutionEvidenceArtifactAdapter` 要求 repository projector、独立 bundle verifier 和 authenticated ArtifactPorts，四个 projection 分片分别持久并 readback 后，最后发布 derivation manifest；manifest 固定每个分片的 envelope/ref/digest、source commitment、ruleset digest、projection receipt 和 attestation，新实例恢复时逐项重新认证。真实文件测试以 secret、email 和 prompt-injection canary 证明：Raw 密文无明文 canary且可由测试 authority 解密，所有 model/trusted/receipt/manifest 文件无 secret/PII，注入证据在重启后仍为 quarantined，manifest 替换失败关闭；联合回归 72 通过、1 项按平台跳过，代码提交为 `bc24db7a0b`。随后 `f668598a42` 将同一 Unicode-aware secret/PII policy 接入唯一 Candidate canonical builder，在任何 digest/临时文件/最终文件产生前拒绝普通及混淆 canary；既有 Registry 的 strict schema、kebab-case、size、realpath、symlink、single-link 和 descriptor readback 继续生效，Candidate+Projector 74/74 通过。未关闭项仍包括真实 Agent/tool result 的 pre-model 接线与旧直通路径退役、生产 KMS/HSM、60 秒删除/撤销传播、Candidate 语义恶意指令审查及相对 parent 的 capability/permission diff、人工 quorum，以及 1,000 条对抗轨迹验收。
...
总计 20 项：**6 项已完成、2 项仓库闭环、5 项部分完成、7 项待完成**。计入本次 EVO-P0-4 Candidate plaintext guard 后共有 **50 个基础批次**。这两个计数维度不能混用：基础提交数量不代表路线项完成数量。
...
| P0 | EVO-P0-4 | Raw、入模投影与 Skill 编译安全边界 | 🟡 部分完成 | attested source→Raw/model/trusted projection、外部 encryptor 驱动的 ciphertext-only Raw ArtifactStore、secret/PII 脱敏、injection trust/quarantine、四分片 authenticated ArtifactPorts 与最后提交的 derivation manifest，以及 Candidate strict schema/name/size/path/symlink + Unicode-aware secret/PII pre-write guard 已落地；提交 `bc24db7a0b`、`f668598a42`；详见 §1.1、§5.4 | 真实 Agent/tool pre-model ingress、生产 KMS/HSM、删除/撤销传播、Candidate 恶意指令与 parent-relative capability diff、人工 quorum 与 1,000 条生产攻击轨迹验收 |
