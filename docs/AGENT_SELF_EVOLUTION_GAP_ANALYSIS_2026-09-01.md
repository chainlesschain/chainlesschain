...
| EVO-P1-7 Memory 与多 Agent 权力分离 | 控制协议部分完成 | shared canonical event/projection/snapshot 已强制四层；child/proposer→critic+evaluator→governor、promotion-controller、human policy governor 权限路径与 receipt/content/lineage 绑定已落地；runtime actor 使用 tenant-bound branded authority；增量 state 绑定上一 projection digest；compaction 强制八类保真字段并失败回退；12/12、session-core 531/531 通过 | 接入真实持久 event/snapshot adapter 与 PostCompact hook；为 critic/evaluator/promotion/policy receipt 接入 branded resolver 并由 production composition root 固定 |
...
| P1 | EVO-P1-7 | 结构化 Memory、Compaction 与多 Agent 权力分离 | 🟡 部分完成 | shared canonical event/projection/snapshot、四层规则、semantic 多角色 gate、promotion-only procedural、human-only policy、tenant-bound branded actor、digest-bound 增量 state、八类 compaction 保真及失败 rollback 已落地；12/12、session-core 531/531 通过；详见 §1.1、§6.7 | 接入真实持久 event/snapshot adapter、PostCompact hook 和 critic/evaluator/promotion/policy receipt branded resolver |
