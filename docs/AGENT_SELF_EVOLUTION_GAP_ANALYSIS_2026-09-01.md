...
本批次开始实现 EVO-P1-7 结构化 Memory 与权力分离：共享 `session-core` 新增 canonical append-only memory event/projection/snapshot，明确 `episodic/semantic/procedural/policy` 四层。episodic 只允许首次追加或 governor tombstone；子 Agent/Proposer 的 semantic 事实只进入带 evidence 的提炼队列，必须由独立 critic+evaluator receipt 和 governor 才能接受，且接受时不可替换 content/artifact/evidence lineage；procedural 只接受 promotion-controller 与 promotion receipt；policy 只接受非自动 human governor 与 policy receipt。memoryId 不得跨层复用，metadata 拒绝 raw content/secret，增量投影 state 必须由上一 projection digest 绑定。Compaction 强制保留 requirements、decisions、open risks、failed attempts、tests、goal state、delegated tasks 和 memory lineage；`PostCompact` verifier 或持久确认失败/抛错均保持并返回上一快照。新模块 11/11，`session-core` 全量 530/530 通过。真实持久 event/snapshot adapter、生产 composition root 对角色端口的不可替换绑定与真实 PostCompact hook 接线仍未完成，因此 P1-7 保持部分完成。
...
| P1 | EVO-P1-1、P1-3、P1-8、P1-9 已完成，P1-2 仓库闭环，P1-7 控制协议部分完成；其余主能力仍待验收 | shared EvolutionRun 已统一 CLI/Desktop/Graph 的 Raw/Wiki/Registry/Eval 投影；Wiki Maintainer 已闭环认证 evidence、确定性 pattern/index/log/impact、生命周期、ArtifactPorts 不可变 revision 与真实 Ledger CAS/event/reopen；结构化 Memory 已具备四层写入规则、多角色 receipt gate 与可回退 compaction；SkillInvocationReceipt 与有界评分循环已闭环 | 生产目标矩阵 Eval、统一 Registry/Agent ingress、Memory 持久 adapter/production composition root 仍未完成；真实流量接入归 P2 |
...
| EVO-P1-7 Memory 与多 Agent 权力分离 | 控制协议部分完成 | shared canonical event/projection/snapshot 已强制 episodic/semantic/procedural/policy 四层；child/proposer→critic+evaluator→governor、promotion-controller、human policy governor 权限路径与 receipt/content/lineage 绑定已落地；增量 state 绑定上一 projection digest；compaction 强制八类保真字段并在 PostCompact/persistence failure 时恢复上一快照；11/11、session-core 530/530 通过 | 接入真实持久 event/snapshot adapter 与 PostCompact hook；由 production composition root 强制不可替换的 proposer/critic/evaluator/governor/promotion/policy authority |
...
总计 20 项：**6 项已完成、1 项仓库闭环、6 项部分完成、7 项待完成**。计入本次 EVO-P1-7 控制协议后共有 **28 个基础批次**。这两个计数维度不能混用：基础提交数量不代表路线项完成数量。
...
| P1 | EVO-P1-7 | 结构化 Memory、Compaction 与多 Agent 权力分离 | 🟡 部分完成 | shared canonical event/projection/snapshot、四层写入规则、semantic 提炼队列与 critic+evaluator+governor gate、promotion-only procedural、human-only policy、digest-bound 增量 state、八类 compaction 保真及 PostCompact/persistence failure rollback 已落地；11/11、session-core 530/530 通过；详见 §1.1、§6.7 | 接入真实持久 event/snapshot adapter、PostCompact hook，并由 production composition root 强制 branded proposer/critic/evaluator/governor/promotion/policy authority |
