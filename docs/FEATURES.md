...
**Last Updated**: 2026-06-12 (v5.0.3.106)
...
- **146 built-in skills** with handlers in `main/ai-engine/cowork/skills/builtin/`
- **/skill commands** parsed via `skills-ipc.js`
- **Parser**: `skill-md-parser.js` (YAML frontmatter + Markdown body)

#### Built-in Skills Overview (146 total)
...
| 个人数据中台（PDH）      | v5.0.3.101+ | 2 | pdh-im-collect（微信 4.0 / QQ-NT 一键 IM 采集）, pdh-android-collector（安卓本地采集器构建运行手册）                                                                                                                                                                                                       |
...
> 注：上表 13 组 builtin 目录小计 = 137 + 「Agent/研究/协作补充」7 + 「个人数据中台」2 = **146** 个 `builtin/*/SKILL.md`（与磁盘一致）。`video-editing`（标 `1*`）是 CutClaw CLI/IPC 技能，**没有独立 `builtin/` SKILL.md 目录**，单列展示，不计入 146。
...
| `agent`    | —                                              | Agentic AI session with 10 tools + 146 skills + Plan Mode + Sub-Agent Isolation (headless)|
| `skill`    | list, categories, info, search, run            | 146 built-in skill management (headless)                           |
...
### 1. 概述
见正文头部。本文：ChainlessChain Feature Index。
