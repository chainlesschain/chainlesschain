...
**Last Updated**: 2026-06-10 (v5.0.3.104)
...
| Skills System (145 built-in)  | `main/ai-engine/cowork/skills/index.js`, `builtin/`                      | —                                                                                  |
...
- **145 built-in skills** with handlers in `main/ai-engine/cowork/skills/builtin/`
- **/skill commands** parsed via `skills-ipc.js`
- **Parser**: `skill-md-parser.js` (YAML frontmatter + Markdown body)

#### Built-in Skills Overview (145 total)
...
| 个人数据中台（PDH）      | v5.0.3.101 | 1 | pdh-im-collect（微信 4.0 / QQ-NT 一键 IM 采集）                                                                                                                                                                                                       |
...
> 注：上表 13 组 builtin 目录小计 = 137 + 「Agent/研究/协作补充」7 + 「个人数据中台」1 = **145** 个 `builtin/*/SKILL.md`（与磁盘一致）。`video-editing`（标 `1*`）是 CutClaw CLI/IPC 技能，**没有独立 `builtin/` SKILL.md 目录**，单列展示，不计入 145。
...
| `agent`    | —                                              | Agentic AI session with 10 tools + 145 skills + Plan Mode + Sub-Agent Isolation (headless)|
| `skill`    | list, categories, info, search, run            | 145 built-in skill management (headless)                           |
