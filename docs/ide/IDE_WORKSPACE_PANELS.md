...
Electron 桌面客户端即可在 VS Code / VSCodium 中打开页面。JetBrains 也提供
原生模型配置表单，可从聊天工具栏的 LLM 菜单或 Tools 菜单进入。
...
[启动指南](../features/evolution-workbench/EVOLUTION_WORKBENCH_STARTUP.md)。`local-test` 始终显示测试标识；
不代表已接入真实组织身份或真实审批服务。
...
VS Code / VSCodium `0.37.91`、JetBrains `0.4.118` 源码候选版将保存与连接测试
分别显示。保存操作必须重新读取 CLI 配置并确认字段一致，才会报告成功。
JetBrains 提供“保存配置”“重新加载已保存配置”和“测试已保存连接”三个操作；
有未保存修改时测试按钮不可用，视觉模型留空会清除旧覆盖值。

判断是否保存成功时，先重新加载已保存配置，再关闭并重新打开表单检查文本/视觉
模型和地址。密钥栏始终留空，只显示是否已有密钥；留空不代表原密钥丢失。
如果提示“保存结果未确认”，先重新加载检查，避免反复提交。连接测试失败也不
代表配置未保存；CLI `0.166.36` 源码候选版修复了推理模型因测试输出额度过小而
误报失败的问题，该版本仍须通过发布矩阵后才能作为公开升级版本。

JetBrains 已确认保存的配置会在当前已提交任务结束后、下一次请求开始前应用到
已有聊天页，并保留会话 ID。关闭配置窗口不会取消已经提交的保存操作；仅关闭
尚未保存的表单则丢弃修改。
