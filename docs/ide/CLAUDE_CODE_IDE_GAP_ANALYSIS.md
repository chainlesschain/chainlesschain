...
- ~~agent 一等工具化（`browser_state` 进 AGENT_TOOLS）为后续增量~~ ✅ 已落地（2026-07-10）：新 agent 工具 `browser_state`（extension tier，工具集 24→25），封装 `chrome-connector.captureState`——URL/title/tabs + watch 窗口 console/失败网络 + DOM 快照（agent 侧 cap 默认 40k，收紧于 CLI 命令的 150k）+ 可选截图（写入生成的临时路径，刻意不收 agent 指定路径防「读」工具覆盖任意文件）；策略 LOW/READ/plan-mode allow/read-only。9 项新单测（假 playwright 经 `_deps` 真 dispatch）+ 计数回归 5 处同步。详见 `docs/cli/CLAUDE_CODE_CLI_GAP_ANALYSIS.md` 第二十七批。
