...
> 文档定位：继承 `docs/research/ide/CLAUDE_CODE_IDE_GAP_ANALYSIS.md` 的增量审计，不重复 2026-07-10、2026-07-11 已完成的实施清单
...
后续批次又用双端一致的单文件 2 MiB、changeset 64 文件/8 MiB 预算关闭大文件/二进制/
聚合容量降级代码缺口；超限条目在原生文档创建前 fail-closed，返回不含正文的 skipped 元数据，
JetBrains 多文件统计也不再分配无界二维 LCS。随后双端 `openMultiDiff` 已支持逐项
modify/create/delete/rename/mode-change，独立守卫源与 rename 目标，使用不覆盖 create/
rename、真实 delete 和 POSIX chmod 语义；不支持 mode-change 的宿主结构化降级，混合批次
逐项回传成功/失败且不静默文本覆盖。相关 C/T/H 代码缺口已关闭，当前只保留真实宿主 UI
矩阵验收。

同日，双端 Doctor 增加共享 JSON fixture 约束的运行时兼容性孪生纯核：主动探测
`cc --version`，把最低 CLI 版本、Bridge 状态和 workspace trust 汇总为唯一的
`可运行 / 可降级运行 / 需要修复` 结论。CLI 缺失、过旧或同名 `cc` 实为 GCC/Clang
时要求修复，Bridge 停止或 workspace 受限时明确降级原因；JetBrains 还展示插件和平台版本。
该兼容性结论的 C/T/H seam 已关闭，剩余真实版本矩阵、Node/Java 和离线缓存诊断。

随后两端为四个核心只读 IDE 工具接入共享 fixture 约束的 `cc-ide-context/v2`：匿名
workspaceId、document URI/version、dirty、权限来源和采集时间/新鲜度；多文档
diagnostics/editors 的每条记录保留各自 URI/version/dirty。metadata 是 additive，
宿主探测失败或旧 facade 未实现时原 payload 不变。后续批次又复用同一宿主探针，
...
- 主基线：`docs/research/ide/CLAUDE_CODE_IDE_GAP_ANALYSIS.md:19-66`。
- 产品工作流收口：`docs/research/ide/CLAUDE_CODE_IDE_GAP_ANALYSIS_2026-07-10.md:23-31`。
...
- `docs/research/ide/CLAUDE_CODE_IDE_GAP_ANALYSIS_2026-07-10.md:23-31`
- `docs/internal/ide-plugin-claude-code-gap-analysis-2026-07-11.md:12-29`
- `docs/research/ide/CLAUDE_CODE_IDE_GAP_ANALYSIS.md:25-43`
...
证据：`docs/research/ide/CLAUDE_CODE_IDE_GAP_ANALYSIS.md:57-61`。
...
每个环境验证连接、重连、Selection、Diagnostics、Terminal、`@mention`、Diff、审批、取消、Resume、中文/空格路径、Symlink、Multi-root 和网络闪断。验收口径见 `docs/research/ide/CLAUDE_CODE_IDE_GAP_ANALYSIS.md:125-149`。
...
- `docs/research/ide/CLAUDE_CODE_IDE_GAP_ANALYSIS.md:62-63`
...
- Rename、Delete、Mode Change、Create/Delete 混合 Changeset：双宿主显式生命周期和 changeset/mode-change 组合语义已完成，剩真实宿主 UI 矩阵验收。
...
测试 `agent-tool-admission.test.js` 13（tier 归一 fail-closed + extension 五闸单独/穷举 + 未知 tier=extension + mvp 仅 policy+budget + 归因记录/否决 unmet + 无 token 单行 + 宿主环境契约解析/限长/限工具数/非决策字段剥离），另有 `agent-core-tool-admission.test.js` 3 项运行时断言。2026-07-22 已接入 `agent-core.executeTool` / `agentLoop` 的可选 `toolAdmission: { enforce: true, ...signals }` 会话准入 seam：Extension Tier 在执行前收集 capability/policy/permission/budget/UI 未满足项并 fail-closed，成功或拒绝结果均带无 token `toolAttribution`（含 `toolCallId`）；headless/stream runner 和子 Agent 会透传该策略。VS Code 与 JetBrains 聊天宿主现已在启动子进程时注入有界、无秘密的 `CC_TOOL_ADMISSION`（含宿主来源、五闸信号及 `publish_artifact`/`notify` 显式拒绝覆盖）；CLI 对非法 JSON、非布尔决策、非法工具名、超限信封一律以配置错误拒绝启动，不能静默退回无治理会话。未提供该环境变量时仍保持兼容的既有路径；MCP 工具是否真实存在继续由运行时 descriptor loader 与 managed policy 决定，宿主信号不会凭空启用已禁用或不可达的 server。
...
**已接线（2026-07-13 续③，`renderProtocolDoc` → CI byte-diff 签入副本落地）**：上一笔的剩项之一「renderProtocolDoc 接进 CI byte-diff 签入副本」现收口，照搬既有 [`gen-cli-reference.mjs`](../../../packages/cli/scripts/gen-cli-reference.mjs) 的「单源生成 → 签入副本 → CI byte-diff」范式：新增生成器 [`gen-protocol-doc.mjs`](../../../packages/cli/scripts/gen-protocol-doc.mjs)（`--out` 写 / `--check` 逐字节比对、drift → exit 1 提示 `npm run docs:protocol` 重生成 / 默认打印），从**唯一**源 [[capability-manifest.js]] `renderProtocolDoc()` 投影；两个 npm 脚本 `docs:protocol`（重生成）/ `docs:protocol:check`（drift-check）；签入生成物 [`docs/cli/PROTOCOL_CAPABILITY_MANIFEST.generated.md`](../../cli/PROTOCOL_CAPABILITY_MANIFEST.generated.md)（协议版本 + 可协商 wire 特性表 + 权限模式；放 `docs/cli/` 而非 prettier 托管 + `.md`-ignored 的 `packages/cli/`，故 formatter 不会重排、byte 比对恒有效）。**CI byte-diff 门**=`capability-manifest.test.js` 新增 1 断言（读签入副本 `expect(committed).toBe(renderProtocolDoc())`），跑在测试套件内故 drift **不可能 merge 绿**——manifest 加 v2 wire 字段但没跑 `docs:protocol` 重生成即测试红。round-trip 实证：clean `docs:protocol:check` → exit 0 / 篡改副本 → exit 1 / 还原 → exit 0；capability-manifest 16→17 绿。剩项（覆盖率指标 / 离线协议回放）仍开放。

**已落地（2026-07-13 续④，离线协议回放纯核 + 离线工具）**：剩项之二「离线协议回放」现收口为纯逻辑模块 [`protocol-replay.js`](../../../packages/cli/src/lib/protocol-replay.js)。[[event-seq-replay.js]] 回放是为**传输可靠性**（丢包重发），本模块回放是为**协议兼容**——拿一段录制的 stream-json 会话 + 一个协商上下文（server offer × client offer），**无需活体对端**离线回答两问：①**REPLAY**：这些帧在**本协商**下的线上形态是什么（未通过协商的特性字段被剥离，与活体 emitter 的 field-gate 剥离一致）；②**AUDIT**：一段录制是否**遵守**某协商——任何帧是否携带协商已 gate-OFF 的可协商 wire 字段（vN-only 客户端绝不该收到 vN+1 字段，违反即兼容 bug，本模块确定性、离线、可入 CI 抓）。可 gate 的 wire 字段集（`{seq, trace_id, tool_use_id}`）由**唯一**源 [[capability-manifest.js]] `toFieldGate()` 投影、不会漂移；非协商字段（type/content/name…）恒透传。**fail-closed**：协商 `ok:false` → gate 空 → 每个可 gate 字段一律 OFF。`sessionDigest` 键序无关 sha256 → golden 录制可 pin，任何改动协商/gating 而改变线上形态的变更即测试红（延续单源防漂移哲学）。

- 纯核导出：`gateFromNegotiation` / `replayFrame` / `replaySession`（回放到线上 + digest）/ `auditRecordedSession`（不剥离、报违规、fail-closed）/ `sessionDigest`。
- **离线工具** [`replay-protocol.mjs`](../../../packages/cli/scripts/replay-protocol.mjs)：读 NDJSON（或 JSON 数组）录制会话 + 协商 flag（`--server/client-version/-min/-features`），`--replay` 打印线上帧 + digest / `--audit` 报兼容违规并 exit 1。签入 golden fixture [`__tests__/fixtures/protocol-session.golden.ndjson`](../../../packages/cli/__tests__/fixtures/protocol-session.golden.ndjson)。
- live 实证：full 协商 audit → 0 违规 exit 0；降档 client（v1 只留 `event_seq`）audit → 6 违规（4 trace_id + 2 tool_use_id）exit 1。测试 `protocol-replay.test.js` 17（gate 三态含 fail-closed + replayFrame 剥离/不变异/非对象透传 + replaySession full/降档/空输入 + audit 遵守/违规带 index+field+type/incompat 全违规/跳过非对象 + golden 全能力字节等同&降档剥 5 字段 + digest 键序无关/值敏感）。capability-manifest/negotiation 全套回归 58 绿。剩项（覆盖率指标）仍开放。

**已落地（2026-07-13 续⑤，治理覆盖率指标纯核 + 离线报告工具 — P1-9 收官）**：最后一项「覆盖率指标」现落为纯逻辑模块 [`governance-coverage.js`](../../../packages/cli/src/lib/governance-coverage.js),把 §11.3 两条验收指标变成可计算、可 CI 门控的度量:①**高风险 Tool Call Ledger/Trace 覆盖率**——每个高风险(不可逆副作用)工具调用必须**既有** side-effect-ledger 记录**又有** trace span;②**Plugin/MCP/Skill/Hook 来源、版本、权限可追溯率**——每个外部来源的工具/技能调用必须携带完整溯源(source + version + scope + 已记录的权限决策)。

- **不重定义「高风险」**:直接复用 [[side-effect-ledger.js]] `classifyToolSideEffect`(**唯一**源:write/edit/notebook→file-write、shell/code→shell、仅 `git push`、publish/schedule/notify/browser_act→network-mutation;只读/本地可恢复→null),故指标与账本实际记录的东西不会漂移。溯源行=[[agent-tool-admission.js]] `buildToolAttribution` 记录。
- **纯核导出**:`isHighRiskToolCall` / `computeSideEffectCoverage`({toolCalls, ledgerOpIds, tracedOpIds}→highRiskCount + ledger/trace 覆盖率 + **具体未覆盖项** uncoveredLedger/uncoveredTrace,gap 可操作而非只给数字)/ `computeProvenanceCoverage`(仅**外部来源/extension-tier** 入分母,内置 mvp 无 source 排除;缺 source/version/scope/permission 列进 missing)/ `computeGovernanceCoverage`(两指标合一,`ok` 仅在双 100% 才真,fail-closed)/ `summarizeGovernanceCoverage`(无 token 单行)。空总体→真空满覆盖(ratio=1)。
- **离线工具** [`coverage-report.mjs`](../../../packages/cli/scripts/coverage-report.mjs):读 run-summary JSON(toolCalls/ledgerOpIds/tracedOpIds/attributions),打印两指标 + 逐条 gap,**任一 gap → exit 1**,可对录制 run 工件在 CI 强制 §11.3「100%」。
- live 实证:全覆盖 → `ledger 100% / trace 100% · provenance 100% · OK` exit 0;ledger+溯源双 gap → `ledger 50% / trace 50% · provenance 0% · GAPS` 逐条列出(no ledger entry / no trace span / incomplete provenance missing …)exit 1。测试 `governance-coverage.test.js` 11(高风险判定复用分类器 + 副作用覆盖 100%/ledger gap fail-closed/无 opId 永不覆盖/无高风险真空满 + 溯源全可追溯/内置排除/缺项列 missing + 合并 ok 仅双 100%/summary 无 token 带百分比计数)。**P1-9 六项(单源 negotiator+`--capabilities` / 脱敏诊断包 `cc doctor --export-bundle` / renderProtocolDoc CI byte-diff / 离线协议回放 / 覆盖率指标)全部收官**;唯一长期项=覆盖率指标接进真实 run 的 ledger/span/attribution 采集面(需 headless/WS 热路径采集,与 §8.1 长期联动)。
...
证据：`docs/research/ide/CLAUDE_CODE_IDE_GAP_ANALYSIS.md:28,62-66,194-200`。
...
**已接线（2026-07-13 续，Trace 导出面落定，三导出面全闸）**：第三个导出面 **Trace** 现补齐——[[span-recorder.js]] `toOtlp()` 是 `cc agent --otlp` / `cc team run --otlp` / `cc eval` 把 span 导给 OTel collector（可能是外部/第三方）的边界，span attribute（工具 `command` 属性、重定向 URL）与 exception message（`recordException` 写进 event）都是 `stringValue`，会夹带 provider token / JWT / 连接串。`toOtlp({redact=true})` 默认对每个 span/event 的 `stringValue` 过 `redactSecrets`（数字/布尔属性、traceId/spanId 结构字段不动）；仅真秘密子串被替换故无秘密的 span **逐字节不变**，三个调用点全走无参 `.toOtlp()` = 默认脱敏；`{redact:false}` 供完全可信的本地 collector 保原值。测试 `telemetry-recorder.test.js` +2（默认脱敏 span attr + exception msg 双清且非秘密属性字节不变 / `redact:false` 保原值）；agent-core-telemetry·eval-trend·team-io 29 回归绿。**8.1 三导出面（Transcript / Diagnostic-Bundle / Trace）全部强制脱敏闸完成**；剩项=与覆盖率指标长期联动。

证据：`docs/research/ide/CLAUDE_CODE_IDE_GAP_ANALYSIS.md:51-52,117-123`。
...
证据：`docs/research/ide/CLAUDE_CODE_IDE_GAP_ANALYSIS.md:36,52-54`。
...
>
> - **12.1** `docs/research/ide/CLAUDE_CODE_IDE_GAP_ANALYSIS.md:8,74` 版本号 `0.37.4`/`0.4.46` → `0.37.14`/`0.4.58`。
> - **12.2** `docs/research/ide/CLAUDE_CODE_IDE_GAP_ANALYSIS_2026-07-10.md:306,318` 的 managed CLI「刻意不做」标为历史决策并划除，记录 0.37.13/0.4.57 已发布的 Managed CLI runtime（按需 npm 下载到全局存储 + sha512 + 回滚 + 显式 path 不被替换）。
> - **12.3** `docs/research/ide/CLAUDE_CODE_IDE_GAP_ANALYSIS.md:64-66,196-197` 改写为「trace id 传播已落地，剩项=脱敏诊断包/覆盖率/离线回放」，消除「trace 未贯穿」的误读。
...
`docs/research/ide/CLAUDE_CODE_IDE_GAP_ANALYSIS.md:8-9,74-82` 仍出现 VS Code `0.37.4`、JetBrains `0.4.46`；同文 `19-23` 和 Changelog 已是 `0.37.14`、`0.4.58`。历史章节不应继续作为当前版本事实源。

### 12.2 Managed CLI 历史决策过期

`docs/research/ide/CLAUDE_CODE_IDE_GAP_ANALYSIS_2026-07-10.md:304-318` 曾判定 managed/private CLI“不做”，但后续已实现：
...
`docs/research/ide/CLAUDE_CODE_IDE_GAP_ANALYSIS.md:28` 已确认 Trace 字段贯穿；`64-66` 应改写为脱敏诊断包、覆盖率、协议回放和真实验收，避免笼统写“Trace 尚未贯穿”。
