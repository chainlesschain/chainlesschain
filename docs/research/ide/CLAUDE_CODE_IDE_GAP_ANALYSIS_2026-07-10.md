...
- 已收口：Diff Review UX（P0 #4）。逐 hunk/逐文件裁决、diff 内 request changes、JB native diff viewer 历史已落地但一直缺状态标注；2026-07-10 审计补齐两个真缺口——JetBrains 右栏改可编辑并在 Accept/Pick hunks/Request changes 全路径回读用户修改（原来静默丢弃），CLI accepted 结果新增 `userAmendments` 行级 -/+ 摘要（agent 原来只见 `userEdited` 布尔）；JB 批注新增 `12:`/`12-15:` 行锚点前缀（`ReviewNote` 纯核）。详见 §4。
- 已全清（2026-07-10 尾批）：本文档剩余的全部非 CLI-blocked 项——#2 配对 URI 的 IDE 内 QR 渲染（纯核 QR 编码器孪生 + 双端胶水）与 relay 设置面（双端设置 → `--relay-url`/`--peer-id`）；#5 行范围/PDF 页码 mention 语法经审计确认早已落地（`@file#L12-40` / `@file.pdf#1-3`，stale 记录已更正）；#8 `browser_state` 一等 agent 工具（工具集 24→25）。至此 14 项中除明确标注 CLI-blocked / product-blocked / 刻意不做的判定项外，无遗留工作项。
...
- **配对 URI 的 IDE 内 QR 渲染** ✅：两端插件都不带 QR 依赖（vsix 无 node_modules / JB zip 无 QR 库），新建纯核 QR 编码器孪生（`vscode-extension/src/qr-code.js` + `com.chainlesschain.ide.QrCode`；byte mode / ECC M / 自动版本 1-40 / ISO 18004 掩码择优）。正确性=独立解码器 jsQR 逐用例 decode round-trip 验证 + 与参考 npm `qrcode` 库交叉核对（掩码相同的用例矩阵逐字节一致）；两端断言同一组 fixture（行 hex + SHA-256）保证双端渲染完全一致。胶水：VS Code 配对通知加「Show QR」+ 菜单「Show pairing QR」（无脚本静态 webview，两主题下均黑白呈现，宿主停止/意外退出即关闭防止过期一次性 URI 残留可扫；URI HTML 转义）；JetBrains 配对对话框顶部渲染 QR（BufferedImage）+ 运行中菜单「Show pairing QR」可重开。超长 URI 超出 QR 容量时退回复制引导。测试：JS 8 项（v1/v12/v21 多块 fixture、结构不变量、容量 null、SVG 形状、webview 转义/降级）+ JB QrCodeTest 5 项 + smoke 795/0。
- **relay（E2EE 跨网）配置的 IDE 设置面** ✅：VS Code 设置 `chainlesschain.remote.relayUrl` / `.peerId`（en+zh 描述，每次 start 时读取即改即生效）；JetBrains Remote Control 对话框加「Relay settings…」（两字段输入，PropertiesComponent 应用级持久化，start 对话框显示当前配对模式）。纯核孪生 `buildRemoteControlStartArgs` 增可选 `{relayUrl, peerId}` → `--relay-url`/`--peer-id`；空白值不传 flag（清空的设置回落 CLI 自身 `CC_REMOTE_SESSION_RELAY_URL` → `remoteControl.relayUrl` 解析链，flag 存在时按 CLI 优先级胜出），peerId 与 relayUrl 相互独立（relay 可来自 env/config）。测试：VS remote-handoff 10（新 relay flag 矩阵）+ JB RemoteHandoffTest 9/0。
...
状态：已收口（2026-07-10 审计 + 补齐；此前多轮增量落地但本条一直缺状态标注）。
...
- 逐文件 / 逐 hunk accept/reject（双端）：VS Code `openDiff` 四路裁决（Accept / Request changes… / Pick hunks… / Reject）+ `openMultiDiff`（Accept all / Pick files… / Reject）；JetBrains 同构四路 + Choose files…。hunk 引擎为纯核孪生（VS `diff-hunks.js` / JB `DiffHunks.java`，LCS + 尺寸守卫，全选==modified / 全不选==original 不变量成对测试）。
- diff 中 request changes（双端）：批注不落盘、经 `changes-requested` verdict 回喂 agent 修订重提（CLI `formatReviewComments` 渲染行锚点反馈）。
- CLI 协议全链：Edit/Write 的权限 ask 在交互会话 + IDE bridge 时路由为编辑器阻塞式 openDiff 审阅（`tryIdeDiffApprovalForEdit`），accepted=IDE 已写文件跳过工具自身执行 / rejected=拒 / changes-requested=批注回喂；`CC_IDE_DIFF_APPROVAL=0` 逃生门。
- 原生渲染确认：VS 用 `vscode.diff`/`vscode.changes`；**JetBrains 全程 `DiffManager` + `SimpleDiffRequest(Chain)` 原生 diff viewer，diff 路径零 HTML 渲染**（第 4 子项本已满足，仅缺记录）。

本轮补齐（2026-07-10 审计发现的两个真缺口 + 一个半缺口）：

- **JetBrains 审阅中编辑丢失（缺口）**：右栏原为只读（`factory.create`），Accept 写回的仍是原始 `modifiedText`，用户在 native viewer 里的修改被静默丢弃。修=右栏改 `createEditable`，裁决后回读右栏文档；未动时保留 CLI 的 byte-exact 原文（diff 文档会归一化行分隔符），动过则用户版本胜出——Accept 写回/`finalText`、Pick hunks 的目标、`reviewedText` 全部以回读文本为准（VS Code 契约对齐）。
- **agent 只见 `userEdited` 布尔（缺口）**：CLI accepted 分支原来只标 `userEdited: true`，agent 看不到用户具体改了什么（其对文件的心智模型仍是自己的原提案）。修=新纯核 `summarizeUserAmendments`（proposal→finalText 的 -/+ 行级摘要，按 final 文件 1-based 行锚定分块；复用 `simpleDiff` LCS + 1M cell 守卫降级粗块 + 行数/字符双 cap），随工具结果 `userAmendments` 字段回喂。
- **JetBrains 批注无行锚点（半缺口，VS 有）**：JB 批注走 diff 关闭后的模态输入，没有 VS 的“当前选区”可用——锚点改随文本走：`12:` / `12-15:` 前缀（1 基，全角冒号也认），新纯核 `ReviewNote.parse` 解析为 VS 同形 `{line, endLine, lineText, note}`（0 基 + lineText 取自回读文本）；越界/无效前缀降级为整体批注绝不造假锚。提示语进 CcBundle en/zh。

测试：CLI `ide-diff-approval.test.js` 31 项（新增 summarize 5 组 + userAmendments 断言）+ ide-context/interop 115 项；JB `ReviewNoteTest` 11 项 JUnit + PureLogicSmokeMain 795 断言（新增 reviewNote 20 条）+ SDK 编译验证。

边界（记录判定）：

- multi-diff 各栏为只读（整 changeset 的按文件 accept/reject），in-place 编辑只在单文件 openDiff——与 VS Code 行为一致，不另造。
- JB 行锚点是文本前缀约定而非选区捕获——JetBrains 的模态审阅流下没有稳定的“diff 关闭后选区”API 面；前缀语法可测、可回放，且 CLI 侧渲染两端同形。
...
- ~~文件行范围 / PDF 页码 mention 语法~~ ✅ 审计发现早已落地（2026-06-16/06-30 landed，本条一直是 stale 记录）：CLI 端 `file-ref-expander.js` `parseLineRange` 解析 `@file#L12-40` / `#L12` / `#12-40`（PDF 走 `@file.pdf#1-3` 页码区间），expander 单测 37 项绿；IDE 端双端有选区→引用命令（VS `chainlesschain.chat.insertReference`（`insert-reference.js` 纯核 12 测）/ JB `InsertFileReferenceAction`（`Mentions.formatInsertReference`，MentionsTest 覆盖）），发出 `@path#L5-10` 形式；手工输入同语法 CLI 直接展开。
...
- ~~agent 一等工具化（`browser_state` 进 AGENT_TOOLS）为后续增量~~ ✅ 已落地（2026-07-10）：新 agent 工具 `browser_state`（extension tier，工具集 24→25），封装 `chrome-connector.captureState`——URL/title/tabs + watch 窗口 console/失败网络 + DOM 快照（agent 侧 cap 默认 40k，收紧于 CLI 命令的 150k）+ 可选截图（写入生成的临时路径，刻意不收 agent 指定路径防「读」工具覆盖任意文件）；策略 LOW/READ/plan-mode allow/read-only。9 项新单测（假 playwright 经 `_deps` 真 dispatch）+ 计数回归 5 处同步。详见 `docs/research/cli/CLAUDE_CODE_CLI_GAP_ANALYSIS_2026-07-09.md` 第二十七批。
