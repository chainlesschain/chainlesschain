...
- `getSelection/getActiveFile/getDiagnostics/getOpenEditors` 已在真实 VS Code/JetBrains facade 上附加共享 fixture 约束的 `cc-ide-context/v2`：匿名 workspaceId、document URI/version、dirty、权限来源和采集新鲜度；多文档记录保留各自 URI/version/dirty，旧宿主无 metadata 时原 payload 不变。后续批次又将同一 metadata 接入 `getHover/goToDefinition/findReferences/renamePreview/getCallHierarchy/getSymbolInfo/getProjectModel` 七个 LSP/PSI 语义工具；位置型工具锚定查询源文档，项目模型返回 workspace 级 context，探针异常时保持旧 payload。`openDiff/openMultiDiff` 也已在审阅完成后返回源/rename 目标或逐 changeset 的 document context。四核心只读、语义/项目模型和 Diff 写入审阅的 C/T/H 已完成，测试结果/coverage、debug、Notebook 执行和未来写 API 仍需后续扩展。
...
本文件正文保留 2026-07-17 的历史审计快照，正文中关于“V3 未接线”、后台 Agent 与 worktree 互斥、VS Code 无 Extension Host、JetBrains GUI 未通过、ACL fail-open 以及仅输出本地 OTLP JSON 的表述，均不再代表当前代码状态。它们已分别由当前实现、针对性测试、VS Code Extension Host smoke、JetBrains GUI smoke、fail-closed ACL 政策和 OTLP HTTP/HTTPS exporter 覆盖；2026-07-22 又补上了 `agent-core.executeTool` / `agentLoop` 的会话级 Tool Admission 与 attribution seam，并由 VS Code、JetBrains 聊天宿主通过有界且 fail-closed 的 `CC_TOOL_ADMISSION` 环境契约传入真实宿主决策。两端 Plan Review 已增加 `cc-plan-review/v1` 版本化草稿/决策快照，按 session 在 workspace/project 状态中持久化并于重启后恢复活动审阅；2026-07-23 又将 Markdown item comment/Reviewer Notes 解析为 item/file/line/column/turn 结构化记录，并通过 `plan_item_id` 把真实工具开始/结束逐项回写执行进度。同日继续落地旧/新 plan ID 的 added/removed/changed Diff，以及包含权限模式、批准项和允许工具的不可扩大审批执行锁。VS Code 与 JetBrains 的 IDE 诊断导出 seam 也已关闭：两端经用户选定目标、私有同目录临时文件和 schema/隐私契约校验后才替换目标，无效产物不破坏旧文件。当前仍未完成的项目只以 2026-07-22 增量报告中的 C/T/H/E/D 状态为准。
...
`DiffHunks`。容量降级 C/T/H 已关闭。随后双端 `openMultiDiff` 已支持每项显式
modify/create/delete/rename/mode-change，独立守卫源与 rename 目标，并以不覆盖 create、
不覆盖 rename、真实 delete 和 POSIX chmod 执行混合批次；未知/畸形操作在打开原生文档前
结构化跳过，Windows 等不支持 mode-change 的宿主返回 content-free
`skippedUnsupported`，逐项成功/失败通过 `appliedOperations` / `failedOperations` 回传，
不会静默退化为文本覆盖。mode-change/混合生命周期 changeset 的 C/T/H 代码缺口已关闭，
Diff 只剩真实多宿主 UI 矩阵验收。
...
| 用量归因、Trace、诊断包、协议回放        | [`usage-report.js`](../../../packages/vscode-extension/src/usage-report.js)、[`span-recorder.js`](../../../packages/cli/src/lib/telemetry/span-recorder.js)、[`diagnostic-bundle.js`](../../../packages/cli/src/lib/diagnostic-bundle.js)、[`diagnostic-export.js`](../../../packages/vscode-extension/src/diagnostic-export.js)                  | 主体与双 IDE 导出已接线        |
...
1. 将以下文档标为历史基线，不再继续追加实现日志：
   ...
   - [`ide/CLAUDE_CODE_IDE_GAP_ANALYSIS.md`](./CLAUDE_CODE_IDE_GAP_ANALYSIS_2026-07-10.md)
