...
hunk 选择、行批注与最终写入，不保存代码正文。后续批次已完成 CLI 可信
session/turn/toolUse 绑定和 stream/headless/WebSocket 副作用账本持久化，审计元数据不进入
模型可见的工具结果。后续可恢复 tracker 已把下一次同 session/path 审阅的实际结果关联回
Request Changes 原记录，并为无重新提议/中断提供明确终态；`followUpRequested` 不再是唯一
证据。显式 rename/delete 意图也已通过
`delete_file` / `move_file`、`openDiff` operation/targetPath、双端独立路径守卫和不覆盖目标的
...
双端 Doctor 也已由共享 JSON fixture 约束的孪生纯核输出唯一
`可运行 / 可降级运行 / 需要修复` 结论：CLI 缺失、过旧或被 GCC/Clang 同名命令遮蔽时
要求修复，Bridge 停止或 workspace 受限时明确降级原因；JetBrains 同时展示插件和平台版本。
兼容性结论的 C/T/H seam 已关闭，真实版本组合、Node/Java 和离线缓存诊断仍保留。
随后，`getSelection/getActiveFile/getDiagnostics/getOpenEditors` 已附加双端共享 fixture
约束的 `cc-ide-context/v2`，统一匿名 workspaceId、document URI/version、dirty、
权限来源与采集新鲜度；多文档记录携带各自 URI/version/dirty，旧 facade 保持原 payload。
后续批次又把同一封套接入七个 LSP/PSI 语义工具：`getHover`、`goToDefinition`、
`findReferences`、`renamePreview`、`getCallHierarchy`、`getSymbolInfo` 和
`getProjectModel`。位置型工具记录查询源文档的 URI/version/dirty，项目模型使用
workspace 级 metadata；探针缺失或异常时不改变旧返回。四核心只读工具和语义/项目模型
的 C/T/H seam 已关闭。随后 `openDiff/openMultiDiff` 也在审阅完成后返回 Context v2：
单文件保留源文档 context，rename 另带目标 context；多文件带 workspace context 和按
输入顺序排列的源/rename 目标 `documentContexts`。Diff 写入审阅的 metadata seam 已关闭；
测试结果/coverage、debug、Notebook 执行和未来写 API 仍开放。
...
| 领域   | 已有能力                                                                                                                     |
| ------ | ---------------------------------------------------------------------------------------------------------------------------- |
| 发布   | VS Code 0.37.14 已发 Open VSX（官方 Microsoft Marketplace 未发，缺 `VSCE_PAT`）；JetBrains 0.4.58 已发 JetBrains Marketplace |
...
与协议录制回放均已完成代码和定向测试；双端 Runtime Doctor 也已把 CLI 下限、Bridge 和
workspace trust 汇总为共享 fixture 约束的三态兼容性结论。本节剩余目标是把
trace/ledger/attribution 的真实 run 采集继续贯穿模型、tool、approval、diff、checkpoint，
并完成真实版本矩阵、宿主 UI、发布门和长期运行验收。
