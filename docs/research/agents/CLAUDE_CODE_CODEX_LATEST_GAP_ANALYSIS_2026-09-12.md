...
> 后续实施：[第一批 G01/G02](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_IMPLEMENTATION_2026-09-12.md)、[第二批 G06 中文词法召回](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G06_IMPLEMENTATION_2026-09-12.md)、[第三批 G03 模型能力 Profile 基础层](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G03_MODEL_PROFILE_IMPLEMENTATION_2026-09-13.md)、[第四批 G04 App Server 接线前安全修复](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G04_APP_SERVER_SAFETY_IMPLEMENTATION_2026-09-13.md)、[第五批 G05 插件作者 Eval](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G05_PLUGIN_EVAL_IMPLEMENTATION_2026-09-13.md)、[第六批 G07 沙箱能力矩阵](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G07_SANDBOX_CAPABILITIES_IMPLEMENTATION_2026-09-13.md)、[第七批 G10 持久路径容量测量](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G10_PERSISTENT_CAPACITY_IMPLEMENTATION_2026-09-13.md)、[第八批 G08 非阻塞澄清](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G08_DEFERRED_QUESTIONS_IMPLEMENTATION_2026-09-13.md)、[第九批 G03 OpenAI Responses 主链](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G03_OPENAI_RESPONSES_IMPLEMENTATION_2026-09-13.md)、[第十批 G09 生产旅程验证](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G09_PRODUCTION_JOURNEY_IMPLEMENTATION_2026-09-13.md)、[第十一批 G11 远控安装就绪](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G11_REMOTE_INSTALL_READINESS_IMPLEMENTATION_2026-09-13.md)、[第十二批 G10 后台游标分页](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G10_BACKGROUND_PAGINATION_IMPLEMENTATION_2026-09-13.md)、[第十三批 G04/G05/G06/G08/G10 未完成项推进](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_CLOSURE_IMPLEMENTATION_2026-09-14.md)。下文保留审计时点结论，不将后续代码修改追溯为当时已有能力。<br>
...
> 决策口径更新：2026-09-15，将下一轮优先级、路线图与验收收敛到实际任务成功率、首次运行成功率和维护成本，见 1.3、4、8、9.3 和 12。本次为文档调整，未重新核验外部版本或执行真实任务评测；历史证据保留原日期与范围。
...
2026-09-12 审计确认的主要问题如下；后续工程交付见 1.2，下一轮决策以 1.3 和第 4 节为准：
...
未能运行；标签工作流在具备 JDK 21 的受控 runner 上已完成完整发布门禁，因此该
本地环境限制不影响上述发布结论。
...
| --- | ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| G01 | 阶段完成，待目标环境验证            | 已区分部署准入、命令级准入和实际任务就绪；setup、doctor、CLI 与 IDE 已接线；CLI `0.166.48` 与 VS Code `0.37.98` 已形成公开安装产物 | 真实身份及指定 OS/IDE 到真实模型任务的完整旅程                                 | [第一批 G01/G02](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_IMPLEMENTATION_2026-09-12.md)                                                                                                                                                                                                     |
| G02 | 核心实现及发布矩阵完成，待正式实测  | 已实现严格比较门，并分离执行成功、产物检查与证据完整性；`43c6bba51a` 的 `CLI CI` 与 `CLI Strict Sandbox` 三平台矩阵均通过 | 真实 provider 对照及 Windows 同等并行负载稳定性                                | [第一批 G01/G02](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_IMPLEMENTATION_2026-09-12.md)                                                                                                                                                                                                     |
| G03 | 三阶段完成，待生产验收              | 已交付版本化模型能力 Profile、OpenAI Responses 主链，以及独立 `reasoning_tokens` 归账                                  | 真实账号/模型认证、预算误差和精确 SHA 三平台旅程                               | [Profile 基础层](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G03_MODEL_PROFILE_IMPLEMENTATION_2026-09-13.md)、[Responses 主链](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G03_OPENAI_RESPONSES_IMPLEMENTATION_2026-09-13.md)                                                                                      |
| G04 | `0.154.0` 矩阵已接入，待正式旅程    | 已关闭终态/fallback 反例，将 `0.154.0` 加入 fail-closed 适配器与三平台 CI 定义，并完成 Windows 真实协议探针            | 干净精确 SHA 三平台 App Server 旅程及是否接入产品的独立决策                    | [G04 安全修复](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G04_APP_SERVER_SAFETY_IMPLEMENTATION_2026-09-13.md)、[本轮推进](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_CLOSURE_IMPLEMENTATION_2026-09-14.md)                                                                                                       |
| G05 | 统计/留出集及发布验证完成，待真实效果验收 | 已交付双臂报告、1–20 次重复采样、平衡顺序、95% 区间及 payload 外 reviewer holdout；对应 CLI 精确提交已通过发布门并公开发布 | 固定付费模型对照及独立模型 grader                                               | [G05 插件 Eval](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G05_PLUGIN_EVAL_IMPLEMENTATION_2026-09-13.md)、[本轮推进](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_CLOSURE_IMPLEMENTATION_2026-09-14.md)                                                                                                            |
| G06 | 混合召回接口完成，待真实效果验收    | 已保留中文词法基线，并增加 revision/digest 绑定、治理先行的宿主语义候选、模型请求候选源接线与 hybrid rerank            | 内置 embedding/index 来源、真实语料、跨语言/冲突记忆效果、容量及三平台验收     | [G06 中文词法召回](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G06_IMPLEMENTATION_2026-09-12.md)、[本轮推进](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_CLOSURE_IMPLEMENTATION_2026-09-14.md)                                                                                                                     |
| G07 | 能力合同完成，能力补齐中            | 已交付沙箱能力报告、配置前失败关闭及运行后 `applied` 证据                                                              | 不可绕过的域名级 egress，以及 macOS/Windows/特殊 stdio 组合的实机矩阵          | [G07 沙箱能力矩阵](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G07_SANDBOX_CAPABILITIES_IMPLEMENTATION_2026-09-13.md)                                                                                                                                                                          |
| G08 | standalone 恢复合同完成，待真实旅程 | 已为 Headless 持久化 deferred 生命周期，提供私有恢复投影、重连重放、严格 binding、一次性消费确认与真实 Node 子进程恢复 | 真实 CLI/IDE/App Server 客户端恢复及精确 SHA 三平台多宿主旅程                  | [G08 非阻塞澄清](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G08_DEFERRED_QUESTIONS_IMPLEMENTATION_2026-09-13.md)、[本轮推进](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_CLOSURE_IMPLEMENTATION_2026-09-14.md)                                                                                                    |
| G09 | 验收工具完成，真实旅程待执行        | 已交付 fail-closed 生产旅程验证器、受保护手动 attestation workflow 与回执合同                                          | 受信生产身份、真实 provider/KMS/witness、人工审批、cohort 观察及 rollback 演练 | [G09 生产旅程验证](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G09_PRODUCTION_JOURNEY_IMPLEMENTATION_2026-09-13.md)                                                                                                                                                                            |
| G10 | 分页/只读索引完成，容量验收待执行   | 已交付容量 harness、稳定游标分页，以及内容无关、文件身份绑定的后台 Agent 只读索引与权威全扫回退                        | 精确 SHA 三平台 formal 曲线、性能 SLO、持久 Memory 索引及归档策略              | [G10 容量测量](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G10_PERSISTENT_CAPACITY_IMPLEMENTATION_2026-09-13.md)、[G10 后台游标分页](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G10_BACKGROUND_PAGINATION_IMPLEMENTATION_2026-09-13.md)、[本轮推进](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_CLOSURE_IMPLEMENTATION_2026-09-14.md) |
| G11 | 安装就绪诊断与主渠道回读完成，真实旅程待执行 | 已扩展 Remote Doctor；Open VSX `0.37.98` 与 JetBrains Marketplace `0.4.123` 已完成发布回读，并 fail-visible 展示未知状态 | Microsoft VS Code Marketplace 可选回填、双设备连接/断网重连/撤销及成功率/耗时测量 | [G11 远控安装就绪](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G11_REMOTE_INSTALL_READINESS_IMPLEMENTATION_2026-09-13.md)                                                                                                                                                                      |
...
基于 `e1397ca6046b35c97d170b1768f5462642ba4818` 的当前工作树，本轮完成五项可本地闭环工作：G04 的 `0.154.0` 精确兼容矩阵与 Windows 真实协议探针、G05 的重复采样/95% 区间/reviewer holdout、G06 的 digest 绑定受治理语义候选接口及模型请求候选源接线、G08 的 standalone Headless deferred 权威事件/私有恢复投影与真实 Node 子进程恢复，以及 G10 的内容无关、文件身份绑定后台 Agent 分页只读索引。代码、边界与定向验证详见[第十三批实施记录](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_CLOSURE_IMPLEMENTATION_2026-09-14.md)。
...
### 1.3 下一轮只围绕三项结果决策（2026-09-15）

11 项均已有阶段性工程交付，但本文尚无按统一口径测得的三项指标基线。下一轮先回答“哪些用户任务仍失败、首次使用卡在哪里、每次修复需要维护多少处”，再决定新增能力。以下为拟采用的测量与决策规则，不是已取得的结果或生产承诺。

| 主指标 | 统一口径 | 必须同时报告 |
| --- | --- | --- |
| 实际任务成功率 | 在冻结任务集、时间和费用预算内，执行成功、独立产物验收通过且证据完整的任务运行数 / 全部计划任务运行数 | 成功数/总数、按任务和 OS/入口/model 分层的结果、失败归因、重试与人工补救次数；超时、阻断、未执行和证据缺失均不得计为成功 |
| 首次运行成功率 | 从干净环境按公开安装与配置指引开始，在预设预算内完成首个真实任务，且未接受额外人工修复的旅程数 / 全部计划首次旅程数 | 安装→配置→认证/部署→工具调用→产物验收各步流失数、端到端耗时、卡点；仅安装成功或 READY 不算成功 |
| 维护成本 | 固定观察窗口内，相关路径的故障修复、支持排障、兼容适配、发布维护和非功能测试故障排查总人时；同时报告每 100 次任务尝试的维护人时 | 原始总人时、任务量、分类工时、一次性建设成本；共享合同变更需修改的独立实现数、CI 非代码故障重跑数作为辅助指标 |

任务运行包含预设预算内的自动重试，不能把重试重新计为独立成功样本；人工修改产物或修复环境后的结果另列“补救后成功”，不回写原始结果。首次旅程允许按文档正常配置凭据、完成授权与提供必要信息，额外人工修复指文档外排障、改配置或改代码。所有重试、失败和补救消耗均进入成本记录。

试点先冻结 30–50 个真实项目任务，覆盖缺陷修复、跨文件修改、测试补齐、中文记忆辅助任务和中断恢复；同时为承诺支持的 OS/入口组合登记首次旅程。模型任务评测与首次安装评测分开统计，避免只在已配置环境测任务成功率而漏掉安装流失。样本和权重在运行前固定；不支持的组合事前列出，不能在失败后移出分母。

### 1.4 收敛规则

- **先测后扩。** 每项工作必须关联一个已复现失败或已测成本，注明目标指标、负责人、现有组件、验收证据和停止条件；没有基线时先建立基线。
- **只保留一个核心判定来源。** readiness、模型能力、执行终态、评测证据和记忆权限继续由共享层定义，CLI/IDE 负责输入与展示。新增适配需说明现有路径不能满足的具体任务，以及后续兼容维护责任。
- **按失败影响排序。** 首次任务阻断、错误成功状态、权限越界和重复副作用优先关闭；其他工作按受影响任务占比、预计收益、实现及持续维护人时排序。收益尚未实测时标为假设。
- **有证据才扩大。** 每批只推进一个主要失败原因的修复及必要验证。真实对照未显示收益、维护负担超过事前预算或关键分层回归时，保持现有默认路径，暂停扩展并记录结论。

### 1.5 三指标基线统计入口实施（2026-09-15）

已新增[只读统计入口及使用说明](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_OUTCOME_METRICS_IMPLEMENTATION_2026-09-15.md)，复用现有 Eval 执行/产物/证据判定，按冻结计划统计未运行样本、首次运行各阶段、失败与重试费用及维护工时。计划变更、重复证据、人工补救、错误版本、窗口外记录和未知成本均有独立处理；完整输入只标记 `BASELINE_RECORDED`，不会产生效果提升 PASS。

本地验证：Windows 定向 10 文件、189 项测试全部通过，ESLint、JSON/JSONL 证据解析与差异检查通过；该结果仅证明统计、费用归因、模型配置和证据合同，不计入真实任务成功率。

本批随后使用本机火山模型执行了[Windows CLI 小规模真实任务试点](../../cli/evidence/outcome-metrics/2026-09-15/README.md)：修复通用 Claude settings 模型错误覆盖 Volcengine 配置的问题后，两次可比运行共 20/20 个内置编码任务成功，无关改动率 0%，耗时 p50 29.798 秒、p95 68.692 秒，总记录费用 USD 0.0672595；严格本地 Eval 门通过。首轮 10/10 用于发现并补齐费用归因，因无费用字段不计入上述成本聚合。

该试点绑定本地 dirty source fingerprint 和操作者声明的 endpoint identity，不是发布产物或生产 attestation。干净公开安装、预先冻结的 30–50 个真实项目任务、多平台/IDE 分层及维护观察窗口仍缺证据，G01–G11 的真实验收边界保持不变。
...
## 4. 按结果重新排序（2026-09-15）

本节依据 1.2 的阶段性交付记录安排下一轮，不重新认定工程状态。P0 为基础旅程阻断及证据/安全门，P1 为已测高频失败的修复，P2 为有收益证据后再扩展的能力。条件性 P0 只在对应接线、晋升或规模承诺前生效。

| 顺序 | GAP 与主指标 | 下一步交付 | 扩展或退出条件 |
| --- | --- | --- | --- |
| 先做：P0 | G01、G02、G09 的基础真实旅程；首次运行与实际任务成功率 | 用已发布产物建立干净安装到真实任务的基线，复用现有严格 Eval，定位首个阻断环节 | 有逐旅程证据、分层成功数/总数；错误成功状态关闭，最大阻断原因有可复现修复；不以生产演化试点作为普通任务前置条件 |
| 随失败排序：P1 | G03、G06；实际任务成功率 | 在现有 Profile/Responses、词法/混合召回接口上验证真实任务，修复已测模型调用、预算或召回失败 | 固定任务对照显示收益；不因模型或检索名称增加而视为进展 |
| 按关键路径处理：P1 | G07、G08；实际任务成功率与维护成本 | 对承诺支持组合完成隔离、取消及恢复旅程，核对跨宿主终态 | 越权、错误成功和重复副作用为硬阻断；体验扩展由失败占比决定 |
| 贯穿各批 | G01/G02/G03/G08 共享合同；维护成本 | 记录同类修复涉及的独立实现和工时，仅针对实际分叉收敛共享逻辑 | 成功率满足事前约定，维护总工时及归一化成本符合预算；不另建同职责内核 |
| 按证据启动：P2 | G05；实际任务成功率 | 使用已有插件双臂评测验证单个插件收益 | 重复采样及独立验收支持收益后再扩大；无收益则停止扩展 |
| 按证据启动：P2 | G10、G11；维护成本与首次运行成功率 | 测真实容量曲线、安装卡点及双设备旅程 | 性能越过预设 SLO 或渠道/连接失败构成主要流失后，再投入索引、归档或向导建设 |
| 保留条件门：P2/条件性 P0 | G04、G09 自动晋升部分；收益与维护成本 | 分别记录 App Server 接线、单 Skill 晋升试点的具体任务收益与维护预算 | 接线前完成精确版本真实矩阵；晋升前完成真实身份、效果与回滚验收；无收益证据时不扩大默认依赖 |

第 5–7 节保留原审计问题、实现过程与分项验收，阅读其中“当前”“建议新增”时须结合日期和 1.2 后续状态；不能将历史缺口重新排成建设任务。
...
**后续实施进展（2026-09-13）。** [G03 基础层实施](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G03_MODEL_PROFILE_IMPLEMENTATION_2026-09-13.md) 已加入版本化 profile、provider 隔离的窗口来源、只读 `cc llm capabilities`、规划与 Anthropic 实际输出 cap 绑定，以及请求前的非法预算拒绝。诊断不读取凭据、不联网、不写配置，并始终标记未经过运行验证。OpenAI 主链仍未接入原生 Responses，真实 reasoning/tool/stream/usage 旅程也未验收；因此该项只能标记为基础层完成，不能整体关闭。

**第二阶段实施进展（2026-09-13）。** [G03 OpenAI Responses 主链](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G03_OPENAI_RESPONSES_IMPLEMENTATION_2026-09-13.md) 已让版本化 profile 对精确官方目标选择 `openai-responses`，接入无状态 input item、function call/output、加密 reasoning 连续性、流式文本/summary、取消、不完整终态以及 cached input usage；第三方兼容 endpoint 保持 Chat Completions。本地 fake-provider 与真实本地 `read_file` 两轮合同通过，但没有访问付费模型或真实账号，`runtimeVerified` 仍为 false；独立 reasoning token 全链归账、预算误差和精确 SHA 三平台真实旅程仍待交付，因此仍不把 G03 标为 production-complete。
...
**后续实施进展（2026-09-13）。** [G04 接线前安全修复](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G04_APP_SERVER_SAFETY_IMPLEMENTATION_2026-09-13.md) 已按 `turn.status/error` 保留 failed/interrupted 终态，并在 `turn/start` 提交结果未知或已观察到接收时禁止启动第二条 fallback；对应 fault-injection 合同已通过。适配器仍无产品调用方，`0.154.0` 仍未加入 fail-closed 矩阵，也没有同一候选 SHA 的三平台真实 App Server 证据，因此只关闭代码反例，不关闭版本兼容与接线验收。

**第三阶段实施进展（2026-09-14）。** [本轮推进记录](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_CLOSURE_IMPLEMENTATION_2026-09-14.md) 已将精确 `0.154.0` 加入适配器和三平台 workflow 定义，`0.154.1` 继续 fail-closed。本机已使用真实 Codex CLI `0.154.0` 生成 305 个 schema 文件并完成 `initialize → initialized → thread/list` stdio 握手。当前工作树尚未合入，兼容脚本要求的干净精确 SHA 三平台聚合回执也未产生，产品接线仍需独立决策。
...
**后续实施进展（2026-09-13）。** [G05 插件作者 Eval](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_G05_PLUGIN_EVAL_IMPLEMENTATION_2026-09-13.md) 已交付 `cc plugin eval <dir>`、绑定 manifest/payload/suite digest 的声明式任务、关闭全部插件的 control 与仅加载候选快照的 candidate，以及 JSON/HTML 双臂报告。报告分开保留执行、产物、触发、无关改动、usage/成本和增益阈值，dry-run 不能得到正式 PASS。当前没有付费真实模型结果、重复采样或独立 holdout，因此产品入口已实现，效果验收仍未完成。

**第二阶段实施进展（2026-09-14）。** [本轮推进记录](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_CLOSURE_IMPLEMENTATION_2026-09-14.md) 已加入 1–20 次重复采样、由 suite digest 决定并交替的平衡双臂顺序、Wilson/Newcombe 95% 区间，以及插件 payload 外独立绑定的 reviewer holdout。CLI 提供 `--samples`、`--arm-order`、`--holdout`，报告不泄露 reviewer 的绝对路径。固定付费模型结果、组织上独立的模型 grader 和发布提交门仍待执行。
...
**后续实施进展（2026-09-14）。** [本轮推进记录](../../reports/implementation/claude-code-codex/CLAUDE_CODE_CODEX_GAP_CLOSURE_IMPLEMENTATION_2026-09-14.md) 在保留纯词法默认行为的基础上，加入宿主提供的 `semanticCandidates`：每个分数绑定 `memoryId/revision/recordDigest`，Kernel 先执行 lifecycle、expiry、scope 与 sink 过滤，陈旧证据失败关闭，再以词法/语义较强者参与轻量 rerank。模型请求路径现通过 `contextMemorySemanticCandidateProvider` 把 query、session、scope、sink 和上限交给宿主索引，且不暴露 canonical memory 内容；非数组回应失败关闭，关闭 canonical 规划时不调用该源。内置 embedding/index、删除同步、真实语料效果与容量仍待完成。
...
以下为建议观察窗口，从实际启动日计时；先复用已交付能力，按退出条件推进。缺少真实账号、目标设备或样本时保留“待验证”，不以日期到达代替验收。

| 阶段 | 重点 | 可审阅交付物 | 退出条件 |
| --- | --- | --- | --- |
| 第 1–14 天：建立基线 | G01/G02/G09 基础旅程及维护工时记录 | 冻结任务集与首次旅程矩阵、版本/环境清单、三指标基线、失败原因排序 | 分母完整，独立验收可重建；未运行项明确列出，缺证据不获 PASS；修复前冻结改进目标和成本上限 |
| 第 15–30 天：关闭最大卡点 | 优先首次运行阻断，其次任务失败最多的原因 | 一个主要原因的修复、相同任务与环境的前后对照、跨入口回归证据 | 目标指标达到预设改善幅度，其他指标不超出约定退化界限，安全与发布门满足要求 |
| 第 31–60 天：验证稳定性与维护负担 | 承诺支持的 OS/IDE、模型、恢复和权限路径 | 重复运行结果、关键分层结果、维护工时与分叉清单 | 收益在重复运行中成立；维护成本符合预算，薄弱分层不被总平均掩盖 |
| 第 61–90 天：决定扩大或停止 | 仅处理失败与成本数据支持的 G05/G10/G11 或条件性试点 | 收益/成本复盘、保留与暂停清单、必要的回滚证据 | 有收益且维护可承受才扩大支持；无收益、证据不足或成本超限则停止扩展并保留边界 |

每批只选一个主要失败原因。G04 接线和 G09 自动晋升仍需各自条件门，不因路线图进入后半程自动启动。
...
复用现有 Eval 报告、旅程回执和 CI 证据；维护人时以同一批次的工单/工时记录补充，不新增一套任务终态协议。每批报告至少包含：

| 内容 | 必填证据 |
| --- | --- |
| 可比性 | 基线与候选精确 SHA/产物版本、任务集及验收器 digest、OS/入口/provider/model、权限、预算、重复次数、计划与实际运行数 |
| 三指标 | 按 1.3 口径记录成功数/总数、分层结果与区间；维护总人时、每 100 次尝试人时、一次性建设人时及观察窗口 |
| 失败成本 | 失败、重试和补救全部调用费用与耗时；每个成功任务费用 = 全部尝试费用 / 成功数，成功数为零时标为不可计算并保留总费用 |
| 归因 | 安装、配置/身份、模型协议、工具/权限、检索/上下文、恢复、产物错误、环境故障、证据缺失；保留原始错误与回执链接 |
| 决策 | 负责人、事前阈值、实测差值、通过/未通过/证据不足、下一步及停止或回滚条件 |

阈值先测基线再冻结：成功率用百分点变化表示，维护成本同时核对绝对人时和单位任务变化；记录最小有意义改善、允许退化上限和维护预算。任何阈值尚未确定时均不能给出正式效果 PASS。维护观察窗口、支持范围及工时分类必须一致，不能通过缩减支持范围或把排障改名为开发来制造成本下降。

同一任务的多次运行按任务聚合报告不确定性，避免把重复运行当作完全独立样本。样本不足、关键 OS/入口未覆盖或维护工时缺失时标为“证据不足”，不能写成零成本或无回归。任务集调整时另起基线，保留旧集结果。

正式扩大采用三个条件：目标主指标达到事前阈值；其余两项不越过约定退化上限；执行/产物/证据、安全、权限和对应发布门全部通过。首次运行明确阻断是正确诊断，但仍是未完成旅程。越权、错误成功状态和重复副作用不能用平均成功率提升抵消。
...
| G03 Responses、治理投影、CLI 诊断与韧性回归     | 7 files、164 passed；0 次真实 provider 请求                | 首条主链已接线；保留 runtime 未认证、reasoning 明细和真实矩阵边界     |
...
下一轮先交付一份可复现的三指标基线，再关闭影响最大的首次运行或实际任务失败原因。复用已交付的 readiness、严格 Eval、模型 Profile、记忆和恢复能力；每批用相同任务与环境验证收益，并记录持续维护人时。

里程碑以“多少任务完成、多少首次旅程成功、需要多少维护投入”验收，绑定版本、环境和证据。G03/G06 等优化由失败数据排序，G05/G10/G11 扩展由收益与成本决定；G04 接线和 G09 自动晋升保留独立条件门。当前三指标仍待建立统一基线，不能根据阶段性交付数量宣称成功率提高或维护成本下降。
