...
>
> This file is a thin index — detailed command listings live in [`docs/cli/`](./).
...
| Group | File | Covers |
|-------|------|--------|
| Managed Agents & Hosted API | [`cli/managed-agents.md`](./managed-agents.md) | `agent -p` (headless) + 旗标 (`--mcp-config`/`--no-mcp`/`--permission-prompt-tool`/`--include-partial-messages`/`--input-format`/`--settings`), `agents` (子代理), `context` (上下文占用), `memory store/recall/consolidate`, `session policy/tail/usage/park/unpark/end`, `cost`, `checkpoint`, `compact`, `goal`, `stream`, `output-style` (人格), WS routes (Phase D–I) |
| Core Phases 2–7 · Init · Cowork | [`cli/core-phases.md`](./core-phases.md) | `import/export`, `mcp`, `did/encrypt/auth/audit`, `p2p/wallet/org/plugin`, `init/persona`, `cowork`, `hook/workflow/hmemory/a2a`, `sandbox/evolution/evomap/dao` |
| Phase 8 · Blockchain & Enterprise | [`cli/blockchain-enterprise.md`](./blockchain-enterprise.md) | `compliance/threat-intel`, `pqc`, `nostr/matrix/activitypub/scim/terraform`, `hardening/stress/reputation/sla/tech/dev/collab/marketplace/incentive/kg/tenant/governance/recommend/crosschain/privacy/inference/trust/social/fusion/infra` |
| Observability & Code Intel | [`cli/observability.md`](./observability.md) | `codegen` (Phase 86), `ops` (AIOps Phase 25), `perception` (Phase 84), `dbevo` (Phase 80), `federation` (Phase 58) |
| Platform Services | [`cli/platform.md`](./platform.md) | `lowcode`, `evomap`, `cli-anything`, `serve`, `ui`, `orchestrate` |
| Video Editing Agent | [`cli/video.md`](./video.md) | `video edit/deconstruct/plan/assemble/render/assets` (CutClaw-inspired) |
...
chainlesschain agent -p "..." --mcp-config m.json # 临时挂 MCP server (工具 → mcp__<server>__<tool>)
...
> (`session policy/tail/usage/park/unpark/end`), see [`cli/managed-agents.md`](./managed-agents.md).
...
> 详见 [`cli/managed-agents.md`](./managed-agents.md)：`cost` 的定价规则 / JSON 形状 /
...
> `--goal-assess` 跑完 LLM 自评进度。详见 [`cli/managed-agents.md`](./managed-agents.md) 的 Goal 节
> （解析优先级 / drift / stream-json 事件）。

## MCP · Subagents · Context (Claude Code 对标)

```bash
# MCP —— 把工具喂给 agent 循环（工具名 mcp__<server>__<tool>；headless + 交互都支持）
chainlesschain agent -p "..." --mcp-config ./mcp.json    # 临时挂载（{"mcpServers":{...}}）
chainlesschain mcp add weather -c npx -a "-y,@modelcontextprotocol/server-weather" --auto-connect
chainlesschain agent -p "纽约天气?"                       # 注册的 auto-connect server 自动可用
chainlesschain agent -p "..." --no-mcp                   # 跳过注册自动连接（--mcp-config 仍生效）
chainlesschain agent -p "..." --mcp-config m.json --permission-prompt-tool mcp__auth__approve  # 审批交 MCP 工具

# Subagents —— .chainlesschain/agents/*.md（原生优先）或 .claude/agents/*.md（可移植）
chainlesschain agents list | show <name> | run <name> "<task>" | new <name> [--tools ...] [--claude]
#   正文=system prompt；frontmatter tools/model；主 agent 经 spawn_sub_agent({agent}) 委派

# Context —— 上下文窗口占用（/context 对标）
chainlesschain context [<sessionId>] [--json] [--model <m>]   # 按角色拆分 token + 占比 + 余量
```

> 详见 [`cli/managed-agents.md`](./managed-agents.md) 的 **MCP / Subagents / Context** 三节
> （ad-hoc + 注册合并进单 client、permission-prompt-tool 协议、`cc agents run` → `runAgentHeadless`、
> `spawn_sub_agent` 的 `agent` 委派 + model 覆盖、`--include-partial-messages` 真增量覆盖的 provider）。
...
### 1. 概述
见正文头部。本文：CLI Commands Reference。
