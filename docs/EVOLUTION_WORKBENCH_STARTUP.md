...
偏移，拒绝漏页、重复版本或读取期间的状态变化。VS Code 页面每页显示 25 个版本，搜索和统计
覆盖已验证的完整集合；超过第 500 个的版本也可进入审核、比较与回滚。

JetBrains 源码候选 `0.4.116` 同样读取完整分页集合，整次读取上限 45 秒，单个 CLI
调用上限 30 秒；不完整集合不能用于比较、审批或回滚。弹窗内允许后台结果更新界面，
关闭后忽略迟到结果。加载、比较和人工决策（包括确认弹窗）一次只运行一项；写操作
缺少有效结果回执时只刷新当前状态，显示“结果未确认”，不自动重发或声称已经拒绝。
JetBrains 仍使用宿主提供的 CLI 与签名部署环境；上述 profile 设置仅适用于 VS Code。
