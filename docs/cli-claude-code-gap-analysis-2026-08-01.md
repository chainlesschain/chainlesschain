...
6. 完成真实磁盘、pipe、终端与长期运行矩阵：`b4990364f2` 已关闭 EPIPE 直接 `process.exit(0)` 绕过 cleanup，并覆盖 single-turn、stream-json live input 与 REPL early-close 的本地故障注入；现存实现任务仍包括 stdout/stderr `write(false)` backpressure、stream 所有异常路径的顶层 `finally`，以及 ENOSPC/EROFS 的 commit-state 语义。仍须覆盖真实 OS pipe、TTY、SSH、screen reader、Windows/macOS clipboard 与键盘布局、1,000+ turns、超大 MCP output、20+ 并发 Agent、FD/handle/orphan/worktree 清理和长期 soak。各矩阵必须报告 p95、RSS、I/O、FD/handle/process-descendant 差值与 cleanup deadline，不能只记录“测试最终通过”。
