...
| P0 Session 事实源与可恢复 Agent runtime            | 仓库安全闭包继续收口，发布门未关 | `ca4fe4ef73`、`199b2cb7c6`、`4c656ac7ec` 完成索引、增量持久化、WS 恢复和测试工作区隔离；`a72d75d153` 收口 MCP authority 读写、transcript 锚点、严格 started→terminal 状态、Headless/Stream/Cowork 恢复与 semantic-compaction 计费；`b21ba58c2c` 对 ledger 字段、scope、摘要类型、标识长度和跨 session 恢复做二次规范化与 fail-closed 校验。`5c828517df` 只完成首批 ledger admission，不能单独代表全部宿主入口闭环；后续 `38ab06cfdd`、`d1bbabef62`、`77b3d8eda9`、`c208d0aace` 加固 malformed projection、动态 latch、冻结 ticket/client、transport outcome-unknown 与保守 effect 判定，`54228b67d4`、`a83b6a9908`、`c9244db57d`、`ab32a57e4b` 再完成 REPL 原子恢复及 REPL/Headless/Stream/WS 已知辅助 `callTool` 路径的共享 authority。最新本地相关矩阵首轮 363 项中 362 项通过，唯一失败为既有 15 秒测试在并行负载下超时，单文件复跑 12/12 通过；随后 WS 定向 122/122、REPL 与 Host runtime 定向 67/67 通过。人工 adjudication 协议、真实 kill/restart/恶意 MCP 长期矩阵和待发布 exact commit 的 `CLI CI`、`CLI Strict Sandbox` 全平台权威矩阵仍未关闭 |
...
| P0 exact-SHA 发布门禁与原生发行链                  | 门禁已实现，发布阻断             | `6d229d0df4`、`55b3c55a1c`：三平台 exact-SHA gate、不可变 npm tarball、六目标签名 manifest、完整同版本身份校验、versioned asset 禁止覆盖和 stable channel 激活协议；发布契约 28 项通过。`b6a2c096ea` 将 MCP recovery 边界文件与测试加入 `CLI Strict Sandbox` 触发范围。没有待发布精确提交上的完整 `CLI CI` + `CLI Strict Sandbox` 权威矩阵，不得发布                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| P0 签名更新与回滚链                                | 事务批次已提交，5 项审计整改中   | `4d3a4ee788` 提供签名 manifest、健康检查与初版自动回滚；`8990999771` 提交原生 installer/OTA 锁、状态、sidecar、alias、lineage 与结果消费事务加固，核心定向测试 131/131 通过。`a1c9eed07e` 已关闭“覆盖下载目标后无法恢复旧文件”的 P1，下载器 30/30、相邻更新矩阵 122/122 通过，最终只读复审在该批范围内为 P0=0、P1=0。仍待整改旧 `.previous` 代际恢复、POSIX alias 回滚、锁释放 TOCTOU、Windows 掉电耐久性及 restart 成功/失败结果歧义；完成整改与真实安装/升级/回滚矩阵前不得标记为 release-ready                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
本轮安全批次已覆盖真实 Windows Unicode 路径及目录/文件 ACL 修复。Windows PowerShell ACL 的冷启动延迟仍列为后续 P2 性能优化，不影响当前安全语义与提交边界。`a72d75d153` 的 `CLI Strict Sandbox` 已在 Ubuntu 24.04、macOS 15、Windows 三个平台通过；这只是该精确提交的实现证据，不能授权后续 `b21ba58c2c`、`c2ffbb7f22`、`5c828517df`、`ab32a57e4b` 或更晚提交发布。npm/原生发行物不得依据本地测试、旧提交矩阵或仅成功生成的 artifact 发布，必须以待发布精确提交上的 `CLI CI` 与 `CLI Strict Sandbox` 全矩阵、签名/安装/升级/回滚和公开渠道回读全部通过为最终门禁。

### 2026-08-02 原生事务提交后的审计复核

`8990999771` 已同步到 GitHub 与 Gitee，因而该批实现与测试不会随当前工作树误删；这只表示提交边界已固定，不表示发布验收完成。审计期间 15 个范围文件三轮 SHA-256 保持一致，7 个核心事务测试文件 131/131 通过，扩展 8 文件矩阵为 141/142；唯一失败是 `cli-aliases.test.js` 的动态导入在单独复跑时仍超过 5 秒。

`a1c9eed07e` 随后完成下载目标替换事务：覆盖前通过同目录硬链接保留旧 inode，提交后验证、目录持久化或恢复失败均采用 fail-closed 结果；锁在快照期间或提交后丢失时不再写 `outputPath` 或删除外部锁，并保留可验证恢复副本供人工处理。三轮只读审计共关闭 6 个提交阻断问题，下载器定向测试 30/30、checker/downloader/applier/auto-update 相邻矩阵 122/122、Prettier、ESLint、`node --check` 与 `git diff --check` 均通过。该提交已同步 GitHub 与 Gitee；restore 临时硬链接删除后的目录 fsync 仍作为 P2，锁释放 verify-then-unlink TOCTOU 仍作为独立 P1。

后续整改按以下顺序执行：

1. 在 target、`.previous`、lineage 和 alias 之间建立可恢复的完整事务快照，任何阶段失败都恢复升级前的全部 generation 与链接状态。
2. **已完成（`a1c9eed07e`）**：下载目标覆盖前保存旧 destination，rename 后的验证或目录持久化失败恢复旧文件；锁丢失时保留恢复副本并停止写目标路径。
3. 将锁释放改为不会删除后继事务锁的所有权协议，关闭“验证 token 后按路径删除”的 TOCTOU。
4. 为 Windows sidecar 与 PowerShell installer 补足文件和目录元数据持久化；把“更新已提交但 restart 失败”表达为独立结果，禁止误报为 apply 未发生。
5. 增加 POSIX/PowerShell 真实故障注入、旧 generation、alias rollback、重复 result 消费和强杀/断电阶段测试；随后再跑 exact-SHA 的 Linux、Windows、macOS `CLI CI` 与 `CLI Strict Sandbox`。
