...
| P1 默认 Agent 与命令面 manifest                    | 基础完成，长尾迁移进入第二批              | `6b2b394fd1`：修复 14 个域的嵌套帮助路由和跨 OS help 漂移，175 条命令写入 stability/category/visibility/replacement 元数据，默认命令与核心分组由 manifest 驱动；`c50d2f8a53` 以 `dao`、`evomap` 完成首批 `lab` 迁移，`1f2a9caf3d` 补强 lifecycle 契约。当前 `0.162.194` 候选再迁移 8 个明确标注旧版/in-memory governance 的长尾入口，原顶层命令仍至少保留两个 minor cycle，只在 stderr 发弃用提示；registered graph 仍为 175、净增长 0，推荐面由 174 降到 166，manifest、README 与四种 shell completion 同源生成。其余长尾迁移、真实 usage telemetry 和兼容窗口后的实际移除仍待完成                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
- `47c6afc550` 将 macOS 真实 host job 从会滚动的 `macos-latest` 固定到 `macos-15`，并以 workflow 契约测试锁定 runner；同时修复 Unix 平台 checkpoint duplicate archive fixture 的权限确定性。`fa983db9f5` 进一步把 install/initial/restart 的 `--user-data-dir` 分离、共享同一已安装 extensions 目录，消除两次 Electron 启动复用 profile mutex 的耦合；VS Code CLI install/list 子进程上限为 120 秒，两个 macOS smoke step 各自上限为 15 分钟。
