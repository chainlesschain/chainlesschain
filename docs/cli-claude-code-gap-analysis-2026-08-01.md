...
- `05555dddea` 为 relay host 保留一个随机 loopback-only Chromium 启动开关，只用于触发 VS Code test runner，journey 不连接该端点且不再安装 WebSocket client；测试 driver 同时以仅在 `dom-relay` 环境生效的 startup activation 调用与 `extensionTestsPath` 共用的 `runOnce()`，任一路径启动都不会重复执行。journey artifact 目录在 host 前创建，installed VSIX、activation、command 与 bridge 里程碑也在 WebView snapshot 前落盘。更新后的本地扩展单测仍为 **54/54 passed**，Prettier、workflow actionlint 与 `git diff --check` 通过；这些改动必须在包含本节文档的后续 exact SHA 重新验证，当前仍为 **release NO-GO**。
