...
| P1-2 后台 Agent 恢复、隔离与预算取消 | **部分完成**         | `1724f05300` / `0edc932aa5` 已形成 pre-main bootstrap、durable PID、独立 keeper 与专项三平台 soak gate 的本地候选；仍须让同一最终 exact SHA 通过 CLI CI、CLI Strict Sandbox 和 20-Agent/7200 秒/1000-cycle formal keeper soak，回读三平台 kill/restart/并发/清理 artifact 后才能关闭。 |
