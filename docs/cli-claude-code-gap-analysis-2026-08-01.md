...
| P0 Session 事实源与可恢复 Agent runtime            | 仓库闭包已提交，发布门未关 | `ca4fe4ef73`、`199b2cb7c6`、`4c656ac7ec` 完成索引、增量持久化、WS 恢复和测试工作区隔离；`a72d75d153` 收口 MCP authority 读写、transcript 锚点、严格 started→terminal 状态、Headless/Stream/Cowork 恢复与 semantic-compaction 计费；`b21ba58c2c` 对 ledger 字段、scope、摘要类型、标识长度和跨 session 恢复做二次规范化与 fail-closed 校验；`5c828517df` 让 REPL、Headless、Stream、Cowork 与 WS 在恢复异常时真正阻断 MCP replay，而不再只显示提示。最新本地独立组合回归 16 个文件、371 项全部通过；仍待发布 exact commit 的 `CLI CI` 与 `CLI Strict Sandbox` 全平台权威矩阵 |
...
本轮安全批次已覆盖真实 Windows Unicode 路径及目录/文件 ACL 修复。Windows PowerShell ACL 的冷启动延迟仍列为后续 P2 性能优化，不影响当前安全语义与提交边界。`a72d75d153` 的 `CLI Strict Sandbox` 已在 Ubuntu 24.04、macOS 15、Windows 三个平台通过；这只是该精确提交的实现证据，不能授权后续 `b21ba58c2c`、`c2ffbb7f22`、`5c828517df` 或更晚提交发布。npm/原生发行物不得依据本地测试、旧提交矩阵或仅成功生成的 artifact 发布，必须以待发布精确提交上的 `CLI CI` 与 `CLI Strict Sandbox` 全矩阵、签名/安装/升级/回滚和公开渠道回读全部通过为最终门禁。
