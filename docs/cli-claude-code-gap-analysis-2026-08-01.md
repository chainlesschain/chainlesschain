...
上述增量只证明 **production Node WHATWG `ReadableStream` 上的有限 HTTP response body、非 2xx HTTP body 零读取/零传播、HTTP/SSE `200` JSON-RPC error 的固定投影与防误重试、无 byte reader 成功 response 的固定 fail-closed、后台 SSE 单事件、`ws` 单消息 wire/解压 payload 内存上限及普通 JSON 配置收紧、peer `1009` 的方向中立 outcome-unknown、泛 WS close 的固定投影/结果未知防重放/elicitation waiter 清理，以及 HTTP discard-response 与普通 tool/resource 主请求的 cancel/deadline/断连生命周期**。合法 `-32042` 的四个 allowlist 字段仍会按协议进入显式用户 elicitation，不能外推为“所有 peer 内容都被删除”。长期 SSE GET 仍未纳入同一 caller-cancellation 语义；非标准 fetch 若忽略 `AbortSignal`，宿主可以退休 controller registry 并拒绝 late success，却不能强制一个永不 settle 的 adapter Promise/底层请求结束；非标准 response 若没有可用 `body.cancel()` 也只能保证零读取，不能强制其底层 socket 关闭。response callback 的 deadline 诊断仍沿用 adapter/Node 的 AbortError 文案；`longRunning` 成功响应或 SSE 仍可能在上限内慢滴答。单消息上限不覆盖消息速率、累计流量或 JSON 对象图放大。WS/stdio/SSE caller cancellation、recovery generation、即时撤权、stdio 完整行/stderr/result/schema、Skill、进程树、三平台 artifact 与长期 soak 也都不在本节范围内，因此完整 Skill/MCP 产品项继续 **NO-GO**。
