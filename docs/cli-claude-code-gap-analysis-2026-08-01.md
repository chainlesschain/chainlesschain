...
| P0 Session 事实源与可恢复 Agent runtime            | 仓库闭包已提交，发布门未关 | `ca4fe4ef73`、`199b2cb7c6`、`4c656ac7ec` 完成索引、增量持久化、WS 恢复和测试工作区隔离；`a72d75d153` 进一步把 MCP authority 读写、transcript 锚点、严格 started→terminal 状态、Headless/Stream/Cowork 恢复与 semantic-compaction 计费收口。合并矩阵 462 项通过、1 项跳过，独立复审矩阵 185 项通过；仍待 exact release commit 的跨 OS/重启矩阵 |
| P0 Skill 受控执行与内容身份授权                    | 已完成基础                 | `c841a58e2b`：用户可写 Skill 默认不受信，执行前异步复核精确 digest，经隔离 Agent 与 host-owned 工具边界运行，并透传 LLM 配置、命令侧失败关闭；`a72d75d153` 接通 runtime 的异步 materializer 与子 Agent 用量来源归因；真实宿主/恶意 Skill 长期矩阵仍未完成                                                                                     |
...
本轮安全批次已覆盖真实 Windows Unicode 路径及目录/文件 ACL 修复。Windows PowerShell ACL 的冷启动延迟仍列为后续 P2 性能优化，不影响当前安全语义与提交边界。`b1b0570091` 的 `CLI Strict Sandbox` 曾在 Ubuntu 24.04、macOS 15、Windows 三个平台通过；这只是该精确提交的实现证据，不能授权后续提交发布。npm/原生发行物不得依据本地测试、旧提交矩阵或仅成功生成的 artifact 发布，必须以待发布精确提交上的 `CLI CI` 与 `CLI Strict Sandbox` 全矩阵、签名/安装/升级/回滚和公开渠道回读全部通过为最终门禁。
