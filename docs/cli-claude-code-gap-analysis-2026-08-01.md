...
### 16.7 MCP HTTP 有限响应与 SSE 单事件字节上限增量
...
该增量只证明 **production Node WHATWG `ReadableStream` 上的有限 HTTP response body 与后台 SSE 单事件内存上限**。WebSocket 的 `maxPayloadBytes` 仍可被 raw/direct/plugin 配置抬高，且超限尚无专用稳定码；HTTP notification、server response callback 与 session DELETE 仍未统一取消返回正文或设置 deadline；非认证错误的有界 200 字符预览仍可能把正文、控制字符或 secret 传播到 stderr/tool-result/model/session，`longRunning` 服务也可能用 4 KiB 内慢滴答永久占住预览。无 stream reader 的 Response-like 兼容分支只能在 `text()` 返回后做字节检查，不构成前置内存上限。stdio 完整行/stderr/result/schema、Skill、取消/撤权、进程树、三平台 artifact 与长期 soak 也都不在本节范围内，因此完整 Skill/MCP 产品项继续 **NO-GO**。
...
3. 完成 Skill/MCP 真实恶意矩阵：第 16.7 节只关闭 production Node `ReadableStream` 的有限 HTTP response body 与后台 SSE 单事件上限；仍须硬钳制 WebSocket payload 并提供专用稳定码，给 HTTP notification/server response/DELETE 补 body cancel 与 deadline，取消或净化会传播到 stderr/tool-result/model/session 的错误正文预览，并处理 `longRunning` 慢滴答与无 reader 兼容分支。默认不得在 dispatch 后自动重放 outcome-unknown 的非幂等 `tools/call`；AbortSignal/recovery generation 必须贯穿 agent-core 到 HTTP/WS/SSE/stdio transport，并按 deadline 清理完整进程树。stdio complete line/stderr/malformed aggregate、最终 result/ledger/stream、tool schema/description 与 Skill 文件/递归总量都需要 host-owned 不可放宽的字节/深度预算；token 估算失败不得返回 0 后 fail-open。还须覆盖 run_skill 父级取消、即时 revoke、可执行字节身份绑定，以及“任意 stdio command 等价于信任本地代码执行”的明确边界。现有 Session Host MCP 场景只证明 ledger recovery fence，不是真实 transport 恶意矩阵。
