...
上述增量只证明 **production Node WHATWG `ReadableStream` 上的有限 HTTP response body、后台 SSE 单事件、`ws` 单消息 wire/解压 payload 内存上限，以及 HTTP discard-response 的 cancel/deadline/断连准入生命周期**。已经 dispatch 的普通 HTTP 主请求和长期 SSE GET 仍未纳入 discard controller；非标准 fetch 若忽略 `AbortSignal`，只能保证宿主 controller registry 按时退休，不能强制其底层 Promise/请求结束；response callback 的 deadline 诊断仍沿用 adapter/Node 的 AbortError 文案。非认证错误的有界 200 字符预览仍可能把正文、控制字符或 secret 传播到 stderr/tool-result/model/session，`longRunning` 服务也可能用 4 KiB 内慢滴答永久占住预览。无 stream reader 的 Response-like 兼容分支只能在 `text()` 返回后做字节检查，不构成前置内存上限。泛 WebSocket close 也未立即清理 per-server elicitation/URL completion waiters；孤立 peer `1009` 无法区分本地 inbound 超限与 peer 拒绝 outbound，普通配置 loader 还不能把更小 `maxPayloadBytes` 传播到 sink。单消息上限不覆盖消息速率、累计流量或 JSON 对象图放大。stdio 完整行/stderr/result/schema、Skill、主请求取消/撤权、进程树、三平台 artifact 与长期 soak 也都不在本节范围内，因此完整 Skill/MCP 产品项继续 **NO-GO**。
...
3. 完成 Skill/MCP 真实恶意矩阵：第 16.7 节关闭了 production Node `ReadableStream` 的有限 HTTP response body、后台 SSE 单事件、`ws` 单消息 wire/解压上限，以及 HTTP notification/server response/DELETE 的 body cancel、宿主 deadline 与断连准入；仍须取消或净化会传播到 stderr/tool-result/model/session 的错误正文预览，处理普通在途主请求、长期 SSE、`longRunning` 慢滴答、无 reader 兼容分支、泛 WS close 的 elicitation/URL waiter 清理，以及消息速率/累计流量/JSON 对象图预算。默认不得在 dispatch 后自动重放 outcome-unknown 的非幂等 `tools/call`；AbortSignal/recovery generation 必须贯穿 agent-core 到 HTTP/WS/SSE/stdio transport，并按 deadline 清理完整进程树。stdio complete line/stderr/malformed aggregate、最终 result/ledger/stream、tool schema/description 与 Skill 文件/递归总量都需要 host-owned 不可放宽的字节/深度预算；token 估算失败不得返回 0 后 fail-open。还须覆盖 run_skill 父级取消、即时 revoke、可执行字节身份绑定，以及“任意 stdio command 等价于信任本地代码执行”的明确边界。现有 Session Host MCP 场景只证明 ledger recovery fence，不是真实 transport 恶意矩阵。
