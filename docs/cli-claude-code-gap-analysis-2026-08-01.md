...
上述增量只证明 **production Node WHATWG `ReadableStream` 上的有限 HTTP response body、成功正文的 host-owned 绝对 deadline、非 2xx HTTP body 零读取/零传播、HTTP/SSE `200` JSON-RPC error 的固定投影与防误重试、无 byte reader 成功 response 的固定 fail-closed、response callback deadline 的固定投影、后台 SSE 单事件、`ws` 单消息 wire/解压 payload 内存上限及普通 JSON 配置收紧、stdio 完整/未终止 stdout frame 的 UTF-8 字节上限、peer `1009` 的方向中立 outcome-unknown、泛 WS close 的固定投影/结果未知防重放/elicitation waiter 清理，以及 HTTP discard-response 与普通 tool/resource 主请求的 cancel/deadline/断连生命周期**。合法 `-32042` 的四个 allowlist 字段仍会按协议进入显式用户 elicitation，不能外推为“所有 peer 内容都被删除”。长期后台 SSE GET 仍未纳入同一 caller-cancellation 语义，也仍可能在单事件上限内长期慢滴答；非标准 fetch 若忽略 `AbortSignal`，宿主可以退休 controller registry 并拒绝 late success，却不能强制一个永不 settle 的 adapter Promise/底层请求结束；非标准 response 若没有可用 `body.cancel()` 也只能保证零读取，不能强制其底层 socket 关闭。单消息/frame 上限不覆盖消息速率、累计流量、很多小 malformed frame 或 JSON 对象图放大。WS/stdio/SSE caller cancellation、recovery generation、即时撤权、stdio stderr/result/schema、Skill、进程树、三平台 artifact 与长期 soak 也都不在本节范围内，因此完整 Skill/MCP 产品项继续 **NO-GO**。
...
3. 完成 Skill/MCP 真实恶意矩阵：第 16.7 节关闭了 production Node `ReadableStream` 的有限 HTTP response body、有限成功正文的 host-owned 绝对 deadline、非 2xx error body 零读取/零传播、HTTP/SSE `200` JSON-RPC error 固定投影与防误重试、无 byte reader 成功 response 的固定 fail-closed、response callback deadline 的固定投影、malformed HTTP JSON 和 WS malformed/binary frame 的固定 non-retryable error、后台 SSE 单事件、`ws` 单消息 wire/解压上限及普通 JSON 配置收紧、stdio 完整/未终止 stdout frame 的 UTF-8 字节上限、peer `1009` 的方向中立 outcome-unknown、泛 WS close 的固定投影/结果未知防重放/elicitation waiter 清理、HTTP notification/server response/DELETE 的 body cancel，以及普通 HTTP tool/resource 在途请求的 caller cancel、宿主 deadline、断连和 late-success fencing。明确的有效 RPC application rejection 结算为已知失败；真正的 dispatch/transport 不确定性对非幂等 `tools/call` 继续保持 outcome-unknown 且不自动重放。仍须处理长期后台 SSE 的 caller lifecycle / 慢滴答，以及消息速率/累计流量/JSON 对象图预算。HTTP caller `AbortSignal` 已从 agent-core 贯穿到 transport；recovery generation 以及 WS/SSE/stdio 的 caller `AbortSignal` 仍须贯穿，并按 deadline 清理完整进程树。stdio stderr/malformed aggregate、最终 result/ledger/stream、tool schema/description 与 Skill 文件/递归总量都需要 host-owned 不可放宽的字节/深度预算；token 估算失败不得返回 0 后 fail-open。还须覆盖 run_skill 父级取消、即时 revoke、可执行字节身份绑定，以及“任意 stdio command 等价于信任本地代码执行”的明确边界。现有 Session Host MCP 场景只证明 ledger recovery fence，不是真实 transport 恶意矩阵。
