...
本节是面向继续开发的**当前清单**，优先级高于第 14～17 节保留的历史阶段判定。本次发布核验基线为最终 release SHA `a0631cb4f97f45ff7fcef9c19d346ed2b8387da6`；公网 npm `latest` 为 `chainlesschain@0.163.8`，其 release SHA 同为该提交。PR #180 的 outcome-unknown 人工裁决、PR #183 的五域迁移/回滚和 PR #182 的因果可观测性已先进入 `0.163.7`；其后的磁盘故障矩阵、governed recovery、正式 soak verifier、scheduler transaction-time fencing、governed merge review 与 MCP resource templates 已进入 `0.163.8`。两个版本的完整发布证据分别见第 18.8 节和第 18.12 节。

### 18.1 三种统计口径

| 口径                     | 当前结论                                              | 说明                                                                                                                                |
| ------------------------ | ----------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| 原始 P0-1～P2-4 共 15 项 | **10 完成、5 部分完成、0 完全未开始；5 项未完全关闭** | 分层统计为 P0 **4 完成/1 部分**、P1 **3 完成/3 部分**、P2 **3 完成/1 部分**；P2-2、P2-3 已正式关闭，P2-4 仍只占一个“部分完成”条目。 |
...
| P1-2 后台 Agent 恢复、隔离与预算取消 | **部分完成**         | `1724f05300` / `0edc932aa5` 已形成 pre-main bootstrap、durable PID、独立 keeper 与专项三平台 soak gate；`04bf5177ec` / `a37715573a` / `d05b4992dc` 又关闭本地 security-anchor 污染和无界 Windows cleanup，`a37a0ab838` 增加严格三平台 aggregate。本地候选仍须让同一最终 exact SHA 通过 CLI CI、CLI Strict Sandbox 和 20-Agent/7200 秒/1000-cycle formal keeper soak，回读三平台 kill/restart/并发/清理 artifact 后才能关闭。 |
...
| 产品任务                        | 当前判定                           | 下一关闭条件                                                                                                                                                                                                                                                                                           |
| ------------------------------- | ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 3. Skill/MCP 真实恶意矩阵       | **部分完成 / NO-GO**               | `b22e65d1e9` 已收紧直接 builtin loader，`2d4dff4052` 已把 worker/child 等新执行上下文绑定到强制 OS sandbox 并加入 fresh-isolate 探针；仍须完成 macOS runtime `exec/open` 原子绑定、任意 native/shared-library 递归闭包、远端 revoke/distributed authority，并补齐同一 exact SHA 的三平台长期恶意证据。 |
...
当前总判定为：**公网 CLI npm `0.163.8` 的 exact-SHA 发布子链为 GO；完整 CLI 产品与 native 公开发行仍为 NO-GO**。`0.163.8` release commit `a0631cb4f97f45ff7fcef9c19d346ed2b8387da6` 已完成最终 SHA 双门禁、不可变发布和独立回读；P2-2 已在后续 `main` 提交关闭。P2-4 的首个有效正式长期 soak segment 已在后续 `main@7d3120fc1e` 成功，正式计数为 1/4；其余三个 segment 与 campaign verifier 尚未完成。长期 soak、Skill/MCP 恶意矩阵、代表性 alias telemetry、P1-2 exact-SHA formal keeper 矩阵和签名 native 发行仍未完成。
...
### 18.11 2026-08-14 `0.163.8` 发布候选范围与当时剩余六项

正式发布前的 `release/cli-0.163.8` 候选以 `main@bdad2b2d57609a66dca753bd8575773334618da3` 为基线，只增加 CLI version、根 lockfile、根 CHANGELOG、内置 changelog artifact 与本进度文档五个发布文件。它收录 `0.163.7` 之后已经进入 `main` 的下列增量，不包含仍未合并的 P2-2 native clipboard-image host：
...
| 证据阶段              | 精确结果                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PR head 门禁          | [PR #194](https://github.com/chainlesschain/chainlesschain/pull/194) 最终 head `84b120dad0aebf6fc697328f0a884affe82f9696` 的 [CLI CI `31801390253`](https://github.com/chainlesschain/chainlesschain/actions/runs/31801390253) 为 **53/53 success**，[CLI Strict Sandbox `31801389981`](https://github.com/chainlesschain/chainlesschain/actions/runs/31801389981) 为 Ubuntu、macOS、Windows **3/3 success**，required checks **6/6 success**。两套发布门均绑定该 exact PR head。                                                                                                                                                                 |
...
- [PR #190](https://github.com/chainlesschain/chainlesschain/pull/190) 的 final head `12dc76720108d03ff0fd763fece81a39874f81fa` 包含诊断提交 `ee2073c0ee7ff84a3e55975961d9dea2f53be5e1` 的 lazy `CGImage` TIFF 解码修复，并以 merge commit `affafa7f0f6fede3274e503d2387ce493e74bfd0` 进入 `main`。PR head 与 merge commit 的完整 tree 均为 `2fbaeb173bd548afa401c36ddb4deb4fe6ea1f79`；生产实现 `clipboard-image.js` 的 blob 为 `6d7bfbb2f4374268e8b528b2feeb2f2ddf309a7b`，目标测试 blob 为 `0f3544db6a1ebd3b07c5f4e752dd77ff97a9d94e`。macOS 路径使用 `CGImageSourceCreateImageAtIndex`、`CGImageGetWidth` 与 `NSBitmapImageRep.initWithCGImage`，不再依赖会触发大 TIFF eager decode 的 `PixelWidth` metadata 探测。
- PR head 的 [Clipboard Host `31808440289`](https://github.com/chainlesschain/chainlesschain/actions/runs/31808440289) 为 Linux、Windows、macOS 与 aggregate **4/4 success**，[CLI CI `31808440588`](https://github.com/chainlesschain/chainlesschain/actions/runs/31808440588) 为 **53/53 success**，[CLI Strict Sandbox `31808440296`](https://github.com/chainlesschain/chainlesschain/actions/runs/31808440296) 为 **3/3 success**。最终 merge SHA 的 [CLI CI `31810849262`](https://github.com/chainlesschain/chainlesschain/actions/runs/31810849262) overall success，[CLI Strict Sandbox `31810848956`](https://github.com/chainlesschain/chainlesschain/actions/runs/31810848956) 为 **3/3 success**。
- 为消除 PR head 与 merge SHA 名义差异，又对最终 `main@affafa7f0f6fede3274e503d2387ce493e74bfd0` 触发 [Clipboard Host `31813967006`](https://github.com/chainlesschain/chainlesschain/actions/runs/31813967006)，Ubuntu job `94811065538`、Windows `94811065577`、macOS arm64 `94811065635` 与 aggregate `94811975926` 全部 success。macOS 真实 TIFF fallback 使用 `osascript+jxa-tiff`，输出 `3×2` 像素，`pixelSha256=211e8fe1dbf894469ecc528cc6b9209788f1a7aa6cc586b272fd43de5f2439c2`，与 fixture 完全一致。aggregate artifact `9224339398` 的 digest 为 `sha256:212176e8319ed4c32bd6ee332dd01c2f31cc00eba4bddb64f39bafcd0c76ac86`；Linux、Windows、macOS artifacts 分别为 `9224289742`、`9224333668`、`9224314344`。
- 因此 P2-2 的 production 系统剪贴板图片 adapter、真实 macOS TIFF 边界和最终 exact-SHA 三平台 host/双门均已关闭，P2-2 从“部分完成”更新为**已完成**。该提交发生在 `0.163.8` 发布后，只代表当前 `main` 已具备此能力，不表示 npm `0.163.8` tarball 包含 PR #190。

### 18.14 2026-08-15 P1-2 hard-kill keeper 本地候选与正式门禁边界

- 提交 `1724f05300` 把后台 turn 的启动拆成 host-owned pre-main bootstrap、worker 与 sibling keeper 三方协议。native child 在进入用户代码前先通过 barrier 交付可持久化 PID/identity；keeper 使用 `ARM / RELEASE / RETIRE` 单调状态持有启动窗口。worker 在 supervisor 完成 PID commit 前被 SIGKILL/OOM 时，keeper 负责终止并确认 owned process tree；正常 handoff 后才释放给 supervisor，退休前继续保留 bounded cleanup 证据。该切片还覆盖 stale generation、重复/乱序消息、PID identity 不一致、keeper/worker 提前退出与 terminal settlement 的失败闭合。
- 提交 `0edc932aa5` 新增独立 `CLI Background Agent Keeper Soak`。PR 路径使用 3 agents、5 秒、至少 2 cycles 的 smoke；schedule/manual formal profile 在 Ubuntu、Windows、macOS 上固定 Node `22.12.0`、20 agents、每平台至少 7200 秒与 1000 cycles，交替 hard-kill/cooperative stop，30 秒 cleanup deadline，并记录 RSS、FD/handle 与 orphan 趋势。workflow 会精确 checkout 请求 SHA、要求 clean tree，并逐平台上传不可缺失 artifact。
- 两个提交形成了可审查实现和 formal gate，但尚未推送并取得其自身最终 exact SHA 的 GitHub CLI CI、CLI Strict Sandbox 与 keeper soak artifact，因此当前仍是 **P1-2 部分完成**。2026-08-15 的一次共享工作树复跑也不能替代正式证据：组合 realspawn 运行在 5 分钟上限内报告 4 个宿主场景失败且没有完整汇总；拆出的 unit 组合在 4 分钟内未返回汇总。该复跑不计为通过，后续必须在干净 runner 重新执行并保留完整日志/artifact。
- 后续在 detached `8fa9a3272e` 隔离 worktree 和完整依赖树上逐文件复跑，`background-agent-keeper-protocol`、`background-agent-worker-termination`、`background-agent-keeper-soak`、`background-stability-matrix` 与 `background-stability-matrix2` 合计 **5 files / 48 passed**；这表明先前组合 unit 无汇总包含共享依赖/并发污染，而不是这 48 项的可复现断言失败。单独 `background-stability-realspawn` 仍在 5 分钟内没有终态汇总，诊断被终止，遗留的一个 worker 与一个 keeper 精确 PID 已清理；它仍不计为通过，也不降低三平台 formal gate 要求。

### 18.15 2026-08-15 Node capsule builtin capability 收紧

- 提交 `b22e65d1e9` 关闭 Node 22 capsule 的一条直接权限旁路。旧 capsule 允许所有 Node builtin，且没有覆盖 `process.getBuiltinModule`、`process.binding` 与 `process._linkedBinding`；恶意包即使没有静态导入，也可在运行期取得未审计 builtin 或内部 binding。新 materialization v4 / capsule v3 在 immutable VFS 构建审计后，从 esbuild metafile 导出唯一静态 external builtin 集，并由 host 把 exact `foo` / `node:foo` allowlist 写入最终 capsule。worker 删除或复制 host marker 都会在发布前失败关闭；最终 capsule bytes、manifest digest 与 generation 共同绑定该策略。
- runtime 把 `Module._load`、`Module._resolveFilename` 与 `process.getBuiltinModule` 固定到这份 allowlist；未静态导入的已知 builtin 返回 `CC_MCP_STDIO_BUILTIN_MODULE_BLOCKED`。`process.binding`、`process._linkedBinding` 与既有 `process.dlopen` 均不可改写并固定拒绝。manifest validator 要求 `externalBuiltins` 与派生 allowlist 精确一致，读取方返回的嵌套策略也做 deep freeze，旧 schema 不能被静默复用。
- 本地聚焦结果为 capsule/materialization **4 files / 75 passed**；identity/Broker 的 `capsule|MCP|materialized` 相邻筛选为 **2 files / 25 passed / 271 未命中筛选项 skipped**；关闭 live 条件的集成为 **17 passed / 3 skipped**。目标 ESLint 为 0 errors，Node syntax、Prettier 与 `git diff --check` 通过。一次开启 `CC_SANDBOX_LIVE=1` 的 Windows 运行得到 **17 passed / 1 platform skip / 2 host failures**：WMI process-start observer 被本机拒绝，AppContainer profile API 返回 `0x800706d9`（所需 Windows 服务不可用）；严格实现按预期失败关闭，但该结果不能写成 20/20，也不能替代 GitHub Windows runner。
- 本切片只关闭“当前 capsule 直接取得未审计 builtin/internal binding”的旁路，不关闭第 18.4 节整项。`worker_threads` / `child_process` 等新执行上下文不会自动继承同 realm JavaScript guard；后继第 18.17 节把它们显式绑定到产品强制 OS sandbox，并加入对应 fresh-isolate 探针。macOS descriptor-bound runtime exec、任意 native/shared-library 递归闭包、远端 revoke/distributed authority 和同一最终 SHA 的三平台长期恶意矩阵仍未完成，因此产品任务 3 保持 **NO-GO**。
...
- 提交 `2d4dff4052` 将 materialization/capsule/builtin-policy 升到 v5/v4/v2。构建后会从已审计的 static external builtin 集派生 `executionContextBuiltins`，覆盖 `child_process`、`cluster`、`worker_threads`、`vm` 与 `inspector` 系列，并把非空集合固定标记为 `required-mcp-os-sandbox-boundaries-v1`；manifest validator 要求该集合、静态 allowlist 和 isolation 标记都能从 external imports 唯一重建，读取结果继续 deep-freeze。旧 generation 不会跨 schema 静默复用。
...
### 18.19 2026-08-15 P1-2 keeper 本地复现修复与聚合合同

- 第 18.14 节记录的 realspawn 无终态已定位到测试环境污染，而不是 keeper hard-kill 断言失败：fake child 的 anti-rollback witness 没有隔离 `CHAINLESSCHAIN_SECURITY_ANCHOR_HOME`，回落到宿主 `%LOCALAPPDATA%` 后因 `EPERM` 进入 interaction recovery。`04bf5177ec` 让 realspawn 与 keeper soak fixture 使用 test-root 内独立 security anchor，恢复原环境，并为测试清理的 Windows `taskkill` 设置 10 秒上限；`d05b4992dc` 根据冷 Windows 25.318 秒实测把首个 PID 用例的显式预算与另外两个慢宿主用例统一为 45 秒。
- 旧 RETIRE client 固定等待 10 秒，但 keeper 的 Windows 最坏路径包含两个 target 的 `taskkill`、WMIC/PowerShell identity probe、两次 durable state lock 与 cleanup confirm。`a37715573a` 把所有同步 `taskkill` 限为 10 秒，并从共享协议推导独立 RETIRE 上限：`2 × (10s + 5s + 10s) + 2 × 5s + 2s + 8s margin = 70s`；HELLO/ARM 仍为 10 秒，RETIRE override 只能缩短，超时后保留 armed turn 并失败关闭。定向 keeper 矩阵为 **80 passed / 7 skipped**，client/protocol 最终 6/6 与 taskkill timeout fail-close 1/1 通过。
- 三个修复提交 cherry-pick 到隔离 worktree 后的 exact tree `bc7b0fdd69c2f567344d2823fc71c898e7fd3ba0` 已完整运行 realspawn：**1 file / 4 passed**，131.25 秒，无新增 fixture/PID 残留且 worktree 保持 clean。`a37a0ab838` 另为 keeper workflow 增加 formal/smoke 分离和 always-run 三平台 aggregate：formal 逐平台强制至少 7200 秒、20 agents、1000 cycles，拒绝缺失/重复 OS、SHA/字节篡改、PID/worktree 残留、violations 与资源趋势超限；PR smoke 的 schema、`qualifyingEvidence=false` 和 `releaseGateEligible=false` 均不可冒充正式证据。该合同 10/10 单测与 2-agent/2-cycle Windows 真实 smoke 通过。
- 以上仍是**本地候选**。keeper workflow 尚未在包含这些提交的远端默认分支产生 artifact；P1-2 只有在最终 clean exact SHA 的 CLI CI、CLI Strict Sandbox 和 Linux/macOS/Windows formal keeper aggregate 全部成功后才可关闭，不能以本节本地结果替代。

### 18.20 2026-08-15 MCP live evidence 内容绑定

- `737038d30e` 把 formal MCP evidence 升级为内容级 v2：每个平台的两小时 host/race soak 在开始前读取该平台 fresh-isolate Vitest JSON，严格核对 reporter totals、唯一目标测试文件、host-effects boundary、capsule byte replacement、Windows WMI compile/calibration 与 live-mode negative gate 的平台状态，并记录 raw JSON SHA-256 与测试源码 SHA-256。aggregate 除验证三份 outer soak evidence 的同一 exact SHA、干净 tree、时长/次数和零 escape 外，还会重新下载、受限读取并解析 Linux/macOS/Windows 三份 raw live JSON，要求其内容与各平台 outer boundary 逐字段一致。
- formal 与 PR smoke 使用不同 result/aggregate schema；默认 verifier 拒绝 smoke，只有显式 `--allow-smoke` 才生成 `qualifyingEvidence=false`、`releaseGateEligible=false` 的 non-qualifying aggregate，且 smoke 不允许消费 formal live evidence。聚合结果明确记录每平台只有 **1 个** `single-live-sample-before-time-based-host-race-soak`，`longRunning=false`、`qualifyingForLongRunningMatrix=false`，不得把一次 live sample 与另一条两小时 soak 并列后外推为三平台长期 Worker 恶意矩阵。
- 本地合同验证为 **2 files / 14 tests passed**，actionlint、Prettier、Node syntax 与 `git diff --check` 通过；另用真实 Vitest JSON reporter 核对到 18 passed / 3 platform/live skips，确认 parser 使用的 `passed/skipped`、test path、ancestor/fullName 与 sentinel 名称匹配实际输出。该运行没有开启 `CC_SANDBOX_LIVE=1`，不计 live 平台通过。当前仍没有 `737038d30e` 后继最终 exact SHA 的三平台 formal artifact；macOS descriptor-bound atomic runtime exec、任意 shared-library closure、durable distributed revoke 以及长期重复 fresh-isolate 矩阵也仍未实现，因此第 18.4 节任务 3 保持 **NO-GO**。

### 18.21 2026-08-15 MCP 长期重复 fresh-isolate 证据合同

- `2522077dfe` 把第 18.20 节的一次性 live sample 升级为 formal v3 长期重复合同。每个平台由同一个两小时 soak orchestrator 直接启动仓库中精确安装的 Vitest entry，不使用 `npx`；三个独立 fresh process 的 slot 固定为 `000/001/002`，以第一份样本的 monotonic start 为锚，分别在 `+0/+3600/+7200s` 调度。每份 raw JSON 都绑定 exact SHA、OS、slot、测试源码 SHA-256、raw SHA-256、平台 sentinel、reporter `startTime`、test-file start/end 和外层 sample wall interval；复制旧 raw 冒充后续 fresh run 会因时间区间不匹配而失败关闭。
- formal aggregate 必须从 Linux、macOS、Windows 下载并精确消费 **9 份** raw JSON，拒绝缺失、额外或重复文件，逐份重读后与平台 outer evidence 完整匹配，并要求每个平台至少 3 个样本、首末 start span 至少 7200 秒、最大调度迟到不超过 1200 秒。PR smoke 固定为 0 live sample，继续使用 non-qualifying schema，`qualifyingEvidence=false`、`releaseGateEligible=false`。本地合同为 **1 file / 11 tests passed**；actionlint、Node syntax、Prettier 与 `git diff --check` 通过，另以未开启 live 条件的真实 reporter 输出核对时间字段结构，该运行不计 live 证据。
- 该提交只形成可执行且可聚合的候选合同，尚未取得包含它的最终 exact SHA 三平台两小时 artifact；aggregate 继续固定 `completeSkillMcpSecurityClosure=false`。因此它不能关闭 macOS atomic runtime exec、任意 shared-library closure 或 distributed revoke，第 18.4 节任务 3 仍为 **部分完成 / NO-GO**。

### 18.22 2026-08-15 Linux native ELF 启动依赖图增量

- `3e1ef7d6bf` 把 Linux strict native Plugin 的启动期证据从 entry 的 direct `DT_NEEDED` 集扩展为 descriptor-bound 递归图：从 `PT_INTERP` 和 entry 的 `DT_NEEDED` 出发，沿已经按 loader 实际 name/destination 解析并 pin 住的 root-owned FD 递归读取所有 ELF dependency；每个成员都重新解析、SHA-256、前后 `fstat`，并把 member/edge 图纳入 digest。实现拒绝缺失二级依赖、basename/SONAME 歧义、dependency interpreter、`RPATH/RUNPATH/AUDIT/FILTER` 等 loader 指令，允许有界 cycle，并设置 256 files、1024 edges、512 MiB 的绝对上限。补强前完整套件为 **286/286 passed**；最终文件共 287 项，SONAME/递归筛选为 **9 passed / 278 skipped**。Node syntax、Prettier 与 `git diff --check` 通过。
- 该证据名称明确限定为 `initial-pt_interp-and-recursive-dt_needed-attested-system-graph`，Broker 仍强制 `sharedLibraryClosure=false`。它只证明 Linux loader 的递归**启动图**，不覆盖运行期 `dlopen`、可执行动态生成代码或可写 scratch 中的后续加载；Windows delay-load/`LoadLibrary` 与 macOS `@rpath`/`dlopen`/dyld 也没有闭环。故“任意 shared-library closure”仍未完成，不能据此改变任务 3 的 NO-GO。

### 18.23 2026-08-15 同机 Remote approval membership epoch fence

- `465a6ad41a` 为 client-hosted Remote approval 增加同机 durable membership authority。session create、member join/revoke 与 session close 先在 machine security-anchor 下用严格跨进程锁、fsync 和 atomic replace 追加单调 epoch 事件，再改变当前 server 的内存 registry；未知持久化结果会把本地 session/member quarantine。server 只转发由自身授权得到的 `from + sessionEpoch + membershipEpoch`，host bridge 在同一 membership lock 内复核 active principal/session/scope/epoch，并在锁释放前执行同步 approval-store CAS；缺字段、旧 epoch、已撤销/过期 membership、损坏或不可用 store 均在 approval CAS 前失败关闭。durable approval record 与审计 provenance 同时保留 remote/session/member epoch。
- 扩大后的定向矩阵为 **12 files / 140 tests passed**，其中真实 WebSocket integration 7/7；覆盖 authority 实例重建、重复/乱序 epoch、revoke 后 late frame、rejoin 新 epoch、状态故障 quarantine、async/thenable fenced callback 拒绝，以及 legacy client-hosted approval 缺 durable binding 时拒绝。目标 ESLint 为 0 errors（`ws-server.js` 仅 14 个既有 warnings），Prettier、Node syntax 与 `git diff --check` 通过。
- 本切片的 principal 只是当前 server connection/relay identity，且 server/registry 不会在重启后从该 store 恢复 active session。它只证明同 OS 用户、同 machine/store、当前 server 进程内 membership transition 与 approval CAS 的线性化；不证明跨主机 consensus/quorum、server restart recovery、所有观察进程退出后的 offline rollback 检测，亦不撤销已经赢得 CAS 并 dispatch 的副作用。因此 durable **distributed** revoke 仍未关闭，当前 10 完成/5 部分完成、六项专项 3 完成/3 未完成及产品 **NO-GO** 判定均不变。

### 18.24 2026-08-15 Linux runtime pathname loader closure 候选

- 未发布源码候选把第 18.22 节的递归启动图扩展为 Linux `dynamic-native` 的窄口径 **runtime pathname loader closure candidate**。最终 bwrap namespace 只挂载逐文件 descriptor-pinned、SHA-256 绑定的 Plugin 全树、Node runtime、system DSO 与 `/etc/ld.so.cache`；synthetic root 和用于遮蔽 supervisor staging 的唯一 `/run` tmpfs 均在 target 前 remount read-only，`/tmp`、`/var/tmp`、HOME、procfs、devfs 与 writable scratch 不存在。behavior probe 与最终 target 使用完全相同的 ordered policy args。dynamic-only seccomp 在 x64/arm64/riscv64 上以 `EPERM` 拒绝 `recvmsg`、`recvmmsg`、`pidfd_getfd`、`open_by_handle_at`、`unshare`/`setns`、旧/新 mount API 与 classic `clone` 的 `CLONE_NEWUSER|CLONE_NEWNS` flags；不可检查用户指针的 `clone3` 整体返回 `ENOSYS`，让 glibc 回退到受 flags 过滤的 classic `clone`。这同时关闭 `SCM_RIGHTS`、跨进程 FD、filesystem handle 与 nested writable mount 的 loader-input 引入面。
- adversarial live 合同要求 descriptor-mounted approved `.so` 的 `dlopen`/`dlsym` 成功；host 中存在但 namespace 未挂载的同/异 SONAME、写入 `/tmp`/`/var/tmp`/`/run`/HOME/Plugin tree 的 drop-in、nested user/mount namespace tmpfs、`/proc/self/fd`、`/dev/fd`、继承 regular FD、pidfd 与 file-handle 重开全部失败。entry 与 approved `.so` 在 Broker admission 后、真实 native spawn 前被原 host pathname 同 inode 改写时，sandbox 仍必须执行 snapshot/pinned 的原始字节。非沙箱 control 在宿主允许 unprivileged userns 时必须实际完成 nested tmpfs drop-in `dlopen`；宿主自身禁用 userns 时只记录 unsupported，不能冒充 seccomp 证据。
- 该 capability 只覆盖 libc/ELF loader 的 pathname resolution；Broker 继续强制 legacy/broad `sharedLibraryClosure=false`，匿名 JIT、可执行动态生成代码和恶意 custom in-process loader 明确不在声明内。Windows DLL/delay-load/`LoadLibrary` 与 macOS `@rpath`/`dlopen`/dyld 仍未闭环。本机 WSL1 没有可用 bwrap/Node，只有 C 编译和非沙箱正向对照；尚未形成包含该候选的最终 clean exact-SHA Linux CI live artifact。因此原始任务仍为部分完成，当前 10 完成/5 部分完成、六项专项 3 完成/3 未完成及完整产品 **NO-GO** 判定均不变。
- 当前只验证 plain Node runtime；yao-pkg packed `process.execPath` 因未设置 `PKG_EXECPATH=PKG_INVOKE_NODEJS` 会 probe fail-closed，尚未支持。

### 18.25 2026-08-15 `0.164.0` CLI 发布候选范围与门禁

- `release/cli-0.164.0` 以 `8f263ecc2fd5b314953f88f2a4f31b5756f2a0ff` 为功能基线。该基线包含 `0.163.8` 之后已经提交的 clipboard image、scoped runtime/execution-location authority、IDE context center、P1-2 keeper containment 与正式聚合合同、MCP fresh-isolate 内容绑定和长期重复合同、governed marketplace 选择/安装/影响/readback、同机 Remote approval membership epoch fence、Linux runtime pathname loader closure 候选，以及 macOS signed/root launcher 的 fail-closed 安装与实机门合同。本次 release 分支只增加 CLI version、根 lockfile、根 CHANGELOG、内置 changelog artifact 与本进度文档，不把共享工作树中的其他未提交修改混入候选。
- `0.164.0` 只满足既定 alias removal floor 的版本条件，不满足代表性数据条件。权威 `0.163.1` 报告仍是 **0 retain / 0 remove / 25 insufficient-data**；操作结果继续为 25/25 compatibility alias 全部保留。没有获批 Collector/cohort export、Linux/macOS/Windows reporting coverage、非零 accepted points、sample rate 与逐命令 legacy/replacement usage 时，不得仅因版本号到达 floor 删除入口或把 P1-1 标记完成。
- 未提交的 remote `run_shell` lease-consumption vertical 明确排除在本候选外。其 coordinator 27/27、ApprovalGate 36/36、七类 shell dispatch 9/9、run_code/browser fail-closed 22/22 和 bridge 单独 9/9 均通过，但扩大 remote 矩阵为 69/70，server-restart 用例在并行负载下等待事件超时；因此它不满足稳定绿色门，不能为发布降级或选择性计入。
...
### 18.27 2026-08-16 `0.164.0` npm 发布子任务正式闭环

- 第 18.26 节记录的队列态已经结束：[Publish CLI release to npm `31912844032`](https://github.com/chainlesschain/chainlesschain/actions/runs/31912844032) 已在不可变 tag `v-npm-0-164-0` 所绑定的 exact SHA `313dec85cffa09dbb183be17d2b6597e303bed5f` 上完成且结论为 success。公网 npm 当前回读 `latest=0.164.0`；公开 tarball 为 `https://registry.npmjs.org/chainlesschain/-/chainlesschain-0.164.0.tgz`，registry SHA-1 为 `2d13836305c6841b7fe5bcd059bc8ef25127fc3d`，integrity 为 `sha512-7Xi0/QG7z0ce15m3EQhP/QVf8aU9cByy4AKqHb8uSlIREW2e+CV9llOthFbgPaOa9lXg1/2nHlkY9buzQdt8pA==`。
- 独立公网回读 [CLI npm release readback `31913903124`](https://github.com/chainlesschain/chainlesschain/actions/runs/31913903124) 同样绑定上述 SHA 并成功。不可变 artifact `cli-npm-public-readback-313dec85cffa09dbb183be17d2b6597e303bed5f` 的 ID 为 `9254431092`、artifact digest 为 `sha256:05ced978391aea9fc30ab295cf56aeba12f07f00b7b161958fa3521da2782509`；回读的 registry tarball 与发布候选逐字节一致，大小为 `6496711` bytes，SHA-256 为 `4d7f36f11bd2bfc1b85eee87392b74bd948e7cc33698cd35c87bff72fb3b4c16`，SHA-512 为 `ed78b4fd01bbcf471ed799b711084ffd055ff1a53d701cb2e002aa1dbf2e4a5211116d9ef8257d9653ad8456e03da39af655e0d7fda71e5918f5bbb341db7ca4`。`npm audit signatures` 对 Trusted Publishing provenance 的校验结果为 invalid 0、missing 0，并绑定 `refs/tags/v-npm-0-164-0`、`.github/workflows/npm-publish.yml` 与发布运行 `31912844032`。
- 因此 `0.164.0` 的 **npm 发布运行态子任务已关闭**。它只关闭 npm tarball/provenance/公网回读链，不等同于签名 native 发行、alias telemetry、keeper、scheduler campaign 或 Skill/MCP 安全闭环；原始统计仍为 **10 完成 / 5 部分完成 / 0 完全未开始**，去重后的实施队列仍为第 18.26 节的 **5 个工作包**，完整产品继续 **NO-GO**。

### 18.28 2026-08-17 `0.165.1` 发布闭环、长时门启动与 MCP Linux 预检修复

本节是 `2026-08-17 23:10 +08:00` 的最新判定入口，优先于第 18.26～18.27 节的运行态快照，但不改写历史证据。

- 当前 `main` 与不可变 tag `v-npm-0-165-1` 均指向 `1a10ed7c8fd14d12f7760e948ff8efe36c766602`。该 exact SHA 的 [CLI CI `32038591204`](https://github.com/chainlesschain/chainlesschain/actions/runs/32038591204) 与 [CLI Strict Sandbox `32038590960`](https://github.com/chainlesschain/chainlesschain/actions/runs/32038590960) 均成功；专用 [npm publish `32038590940`](https://github.com/chainlesschain/chainlesschain/actions/runs/32038590940) 和独立 [readback `32039659372`](https://github.com/chainlesschain/chainlesschain/actions/runs/32039659372) 也均成功。公网 `latest=0.165.1`，tarball 为 `chainlesschain-0.165.1.tgz`，大小 `6,563,068` bytes，SHA-1 `51ce645bebbe63ae168386cb8b6122a67c7d0813`、SHA-256 `5806d34b6659dcc632dad31db3acdc2f4fa07fb924b7d93de1d22637a3186241`、SHA-512 `5b87165e5ded854c824d11bded46f016af62618a147bfffc500139c7e00a5b982ceac0632a1102c164d0d9755da9686c8cf194a9b82ebaa27b6ffd698d6d4815`，与发布候选逐字节一致；provenance audit 为 `invalid=0`、`missing=0`。readback artifact `9291704204` 的 digest 为 `sha256:31ade86e67299b0415bc07d10d47d87ed534b76a727fafd431317511e8b6e9a5`。这只关闭 npm 子链，不关闭签名 native 发行或以下五个工作包。
- P2-4 已为上述 exact SHA 固定 repository variables：`CLI_SCHEDULER_SOAK_PINNED_SHA=1a10ed7c8fd14d12f7760e948ff8efe36c766602`、campaign `p2-4-scheduler-long-soak-1a10ed7c8fd1`、seed `1592598566`。首个 [Scheduler Kernel Soak `32040735070`](https://github.com/chainlesschain/chainlesschain/actions/runs/32040735070) 已进入 Linux、Windows、macOS 的 7200 秒正式执行；在三平台与 aggregate 成功前它仍是**进行中且计数为 0/4**。后续 schedule 只有在继续绑定相同 SHA/campaign/seed、相邻启动不超过 30 小时且最终四段覆盖至少 72 小时时才能交给 campaign verifier；排队、失败、取消或单平台结果仍不计段。
- [CLI Reliability Soak `32040628208`](https://github.com/chainlesschain/chainlesschain/actions/runs/32040628208) 的首次 formal MCP 矩阵在 Linux 第一个 fresh-isolate 样本即失败。outer artifact `9291894750` 和 raw artifact `9291894642` 证明不是逃逸：Broker 因 runner 未安装并证明所需 bubblewrap feature set，以 `linux_bwrap_unavailable:binary_missing_or_unattested` 在任何 host cycle 前失败关闭；因此该 run 已请求取消且不能计长期恶意证据。提交 `414c8b94ab` 的 [PR #216](https://github.com/chainlesschain/chainlesschain/pull/216) 将 Strict Sandbox 已使用的 bubblewrap 0.11.2 构建、源包 SHA-256、descriptor/security option 探针和 private mount 设置提取为共享 action，并让 Linux MCP formal soak 使用同一 action；本地 workflow 合同为 **2 files / 28 tests passed**，actionlint、YAML parse 与 `git diff --check` 通过。PR 最终 exact SHA、双门和合并后 formal 三平台 artifact 尚未完成，故 Skill/MCP 项仍为**部分完成 / NO-GO**。
- 当前 [macOS MCP launcher gate `32031398565`](https://github.com/chainlesschain/chainlesschain/actions/runs/32031398565) 的 x64/arm64 **unsigned contract** 成功，但 protected tag、exact-SHA release gates、signed-root live 与 sealed release evidence jobs 均因普通 `main` push 条件而 skip；它不能被写成签名/公证实机闭环。仓库仍只有 `pypi` environment，没有 `native-production` 或组织签名/公开渠道授权，因此 native 签名发行继续是外部前置 blocker。
- 当前仍没有获批的代表性 alias Collector/cohort export、三平台 reporting coverage、非零 accepted points、sample rate 与逐命令 legacy/replacement usage；`0.165.1` 版本增长不替代观察数据，25/25 compatibility alias 继续保留。当前 `main` 也没有 20-Agent、7200 秒、1000-cycle 的三平台 keeper formal aggregate；PR smoke、单平台或旧 SHA 不得替代。

因此第 18.26 节归并的 **5 个不重复工作包没有提前减少**：原始 15 项仍为 **10 完成 / 5 部分完成 / 0 完全未开始**，六项产品任务仍为 **3 完成 / 3 未完成**。本节的实质进展是完成 `0.165.1` npm 子闭环、启动当前 exact SHA 的 P2-4 campaign，并把 MCP Linux formal 预检缺口收敛为正在验证的 PR #216；完整 CLI 产品、签名 native 公开发行、alias 最终删除、keeper、P2-4 campaign 与 Skill/MCP 完整恶意闭环仍为 **NO-GO**。

### 18.29 2026-08-18 可靠性修复合并、远端 revoke 复核与 P2-4 首段成功

本节是 `2026-08-18 01:15 +08:00` 的后继运行态，优先于第 18.28 节的 PR/长时门进行中描述，但不把仍在排队或尚未达到观察窗的证据预写为成功。

- [PR #216](https://github.com/chainlesschain/chainlesschain/pull/216) 已以 merge commit `cb76d13d6c5b4e546beae2a2f7eda60c4772f5bd` 进入 `main`。其 final head `ed8199673b5b4f2318c45f1fd9991fccbb11a908` 的 [CLI Strict Sandbox `32041592671`](https://github.com/chainlesschain/chainlesschain/actions/runs/32041592671) 为 Ubuntu 24.04、macOS 15、Windows **3/3 success**；[CLI Reliability Soak smoke `32041592690`](https://github.com/chainlesschain/chainlesschain/actions/runs/32041592690) 的 9 个平台 jobs 与 MCP/scheduler 两个 aggregate 共 **11/11 success**。MCP smoke aggregate artifact `9292297401`（digest `sha256:cdfa36babcbac4aa5d8526e25b59df5e4729fe3b8cabc8255f8b7ca7a5abda08`）仍明确是 non-qualifying smoke，但已证明共享 bubblewrap 0.11.2 setup 修复了第 18.28 节的 Linux `binary_missing_or_unattested` 预检失败，且没有把失败关闭误报成逃逸。
- [PR #215](https://github.com/chainlesschain/chainlesschain/pull/215) 的 final head `89c498cc46fd96fa4952c4e6aa255a884738cd73` 已通过 [CLI CI `32042061014`](https://github.com/chainlesschain/chainlesschain/actions/runs/32042061014) **53/53 jobs success**、[CLI Strict Sandbox `32042060875`](https://github.com/chainlesschain/chainlesschain/actions/runs/32042060875) **3/3 success**，并以 merge commit `089336992dfee3a70332bb16e267601d8d7656a2` 进入 `main`。同一 head 的 [Background Agent Keeper Soak smoke `32042060859`](https://github.com/chainlesschain/chainlesschain/actions/runs/32042060859) 为 Linux、macOS、Windows 与 aggregate **4/4 success**；aggregate artifact `9292283019` 的 digest 为 `sha256:95259028a1d5ccba4af6fdb191ea8b841baeea1b944d31cd81146b560026cdf3`。该 smoke 只证明候选合同可执行，不能替代 20-Agent/7200 秒/1000-cycle formal 退出门。
- [PR #217](https://github.com/chainlesschain/chainlesschain/pull/217) 已以 merge commit `7ac283021fb18aba833020455a684355c44085c0` 进入 `main`。它新增 `15:06 UTC` 自动 dispatcher，只从不可变 tag `v-npm-0-165-1` 调度 exact SHA `1a10ed7c8fd14d12f7760e948ff8efe36c766602`，并固定 campaign `p2-4-scheduler-long-soak-1a10ed7c8fd1`、seed `1592598566`；dispatcher 对 tag 的直接 commit 解析、成功/活动 segment 数、重复/重叠和最多四段实行失败关闭。旧 default-branch daily schedule 已移除，避免 `github.workflow_sha` 随 `main` 前进而产生无法被同一 campaign verifier 聚合的 3×2 小时运行。PR final head `fc46fe08908d631ecd6b5e6462609c5cbb8c957a` 的 6 个 branch-protection required checks 全绿，[Scheduler Kernel Soak smoke `32045604453`](https://github.com/chainlesschain/chainlesschain/actions/runs/32045604453) 三平台与 aggregate **4/4 success**；aggregate artifact `9292958348` 的 digest 为 `sha256:ac8da6021a253a0d10131c576fcd2b611711e61fd9a617f59d7428e280cef728`。
- [PR #219](https://github.com/chainlesschain/chainlesschain/pull/219) 随后以 merge commit `935baea08fa789f27d366977d81d19c12130fad0` 进入 `main`，把上述固定时刻 dispatcher 改为每小时 `:06` 轮询。planner 只有在距最后一个成功 segment 的最早平台开始时间达到 24 小时、没有活动 segment 且成功数小于 4 时才调度下一段；恰好取得四段后则自动从 default branch 调度 campaign verifier，并用包含 campaign/SHA 的 run name 阻止重复 verifier、允许失败 verifier 后续重试。final head `547bbef542a811729f683fcfb770eb22effa4a2d` 的 6 个 branch-protection required checks 全绿，[Scheduler Kernel Soak smoke `32050180934`](https://github.com/chainlesschain/chainlesschain/actions/runs/32050180934) 三平台与 aggregate **4/4 success**；aggregate artifact `9294866805` 的 digest 为 `sha256:67147a695f77b72dc65b1e118f404ff9782bcb670cac7ec98f32039ea48eaa39`。同一 head 的手工 [dispatcher smoke `32050243753`](https://github.com/chainlesschain/chainlesschain/actions/runs/32050243753) 与最终 main 的 [dispatcher run `32053718793`](https://github.com/chainlesschain/chainlesschain/actions/runs/32053718793) 均成功，并因当前只有 1/4 且未达到 24 小时间隔而同时跳过 segment 与 verifier dispatch；这证明 PR head 与合并后 default branch 的时间门都实际失败关闭，不代表第二段已开始。
- P2-4 的首个当前 campaign 正式 segment [Scheduler Kernel Soak `32040735070`](https://github.com/chainlesschain/chainlesschain/actions/runs/32040735070) 已完成且 Linux、macOS、Windows、aggregate **4/4 success**。aggregate artifact `cli-scheduler-soak-aggregate-1a10ed7c8fd14d12f7760e948ff8efe36c766602-1` 的 ID 为 `9293937035`、digest 为 `sha256:0dd421cd2e5744d21cf74c704117de639ac6737ec7a19b3c3f7b9ce2f7cd8da2`；回读 JSON 为 `result=passed`，source/control-plane SHA 均为 `1a10ed7c8fd14d12f7760e948ff8efe36c766602`，profile 为每平台 `7200s`、100 rounds、1000 steady occurrences，连续覆盖 `7343.051s`，三平台合计 300 rounds、3000 steady occurrences、600 hard kills、3603 effects。该 run 的最早平台开始时间 `2026-08-17T14:57:48.079Z` 定义当前 `T0`，正式计数更新为 **1/4**；仍须取得后续三段并让四次启动覆盖至少 72 小时、相邻启动不超过 30 小时，再通过 campaign verifier。
- `0.165.1` 已包含生产远端 revoke/dispatch fence，而不是第 18.23/18.28 节旧描述中的“尚未接线”：生产 WebSocket server、Remote Session 协议、host receipt store、approval bridge 与 Agent `run_shell` 在副作用前在线消费签名 one-shot lease；没有 consume 边界的路径拒绝 Remote approval，双 WS server/独立 host root 的 revoke-vs-consume 集成场景也已进入 exact release tree。该实现由一个在线、强一致、durable coordinator 线性化，能够在 consume 之前跨 host 阻断已撤销成员；它**不**声明 quorum/consensus 可用性，也不能撤销已经赢得 consume 并开始的外部副作用。`0.165.1@1a10ed7c8fd14d12f7760e948ff8efe36c766602` 的 CLI CI `32038591204`（52 jobs success）与 Strict Sandbox `32038590960`（3/3 success）是其发布级 exact-SHA 门禁。故“生产远端 revoke 未接线”缺口已关闭，剩余边界应改写为单 coordinator 的可用性/非共识限制，而不是继续把代码垂直切片列为缺失。
- 当前最终 main 候选为 `935baea08fa789f27d366977d81d19c12130fad0`；其显式 exact-ref [CLI Strict Sandbox `32052727554`](https://github.com/chainlesschain/chainlesschain/actions/runs/32052727554) 已三平台 **3/3 success**，显式 [CLI CI `32053616009`](https://github.com/chainlesschain/chainlesschain/actions/runs/32053616009) 仍在队列中。自动 push CLI CI `32052502121` 被同 SHA 的显式 dispatch 通过 workflow concurrency 取代，不能计通过。前一 main SHA `7ac283021fb18aba833020455a684355c44085c0` 的 CLI CI `32049297660` attempt 1 有两个集成事件等待超时；attempt 2 的两个失败 shard 均已成功，但 PR #219 合并触发的 main concurrency 在 Windows `verify-cli` 结束前取消了该旧 run，因此同样不能写成完整 CLI CI success。20-Agent/7200 秒 keeper formal 和两小时 MCP formal 尚未启动，P1-2 与 Skill/MCP 长期证据不能在本快照提前关闭。
- 外部前置没有变化：GitHub environments 仍只有 `pypi`，没有 `native-production`；repository secrets 也没有 macOS Developer ID/notary、Windows Authenticode、Linux/native channel 所需身份。alias 侧没有获批 Collector/cohort export 或三平台非零 accepted points。仓库内 workflow/preflight 可以失败关闭，但不能自行生成组织签名身份、公开渠道授权或代表性用户数据；在这些输入到位前，native 与 alias 两个工作包仍保持 **NO-GO**，25/25 compatibility alias 继续保留。

因此本快照把 P2-4 从 0/4 推进为 **1/4**，并关闭“生产远端 revoke 尚未接线”这一过时子缺口，但尚未减少第 18.26 节的五个去重工作包：原始 15 项仍为 **10 完成 / 5 部分完成 / 0 完全未开始**，六项产品任务仍为 **3 完成 / 3 未完成**。只有 final main 双门、keeper/MCP formal、其余三段 scheduler 与 verifier 分别形成终态后，才更新完成数；签名 native 与代表性 telemetry 仍需仓库外授权/数据。

### 18.30 2026-08-18 最终 main 可靠性门与 keeper 失败闭环

本节记录第 18.29 节之后已经合并的三个 keeper 修复批次及正式运行证据。历史失败仍保留为诊断证据；只有同一最终 exact SHA 的完整门禁和 formal aggregate 才能改变完成计数。

- [PR #220](https://github.com/chainlesschain/chainlesschain/pull/220) 修复首个 POSIX destructive identity probe 的瞬时不可用窗口：只有连续两次 fresh exact identity match 才允许首次 signal，retired/reused 会收敛退出，持续 unknown 则零 signal 失败关闭。其 final head `50ff3a396b7f89a81edb99c46bd861d3d1c072c2` 的 [CLI CI `32056788944`](https://github.com/chainlesschain/chainlesschain/actions/runs/32056788944) 为 **53/53 success**、[CLI Strict Sandbox `32056788628`](https://github.com/chainlesschain/chainlesschain/actions/runs/32056788628) 为 **3/3 success**、[keeper smoke `32056788661`](https://github.com/chainlesschain/chainlesschain/actions/runs/32056788661) 为 **4/4 success**；merge commit `d2a3191b1c670702fc212d9e7e580955a3bad3cb` 的 [CLI CI `32061526591`](https://github.com/chainlesschain/chainlesschain/actions/runs/32061526591) 为 52 success、1 条件 skip、0 failure，[CLI Strict Sandbox `32061526309`](https://github.com/chainlesschain/chainlesschain/actions/runs/32061526309) 为 **3/3 success**。旧 main 的 [keeper formal `32055103982`](https://github.com/chainlesschain/chainlesschain/actions/runs/32055103982) 在首轮约 16.927 秒以 `creation-time-probe-failed` 失败，因此只计失败诊断，不计正式矩阵。
- `d2a3191b1c670702fc212d9e7e580955a3bad3cb` 的 [CLI Reliability Soak `32063505903`](https://github.com/chainlesschain/chainlesschain/actions/runs/32063505903) 已完成且 **11/11 jobs success**。MCP formal aggregate artifact `9302945230`（digest `sha256:ddca646021ee213c8e109bd8dff27662ec26c25dce315b1259aeee3079bd7213`）覆盖 Linux、macOS、Windows 每平台 7200 秒、100 host cycles、3 个 fresh-isolate 样本，三平台共 300 host cycles、900 个未批准副作用样本、900 次零未批准 transport/mutation/ledger write、300 approved mutation probes 与 600 stale-host-read policy samples；首末样本最短跨度为 `7200.595869209s`。scheduler reliability aggregate artifact `9302918383`（digest `sha256:1bd6438c2e149874b6f5e35671488c97f7050532a3c3b8c0237192f37eda87c3`）覆盖每平台 7200 秒、1000 cycles，三平台共 3000 cycles、9000 logical occurrences/jobs、18000 claims、3000 contention rejects、3000 stale-lease rejects、0 duplicate execution 与 0 invariant violation。MCP aggregate 仍明确写入 `completeSkillMcpSecurityClosure=false`，所以它关闭的是本轮长期重复矩阵，不把 macOS atomic runtime exec 或任意 shared-library closure 外推为完成。
- 同一 `d2a3191b1c670702fc212d9e7e580955a3bad3cb` 的 [Background Agent Keeper Soak `32063505570`](https://github.com/chainlesschain/chainlesschain/actions/runs/32063505570) **没有通过**。macOS 在约 4256.389 秒、5100 cycles 后遇到 `kill EPERM` 之后的 `process-group-state-probe-failed`；Windows 在约 7557.440 秒、1340 cycles 后，agent 14 的重启 readiness 超过 120 秒，keeper 日志显示 `Could not acquire state lock`，磁盘状态停在 `turn_launch_uncertain`。两平台 failure cleanup 最终均确认 process tree 退休和 worktree 删除，但失败本身不能被 cleanup 成功抵消。
- [PR #221](https://github.com/chainlesschain/chainlesschain/pull/221) 把 PR #220 的 fresh exact-match 规则扩展到 post-`EPERM` process-group probe；持续 unknown 继续零重发、失败关闭。final head `67b2b53e104012b2fd83f7123da89a3eac1f4861` 的 [CLI CI `32072080058`](https://github.com/chainlesschain/chainlesschain/actions/runs/32072080058) 为 **53/53 success**、[CLI Strict Sandbox `32072078950`](https://github.com/chainlesschain/chainlesschain/actions/runs/32072078950) 最终为 **3/3 success**、[keeper smoke `32072079081`](https://github.com/chainlesschain/chainlesschain/actions/runs/32072079081) 为 **4/4 success**；该 PR 已以 merge commit `fa45e934d079ed03efc378e49d4420e33a7cdf2f` 进入 `main`。
- [PR #222](https://github.com/chainlesschain/chainlesschain/pull/222) 关闭上述 Windows 状态锁残留：owner 只有在临界区结束后才写 exact-token release marker；后续 contender 必须同时匹配 owner、release marker，并通过独占 claimant marker 才能完成 Windows 瞬时共享错误留下的清理。错误 token、损坏 marker 或仍存活的其他 claimant 均保持失败关闭，死 claimant 才能被精确接管；keeper workflow 的路径过滤也加入锁实现与回归测试。final head `1b7c29e54d8855c45d0a56cd25c36522e64c5986` 的 [CLI CI `32077175615`](https://github.com/chainlesschain/chainlesschain/actions/runs/32077175615) 为 **53/53 success**、[CLI Strict Sandbox `32077175438`](https://github.com/chainlesschain/chainlesschain/actions/runs/32077175438) 为 **3/3 success**、[keeper smoke `32077175469`](https://github.com/chainlesschain/chainlesschain/actions/runs/32077175469) 为 **4/4 success**。smoke aggregate artifact `9304157633`（digest `sha256:385a5a4fd32c4234a2e02fb9d0e85c4aaae4fd4565ec9377bdce2810864ef456`）记录 9 agents、21 cycles、0 violation、0 surviving PID/worktree，并保持 `qualifyingEvidence=false`、`releaseGateEligible=false`。该 PR 已以 merge commit `d5f646b0106d2f994a55026091ed4e998421706f` 进入 `main`。
- 最终 `main@d5f646b0106d2f994a55026091ed4e998421706f` 的 keeper formal [run `32079799984`](https://github.com/chainlesschain/chainlesschain/actions/runs/32079799984)、Reliability formal [run `32079803745`](https://github.com/chainlesschain/chainlesschain/actions/runs/32079803745)、CLI Strict Sandbox [run `32079766766`](https://github.com/chainlesschain/chainlesschain/actions/runs/32079766766) 与 CLI CI [run `32079766848`](https://github.com/chainlesschain/chainlesschain/actions/runs/32079766848) 已精确绑定并启动，但在本句记录时尚未形成全部终态；在 keeper 三平台与 aggregate 成功前，P1-2 仍为**部分完成**，不得以 PR smoke 替代。
- P2-4 自动控制面继续正常工作：真实 schedule 触发的 [dispatcher `32076478225`](https://github.com/chainlesschain/chainlesschain/actions/runs/32076478225) 已在当时 `main@fa45e934d079ed03efc378e49d4420e33a7cdf2f` 成功完成 control-ref verification 与 planner，并因仍为 1/4 且距上一成功 segment 不足 24 小时而跳过 segment/verifier dispatch。正式 campaign 继续固定 `v-npm-0-165-1@1a10ed7c8fd14d12f7760e948ff8efe36c766602`、campaign `p2-4-scheduler-long-soak-1a10ed7c8fd1`、seed `1592598566`；当前仍为 **1/4**，必须等待后续三段和最终 verifier。

因此在上述最终 formal 仍进行中的快照，统计暂不提前变化：原始 15 项仍为 **10 完成 / 5 部分完成 / 0 完全未开始**，六项产品任务仍为 **3 完成 / 3 未完成**，去重实施队列仍为 **5 个工作包**。若最终 exact-SHA keeper formal 成功，才把 P1-2 更新为完成，并相应改为 **11 完成 / 4 部分完成**、去重 **4 个工作包**；native 签名发行、alias 代表性 telemetry、P2-4 72 小时 campaign 和 Skill/MCP 完整安全闭环仍不能由该结果代替。

### 18.31 2026-08-18 `0.165.2` exact-SHA 发布闭环与最终 formal 重启

本节优先于第 18.30 节的运行中快照，将已经完成的修复、精确提交门禁和 npm 发布先行归档；新启动的长期运行仍按未完成处理，待 aggregate 形成终态后再单独补记，避免以排队或局部平台结果预写成功。

- [PR #224](https://github.com/chainlesschain/chainlesschain/pull/224) 补齐 logical release marker handoff 与 marketplace cache 清理边界，并以 merge commit `04e5753e5119d7e1aed34277143a1342583a03ee` 进入 `main`。其 final head `91f8d1f34d8d7792f0c77e62110f8034fc2551a0` 的 [keeper smoke `32095800673`](https://github.com/chainlesschain/chainlesschain/actions/runs/32095800673) 为 Linux、macOS、Windows 与 aggregate **4/4 success**；aggregate artifact `9310154961` 的 digest 为 `sha256:9236ad9bfe229f5bc50de9024973897378dbd737714bc2cea209bb5423c52113`。[CLI Strict Sandbox `32095800822`](https://github.com/chainlesschain/chainlesschain/actions/runs/32095800822) 为 **3/3 success**，[CLI CI `32095800904`](https://github.com/chainlesschain/chainlesschain/actions/runs/32095800904) attempt 2 为 **53/53 success**。第 18.30 节绑定旧 `d5f646b0` 的 formal，以及随后绑定 `04e5753e` 但被新发布候选取代的运行，均已取消且不计成功证据。
- [PR #226](https://github.com/chainlesschain/chainlesschain/pull/226) 将 CLI 升级到 `0.165.2`，同时修复 stale version/API-key fixtures、独占文件锁释放后的 detached cleanup、scheduler 正常 lease 时长与 IDE doctor 最低版本 fixtures。final head `98956881e352571cb30e2fd5ccb38af2904226e3` 的 [CLI CI `32106878622`](https://github.com/chainlesschain/chainlesschain/actions/runs/32106878622) 为 **53/53 success**，[CLI Strict Sandbox `32106878590`](https://github.com/chainlesschain/chainlesschain/actions/runs/32106878590) attempt 2 为 **3/3 success**，PR Tests、keeper smoke 与 reliability smoke 也全部成功。该 PR 已合并为 exact merge SHA `775d5667bc7bdcf8fed3f834d3ba2eda43526128`。
- 上述 merge SHA 的 [CLI Strict Sandbox `32110230842`](https://github.com/chainlesschain/chainlesschain/actions/runs/32110230842) 为 Ubuntu、macOS、Windows **3/3 success**；[CLI CI `32110231013`](https://github.com/chainlesschain/chainlesschain/actions/runs/32110231013) attempt 2 完整成功，包括三个平台的最终 `verify-cli`。attempt 1 仅在 Windows unit shard 的 `checkpoint-store.test.js` 遇到一次无 stderr 的 `git rev-parse --git-dir` 瞬时 exit 1；该精确测试在本地连续 **6/6** 成功，attempt 2 的同一 Windows 完整 shard 与全部下游门禁成功，因此保留为瞬时诊断记录而不是隐藏或忽略失败。
- 不可变 lightweight tag `v-npm-0-165-2` 已精确指向 `775d5667bc7bdcf8fed3f834d3ba2eda43526128`。生产 [Publish CLI to npm `32114134884`](https://github.com/chainlesschain/chainlesschain/actions/runs/32114134884) 已完成 exact-SHA gate、完整测试、tarball/SBOM、Trusted Publishing、provenance 与发布后 registry readback并成功；npm `latest` 已是 `chainlesschain@0.165.2`，tarball 为 `https://registry.npmjs.org/chainlesschain/-/chainlesschain-0.165.2.tgz`，integrity 为 `sha512-yru3Z+hrKo8NX5AVByvok7rioFoc76zxJ/IaBrhvyUyyjoUtQ/gL7GiGQ9Sbp5sulGNTgJC4cqO3k54OBAyF8w==`，shasum 为 `ac0fda8bd6523d4a91d7d46b8aa9bc4136b281bb`。这关闭的是 CLI `0.165.2` 的 npm 发布子链，不等同于 native 签名发行或所有差距项完成。
- 最终发布 merge SHA 上的 [Background Agent Keeper Soak `32115802655`](https://github.com/chainlesschain/chainlesschain/actions/runs/32115802655) 已按 20 agents、7200 秒、三平台加 aggregate 启动；[CLI Reliability Soak `32115806719`](https://github.com/chainlesschain/chainlesschain/actions/runs/32115806719) 也已按 7200 秒启动 disk/resource、scheduler reliability 与 malicious MCP 三平台矩阵及 aggregates。本快照两者仍在执行，故尚不计 formal 成功，也不提前改变 P1-2 或 Skill/MCP 状态。
...
### 18.33 2026-08-18 `0.165.2` formal 终态、P2-4 新 campaign 首段与 native 签名门

- exact release SHA `775d5667bc7bdcf8fed3f834d3ba2eda43526128` 的 [CLI Reliability Soak `32115806719`](https://github.com/chainlesschain/chainlesschain/actions/runs/32115806719) 已完成且 **11/11 jobs success**。MCP formal aggregate artifact `9321446634`（archive digest `sha256:cfe388ec00d0b3de1a3f683d020e11f9d5527e55eda4d969beb218dd880049b0`）覆盖 Linux、macOS、Windows 每平台 7200 秒、100 host cycles 与 3 个 fresh-isolate 样本，三平台合计 300 cycles、900 个未批准副作用样本、0 次未批准 transport/mutation/ledger write、300 approved mutation probes、600 stale-host reads，并取得至少 `7200.647s` 的 fresh-isolate 首末跨度。其 aggregate 仍明确写入 `completeSkillMcpSecurityClosure=false`，因此只关闭本轮三平台长期恶意重复矩阵；macOS atomic runtime `exec/open` 与任意 native/shared-library 递归闭包仍是独立 NO-GO 边界。
- 同一 reliability run 的 scheduler aggregate artifact `9321369314`（archive digest `sha256:b1dcbe4d29a7ca831f8052c2c22494f540457f92b8aac2579f28432173c7ab9c`）也为 `passed`：每平台 7200 秒、1000 cycles，三平台合计 3000 cycles、9000 logical jobs/occurrences、18000 claims、3000 contention rejects、3000 stale-lease rejects，且为 0 duplicate execution、0 invariant violation。该证据验证 scheduler 内核长期重复性质，不替代 P2-4 跨至少 72 小时的四段 campaign。
- exact release SHA 的 [Background Agent Keeper Soak `32115802655`](https://github.com/chainlesschain/chainlesschain/actions/runs/32115802655) 已形成失败终态，而不是继续运行：Linux 在 4160 cycles 后因 live state lock 导致 termination/cleanup 失败关闭，Windows 在 1680 cycles 后因 agent 13 readiness 超过 120 秒且 keeper 无法取得 state lock 失败，macOS 在 6980 cycles 后因两个 agent 的 process-tree cleanup 长时间未确认失败。Windows 与 macOS 的最终 failure cleanup 确认进程和 worktree 退休；Linux artifact 明确记录 cleanup 未确认。故 P1-2 仍为**部分完成**，不得把 reliability success 或 cleanup 成功平台替代 keeper formal aggregate。
- P2-4 已切换并固定到当前公开版本 `v-npm-0-165-2@775d5667bc7bdcf8fed3f834d3ba2eda43526128`、campaign `p2-4-scheduler-long-soak-775d5667bc7b`、seed `1592598566`。其第 1 段 [Scheduler Kernel Soak `32118985094`](https://github.com/chainlesschain/chainlesschain/actions/runs/32118985094) 为 Linux、macOS、Windows 与 aggregate **4/4 success**；aggregate artifact `9322734948`（archive digest `sha256:3e82fd3b1b620eafa1906c79ff8c22ddd9350b00d2ddbd96b921889866190e10`）回读为 `passed`，连续覆盖 `8893.676s`，每平台 7200 秒，三平台合计 300 rounds、3000 steady occurrences、600 hard kills、3603 effects。最早平台开始时间 `2026-08-18T09:07:09.268Z` 定义新 campaign 的 `T0`，当前正式计数为 **1/4**；仍须让后三段相邻启动间隔保持 24 至 30 小时、四次启动覆盖至少 72 小时，并通过最终 verifier。
- [PR #231](https://github.com/chainlesschain/chainlesschain/pull/231) 已以 merge commit `8dc1c6750927a163816cb79642c6fca6e7090f9a` 进入 `main`，把 native install/upgrade/rollback/readback 改为只接受与 manifest 精确绑定且签名、渠道、版本、哈希全部匹配的公开字节；缺失凭据、签名或公开渠道会失败关闭。final head `edfcd8295d15792862bda976db7e018db601cc58` 的 [CLI CI `32124313371`](https://github.com/chainlesschain/chainlesschain/actions/runs/32124313371) 为 **53/53 success**，[CLI Strict Sandbox `32124342632`](https://github.com/chainlesschain/chainlesschain/actions/runs/32124342632) 为 Ubuntu、macOS、Windows **3/3 success**，PR Quick Tests attempt 2 `32124313153` 成功。该合并关闭仓库内 signed-distribution gate 缺口，但仓库外仍没有 macOS Developer ID/notary、Windows Authenticode、Linux/Sigstore 与 Homebrew/WinGet 等生产身份和公开渠道授权，所以 P0-3/P1-5 的 native 正式发行仍是 **部分完成 / NO-GO**。

因此本节新增了两个已完成证据切片（release reliability formal 与当前 P2-4 segment 1），并合并了 native 失败关闭门；但 keeper formal 已失败、P2-4 仍为 1/4、Skill/MCP aggregate 明示非完整闭环，native 与 alias 仍受仓库外前置阻塞。原始 15 项仍为 **10 完成 / 5 部分完成 / 0 完全未开始**，六项产品任务仍为 **3 完成 / 3 未完成**，去重实施队列仍为 **5 个工作包**。

### 18.34 2026-08-19 `0.165.3` exact-SHA 发布闭环与 P1-2 keeper 正式关闭

- [PR #230](https://github.com/chainlesschain/chainlesschain/pull/230) 将 keeper 的状态锁恢复、进程树退休与不确定启动恢复修复汇总到最终 release head `7d3eb9ea8046330e2b264bf2309337ebbbbcbacc`，并以 merge commit `9664fb47a620ea89a41cc5f20bb3f7abb5099c26` 进入 `main`。该 exact source SHA 的 [CLI CI `32202407687`](https://github.com/chainlesschain/chainlesschain/actions/runs/32202407687) 为 **53/53 success**，[CLI Strict Sandbox `32202407501`](https://github.com/chainlesschain/chainlesschain/actions/runs/32202407501) 为 Ubuntu、macOS、Windows **3/3 success**，[keeper smoke `32202407421`](https://github.com/chainlesschain/chainlesschain/actions/runs/32202407421) 为三平台与 aggregate **4/4 success**。
- 同一 exact source SHA 的正式 [Background Agent Keeper Soak `32202463230`](https://github.com/chainlesschain/chainlesschain/actions/runs/32202463230) 已按 20 agents、7200 秒、1000 cycles 上限运行，Linux、macOS、Windows 与 aggregate **4/4 success**。aggregate artifact `9351159648`（archive digest `sha256:ca353e1246760ff69930a1b46bc8939c9b9a871a1547f262ad6bea96f669325a`）通过 exact-SHA、正式 release profile、三平台完整矩阵、零残留进程/工作树和最终 aggregate 校验。它直接取代第 18.33 节失败的旧 `0.165.2` keeper formal，满足 P1-2 的三平台长期 spawn/PID/keeper/restart 退出门，因此 **P1-2 从部分完成更新为完成**。
- annotated tag `v-npm-0-165-3` 的 tag object 为 `62b3fb604acec97fa6d335bfc0e8463ae510b2ef`，剥离后精确指向 `7d3eb9ea8046330e2b264bf2309337ebbbbcbacc`。生产 [Publish CLI to npm `32212502480`](https://github.com/chainlesschain/chainlesschain/actions/runs/32212502480) 的 exact-SHA gate、完整测试、不可变 tarball/SBOM、Trusted Publishing、provenance 与发布后回读均成功；发布 artifact `9352021029` 的 archive digest 为 `sha256:c4817caf1b5c2197095eadad02cd8fe595650a7eb74ddc86f9f9c621570e98bb`。公网 npm `latest` 已是 `chainlesschain@0.165.3`，tarball 为 `https://registry.npmjs.org/chainlesschain/-/chainlesschain-0.165.3.tgz`，integrity 为 `sha512-pv5c6gZ6PEyjD8B/0/XRmtohRvqzpl11AwOe9vyY86oTchOWABhXlitHxVlppccjq+URB/mYedoWmpYruAQctQ==`，shasum 为 `bd977831f8f429f1eeeda1f426639bdb3124ea52`。
- 独立 [CLI Public Release Readback `32215344457`](https://github.com/chainlesschain/chainlesschain/actions/runs/32215344457) 已从合并后的 `main@9664fb47a620ea89a41cc5f20bb3f7abb5099c26` 重新解析不可变 tag，校验 registry 字节、签名 provenance 与发布 workflow artifact 完全一致并成功；readback artifact `9352167608` 的 archive digest 为 `sha256:3cb78e11b6380477aee39c181dd58bdbb1652c24c9b62f7bbe93f81db55397c1`。这关闭 `0.165.3` 的 npm 发布子链，但不替代 native 安装包签名、公网 native 渠道、alias 代表性 telemetry 或 Skill/MCP 剩余平台边界。
- P2-4 仍固定 `v-npm-0-165-2@775d5667bc7bdcf8fed3f834d3ba2eda43526128`、campaign `p2-4-scheduler-long-soak-775d5667bc7b`、seed `1592598566`。第 1 段最早平台开始时间为 `2026-08-18T09:07:09.268Z`；真实 schedule dispatcher [run `32213823192`](https://github.com/chainlesschain/chainlesschain/actions/runs/32213823192) 在 `2026-08-19T03:56Z` 成功核验固定 tag/SHA 后，因尚未满足 86400 秒最小间隔而保持 **1/4**、未提前派发。后三段必须继续保持相邻启动间隔 24 至 30 小时，四次启动覆盖至少 72 小时后再由 verifier 形成终态。

因此本节正式关闭 P1-2 keeper 工作包。原始 15 项更新为 **11 完成 / 4 部分完成 / 0 完全未开始**；六项产品任务仍为 **3 完成 / 3 未完成**；去重实施队列降为 **4 个工作包**：native 签名发行、alias 代表性 telemetry、P2-4 72 小时 campaign、Skill/MCP 完整安全闭环。四项仍各自失败关闭，不以本次 npm 发布或 keeper 成功相互替代。

### 18.35 2026-08-19 native workflow 解析修复与外部退出门复核

- [PR #236](https://github.com/chainlesschain/chainlesschain/pull/236) 已以 merge commit `7020a6ea07a68fb7021c3f99989b9a7ba8c5b7b0` 进入 `main`。final head `689839f5ecf1296fd6da3b0c8f9ab7b6dcb0e904` 将 `.github/workflows/cli-native-release.yml` 中引用 `runner.temp` 的变量从不允许该 context 的 job-level `env` 移到实际消费步骤；此前每次 push 都会在创建 job 前产生的零 job workflow 解析失败因此消失，tag-only 触发语义保持不变。相关 release workflow contract **16/16 passed**，目标 Prettier、Node syntax、`actionlint` 与 `git diff --check` 均通过；PR 的 Linux/Windows Full Test、PDH 与 Lint 必需门也全部成功。该修复恢复 native release workflow 的可解析、可触发状态，不授权在缺少生产身份时发布。
- 仓库外前置复核没有出现可用的新授权：repository environments 仍只有 `pypi`，没有 native workflow 要求的 `native-production`，也没有 macOS atomic launcher 要求的 `cli-macos-mcp-launcher-production`；repository secrets 列表没有 CLI macOS Developer ID/notary、Windows Authenticode、Linux/Sigstore 或公开 native channel 所需身份。`main@9664fb47a620ea89a41cc5f20bb3f7abb5099c26` 的 [macOS MCP Launcher Release `32212456966`](https://github.com/chainlesschain/chainlesschain/actions/runs/32212456966) 仅有 unsigned x64/arm64 contract 成功，exact-SHA release gate、protected tag、signed-root live 与 aggregate 均为 skipped。因此 native 签名公开发行和 Skill/MCP 的 macOS atomic runtime 证据继续 **NO-GO**；仓库内失败关闭实现不能生成组织签名身份。
- Alias 侧同样没有可替代的真实输入：`REPOSITORY_APPROVAL_PUBLIC_KEYS` 仍为空，仓库 evidence 仍只有 `0.162.200` 与 `0.163.1` 两轮无代表性 Collector/cohort export 的报告；没有三平台非零 accepted points、sample rate 或逐命令 legacy/replacement usage。25/25 compatibility alias 必须继续保留，不能把 `0.165.3` 的版本增长解释为 adoption evidence。
