...
上述增量只证明 **production Node WHATWG `ReadableStream` 上的有限 HTTP response body、后台 SSE 单事件、`ws` 单消息 wire/解压 payload 内存上限，以及 HTTP discard-response 与普通 tool/resource 主请求的 cancel/deadline/断连生命周期**。长期 SSE GET 仍未纳入同一 caller-cancellation 语义；非标准 fetch 若忽略 `AbortSignal`，宿主可以退休 controller registry 并拒绝 late success，却不能强制一个永不 settle 的 adapter Promise/底层请求结束；response callback 的 deadline 诊断仍沿用 adapter/Node 的 AbortError 文案。非认证错误的有界 200 字符预览仍可能把正文、控制字符或 secret 传播到 stderr/tool-result/model/session，`longRunning` 服务也可能用 4 KiB 内慢滴答永久占住预览。无 stream reader 的 Response-like 兼容分支只能在 `text()` 返回后做字节检查，不构成前置内存上限。泛 WebSocket close 也未立即清理 per-server elicitation/URL completion waiters；孤立 peer `1009` 无法区分本地 inbound 超限与 peer 拒绝 outbound，普通配置 loader 还不能把更小 `maxPayloadBytes` 传播到 sink。单消息上限不覆盖消息速率、累计流量或 JSON 对象图放大。WS/stdio/SSE caller cancellation、recovery generation、即时撤权、stdio 完整行/stderr/result/schema、Skill、进程树、三平台 artifact 与长期 soak 也都不在本节范围内，因此完整 Skill/MCP 产品项继续 **NO-GO**。
...
3. 完成 Skill/MCP 真实恶意矩阵：第 16.7 节关闭了 production Node `ReadableStream` 的有限 HTTP response body、后台 SSE 单事件、`ws` 单消息 wire/解压上限、HTTP notification/server response/DELETE 的 body cancel，以及普通 HTTP tool/resource 在途请求的 caller cancel、宿主 deadline、断连和 late-success fencing；dispatch 后的非幂等 `tools/call` 保持 outcome-unknown 且不自动重放。仍须取消或净化会传播到 stderr/tool-result/model/session 的错误正文预览，处理长期 SSE、`longRunning` 慢滴答、无 reader 兼容分支、泛 WS close 的 elicitation/URL waiter 清理，以及消息速率/累计流量/JSON 对象图预算。HTTP caller `AbortSignal` 已从 agent-core 贯穿到 transport；recovery generation 以及 WS/SSE/stdio 的 caller `AbortSignal` 仍须贯穿，并按 deadline 清理完整进程树。stdio complete line/stderr/malformed aggregate、最终 result/ledger/stream、tool schema/description 与 Skill 文件/递归总量都需要 host-owned 不可放宽的字节/深度预算；token 估算失败不得返回 0 后 fail-open。还须覆盖 run_skill 父级取消、即时 revoke、可执行字节身份绑定，以及“任意 stdio command 等价于信任本地代码执行”的明确边界。现有 Session Host MCP 场景只证明 ledger recovery fence，不是真实 transport 恶意矩阵。
