...
| P0 签名更新与回滚链                                | 事务批次已提交，审计整改中       | `4d3a4ee788` 提供签名 manifest、健康检查与初版自动回滚；`8990999771` 提交原生 installer/OTA 锁、状态、sidecar、alias、lineage 与结果消费事务加固，核心定向测试 131/131 通过。提交后只读审计仍确认 6 项 P1：旧 `.previous` 代际恢复、POSIX alias 回滚、下载目标旧文件恢复、锁释放 TOCTOU、Windows 掉电耐久性及 restart 成功/失败结果歧义；扩展矩阵 141/142，唯一失败为既有 `cli-aliases` 动态导入超过 5 秒。完成整改与真实安装/升级/回滚矩阵前不得标记为 release-ready                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
2. 下载目标覆盖前保存旧 destination，rename 后的验证或目录持久化失败必须恢复旧文件。
