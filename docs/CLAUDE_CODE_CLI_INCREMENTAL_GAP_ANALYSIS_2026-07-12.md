...
**仍欠（LSP 多根 + 退避 default-on）**：LSP 多根 workspace（需 agent-core 把多个 root 传进
manager/`_getSharedCodeIntel`）；退避 default-on（需 agent-core 处构造 LSPManager 时传 base，属
agent-core 接线）；`DiagnosticsScheduler` 随未来流式编辑架构一并接（见上，非本 seam）留待。
