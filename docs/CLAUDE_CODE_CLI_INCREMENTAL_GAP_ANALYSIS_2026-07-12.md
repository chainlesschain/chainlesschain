...
做 **tool-time 注入**（属 agent-core 接线）；把 `sparsePaths`/`symlinkDirectories` 经
subagent-contract/team-worktree 从任务定义**透传**到 `createWorktree`（现能力就绪、opt-in，
但上层 spawn 尚未供给，属 agent-core/team 接线）；additional roots 变更发 MCP
`roots/list_changed`；`/context` 的 skill/MCP schema 来源归因（需运行时加载技能/MCP
注册表，非持久会话可得）留待。
...
派生，尚未落显式持久表）；外部 Session Mirror 的加密/保留期/删除/key rotation 属
独立基建，未起。
