...
做 **tool-time 注入**（属 agent-core 接线）；`worktree.sparsePaths` sparse-checkout
...
MCP event；**Monitor 事件的 `event_id`/去重/backpressure/大小上限/authority 纯核已落**
（[[monitor-event.js]]），仍需把它接进真正的 monitor firing seam + 加审计记录（
