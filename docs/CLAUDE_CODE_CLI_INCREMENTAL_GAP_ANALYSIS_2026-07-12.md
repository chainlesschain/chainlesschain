...
create 透传 `expiresAt` + `jitterMs`（store 已支持 per-entry jitter，缺的是 `agent-core.js`
schedule 工具那处 1-2 行接线 + 工具 schema 加 `jitter` 字段）；Monitor 再扩到 WebSocket/SSE/入站 HTTP
