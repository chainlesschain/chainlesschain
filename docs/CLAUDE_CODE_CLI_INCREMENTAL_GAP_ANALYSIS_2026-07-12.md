...
**已接线（2026-07-16，`waiting_permission` 真实生产者）**：补上「消费侧契约已就绪、
无人生产」的缺口——生产点其实**不在 daemon** 而在 agent **子进程**：worker 每回合以
`CC_BACKGROUND_AGENT_ID` 环境变量 spawn headless 子进程（此前无人消费该变量），而普通
headless 里真正**阻塞等人**的只有两个 confirmer：`--permission-prompt-tool`（等 MCP 工具
裁决）与 `--remote-control`（等配对设备上的人，fail-closed 超时）。新纯核
[`background-phase-reporter.js`](../packages/cli/src/lib/background-phase-reporter.js)
（agentId 缺省读 env、IO 注入、终态守卫**绝不复活** completed/stopped 记录、best-effort
永不破坏审批本身）：`wrapConfirmer` 包住这两个 confirmer——审批 pending 期间把
`phase:"waiting_permission"` + 活跃 `pendingApprovals` 计数写进共享 state 文件（重叠审批
计数、最后一个 settle 才恢复 `phase:"turn"`，抛错也 finally 清理），Dashboard 的
"Needs input" 分组从此有真数据。**默认字节不变**：前台运行（无 env）→ `wrapConfirmer`
返回**同一函数对象**、零读零写。worker 侧在 `startTurn` 与 idle 两处重置
`pendingApprovals:0`——子进程被 SIGKILL 于审批中途时残留计数不会把会话永久钉在
"Needs input"。测试：`background-phase-reporter.test.js` 9（禁用零 IO+对象恒等 / 窗口
写入恢复 / 重叠计数 / 终态不复活 / IO 故障不抛 / wrap 往返+抛错清理 / dashboard 契约
round-trip 到 needs-input）+ `headless-runner.test.js` +2（remote-control confirmer 包裹
...
compound 两笔 / 非安装 null / resolveInstallPolicy env-wins·settings·关 / riskFloor 只升 / 审计注入-fs +失败不抛）+ `shell-approval.test.js` +4（默认无 install 字节不变 / riskFloor 抬高致 strict 拒 /
...
**已接线（2026-07-16 续，install 分类扩到 `run_code` 自动安装 + `python -m pip` 识别）**：
统一分类器此前只覆 `run_shell`——`run_code` 的 pip 自动安装（`<interpreter> -m pip install
<pkg>`）只进 dependency-install-policy 自己的审计，统一「下载并运行第三方代码」trail 看不见它；
且 `classifyInstallSegment` 根本不认 `python -m pip`（bin=python 不在管理器表）。两处补齐：
①分类器新增**模块调用剥壳**——`python`/`python3`/`pythonX.Y`/`py` 后跟 `-m` 时把模块当 bin，
`python3 -m pip install x`（含 `sudo` 包裹）与直调 `pip` 分类完全一致，`python -m http.server`/
`-m venv`/`setup.py install` 不误报；②run_code 自动安装的 installed/failed 尝试在**同一** opt-in
policy（`CC_INSTALL_AUDIT`/settings `installPolicy`）开启时经 `recordInstallCommandAudit` 落
`install-commands.jsonl`（`source:"run_code_auto_install"` + pip 分类 + global 判定）。**默认
字节不变**：policy 未开→零写；审计 best-effort 绝不影响安装本身。测试：install-command-policy
...
**已接线（2026-07-18，权限门-over-WS binding 生产者——producer 缺口闭合）**：补上「校验就绪、
无生产者」的最后一环。审计确认此前 WS 路径（IDE/桌面桥）**根本没有 approvalGate**：
`ws-agent-handler` 的 loopOptions 不带 gate，CONFIRM 级决定一律 no-confirmer fail-closed，
interaction-adapter 里早已就绪的 binding 校验（`_resolvePending` 篡改回显→deny）从无人喂。新纯核
[`ws-approval-gate.js`](../packages/cli/src/gateways/ws/ws-approval-gate.js)：
`buildWsApprovalConfirmer`（镜像 headless-stream `interactiveConfirm` 配方——
`approvalBindingDigest({toolCallId: "<sessionId>:appr-<seq>", args, policyDigest})`，经
`askConfirm` **对象形第二参**把 binding + 结构化 `approval` 上下文外发；default **false**、
transport 抛错/超时/断连一律 fail-closed false）+ `createWsApprovalGate`（session 级 gate：
policy **逐调用**从持久单例解析——`sessions.policy.set` over WS 中途改档即刻生效，且绝不
mutate 单例的全局 confirmer 以免跨 session 竞态；装载失败返 null 降级 legacy 无门路径）。
`interaction-adapter.js` 的 `askConfirm(question, {default, binding, ...extra})` 对象形 +
`_ask` 第四参透传 binding（布尔形调用逐字节不变）。`ws-agent-handler` 构造器接受注入
`approvalGate`、否则 `CC_WS_APPROVAL_GATE=1` 时首 turn 懒建；**默认字节不变**（env 未设→
loopOptions.approvalGate 恒 null，与 agent-core `options.approvalGate || null` 默认完全一致）。
消费端（`_resolvePending` 校验 + `handleSessionAnswer` binding 透传）早已就绪，故
producer→consumer 全链路闭合：请求带 binding 外发→客户端 approve 必须回显→篡改回显被判
deny、如实回显 allow。测试：`ws-approval-gate.test.js` 9（confirmer binding 可验证/参数
篡改不匹配/逐请求不复用/超时 fail-closed/非 true 拒 + gate CONFIRM 路由/中途改档 trusted
免确认/confirmer 拒→deny/装载失败→null + **端到端真 adapter**：外发带 `ab_` binding、篡改
回显→deny、如实回显→allow）+ `interaction-adapter.test.js` +2（对象形外发 binding+approval
上下文且篡改回显 deny / 无 binding 对象形不加字段）+ `ws-agent-handler.test.js` +2（默认
approvalGate null 字节不变 / 注入 gate 透传）；remote-session-protocol / headless-stream-
approvals / slot-filler / ws-session 套回归绿（`5bf74014c8`）。

**仍欠**：agent-sdk 的 `ApprovalRequestEvent` 可选加 `binding` 回显（需 bump SDK 版本，留待下次
SDK 发版）；WS gate default-on（现 opt-in `CC_WS_APPROVAL_GATE=1`——default-on 会把此前静默
fail-closed 的 CONFIRM 变成向客户端提问，需 IDE 插件侧确认全部客户端能应答审批卡片后翻开，
属产品决策）；repo 配置不得开 bypass/auto 与 managed deny 放宽的强制项亦未做（注：permission
mode 来自 CLI flag 而非 project settings，故经 settings 的 mode 升级路径本就不存在）。
...
**已接线（2026-07-16，`SubtreeInstructionLoader` tool-time 注入）**：把前面已建+已测的
纯核 `SubtreeInstructionLoader.onAccess` 真正接进 agent-core 的工具执行路径——当一个工具
**首次访问**某子树内的路径时，把该子树自带的 `cc.md`/`CLAUDE.md`/`AGENTS.md` 注入到该次
工具结果里（复用 `newDiagnostics` 的**同一投递通道**：附到 tool result 的 `subtreeInstructions`
字段），**每子树每进程恰注入一次**。实现：模块级 `_subtreeLoaderPool`（按 root 缓存一个有状态
loader，镜像 `_getSharedCodeIntel` 池的生命周期，`_resetSubtreeInstructionLoaders` 测试可重置），
`_subtreeInstructionsFor(accessedPath, cwd)` 调 `onAccess` 取**新增**文件、按 `DEFAULT_MAX_FILE_BYTES`
读取（截断标 `truncated`）、fail-open（任何错误→null，绝不因注入失败破坏工具）；
`_withSubtreeInstructions(result, accessedPath, cwd)` 只装饰**成功**结果（error 结果不挂规则）。
接进 **read_file**（两条 content 返回）/ **list_dir** / **write_file** / **edit_file** /
**edit_file_hashed**（后三者链在既有 `_withPostEditDiagnostics` 外层）；`instructionExcludes` 经
settings 读出并 honor。系统提示新增一行契约让模型把 `subtreeInstructions` 当作该子树的权威规则。
**默认字节不变**：子树内无新 cc.md/CLAUDE.md/AGENTS.md（常态）→ `onAccess` 返 []→ 不挂字段、零成本；
`CC_SUBTREE_INSTRUCTIONS=0` 整体关闭。**未改 agent-core 导出**除新增测试钩子
`_resetSubtreeInstructionLoaders`（已同步 lib shim + convergence 门）。测试：
`agent-core-subtree-instructions.test.js` 7（首次注入 / 二次 dedup / 无子树规则不挂 / list_dir 注入 /
...
**已接线（2026-07-16，headless `--add-dir` 启动根 seed → MCP `roots/list_changed`）**：REPL `/add-dir`
路径已会广播根变更；补上 headless（`cc agent -p --add-dir` 与 stream-input `runAgentHeadlessStream`）
的启动根 seed。此前 headless 以额外根启动却**从不**把完整根列表告知已连 MCP 服务器——它们的
`roots/list` 永远只见 cwd。两个 runner 在 MCP 客户端解析完成后、`additionalDirectories.length > 0` 时，
经已测纯核 `workspaceRootDirs(cwd, additionalDirectories)`（cwd 优先 + 解析绝对 + 去重）+
`notifyMcpRootsChanged([mcp.mcpClient], dirs)`（`setRoots` 只在列表**真变**时才发 `roots/list_changed`）
seed 一次。**默认字节不变**：无 `--add-dir`→`workspaceRootDirs=[cwd]`→`setRoots` 不发通知=no-op；
best-effort（根广告绝不阻断 run）。测试：`headless-runner.test.js` +2（注入 `resolveAgentMcp` 返回带
`setRoots` spy 的 mcpClient：有 `--add-dir` 时 seed 全根列表且 cwd 居首 / 无 `--add-dir` 时不 seed）；
stream 路径接线与 runner **逐字节对称**（同两个已测纯核 + 同 guard），由 runner 测试 + `add-dir.test.js`
纯核测试共同锁定。headless-stream/negotiation/add-dir 回归 57 绿。**未改导出**。

**仍欠**：`/context` 的 skill/MCP schema 来源归因（诚实边界：
...
- **命令面**（`cc session index` 加 option，非新顶层命令→命令数 175 不变）：`--mirror-delete <ids…>`
  ...
  失败上报+缺钥抛错）= 全绿；session-index 12 回归绿。

**已接线（2026-07-18，agent loop 事件实时喂 TurnBindingLog + 每 turn `persistTurnBinding`——
显式持久表落地）**：headless-runner 现从**已产出的事件流**实时喂显式绑定表（agent-core 导出
不动）：外层每 drain 一 turn——turn 起点 `startTurn("<nonce>:t<seq>", {conversationOffset:
messages.length})`；`checkpoint` 事件→`bindCheckpoint(event.id)`（log 内建保**最早**=
pre-mutation 快照）；`tool-executing`→`recordToolCall`（provider `call.id` 不在 runner 可见
事件流上，故合成 `<nonce>:c<seq>`——镜像 side-effect ledger 的 op-id 方案，run 内稳定、跨
resume 不碰撞）；`tool-result` 的 `result.policy`→`recordPermissionDecision("<callId>:perm:
<via|decision>")`、`result.subAgentId`（前台 `spawn_sub_agent`）→`recordChildAgent`；
`background-sub-agent-result`→`recordChildAgent`。turn settle 即 `persistTurnBindingLog`——
整表快照落链式 `turn_checkpoint_binding` 事件（latest-wins；`rebuildMessages` 是 allowlist
故天然忽略，不污染模型上下文）；resume 时倒扫最近快照 `TurnBindingLog.fromJSON` 续表、新
turn 追加其后。**dirty-gated 默认字节不变**：非持久化 run 零喂零写；持久化但 turn 无 tool/
checkpoint/decision/child → 零事件（纯问答 run 不变）。coverage 语义随表免费而来（该 turn
跑过 shell → PARTIAL 诚实降档，绝不过度承诺可恢复性）。测试：
`headless-runner-turn-binding.test.js` 5（单 turn 全信号：checkpoint + 2 tool + policy 决定
挂对第二个 call + 后台 child、快照 round-trip 真 log coverage=partial / 前台 spawn 子 id /
无 tool 持久 turn 零事件 / 非持久 run 零写 / resume 续表 2 turns 旧 turn 原样）；
...
boundSessions / foldPruneStale + IO 端到端：append→take 恢复即清 / remove 消费不复现 / session 隔离 +陈旧 prune / 空队列纯读不重写 / 空 session id 拒 / 损坏文件 best-effort）+ `async-hook-supervisor.test.js`
...
PermissionRequest hook 测试全绿）；default-flip 是产品决策（开启后每次都跑完全部 hook，改变 hook 执行量 +副作用）。测试：`hook-runner.test.js` +5（默认 ask 掩盖 block / mergeStrict later-block 胜且两 hook 都跑
...
**已接线（2026-07-16，PreToolUse strict-merge 真并行 via async spawn）**：把 mergeStrict 从
「顺序跑完再合并」升级为**真并行**。`hook-runner.cjs` 抽出共享纯核 `interpretHookOutcome(res)`
（退出码/JSON-stdout 协议单一真源，sync 与 async 路径复用绝不漂移）+ 把熔断器抽成
`_breakerShortCircuit`/`_breakerRecord`（sync/async 共用一套）；新增**异步单-hook** `runCommandHookAsync`
（`_deps.spawn` 异步 spawn + stdin 喂 payload + 有界 stdout/stderr 收集 + 硬超时 **SIGKILL** 挂死 hook
后按非阻塞错误上报）+ **并行批** `runHooksParallel`（`Promise.all` 并发跑全部 + `mergeHookDecisions`
取最严；wall-clock = 最慢单 hook 而非求和；合并与顺序无关故结果同顺序 strict-merge）。`agent-core.js`
的 `runSettingsPreToolUseHooks`（本就 async）在 `CC_HOOK_STRICT_MERGE=1` 时改 `await runHooksParallel`，
否则走原 sync 短路 runner——**默认字节不变**。`runSettingsPermissionRequestHooks` 是 **sync** 故保留
顺序 strict-merge（同款最严结果，不引入 sync→async 涟漪）。测试：`hook-runner.test.js` +11（
`interpretHookOutcome` 四态 / `runCommandHookAsync` stdin+exit-0 决策·exit-2·error 事件·**假定时器超时
SIGKILL** / `runHooksParallel` 并发 spawn 全部+乱序结算 later-block 胜·allow 浮出·全 continue）——⚠️同命令
两次须把 fake child 收进**数组**不能按命令 key（否则第一个 child 丢失→真 60s hook 超时）；
`agent-core-hooks.test.js` 的 mergeStrict e2e 现经真 async 并行跑真 `node -e` hook 仍绿。**仍欠**：
`runSettingsPermissionRequestHooks` 若要真并行需转 async（涟漪至其 sync caller，留待）；default-flip（产品决策）。

**已接线（2026-07-16，trace_id/parent_id 从 agent-core context 线程进 envelope）**：
`buildHookEnvelope` 的 trace/parent 槽位一直存在但所有生产者传 null，且 agent-core 的工具
hook payload 根本不走 envelope。补齐：①`withDeliveryId` 现把 `trace_id`/`parent_id` **也
stamp 到投递 payload**（仅当 caller 线程了它们；缺省**整字段省略**，legacy payload 字节不变）
并导出；`runObserveHooks` opts 透传两字段。②agent-core 把 `runId` 提前到 toolContext 构造
之前并挂 `hookTraceId`（=本 run）/`hookParentId`（=spawn 方 run，经 `hookParentTraceId`
option）；PreToolUse / PostToolUse / PermissionRequest / SubagentStop 四个 payload 构造点
现带 `event_id` + `trace_id` + `parent_id`——`CC_HOOK_EVENT_LOG` 开启时这些 DECISION 事件顺带
可被 `cc hook replay` 寻址。③`spawn_sub_agent` 与隔离 `run_skill` 把父 run id 线程进子 loop
（`SubAgentContext` 存储+转发），子 agent 的 hook envelope 跨深度关联回父 trace。测试：
settings-hook-events +3（线程/省略/withDeliveryId 双态）+ agent-core-hooks +3（executeTool
线程/省略 + agentLoop 自 stamp runId 且顶层无 parent）；hook-runner/telemetry/subagent 回归
绿，未改 agent-core 导出（`8e62a1e80a`）。
...
**已接线（2026-07-16，`capDiagnostics` token cap 替裸 cap 20）**：`_postEditDiagnostics` 此前对
过滤后的诊断做 `diags.slice(0, 20)`——**未排序**的前 20 条，一堆 warning 可能挤掉真正的 error。
现改用纯核 `capDiagnostics(diags, {maxTokens})`：**按 severity 优先**（error 先于 warning）+
**token 预算**封顶（默认 2000，`CC_EDIT_DIAGNOSTICS_TOKENS` 可覆盖），保证模型先看到最严重的诊断、
且不会被跑飞的诊断洪流刷屏。返回 `.kept` 数组，形状不变（现有
`agent-core-code-intel.test.js` 的 `Array.isArray`+`.some(error)` 断言全绿）。**注**：
`DiagnosticsScheduler`（debounce/throttle）**刻意不接**此 seam——`_postEditDiagnostics` 是
**同回合内联**反馈（编辑后当轮就把新错误回喂模型），debounce/throttle 是**流式编辑**执行模型
（把连打合并、延迟成一次异步 run），套进内联路径会破坏「同回合看见诊断」这一核心语义。调度器
属未来流式编辑架构，非本 seam 的正确归宿。

**已接线（2026-07-16，LSP 多根 workspace + 退避 default-on）**：两项 agent-core 接线补齐——
①**退避 default-on**：`_getSharedCodeIntel` 构造池化 CodeIntelligence 时经新增 `lspOptions`
透传 `restartBackoffBaseMs`，**默认 1000ms**（1s→2s→4s→8s 封顶），启动即崩的 server 不再毫秒级
连爆 `maxRestarts` 次；`CC_LSP_RESTART_BACKOFF_MS` 覆盖 base、`=0` 恢复 legacy 立即重生。
②**多根 workspace**：新纯核 [`workspace-roots.js`](../packages/cli/src/lib/lsp/workspace-roots.js)
（`workspaceRootsFor` cwd 优先去重 / `pickRootForFile` **containment 最深根胜**、根外回落 cwd、
无分隔符前缀混淆 / `mergeWorkspaceSymbolResults` 逐 symbol stamp `root`、任一根可用即可用、全
不可用折叠 reasons）；`code_intelligence` 现按**包含该文件的根**（而非恒 cwd）key 共享 server
池——`--add-dir` 根内的文件拿到**它自己项目**的语言服务器；`workspace_symbols` 对每个根扇出并
