...
| P1     | P1-10 | 有界队列、模块边界与增量 conformance   | 🟡 部分完成 | 既有 backlog 有界化与旧 interceptor 退役；RSS/Email production wiring、Email 有界凭据/状态/附件、signaling、IPFS，以及 native Realtime/Yjs retained/recovery、Federated/Social collab transport、awareness/document state、production Gossip 与 dormant Mesh adapter retained state 已收口；50 个 social initializer 已分为 29 个 startup 与 21 个 dormant，11 个未接线 manager 不再默认创建；Deep Link、MTC/Gossip wiring listener 已收敛为 main 单一所有权；详见 §12.22～12.27、§12.43～12.49                                                                                                                                                     | 继续全量普查并有界化或删除其余 live transport/event/message/tool backlog，拆分剩余超大模块 ports/adapters/state machines，并扩展真实多主机 crash/recovery/migration conformance matrix      |
