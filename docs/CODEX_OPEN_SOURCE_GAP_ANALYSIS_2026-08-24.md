...
精确提交 `5bddb9ce27` 关闭了 §12.72 独立终审中最后两个非阻塞 LOW：共享 live-main verifier 不再先把 chunked 响应完整读入 `arrayBuffer()`，P1-10 Jobs/Artifacts 分页也不再直接调用无界 `response.json()`。
