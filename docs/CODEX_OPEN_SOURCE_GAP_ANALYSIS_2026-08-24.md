...
| P1-11 | 部分随 CLI 发布                             | Graph 数据来源/信任/敏感度/allowedSinks 传播与审计 declassification；orchestrate webhook 增加 HMAC、时间窗、delivery replay、body/rate cap，并保留可信 origin；MCP consent 缓存键只保留稳定摘要、不再保留原始参数；Desktop 外部 Skill 已完成签名/containment、一次性强沙箱 worker、有界协议与默认拒绝 capability broker，详见 §12.30～12.31                                                                                                                           | bundled legacy Skill 逐项最小 capability 迁移、所有 webhook vendor 原生签名，以及 filesystem/env/network port 接入 SecretStore/路径 authority/全产品统一 egress broker 后的三平台 Desktop 验收尚未完成 |
...
- 本切片只改变 Desktop 内部代码和测试，不改变 protocol、SDK、CLI、IDE 或公开包版本，也没有形成签名 Desktop 安装包，故不发布制品。该节留下的独立执行器与 capability broker 已由 §12.31 关闭；P1-11 整体仍需迁移 bundled legacy Skill 的逐项最小 capability、vendor 原生 webhook 签名和全产品统一 egress broker。
...
- 本切片只改变 Desktop 内部代码、测试与打包资源声明，不改变公开 protocol/SDK/CLI/IDE 包版本，也未形成签名 Desktop 安装包，故不发布制品。P1-11 仍为“部分完成”：剩余 bundled legacy Skill 最小 capability 迁移、vendor 原生 webhook 签名，以及把 filesystem/env/network 特权端口接入 SecretStore、路径 authority 与全产品统一 egress broker 后的三平台签名 Desktop 验收。
...
| P1     | P1-11 | Skill 供应链、数据来源与选择性网络出口 | 🟡 部分完成 | Graph data lineage/declassification 与 webhook HMAC/replay/body/rate cap 已发布；MCP consent 缓存键只留 SHA-256；Desktop 外部 Skill 已完成签名/containment、精确 source snapshot、一次性 ProcessExecutionBroker 强沙箱 worker、Node permission + VM、固定有界协议、超时强杀与默认拒绝 capability broker；Skill Creator 不再生成 Node 直连模板，详见 §12.30～12.31                                                                                                          | bundled legacy Skill 最小 capability 迁移、vendor 原生 webhook 签名、filesystem/env/network 特权端口接入 SecretStore/路径 authority/全产品统一 egress broker，以及三平台签名 Desktop 验收 |
