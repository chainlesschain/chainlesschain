...
| P1-3  | 🟡 仓库接线 70%～80%                        | App Server 复用真实 CLI agent loop；固定 Graph capability、Desktop `$team` canonical/shadow、Cowork/Scheduler result receipt、authority generation/lease/head CAS、旧 Desktop writer read-only；CLI Team local ready/message/handoff/dynamic/effect writer 已由 Graph 独占并补齐 cut-point 恢复与跨进程 writer fencing                                                                                                                                 | CLI Team distributed 命令的 Graph writer/outbox、旧 Desktop 入口的功能性 Graph adapter、逐面 canary/migration/rollback、同 SHA 三平台矩阵；详见 §6.9.6.7.5～§6.9.6.7.6       |
...
- 若只按“唯一内核切换”这个独立 epic 计，当前约完成 **25%～35%**，仍剩 **65%～75%**。原因是共享 contract 与测试门已经存在，但五个 production surface 的 writer authority 尚未实际迁移。
...
| Desktop `$team`                          | renderer IPC → `workflow-command-runner` → in-process skill handler → `SubRuntimePool` → `src/main/sub-runtime/index.js`                                                                                                                                                                             | parser、plan approval、structured tasks、scope conflict、进程 admission 与 truthful runtime claims 已有 | runner 明确不经过 Coding Agent bridge：[`workflow-command-runner.js`](../desktop-app-vue/src/main/ai-engine/code-agent/workflow-command-runner.js#L3)；`$team` 仍直接创建 pool：[`team/handler.js`](../desktop-app-vue/src/main/ai-engine/cowork/skills/builtin/team/handler.js#L295)；child 明确为 simulated：[`sub-runtime/index.js`](../desktop-app-vue/src/main/sub-runtime/index.js#L43)。需要改成 Graph 调度、Agent Kernel 执行和 canonical projection |
| Desktop Specialized Agents               | 生产注册 `agents:*` IPC，并由 `AgentCoordinator` 自己分解、分派、维护 session/task 状态和调用 `agent.execute()`：[`agents-ipc.js`](../desktop-app-vue/src/main/ai-engine/agents/agents-ipc.js#L64)、[`agent-coordinator.js`](../desktop-app-vue/src/main/ai-engine/agents/agent-coordinator.js#L320) | 无真实 agent 时已诚实返回 simulated/pending；真实结果要求 terminal evidence                             | 禁止继续作为 task/run writer；保留选择器/模板能力，分派、attempt、terminal、cancel 和恢复改走 Graph adapter                                                                                                                                                                                                                                                                                                                                                  |
| Desktop WorkflowManager                  | 主进程注册 14 个 `workflow:*` IPC，`WorkflowManager`/`WorkflowPipeline` 在进程内维护执行状态并直接驱动 stage：[`phase-8-9-extras.js`](../desktop-app-vue/src/main/ipc/phases/phase-8-9-extras.js#L379)、[`workflow-pipeline.js`](../desktop-app-vue/src/main/workflow/workflow-pipeline.js#L159)     | UI、stage 模型、质量门、progress event 可复用                                                           | definition 转 canonical IR；start/pause/resume/retry/cancel 变成 Graph command；旧 state machine 退为 importer/只读 projection                                                                                                                                                                                                                                                                                                                               |
...
| CLI Team                                 | child 的真实 Agent turn 已通过 `cc agent` 进入 Agent Kernel；TeamRunner、lease/fence、worktree、message/handoff 能力成熟                                                                                                                                                                             | 单任务执行内核无需重写；已有 canonical collaboration projection                                         | projection 仍明确只读，`TeamMailbox`/`TaskLeaseRegistry` 仍是 writer：[`team-graph-projection.js`](../packages/cli/src/lib/agent-team/team-graph-projection.js#L260)。需先做首个 Graph canary，再逐步迁 ready frontier、attempt、message/handoff、artifact/terminal                                                                                                                                                                                          |
...
7. **distributed registry 基础能力**：`TeamDistributedQueue` 已在同一文件锁事务中提供 custody handoff、target lease/fence、预算 reservation 转移、恢复刷新以及 canonical task/handoff projection API；但 distributed CLI 进程尚未建立唯一 Graph dispatch writer/outbox，因此本项只算基础能力完成，不能把 distributed Team 标为 canonical cutover。
...
- 熟悉代码的工程人员继续推进，仓库及验收侧约剩 **13～23 人日**：CLI Team distributed Graph writer/outbox、shadow message/handoff diff 与真实 crash/cancel/reconcile 约 2～4 人日；Desktop Specialized/Workflow 功能性 adapter 与 UI canonical projection 3～5 人日；逐 store migration/cut-point/rollback 3～5 人日；真实 Desktop/CLI 三平台 journey、canary 与 soak 5～9 人日。Browser 仍保持 truthful non-durable；升级为 durable 另计。
...
| P1     | P1-3  | canonical Agent Kernel 与 rollout      | 🟡 部分完成 | 真实 CLI loop、固定 App Server Graph capability、Desktop `$team` canonical/shadow、Cowork/Scheduler receipt-first terminal adapter、authority generation/lease/head CAS、旧 Desktop writer read-only；CLI Team local canonical ready/message/handoff/dynamic/effect 与主要 RPO=0 cut point 已落地；仓库接线约 70%～80%                             | distributed Team Graph writer/outbox、旧 Desktop 入口功能性 Graph adapter、逐面 canary/migration/rollback 与同 SHA 三平台矩阵；剩余约 13～23 人日，详见 §6.9.6.7.5～§6.9.6.7.6                      |
