...
| P1     | P1-10 | 有界队列、模块边界与增量 conformance   | 🟡 部分完成 | 既有 backlog 有界化与旧 interceptor 退役；RSS/Email production wiring、Email 有界凭据/状态/附件、standalone/Electron embedded signaling、IPFS content transport/admission/增量 GC，以及 native Realtime/Yjs 协作接线与传输边界已完成；详见 §12.22～12.28                                                                                                                                                                                                                                                                                         | 继续全量普查并有界化或删除其余 transport/event/message/tool backlog，收口 collab offline/Yjs retained state，拆分剩余超大模块 ports/adapters/state machines，并扩展 crash/recovery/migration conformance matrix |
