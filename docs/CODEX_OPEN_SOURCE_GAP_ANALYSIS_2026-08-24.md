...
| ----- | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P1-1  | 协议与审批 validator 增量已发布             | canonical JSON Schema；TS/Python/Kotlin/Swift 确定性 codegen；冻结 v1 baseline 和 breaking-change 检查；protocol `0.1.2` 与 TS/Python SDK `0.2.2` 已公开；同一 schema 生成 JS/TS/Python runtime validator，并以同一 ApprovalDecision fixture 验证协议包、两套 SDK 与 CLI                         | 旧客户端手工事件 union 尚未全部迁移；CLI/Desktop/IDE 同 fixture 的三端 conformance 尚未完成                                                                                   |
...
| P1-9  | 公开结构化审批增量已发布                    | 可认领/恢复 HumanTask、quorum/职责分离与 cancel/decision CAS；canonical ApprovalDecision、binding/requested permissions、N-1 boolean 兼容已公开到 TS/Python SDK 与 App Server；CLI 已实现 exact turn/session grant、authority-event 恢复、损坏丢弃和持久化失败降级                               | Desktop/IDE 审批界面、统一 hook/tool policy event，以及跨产品 approval-vs-cancel/quorum/race/restart conformance 尚未完成                                                     |
...
| P1     | P1-1  | 单一协议 Schema 与多语言 codegen       | 🟡 部分完成 | protocol `0.1.2`、TS/Python SDK `0.2.2` 已发布；JS/TS/Python validator 与 Kotlin/Swift codegen 共用 canonical schema | 迁移旧客户端手工 union；完成 CLI/Desktop/IDE 同 fixture conformance                                            |
...
| P1     | P1-9  | durable HumanTask 与统一策略事件       | 🟡 部分完成 | HumanTask 核心、quorum/职责分离/CAS 与公开结构化审批、exact grants 已发布                                            | Desktop/IDE 审批 UI、统一 hook/tool policy event、跨产品 race/restart conformance                              |
