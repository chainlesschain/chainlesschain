...
> 最新未发布 Team custody/handoff 验证基线：`982b13a41f4697898d453c46b18621d339b60bad`（CLI-only；协议与 SDK 内容及版本不变）
...
此后的未发布 CLI 源码先把 idle target followup 物化为定向、高优先级、独立 lease/fence 的真实新 turn，并完成 processed-before-ACK 两进程恢复；最新精确验证提交 `982b13a41f4697898d453c46b18621d339b60bad` 又增加 canonical Team message/handoff 只读投影、真实 child `team_handoff` 工具，以及 `OFFERED → ACCEPTED | REJECTED → COMMITTED | REVOKED | EXPIRED` custody 状态机。commit 在同一持久化 CAS 中撤销 source lease、为 target 建立新 AssignmentAttempt/lease/fence；source 迟到写稳定失败，commit-before-dispatch 崩溃可跨两个真实 Node 进程恢复且副作用仅执行一次。该 SHA 的 Linux/Windows/macOS `CLI CI` 与 `CLI Strict Sandbox` 已全绿。P1-6 仍因真实 provider、长时离线/poison/reorder、跨进程 rate limit、SessionMessageFabric/全产品 authoritative adapter 切换与 soak 未完成而保持开放；该增量尚未发布，也没有修改 Agent Protocol 或 TS/Python SDK 包，因此当前不创建协议/SDK 空版本。
...
| ----- | ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| P1-1  | 核心、协议包与生成 SDK 均已协调发布             | canonical JSON Schema；TS/Python/Kotlin/Swift 确定性 codegen；运行时 validator；冻结 v1 baseline 和 breaking-change 检查；`@chainlesschain/agent-protocol@0.1.1` 与 TS/Python SDK `0.2.1` 已公开发布                                                                                            | 旧客户端手工事件 union 尚未全部迁移；CLI/Desktop/IDE 同 fixture 的三端 conformance 尚未完成                                                                                                                        |
...
| P1-6  | 消息 adapter 已发布；custody/handoff 增量待发布 | `0.166.2` 已发布 admitted/delivered/read/processed/dead-letter 与真实 child 消息工具；后续源码已落地 idle followup 新 turn、canonical message/handoff 只读投影、真实 `team_handoff`、offer/accept/reject/commit/revoke/expire、custody/lease/fence 原子转移及两进程 commit-before-dispatch 恢复 | 最新 CLI-only 精确 SHA 的 Linux/Windows/macOS CLI/strict 门禁已全绿但尚未发 npm；真实 provider、长时离线/poison/reorder、跨进程 rate limit、SessionMessageFabric/全产品 authoritative adapter 切换与 soak 仍未完成 |
...
- 本增量只修改 CLI，不改变 canonical protocol schema、生成绑定或 SDK 包内容；registry 当前仍为 `chainlesschain@0.166.2`、`@chainlesschain/agent-protocol@0.1.1`、`@chainlesschain/agent-sdk@0.2.1` 与 `chainlesschain-agent-sdk==0.2.1`。因此协议/SDK 不需要重复发布；CLI 如需向 npm 用户交付，必须创建高于 `0.166.2` 的新版本与新不可变标签，不能复用旧发布标签。
...
[`TeamMailbox`](../packages/cli/src/lib/agent-team/team-mailbox.js) snapshot 已升级到 v3：实时 receive 不推进 processed cursor，未终态消息会 at-least-once 重投；ACK 区分 read/processed/dead-letter，稳定 consumer key 冲突失败关闭，out-of-order terminal receipt 不跳过更早消息；send/followup 要求稳定 message id 并保存 payload digest、causation/correlation 与 sender attempt。`--state` 下每次 send/receive/ack/followup/wake/handoff mutation 都走 team state 原子 checkpoint；未使用 `--state` 时 prompt 明确声明 process-local。最新未发布的 [`TeamRunner`](../packages/cli/src/lib/agent-team/team-runner.js) 已将 idle target followup 物化为只允许目标 holder claim 的高优先级动态 task，并把 offer/accept/commit/revoke/expire custody 与目标 AssignmentAttempt/lease/fence 绑定；新 turn/transfer 复用原任务的 collaboration session、权限、预算、scope/worktree 契约。

这关闭了真实 CLI child 的 mid-turn adapter、idle target 新 turn admission、TeamMailbox→canonical Graph message/handoff 只读投影和 Team custody 原子交接四个源码缺口。父进程在 followup admission 后、wake task 持久化前崩溃时会启动对账；未 ACK 的 wake turn 最多重试 3 次后稳定进入 dead-letter。processed-before-ACK 与 commit-before-dispatch 均有两个真实 Node 进程的恢复 fixture，分别证明业务副作用和 target execution 只发生一次；handoff commit 后 source 的迟到写由 fencing 稳定拒绝。尚未完成的是接收方长时离线/poison/reorder soak、跨进程 rate limit、SessionMessageFabric/全产品 authoritative Graph runtime 切换和真实 provider journey，因此 P1-6 继续开放。

建议：

1. 已完成真实 CLI child 的 holder/attempt/lease-bound `send / receive / ack / followup / handoff` 宿主工具、有界本地桥、idle target 持久化新 turn admission，以及 TeamMailbox v3 到 canonical Graph message/handoff 的确定性只读投影；下一步把相同语义接入 [`session-message-fabric.js`](../packages/cli/src/lib/session-message-fabric.js#L1) 和 authoritative Graph event ledger，并补跨进程 rate limit 与长时恢复。
...
3. 已在 TeamMailbox/真实 child adapter 明确采用 at-least-once delivery + 幂等 consumer，不宣称 exactly-once，并区分 admitted/delivered/read/processed/dead-letter，记录 causation/correlation、sender attempt/lease 与 payload digest；processed-before-ACK 与 commit-before-dispatch 已有真实两进程恢复 fixture。仍需把相同契约合并到 SessionMessageFabric/authoritative Graph Kernel，补 conversation/revision、poison message 长时重试及跨 channel 重排矩阵。
...
### 11.1 本次 P1/P2 实现回归（更新至 2026-08-25）
