...
| P1     | P1-10 | 有界队列、模块边界与增量 conformance   | 🟡 部分完成 | App Server/Agent IPC、Cowork AgentPool 与 MCP consent 子项已关闭；可选 MCP metrics 已有有界 retention；`64cfb1752c` 删除 456 行无引用 direct-spawn 旧 `StdioTransport`，生产 MCP stdio 只保留 Broker 默认路径，并完成三平台 fallback、coverage 和综合门禁                                                                                  | 下一项为 MCP HTTP SDK 增加 SSE client、并发 RPC、请求字节与慢消费者边界；之后继续其余 backlog、模块拆分及 crash/recovery/migration conformance matrix              |
