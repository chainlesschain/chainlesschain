...
- 本增量实质修改 protocol、TS/Python SDK 和 CLI 四个发布包，因此需要补丁版本；manifest 已准备为 protocol `0.1.2` → TS/Python SDK `0.2.2` → CLI `0.166.3`，但当前尚未形成并验证实际 release commit，仍不能发布。每个不可变标签必须指向该 release commit，并满足各自 exact-SHA Linux/Windows/macOS 门禁；旧标签和旧 SHA 的成功结果均不可复用。
