...
- `@chainlesschain/agent-protocol@0.1.3` 仍是未发布候选。只有包含上述 Android 门禁与 Desktop 迁移的同一精确 SHA 在 Agent Protocol 三平台、IDE、Desktop 和 Android 相关矩阵全部成功后，才创建新的不可变 OIDC 标签并发布；若 Actions 暴露新错误，则继续逐项修复，不回退为软失败。
...
| P1     | P1-1  | 单一协议 Schema 与多语言 codegen       | 🟡 部分完成 | protocol `0.1.2`、TS/Python SDK `0.2.2` 已发布；Kotlin/Swift 已生成严格审批 union，Android/JetBrains/Desktop 已消费 canonical 决定，`0.1.3` 待新 SHA 门禁 | 其余旧客户端 union、Swift 生成 client 生产消费、CLI/Desktop/IDE 完整事件流 conformance                         |
...
| P1     | P1-9  | durable HumanTask 与统一策略事件       | 🟡 部分完成 | HumanTask 核心与公开结构化审批已发布；VS Code/JetBrains/Android PDH/Desktop 二元 UI 发出最小权限决定，Kotlin/Desktop 客户端使用 canonical union           | 可审阅的 turn/session grant UI、统一 hook/tool policy event、跨产品 cancel/quorum/race/restart conformance     |
