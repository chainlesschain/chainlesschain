...
| P2-2 | CLI 核心已发布，Desktop 统一 overlay 已接入 | `cc team graph inspect/diff/eval` 可从持久事件生成 Agent Tree、Task Graph、Artifact/Message/Effect/Timeline、critical path、blocked root 和 time-travel；Desktop 共享调试器已接入 Coding Agent、Workflow Manager 与 Specialized Agents，支持 canonical trace/task graph、critical path/slack、预算热图、metadata-only 因果下钻、attempt/lease/worktree/commit 与 snapshot diff/time-travel | 仍需让每个 Desktop GraphRun 通过固定 capability 取得 durable trace/snapshot 历史，并以同一 fixture 关闭 Desktop/VS Code/CLI 的跨产品回放矩阵 |
...
| P2     | P2-2  | Graph topology/timeline 调试器         | 🟡 部分完成 | CLI inspect/diff/eval 与完整 topology/timeline/time-travel 已发布；VS Code Team Monitor 已投影无正文 message/handoff/custody；Desktop 共享 overlay 已接入 Coding Agent、Workflow Manager、Specialized Agents，支持 canonical topology、critical path/slack、预算热图、metadata-only 因果下钻、attempt/lease/worktree/commit、Artifact/Effect lineage 及有界 snapshot diff/time-travel；详见 §12.28 | 为每个 Desktop GraphRun 接入固定 capability 的 durable trace/snapshot 历史，并以同一故障 fixture 关闭 CLI/Desktop/VS Code 跨产品 blocked-root、revision diff 与 time-travel 回放矩阵 |
