...
上述内容关闭模块 111 的仓库内产品化清单；其最终发布授权仍必须绑定本增量的合并 SHA，重新完成适用 CLI CI、CLI Strict Sandbox 与 Record & Replay Linux/Windows/macOS 旅程。完成该矩阵前，只声明“仓库实现已关闭”，不声明新 CLI 产品面已经随旧 npm 版本发布。

## 13. 全量任务完成情况（截至 2026-08-30）

状态口径：`✅ 已完成` 表示该编号自己的代码、确定性验证及应有发布边界已经关闭；`🟡 部分完成` 表示核心或公开基线已落地，但该编号定义的产品切换、跨端矩阵或外部验收尚未全部完成；`⏳ 待完成` 表示目前主要只有门禁或设计准备，关键目标尚未执行。总计 26 项：15 项已完成、11 项部分完成、0 项待完成。
...
| P0     | P0-6  | 关闭 Desktop/Cowork 已知执行旁路       | 🟢 当前树闭环 | 2026-08-30 回归修复删除 generic IPC 环境变量旁路；生产源码生成的固定 IPC capability 只允许有 main/scoped-preload 权威的精确 channel，无权威 renderer channel 显式 denied；realpath、raw MCP/network/Broker fail-closed 历史边界保持                                                                                                                                                                                                                                                                                                                                                                         | 下一发布前由最终 exact SHA 重跑三平台 CLI/Desktop 矩阵；全产品唯一 Broker 收敛归 P1-3/P1-11                                                                                                      |
| P0     | P0-7  | 沙箱、审批与审计 fail closed           | 🟢 当前树闭环 | 默认 workspace-write/network-off/fail-closed；真实 `run_shell` 缺 gate 拒绝；ApprovalGate 策略加载/落盘失败在 Desktop、普通 CLI 与 auto-mode 均拒绝；高风险 shell 在持久 audit admission 成功前不 spawn                                                                                                                                                                                                                                                                                                                                                                                      | 历史三平台 native strict boundary 已通过；包含本次回归修复的最终 SHA 仍须重跑权威矩阵                                                                                                               |
| P0     | P0-8  | 秘密、数据密钥与持久审计迁移           | 🟢 当前树闭环 | SecretStore/keyRef/wrapped DEK、最小子进程环境与事务迁移保持；进程审计现含 actor/session/authorization/policy/sandbox/result，命令与承载命令的 argv 脱敏，审计不可用时 spawn 前失败关闭                                                                                                                                                                                                                                                                                                                                                                      | 最终 exact SHA 发布矩阵待跑；生产 KMS/HSM 可后续增强                                                                                                                                                                  |
...
| P2     | P2-4  | Record & Replay → Skill                | 🟢 仓库闭环 | 稳定 CLI 产品面、真实 Playwright recorder、owner-private versioned store、人工审阅/策略/CAS/审计/保留期、URL/HTML 精确边界、现有 Skill loader 安装与安全撤销均已落地；本地 23 项聚焦测试、实际 CLI E2E、真实 Chromium 五动作旅程和网络逃逸负测通过；双站独立用户/设计文档与左侧导航入口已完成；详见 §12.68、§12.70 | 本增量最终合并 SHA 需重新通过 CLI/Strict 与 Record & Replay Linux/Windows/macOS exact-SHA 聚合门，之后才可声明该产品入口已发布 |
...
| P2     | P2-6  | Graph/Agent 真实旅程与发布矩阵         | ✅ 已完成   | 基础发布矩阵持续全绿；CLI `0.166.10@dcb28f5930` 已完成 CLI/Strict 三平台、OIDC 发布和独立公网回读，Open VSX `0.37.71` 已公开，JetBrains `0.4.102` 已通过六宿主并上传；精确候选 `2932aad32c` 又以真实 provider 在 Linux/Windows/macOS 完成 Graph/Agent、crash/resume/merge/message/worktree 旅程及聚合门                                                                                                                                                                                                                                                                                                                                                                   | —；JetBrains 公开可见性仍是外部人工审核状态，不改变仓库侧完成状态                                                                                                                                                              |
