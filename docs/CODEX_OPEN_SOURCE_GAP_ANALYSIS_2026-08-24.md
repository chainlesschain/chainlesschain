...
| ----- | ---------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P1-1  | 协议与审批 validator 增量已发布                      | canonical JSON Schema；TS/Python/Kotlin/Swift 确定性 codegen；冻结 v1 baseline 和 breaking-change 检查；protocol `0.1.2` 与 TS/Python SDK `0.2.2` 已公开；同一 ApprovalDecision fixture 已覆盖协议包、两套 SDK、CLI、VS Code、JetBrains、Android PDH 与 Desktop 审批切片                                          | 其余旧客户端手工事件 union、Swift 生产消费尚未全部迁移；CLI/Desktop/IDE 的完整事件流 conformance 尚未完成                                                                     |
| P1-2  | MVP 已发布，产品 pilot 与实验 WebSocket 已实现待门禁 | stdio JSON-RPC initialize、thread start/resume/fork/read、turn start/interrupt、item/approval 通知、能力协商和有界队列已发布；Desktop 与 VS Code 复用同一固定 capability SDK client；CLI 已实现强鉴权、输入/输出有界且明确标记实验级的 WebSocket 入口                                                             | 跑齐同一精确 SHA 的 Desktop/IDE 门禁与至少 30 分钟 overload/RSS soak；权威结果成功后发布并公网回读相关 CLI/SDK/IDE 候选                                                       |
...
| P1-10 | 部分随版本发布                                       | App Server stdio/WebSocket transport、输入/输出与 SDK client 有界化；WebSocket 接入 `OVERLOADED + retry_after_ms` 和慢消费者断路；协议和内核接口随实现提交 fixture                                                                                                                                                | 旧 transport/event/tool 队列普查、跨端 conformance matrix 和精确 SHA 的 30 分钟 RSS/过载门禁尚未完成                                                                          |
...
| P1     | P1-1  | 单一协议 Schema 与多语言 codegen       | 🟡 部分完成 | 已发布 protocol `0.1.4`、TS/Python SDK `0.2.3`；候选链 `b33a7121b1`、`e84fa20744`、`9ba64b5eef`、`c8b15e1ff8` 已完成 Android 生成类型消费、37-event payload union、TS/Python/Kotlin/Swift codegen、Desktop/CLI/IDE causal conformance 与 SDK 手写 wire union 清零；本地 protocol 13/13、Python 19/19、TS build、VS Code 7/7 | GitHub Actions 官方重大故障恢复后，跑齐同一精确 SHA 的 Protocol/Python/CLI/Strict/IDE/ARM64/Android/iOS/Desktop 门禁，并发布及公网回读 protocol `0.1.5`、SDK `0.2.4`、CLI `0.166.5`、VS Code `0.37.70`、JetBrains `0.4.101` |
| P1     | P1-2  | CC App Server                          | 🟡 部分完成 | stdio MVP 已发布；`d565f76310` 完成 Desktop/VS Code 固定 capability pilot；`32a6ecfb01` 完成强 token、远程 TLS、固定子协议、有界输入/输出、过载重试和慢消费者断路的实验 WebSocket；精确提交 5.30 秒非资格 smoke 为 1,919/1,919 回收、1,808 个预期过载、0 意外错误、队列不越界                                               | Actions 恢复后跑齐同一精确 SHA 的 Desktop/IDE/CLI/Strict 门禁与至少 30 分钟正式 overload/RSS soak，随后发布及公网回读相关候选                                                                                               |
...
| P1     | P1-10 | 有界队列、模块边界与增量 conformance   | 🟡 部分完成 | App Server stdio/WebSocket/SDK 队列有界化；WebSocket 已具备 receive/output/bytes/payload/connection cap、`OVERLOADED + retry_after_ms`、慢消费者断路与清理 deadline；严格审批、37-event fixture 和跨端 causal conformance 已进入硬门禁                                                                                      | 验证本轮精确 SHA；完成旧 transport/event/tool 队列普查，并让 30 分钟 RSS/过载门禁取得正式成功证据                                                                                                                           |
...
| P2     | P2-6  | Graph/Agent 真实旅程与发布矩阵         | ⏳ 待完成   | `e7a059d3...` 的 Protocol/Python SDK、`6b161992...` 的 CLI/Strict/Android、`33603c631e...` 的 IDE/ARM64 与 `afe376246b...` 的 iOS/RemoteSession 矩阵全绿；npm/PyPI/Open VSX 已公开，JetBrains `0.4.99` 已公开、`0.4.100` 已上传待审                                                                                         | 使用真实 provider secret 跑出 Linux/Windows/macOS Graph/Agent 聚合全绿，并完成 crash/resume/merge/消息恢复旅程                                                                                                              |
