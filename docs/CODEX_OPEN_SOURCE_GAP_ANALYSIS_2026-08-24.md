...
在已发布的 `0.166.1` 基线之后，`chainlesschain@0.166.2` CLI-only 补丁补上了真实 `cc team --agent` child 的实时 `team_send / team_receive / team_ack / team_followup` 宿主工具、attempt/lease/fence 绑定的私有本地桥、at-least-once 回执与幂等消费、`--state` mutation checkpoint，以及禁止嵌套 subagent 继承父 attempt 通道的隔离。精确提交 `f868e142068c33d203601cddd7643fd8ad9c4ffb` 已通过 Linux/Windows/macOS `CLI CI` 与 `CLI Strict Sandbox`，并由不可变标签 `v-npm-0-166-2` 通过 GitHub OIDC/Trusted Publishing 正式发布；公共 registry、签名 provenance 和不可变制品字节级回读均通过。此后的未发布 CLI 源码增量又把 idle target followup 物化为定向、高优先级、独立 lease/fence 的真实新 turn，复用原 collaboration session/权限/预算/workspace 契约，支持消息已提交但 wake task 未落盘的启动对账、最多 3 次 wake 后死信，并以两个真实 Node 进程验证 processed-before-ACK 恢复不重复副作用。canonical Graph message/handoff 投影、Team custody lease 原子转移、真实 provider 和长时/三平台恢复矩阵仍开放，因此 P1-6 仍不能标为完成，也尚未形成新的可发布版本。
...
| P1-6  | 核心与 child adapter 已发布；idle wake 增量待发布 | `0.166.2` 已发布 admitted/delivered/read/processed/dead-letter、真实 child 工具及 attempt/lease/fence 绑定；后续源码已将 idle followup 物化为复用原 session/contract 的定向 lease-bound turn，并补启动对账、3 次 wake 上限/死信与 processed-before-ACK 两进程恢复              | 新增源码尚未通过精确 SHA 三平台 CLI/strict 门禁、未发 npm；canonical Graph event 投影、Team handoff custody/lease 原子转移、真实 provider 与长时离线/重排矩阵仍未完成         |
...
[`TeamMailbox`](../packages/cli/src/lib/agent-team/team-mailbox.js) snapshot 已升级到 v3：实时 receive 不推进 processed cursor，未终态消息会 at-least-once 重投；ACK 区分 read/processed/dead-letter，稳定 consumer key 冲突失败关闭，out-of-order terminal receipt 不跳过更早消息；send/followup 要求稳定 message id 并保存 payload digest、causation/correlation 与 sender attempt。`--state` 下每次 send/receive/ack/followup/wake mutation 都走 team state 原子 checkpoint；未使用 `--state` 时 prompt 明确声明 process-local。最新未发布的 [`TeamRunner`](../packages/cli/src/lib/agent-team/team-runner.js) 已将 idle target followup 物化为只允许目标 holder claim 的高优先级动态 task：新 turn 获取独立 lease/fence，但复用原任务的 collaboration session、权限、预算、scope/worktree 契约；active target 仍返回 `target_active`，从未完成普通 turn 的 target 仍保持 durable queued，而不是伪造 wake。

这关闭了真实 CLI child 的 mid-turn adapter 和 idle target 新 turn admission 两个源码缺口。父进程在 followup admission 后、wake task 持久化前崩溃时会在恢复启动阶段对账；未 ACK 的 wake turn 最多重试 3 次后稳定进入 dead-letter。新增两进程 fixture 还覆盖“业务处理已持久化但 ACK 前进程退出”，恢复 turn 通过稳定 consumer key 只补 ACK，副作用计数保持 1。尚未完成的是接收方长时离线/poison/reorder soak、SessionMessageFabric/Graph message 的统一 durable projection，以及 handoff custody 与 AssignmentAttempt/lease 的原子转移；在这些边界及精确 SHA 发布门禁通过前 P1-6 继续开放。

建议：

1. 已完成真实 CLI child 的 holder/attempt/lease-bound `send / receive / ack / followup` 宿主工具、有界本地桥，以及 idle target 的持久化新 turn admission；下一步把 TeamMailbox v3 receipt/idempotency 语义投影到更成熟的 [`session-message-fabric.js`](../packages/cli/src/lib/session-message-fabric.js#L1) / canonical Graph message 账本，并补 TTL、跨进程 rate limit 和长时恢复。
...
3. 已在 TeamMailbox/真实 child adapter 明确采用 at-least-once delivery + 幂等 consumer，不宣称 exactly-once，并区分 admitted/delivered/read/processed/dead-letter，记录 causation/correlation、sender attempt/lease 与 payload digest；processed-before-ACK 已有真实两进程恢复 fixture。仍需把相同契约合并到 SessionMessageFabric/Graph Kernel，补 conversation/revision、poison message 长时重试及跨 channel 重排矩阵。
...
- 本地验证：TeamRunner/bridge/broker 定向 3 文件 77 项通过；扩展 Team + 跨进程恢复回归 11 文件 181 项通过；全部修改 JS/MJS ESLint 0 error，Prettier 与 `git diff --check` 通过。该增量尚未提交精确 release SHA，也未经过 GitHub Linux/Windows/macOS `CLI CI` 与 `CLI Strict Sandbox`，因此当前不发布、不改协议或 SDK 版本；待剩余 projection/custody 边界形成可发布增量后再决定 CLI patch。
