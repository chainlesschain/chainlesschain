...
| P1     | P1-1  | 单一协议 Schema 与多语言 codegen       | 🟡 部分完成 | protocol `0.1.2`、TS/Python SDK `0.2.2` 已发布；VS Code/JetBrains/Android PDH 审批切片开始共用 canonical fixture | Desktop/其余旧客户端 union、Kotlin/Swift 生成 client 生产消费、CLI/Desktop/IDE 完整事件流 conformance          |
...
| P1     | P1-9  | durable HumanTask 与统一策略事件       | 🟡 部分完成 | HumanTask 核心与公开结构化审批已发布；VS Code/JetBrains/Android PDH 二元 UI 发出最小权限决定并回显 exact binding | Desktop 与 grant UI、统一 hook/tool policy event、跨产品 cancel/quorum/race/restart conformance                |
