...
| P1-4  | 核心随 CLI 已发布；补偿 CI 已过、待版本发布 | typed/versioned Graph IR、digest、N/N-1 upcast/dry-run；effect-before-compile 的引用/环/端口/能力/预算/write-scope/loop/subgraph/Trigger/Region 校验；发布后增量增加补偿声明冲突、effect class、handler 独占与正向图隔离检查，并补充 N-1 dry-run/upcast 失败 fixture | loop/subgraph 的完整生产执行语义与跨真实已发布 schema 版本的完整 migration corpus 尚未完成；补偿增量尚未进入新标签/包版本 |
