...
| P1-3  | 🟡 仓库接线 65%～75%                        | App Server 复用真实 CLI agent loop；固定 Graph capability、Desktop `$team` canonical/shadow、Cowork/Scheduler result receipt、authority generation/lease/head CAS、旧 Desktop writer read-only 与 Browser non-durable gate 已落地                                                                                                                                                                      | CLI Team ready/message/handoff/distributed 单写者、旧 Desktop 入口的功能性 Graph adapter、逐面 canary/migration/rollback、同 SHA 三平台矩阵；详见 §6.9.6.7.5                  |
