...
| P1     | P1-10 | 有界队列、模块边界与增量 conformance   | 🟡 部分完成 | App Server/Agent IPC、Cowork AgentPool、MCP consent/metrics 与 direct-spawn 旁路子项已关闭；MCP HTTP SDK 已补 SSE/RPC/body/output 上限，`cd29f0e5d9` 又为 MCP stdio SDK 增加输入行、在途 handler、输出消息/队列字节硬上限、结构化 overload、drain deadline 及三平台/coverage/综合门禁                                  | 下一项为 `community-registry.js` 增加 registry HTTP response body 字节上限与异常流清理；之后继续其余 backlog、模块拆分及 crash/recovery/migration conformance matrix |
