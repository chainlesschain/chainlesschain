...
| P1-12 | 全量生产证据 gate 已闭环，待真实 rollout    | CLI Team/Cowork/Scheduler/Desktop/Browser 的 machine-readable claims、单一 writer 约束、shadow equivalence、terminal-evidence/cutover gate；23 个入口有唯一 rollout key，逐 store RPO=0、回滚保权和同 SHA 三平台证据由 hash-chain ledger 强制；13 retire 有 Graph 替代和历史只读契约，3 个 Browser/Remote 入口明确 non-durable + disabled；精确候选三平台真实运行及 60/60 聚合门已通过；§6.9.6.7.25 新增覆盖 20 durable entry、九类 projection、三种 rollback、全部旧 writer/mutation、3 disabled entry 与 OIDC provenance 的 fail-closed production aggregate gate                                                                                                                                                                                                                                                                                                                                                                                                                              | 在真实 `graph-kernel-production` 环境生成 staged rollout/观察 evidence，并让受保护 workflow 对同一精确 SHA 产出通过 receipt；缺少该 artifact 时不得改绿                                                                     |
