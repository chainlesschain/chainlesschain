...
| ----- | ------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P1-1  | 协议与审批 validator 增量已发布             | canonical JSON Schema；TS/Python/Kotlin/Swift 确定性 codegen；冻结 v1 baseline 和 breaking-change 检查；protocol `0.1.2` 与 TS/Python SDK `0.2.2` 已公开；同一 schema 生成 JS/TS/Python runtime validator，并以同一 ApprovalDecision fixture 验证协议包、两套 SDK、CLI、VS Code、JetBrains 与 Android PDH 的审批切片 | Desktop 与其余旧客户端手工事件 union 尚未全部迁移；CLI/Desktop/IDE 的完整事件流 conformance 尚未完成                                                                          |
...
| P1-9  | 公开结构化审批增量已发布                    | 可认领/恢复 HumanTask、quorum/职责分离与 cancel/decision CAS；canonical ApprovalDecision、binding/requested permissions、N-1 boolean 兼容已公开到 TS/Python SDK 与 App Server；CLI 已实现 exact turn/session grant；VS Code、JetBrains 与 Android PDH 的二元审批 UI 已改为最小权限结构化决定并回显 exact binding     | Desktop 审批界面、可审阅的 turn/session grant UI、统一 hook/tool policy event，以及跨产品 approval-vs-cancel/quorum/race/restart conformance 尚未完成                         |
...
- 上一客户端迁移提交 `bc5c969f30d9a72e05bb192364aab370ac494bb7` 的 Android 汇总额外暴露 `CcExecServiceTest.executeArgv timeout kills runaway` 偶发失败：超时只杀直接子进程，而后代仍持有 stdout/stderr 管道，drain 协程等待到 `sleep 30` 自然退出并触发 `UncompletedCoroutinesError`。本增量在超时/取消后关闭进程管道并取消、join 两个 drain，避免无关后代把调用拖过 coroutine 测试上限；Windows 编译与非 Unix 分支通过，最终关闭仍以新 SHA 的 Linux Android workflow 为准。
- 同一 SHA 还存在一个被 job 级 `continue-on-error` 隐藏的红项：Android CI/CD `Security Scan` 拉取 MobSF `latest` 后调用不存在的 `python manage.py scan`，实际从未扫描 APK。本增量固定 MobSF v4.5.2 多架构镜像摘要，启动官方服务入口，以随机 API key 调用 `/api/v1/upload`、`/api/v1/scan`、`/api/v1/report_json`，校验 JSON 并上传报告；该 job 已移除软失败，协议生成源码变化也会触发 Android CI/CD。CI 完整性契约已增加对应回归，当前 21/21 通过；真实 APK 扫描仍以新 SHA 的 hosted Linux job 为权威结果。
...
- P1-1 剩余范围进一步缩小为 Desktop/其余客户端手工 union、Swift 生成类型的真实生产消费，以及 CLI/Desktop/IDE 完整事件流 conformance；P1-9 的统一策略事件与跨产品 cancel/quorum/race/restart 仍未关闭。
...
| P1     | P1-1  | 单一协议 Schema 与多语言 codegen       | 🟡 部分完成 | protocol `0.1.2`、TS/Python SDK `0.2.2` 已发布；Kotlin/Swift 已生成严格审批 union，Android/JetBrains 已生产消费 Kotlin，`0.1.3` 待新 SHA 门禁 | Desktop/其余旧客户端 union、Swift 生成 client 生产消费、CLI/Desktop/IDE 完整事件流 conformance                 |
| P1     | P1-2  | CC App Server                          | 🟡 部分完成 | stdio JSON-RPC MVP、thread/turn/item/approval、能力协商、有界队列和 TS client 已随 CLI/SDK 发布                                               | Desktop/IDE pilot、实验 WebSocket 与 30 分钟 overload soak                                                     |
| P1     | P1-3  | canonical Agent Kernel 与 rollout      | 🟡 部分完成 | 真实 CLI loop、rollout hash chain/SQLite/checkpoint/compact/resume/fork 与 terminal evidence 已发布                                           | Desktop `$team`、旧 runtime 和全部执行入口切到唯一内核                                                         |
...
| P1     | P1-9  | durable HumanTask 与统一策略事件       | 🟡 部分完成 | HumanTask 核心与公开结构化审批已发布；VS Code/JetBrains/Android PDH 二元 UI 发出最小权限决定，Kotlin 客户端改用生成 union                     | Desktop/Swift 与 grant UI、统一 hook/tool policy event、跨产品 cancel/quorum/race/restart conformance          |
| P1     | P1-10 | 有界队列、模块边界与增量 conformance   | 🟡 部分完成 | App Server/SDK 队列有界化，接口变更携带 fixture，发布 SHA 广泛矩阵通过                                                                        | 旧 transport/event/tool 队列普查、跨端矩阵、30 分钟 RSS/过载门禁                                               |
...
| P2     | P2-2  | Graph topology/timeline 调试器         | 🟡 部分完成 | CLI inspect/diff/eval、Agent/Task/Artifact/Message/Effect/Timeline、critical path 与 time travel 已发布                                       | Desktop Team Monitor 交互 UI、预算热图与跨产品回放                                                             |
...
| P2     | P2-6  | Graph/Agent 真实旅程与发布矩阵         | ⏳ 待完成   | `67fdfd...` 的 CLI/Strict/E2E/CI/IDE 与 OIDC 发布、provenance、独立回读均通过                                                                 | 使用真实 provider secret 跑出 Linux/Windows/macOS Graph/Agent 聚合全绿，并完成 crash/resume/merge/消息恢复旅程 |
