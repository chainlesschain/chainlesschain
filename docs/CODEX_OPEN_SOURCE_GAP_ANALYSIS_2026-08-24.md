...
### 12.69 `0.166.11`/IDE 候选复核与外部门禁实况（2026-08-30）
...
- [npm publish run 33274068057](https://github.com/chainlesschain/chainlesschain/actions/runs/33274068057) 在首次发布 `@chainlesschain/context-memory-kernel@0.1.0` 时因 npm scope/token 返回 `E404` 而失败；发布链按顺序停止，所以公共 registry 仍是 `chainlesschain@0.166.10`、`@chainlesschain/agent-sdk@0.2.5`，Context/Memory Kernel 包尚不存在。CLI `0.166.11` 与 TS SDK `0.2.6` 必须在更新具备 `@chainlesschain` scope 写权限且允许自动化发布的 granular token 后重新跑同一标签发布门，不能本地 `npm publish` 绕过 Actions。
- 源码候选为 VS Code/Open VSX `0.37.74` 与 JetBrains `0.4.105`；公共 registry 复核分别仍为 `0.37.73` 和已批准/列出的 `0.4.103`。两项新版本应绑定最终合并 SHA 重新完成各自发布工作流后再建不可变标签；CLI/SDK 尚未形成完整公开依赖链时不提前发布 IDE 标签。
...
因此当前可以提交仓库实现并让最终合并 SHA 进入 CLI/IDE 正式矩阵；只有该 SHA 的权威门全绿后才创建新的 CLI/IDE 发布标签。即使 CLI/IDE 发布成功，P1-10、P1-11、P1-12、P2-3 及 P1-3 production close 仍必须保持部分完成，直到上述外部 producer/凭据/预算/真实流量证据实际通过。
