...
| P1-3  | 🟡 仓库接线 81%～89%                        | App Server 复用真实 CLI agent loop；固定 Graph capability、Desktop `$team`/Specialized Agents/WorkflowManager canonical+shadow adapter、Cowork/Scheduler result receipt、authority generation/lease/head CAS、旧 Desktop writer read-only；CLI Team local 与 distributed dispatch/settle/cancel 已接入唯一 Graph writer，shadow message/handoff、cut-point 恢复、跨进程 writer/lease fencing 和迟到结果拒绝已落地 | Desktop 真实 IPC/provider crash-resume-reconcile 与 renderer 投影、逐 store migration/canary/rollback、跨机器 custody/长时恢复及同 SHA 三平台矩阵；详见 §6.9.6.7.5～§6.9.6.7.8 |
...
- 若只按“唯一内核切换”这个独立 epic 计，2026-08-27 审计起点约完成 **25%～35%**；经 §6.9.6.7.5～§6.9.6.7.8 的隔离实现后，当前严格 production-close 口径约完成 **64%～72%**。剩余权重集中在 Desktop 真实进程/renderer 验收、逐 store cutover/rollback 和同一精确 SHA 三平台发布梯子，而不是 Kernel 核心原语。
