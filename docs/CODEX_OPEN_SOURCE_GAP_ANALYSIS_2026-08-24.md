...
| P1-10 | 部分随版本发布                              | App Server stdio/WebSocket/SDK、Agent IPC 与 Desktop Cowork AgentPool 已有队列、帧、并发或 admission 上限；接入结构化 `OVERLOADED`、慢消费者断路及 deadline；协议/内核 fixture、跨端 causal conformance、30 分钟 RSS/过载门、CLI `0.166.6` 发布回读和 Cowork 三平台完整 unit roots 均已完成                       | 继续普查并有界化其余旧 transport/event/message/tool backlog，拆分剩余超大模块的 ports/adapters/state machines，并扩展 crash/recovery/migration conformance matrix             |
...
| P1     | P1-10 | 有界队列、模块边界与增量 conformance   | 🟡 部分完成 | App Server stdio/WebSocket/SDK 与 Agent IPC 上限、慢消费者断路、30 分钟 RSS/过载门、CLI `0.166.6@f2a249bf3d` 发布回读已关闭；`a8484915fa` 又为 Desktop Cowork AgentPool 增加 128 项 admission 上限、结构化 `OVERLOADED`、可观测状态及 waiter timer 清理，并通过 Linux/macOS/Windows 完整 unit roots     | Agent IPC 与 Cowork AgentPool 子项已关闭；继续普查其余旧 transport/event/message/tool backlog，拆分剩余超大模块，并扩展 crash/recovery/migration conformance matrix |
