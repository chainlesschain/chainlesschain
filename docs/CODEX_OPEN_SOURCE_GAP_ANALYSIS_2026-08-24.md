...
| P1     | P1-10 | 有界队列、模块边界与增量 conformance   | 🟡 部分完成 | App Server/Agent IPC、Cowork AgentPool 与 MCP consent 子项已关闭；`2cef0b5586` 又为可选 MCP metrics 增加样本/维度/错误数量与字节上限、overflow bucket、敏感大 context 摘要和三平台 37-test 门禁                                                                                                            | 下一项删除无引用的 direct-spawn 旧 `StdioTransport`；之后继续其余 backlog、模块拆分与 crash/recovery/migration conformance matrix                                  |
