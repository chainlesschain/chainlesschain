...
| P1     | P1-1  | 单一协议 Schema 与多语言 codegen       | 🟡 部分完成 | protocol `0.1.3`、TS/Python SDK `0.2.2` 已发布；Kotlin/Swift 严格审批 union 已由 Android/JetBrains/Desktop/iOS/Phone/Wear 消费，Web/VS Code 也使用 canonical validator/决定 | `ApprovalDecision` 以外完整事件 union 的全语言生产消费，CLI/Desktop/IDE 端到端事件流 conformance             |
...
| P2     | P2-6  | Graph/Agent 真实旅程与发布矩阵         | ⏳ 待完成   | `7eac0f3...` 的 Protocol/IDE/E2E/Android 矩阵与 OIDC 发布、provenance、独立回读均通过；CLI `67fdfd...` 发布证据继续有效                                   | 使用真实 provider secret 跑出 Linux/Windows/macOS Graph/Agent 聚合全绿，并完成 crash/resume/merge/消息恢复旅程 |
