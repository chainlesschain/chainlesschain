...
| ----- | ---------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P1-1  | 已完成并发布                                        | canonical JSON Schema；37-event payload union；TS/Python/Kotlin/Swift 确定性 codegen/validator；Android/iOS/Desktop/CLI/VS Code/JetBrains 生产消费与 causal conformance；protocol `0.1.5`、TS/Python SDK `0.2.4`、CLI `0.166.5`、VS Code `0.37.70` 已公开，JetBrains `0.4.101` 已上传待人工审核 | 仓库实现、精确候选矩阵与应有发布边界均已关闭；JetBrains Marketplace 公开可见性为外部人工审核状态，不阻塞后续任务                                                            |
...
| P1-10 | 部分随版本发布                                       | App Server stdio/WebSocket transport、输入/输出与 SDK client 有界化；WebSocket 接入 `OVERLOADED + retry_after_ms` 和慢消费者断路；协议/内核 fixture、跨端 causal conformance 与精确 SHA 的 30 分钟 RSS/过载门禁已完成                                                                                  | 继续普查并有界化旧 transport/event/message/tool backlog，拆分剩余超大模块的 ports/adapters/state machines，并扩展 crash/recovery/migration conformance matrix                 |
...
| P1     | P1-1  | 单一协议 Schema 与多语言 codegen       | ✅ 已完成   | `2f5b0f263a` 的 Protocol/Python/CLI/Strict/IDE/ARM64/Android/iOS/Desktop 相关矩阵全绿；37-event payload union、四语言 codegen/validator、主要生产消费与跨端 causal conformance 已闭环；protocol `0.1.5`、TS/Python SDK `0.2.4`、CLI `0.166.5`、Open VSX `0.37.70` 已公开，JetBrains `0.4.101` 上传成功 | —；JetBrains `0.4.101` 公开可见性等待外部人工审核，不改变仓库侧完成状态                                                                                                                                                        |
...
| P2     | P2-6  | Graph/Agent 真实旅程与发布矩阵         | ⏳ 待完成   | `2f5b0f263a` 的 Protocol/Python/CLI/Strict/IDE/ARM64/Android/iOS、E2E/Full/Code Quality 与 30 分钟 App Server soak 均绿；protocol `0.1.5`、SDK `0.2.4`、CLI `0.166.5`、Open VSX `0.37.70` 已公开，JetBrains `0.4.101` 已上传待审                                                                         | 使用真实 provider secret 跑出 Linux/Windows/macOS Graph/Agent 聚合全绿，并完成 crash/resume/merge/消息恢复旅程                                                                                                              |
