...
> 最新 canonical Agent 事件发布基线：协议/Python 精确提交 `e7a059d3ed759832d63bbfc33127bd472390ed70` 已发布 protocol `0.1.4` 与 Python SDK `0.2.3`；CLI/TS SDK 精确提交 `6b1619926c5aadc4586e17994b607169b2ae58ae` 已发布 CLI `0.166.4`、TS SDK `0.2.3` 与 Open VSX `0.37.69`，JetBrains `0.4.99` 已成功上传并等待 Marketplace 人工审核
...
| P1     | P1-1  | 单一协议 Schema 与多语言 codegen       | 🟡 部分完成 | protocol `0.1.4`、TS/Python SDK `0.2.3` 已发布；严格审批 union 已由主要产品消费；37 个 stream event discriminator 与 envelope 已从单一 schema 生成到 TS/Python/Kotlin/Swift 和 VS Code vendor | 各事件 payload 级完整生成 union、Kotlin/Swift 新 envelope 的全生产消费、CLI/Desktop/IDE 端到端 causal event-stream conformance |
...
| P1     | P1-10 | 有界队列、模块边界与增量 conformance   | 🟡 部分完成 | App Server/SDK 队列有界化；严格审批与 37-event canonical fixture 随生成绑定进入 TS/Python/Kotlin/Swift/VS Code，精确发布 SHA 的广泛矩阵通过                 | 旧 transport/event/tool 队列普查、CLI/Desktop/IDE causal conformance、30 分钟 RSS/过载门禁                      |
...
| P2     | P2-6  | Graph/Agent 真实旅程与发布矩阵         | ⏳ 待完成   | `e7a059d3...` 的 Protocol/Python SDK 与 `6b161992...` 的 CLI/Strict/IDE/ARM64/Android 矩阵全绿；npm/PyPI/Open VSX 发布、provenance 与公网回读通过，JetBrains 上传待审 | 使用真实 provider secret 跑出 Linux/Windows/macOS Graph/Agent 聚合全绿，并完成 crash/resume/merge/消息恢复旅程 |
