...
| P1     | P1-1  | 单一协议 Schema 与多语言 codegen       | 🟡 部分完成 | protocol `0.1.4`、TS/Python SDK `0.2.3` 已发布；严格审批 union 已由主要产品消费；37 个 stream event discriminator 与 envelope 已从单一 schema 生成到 TS/Python/Kotlin/Swift 和 VS Code vendor；JetBrains `0.4.100` 的生产事件分派已消费生成 Kotlin enum 并完成上传 | 各事件 payload 级完整生成 union、Android/其余 Kotlin 与 Swift 新 envelope 的生产消费、CLI/Desktop/IDE 端到端 causal event-stream conformance |
...
| P2     | P2-6  | Graph/Agent 真实旅程与发布矩阵         | ⏳ 待完成   | `e7a059d3...` 的 Protocol/Python SDK、`6b161992...` 的 CLI/Strict/Android 与 `33603c631e...` 的 IDE/ARM64 矩阵全绿；npm/PyPI/Open VSX 已公开，JetBrains `0.4.99` 已公开、`0.4.100` 已上传待审 | 使用真实 provider secret 跑出 Linux/Windows/macOS Graph/Agent 聚合全绿，并完成 crash/resume/merge/消息恢复旅程 |
