...
| P1-2  | MVP 已发布，Desktop/IDE pilot 已实现待门禁  | stdio JSON-RPC initialize、thread start/resume/fork/read、turn start/interrupt、item/approval 通知、能力协商和有界队列已发布；Desktop 与 VS Code 现复用同一份固定 capability SDK client，默认关闭且不暴露任意 RPC                                                               | 跑齐精确 SHA 的 Desktop/IDE 门禁；实现明确标记实验级的 WebSocket 入口，并完成至少 30 分钟 overload/RSS soak                                                                   |
...
- P1-2 仍为“部分完成”：Desktop/VS Code pilot 的实现边界已关闭但权威矩阵尚未通过；实验 WebSocket transport 与至少 30 分钟 overload/RSS soak 仍需逐项完成。该提交继续属于 SDK `0.2.4` 与 VS Code `0.37.70` 候选，不在精确门禁前发包，也不为 Desktop feature flag 单独提升应用商店版本。
...
| P1     | P1-2  | CC App Server                          | 🟡 部分完成 | stdio MVP 已发布；`d565f76310` 实现 SDK 固定 capability client、Desktop Broker/fixed IPC pilot、VS Code fixed-command pilot 与双端 vendor 字节一致性；本地 SDK build、VS Code 132/132、Desktop main build 通过 | 跑齐精确 SHA 的 Desktop/IDE 门禁；实现实验 WebSocket transport，并完成至少 30 分钟 overload/RSS soak            |
...
| P1     | P1-10 | 有界队列、模块边界与增量 conformance   | 🟡 部分完成 | App Server/SDK 队列有界化；严格审批、37-event canonical fixture 与 CLI/Desktop/IDE causal partial-order conformance 已实现并进入硬门禁                  | 验证本轮 causal 候选的精确 SHA；完成旧 transport/event/tool 队列普查与 30 分钟 RSS/过载门禁                    |
