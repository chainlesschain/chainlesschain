...
| P1-4  | 核心随 CLI 已发布；结构化控制流增量待 CI/版本 | typed/versioned Graph IR、digest、N/N-1 upcast/dry-run；effect-before-compile 校验；补偿隔离；新增多节点 bounded loop 的 iteration frame/确定性 attempt、证据绑定 decision、cap fail-closed 与恢复，以及 digest-pinned subgraph 父子 GraphRun、深度/递归 guard、恢复和取消级联 | effectful loop 的 iteration-scoped receipt/idempotency/compensation 仍 fail closed；subgraph typed input/output 映射与预算切片、生产 adapter 切换、跨真实已发布 schema 的完整 migration corpus 尚未完成；增量尚未进入新标签/包版本 |
...
- Trace projection 新增 iteration/subgraph graph。本地 Graph 定向集共 6 个文件、57 项通过，修改文件 ESLint 无错误；CLI 全量本地 `npm test` 运行 5 分钟仍未给出用例摘要后已主动停止，不计为通过。本增量未创建 npm/PyPI 版本，也不能沿用前述 `161d681...` 的 Actions 结果，必须以本增量精确提交的新矩阵为准。
