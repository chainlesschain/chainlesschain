...
> 最新未发布结构化审批候选：精确源码提交 `dc855f9adc5027281d113b8b51559497c9a6c534` 已完成同源 `ApprovalDecision` validator、N-1 boolean 兼容和最小范围 turn/session grant，并通过协议、CLI、Strict Sandbox、Python SDK 与桌面 E2E 权威矩阵；通用 `CI Tests` 又发现 WebShell 集成夹具未注入必需 MCP 安全策略，修复后的新提交仍须重新完成 exact-SHA 门禁，不复用旧发布标签
...
最新未发布增量开始收口 P1-1/P1-9 的公开审批断层：canonical schema 生成的 JavaScript/TypeScript/Python validator 与同一份合法/非法 fixture 现已覆盖协议包、TS SDK、Python SDK 和 CLI App Server；两套 SDK 的回调可返回 `acceptOnce / acceptForTurn / acceptForSession / decline / cancel`，请求携带 exact binding 与最小 `requested_permissions`，非法、异常、binding 缺失或不匹配均失败关闭。CLI 只复用精确匹配 tool、args、cwd、policy 的 grant；turn grant 不落盘，session grant 只有在 tamper-evident authority event 持久化成功后才生效，损坏恢复会全部丢弃，持久化失败降级为 `acceptOnce`。旧 boolean 回调和直接 `respondApproval(..., boolean)` 保留 N-1 wire 兼容。精确源码提交 `dc855f9adc5027281d113b8b51559497c9a6c534` 的协议、CLI、Strict Sandbox、Python SDK 与 E2E 矩阵已通过；通用 `CI Tests` 的三平台完整 fallback 另发现一处陈旧 WebShell 测试夹具，本地修复及相关 41 项回归已通过，等待后继提交重跑。本增量改变四个公开包的内容，不能沿用 `agent-protocol-oidc-v0.1.1`、SDK `0.2.1` 或 `v-npm-0-166-2`；只有实际 release commit 同时通过要求的精确 SHA 矩阵后，才考虑依次发布 protocol `0.1.2`、TS/Python SDK `0.2.2` 和 CLI `0.166.3`。
...
| ----- | ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P1-1  | 已发布基线；审批 validator 增量待发布           | canonical JSON Schema；TS/Python/Kotlin/Swift 确定性 codegen；冻结 v1 baseline 和 breaking-change 检查；已发布 protocol `0.1.1`/SDK `0.2.1`；新候选从同一 schema 生成 JS/TS/Python runtime validator，并以同一 ApprovalDecision fixture 验证协议包、两套 SDK 与 CLI                                             | 新候选尚未通过精确 SHA 权威矩阵或发包；旧客户端手工事件 union 尚未全部迁移；CLI/Desktop/IDE 同 fixture 的三端 conformance 尚未完成                                                                                        |
...
| P1-6  | 消息 adapter 已发布；custody/Session 增量待发布 | `0.166.2` 已发布真实 child 消息工具；后续源码已落地 idle followup 新 turn、canonical message/handoff 投影、完整 custody 状态机，以及 state-backed `cc team` 的 SessionMessageFabric adapter、legacy v3 migration、跨进程 rate limit/offline recovery、processed-before-ACK、poison dead-letter 和锁内总字节背压 | 最新后继提交的完整 CLI CI 已通过，运行时代码父提交的三平台 Strict 也已通过；真实 provider、长时离线/poison/reorder 正式 soak、分布式 custody、全产品 authoritative adapter/旧 writer 切换仍未完成，故 P1-6 不关闭也不发包 |
...
| P1-9  | 核心已发布；公开结构化审批增量待发布            | 已有可认领/恢复 HumanTask、quorum/职责分离与 cancel/decision CAS；新候选把 canonical ApprovalDecision、binding/requested permissions、N-1 boolean 兼容暴露到 TS/Python SDK 与 App Server，并在 CLI 实现 exact turn/session grant、authority-event 恢复、损坏丢弃和持久化失败降级                                | 新候选尚未通过精确 SHA 权威矩阵或发包；Desktop/IDE 审批界面、统一 hook/tool policy event，以及跨产品 approval-vs-cancel/quorum/race/restart conformance 尚未完成                                                          |
...
这关闭了“公开 SDK 只有 boolean”这一实现缺口，但没有关闭整个 P1-9。仍需把结构化决定接入 Desktop/IDE 人机界面，统一 hook/tool policy event，并以跨产品 fixture 覆盖 replan 后旧审批、approval-vs-cancel、多人 quorum、职责分离和 crash/restart race。当前包版本仍是 protocol `0.1.1`、TS/Python SDK `0.2.1`、CLI `0.166.2`；候选必须通过精确 SHA 权威门禁后才能创建新补丁版本和不可变标签。
...
- [通用 `CI Tests`](https://github.com/chainlesschain/chainlesschain/actions/runs/32847784569) 的 Full Test Suite、构建、数据库、PDH、代码质量和 CLI 安装均成功，但三平台 Unit Tests 的 fail-closed 全量 fallback 同时在 `web-shell-integration.test.js` 两项失败；其余分别有约 28,229 项通过。原因是安全收紧后 `mcp.call_tool` 与 `mcp.read_resource` 已要求显式 MCP policy，集成测试仍只注入 manager，因而正确收到 `ok:false`。当前修复为测试注入 allow-policy stub，并额外断言 tool/resource 调用确实先经过 policy；WebShell handler + 真实 WS 往返相关 2 文件 41 项和 Prettier 本地通过。该修复必须在后继精确 SHA 的 Windows/macOS/Linux fallback 中全绿后才计为权威关闭。
- 本增量实质修改 protocol、TS/Python SDK 和 CLI 四个发布包，因此完成后需要补丁版本，但当前后继提交的门禁尚未完成，不能发布：预期顺序为 protocol `0.1.2` → TS/Python SDK `0.2.2` → CLI `0.166.3`，每个不可变标签必须指向实际 release commit，并满足各自 exact-SHA Linux/Windows/macOS 门禁；旧标签和旧 SHA 的成功结果均不可复用。
