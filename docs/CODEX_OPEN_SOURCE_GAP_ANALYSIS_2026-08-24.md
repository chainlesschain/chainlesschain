...
> 最新未发布结构化审批候选：Agent Protocol、TS/Python SDK 与 CLI 已完成同源 `ApprovalDecision` validator、N-1 boolean 兼容和最小范围 turn/session grant；等待精确提交的权威矩阵，不复用旧发布标签
...
- [`E2E Tests` 运行 32841347952](https://github.com/chainlesschain/chainlesschain/actions/runs/32841347952) 的 Windows/macOS/Linux 三个 job 均在 `simple-api.e2e.test.ts` 以 `[IPC] invoke unavailable for channel "initial-setup:get-status"` 失败，三平台没有第二种产品断言错误。根因是 `sandbox: true` 的 Electron preload 顶层引入相对 CommonJS 模块，导致 preload 在 `contextBridge` 暴露 typed API 前整体终止；当前候选已把很小的 legacy IPC gate 保持在单文件内，并增加“sandboxed preload 只能 require electron”的静态回归。E2E helper 同时改为等待/调用 `initialSetup` 与 `team` typed capability，删除已默认关闭的 generic IPC fallback。桌面主进程构建和该防回归测试 3 项本地通过；只有新提交的三平台 E2E 重跑全绿后才能把该事故标为关闭。
- 本增量实质修改 protocol、TS/Python SDK 和 CLI 四个发布包，因此完成后需要补丁版本，但现在没有发布资格：预期顺序为 protocol `0.1.2` → TS/Python SDK `0.2.2` → CLI `0.166.3`，每个不可变标签必须指向实际 release commit，并满足各自 exact-SHA Linux/Windows/macOS 门禁；旧标签和旧 SHA 的成功结果均不可复用。
