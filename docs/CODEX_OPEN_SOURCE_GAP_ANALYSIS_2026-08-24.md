...
| P1-3  | 🟡 仓库接线约 90%～94%                      | App Server 复用真实 CLI agent loop；固定 Graph capability、Desktop `$team`/Specialized Agents/WorkflowManager canonical+shadow adapter、Cowork/Scheduler result receipt、authority generation/lease/head CAS；`af7910bd66` 将 inventory 扩为 23 个入口：7 migrate、13 retire、3 non-durable disabled，并要求每个 retire 有 Graph 替代入口、每个 retire/disabled mutation 有直接 guard；Browser direct engine/AI automation 旁路已下沉 gate | 23-entry 最终候选的完整依赖门、Linux/Windows/macOS 真实 provider + 60/60 entry-platform-store artifact、五 surface 真实 shadow/canary/default/read-only 推进、13 retire 替代可达性/旧 writer 观察窗口及跨机器长时 soak；详见 §6.9.6.7.15 |
...
| P1     | P1-3  | canonical Agent Kernel 与 rollout      | 🟡 部分完成 | 真实 CLI loop、固定 App Server Graph capability、Desktop `$team`/Specialized Agents/WorkflowManager canonical+shadow adapter、Cowork/Scheduler receipt-first terminal adapter、authority generation/lease/head CAS、CLI Team local/distributed canonical dispatch 与主要 RPO=0 cut point 已落地；`af7910bd66` 将生产 inventory 从历史 11 项扩为 23 项（7 migrate + 13 retire + 3 disabled），补齐旧 AI/autonomous/cowork/pipeline/P2P/Browser AI writer、退役替代契约、直接 mutation guard 与 Browser core-engine gate；仓库接线约 90%～94% | 最终候选完整依赖门、Linux/Windows/macOS 真实 provider + 60/60 store matrix、五 surface 真实 shadow/canary/default/read-only、13 retire 替代可达性/旧 writer 观察窗口及跨机器 custody/长时 soak；剩余约 5～10 人日，详见 §6.9.6.7.15 |
...
| P1     | P1-12 | Graph Kernel 集成、双写验证与迁移切换  | 🟡 部分完成 | 五 surface/23 入口 machine-readable inventory、唯一 rollout key、run-scoped authority、shadow adapter、migration saga、receipt cut-point recovery、retire replacement/historical-read-only 契约与 disabled direct-engine gate 已落地；hash-chain ledger 强制逐 store RPO=0、回滚保留既有 canonical authority 和 Linux/Windows/macOS 同 SHA 证据；Browser 标明 non-durable 且默认关闭 | 五 surface 分别产生并通过真实 shadow/canary/default/read-only、完整 projection equivalence、13 retire 替代/零旧 writer 观察、回滚演练、旧 shell 下线与本候选 SHA 三平台真实运行证据；关闭条件见 §6.9.6.7.4 与 §6.9.6.7.15 |
