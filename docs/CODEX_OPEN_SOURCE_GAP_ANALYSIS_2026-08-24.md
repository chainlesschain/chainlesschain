...
| P1-3  | 🟡 仓库接线约 94%～97%                      | App Server 复用真实 CLI agent loop；固定 Graph capability、Desktop `$team`/Specialized Agents/WorkflowManager canonical+shadow adapter、Cowork/Scheduler result receipt、authority generation/lease/head CAS；`3cce414988` 保持 23-entry inventory（7 migrate、13 retire、3 disabled），将 13 retire 扩展为 360 个直接 guarded mutation、15 条替代边与 32 个历史读方法，并修正 legacy Workflow 的 durable SQLite 事实与只读回读 | 最终候选完整依赖门、Linux/Windows/macOS 真实 provider + 60/60 entry-platform-store artifact、五 surface 真实 shadow/canary/default/read-only 推进、13 retire 替代可达性/旧 writer 观察窗口及跨机器长时 soak；详见 §6.9.6.7.17 |
...
| P1-12 | 持久化切流门禁就绪                          | CLI Team/Cowork/Scheduler/Desktop/Browser 的 machine-readable claims、单一 writer 约束、shadow equivalence、terminal-evidence/cutover gate；23 个入口有唯一 rollout key，逐 store RPO=0、回滚保权和同 SHA 三平台证据由 hash-chain ledger 强制；13 retire 有 Graph 替代和历史只读契约，3 个 Browser/Remote 入口明确 non-durable + disabled                                                                                       | 尚未产生本候选 SHA 的 Linux/Windows/macOS 真实运行证据，也未完成真实 shadow/canary/default/read-only、完整回滚演练、替代入口可达性观察或旧 writer 下线；因此不得将 P1-12 标为完成                                             |
...
| P1     | P1-3  | canonical Agent Kernel 与 rollout      | 🟡 部分完成 | 真实 CLI loop、固定 App Server Graph capability、Desktop `$team`/Specialized Agents/WorkflowManager canonical+shadow adapter、Cowork/Scheduler receipt-first terminal adapter、authority generation/lease/head CAS、CLI Team local/distributed canonical dispatch 与主要 RPO=0 cut point 已落地；`3cce414988` 保持 23-entry inventory（7 migrate + 13 retire + 3 disabled），将 13 retire 扩为 360 个直接 guarded mutation、15 条替代边与 32 个历史读方法，修正 legacy Workflow durable SQLite 并补历史只读回读；仓库接线约 94%～97% | 最终候选完整依赖门、Linux/Windows/macOS 真实 provider + 60/60 store matrix、五 surface 真实 shadow/canary/default/read-only、13 retire 替代可达性/旧 writer 观察窗口及跨机器 custody/长时 soak；剩余约 5～10 人日，详见 §6.9.6.7.17 |
...
| P1     | P1-12 | Graph Kernel 集成、双写验证与迁移切换  | 🟡 部分完成 | 五 surface/23 入口 machine-readable inventory、唯一 rollout key、run-scoped authority、shadow adapter、migration saga、receipt cut-point recovery、360 个 retire mutation 直接 guard、replacement/historical-read-only 契约与 disabled direct-engine gate 已落地；hash-chain ledger 强制逐 store RPO=0、回滚保留既有 canonical authority 和 Linux/Windows/macOS 同 SHA 证据；Browser 标明 non-durable 且默认关闭                                                                                                                     | 五 surface 分别产生并通过真实 shadow/canary/default/read-only、完整 projection equivalence、13 retire 替代/零旧 writer 观察、回滚演练、旧 shell 下线与本候选 SHA 三平台真实运行证据；关闭条件见 §6.9.6.7.4 与 §6.9.6.7.17           |
