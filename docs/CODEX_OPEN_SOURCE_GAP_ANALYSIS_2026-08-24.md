...
| P1     | P1-10 | 有界队列、模块边界与增量 conformance   | 🟡 部分完成 | Remote AI/Browser diagnostics、LLM/progress、telemetry、renderer backlog 与 RSS ingestion 已补 admission/bytes/concurrency/retention/teardown；旧 API interceptor 已退役；详见 §12.22～12.24，`ef1e8dc24c` 六门禁全绿                                                                                   | 决定 RSS/Email 未接线 adapter 接入或退役；接入 Email 前关闭 client/timer Map、动态 update column 与无默认上限查询。继续 live backlog、模块拆分和 crash/recovery/migration matrix; verify production wiring |
