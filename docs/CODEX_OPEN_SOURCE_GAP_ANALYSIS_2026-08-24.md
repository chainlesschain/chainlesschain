...
| P1-3  | 🟡 仓库接线 97%～99%                        | App Server 复用真实 CLI agent loop；固定 Graph capability、Desktop `$team`/Specialized Agents/WorkflowManager canonical+shadow adapter、Cowork/Scheduler result receipt、authority generation/lease/head CAS、旧 Desktop writer read-only；CLI Team local/distributed authority、默认 renderer fixed IPC、durable run binding、无输入恢复、应用重启 hydration、固定审计对账、按入口 cutover ledger、Electron embedded runtime kill/reopen、全部 9 个 durable entry 的 ledger 路由/逐运行 authority 固定及 same-SHA gate 接线已落地 | 打包 Electron 原生 ABI/window process-kill E2E、执行真实逐 store migration/canary/rollback 演练、跨机器长时 soak 及本候选 SHA 三平台真实 provider 矩阵；详见 §6.9.6.7.5～§6.9.6.7.13 |
