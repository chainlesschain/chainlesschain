...
| P1-10 | 部分随版本发布                              | App Server stdio/WebSocket/SDK、Agent IPC、Desktop Cowork AgentPool 与 MCP consent 已有队列、帧、并发、缓存或 admission 上限；接入结构化 `OVERLOADED`、慢消费者断路及 deadline；协议/内核 fixture、跨端 causal conformance、30 分钟 RSS/过载门、CLI `0.166.6` 发布回读及相关三平台测试均已完成                  | 继续普查并有界化其余旧 transport/event/message/tool/metrics backlog，拆分剩余超大模块的 ports/adapters/state machines，并扩展 crash/recovery/migration conformance matrix     |
...
- 本切片没有改变 npm、IDE 或桌面应用版本，也没有形成新的签名安装包，因此无需发布。JetBrains `0.4.102` 已在既有发布运行中上传，公开 API 仍显示 `0.4.100`，剩余事项只有 Marketplace 外部人工审核，不重复上传、不阻塞后续任务。P1-10 整体仍为“部分完成”；下一项已定位为 MCP performance monitor 的无界 latency series/维度 Map，继续按独立提交和精确 SHA 门禁处理。
...
| P1     | P1-10 | 有界队列、模块边界与增量 conformance   | 🟡 部分完成 | App Server/Agent IPC 与 Cowork AgentPool 上限、慢消费者断路、30 分钟 RSS/过载门、CLI `0.166.6` 回读已关闭；`269361d26b` 又为 Desktop MCP consent 增加全局 64/单服务 8 的 admission、`OVERLOADED`、1,024 项缓存、异常清理和三平台 105-test 门禁                                                           | 已关闭 Agent IPC、Cowork AgentPool 与 MCP consent 子项；下一项为 MCP metrics retention，之后继续其余 backlog、模块拆分与 crash/recovery/migration conformance matrix |
