...
| P2     | P2-3  | Rollout 与协作质量 Eval                | 🟡 部分完成 | deterministic multi-seed、故障注入、schedule equivalence 与阈值 gate 已发布；受保护真实模型 producer 已加入同任务集的 `cc exec` 单 Agent control 与 `cc team run` Graph candidate、至少 1,800 秒/三轮 soak、冻结质量/安全/成本/延迟阈值、Linux/Windows/macOS 同 SHA 聚合和 OIDC attestation；详见 §12.66                                                                                                                                                                                                                                                                                                                                                       | 仓库实现与 producer 已闭环；只剩配置真实模型 secret/provider/budget，并在受保护 `main` 的同一精确 SHA 跑出三平台 aggregate success；本地短跑、contract 或部分矩阵不能替代                      |
