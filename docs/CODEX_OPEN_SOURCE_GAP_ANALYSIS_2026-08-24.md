...
| P1     | P1-10 | 有界队列、模块边界与增量 conformance   | 🟡 部分完成 | App Server/Agent IPC、Cowork AgentPool、MCP consent/metrics 与 direct-spawn 旁路子项已关闭；`f64c50bc1a` 又为 MCP HTTP SDK 增加 SSE/RPC admission、UTF-8 body、event/buffer 硬上限、结构化 overload、慢消费者断路及三平台/coverage/综合门禁                                                                                              | 下一项为 MCP SDK `MCPStdioServer` 增加输入行、并发 handler 和 stdout 背压边界；之后继续其余 backlog、模块拆分及 crash/recovery/migration conformance matrix          |
