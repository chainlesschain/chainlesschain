...
| P1-10 | 部分随版本发布                              | App Server stdio/WebSocket/SDK、Agent IPC、Desktop Cowork AgentPool、MCP consent、可选 metrics monitor 与 MCP HTTP/stdio SDK 已有队列、帧、输入行、并发、缓存、retention、连接或 admission 上限；生产 MCP stdio 已清除未引用的 direct-spawn 旧旁路并由静态边界测试锁定 Broker 默认路径；接入结构化 `OVERLOADED`、慢消费者断路及 deadline；RSS/Email production wiring、Email 有界凭据/状态/附件、standalone/Electron embedded signaling，以及 IPFS 内容读写/admission/增量 GC 边界已完成 | 继续全量普查并有界化或删除其余 transport/event/message/tool backlog，拆分剩余超大模块 ports/adapters/state machines，并扩展 crash/recovery/migration conformance matrix；详见 §12.22～12.27 |
| P1-11 | 部分随 CLI 发布                             | Graph 数据来源/信任/敏感度/allowedSinks 传播与审计 declassification；orchestrate webhook 增加 HMAC、时间窗、delivery replay、body/rate cap，并保留可信 origin；MCP consent 缓存键只保留稳定摘要、不再保留原始参数；Desktop 外部 Skill 的 realpath containment、Ed25519 lock、signer pinning、capability manifest、同步导入及 Electron main `require` 旁路已关闭，详见 §12.30                                                                                                             | 外部 Skill 的生产级独立执行器/capability broker（当前未注入即拒绝）、bundled legacy Skill 逐项最小 capability 迁移、所有 webhook vendor 原生签名和全产品统一 egress broker 迁移尚未完成     |
...
- 只有物理位于应用 bundled root 的 package-owned handler 可继续在 Electron main 内加载。managed/workspace/marketplace handler 必须同时满足签名有效、signer 受信和 capability manifest 有效；主进程只把再次校验后的精确 source bytes 交给注入的 isolated executor，不再传可变 host path，也绝不直接 `require`。当前产品尚未注入该生产执行器，因此外部代码型 Skill 默认返回 `CC_SKILL_EXTERNAL_HANDLER_ISOLATION_REQUIRED`，不会以兼容性为由回退到主进程执行。
...
- 本切片只改变 Desktop 内部代码和测试，不改变 protocol、SDK、CLI、IDE 或公开包版本，也没有形成签名 Desktop 安装包，故不发布制品。P1-11 继续保持“部分完成”：外部 Skill 的直接主进程执行漏洞已关闭，但要恢复第三方代码型 Skill 的生产可用性，仍需实现真实独立执行器及 capability broker；同时还需迁移 bundled legacy Skill 的逐项最小 capability、vendor 原生 webhook 签名和全产品统一 egress broker。
...
| P1     | P1-10 | 有界队列、模块边界与增量 conformance   | 🟡 部分完成 | 既有 backlog 有界化与旧 interceptor 退役；RSS/Email production wiring、Email 有界凭据/状态/附件、standalone/Electron embedded signaling，以及 IPFS content transport/admission/增量 GC 边界已完成；详见 §12.22～12.27                                                                                                                                                                                                                                                                                                                            | 继续全量普查并有界化或删除其余 transport/event/message/tool backlog，拆分剩余超大模块 ports/adapters/state machines，并扩展 crash/recovery/migration conformance matrix                     |
| P1     | P1-11 | Skill 供应链、数据来源与选择性网络出口 | 🟡 部分完成 | Graph data lineage/declassification 与 webhook HMAC/replay/body/rate cap 已发布；MCP consent 缓存键只留 SHA-256；Desktop 外部 Skill 已完成 realpath/non-symlink/size 边界、解析与执行 digest 重验、Ed25519 component lock、实际 SPKI signer pin、显式 capability manifest、签名锁同步和 main-process `require` fail-closed；Skill Creator 的 bundled-root 写入及 direct-require 旁路也已关闭，详见 §12.30                                                                                                                                        | 生产级外部 Skill 独立执行器/capability broker（当前 default deny）、bundled legacy Skill 最小 capability 迁移、vendor 原生 webhook 签名、全产品统一 egress broker                           |
