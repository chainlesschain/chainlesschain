...
| P1-3  | 🟡 仓库接线 94%～97%                        | App Server 复用真实 CLI agent loop；固定 Graph capability、Desktop `$team`/Specialized Agents/WorkflowManager canonical+shadow adapter、Cowork/Scheduler result receipt、authority generation/lease/head CAS、旧 Desktop writer read-only；CLI Team local/distributed authority、默认 renderer fixed IPC、durable run binding、无输入恢复、应用重启 hydration、固定审计对账、按入口 cutover ledger、真实磁盘 kill/reopen journey 及 same-SHA gate 接线已落地 | 打包 Electron 原生 ABI/window process-kill E2E、执行逐 store migration/canary/rollback 演练、跨机器长时 soak 及本候选 SHA 三平台真实 provider 矩阵；详见 §6.9.6.7.5～§6.9.6.7.12 |
...
| P1     | P1-3  | canonical Agent Kernel 与 rollout      | 🟡 部分完成 | 真实 CLI loop、固定 App Server Graph capability、Desktop `$team`/Specialized Agents/WorkflowManager canonical+shadow adapter、Cowork/Scheduler receipt-first terminal adapter、authority generation/lease/head CAS、旧 Desktop writer read-only；CLI Team local/distributed canonical dispatch、ready/message/handoff/dynamic/effect、shadow collaboration、cancel/reconcile 与主要 RPO=0 cut point 已落地；`b3f6e87a37` 完成 Desktop durable binding/重启 hydration/固定审计对账，`6681b3e263` 完成按入口持久化 cutover ledger、逐 store RPO=0 与回滚保权门禁；`b364d12922`、`a6d521dd56`、`a00bb9ce6a` 又完成真实磁盘 kill/reopen、distributed journey 与 same-SHA gate 接线，仓库接线约 94%～97% | 打包 Electron 原生 ABI/window process-kill E2E、按 ledger 产出真实逐 store canary/migration/rollback 证据、跨机器 custody/长时 soak 及本候选 SHA 三平台真实 provider 矩阵；剩余约 3～7 人日，详见 §6.9.6.7.5～§6.9.6.7.12 |
