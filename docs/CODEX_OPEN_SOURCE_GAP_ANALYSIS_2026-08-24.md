...
- 本地全仓 `actionlint` 通过，CI integrity 更新后为 36/36；单线程重跑完整 P1-10 根级合同为 81 项中 78 pass、3 个 Unix-only 项在 Windows 按设计 skip。Prettier、Node syntax 和 `git diff --check` 继续作为提交前门禁。远端 PR 仍必须等待该 required context 实际成功，不能用本地结果替代。
