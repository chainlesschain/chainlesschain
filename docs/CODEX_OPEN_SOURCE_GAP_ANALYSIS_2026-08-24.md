...
| P1     | P1-1  | 单一协议 Schema 与多语言 codegen       | 🟡 部分完成 | protocol `0.1.4`、TS/Python SDK `0.2.3` 已发布；严格审批 union 已由主要产品消费；37 个 stream event discriminator 与 envelope 已从单一 schema 生成到 TS/Python/Kotlin/Swift 和 VS Code vendor；JetBrains `0.4.100` 与 iOS RemoteSession 已分别消费生成 Kotlin/Swift event type/envelope | 各事件 payload 级完整生成 union、Android/其余 Kotlin 与其他产品 event consumer、CLI/Desktop/IDE 端到端 causal event-stream conformance |
