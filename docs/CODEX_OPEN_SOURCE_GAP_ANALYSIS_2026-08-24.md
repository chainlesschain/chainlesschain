...
| P1-6  | 核心与真实 child adapter 均已随 CLI 发布    | 已发布核心包含 admitted/delivered/read/processed/dead-letter、去重/背压/因果和 custody handoff；`0.166.2` 把真实 child 的 `team_send/team_receive/team_ack/team_followup` 接入宿主，绑定 holder/task/attempt/lease/fence，并持久化消费回执、幂等键、payload digest 与 causation/correlation | 精确 SHA 三平台 CLI/strict 门禁、OIDC 发布与独立公网回读已通过；接收方离线/长时恢复、idle followup 真正唤醒新 turn、canonical Graph event 投影及 handoff lease 转移矩阵尚未完成 |
