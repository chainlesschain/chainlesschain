...
| P1-3  | 🟡 仓库接线约 99%                           | App Server 复用真实 CLI agent loop；固定 Graph capability、Desktop `$team`/Specialized Agents/WorkflowManager canonical+shadow adapter、Cowork/Scheduler result receipt、authority generation/lease/head CAS、旧 Desktop writer read-only；11 个入口已精确分类为 7 个 durable migrate、2 个 process-local retire、2 个 non-durable disabled；打包 Electron/ASAR/BrowserWindow 与 CLI 外部 kill journey 已按 `(surface, entry, store)` 产出本机 20/20 migration receipt，same-SHA 三平台 store/provider 聚合门及 ledger 消费已接线 | 本候选最终 SHA 的 Linux/Windows/macOS 真实 provider + 60/60 entry-platform-store artifact、真实 shadow/canary/default/read-only 推进、旧 writer 观察窗口及跨机器长时 soak；详见 §6.9.6.7.5～§6.9.6.7.14 |
...
| P1-12 | 持久化切流门禁就绪                          | CLI Team/Cowork/Scheduler/Desktop/Browser 的 machine-readable claims、单一 writer 约束、shadow equivalence、terminal-evidence/cutover gate；11 个入口有唯一 rollout key，逐 store RPO=0、回滚保权和同 SHA 三平台证据由 hash-chain ledger 强制；现有 Graph Agent Real Journey 已接入 ledger 可消费的精确 schema/digest，Browser 明确 non-durable | 尚未产生本候选 SHA 的 Linux/Windows/macOS 真实运行证据，也未完成真实 shadow/canary/default/read-only、完整回滚演练或旧 writer 下线；因此不得将 P1-12 标为完成 |
...
| P1     | P1-3  | canonical Agent Kernel 与 rollout      | 🟡 部分完成 | 真实 CLI loop、固定 App Server Graph capability、Desktop `$team`/Specialized Agents/WorkflowManager canonical+shadow adapter、Cowork/Scheduler receipt-first terminal adapter、authority generation/lease/head CAS、旧 Desktop writer read-only；CLI Team local/distributed canonical dispatch、ready/message/handoff/dynamic/effect、shadow collaboration、cancel/reconcile 与主要 RPO=0 cut point 已落地；`b3f6e87a37` 完成 Desktop durable binding/重启 hydration，`6681b3e263` 完成 entry ledger，`b364d12922`/`a6d521dd56`/`a00bb9ce6a` 完成真实磁盘恢复、distributed journey 与 gate 接线，`dd658b7a3b`～`8c00d173c3` 完成生产 resolver/逐运行保权，`8f42bd6686` 又完成 7 migrate + 2 retire + 2 disabled 精确分类、打包 Electron 三入口强杀恢复、20 个 entry-store receipt、60-slot 同 SHA 三平台门及 ledger 消费；仓库接线约 99% | 最终候选 SHA 的 Linux/Windows/macOS 真实 provider + 60/60 store matrix、真实 shadow/canary/default/read-only、旧 writer 观察窗口及跨机器 custody/长时 soak；剩余约 2～5 人日，详见 §6.9.6.7.5～§6.9.6.7.14 |
...
| P1     | P1-12 | Graph Kernel 集成、双写验证与迁移切换  | 🟡 部分完成 | 五 surface/11 入口 machine-readable inventory、唯一 rollout key、run-scoped authority、shadow adapter、migration saga 原语与 receipt cut-point recovery 已落地；hash-chain ledger 强制逐 store RPO=0、回滚保留既有 canonical authority 和 Linux/Windows/macOS 同 SHA 证据；现有 Graph Agent Real Journey 已接入 ledger artifact schema/digest，Browser 标明 non-durable 且默认关闭 | 五 surface 分别产生并通过真实 shadow/canary/default/read-only、完整 projection equivalence、回滚演练、旧 writer/旧 shell 下线与本候选 SHA 三平台真实运行证据；关闭条件见 §6.9.6.7.4 |
