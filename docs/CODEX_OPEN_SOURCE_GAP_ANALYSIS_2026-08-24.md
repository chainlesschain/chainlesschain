...
| P1-10 | 部分随版本发布                              | App Server stdio/WebSocket/SDK、Agent IPC、Desktop Cowork AgentPool、MCP consent、可选 metrics monitor 与 MCP HTTP/stdio SDK 已有队列、帧、输入行、并发、缓存、retention、连接或 admission 上限；生产 MCP stdio 已清除未引用的 direct-spawn 旧旁路并由静态边界测试锁定 Broker 默认路径；接入结构化 `OVERLOADED`、慢消费者断路及 deadline；RSS/Email production wiring、Email 有界凭据/状态/附件、signaling、IPFS，以及 native Realtime/Yjs retained/recovery、Federated/Social collab transport、awareness/document state、production Gossip 与 Mesh retained state 已收口 | 继续全量普查并有界化或删除其余 live transport/event/message/tool backlog，拆分剩余超大模块 ports/adapters/state machines，并扩展真实多主机 crash/recovery/migration conformance matrix；详见 §12.22～12.27、§12.43～12.48 |
| P1-11 | 部分随 CLI 发布                             | Graph 数据来源/信任/敏感度/allowedSinks 传播与审计 declassification；orchestrate webhook 已有 HMAC、时间窗、delivery replay、body/rate cap，并在仓库中补齐 DingTalk、Feishu、WeCom vendor 原生验签及密文解密且保留可信 origin；MCP consent 缓存键只保留稳定摘要、不再保留原始参数；Desktop 外部 Skill 已完成签名/containment、一次性强沙箱 worker、有界协议与默认拒绝 capability broker；bundled executable Skill 已完成 145/145 最小 capability/摘要审计，14/14 个 HTTP Skill 及 `network-diagnostics` 多协议出口已全部接入 branded host broker；15/15 个直接 environment handler 已收敛；15 个 shell process handler 已分两批改为 branded、无 shell 的固定 argv authority，详见 §12.30～12.42 | 剩余 14 个直接 `child_process` handler、2 个媒体执行面、filesystem 特权面、生产 SecretStore/配置/network/process authority 与 approval 接线和三平台签名 Desktop 验收尚未完成 |
...
### 12.48 P1-10 Mesh retained state 与模拟 transport fail-closed（2026-08-28）

生产提交 `7dfa449951f83f98e96933a75e4bd3033361558e` 处理 `social-initializer` 默认创建、但没有真实 BLE/Wi-Fi Direct adapter 的 `MeshSocial`。该实例此前没有被 main 持有和退出回收，peer registry 只有 TTL 没有容量，离线队列只有条目数没有字节预算，metadata/message/queue data 直接保留调用方对象：
...
- metadata、direct/broadcast payload 和 offline queue data 在保留前 JSON clone，防止调用方事后修改 retained state；peer cleanup 使用可注入时钟，discovery timer `unref` 且只能保留一个。destroy 幂等清除 timer、peer、queue、字节账本和 listener，销毁后 initialize/discovery/query/send/register 等路径均由 lifecycle fence 拒绝；main 现在持有 startup 实例并在退出时调用同一实例的 destroy。
- 该模块仍只是模拟 transport，因此本次刻意不把 `meshSocial` 注入 `registerFutureIPC`，避免 renderer 收到伪造的“已发送”成功。selector 把 boundary/manager/future IPC/bootstrap/main/phase 映射到 Mesh 边界和 phase 契约；回归同时锁定 shutdown ownership 与“模拟 transport 不得暴露到 IPC”的负向条件。
- 本地 Mesh + IPC phase 为 2 files、55/55，最终 selector 等价组合为 25 files、532/532，selector integrity 33/33；TypeScript、main build、Node syntax、Prettier、ESLint 与 diff check 通过。包含生产提交的精确验证头 `5edef7544b6dbb328156dd2c41fcdb61600eb4d6` 已由 [CI Tests](https://github.com/chainlesschain/chainlesschain/actions/runs/33166486028)、[Full Test Automation](https://github.com/chainlesschain/chainlesschain/actions/runs/33166486068)、[Code Quality & Security](https://github.com/chainlesschain/chainlesschain/actions/runs/33166486046)、[E2E Tests](https://github.com/chainlesschain/chainlesschain/actions/runs/33166486027)、[Documentation Generation](https://github.com/chainlesschain/chainlesschain/actions/runs/33166486067) 与 [macOS MCP launcher gate](https://github.com/chainlesschain/chainlesschain/actions/runs/33166486056) 全部验证成功；Linux/macOS/Windows selector、Linux/Windows 综合套件和完整 unit/coverage 均直接通过，没有依赖 fallback 或诊断重跑替代。本切片没有改变公开包或签名桌面制品版本，因此无需发布。
...
| P1     | P1-10 | 有界队列、模块边界与增量 conformance   | 🟡 部分完成 | 既有 backlog 有界化与旧 interceptor 退役；RSS/Email production wiring、Email 有界凭据/状态/附件、signaling、IPFS，以及 native Realtime/Yjs retained/recovery、Federated/Social collab transport、awareness/document state、production Gossip 与 Mesh retained state 已收口；详见 §12.22～12.27、§12.43～12.48                                                                                                                                                                                                                                                              | 继续全量普查并有界化或删除其余 live transport/event/message/tool backlog，拆分剩余超大模块 ports/adapters/state machines，并扩展真实多主机 crash/recovery/migration conformance matrix     |
| P1     | P1-11 | Skill 供应链、数据来源与选择性网络出口 | 🟡 部分完成 | Graph data lineage/declassification 与 webhook HMAC/replay/body/rate cap 已发布；CLI 仓库已补齐 DingTalk/Feishu/WeCom vendor 原生验签、密文解密、receiver binding 和显式无回退 auth mode；MCP consent 缓存键只留 SHA-256；Desktop 外部 Skill 已完成签名/containment、精确 source snapshot、一次性 ProcessExecutionBroker 强沙箱 worker、Node permission + VM、固定有界协议、超时强杀与默认拒绝 capability broker；bundled executable Skill 已完成 145/145 精确 capability/catalog/摘要审计；14/14 个 HTTP Skill、多协议网络诊断、15/15 environment handler 和两批共 15 个 shell process handler 已收敛为 branded host authority，详见 §12.30～12.42 | 剩余 14 个直接 `child_process` handler、2 个媒体执行面、84 个 filesystem reader、22 个 filesystem writer、生产 host authority/approval 接线，以及三平台签名 Desktop 验收 |
| P1     | P1-12 | Graph Kernel 集成、双写验证与迁移切换  | 🟡 部分完成 | 五 surface/23 入口 machine-readable inventory、唯一 rollout key、run-scoped authority、shadow adapter、migration saga、receipt cut-point recovery、360 个 retire mutation 直接 guard、replacement/historical-read-only 契约与 disabled direct-engine gate 已落地；hash-chain ledger 强制逐 store RPO=0、回滚保留既有 canonical authority；`2932aad32c` 的 Linux/Windows/macOS 真实 provider 与 60/60 同 SHA 证据通过；Browser 标明 non-durable 且默认关闭                                                                                        | 五 surface 分别完成真实 shadow/canary/default/read-only、完整 projection equivalence、13 retire 替代/零旧 writer 观察、回滚演练与旧 shell 下线；关闭条件见 §6.9.6.7.4 与 §6.9.6.7.18        |
