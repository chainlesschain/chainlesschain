...
| P1-10 | 部分随版本发布                              | App Server stdio/WebSocket/SDK 与 Agent IPC 已有全局/per-agent 队列、帧、并发和 deadline 上限；接入 `OVERLOADED + retry_after_ms`、慢消费者断路及旧入口 admission；协议/内核 fixture、跨端 causal conformance、30 分钟 RSS/过载门和 CLI `0.166.6` 精确 SHA 三平台发布/公网回读均已完成                            | 继续普查并有界化其余旧 transport/event/message/tool backlog，拆分剩余超大模块的 ports/adapters/state machines，并扩展 crash/recovery/migration conformance matrix             |
...
| P1     | P1-10 | 有界队列、模块边界与增量 conformance   | 🟡 部分完成 | `0.166.5` 已完成 App Server stdio/WebSocket/SDK 上限、慢消费者断路、37-event causal conformance 与精确 SHA 的 30 分钟 RSS/过载门；`0.166.6@f2a249bf3d` 又关闭 Agent IPC 全局/per-agent 上限、旧入口 backlog admission，并完成 CLI CI、Strict Sandbox、OIDC 发布和独立 registry/provenance 回读          | Agent IPC 子项已关闭；继续普查其余旧 transport/event/message/tool backlog，拆分剩余超大模块，并扩展 crash/recovery/migration conformance matrix                |
