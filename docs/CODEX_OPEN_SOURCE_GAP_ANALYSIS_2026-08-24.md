...
| P1-9  | 仓库实现与权威矩阵闭环                      | 既有 HumanTask、统一 policy event、exact turn/session grant 与 Desktop/IDE settlement CAS 之外，Android/iOS/Web 已接入可审阅持久 grant、共享 fixture 和单赢家结算；Desktop App Server 以 durable HumanTask 提供 quorum/职责分离产品面，等待释放 Agent slot，actor 由 main authority 派生，重启恢复和 revision/attempt/operation digest 绑定均失败关闭；移动端精确 SHA 与 Desktop/Graph 三平台精确 SHA 权威矩阵全部成功，详见 §12.52、§12.54～12.56、§12.59、§12.62、§12.64                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | 仓库内代码与确定性/三平台验证已关闭；本轮 CLI runtime 字节尚未形成新的 npm 发布，真实全产品 authoritative adapter 切换与旧 writer 下线统一归 P1-12                                                                          |
...
> **2026-08-30 实际完成情况：仓库实现差距已关闭，正式 production close 等待外部 exact-SHA 证据。** `packages/context-memory-kernel` 已成为独立 canonical schema/runtime；CLI 与 Desktop 默认 canonical，旧 mutation API 失败关闭，VS Code/JetBrains 为只读 canonical projection。实现提交依次为 `b126f0e7e9`（Kernel/schema/adapters）、`207af1a327`（production default 与 legacy fence）和 `64c7be019e`（物理 purge、崩溃恢复与跨端 conformance），后续发布门与文档属于同一候选分支。仓库完成不等于已获得 GitHub 托管三平台与 30 分钟 soak 的正式发布凭证。

| 交付层                | 当前状态                            | 可验证证据                                                                                                                                   | 尚未完成                                                        |
| --------------------- | ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------- |
| 用户文档与信息架构    | **100%** | 用户总览、命令页与模块 108 已更新到 canonical/legacy-fenced 状态 | 最终发布后补 attested run/receipt 链接 |
...
| CLI/Desktop cutover | **100% repository** | provider 前强制 ContextPlan；canonical durable memory/JSONL compaction；Desktop main 默认 canonical；所有已清点旧 writer 为 `legacy_read_only` | 等待最终候选三平台 Actions |
| IDE/SDK/App Server | **100% repository** | 固定七个方法、additive lifecycle events、VS/JetBrains bounded projection、TS/Python generated contract，共用单一 TSV fixture | 等待 JetBrains JDK 21 与 SDK CI matrix |
| 删除/隐私/恢复 | **100% first-party online stores** | 最小 tombstone、旧 SQLite/session-core 物理 purge、显式 purge receipt、partial/reconcile、进程终止重启与离线回灌 fence | 外部备份保留窗口仍由部署 policy 定义 |
| 发布关闭 | **门已实现，证据待运行** | 26 槽 requirements、三平台 CI、30 分钟 soak、evidence assembler 和 production-close attestation workflow | 必须在最终候选 SHA 上运行，不能用本地结果替代 |
...
因此 P1-9 的**仓库实现、移动端权威构建与 Desktop/Graph 三平台矩阵已经闭环**。新的 CLI runtime npm 发布属于尚未执行的外部发布动作；全产品 authoritative adapter rollout、旧 writer 观察与退役继续由 P1-12 统一管理，不再为 P1-9 建设第二套审批内核。

### 12.65 P1-11 受保护签名 Desktop producer 仓库闭环（2026-08-29）

本增量关闭 §12.61 保留的仓库侧 producer 缺口，但继续把尚未发生的外部签名矩阵标为未完成：

- 新增受保护的 `Desktop Signed Skill Platform` producer，三平台签名 job 固定使用 `desktop-signed-qualification` GitHub Environment，且只接受 `main` 或 `v*` 标签当前 head 的完整 40 位 SHA；输入 SHA 与 Actions source SHA 不同、从普通分支运行、任一 secret 缺失或任一平台失败都会停止。Linux 使用 GitHub OIDC keyless Sigstore 并回验 transparency log/证书 workflow claims；Windows 要求有效 Authenticode signer 与独立 timestamp certificate；macOS 开启 hardened runtime/entitlements，要求 Developer ID、notary acceptance、stapling、`codesign --strict` 与 Gatekeeper assessment。每个平台在旅程全部通过后才对精确 installer bytes 生成 GitHub OIDC provenance attestation，并将签名安装包独立保留 30 天。
...
| P1     | P1-9  | durable HumanTask 与统一策略事件       | 🟢 仓库闭环 | protocol `0.1.5` 与 TS/Python SDK `0.2.4` 已公开；Desktop/VS Code/JetBrains/Android/iOS/Web 均有可审阅 exact grant 与单赢家 settlement，跨产品 fixture 已消费；Desktop App Server/Graph debugger 提供 durable quorum/职责分离 HumanTask 产品面，main-owned actor、精确 digest binding、重启恢复与等待释放 slot 均失败关闭；移动端精确 SHA 和 Linux/Windows/macOS Graph/真实 Electron/真实 provider 聚合门全部成功；详见 §12.52、§12.54～12.56、§12.59、§12.62、§12.64                                                                                                                                                                                                     | 新 CLI App Server runtime 字节尚未另发 npm 版本；全产品 authoritative adapter rollout、旧 writer 观察和下线统一归 P1-12                                                                                                        |
| P1     | P1-10 | 有界队列、模块边界与增量 conformance   | 🟡 部分完成 | 仓库实现与确定性本地验证已闭环：P2P、Device Sync、Connection Pool、native collaboration、MTC、DID/Organization/Sync 及 29 个 active social manager 的 backlog、retained state、listener、timer、task 和 teardown 已严格有界；拆出 boundary/registry/lifecycle owner；fail-closed conformance inventory/evidence gate 已固定跨产品、crash/recovery、migration、双节点与 soak 条件；本地最终 selector 52/52 files、1,074 passed、1 skip，type-check、ESLint 与 `build:main` 通过；详见 §12.22～12.27、§12.43～12.50、§12.63                                                                                                                                                 | 只剩权威外部环境的同 SHA 物理多主机、packaged Electron crash/recovery、跨版本 migration 与长时 bounded-runtime soak；本机模拟或缺证据不得关闭编号                                                                              |
| P1     | P1-11 | Skill 供应链、数据来源与选择性网络出口 | 🟡 部分完成 | Graph data lineage/declassification、webhook、外部 Skill containment、bundled capability/authority、加密产品凭据与审批接线均已闭环；受保护 producer 已加入 exact protected SHA、Linux OIDC Sigstore、Windows Authenticode timestamp、macOS Developer ID/notarization/stapling、三平台 fresh install、安装后 packaged ASAR 启动和七项真实 Skill authority 旅程；所有 signature/install/launch/journey receipt 绑定同 installer/commit/same-run challenge 并由 aggregate 重算、验证和 OIDC attestation；详见 §12.30～12.42、§12.51、§12.53、§12.57～12.58、§12.60～12.61、§12.65                                                                                            | 仓库实现与 producer 已闭环；只剩配置 Windows/macOS 签名/公证及 Google OAuth/Notion/Tavily Actions secrets，并在受保护 `main` 的同一精确 SHA 跑出 Linux/Windows/macOS aggregate success；本地、未签名或仅 contract 回归不能替代 |
| P1     | P1-12 | Graph Kernel 集成、双写验证与迁移切换  | 🟡 部分完成 | 五 surface/23 入口 inventory、run-scoped authority、shadow/migration/recovery、360 个 retire mutation guard 与 replacement/read-only contract 已落地；三平台真实 provider 与 60/60 store 证据通过；§6.9.6.7.25 新增全 20 durable entry、九类 projection、三种 rollback、全部旧 writer/mutation、3 disabled Browser entry 与 OIDC producer provenance 的 fail-closed production aggregate gate及受保护 workflow                                                                                                                                                                                                                                                            | 在真实 `graph-kernel-production` 环境生成 staged rollout/观察 evidence，并让受保护 workflow 对同一精确 SHA 产出通过 receipt；缺少该 artifact 时不得改绿                                                                        |
