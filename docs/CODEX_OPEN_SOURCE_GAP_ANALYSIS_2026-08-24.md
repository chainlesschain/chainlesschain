...
| P1-11 | 部分随 CLI 发布                             | Graph 数据来源/信任/敏感度/allowedSinks 传播与审计 declassification；orchestrate webhook 已有 HMAC、时间窗、delivery replay、body/rate cap，并在仓库中补齐 DingTalk、Feishu、WeCom vendor 原生验签及密文解密且保留可信 origin；MCP consent 缓存键只保留稳定摘要、不再保留原始参数；Desktop 外部 Skill 已完成签名/containment、一次性强沙箱 worker、有界协议与默认拒绝 capability broker；bundled executable Skill 已完成 145/145 最小 capability/摘要审计，14/14 个 HTTP Skill（10 个固定公网、4 个动态 HTTPS，其中 2 个另含回环模型服务）及 `network-diagnostics` 的 DNS/TCP/ICMP/traceroute 已全部接入 branded host broker，详见 §12.30～12.39 | 剩余 filesystem/env/process 特权面接入 SecretStore/路径 authority/统一 broker、生产 host authority/approval 接线和三平台签名 Desktop 验收尚未完成 |
