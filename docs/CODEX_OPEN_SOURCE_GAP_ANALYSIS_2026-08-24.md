...
截至 2026-08-24 的最新落地状态，Agent 平台发布提交 `40354eb432281c28ed266f2dc6d1458764eb536d` 已通过精确 SHA 的 Linux/Windows/macOS CLI 与 strict sandbox 权威矩阵，以及 Python 3.10/3.12/3.13 SDK conformance；随后已公开发布 `chainlesschain@0.166.0`、`@chainlesschain/agent-sdk@0.2.0` 和 `chainlesschain-agent-sdk==0.2.0`，并完成 npm/PyPI 公网回读。后续又以精确提交 `882c3c9d7f18ee0cc0c766a2b865f8234f7dc4ed` 独立公开发布 `@chainlesschain/agent-protocol@0.1.0`：包入口、schema、compatibility helper、Kotlin/Swift 生成源均已纳入 allowlist tarball，Linux/Windows/macOS 协议矩阵、tokenless GitHub OIDC 发布、签名 provenance 和未登录公网安装回读全部通过。TS/Python SDK 继续分发自身生成绑定且没有把协议包声明为运行时依赖，因此本次无需跟随 bump SDK 版本。

发布后增量继续完成了 Graph Compiler/Kernel 的 inverse-effect compensation：补偿 handler 在编译期与正向图隔离，失败、部分成功或取消后显式启动，按正向成功节点的逆拓扑顺序串行执行，并以 committed receipt 关联原 effect；补偿失败稳定进入 `reconciliation_required`。同时新增 state commit、dispatch/lease、message admission、effect receipt 与 processed/ACK 五类“落盘后、返回前崩溃”故障注入，以及补偿 lineage/critical-path Trace 投影。该增量的精确代码提交 `161d68167a712cb90d59a556428f54e4284d70a5` 已通过 Linux/Windows/macOS CLI CI 与 Strict Sandbox 权威矩阵，但尚未创建新版本标签或执行真实 provider journey，因此不计入 `0.166.0` 的已发布能力。
...
| ----- | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P1-1  | 核心与独立协议包均已发布                            | canonical JSON Schema；TS/Python/Kotlin/Swift 确定性 codegen；运行时 validator；冻结 v1 baseline 和 breaking-change 检查；TS/Python SDK 及 `@chainlesschain/agent-protocol@0.1.0` 已公开发布                                                                                   | 旧客户端手工事件 union 尚未全部迁移；CLI/Desktop/IDE 同 fixture 的三端 conformance 尚未完成                                                                                                                                        |
...
| P1-4  | 核心随 CLI 已发布；结构化控制流增量 CI 已过、待版本 | typed/versioned Graph IR、digest、N/N-1 upcast/dry-run；effect-before-compile 校验；补偿隔离；新增多节点 bounded loop 的 iteration frame/确定性 attempt、证据绑定 decision、cap fail-closed 与恢复，以及 digest-pinned subgraph 父子 GraphRun、深度/递归 guard、恢复和取消级联 | effectful loop 的 iteration-scoped receipt/idempotency/compensation 仍 fail closed；subgraph typed input/output 映射与预算切片、生产 adapter 切换、跨真实已发布 schema 的完整 migration corpus 尚未完成；增量尚未进入新标签/包版本 |
...
| P1-8  | 核心随 CLI 已发布；补偿 CI 已过、待版本发布         | durable Effect/receipt/unknown-outcome/reconcile、取消 fencing、artifact provenance、append-only event、确定性 trace reducer/time travel/diff；发布后增量增加可恢复的逆依赖补偿执行器和五类核心 durable cut-point fault injection                                              | CLI Graph Kernel 的核心切点与三平台 CI 已闭环；全产品/跨进程 outbox-inbox 切点、长时恢复矩阵尚未完成                                                                                                                               |
...
| P2-2 | CLI 核心已发布；结构化投影增量 CI 已过、待版本 | `cc team graph inspect/diff/eval` 可从持久事件生成 Agent Tree、Task Graph、Artifact/Message/Effect/Timeline、critical path、blocked root 和 time-travel；发布后增量增加补偿 lineage、iteration frame/attempt path 与 subgraph child-run graph，并保持正向 critical path 语义 | Desktop Team Monitor 的交互式 topology/timeline UI 尚未接入；Trace 增量尚未进入新标签/包版本           |
...
发布后 Graph 增量又完成以下回归，并已形成精确代码 SHA 的远端基础门禁证据，但尚未创建新版本发布：
...
本次已经完成同一精确 SHA 的 Linux/Windows/macOS CLI 与 strict sandbox 基础发布矩阵，不再将“三操作系统发布矩阵未运行”列为限制。但仍没有执行完整桌面 E2E、30 分钟 overload/fairness soak、真实模型多智能体旅程、全产品 crash/recovery conformance 或渗透测试；`graph-agent-real-journey.yml` 尚未在真实 provider secret 下得到 Linux/Windows/macOS 聚合 job 全绿。因此 P1-12 的 authoritative 产品切换与 P2-6 的真实旅程仍保持未关闭。`@chainlesschain/agent-protocol@0.1.0` 已随后通过独立三平台矩阵与 tokenless OIDC 正式发布；这关闭了协议 npm 包公开发布边界，但不等于旧客户端 union 迁移、跨端 conformance 或 SDK 运行时依赖切换已经完成。
