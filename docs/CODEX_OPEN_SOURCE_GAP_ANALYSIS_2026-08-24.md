...
| P1-4  | 核心随 CLI 已发布；补偿待发布 | typed/versioned Graph IR、digest、N/N-1 upcast/dry-run；effect-before-compile 的引用/环/端口/能力/预算/write-scope/loop/subgraph/Trigger/Region 校验；发布后增量增加补偿声明冲突、effect class、handler 独占与正向图隔离检查，并补充 N-1 dry-run/upcast 失败 fixture | loop/subgraph 的完整生产执行语义与跨真实已发布 schema 版本的完整 migration corpus 尚未完成；补偿增量尚待新精确 SHA 的权威 CI |
...
| P1-8  | 核心随 CLI 已发布；补偿待发布 | durable Effect/receipt/unknown-outcome/reconcile、取消 fencing、artifact provenance、append-only event、确定性 trace reducer/time travel/diff；发布后增量增加可恢复的逆依赖补偿执行器和五类核心 durable cut-point fault injection                                    | CLI Graph Kernel 的核心切点已本地闭环；全产品/跨进程 outbox-inbox 切点、长时恢复矩阵和新精确 SHA 权威 CI 尚未完成            |
...
- 本地定向验证：`graph-kernel-compiler/runtime/fault-injection/observability/adapters` 共 5 个文件、48 项测试通过；本轮修改的 Graph JS 源码与测试 ESLint 通过。真实发布结论必须等待该增量提交后的精确 SHA Linux/Windows/macOS Actions，不能沿用 `40354eb432...` 的结果。
...
| P2-2 | CLI 核心已发布；补偿投影待发布 | `cc team graph inspect/diff/eval` 可从持久事件生成 Agent Tree、Task Graph、Artifact/Message/Effect/Timeline、critical path、blocked root 和 time-travel；发布后 Trace 增量增加补偿计划、source/handler lineage，并保持正向 critical path 语义 | Desktop Team Monitor 的交互式 topology/timeline UI 尚未接入；Trace 增量尚待新精确 SHA CI               |
...
发布后 Graph 增量又完成以下本地回归，但尚未形成新的远端发布证据：
...
- 本轮修改的 Graph JS 源码和测试通过 ESLint；[`graph-agent-real-journey.yml`](../.github/workflows/graph-agent-real-journey.yml) 已把 fault-injection 文件加入三平台确定性前置门禁。该 workflow 仍需在增量提交和真实 provider secret 下运行，故此处只记录“本地完成、CI 待验收”。
