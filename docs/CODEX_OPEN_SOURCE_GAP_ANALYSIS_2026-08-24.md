...
> 最新实时 Team 消息源码候选：`chainlesschain@0.166.2`（CLI-only，待本提交精确 SHA 的 `CLI CI` 与 `CLI Strict Sandbox` 三平台矩阵；尚未发布）
...
在已发布的 `0.166.1` 基线之后，`chainlesschain@0.166.2` CLI-only 源码候选补上了真实 `cc team --agent` child 的实时 `team_send / team_receive / team_ack / team_followup` 宿主工具、attempt/lease/fence 绑定的私有本地桥、at-least-once 回执与幂等消费、`--state` mutation checkpoint，以及禁止嵌套 subagent 继承父 attempt 通道的隔离。该候选尚未通过精确 SHA 的远端三平台门禁，也尚未发布；接收方离线/长时重启矩阵、idle target 的真实 turn wake、canonical Graph message/handoff 投影和 custody lease 转移仍开放，因此 P1-6 仍不能标为完成。
...
| P1-6  | 核心已发布；真实 child adapter 为 `0.166.2` 源码候选 | 已发布核心包含 admitted/delivered/read/processed/dead-letter、去重/背压/因果和 custody handoff；本次候选把真实 child 的 `team_send/team_receive/team_ack/team_followup` 接入宿主，绑定 holder/task/attempt/lease/fence，并持久化消费回执、幂等键、payload digest 与 causation/correlation | 候选尚待精确 SHA 三平台门禁和 npm 发布；接收方离线/长时恢复、idle followup 真正唤醒新 turn、canonical Graph event 投影及 handoff lease 转移矩阵尚未完成                       |
...
截至 2026-08-25，生产 `cc team --agent` adapter 的 prompt-only 断桥已在 `chainlesschain@0.166.2` 源码候选中补上。父进程为每个真实 child 建立随机 endpoint/token 的私有本地 [`TeamMessageBridge`](../packages/cli/src/lib/agent-team/team-message-bridge.js)，child headless runner 直接挂载宿主持有的 [`team_send / team_receive / team_ack / team_followup`](../packages/cli/src/lib/agent-team/team-message-tools.js)，不会启动另一个 MCP executable，也不会扩大现有 MCP trust surface。每次调用都重新核对 TeamRunner 当前 holder、task、attempt、lease 与 fencing token；凭据不进入 prompt，工具标为 `inheritable:false`，嵌套 subagent 会由 [`subagent-inheritance.js`](../packages/cli/src/lib/subagent-inheritance.js) 剔除该通道。

[`TeamMailbox`](../packages/cli/src/lib/agent-team/team-mailbox.js) snapshot 已升级到 v3：实时 receive 不推进 processed cursor，未终态消息会 at-least-once 重投；ACK 区分 read/processed/dead-letter，稳定 consumer key 冲突失败关闭，out-of-order terminal receipt 不跳过更早消息；send/followup 要求稳定 message id 并保存 payload digest、causation/correlation 与 sender attempt。`--state` 下每次 send/receive/ack/followup mutation 都走 team state 原子 checkpoint；未使用 `--state` 时 prompt 明确声明 process-local。followup 当前只如实返回 `target_active` 或 `queued_until_target_turn`，不会虚构已启动一个 idle turn。

这仍只关闭了真实 CLI child 的 mid-turn adapter 源码缺口。接收方跨进程离线、父进程重启与长时恢复矩阵，idle target 的真实 wake/新 turn 调度，SessionMessageFabric/Graph message 的统一 durable projection，以及 handoff custody 与 AssignmentAttempt/lease 的原子转移仍未完成；在这些边界通过前 P1-6 继续开放。

建议：

1. 已完成真实 CLI child 的 holder/attempt/lease-bound `send / receive / ack / followup` 宿主工具与有界本地桥；下一步把 TeamMailbox v3 receipt/idempotency 语义投影到更成熟的 [`session-message-fabric.js`](../packages/cli/src/lib/session-message-fabric.js#L1) / canonical Graph message 账本，并补 TTL、跨进程 rate limit 和 restart recovery。
...
3. 已在 TeamMailbox/真实 child adapter 明确采用 at-least-once delivery + 幂等 consumer，不宣称 exactly-once，并区分 admitted/delivered/read/processed/dead-letter，记录 causation/correlation、sender attempt/lease 与 payload digest。仍需把相同契约合并到 SessionMessageFabric/Graph Kernel，补 conversation/revision、processed-before-ACK 跨进程恢复、poison message 长时重试及跨 channel 重排矩阵。
...
6. 已区分 queue 与 wake intent：`team_send` 只排队，`team_followup` 额外返回目标活跃或等待未来 turn；尚未实现 idle target 的真实 turn admission/wake。Codex V2 将二者映射为 [`QueueOnly / TriggerTurn`](https://github.com/openai/codex/blob/479c8c8924eaafdeb56e86154cd19ff0805839e4/codex-rs/core/src/tools/handlers/multi_agents_v2/message_tool.rs#L1-L23)，但这里不从枚举名推导 durable/reliable 保证；完成通知也不应无条件唤醒，以限制消息风暴和意外重入。
...
在 `0.166.1` 发布基线之后，P1-6 真实 child 消息 adapter 的 `0.166.2` CLI-only 源码候选又完成以下增量：
...
- 候选仅修改 CLI；Agent Protocol 与 TS/Python SDK 内容和版本均不变。`chainlesschain@0.166.2` 必须在提交后绑定精确 SHA，通过 Linux/Windows/macOS `CLI CI` 与 `CLI Strict Sandbox`，才可创建不可变 npm 标签和记录发布证据。
