...
这关闭了 P1-4/P1-8 在 CLI Graph Kernel、协议与生成绑定上的本轮实现/发包边界，但不等于 canonical Graph Kernel 已成为全产品唯一 authoritative runtime。CLI Team/Cowork/Scheduler、Desktop/Browser 的生产 adapter 切换、旧 writer 下线、真实 provider journey、跨进程长时恢复和 30 分钟 overload/消息可靠性 soak 仍未完成，因此 P1-12 与 P2-6 继续保持开放。P1-5 的仓库级 dependency/scope priority donation、aging 和 3 倍 queue-wait SLO 正式门已在后续精确 SHA 三平台矩阵闭环，详见 6.9.6.7.21。
...
| P1-4  | 核心已发布；生产迁移凭据候选待矩阵         | typed/versioned Graph IR、digest、effect-before-compile；多节点 bounded loop；digest-pinned 父子 GraphRun；typed input/output mapping；durable 子图预算预留/实际结算；iteration-scoped effect/receipt/compensation；内置 v0→v1 upcaster、冻结 N/N-1 corpus、备份摘要与回滚恢复；`37fb2d96c2` 又把 N-1 原始 backup、rollback digest 和 replay 校验贯穿生产 GraphRun/App Server crash-resume 快照                                                                                                                                                                                                                                                                                                                                                                                  | 仓库语义与真实运行中 definition migration 候选已闭环；新增提交仍需精确 SHA 三平台矩阵。CLI Team/Cowork/Scheduler 与 Desktop/Browser 的 authority 切换、shadow equivalence 和旧 writer 下线统一归 P1-12，不再重复作为 P1-4 缺口 |
...
| P1-9  | 公开结构化审批增量已发布                    | 可认领/恢复 HumanTask、quorum/职责分离与 cancel/decision CAS；canonical ApprovalDecision、binding/requested permissions、N-1 boolean 兼容已公开到 TS/Python SDK 与 App Server；CLI 已实现 exact turn/session grant；VS Code、Desktop 与 JetBrains 已提供可审阅、可撤销的 exact turn/session grant；仓库新增统一 hook/tool `policy_decision` 投影和 Desktop/WS settlement CAS，并以同一产品无关 fixture 覆盖 Graph 全部 7 个及 Desktop/VS Code/JetBrains 各自适用的 4 个 approval-vs-cancel/race/restart 场景；VS Code 与 JetBrains transport card 均有本地 CAS、失败回滚和重启失效语义，详见 §12.52、§12.54～12.56、§12.59、§12.62 | Android/iOS/Web 的可审阅持久 grant UI 与共享 fixture 消费、多人审批产品面，以及本轮新协议的精确 SHA 发布尚未完成 |
| P1-10 | 部分随版本发布                              | App Server stdio/WebSocket/SDK、Agent IPC、Desktop Cowork AgentPool、MCP consent、可选 metrics monitor 与 MCP HTTP/stdio SDK 已有队列、帧、输入行、并发、缓存、retention、连接或 admission 上限；生产 MCP stdio 已清除未引用的 direct-spawn 旧旁路并由静态边界测试锁定 Broker 默认路径；接入结构化 `OVERLOADED`、慢消费者断路及 deadline；RSS/Email production wiring、Email 有界凭据/状态/附件、signaling、IPFS，以及 native Realtime/Yjs retained/recovery、Federated/Social collab transport、awareness/document state、production Gossip 与 dormant Mesh adapter retained state 已收口；50 个 social initializer 已分为 29 个 startup 与 21 个 dormant，未接线 manager 不再默认创建；Deep Link、MTC/Gossip wiring 及 Friend/Post/Community/Channel manager 的 P2P source listener 已收敛为显式所有权，active social manager、Mobile/Remote、P2P/DID teardown 按依赖逆序执行 | 继续全量普查并有界化或删除其余 live transport/event/message/tool backlog，拆分剩余超大模块 ports/adapters/state machines，并扩展真实多主机 crash/recovery/migration conformance matrix；详见 §12.22～12.27、§12.43～12.50 |
...
| P1-12 | 持久化切流候选门禁通过                      | CLI Team/Cowork/Scheduler/Desktop/Browser 的 machine-readable claims、单一 writer 约束、shadow equivalence、terminal-evidence/cutover gate；23 个入口有唯一 rollout key，逐 store RPO=0、回滚保权和同 SHA 三平台证据由 hash-chain ledger 强制；13 retire 有 Graph 替代和历史只读契约，3 个 Browser/Remote 入口明确 non-durable + disabled；精确候选三平台真实运行及 60/60 聚合门已通过                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | 仍未完成真实 shadow/canary/default/read-only、完整回滚演练、替代入口可达性观察或旧 writer 下线；因此不得把严格 production rollout 标为完成                                                                                |
...
这关闭了 P1-4 中“真实运行中 definition 迁移演练”的仓库实现缺口；CLI Team/Cowork/Scheduler、Desktop/Browser 的 authority 切换、shadow equivalence 和旧 writer 下线仍属于 P1-12，不能继续挂在 P1-4 上重复计算。新提交尚未取得精确 SHA 的远端矩阵，也未形成新的不可变 CLI 发布，故本节当前只声明代码候选完成，不声明公开发布完成。
...
| P2-2 | 仓库实现与跨产品回放矩阵已完成 | `cc team graph inspect/diff/eval` 可从持久事件生成 Agent Tree、Task Graph、Artifact/Message/Effect/Timeline、critical path、blocked root 和 time-travel；Desktop 共享调试器已接入 Coding Agent、Workflow Manager 与 Specialized Agents；新增固定 `graph/history` App Server 能力及 Workflow/Specialized Agents IPC，只返回有界、metadata-only 的 durable event/snapshot 历史；CLI/Desktop/VS Code 使用同一故障 fixture 关闭 blocked-root、revision diff 与 time-travel 一致性矩阵，详见 §12.29 | 协议/SDK/CLI 包字节已变化，仍需精确 SHA 三平台门禁与 OIDC 发布后才能按本文“发布边界已关闭”的严格口径改为完成 |
...
- 本切片没有改变公开协议、CLI/SDK/IDE 包或签名 Desktop 制品，因此无需发布。P2-2 继续保持“部分完成”：Desktop 统一 overlay、预算热图和因果下钻已经关闭；剩余项收窄为通过固定 App Server/IPC capability 向每个 Workflow/Specialized GraphRun 提供 durable trace/snapshot 历史，并用同一故障 fixture 完成 CLI、Desktop 与 VS Code 的跨产品 blocked-root/revision-diff/time-travel 回放矩阵。
...
- 本切片改变了 Agent Protocol schema、TS/Python/Kotlin/Swift 生成绑定、TS SDK 固定 client、CLI App Server 与两份 IDE/Desktop vendored SDK 的包字节，因此本地验证不能替代发布门禁。当前不创建版本、不发布 npm/PyPI/IDE/Desktop 制品；P2-2 在仓库实现上已经关闭，但按本文严格状态口径继续保持“部分完成”，直到精确发布提交通过协议、SDK、CLI CI/Strict Sandbox 三平台矩阵和 OIDC 发布/回读。
...
| P1     | P1-3  | canonical Agent Kernel 与 rollout      | 🟡 部分完成 | 真实 CLI loop、固定 App Server Graph capability、Desktop `$team`/Specialized Agents/WorkflowManager canonical+shadow adapter、Cowork/Scheduler receipt-first terminal adapter、authority generation/lease/head CAS、CLI Team local/distributed canonical dispatch 与 RPO=0 cut point 已落地；23-entry inventory（7 migrate + 13 retire + 3 disabled）锁定 360 个直接 guarded mutation、15 条替代边与 32 个历史读方法；精确候选 `2932aad32c` 的三平台真实 provider、打包 Electron、durable recovery 与 60/60 聚合门全部通过；`751c8df089` 又把 retire replacement 三平台可达性、逐 mutation/历史读覆盖及逐 writer 后置零成功观察变为 ledger 强制证据，仓库实现约 99%～100% | 仓库实现/合并候选已闭环；严格 production close 仍需五 surface staged rollout/rollback、真实生成 13 retire qualification/观察 artifact、跨机器 custody/长时 soak，约 2～4 人日；详见 §6.9.6.7.18～§6.9.6.7.19 |
| P1     | P1-4  | versioned typed Graph IR 与 Compiler   | 🟡 候选待矩阵 | typed IR、structured loop/subgraph、budget/effect/compensation、N/N-1 migration 已发布；`37fb2d96c2` 将原始 backup、rollback digest、replay validation 与迁移摘要贯穿生产 GraphRun/App Server durable crash-resume，仓库验收语义已闭环                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | 新提交的精确 SHA 三平台矩阵与不可变 CLI 发布；生产 adapter 切换、shadow equivalence、旧 writer 下线统一归 P1-12                                                                            |
...
| P1     | P1-9  | durable HumanTask 与统一策略事件       | 🟡 部分完成 | HumanTask 核心与 protocol `0.1.3` 已发布；VS Code、Desktop 与 JetBrains 可审阅、撤销 exact turn/session grant；仓库新增统一 `policy_decision@1`、四语言生成绑定、Hook/tool/WS/Desktop 投影与 settlement CAS；共享 fixture 覆盖 Graph 的 7 个 approval-vs-cancel/quorum/职责分离/race/restart 场景及 Desktop/VS Code/JetBrains 各自适用的 4 个场景，VS Code 与 JetBrains transport card 增加本地 CAS、失败回滚及重启失效，公开 HumanTask runtime projection 通过 canonical validator；详见 §12.52、§12.54～12.56、§12.59、§12.62 | Android/iOS/Web 的可审阅持久 grant UI 与共享 fixture 消费、多人审批产品面，以及本轮新增协议/SDK/CLI 的精确 SHA 发布与 registry/provenance 回读 |
| P1     | P1-10 | 有界队列、模块边界与增量 conformance   | 🟡 部分完成 | 既有 backlog 有界化与旧 interceptor 退役；RSS/Email production wiring、Email 有界凭据/状态/附件、signaling、IPFS，以及 native Realtime/Yjs retained/recovery、Federated/Social collab transport、awareness/document state、production Gossip 与 dormant Mesh adapter retained state 已收口；50 个 social initializer 已分为 29 个 startup 与 21 个 dormant，11 个未接线 manager 不再默认创建；Deep Link、MTC/Gossip wiring 及 Friend/Post/Community/Channel manager 的 P2P source listener 已收敛为显式所有权，active social manager、Mobile/Remote、P2P/DID teardown 按依赖逆序执行；详见 §12.22～12.27、§12.43～12.50                             | 继续全量普查并有界化或删除其余 live transport/event/message/tool backlog，拆分剩余超大模块 ports/adapters/state machines，并扩展真实多主机 crash/recovery/migration conformance matrix      |
| P1     | P1-11 | Skill 供应链、数据来源与选择性网络出口 | 🟡 部分完成 | Graph data lineage/declassification 与 webhook HMAC/replay/body/rate cap 已发布；CLI 仓库已补齐 DingTalk/Feishu/WeCom 原生验签、密文解密、receiver binding 和显式无回退 auth mode；MCP consent 缓存键只留 SHA-256；Desktop 外部 Skill 已完成签名/containment、精确 source snapshot、一次性 ProcessExecutionBroker 强沙箱 worker、Node permission + VM、固定有界协议、超时强杀与默认拒绝 capability broker；bundled executable Skill 已完成 145/145 精确 capability/catalog/摘要审计；84/84 个 filesystem reader（含 21 个 writer）、14/14 个 HTTP Skill、直接 environment handler 和 33/33 个 process executor 均已收敛为 branded host authority，raw `fs`/`child_process`/`fluent-ffmpeg` handler 归零；生产 registry 已接入 main-owned workspace、safeStorage-backed LLM/GitHub/Google OAuth/Notion/Tavily 加密配置、DesktopProcessBroker、精确网络去分类与 fail-closed Hook 审批并覆盖 renderer host port；签名 Desktop matrix 已有同 SHA/三平台/平台信任链/fresh install/packaged launch/七项 Skill authority 的 fail-closed 聚合与 OIDC attestation contract，详见 §12.30～12.42、§12.51、§12.53、§12.57～12.58、§12.60～12.61 | 仍需受保护 producer workflow、Desktop Windows/macOS 签名与公证 secrets，并对同一精确 SHA 跑出 Linux/Windows/macOS 真实安装、启动与 Skill 旅程权威矩阵；本地、未签名或仅 contract 回归不能替代 |
| P1     | P1-12 | Graph Kernel 集成、双写验证与迁移切换  | 🟡 部分完成 | 五 surface/23 入口 machine-readable inventory、唯一 rollout key、run-scoped authority、shadow adapter、migration saga、receipt cut-point recovery、360 个 retire mutation 直接 guard、replacement/historical-read-only 契约与 disabled direct-engine gate 已落地；hash-chain ledger 强制逐 store RPO=0、回滚保留既有 canonical authority；`2932aad32c` 的 Linux/Windows/macOS 真实 provider 与 60/60 同 SHA 证据通过；`751c8df089` 强制 retire replacement、历史读、旧 mutation/writer 的 exact-SHA 结构化 qualification；Browser 标明 non-durable 且默认关闭 | 五 surface 分别完成真实 shadow/canary/default/read-only、完整 projection equivalence、生成 13 retire 替代/零旧 writer 观察 artifact、回滚演练与旧 shell 下线；关闭条件见 §6.9.6.7.4、§6.9.6.7.18～§6.9.6.7.19 |
| P2     | P2-1  | 稳定 `cc exec` facade                  | ✅ 已完成   | `exec` 与 `agent` 共用同一 command/loop/输出契约；manifest/help/completion 和发布矩阵通过                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | —（真实 provider 旅程归 P2-6）                                                                                                                                                              |
| P2     | P2-2  | Graph topology/timeline 调试器         | 🟡 部分完成 | CLI inspect/diff/eval 与完整 topology/timeline/time-travel 已发布；Desktop overlay 已接入三个产品面；固定 `graph/history` App Server/IPC 已向 Workflow 与 Specialized Agents 提供有界 metadata-only durable event/snapshot history；CLI/Desktop/VS Code 共用同一故障 fixture 的 blocked-root、revision diff 与 time-travel 矩阵通过；详见 §12.28～12.29                                                                                                                                                                                                                                                                                             | 仓库实现已关闭；只剩协议/SDK/CLI 精确 SHA 三平台门禁、OIDC 发布与 registry/provenance 回读，完成前不得把本地结果写成正式发布                                                                |
...
| P2     | P2-6  | Graph/Agent 真实旅程与发布矩阵         | ✅ 已完成   | 基础发布矩阵持续全绿；CLI `0.166.6@f2a249bf3d` 已完成 CLI/Strict 三平台和独立公网回读，Open VSX `0.37.71` 已公开，JetBrains `0.4.102` 已通过六宿主并上传；精确候选 `2932aad32c` 又以真实 provider 在 Linux/Windows/macOS 完成 Graph/Agent、crash/resume/merge/message/worktree 旅程及聚合门                                                                                                                                                                                                                                                                                                                                                         | —；JetBrains 公开可见性仍是外部人工审核状态，不改变仓库侧完成状态                                                                                                                           |
