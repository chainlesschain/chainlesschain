...
已新增[只读统计入口及使用说明](./CLAUDE_CODE_CODEX_OUTCOME_METRICS_IMPLEMENTATION_2026-09-15.md)，复用现有 Eval 执行/产物/证据判定，按冻结计划统计未运行样本、首次运行各阶段、失败与重试费用及维护工时。计划变更、重复证据、人工补救、错误版本、窗口外记录和未知成本均有独立处理；完整输入只标记 `BASELINE_RECORDED`，不会产生效果提升 PASS。
