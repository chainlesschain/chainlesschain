...
| 2026-05-18 | OQ-5 | **✅ Plan C 放弃 local terminal** | Phase 6.8 spike (`iOS_Local_Terminal_Spike_Decision.md`) | iOS 沙盒约束让 local terminal 永远是阉割版；Phase 2 远程终端覆盖 95% 场景；a-Shell/Blink 已替代 |
| 2026-05-18 | OQ-6 | **✅ Option 1 不加 watchOS target** | Phase 6.9 spike (`iOS_WatchOS_Spike_Decision.md`) | Phase 4 通知 mirror 自动满足 95% Watch 用户需求；Watch app ROI ~2.5% 用户基数 |
...
## 11. 实施完成总览（2026-05-18 收尾）

> 一晚 sprint 实际 land 的 commit 清单 + 与原 plan 偏差 + 剩余 follow-up。本节是事后归档，便于未来 ramp-up。

### 11.1 已 land commit 清单（按时序）

| Phase | Commit | 说明 |
|---|---|---|
| 6.0 | `b21c4...` 等 | E2E pending，Plan §6.0.8 namespace coverage 审计 + naming drift 修复 |
| 6.1B1 | `841f94bfe` | 5 个 100% wired skill (input/display/app/security/userBrowser) + 81 unit tests |
| 6.1B3 | `5bff51036` | 6 红档桌面子集 skill + 64 unit tests |
| 6.1.7 | `a51e00c5f` | UI 收口 (RemoteInput + RemoteDisplay + SystemTools 9 子 tab) |
| 6.2 | `604515038` | media + browser 主屏 batch + 26 unit tests |
| 6.5 | `328e4bcdc` | workflow + system + history 红档子集 batch 2 + 30 unit tests |
| 6.6.1–6.6.5 | `d059b8948` → `c25a72767` | Desktop skill + DesktopFrameStreamer + RemoteDesktopView + 13 tab |
| 6.7.1+6.7.2 | `d7e029406` | ExtensionCommands 30 method + 32 tests + SystemTools 子 tab |
| 6.4 Action 1 | `d97321abc` | ai-handler.js +7 method 修 Phase 5 残缺 |
| 6.10 | `d648fe7a9` | in-app update banner via App Store Lookup API + 11 unit tests |
| **6.3 step 1** | `d5525c1d1` | knowledge-handler.js +10 method (folders 5 + tags 3 + alias) |
| **6.3 step 2** | `974994215` | knowledge +10 method (versions 4 + star/pin 6) |
| **6.3 step 3** | `874b3b83c` | knowledge +10 method (archive 3 + import-export 4 + tags 高级 3) → **30/30 ✅** |
| **6.3 iOS** | `5b0e82c97` | KnowledgeCommands actor wrap 31 method (30 桌面 + getNote alias) + 35 tests |
| **6.4 commit 1** | `b6da42ef2` | ai +13 method (Conversations 高级 5 + Prompt templates 3 + RAG 5) |
| **6.4 commit 2** | `d23d41cc9` | ai +8 method (Multimodal 4 + Code helpers 4) |
| **6.4 commit 3** | `d0bb48733` | ai +4 method (Agents) → **25/25 ✅** |
| **6.4 iOS** | `cf75e822f` | AIExtendedCommands actor wrap 25 method + 28 tests |
| **6.4 UI** | `b92ffe640` | KnowledgeView + AIExtendedView SwiftUI + 2 VM + 2 新 tab → **15 tab** |

### 11.2 实际 vs 计划偏差

| 维度 | Plan 草案 | 实际落地 | 说明 |
|---|---|---|---|
| Phase 6.3 method 数 | 55 (with RAG/collection) | **30 桌面 + 31 iOS (含 getNote alias)** | OQ-3.2=C 选完整 hybrid，RAG 部分拆到 Phase 6.4 |
| Phase 6.4 method 数 | 45 (含 controlAgent / model 管理) | **25 桌面 + 25 iOS** | OQ-3.3=C 选完整 hybrid；model 管理留 v0.2，原 controlAgent 用更细的 listAgents/runAgent/stopAgent 替 |
| Phase 6.4 SwiftUI scope | 4 sub-feature | **3 sub-tab MVP** (Templates / Code / RAG) | Multimodal (image/audio) + Agents UI 留 v0.2 — 需 image picker / audio recorder infrastructure |
| Phase 6.5 (workflow/system/network) | ~7d 大 sprint | 已在 `328e4bcdc` batch 2 中 land | 用红档子集 batch 模式，时间压缩 |
| Phase 6.0 真机 E2E | 单晚串行 | **仍 pending** (Win dev box 不可验) | 需 Mac + iPhone + 桌面在场 |
| Phase 6.8/6.9 spike | spike + 决策 | 已 spike + decision doc 出（Plan C 放弃 local terminal / Option 1 不加 watchOS） | 见 `iOS_Local_Terminal_Spike_Decision.md` / `iOS_WatchOS_Spike_Decision.md` |

### 11.3 新增模式 / 通用 helper 抽出

1. **`invokeAndDecode` helper 模板**（method ≥ 10 时用）— 首发于 `ExtensionCommands` (Phase 6.7)，复用于 `KnowledgeCommands` / `AIExtendedCommands`。固定签名：
   ```swift
...
2. **桌面 handler 'available' 优雅降级模式** — 查询 method 缺 dep 时 `return { available: false }`，mutating method 缺 dep 时 `throw`。RAG / Agents 5+4 method 全套此模式。

3. **better-sqlite3 `Number → TEXT` "1.0" trap** — 写 TEXT 列时必须 `String(noteId)` 包，否则 `Number(1)` 存为字面 `"1.0"`。memory 待沉淀（暂 inline 在 knowledge-handler `_snapshotVersion` 头注释）。

4. **iOS Inner View + StateObject init 模式** — 解 `@EnvironmentObject` 在 `init()` 不可用：Outer View 读 EnvironmentObject，传 resolved deps 给 Inner View，Inner 用 `_vm = StateObject(wrappedValue: ...)` 在 init 构造。RemoteAIChatView (Phase 5) → KnowledgeView / AIExtendedView (Phase 6.4) 同模式。

5. **OQ-3 hybrid 决策框架** — "iOS 缺 X / 桌面缺 Y" 三选项 (A 锁实际 / B iOS 单向桥 / **C 完整 hybrid**)；OQ-3.2 + OQ-3.3 均选 C 一致路径。

### 11.4 Follow-up（不在此 Plan）

1. **Phase 6.0 真机 E2E** — 15 tab 全跑 + 18 skill + 37 ai + 31 knowledge method + 2 新 UI 流（Knowledge / AIExtended）。需 Mac + iPhone + 桌面。
2. **Multimodal iOS UI v0.2** — image picker (PhotosUI / UIImagePickerController) + audio recorder (AVAudioRecorder) + result display。
3. **Agents iOS UI v0.2** — agent metadata 设计 + 详情 sheet + 实时 stdout 流（类似 chat stream）。
4. **`better_sqlite3_text_number_trap.md` memory** — 沉淀 Number→TEXT "1.0" 坑（5 处显式 String 包修在 knowledge-handler）。
5. **iOS CI 真编 KnowledgeCommands + AIExtendedCommands** — Win 没装 Swift；下次 GitHub Actions iOS 真编 CI 会验。

## 附录：规范章节补全（v5.0.3.108）

> 本文为 gap closure plan。为对齐项目文档标准结构，下列章节以 `见正文` 指引或简述方式补齐若干视角，不重复正文细节。

### 1. 概述

见正文「1. 背景」。iOS 对标 Android Phase 6 Gap Closure Plan：覆盖 Phase 5 之后所有 iOS 工作直至与 Android 功能对齐（knowledge / ai / desktop / extension 等）。

### 2. 核心特性

iOS vs Android 功能 gap 表；子阶段拆分（6.3–6.9）；模块影响 & 测试 plan。

### 3. 系统架构

见正文「4. 范围 & 模块影响」；iOS RemoteSkills 对齐 Android REMOTE commands。

### 4. 系统定位

iOS Phase 6 的**对标 Android 总体 gap closure plan**。

### 5. 核心功能

见正文「5. 子阶段」：6.3 knowledge / 6.4 ai / 6.6 desktop / 6.7 extension / 6.8 local terminal spike / 6.9 watchOS spike。

### 6. 技术架构

iOS CoreP2P RemoteSkills；桌面 handler wire；Knowledge/AIExtended Commands。

### 7. 系统特点

Phase 5.7 真机 E2E pending 时起草；含 Trap T10（A 策略）等决策点。

### 8. 应用场景

规划 iOS 追平 Android REMOTE 功能。

### 9. 竞品对比

本文即 iOS vs Android Phase 6 功能对比（gap 表，见正文 §1.3）。

### 10. 配置参考

各子阶段 OQ；桌面 handler case 扩展。

### 11. 性能指标

各 skill 性能见对应子设计（6.6 帧流 / 6.7 命令往返）。

### 12. 测试覆盖

见正文「6. Test Plan」；iOS CI 真编 KnowledgeCommands + AIExtendedCommands（GitHub Actions，Win 无 Swift）。

### 13. 安全考虑

各 skill 配对信任 + 白名单；desktop/extension 高风险需谨慎。

### 14. 故障排除

better-sqlite3 Number→TEXT "1.0" 坑（见 follow-up 4 memory）；桌面未 wire case。

### 15. 关键文件

iOS RemoteSkills 各 skill；桌面 knowledge/ai handler；KnowledgeCommands / AIExtendedCommands。
...
`iOS_Phase_6_3_6_4_Knowledge_AI_Desktop_Debt.md`、`iOS_Phase_6_6_Desktop_Skill.md`、`iOS_Phase_6_7_Extension_Skill.md`、`iOS_Local_Terminal_Spike_Decision.md`、`iOS_WatchOS_Spike_Decision.md`、`iOS_Phase_6_0_RealDevice_E2E_Plan.md`。
