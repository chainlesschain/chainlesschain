...
见正文「1. 目标 & 非目标」。Phase 14.5 为 `hub.ask` 加 token-by-token 流式输出：桌面 `AnalysisEngine.askStream` + IPC/WS + Android/iOS dispatcher，复用 14.3 dispatcher 祖本与 iOS Phase 5 stream 累积模式。

### 2. 核心特性

`personal-data-hub.ask.progress` wire schema；桌面 askStream（mock + Ollama）；Android + iOS token 流式；ChatBubble + BlinkingCursor。

### 3. 系统架构

见正文 3–6；AnalysisEngine.askStream → IPC/WS → 双端 dispatcher（stream 累积 + LRU 复合 key）。

### 4. 系统定位

PDH `hub.ask` 的**token-by-token 流式输出详细设计**（Phase 14.5）。

### 5. 核心功能

见正文 2–7：wire schema / 桌面 askStream / IPC+WS / Android 数据层+dispatcher / iOS / trap。

### 6. 技术架构

Ollama 流式生成；WS `personal-data-hub.ask.progress`；dispatcher LRU 复合 key（仿 iOS Phase 5）。

### 7. 系统特点

复用 14.3 dispatcher 祖本 + iOS Phase 5 stream 累积模式；HubAskScreen ChatBubble + BlinkingCursor 已就位。

### 8. 应用场景

移动端 `hub.ask` 打字机式流式回答。

### 9. 竞品对比

token 流式 vs 整段返回（用户体验提升）。

### 10. 配置参考

OQ-1~5 决策（见正文）；mock + Ollama 起步。

### 11. 性能指标

首 token 时延 + token 速率（Ollama）。

### 12. 测试覆盖

见正文 7「关键 trap & 防御」；askStream mock 测试。
...
stream 累积乱序 / LRU key 冲突 → 见正文 7 trap。

### 15. 关键文件

`AnalysisEngine.askStream`；WS ask.progress；`HubAskScreen.kt` L151；双端 dispatcher。

### 16. 使用示例

见正文 3「桌面 askStream」与双端 dispatcher 示例。

### 17. 相关文档

见正文链接：父稿 `Personal_Data_Hub_Phase_14_Mobile_Native_Entry.md`、姊妹 `Personal_Data_Hub_Phase_14_3_Sync_Audit_Streaming.md`、`iOS_Phase_5_AI_Chat_Skill.md`。
