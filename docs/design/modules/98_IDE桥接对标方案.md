...
> **状态**: 🚧 Phase 0 ✅ 已落（CLI 发现层 `abe6c561d`）· Phase 1 ✅ 已落 MVP（VS Code 扩展 `packages/vscode-extension/`）· Phase 2+ 评估 — v1.1 细化:env 直连发现 / 多根 workspace / transport 实况(无 ws) / openDiff 阻塞 / 生命周期 / 端到端自验 / 安全权限
> **日期**: 2026-06-10
...
POST JSON-RPC 读 `application/json`),故 server 走 HTTP POST 即通,无需持久 SSE GET 通道。**剩**:
`openDiff` MVP 只"打开 diff"(`{shown:true}`),程序化 accept/reject 回传留 Phase 2;vsce 打包(Phase 4)。
