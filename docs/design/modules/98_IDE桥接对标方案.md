...
> ## 当前发布状态（2026-08-09）
>
> - **VS Code / VSCodium**：`chainlesschain.chainlesschain-ide` **0.37.47** 已在 [Open VSX](https://open-vsx.org/extension/chainlesschain/chainlesschain-ide) 公开，累计下载已突破 **2 万**；源码与公开版一致。该状态不表示 Microsoft VS Code Marketplace 已发布。
> - **JetBrains**：`com.chainlesschain.ide` **0.4.83** 已通过 JetBrains Marketplace 审核并公开（plugin id `32208`，`approve=true`、`listed=true`、`hidden=false`）；源码与公开版一致。
...
> - **完整发布证据**：`ide-vscode-v0.37.47` 与 `ide-jetbrains-v0.4.83` 均指向 `9db081c5a9b24d1c51952e86513b0520620feadd`；[ARM64 11 格真实宿主聚合](https://github.com/chainlesschain/chainlesschain/actions/runs/31269850865)、[Open VSX 发布与回读](https://github.com/chainlesschain/chainlesschain/actions/runs/31271800174)、[JetBrains 上传与 Marketplace 回读](https://github.com/chainlesschain/chainlesschain/actions/runs/31271800206)均成功。VS Code 覆盖 stable/minimum × Linux/Windows/macOS ARM64；JetBrains 覆盖 Linux/macOS 2024.2/2025.2 与 Windows ARM64 2026.2.0.1。
...
> - **版本指针**：VS Code 扩展 **0.37.47**（Open VSX live / source）；JetBrains 插件 **0.4.83**（Marketplace live / source）——正文出现的 `0.2.x` / `0.1.0` 是首发版本号，不是当前版本。
...
| **VS Code 扩展包**          | 官方扩展(marketplace)                                        | 无(仅 Chrome `browser-extension/`)                                                    | **HIGH**               | Phase 1       | ✅ `0.37.47` on Open VSX（Microsoft Marketplace 仍未发布） |
| **JetBrains 扩展包**        | 官方插件                                                     | 无                                                                                    | **HIGH**               | Phase 3(后置) | ✅ `0.4.83` on JetBrains Marketplace                       |
...
**历史时点剩余项**:官方 VS Code Marketplace（需 Azure 订阅,见上,可选）;JetBrains 当时仍待审核。当前 JetBrains 0.4.83 已审核通过并公开，官方 Microsoft VS Code Marketplace 仍未在本页宣称发布。
