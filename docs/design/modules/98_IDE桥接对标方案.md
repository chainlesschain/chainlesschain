...
> - **版本指针**：VS Code 扩展 **0.37.14**（Open VSX live）/ JetBrains 插件 **0.4.58**（JetBrains Marketplace live）——本文头部/正文出现的 `0.2.x` / `0.1.0` 是首发版本号，已远远落后。
...
| **VS Code 扩展包** | 官方扩展(marketplace) | 无(仅 Chrome `browser-extension/`) | **HIGH** | Phase 1 | ✅ `0.37.14` on Open VSX（非官方 MS Marketplace，见 §12.5） |
| **JetBrains 扩展包** | 官方插件 | 无 | **HIGH** | Phase 3(后置) | ✅ `0.4.58` on JetBrains Marketplace |
