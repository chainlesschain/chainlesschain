...
> ## 当前发布状态（2026-08-27）
>
> - **VS Code / VSCodium**：`chainlesschain.chainlesschain-ide` **0.37.71** 已在 [Open VSX](https://open-vsx.org/extension/chainlesschain/chainlesschain-ide) 公开并回读可下载，累计下载突破 **3 万**；该状态不表示 Microsoft VS Code Marketplace 已发布。
> - **JetBrains**：`com.chainlesschain.ide` **0.4.100** 已在 [JetBrains Marketplace](https://plugins.jetbrains.com/plugin/32208-chainlesschain-ide-bridge) 审核通过、listed 并公开；源码/上传身份 **0.4.102** 已通过六宿主门并上传，仍待市场审核。
> - **当前发布能力**：两端消费 Agent Protocol `0.1.5` 从 canonical Schema 生成的 stream event payload union，并通过跨端 causal conformance；VS Code 继续提供 CLI `0.166.6` 的精确授权与无正文 canonical message/handoff 计数，`0.37.71` 另补齐市场 Overview/直接 VSIX 链接一致性和有界 Agent IPC 配对说明。
...
> - **源码与市场边界**：Git 记录中的源码身份为 VS Code **0.37.71** / JetBrains **0.4.102**，精确标签均指向 `42d516c0dd` 且发布工作流成功；公开 `0.37.71/0.4.100` 身份绑定市场制品回读。源码 tag 或上传成功不能冒充已上架版本。
