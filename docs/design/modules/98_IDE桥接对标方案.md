...
> ## 当前发布状态（2026-08-07）
>
> - **VS Code / VSCodium**：`chainlesschain.chainlesschain-ide` **0.37.44** 已在 [Open VSX](https://open-vsx.org/extension/chainlesschain/chainlesschain-ide) 公开，累计下载已突破 **2 万**；源码与公开版一致。该状态不表示 Microsoft VS Code Marketplace 已发布。
> - **JetBrains**：`com.chainlesschain.ide` **0.4.81** 已通过 JetBrains Marketplace 审核并公开（plugin id `32208`，`approve=true`、`listed=true`、`hidden=false`）；源码与公开版一致。
> - **当前公开能力**：双端包含 CLI-owned Sessions Workbench、可恢复 GitHub/Gitee/configured-remote/manual delivery、canonical rewind/branch timeline 与真实宿主 gate；五类 canonical session 已覆盖 Dispatch → `needs_input` → Reply → done、artifact/PR 回读和独立 IDE 进程重启恢复，VS Code 公开版还包含编辑器内联聊天。
> - **完整双端发布证据**：`ide-vscode-v0.37.44`、`ide-jetbrains-v0.4.81` 均指向 `b5177f13c950fcc74be8a2a4aa573f5c422a1b13`；[Open VSX 不可变 VSIX、三平台宿主、发布与回读](https://github.com/chainlesschain/chainlesschain/actions/runs/31090450115)以及[JetBrains 2024.2/2025.2 三平台宿主、上传与 Marketplace 回读](https://github.com/chainlesschain/chainlesschain/actions/runs/31090452195)均成功。
...
> - **版本指针**：VS Code 扩展 **0.37.44**（Open VSX live / source）；JetBrains 插件 **0.4.81**（Marketplace live / source）——正文出现的 `0.2.x` / `0.1.0` 是首发版本号，不是当前版本。
...
> ⚠️ **下表为 2026-06-10 实施前的基线快照**。所有行均已落地（见上方「状态迁移」块 + §3 Phase 日志）；末列 `✅ 现状` 为 2026-07-13 校准，请勿再把"差距等级 / 现状"列当作当前状态。

| 维度                        | Claude Code                                                  | ChainlessChain 现状（2026-06-10 基线）                                                | 差距等级               | 方案          | ✅ 现状（2026-07）                                          |
| --------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------------------------------- | ---------------------- | ------------- | ----------------------------------------------------------- |
| **IDE-as-MCP-server 协议**  | IDE 扩展跑 MCP server,暴露 IDE 工具                          | MCP client/transport(stdio+http+sse+ws)、`setupMcpFromConfig`、`cc mcp scaffold` 全有 | **LOW**(管道现成)      | 复用,不重写   | ✅ 双端 MCP server 上线 + Capability 双向协商               |
| **lockfile 发现协议**       | `~/.claude/ide/<port>.lock`(port+workspaceFolders+authToken) | **无**任何 IDE 发现约定                                                               | **HIGH**               | **Phase 0**   | ✅ `~/.chainlesschain/ide/<port>.json` + owner-only ACL     |
| **集成终端 env 直连**       | 扩展在它拥有的集成终端注入端口 env,CLI 直连(免扫描/免猜)     | **无**                                                                                | **HIGH**               | **Phase 0**   | ✅ `CHAINLESSCHAIN_IDE_PORT/_TOKEN` 注入                    |
| **终端内 IDE 检测**         | CLI 检测 VS Code 集成终端自动连                              | **无**(`TERM_PROGRAM`/env 零判断)                                                     | **HIGH**               | **Phase 0**   | ✅ `isInIdeTerminal` + `--ide`/`--no-ide`                   |
...
| **VS Code 扩展包**          | 官方扩展(marketplace)                                        | 无(仅 Chrome `browser-extension/`)                                                    | **HIGH**               | Phase 1       | ✅ `0.37.44` on Open VSX（Microsoft Marketplace 仍未发布）   |
| **JetBrains 扩展包**        | 官方插件                                                     | 无                                                                                    | **HIGH**               | Phase 3(后置) | ✅ `0.4.81` on JetBrains Marketplace                        |
...
| **交互 REPL `/ide` 命令 ✅ 已落 (2026-06-13)** | `chainlesschain agent` 会话内 `/ide` 显示**本会话**桥接状态:已连→编辑器/端口/工作区 + `mcp__ide__*` 工具;未连→`diagnoseIde` 诊断 + 连接指引。纯函数 `src/repl/ide-status.js`(`ideToolNames`/`renderIdeStatus`,8 测试),REPL handler 紧邻 `/mcp` + 补全列表 + `/help`。等价 `cc ide doctor` 的会话内即时版(Claude-Code `/ide` 平价) |
...
**附带(低成本复用)`@selection`/`@diagnostics` 引用 ✅ 已落 (2026-06-13)**:CLI 已有 `@file` 引用展开(`file-ref-expander`)。现加 `@selection`/`@diagnostics` 伪引用 → 展开时调 `mcp__ide__getSelection`/`mcp__ide__getDiagnostics` 把当前选区 / **整个 workspace 的 error/warning** 注入 prompt(等价 Claude Code 的 at-mention)。纯 CLI 侧增量(`lib/ide-context.js` 加 `findIdeMentions`/`expandIdeMentions` + 两个 formatter),三入口(headless-runner / headless-stream / agent-repl)在 ambient `<ide-context>` 注入处一并展开,不需扩展再改。与 `@path` 同一边界(邮箱/glued 不误匹配),展开短暂(只进在途消息),**不受 `CC_IDE_CONTEXT=0` 影响**(显式请求 > ambient)。未连 IDE / 无选区 / 无问题 → 原样留文 + `@ide:` 警告。17 单测 `ide-mentions.test.js`。

**键盘快捷键(VS Code 0.18.0,Claude-Code 平价)✅ 已落 (2026-06-13)**:此前扩展零绑键,全靠命令面板。现加两个 keybinding —— `Ctrl/Cmd+Esc` 聚焦 Chat 面板(绑自动生成的 `chainlesschainIdeChat.focus`),`Ctrl/Cmd+Alt+K`(editor 聚焦时)把活跃文件作为 `@<path>` 引用插进 Chat 输入框(CLI 端 server-side 展开)。新命令 `chainlesschain.chat.insertReference`(纯函数 `insert-reference.js` 算 ref + `ChatViewProvider.insertReference` 揭示面板;webview 加 `ready` 信号,面板未加载完前插入的引用入队、ready 后 flush,无首开竞速)。11 单测 `vscode-ext-insert-reference.test.js`。
...
**剩(后续打磨)**:~~部分接受(hunk 级)~~(✅ 0.12.0,见 Phase 6)、~~diff 关闭即视为 reject 的事件钩子~~
(✅ 0.14.0,2026-06-12:`tabGroups.onDidChangeTabs` 监听评审 tab 关闭,与按钮竞速单 settle——关 tab
立即解除工具阻塞返 `rejected`+`closedDiff:true`,遗留 toast 的迟到点击被忽略绝不再应用;<1.67 无
tabGroups 的宿主保持按钮兜底。fake-vscode 驱动真 facade headless 测 6 项)。

**热重连 ✅ 已落 (2026-06-12)**:窗口 reload / 扩展升级会让编辑器 MCP server 换 **新端口+新 token** 重启,
此前 CLI 只在启动时解析一次 → 之后整个 session 的 `mcp__ide__*` 工具与选区/诊断注入**静默死亡**。
现 `MCPClient` 加通用 **reconnector 钩子**(`setReconnector(name, fn)`,harness 保持 IDE 无关):
`callTool` 失败且错误为连接形(`isLikelyConnectionError`:fetch failed/ECONNREFUSED/401·403·404/
not found·not connected)→ 调 reconnector 重取 config → disconnect+connect → **重试一次**;
并发失败调用**单飞**共享同一次重连(ide-context 并行 getSelection+getOpenEditors 不会双 connect)。
`loadIdeMcp` 注册 IDE 专属 reconnector = **lockfile 重扫描**(stale env 端口自然 fall through 到
workspace 扫描,正好命中重启后的新实例)。工具级错误(isError/JSON-RPC error)不触发重连。
测试 11(单元 9:重试一次/401 轮换/单飞/null 不重试/tool error 不重连/自愈 + 集成 2:**真 IdeMcpServer
A 停 → B 新端口新 token + lock 换写 → callTool 透明重连**),`ide-hot-reconnect.test.js`。

### Phase 3 — JetBrains 平价 ✅ 协议核+glue 已落(真 SDK 构建通过,marketplace 已上架)
...
~~剩:IntelliJ glue 层需 JetBrains SDK 构建~~(✅ 2026-06-11 真 SDK `buildPlugin` 通过 + marketplace 上架,
见 memory `vscode_ext_publish_openvsx` JetBrains 节)。

**Chat 工具窗第一片 ✅ (0.3.0,2026-06-12)** — JetBrains 侧开始追 VS Code 面板(Phase 6)平价:

- **纯 JDK 会话核**(headless 可测,镜像 VS Code `agent-session.js`/`chat-events.js` 同协议同词表):
  `AgentChatSession`(ProcessBuilder 跑 `cc agent --input/--output-format stream-json
--include-partial-messages`,NDJSON 双向 marshal,reader 线程 try/catch-Throwable 防 EOF race,
  send/sendEvent/interrupt/end/stop,Win 经 cmd.exe 跑 .cmd shim)+ `ChatEvents`(事件→UI 词表映射,
  **delta 去重**同款:流过 delta 则 result 文本置 null;buildSessionArgs provider/model/resume)。
- **glue** `ChatToolWindowFactory`(右停靠 "ChainlessChain" 工具窗,Swing 转写:流式 delta/工具 trace/
  Stop=interrupt 中断单轮/New=重起会话;child 注入本窗口 bridge port/token → 选区/诊断/diff 评审自动点亮)。
- **验证**:`ChatSmokeMain` headless 冒烟 14 项全过(javac --release 8,真 ProcessBuilder ⇆ Node 假
  agent:init/工具/delta/result 双轮 + end() 退出 0 + 死后 send 拒);`buildPlugin` 真 2024.2 SDK 出
  0.3.0.zip,jar 含全部新类 + toolWindow 注册。
...
**历史时点剩余项**:官方 VS Code Marketplace（需 Azure 订阅,见上,可选）;JetBrains 当时仍待审核。当前 JetBrains 0.4.81 已审核通过并公开，官方 Microsoft VS Code Marketplace 仍未在本页宣称发布。
...
- **生命周期**:IDE 中途重启 → 连接悬空,Phase 0 限"启动时一次性解析"。✅ 热重连已落(2026-06-12,见 Phase 2 节):MCPClient reconnector + lockfile 重扫描,失败调用透明重连重试一次。
...
见正文「模块概述 / 功能描述」。IDE 桥接对标方案（Claude-Code IDE Integration Parity v1.1）：IDE 集成桥接。

### 2. 核心特性

IDE 桥接 / VS Code / JetBrains / MCP server。
