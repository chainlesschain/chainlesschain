...
**剩余(device-gated,需真机)**:① `CcVaultGateway` 真实现(现可落地——命令已在设备,包 `LocalCcRunner` 调 `cc hub export/import-events`)② 上层 feature 编排(本机 DID 派生 cipher + 装配资产源 + 把本机已封装块灌入 `InMemoryBackupBlockStore` + 经 `PdhBackupCoordinator.sync` 跑同步 + UI 触发入口)③ 2 真机端到端验收。注:无源远端类型暴露修复(`6c310ad169`)在 v5.0.3.126 tag 之后落地,随下个版本上设备。详见 memory `pdh_backup_engine_8_3`。
