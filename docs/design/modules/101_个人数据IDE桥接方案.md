...
> **状态**: 🟢 方案定稿(15 决策 + 9 段分期)+ **Phase 0 已落地并真机验证**。
> - **Phase 0 ✅**(2026-06-20):cc 侧发现层 `pdh-bridge.js`(`740431618`)+ mcp-config 接线 `loadPdhMcp`(`7e25305f1`)+ Android 协议核 `PdhBridgeProtocol`/`PdhTool`/`PdhLockfileWriter`(`636255ae9`)+ Ktor HTTP shell `PdhBridgeServer`(`14af62228`)+ app 启动接线 `PdhBridgeModule`(`a23324676`)。CLI 33 测 + Android 17 测全绿。**真机 e2e(chopin rooted)**:app 启动起 bridge 于 127.0.0.1:18510 + 写 cc-兼容 lockfile(0600/0700)+ curl 实证 `initialize`→protocolVersion 2024-11-05 / `tools/call pdh_ping`→pong / 错 token→401。剩(Phase 1/2):真采集器替换 stub / cc env 注入 + in-APK cc agent 真连。
