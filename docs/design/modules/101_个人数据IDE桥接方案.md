...
**剩余(device-gated,需真机)**:① DI providers(provide `CcRunnerVaultGateway` + 文件型资产源 + `PdhBackupService`)② UI 触发(§3.5.14 备份/恢复卡按钮 → `syncWith`,配 §3.5.16 设备选择)③ 记忆/技能/instinct 的 cc 导出桥(目前仅 vault 源有 `CcVaultGateway`,其余文件源待各自导出器填暂存文件)④ 2 真机端到端验收。注:无源远端类型暴露修复(`6c310ad169`)在 v5.0.3.126 tag 之后落地,随下个版本上设备。详见 memory `pdh_backup_engine_8_3`。
