...
**剩余(device-gated,需真机)**:① 记忆/技能/instinct 的 cc 导出桥(目前仅 vault 源有 `CcVaultGateway`,其余文件源待各自导出器填暂存文件;v1 先备份 vault)② 2 真机端到端验收(目标设备名→对端 DID 路由、双端播种时序在真机验)。注:无源远端类型暴露修复(`6c310ad169`)在 v5.0.3.126 tag 之后落地,随下个版本上设备。详见 memory `pdh_backup_engine_8_3`。
