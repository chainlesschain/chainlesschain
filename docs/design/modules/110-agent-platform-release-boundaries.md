# 110. Agent Platform 0.166.20 发布与运行时边界设计

> 状态：2026-09-03 核对，CLI 已发布并完成三平台门禁与公共注册表回读
>
> - CLI 精确源码：`75a333971484e78793e693617071e596e27d871f`
> - CLI 不可变标签：`v-npm-0-166-20`
> - 当前 GitHub 主线：`c681e2b91d87c1bb5765008860b2d4fa3d3b4aa3`
> - IDE 源码标签：`ide-vscode-v0.37.80` / `ide-jetbrains-v0.4.109`

## 1. 目标

本设计记录 Agent Platform `0.166.20` 的公共制品、运行时安全、受治理 Skill 演化和后续主线边界，避免以下身份被错误合并：
...
5. IDE 公共 listing 与源码/tag 候选分别回读，上传或 workflow 成功不能替代 Marketplace 可见性。

## 2. 发布身份矩阵

| 表面 | 源码/标签 | 公共状态 | 结论 |
| --- | --- | --- | --- |
| CLI | `v-npm-0-166-20` → `75a3339714` | npm `latest=0.166.20` | 生产推荐 |
| Context/Memory Kernel | 包版本 `0.1.0` | npm 已回读 | 公开 |
| Session Core | 包版本 `0.3.10` | npm 已回读 | 公开 |
| Agent Protocol | 包版本 `0.1.7` | npm 已回读 | 公开 |
| TypeScript Agent SDK | 包版本 `0.2.7` | npm 已回读 | 公开 |
| Python Agent SDK | `0.2.7` | PyPI 已回读 | 公开 |
| VS Code | `ide-vscode-v0.37.80` → `c681e2b91d` | Open VSX `0.37.80` 已回读 | 公开 |
| JetBrains 公共版 | `ide-jetbrains-v0.4.108` → `15bd3636b8` | Marketplace `0.4.108` 已回读 | 公开 |
| JetBrains 源码候选 | `ide-jetbrains-v0.4.109` → `c681e2b91d` | Marketplace 尚未公开 `0.4.109` | source/tag candidate |
| Microsoft VS Code Marketplace | 同一扩展 | 未发现公共记录 | 不作为安装渠道 |
| Desktop/native | 仓库源码与 exact-SHA qualification | 历史资格证据存在 | 不等于当前公共安装包发行 |
| GitHub `main` | `c681e2b91d` | CLI 字段仍为 `0.166.20`，但晚于 npm tag | 源码 head，不改写 npm 制品 |
...
Canonical authority
  ...
  ├─ Human review / Registry transition
...
- 最终用户 review/promote/rollback、active/LKG、kill switch 与 canary 可视控制面；
...
Open VSX `0.37.80` 已公开并推荐 CLI `0.166.20`。JetBrains Marketplace `0.4.108` 已公开，但其打包说明仍对应 CLI `0.166.16`；源码/tag `0.4.109` 已将推荐 CLI 对齐到 `0.166.20`，在 Marketplace 公开回读前只能称为候选。
...
| --- | --- | --- | --- |
| CLI CI（Linux/Windows/macOS） | `75a3339714` | [`33717501794`](https://github.com/chainlesschain/chainlesschain/actions/runs/33717501794) | 成功 |
| CLI Strict Sandbox（三平台） | `75a3339714` | [`33717501564`](https://github.com/chainlesschain/chainlesschain/actions/runs/33717501564) | 成功 |
| npm Trusted Publishing / public-install check | `75a3339714` | [`33717512152`](https://github.com/chainlesschain/chainlesschain/actions/runs/33717512152) | 成功 |
| IDE shared host/browser matrix | `c681e2b91d` | [`33726419225`](https://github.com/chainlesschain/chainlesschain/actions/runs/33726419225) | 成功；公共 listing 仍分别回读 |
