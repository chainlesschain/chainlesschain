...
- **146 内置 Skills + 4 层加载**: 远超 open-agents 的 skills/discovery 简易实现
...
# 自动识别消息里的图片路径（Claude-Code 平价；无需 --image）
echo "描述 ./shot.png" | chainlesschain agent --input-format stream-json --output-format stream-json
...
>
> **图片路径自动识别**（Claude-Code 平价）：除显式 `--image`,消息文本里写一个**磁盘真实存在**的
> 本地图片路径会被自动当视觉输入读图——`detectImagePaths` 扫描受支持扩展名 + 存在性,支持
> `C:\…` / `C:/…` / Git-Bash `/c/…` / `~/…` / IDE 拖拽 `file:///C:/…`(含 `%20`),剥离路径文本,
> 跳过 `http(s)`/`data:`;命中后**该回合切视觉模型**（`resolveVisionLlm`，`llm.visionModel` 或默认
> `doubao-seed-2-0-lite-260215`,只换 model）。生效于 headless stream-json / 交互 REPL / VS Code+JetBrains
> 面板（面板经 `cc agent --input-format stream-json` 驱动,零改插件）。`CC_AUTO_IMAGE=0` 关闭。
...
### 1. 概述
见正文「模块概述 / 功能描述」。Open-Agents 对标补齐方案（Open-Agents Parity v1.0）：对标 Open-Agents 补齐。

### 2. 核心特性
Open-Agents 平价 / 内置工具 / Sub-Agent / prepareCall。

### 3. 系统架构
见正文「架构设计」。

### 4. 系统定位
ChainlessChain 的「Open-Agents 对标补齐方案」。
