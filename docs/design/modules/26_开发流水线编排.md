...
AI 驱动的全生命周期开发流水线编排系统。支持 7 个阶段 (���求→架构→代码→测试→审查→部署→监控)、4 种项��模板、6 种部署策略和门禁审批机制。
...
class DeployAgent extends EventEmitter {
  ...
  async getDeployHistory(options?)             // 部署��史
