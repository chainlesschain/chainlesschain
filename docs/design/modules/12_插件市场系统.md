...
## 9. CLI 受治理安装与激活生命周期（2026-08-17）

桌面端市场与 CLI 统一 runtime 是两个入口：桌面端负责发现、详情和社区交互；CLI 的 `catalog|select|impact|add|upgrade|use|uninstall|evidence` 负责无头环境中的可验证 mutation。稳定版 `0.165.1` 延续 `0.164.0` 的 catalog authority、升级影响审阅和已安装 bytes 回读；下述 activation lifecycle 已在 PR #215 exact head `89c498cc46` 完成门禁并合入主线，但尚未进入 npm tarball。

```text
registry set + candidate evidence
            │
            ▼
 catalog / select / impact ── read-only projection
            │
            ▼ fresh revalidation
 staging ── identity / provenance / payload / policy / consent
            │
            ▼ transaction-owned activation
 version bytes + .active pointer
            │
       ┌────┴────┐
       ▼         ▼
   finalize    rollback
       ...
       └────┬────┘
            ▼
 inert .cleanup-* retirement
```

### 9.1 激活前置条件

- 版本目录及其祖先必须满足路径 containment、非 symlink/junction、普通文件和编码身份约束。
- manifest 的 `name` / `version` 必须与物理目录精确一致；`.plugin-source.json` 必须严格有效。
- 完整 v1/v2 语义绑定会重新哈希当前 payload；semantic strength 不得降低，source switch 与 version downgrade 必须显式批准。
- 公共 `installFromDirectory`、`installFromSource` 与 `updatePlugin` direct API 和 CLI 命令使用同一审批边界，不能绕过 mutation policy。

### 9.2 事务、恢复与运行时阻断

- `.install-*` / `.uninstall-*` 是未完成恢复 namespace。同一 name/scope 的后续 use/install/update/enable/version-uninstall 会拒绝，默认 discovery 也会阻断该插件。
- rollback 先隔离/恢复 bytes，再恢复精确 pointer snapshot，并核对 pointer generation、candidate/predecessor payload 与 source digest。组合 I/O 失败保留可重试 topology，不暴露被拒 payload。
- `listInstalled` 与 `cc doctor` 显示 `runtimeBlocked`、`recovery-required`、inspection version 和 recovery path；无法安全自动判断时，整名卸载后从可信来源重装是显式修复边界。
- 已提交事务先把权威目录原子退役为 `.cleanup-*`，再 best-effort 删除。单纯清理债务不阻断 runtime；若退役与删除同时失败，原 `.install-*` 继续保持权威并失败闭合。

### 9.3 证据边界与剩余风险

实现主提交 `7592c0d5bd` 关闭的是单进程、同 name/scope、串行激活子门，不代表 Marketplace 产品化完成。跨进程 OS lock/durable journal/owner token/generation CAS、跨 scope effective-authority、legacy provenance migration、publisher/组织 trust root、private registry TLS/auth、代理/PAC/custom CA、offline/cache 与长期供应链故障矩阵仍是明确剩余项。
...
见正文「模块概述 / 功能描述」。插件市场系统：插件发现 / 安装 / 管理。

### 2. 核心特性

插件 registry / 安装 / ed25519 签名校验 / 热加载。
...
见正文实现章节；plugin-system / marketplace。
