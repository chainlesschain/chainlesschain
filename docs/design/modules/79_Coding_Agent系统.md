...
| CLI 集成 | `packages/cli/__tests__/integration/ws-session-workflow.test.js` | 19 |
...
IPC：`coding-agent:interrupt` → Main 标记中断 → 通知 bridge → runtime 发出 `session.interrupted` → 转发。
...
| `coding-agent:respond-approval` | R → M | 回传审批 |
...
候选：子代理委派、后台任务、worktree 隔离、持久化任务图、review mode、patch preview、diff 总结。
