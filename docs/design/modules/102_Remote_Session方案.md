...
#### Phase 4：Android Huawei HMS push 真机集成（agconnect 门控 + HmsMessageService，已完成骨架——push 全家收口）

- **形态**：华为最独特——除 Maven 依赖外还需 **AGConnect gradle 插件**（`com.huawei.agconnect`，处理 `agconnect-services.json` 成资源）+ **华为 Maven repo**。故门控与 **Firebase 完全同构**：以 `agconnect-services.json` 存在为闸（华为版 `google-services.json`），且 regId 经 `HmsMessageService.onNewToken` 递送（FCM 的 `FirebaseMessagingService` 对位）。
- **构建门控**（`app/build.gradle.kts` + root `build.gradle.kts` + `settings.gradle.kts`，**三处同一闸**）：`hasAgconnect = file("agconnect-services.json").exists()` → (a) app `apply(plugin="com.huawei.agconnect")` + `src/hms/java` 源集 + `implementation("com.huawei.hms:push:6.11.0.300")`；(b) root buildscript 加 `agcp` 插件 classpath；(c) settings 加华为 Maven repo（pluginManagement 供 agcp classpath + dependencyResolutionManagement `exclusiveContent` 限 `com.huawei.*` group 供 hms:push）。**无 json 时三处全不激活，默认/CI 构建 byte-identical**（门控日志实测 "Huawei HMS disabled"；含 settings/root 改动后完整 config+compile 仍过）。
- **`RemoteSessionHmsService`**（`src/hms/java`，条件源集，继承 `com.huawei.hms.push.HmsMessageService`）：`onNewToken` → `RemoteSessionPushBridge.onNewToken(token, HuaweiTokenProvider.PROVIDER)`；`onMessageReceived` → 本地弹审批通知（`dataOfMap` 只带路由 id）。与 `RemoteSessionFirebaseService` 逐行对位。
...
#### Phase 4：iOS RemoteSession relay 客户端 — WebSocket 连接 + 配对握手（已完成）

- **定位**：iOS `RemoteSessionClient` 的**第二片**——在 E2EE 基座之上补上 networked 层：relay 连接、`register`、加密配对握手、加密控制/事件收发。对标 Android `RemoteSessionClient.kt` 的 connect→register→`sendPairRequest`→`pair.accepted`→CONNECTED 主序。剩余片：自动重连（一次性令牌用尽 → 复用共享密钥恢复）、status/events 独立流化。
- `ios-app/Modules/CoreP2P/Sources/Pairing/RemoteSessionClient.swift`（transport 注入的协议驱动核心，全单测）：
  - transport 抽象 `RemoteSessionWebSocket`（`send`/`close`）+ `RemoteSessionWebSocketListener`（open/text/close/failure）+ `RemoteSessionWebSocketFactory`——镜像 okhttp `WebSocket`/`WebSocketListener`，让协议逻辑与真 socket 解耦、可用 fake 单测。
  - 连接：`connect(uri)` 解析配对 URI、以 `ios-<UUID>` peerId 建 `RemoteSessionCrypto` 并 `pair()`，置 `connecting` 后开 socket；`webSocketDidOpen` 发 `{type:register, peerId, deviceType:mobile, deviceInfo:{protocol:remote-session.e2ee.v1}}`。
  - 握手：收 `registered` → 置 `pairing` 并发 `{type:message, to:hostPeerId, payload:{type:remote-session.pair, mobilePeerId, mobilePublicKey, envelope}}`，envelope 加密 `{type:pair.join, remoteSessionId, token, pushToken?, pushProvider?}`（push 凭据经 `setPushCredentials` 由 app 层从 `RemoteSessionPushTokenResolver` 取值后注入）；宿主解密回 `pair.accepted` → 置 `connected`。
  - 控制/事件：`sendPrompt`/`resolveApproval`/`interrupt`/`updatePushCredentials`（已配对时即发 `push.register`）经加密 `remote-session.encrypted` 上送（未配对返 false）；入站 `remote-session.encrypted` 解密 → `pair.accepted`/`session.revoked`（置 `revoked` 并关闭）/其余经 `onEvent` 回调发出；`offline-message` 解包 `originalMessage`。状态经 `onStatusChange`/`onError` 回调暴露（IDLE/CONNECTING/PAIRING/CONNECTED/DISCONNECTED/REVOKED/ERROR）。
- `ios-app/Modules/CoreP2P/Sources/Pairing/URLSessionRemoteSessionWebSocket.swift`：`URLSessionWebSocketTask` 支撑的生产 transport——delegate 报 open/close、自重排的 receive loop 收帧，全部汇入同一 listener 面；`factory(configuration:)` 直接喂 `RemoteSessionClient(webSocketFactory:)`。
- 验证：`RemoteSessionClientTests` 11（fake socket + 宿主侧 `RemoteSessionCrypto` 完成真 ECDH 解密，对标 Android `RemoteSessionClientPushTest`）：connect→connecting、open→register 形状、`registered`→pair 请求且宿主可解密 `pair.join`、push 凭据随 `pair.join` 上送、`pair.accepted`→connected、配对前 `sendPrompt` 拒绝/配对后加密上送宿主可解、入站事件解密经 `onEvent` 发出、`session.revoked`→revoked+关闭、`updatePushCredentials`→`push.register`、`offline-message` 解包、`disconnect`→关闭+disconnected。Swift 无 Windows 工具链，随 iOS CI 真编译。**协议改动必与 `RemoteSessionClient.kt` + 桌面 relay 同步。**
