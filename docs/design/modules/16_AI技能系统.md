...
AI 技能系统提供 30 个内置技能、统一工具注册表、10 个演示模板以及 Agent Skills 开放标准支持。通过统一注册表聚合 FunctionCaller (60+ 工具)、MCP (8 服务器) 和 Skills (30 技能) 三大工具系统，为 AI 提供结构化的技能感知能力。

### 核心特性

- **30 个内置技能**: 7 个可执行 handler + 23 个文档型技能，覆盖 12 大类别
- **统一工具注册表**: 聚合 3 大工具系统，每个工具都有技能元数据
- **Agent Skills 开放标准**: 13 个扩展字段完整支持
- **10 个演示模板**: 展示技能组合能力的可运行模板
- **自动技能映射**: MCP 服务器自动生成技能、未覆盖工具自动分组
...
│  Layer 0: bundled (30个内置)             │ ← skills/builtin/
└─────────────────────────────────────────┘
高层同名技能覆盖低层
```
...
| 类别       | 技能名                | Handler    | 说明                                    |
| ---------- | --------------------- | ---------- | --------------------------------------- |
| **开发**   | code-review           | -          | 代码审查，分析质量和最佳实践            |
| **开发**   | git-commit            | -          | Git 提交信息生成                        |
| **开发**   | test-generator        | -          | 单元测试生成、Mock 生成、覆盖率分析     |
| **开发**   | performance-optimizer | -          | 性能瓶颈识别和优化建议                  |
| **学习**   | explain-code          | -          | 代码解释，教育用途                      |
| **自动化** | browser-automation    | ✅ (264行) | 浏览器导航/点击/输入/表单/截图/数据提取 |
| **自动化** | computer-use          | ✅ (271行) | 桌面截图/坐标点击/键盘/视觉AI           |
| **自动化** | workflow-automation   | ✅         | 多步骤工作流(条件/循环/并行)            |
| **数据**   | web-scraping          | ✅         | 网页数据抓取(表格/链接/文本)            |
| **数据**   | data-analysis         | -          | CSV/JSON 处理、统计分析、图表推荐       |
| **知识**   | memory-management     | ✅ (252行) | 持久记忆(保存/搜索/日志/洞察)           |
| **知识**   | smart-search          | ✅         | 混合搜索(向量60%+BM25 40%)              |
| **远程**   | remote-control        | ✅         | 远程设备控制(命令/文件传输/剪贴板)      |
| **安全**   | security-audit        | -          | OWASP Top 10 扫描、秘密检测、依赖审计   |
| **DevOps** | devops-automation     | -          | CI/CD 和 Docker 操作自动化              |

### 2.1.1 新增技能 v0.36.0 (15)

| 类别         | 技能名                | Handler | 说明                                       |
| ------------ | --------------------- | ------- | ------------------------------------------ |
| **开发**     | repo-map              | -       | 代码库结构映射（AST符号索引、全局感知）    |
| **开发**     | refactor              | -       | 多文件代码重构（重命名、提取、移动）       |
| **开发**     | onboard-project       | -       | 项目入门分析（架构理解、快速上手指南）     |
| **开发**     | lint-and-fix          | -       | Lint自动修复循环（ESLint→修复→重复）       |
| **开发**     | project-scaffold      | -       | 项目脚手架（模块/页面/技能的样板生成）     |
| **开发**     | mcp-server-generator  | -       | MCP服务器生成（从描述生成完整MCP服务器）   |
| **文档**     | doc-generator         | -       | 文档自动生成（JSDoc、API参考、序列图）     |
| **测试**     | api-tester            | -       | IPC/API测试（发现、生成、健康检查）        |
| **测试**     | test-and-fix          | -       | 测试自动修复循环（运行→分析→修复→重复）   |
| **分析**     | dependency-analyzer   | -       | 依赖分析（导入图、影响分析、CVE可达性）   |
| **数据库**   | db-migration          | -       | 数据库迁移（Schema检查、迁移脚本、漂移）   |
| **知识**     | context-loader        | -       | 智能上下文预加载（意图分析、相关文件）     |
| **安全**     | vulnerability-scanner | -       | 依赖漏洞扫描（CVE、可达性、SBOM、许可证） |
| **DevOps**   | env-doctor            | -       | 环境诊断（运行时、端口、服务健康检查）     |
| **DevOps**   | release-manager       | -       | 发布管理（版本计算、Changelog、Tag）       |
...
| 文件                                            | 行数 | 说明         |
| ----------------------------------------------- | ---- | ------------ |
| `skills/builtin/browser-automation/handler.js`  | 264  | 浏览器自动化 |
| `skills/builtin/computer-use/handler.js`        | 271  | 桌面操作     |
| `skills/builtin/memory-management/handler.js`   | 252  | 持久记忆     |
| `skills/builtin/workflow-automation/handler.js` | -    | 工作流引擎   |
| `skills/builtin/smart-search/handler.js`        | -    | 混合搜索     |
| `skills/builtin/web-scraping/handler.js`        | -    | 网页抓取     |
| `skills/builtin/remote-control/handler.js`      | -    | 远程控制     |
...
Skills System (v0.29.0)
    ↓ 扩展
AI Skills System (v0.36.0)
    ├── 30 内置技能 (扩展自原来的 3→8→15→30)
