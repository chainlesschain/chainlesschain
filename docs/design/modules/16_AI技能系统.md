...
AI 技能系统提供 **146 个桌面内置技能 (100% Handler 覆盖)** + **25 个 Android REMOTE 技能**、统一工具注册表、10 个演示模板以及 Agent Skills 开放标准支持。通过统一注册表聚合 FunctionCaller (60+ 工具)、MCP (8 服务器) 和 Skills (146 技能) 三大工具系统，为 AI 提供结构化的技能感知能力。

### 核心特性

- **146 个桌面内置技能**: 全部配备可执行 handler，100% Handler 覆盖率，覆盖 18+ 大类别
- **25 个 Android REMOTE 技能**: Android 重定位为 REMOTE 遥控器后，25 个 `*Commands.kt`（`remote/commands/`）命令组统一委托桌面执行（24 个已注册进 SeedRegistry）
...
│  Layer 0: bundled (141个内置)            │ ← skills/builtin/
└─────────────────────────────────────────┘
高层同名技能覆盖低层
```
...
## 2. 146 个桌面内置技能 (100% Handler 覆盖)

### 2.1 技能分类总览

所有 146 个技能均配备可执行 Handler，按功能分为 18+ 大类别：
...
| **远程**     | 1      | remote-control                                                                                                                                                                                                                                                                                                    |
| **安全**     | 5      | security-audit, vulnerability-scanner, crypto-toolkit, password-generator, zkp-toolkit                                                                                                                                                                                                                            |
...
| **开发**     | 20     | code-review, git-commit, explain-code, repo-map, refactor, onboard-project, lint-and-fix, project-scaffold, mcp-server-generator, architect-mode, commit-splitter, screenshot-to-code, diff-previewer, task-decomposer, code-runner, git-worktree-manager, github-manager, api-design, frontend-design, create-pr |
| **文档**     | 4      | doc-generator, markdown-enhancer, api-docs-generator, doc-coauthoring                                                                                                                                                                                                                                             |
| **测试**     | 5      | test-generator, api-tester, test-and-fix, bugbot, webapp-testing                                                                                                                                                                                                                                                  |
| **分析**     | 7      | performance-optimizer, dependency-analyzer, impact-analyzer, rules-engine, git-history-analyzer, fault-localizer, debugging-strategies                                                                                                                                                                            |
...
| **浏览器**   | 1      | agent-browser                                                                                                                                                                                                                                                                                                     |
| **协作**     | 4      | planning-with-files, content-publisher, skill-creator, brainstorming                                                                                                                                                                                                                                              |
| **数据库**   | 2      | database-query, db-migration                                                                                                                                                                                                                                                                                      |
| **代码审查** | 1      | pr-reviewer                                                                                                                                                                                                                                                                                                       |
| **其他**     | 10     | word-generator, code-translator, dead-code-eliminator, changelog-generator, mock-data-generator, i18n-manager, research-agent, handler-test-skill, my-custom-skill, test-skill                                                                                                                                    |

### 2.1.1 Android 25 REMOTE 技能

Android v1.0 GA 重定位后统一为 **REMOTE 遥控器**：不再在端上实现本地/文档型技能，而是通过 25 个 `*Commands.kt`（`android-app/.../remote/commands/`）命令组远程委托桌面执行（其中 24 个已注册进 `SeedRegistry`）。

| 类型       | 技能数 | 命令组（`*Commands.kt`）                                                                                                                                                                                                                                         |
| ---------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **REMOTE** | 25     | AI, Application, Browser, Clipboard, Desktop, Device, Display, Extension, File, History, Input, Knowledge, Media, Network, Notification, PersonalDataHub, Power, Process, Project, Security, Storage, System, SystemInfo, UserBrowser, Workflow（各委托桌面 skill 执行；24 已注册进 SeedRegistry）|
...
所有 146 个内置技能目录 (`skills/builtin/{skill-name}/`) 均包含 `handler.js` 和 `SKILL.md`。
...
Skills System (v0.29.0)
    ↓ 扩展
AI Skills System (v1.2.1)
    ├── 146 桌面内置技能 (100% Handler 覆盖, 扩展自 3→8→15→30→90→96→118→128→131→138→146)
    ├── 25 Android REMOTE 技能 (25 *Commands.kt, 24 seeded)
...
- Skills (146 技能) → UnifiedToolRegistry
...
- Android REMOTE Skills → 25 技能 (25 *Commands.kt in remote/commands/, 24 seeded in SeedRegistry)
...
- Community Ecosystem Skills (v1.2.1) → 6 社区生态技能 (brainstorming, debugging-strategies, api-design, frontend-design, create-pr, doc-coauthoring)
- Handler v2.0 Enhancements (v1.2.1) → tavily-search, find-skills, github-manager, self-improving-agent 核心Handler升级
```

---

**最后更新**: 2026-03-10 (v1.2.1)
...
### 1. 概述
见正文「模块概述 / 功能描述」。AI 技能系统：内置技能 + 技能加载 / 门控 / 热加载。

### 2. 核心特性
146 内置技能 / SKILL.md / 懒加载 / 门控检查 / 热加载卸载。

### 3. 系统架构
见正文「架构设计」。

### 4. 系统定位
ChainlessChain 的「AI 技能系统」。

### 5. 核心功能
见正文模块概述与各节。

### 6. 技术架构
见正文实现章节；ai-engine/skill 系统。
