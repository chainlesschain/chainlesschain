...
AI 技能系统提供 **137 个桌面内置技能 (100% Handler 覆盖)** + **28 个 Android 技能**、统一工具注册表、10 个演示模板以及 Agent Skills 开放标准支持。通过统一注册表聚合 FunctionCaller (60+ 工具)、MCP (8 服务器) 和 Skills (137 技能) 三大工具系统，为 AI 提供结构化的技能感知能力。

### 核心特性

- **137 个桌面内置技能**: 全部配备可执行 handler，100% Handler 覆盖率，覆盖 18+ 大类别
...
│  Layer 0: bundled (137个内置)            │ ← skills/builtin/
└─────────────────────────────────────────┘
高层同名技能覆盖低层
```
...
## 2. 137 个桌面内置技能 (100% Handler 覆盖)

### 2.1 技能分类总览

所有 137 个技能均配备可执行 Handler，按功能分为 18+ 大类别：
...
AI Skills System (v1.2.1)
    ├── 137 桌面内置技能 (100% Handler 覆盖, 扩展自 3→8→15→30→90→96→118→128→131→137)
...
- Skills (137 技能) → UnifiedToolRegistry
