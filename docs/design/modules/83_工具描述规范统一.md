...
> **关联文档**: [79. Coding Agent 系统](./79_Coding_Agent系统.md) · [用户文档](https://docs.chainlesschain.com/chainlesschain/coding-agent-tool-descriptor-unification-plan)
...
19. ✅ `docs/claude/CLAUDE-patterns.md` 新增《Canonical Tool Descriptor 规范》章节，固化 canonical shape、镜像方向、读序与禁止项
...
- "哪个技能当前激活" 的 runtime 追踪仍未在 agent loop 中落地，目前由调用方显式传入；下一步会在 `context-engineering.js` 的 `buildOptimizedPrompt` 里消费
...
### 1. 概述
见正文「模块概述 / 功能描述」。工具描述规范统一（Canonical Tool Descriptor v1.0）：统一工具描述符。
