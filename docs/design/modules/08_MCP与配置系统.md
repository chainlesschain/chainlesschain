...
### 1. 概述
见正文「模块概述 / 功能描述」。MCP 与配置系统：MCP 集成 + 统一配置管理（v0.18.0 生产就绪）。

### 2. 核心特性
MCP server 集成 / 工具发现 / 统一配置（unified-config-manager）。

### 3. 系统架构
见正文「架构设计」。

### 4. 系统定位
ChainlessChain 的「MCP 与配置系统」。

### 5. 核心功能
见正文模块概述与各节。

### 6. 技术架构
见正文实现章节；MCP client / unified-config-manager.js。

### 7. 系统特点
见正文（状态 / 版本 / 特性）。

### 8. 应用场景
对接外部 MCP server、统一配置管理。
...
### 13. 安全考虑
见正文安全 / 权限章节；本地加密 + RBAC（如适用）。

### 14. 故障排除
见正文故障 / 已知限制章节。

### 15. 关键文件
MCP client / unified-config-manager.js。
