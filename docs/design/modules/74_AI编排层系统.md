...
- 默认 `--webhook-auth-mode cc` 校验 `X-CC-Webhook-*` HMAC、时间窗和 delivery replay。
- `--webhook-auth-mode vendor` 按 channel 校验原生协议且不回退到 `X-CC-*`：钉钉使用 `timestamp`/`sign`，飞书使用 `X-Lark-Request-*`/`X-Lark-Signature`，企业微信使用 `msg_signature`/`timestamp`/`nonce`。
- 飞书与企业微信密文只在原始请求验签通过后解密；企业微信同时校验密文内的 receive ID，并支持签名的 `GET /wecom` 回调 URL 验证。
...
| Webhook 服务器（CC HMAC DingTalk/Feishu、vendor-native DingTalk） | 3 |
...
- `cc` 与 `vendor` 认证模式显式互斥；vendor 模式缺少对应 channel 凭据时以 503 失败关闭
- 企业微信、钉钉、飞书原生验证分别使用 `CC_ORCHESTRATE_WECOM_TOKEN` + `CC_ORCHESTRATE_WECOM_ENCODING_AES_KEY` + `CC_ORCHESTRATE_WECOM_RECEIVE_ID`、`CC_ORCHESTRATE_DINGTALK_SECRET`、`CC_ORCHESTRATE_FEISHU_ENCRYPT_KEY`
...
4. **Webhook 公网暴露**：服务仍只绑定 loopback；如经反向代理暴露，TLS、代理信任边界与可选 IP allowlist 由部署层配置，不能用来源 IP 替代签名
...
### 1. 概述
见正文「模块概述 / 功能描述」。AI 编排层系统（chainlesschain orchestrate）：多 Agent 编排工作流。

### 2. 核心特性
Orchestrate / 工作流模板 / 代理交接 / 流水线。

### 3. 系统架构
见正文「架构设计」。

### 4. 系统定位
ChainlessChain 的「AI 编排层系统」。
