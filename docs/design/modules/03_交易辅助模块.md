...
见正文「2.3.1 功能描述」。去中心化交易辅助模块利用 AI 和区块链，在去中心化环境下辅助用户达成可信交易，无需传统中介平台。

### 2. 核心特性

数字资产管理；托管服务（escrow）；信用评分；评价 / 评级；知识付费；实时订单匹配引擎；6 智能合约。
...
ChainlessChain 三大核心之一的**去中心化交易辅助**。

### 5. 核心功能

见正文：资产管理 / 托管 / 信用评分 / 评价 / 知识付费 / 实时交易。

### 6. 技术架构

智能合约（6 个）；实时订单匹配引擎；信用评分系统；交易分析报告。

### 7. 系统特点

完成状态 100%（实时交易引擎 + 6 智能合约 + 完整信用评分）。

### 8. 应用场景

数字资产交易、知识付费、托管担保、信用评估。

### 9. 竞品对比

去中心化（无中介抽成）vs 传统中介平台。
...
智能合约托管；交易可追溯；钱包私钥本地 / 硬件密钥。

### 14. 故障排除

订单匹配失败 / 托管异常 → 检查合约状态与余额。

### 15. 关键文件

`asset-manager.js` / `escrow-manager.js` / `credit-score.js` / `review-manager.js` / `knowledge-payment.js` / `realtime-trading-engine.js`。
...
[系统设计主文档](../系统设计_主文档.md)、`docs-site` 交易功能页。
