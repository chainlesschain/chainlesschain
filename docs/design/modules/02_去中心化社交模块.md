...
>
> **实现状态（2026-06-18 / v5.0.3.119）**：本文为高层设计；好友间 P2P E2EE 即时消息已落地并真机验证（加好友扫码/DID + X3DH 会话握手 + 消息 sync.push 投递 + WebRTC 直连 / 信令中继兜底 + 自动重连），实现细节见 **[好友 P2P 消息系统设计（FAMILY-67）](../FAMILY-67_Friend_P2P_Messaging_Design.md)**。好友间音视频通话方案（复用同一信令/TURN/连接管理）见 **[好友 P2P 音视频通话设计方案](../FAMILY-67_Friend_P2P_AudioVideo_Call_Design.md)**（设计阶段）。底层 WebRTC/信令/coturn TURN 基础设施见 [远程操控 Plan AB](../Android_Remote_Operate_Plan_AB.md)。
