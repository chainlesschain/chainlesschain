...
**版本**: v3.1.0
**创建日期**: 2026-02-28
**状态**: ✅ 已实现 (v3.1.0 Desktop) · ✅ CLI Phase 65 + Phase 66 登陆 2026-04-16 (`cc marketplace status-types|invocation-statuses|publish|list|show|status|record|invocations|stats` + `cc incentive contribution-types|tx-types|balance|accounts|mint|transfer|history|contribute|reward|contributions|leaderboard`, skill service lifecycle + invocation record-keeping + stats + token ledger + contribution tracker + leaderboard, `packages/cli/src/lib/skill-marketplace.js` + `packages/cli/src/lib/token-incentive.js`, 45 + 56 tests; real P2P invocation + Context Engineering injection + reputation-weighted rewards + on-chain settlement remain Desktop-only)
...
### 1. 概述
见正文「模块概述 / 功能描述」。Phase 65-66 技能市场与代币激励系统：技能市场 + 代币激励。
