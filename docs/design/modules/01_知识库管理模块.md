...
## 2.1.5 技术选型 (实际实现 v0.20.0)

| 组件       | PC端 (主要)                                      | 移动端 (10%完成) | 说明                          |
| ---------- | ------------------------------------------------ | ---------------- | ----------------------------- |
| 数据库     | **better-sqlite3-multiple-ciphers 12.5.0** (SQLCipher) | 同左             | AES-256加密,生产就绪          |
| 向量数据库 | **ChromaDB 3.1.8** + @chroma-core/default-embed 0.1.9 | ChromaDB-Lite    | 嵌入式向量存储                |
| LLM        | **Ollama** (本地) + 14+云端API                   | MLC LLM (计划)   | 支持Qwen/GLM/GPT等            |
| Embedding  | **Ollama内置** (nomic-embed-text等)              | 同左             | 多模型支持                    |
| Git客户端  | **isomorphic-git**                               | 同左             | 纯JS实现                      |
| 加密库     | node-forge 1.3.1 + tweetnacl 1.0.3 + U盾SDK      | 原生加密 (计划)  | 硬件密钥可选                  |
| UI框架     | **Vue 3.4.0 + Ant Design Vue 4.1.0**             | uni-app (10%)    | TypeScript 5.9.3支持          |
| Markdown   | **Milkdown 7.17.3**                              | 同左             | 所见即所得编辑                |
| 图像处理   | **Sharp 0.33.5 + Tesseract.js 5.1.1**            | 计划             | OCR和图像处理                 |
