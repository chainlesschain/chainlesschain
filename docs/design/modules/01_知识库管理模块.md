...
见正文「2.1.1 功能描述」。知识库管理模块是用户的第二大脑：存储笔记 / 文档 / 对话历史，并提供 AI 增强的检索与问答。

### 2. 核心特性

手动 / 自动数据采集；HybridSearch（Vector + BM25 + FTS5）；永久记忆（PermanentMemoryManager）；OCR（Sharp + Tesseract.js）；Milkdown 所见即所得编辑。

### 3. 系统架构

见正文「2.1.2 架构设计」（数据采集层 / 存储层 / 检索层 / AI 增强层）。

### 4. 系统定位

ChainlessChain 三大核心之一的**个人知识库（第二大脑）**。

### 5. 核心功能

见正文：采集 / 存储 / 混合检索 / RAG 问答 / 永久记忆。

### 6. 技术架构

Ollama embedding（nomic-embed-text）+ LLMManager；SQLite FTS5 + HybridSearch；isomorphic-git；Vue 3.4 + Ant Design Vue + Milkdown。

### 7. 系统特点

混合搜索（Vector + BM25）；跨会话永久记忆（Clawdbot 风格）；本地优先 + 加密。

### 8. 应用场景

个人笔记管理、RAG 知识问答、跨会话记忆沉淀。

### 9. 竞品对比

见正文技术栈对比表（桌面 / iOS / Android 实现矩阵）。

### 10. 配置参考

embedding 模型（nomic-embed-text 等）；HybridSearch 权重；存储路径。
...
知识库检索 / RAG / 永久记忆相关测试（见桌面单测）。

### 13. 安全考虑

本地 SQLCipher 加密；U盾 / Android Keystore 硬件密钥可选；node-forge + tweetnacl。

### 14. 故障排除

embedding 模型未加载 / 检索无结果 → 检查 Ollama 与索引。

### 15. 关键文件

`desktop-app-vue/src/main/`（知识库 / 检索 / 永久记忆）；PermanentMemoryManager；HybridSearch。
...
[系统设计主文档](../系统设计_主文档.md)、`docs-site` 知识库功能页。
