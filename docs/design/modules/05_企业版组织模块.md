...
见正文「功能描述」。企业版组织模块提供组织 / 团队管理、RBAC 权限、组织知识库隔离与协作。

### 2. 核心特性

组织 / 团队管理；RBAC 权限；实时协作编辑（开发中）；组织知识库隔离（开发中）；高级权限管理（开发中）。

### 3. 系统架构

见正文架构设计；实现位置 `desktop-app-vue/src/main/organization/`（12 文件）。

### 4. 系统定位

ChainlessChain 的**企业版组织管理模块**。

### 5. 核心功能

见正文：组织 / 团队 / 成员 / 角色 / 权限管理。

### 6. 技术架构

`desktop-app-vue/src/main/organization/`（12 文件）；RBAC。

### 7. 系统特点

企业能力通过 Profile 叠加；组织知识库隔离（开发中）。
...
企业组织管理 vs 个人版（默认面向个人）。

### 10. 配置参考

组织 / 角色 / 权限配置；`.ccprofile` 企业 Profile。
...
RBAC 细粒度权限；组织数据隔离；审计。

### 14. 故障排除

权限不足 / 组织数据不隔离 → 检查角色与组织 scope。

### 15. 关键文件

`desktop-app-vue/src/main/organization/`（12 文件）。
...
[系统设计主文档](../系统设计_主文档.md)、`docs-site` 企业组织功能页。
