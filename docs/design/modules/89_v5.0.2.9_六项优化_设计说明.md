...
v5.0.2.9 第一阶段（参见 `docs/claude/CLAUDE-patterns.md` Hooks 三件套段）补齐了 SessionStart / UserPromptSubmit / SessionEnd 三个会话级事件的触发点。本轮补齐：
...
S3 阶段（参见 `docs/claude/CLAUDE-patterns.md` Skill-Embedded MCP）让技能在 SKILL.md 内联声明 MCP 服务器。本轮补齐 LLM prompt 端的过滤：当某个技能激活时，只把它 mount 的 MCP 服务器对应的工具暴露给模型，避免 130+ 技能场景下的工具爆炸。
...
参见 `docs/claude/CLAUDE-troubleshooting.md` "Vitest 并行执行 OOM" 段。
...
Category Routing S2（参见 `docs/claude/CLAUDE-patterns.md` 类别路由）原有 5 类别（quick/deep/reasoning/vision/creative）覆盖文本生成场景，但 RAG embedding 调用与语音转写 / TTS 调用没有归类，依旧硬编码 provider。本轮补齐 embedding / audio 两类。
...
- `docs/claude/CLAUDE-patterns.md` — Hooks 三件套 / Skill-Embedded MCP / Category Routing 模式
- `docs/claude/CLAUDE-troubleshooting.md` — Vitest OOM 防御
...
### 1. 概述
见正文「模块概述 / 功能描述」。v5.0.2.9 六项优化设计说明：六项专项优化。
...
### 4. 系统定位
ChainlessChain 的「v5.0.2.9 六项优化」。
