...
- ✅ Phase 0 / Phase 1.1 / Phase 1.4 桌面 web-shell 起步（2026-04-30）：与 P9d 并行，桌面版另起 `desktop-app-vue/src/main/web-shell/` 子树，由 `web-shell-bootstrap.js` 拉起 `web-ui-loader`（HTTP 托管 `packages/web-panel/dist`）+ `ws-cli-loader`（沿用 CLI 的 `packages/cli/src/gateways/ws/ws-server.js` + 新增 WSSessionManager 自动实例化）+ `ws-bridge.js`（统一 envelope `{type, requestId, payload}`），让 BrowserWindow 直接加载 web-panel 渲染层；Phase 1.1 接入 `handlers/skill-list-handler.js`（桌面 SkillToolSystem → web-panel `stores/skills.js`）+ `handlers/fs-handlers.js`（dialog.showOpenDialog/showSaveDialog 桥），并在 SystemSettings 增加"使用 Web Shell（实验）"持久化开关；Phase 1.4 通过 `desktop-app-vue/scripts/prepare-web-shell-vendor.js`（forge `afterCopy` 钩子）把 web-panel dist + ws 静态资产复制到 `<buildPath>/../vendor/web-shell/`，路径数学单测 189 行覆盖；同时落地 ShellStatusBar 接真 IPC（弃 P0 placeholder）+ LoginPage dev-credential hint 在生产环境隐藏 + V6 hard-flip 登录流；新增 8 套 web-shell 单元测试（`web-shell-bootstrap` / `web-ui-loader` / `ws-cli-loader` / `ws-bridge` / `fs-handlers` / `skill-list-handler` / `phase1.4-path-math` / `prepare-web-shell-vendor`）+ phase0-smoke 端到端冒烟。
...
### 1. 概述
见正文「模块概述 / 功能描述」。桌面版 UI ClaudeDesktop 重构计划（P7）：桌面 UI 对标 Claude Desktop 重构。

### 2. 核心特性
桌面 UI 重构 / 对话优先 / Artifact / P7。

### 3. 系统架构
见正文「架构设计」。

### 4. 系统定位
ChainlessChain 的「桌面版 UI 重构计划」。
