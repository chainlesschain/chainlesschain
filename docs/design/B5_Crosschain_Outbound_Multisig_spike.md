...
| 2 | ✅ landed | `packages/cli/src/lib/cross-chain-mtc.js` + `__tests__/unit/lib/cross-chain-mtc.test.js` | 加 4 个 exports：`attachMultisigProvenance(env, prov)` / `stripMultisigSigsForCanonical(env)`（mirror `_stripSigsForPublisher` from MTC v0.11）/ `verifyMultisigProvenance(env, policy?)` / `buildMultiHopBridgeEnvelope(legs, meta, multisigProvenance?)` 第三参数。Provenance shape：`{proposal_id, threshold_m, member_count_n, signers[], partial_sigs[{did,alg,sig:hex}]}`。verifier 检查 12 项（结构 / threshold / 排序 / 算法白名单 / hex sig / DID 一致性 / policy.members 限定）。**17 new unit tests** + 109 既有 cross-chain-mtc 单测 / 22 integration 0 regression。**核心解耦：PR3 verifier 集成 + PR4 envelope ↔ cc_bridges 链接由这 4 export 拼接** |
| 3 | ✅ landed | `cross-chain-mtc.js:verifyMultiHopBridgeEnvelope` + 8 PR3 tests | 第三参数 `options = {expectedMultisigPolicy?, requireMultisig?}`；wrapper 带 provenance OR `requireMultisig=true` 时自动 `verifyMultisigProvenance(wrapper, expectedMultisigPolicy)`；失败返 `MULTISIG_PROVENANCE_INVALID` + 嵌套 `multisig_result`；成功仍返 `{ok, leg_results, multisig_result}`。**backward compat**：不带 provenance 且未要求 → 原行为不变。8 新 unit tests（unchanged backward / requireMultisig missing / valid auto / policy match / BELOW_THRESHOLD / SIGNER_NOT_IN_POLICY / 短路顺序 / SIGNERS_NOT_SORTED）+ 130 既有 cross-chain-mtc 0 regression |
| 4 | ✅ landed | `cross-chain-mtc.js:stageBridgeOp` + `crosschain.js:_bridgeConsume` + `bridge-consume --mtc` 子命令 + 7 PR4 tests | (1) `stageBridgeOp` 在写入前对 `op.multisig_provenance` 做 `verifyMultisigProvenance` 结构性 gate，失败返 `INVALID_MULTISIG_PROVENANCE:<code>:<detail>` 不留脏数据 (2) `_bridgeConsume` 加 `mtc` / `mtcConfigDir` 参，原 multisigContext 加 `thresholdM` + `memberCountN`，op 形态拓展 `multisig_provenance: {proposal_id, threshold_m, member_count_n, signers, partial_sigs}` (3) `bridge-consume` 子命令新 flags `--mtc` + `--mtc-config-dir` 镜像 `bridge` 命令；consume JSON / text output 显示 `mtc.staged` / `mtc.path` / `mtc.reason` (4) closeBatch 透传天然走 — op JSON 写入即包含 provenance，`assembleBridgeBatch → assembleBatch` 将 op 直接作为 leaf，下游 `verifyBridgeEnvelope` 后通过 `envelope.leaf.multisig_provenance` 拿到，无需新加 code path (5) 6 新 unit tests（valid 写文件 / 4 reject 路径：unsorted signers / unsupported alg / m > n / 缺字段 / legacy op 不挂 / closeBatch leaf 携带 provenance round-trip）+ 1 E2E 测 `bridge-consume --mtc` 真把 op + provenance 写进 `staging/` 目录。**137/137 测试全过**（103 unit + 14 E2E + 20 mtc-cli integration） |
...
### 1. 概述
见正文头部。B.5 跨链桥 outbound × M-of-N 多签 spike（v0.9）：跨链 outbound 多签。

### 2. 核心特性
跨链桥 / outbound / M-of-N 多签 / spike。

### 3. 系统架构
见正文架构 / 设计章节。

### 4. 系统定位
ChainlessChain 的「跨链桥多签 spike」。
