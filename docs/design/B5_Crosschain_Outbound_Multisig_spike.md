# B.5 跨链桥 outbound × m-of-n 多签 — spike v0.7

> **Issue**: [#21](https://github.com/chainlesschain/chainlesschain/issues/21) B.5（GA 后续 scope · P1）
> **状态**: 🟢 Layer 1 ✅ 收口 + Layer 2 PR1 ✅ + PR2 ✅ landed (2026-05-15)
> **作者**: 2026-05-15
> **关联**: [m-of-n 应用扩展 v1](MofN_多签_应用扩展_v1.md) / [MTC 跨链桥 v1](MTC_跨链桥_v1.md) / memory `mtc_landing_v0_11.md` Q-COMP-3
> **下一步**: Layer 2 PR3 — wire `verifyMultisigProvenance` into `verifyMultiHopBridgeEnvelope`（envelope 携带 provenance 时强制 m-of-n 检查）+ PR4 把 `_bridgeConsume` 通过 stageBridgeOp 把 multisig provenance 串到 MTC envelope 路径
...
| 3 | ⏳ pending | `cross-chain-mtc.js:verifyMultiHopBridgeEnvelope` | wire `verifyMultisigProvenance` into wrapper verification flow；envelope 携带 provenance 时强制 m-of-n 检查 + policy spec opt-in；外部传 `expectedMultisigPolicy` arg 切换严格模式 |
