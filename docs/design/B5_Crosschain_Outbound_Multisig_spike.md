# B.5 跨链桥 outbound × m-of-n 多签 — spike v0.8

> **Issue**: [#21](https://github.com/chainlesschain/chainlesschain/issues/21) B.5（GA 后续 scope · P1）
> **状态**: 🟢 Layer 1 ✅ 收口 + Layer 2 PR1 ✅ + PR2 ✅ + PR3 ✅ landed (2026-05-15)
> **作者**: 2026-05-15
> **关联**: [m-of-n 应用扩展 v1](MofN_多签_应用扩展_v1.md) / [MTC 跨链桥 v1](MTC_跨链桥_v1.md) / memory `mtc_landing_v0_11.md` Q-COMP-3
> **下一步**: Layer 2 PR4 — 把 `_bridgeConsume` 通过 stageBridgeOp 把 multisig provenance 串到 MTC envelope 路径（staging file 加 multisig field → closeBatch / buildMultiHopBridgeEnvelope 接住）
...
| 4 | ⏳ pending | `cross-chain-mtc.js:stageBridgeOp` + `closeBatch` + `_bridgeConsume` | 让 `cc crosschain bridge --require-multisig --mtc` opt-in 把 multisig provenance 串到 MTC envelope 路径（staging file 加 multisig field → close-batch 时 attach 到 wrapper） |
