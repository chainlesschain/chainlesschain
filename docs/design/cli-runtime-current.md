# CLI Runtime 当前实现核对（Agent Platform 0.166.9）

> 更新时间：2026-08-29。完整门禁的生产推荐版与 npm `latest` 均为 Agent Platform `0.166.9`，绑定不可变 tag `v-npm-0-166-9` 的精确 SHA `222396f6a8429d4b862292a2572067a5cacb1003`。该 SHA 的 Linux/Windows/macOS CLI CI、Strict Sandbox、不可变制品、provenance 与独立公网回读均已闭环。TypeScript Agent SDK `0.2.5`、Python Agent SDK `0.2.6`、Agent Protocol `0.1.6` 与 Open VSX `0.37.73` 已公开；JetBrains Marketplace 仍为 `0.4.103`，源码/标签 `0.4.104` 不冒充公共制品。

## 版本与证据边界

- `0.166.9` 是当前生产推荐与 npm `latest`。`v-npm-0-166-9` 精确指向 `222396f6a8429d4b862292a2572067a5cacb1003`；同一发布提交的 `CLI CI` `33228796205`、`CLI Strict Sandbox` `33232869268` 与 npm 发布/provenance/公网回读 `33232869286` 均成功。
- `@chainlesschain/agent-sdk@0.2.5`、`chainlesschain-agent-sdk==0.2.6` 与 `@chainlesschain/agent-protocol@0.1.6` 已公开。Python `0.2.5` 未发布，由修正安装元数据但不改变 runtime API 的 `0.2.6` 取代；SDK 与 Protocol 保持独立安装和发版边界。
...
- IDE 当前公开版本为 Open VSX `0.37.73` 与 JetBrains Marketplace `0.4.103`。`ide-vscode-v0.37.73` 与 JetBrains 源码 tag `ide-jetbrains-v0.4.104` 指向 `222396f6a8`，IDE 工作流 `33230594120` 成功；Marketplace 未公开 `0.4.104`。微软 VS Code Marketplace 仍未发布。
- CLI/SDK/Protocol 当前公开组合为 `0.166.9/0.2.5 TS + 0.2.6 Python/0.1.6`。Desktop 签名/Skill 资格、真实 UI replay 与 Codex compatibility 都有 exact-SHA 专项证据，但公共 native 分发、生产 relay 与真实 provider 验收仍需独立判断。
...
- IDE 公开版 Open VSX `0.37.73` / JetBrains Marketplace `0.4.103` 只在插件升级结果为 `activated` 后重载 live session；capability widening 必须先展示新增能力并由用户显式批准，`rolled_back` 或不可读结果保持失败闭合。JetBrains 源码 `0.4.104` 不改变 Marketplace 安装口径。
...
- Open VSX 当前公开 `0.37.73`；JetBrains Marketplace 当前公开 `0.4.103`。双端消费 Schema 生成 payload union 并保留未知未来事件，延续 Context Center、权限/副作用证据、Automation Center、durable workflow/Artifact recovery 与无正文协作投影；`0.4.104` 的 revision-bound grant 与 single-winner settlement 等源码增量待 Marketplace 公网回读后才进入安装口径。Microsoft VS Code Marketplace 仍未发布。
...
`0.166.9` 的精确正式发布提交为 [`222396f6a8429d4b862292a2572067a5cacb1003`](https://github.com/chainlesschain/chainlesschain/commit/222396f6a8429d4b862292a2572067a5cacb1003)。同一发布提交的 [CLI CI run 33228796205](https://github.com/chainlesschain/chainlesschain/actions/runs/33228796205)、[CLI Strict Sandbox run 33232869268](https://github.com/chainlesschain/chainlesschain/actions/runs/33232869268)、[npm release/readback run 33232869286](https://github.com/chainlesschain/chainlesschain/actions/runs/33232869286) 与 [IDE release run 33230594120](https://github.com/chainlesschain/chainlesschain/actions/runs/33230594120) 均成功；registry tarball、不可变 workflow artifact 与 provenance 身份已交叉核对。Agent Protocol `0.1.6`、TS SDK `0.2.5`、Python SDK `0.2.6` 与 Open VSX `0.37.73` 均已公开回读。

后续版本仍必须在各自 final exact SHA 上重新完成权威门；GitHub `main` 后续真实协作质量门与 IDE ARM64 Robot 重试修复不能继承 `v-npm-0-166-9` 的发布授权。`222396f6a8` 同时承载 IDE `0.37.73/0.4.104` 源码标签，但 Marketplace 当前仍只公开 JetBrains `0.4.103`，各制品必须按独立发布与公网回读证据处理。
