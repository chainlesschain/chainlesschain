...
> 本目录是 ChainlessChain 的研发设计入口，也是用户文档站与设计文档站的共享设计源。2026-09-08 当前生产推荐版与 npm `latest` 为 Agent Platform `0.166.34@2f8f62d73e`；精确提交的三平台 CLI CI、Strict Sandbox、IDE 与 npm 发布/公共回读已完成。GitHub/Gitee `main@e05e613c8e` 当前内置 CLI `0.166.32`；发布提交位于带 tag 的后继分支，尚未合入 `main`。
...
- Agent Platform `0.166.5` 首次公开 Schema 生成事件；`0.166.7` 完成 Graph authoritative entry cutover；`0.166.14` 公开 Context/Memory、Hooks v2 与 P0 执行安全；`0.166.24` 公开受治理 Evolution Workbench、Skill Retrieval 与知识冲突审核/合并；`0.166.30` 增加原子模型连接与页面化工作台；当前 `0.166.34` 增加有界网页快照、分块续读、长文本检索、结构化 fetch 错误与最低 Node 版本兼容修复。
...
- CLI `0.166.34` 的精确 SHA `2f8f62d73e` 已完成三平台 CLI CI、Strict Sandbox、IDE、npm 发布和公网回读并成为生产推荐；Session Core `0.3.12`、Agent SDK TS/Python `0.2.8` 与 Protocol `0.1.8` 保持公开。历史 Graph/Context/Memory/安全能力继续保留，但每项证据仍绑定其原始 exact SHA。
...
- Open VSX 当前公开 `0.37.89`；JetBrains Marketplace 当前公开 `0.4.114`，累计 Open VSX 下载超过 3.8 万。JetBrains `0.4.115` 仍是源码/tag，不能冒充 Marketplace 已上架版本。两端仍保持 CLI-authoritative 投影；微软 VS Code Marketplace 仍未发布。
...
- 生产基线与 npm `latest` 已统一为 Agent Platform CLI `0.166.34@2f8f62d73e`；Open VSX `0.37.89`、JetBrains Marketplace `0.4.114`、TS/Python SDK `0.2.8`、Session Core `0.3.12` 与 Protocol `0.1.8` 已分别按公共回读记录。
- 新增 10 MB 原始下载预算、20,000 字符默认返回预算、本地快照隔离与生命周期、Unicode 安全分页、长文本 worker 搜索和 typed failure 的设计边界。
...
- 当前公开口径已对齐为 Open VSX VS Code `0.37.89` 与 JetBrains Marketplace `0.4.114`；JetBrains `0.4.115` 仍为源码/tag。Microsoft VS Marketplace 未完成公开回读，不能用 Open VSX 状态替代。
