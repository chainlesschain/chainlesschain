...
> 本文为性能验收方法学文档。为对齐项目文档标准结构，下列章节以 `见正文` 指引或简述方式补齐若干视角，不重复正文细节。

### 1. 概述

见正文头部。Android v1.0 M6 性能 / 续航 / 弱网压测验收，关联 `Android_重新定位_设计文档` §7.2 性能预算；方法学就位，实测值待真机回填。

### 2. 核心特性

8 项性能预算 + 测量方法；弱网矩阵（3G/200ms RTT/5% loss）；回填模板；release-time 必跑命令。

### 3. 系统架构

见 `Android_重新定位_设计文档` §7.2 性能预算 / §7.3 测试矩阵。

### 4. 系统定位

Android v1.0 的**性能 / 续航 / 弱网验收方法学**（M6）。

### 5. 核心功能

见正文 1–7：8 项预算 + 测量 / 弱网 / 回填模板 / release 命令 / CI（v1.1）/ 已知偏离项。

### 6. 技术架构

adb 测量；弱网模拟；后台耗电 / 跨 NAT p50 / ASR 首字等指标。

### 7. 系统特点

实测值待真机回填；CI 集成 v1.1 计划（v1.0 暂不强求）。
...
ASR 首字按实现标注（Google Speech <400ms vs 搜狗/讯飞/离线，见正文 #8）。

### 10. 配置参考

见正文 5「release-time 必跑的命令」。

### 11. 性能指标

8 项预算（见正文 1）；后台耗电 ≤1%/h、跨 NAT p50、ASR 首字等（见正文 7 已知偏离项）。

### 12. 测试覆盖

方法学就位；实测回填模板见正文 4；CI 集成 v1.1。

### 13. 安全考虑

WakeLock 漏导致耗电异常需排查（P2PClient WebRTC keep-alive）。

### 14. 故障排除

见正文 7「已知会偏离预算的项」（后台耗电 / 跨 NAT / ASR 首字）。

### 15. 关键文件

release-time 必跑命令脚本；`Android_重新定位_设计文档.md` §7.2/§7.3。

### 16. 使用示例

见正文 2「测量方法详情」与 5「release-time 命令」。

### 17. 相关文档

`Android_重新定位_设计文档.md` §7.2 性能预算、`Android_M4_D2_E2E.md`、`Android_M3_Real_Device_Test_Plan.md`。
