...
见正文头部。PDH Xhs Mode B（本机 root SQLite fallback）真机 E2E checklist（Phase 7.5.3，Win-first）；v0.1 是低期望 fallback，真机大概率走 v2.0 frida + libshield neuter 路径。

### 2. 核心特性

本机 root DB 采集；defer-recommended framing；写同一 `social-xiaohongshu` adapter（跨路径 dedup）。

### 3. 系统架构

adb su sqlite3 → 本机 DB → `social-xiaohongshu` adapter → vault。

### 4. 系统定位

PDH Xhs Mode B 的**本机 DB 真机 E2E 验收 checklist**（低期望 fallback）。

### 5. 核心功能

见正文场景 X-1~X-5：root 探测 / DB 解析 / vault。

### 6. 技术架构

root + adb su sqlite3；v2.0 frida + libshield neuter + SQLCipher key hook + smali patch（情况 B）。

### 7. 系统特点

v0.1 低期望（libshield 反 frida）；v2.0 frida 4–6 周（smali patch libshield）。

### 8. 应用场景

无 cookie/网络时离线本机采集 fallback（大概率走 v2.0）。

### 9. 竞品对比

Mode B vs C 路径（`PDH_Xhs_Real_Device_E2E.md`，X-S 签名主路径）。

### 10. 配置参考

root；DB_FILENAME_CANDIDATES（P7.5.0 探测）。
...
本文即 Mode B E2E checklist；schema 探测 P7.5.0 fill-in。

### 13. 安全考虑

需 root；本机 DB 高敏感；libshield 反 frida + SQLCipher。

### 14. 故障排除

libshield 反 frida → v2.0 smali patch libshield + frida neuter。

### 15. 关键文件

`social-xiaohongshu` adapter（Mode B 三件套）；vault。
...
`PDH_Xhs_Real_Device_E2E.md`、`PDH_Mode_B_RealDevice_Master_Checklist.md`。
