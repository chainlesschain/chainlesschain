...
> **版本**: v0.3 (实施进度回填稿, 2026-05-11) | **状态**: 🟡 ADR 8/8 accepted · M1/M2/M5 ✅ · M3/M4 部分 · M6/M7 ⏳ | **关联**: Android v0.37.0 → v1.0.0 | **桌面对标**: v5.0.3.47
>
...
>
> v0.2 → v0.3 变更：M1 inventory PR (`2244ced9f`) / M2 D1-D3 / M3 D-share + D-loc 的 JVM-testable 部分 / M4 D1 RemoteSkillRegistry / M5 SignAsService 全部已落地，状态从 ⏳ 回填到 ✅ / ⚠️ 部分。剩余面：M3 D-voice / D-cam / D-push + D-share/D-loc 的 Manifest 接线、M4 ApprovalUI + 桌面 whitelist/approval-channel、M6 性能验收、M7 发版。事实声明（v0.1 → v0.2 已校）未再变更。
...
| `QRPairing`           | 扫码生成 P2P 配对码 / 扫码登录授权          | ZXing + DevicePairingHandler        | ⚠️ 桌面侧 device-pairing-handler.js 已有；Android 侧扫码 UI 状态待 audit（Phase B 任务） | 18 |
...
| **M2** | 3 天    | L1 钥匙层：StrongBox tier + DIDWallet 多身份+助记词 + BiometricGate + QRPairing 收敛 | ✅ D1-D3 / ⚠️ D4 QRPairing audit | 67 单测 + E2E 首启动落 DID（QRPairing Android 扫码 UI 待 audit） |
...
| **M4** | 2 天    | L3 RemoteSkillRegistry + 桌面白名单 + ApprovalUI              | ⚠️ 部分（D1 RemoteSkillRegistry 已落 `df61914ff`） | 55 单测 + 桌面白名单严格生效 + 审批 E2E |
...
|        | **共 11 天**（v0.1 是 10 天，M3 +1 天） |                                       | **已落 ≈ 6 天有效工**；剩 ≈ 7 天（M3 剩 D-voice/D-cam/D-push + D-share/D-loc Manifest 收尾 / M4 ApprovalUI + 桌面 whitelist+approval-channel / M6 / M7 / QRPairing audit） | |
...
- D3: ✅ BiometricGate（统一现有零散 Settings flag）— `4124038d9`；⚠️ QRPairing Android 扫码 UI 状态待 audit（Phase B 任务）+ E2E 首启动
