...
> **版本**: v0.17 (§10 A.1 PR1 landed + 准入条件重评 — Linux LAN 配对 preflight CLI，2026-05-15) | **状态**: 🎉 ADR 8/8 accepted · M1/M2/M5 ✅ · **M3 5/5 code 全落（VoiceMode + CameraOCR + LocationTagger + ShareReceiver + PushNotifier，+3,861 行 / 99 单测，真机待用户出场）** · **M4 D1 ✅ method-level / D2 ✅ desktop-side full + Android RPC 接收器 + **ApprovalUI 4 类 category** + **ProgressViewer***（真机 E2E 仍待） · M6 性能预算文档就位待真机回填 · M7 GA flip ✅ tag `v1.0.0` + GitHub Release published | **关联**: Android **v1.0.0 GA** | **桌面对标**: v5.0.3.48
>
...
- **A.1（P1）桌面端 Linux native 配对** 🟢 PR1 ✅ landed (2026-05-15 [`A1_Linux_Native_Pairing_spike.md`](A1_Linux_Native_Pairing_spike.md))：**准入条件重评** — 原 framing "Linux 需补 mDNS systemd 单元 + Wayland 屏幕权限" 大部分不准确（`@libp2p/mdns` + `bonjour-service` 是纯 JS 不依赖 avahi-daemon；配对 UI 不抓屏，与 Wayland 权限无关）。**真缺口** = headless 模式 + preflight 诊断 + docs，3 PR 拆分。**PR1 ✅**：`cc pair preflight` CLI subcommand 5 项 pure-JS 检查（interfaces / multicast bind / port 5353 holders / platform-aware distro detect / firewall hint 模板 ufw/firewall-cmd/nft/iptables）+ `--json` 模式 exit 码 0/1/2 供 CI + 22 tests (19 unit + 3 E2E)。**PR2 ⏳**：`cc pair init` / `cc pair accept` headless 模式（无 Electron UI 路径，CI runner + SSH 公司开发机）。**PR3 ⏳**：systemd unit template + Linux setup docs。GA 反馈决定 PR2/PR3 优先级
