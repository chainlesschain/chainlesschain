...
> **版本**: v0.4 (QRPairing audit + M4 D2 入账, 2026-05-11) | **状态**: 🟡 ADR 8/8 accepted · M1/M5 ✅ · M2 ✅ D1-D3 / 🟠 D4 scaffold-only · M3/M4 部分 · M6/M7 ⏳ | **关联**: Android v0.37.0 → v1.0.0 | **桌面对标**: v5.0.3.47
>
...
>
> v0.3 → v0.4 变更：(1) **QRPairing audit 揭示 M2 D4 是 scaffold-only 而非 ⚠️ 部分** — 2034 行 UI scaffold 真实，但 `PairingViewModel` 是假 `delay()` stub、`DevicePairingScreen` route 从未接进 app NavGraph、`feature-p2p/ui/QRCodeScannerScreen.kt` wire 到 Safety Numbers 而非 pairing JSON shape。状态从 ⚠️ 改成 🟠 scaffold-only。真落地（~2.5d）推到 v1.1，与 Q4 桌面 Mobile Bridge 面板一起做。(2) M4 D2 ApprovalUI Android 端落地 `eb7489bc4`（596 行 + 22 行 MainActivity）。事实声明未再变更。
...
| 文件                                                          | 状态 | 职责                                  |
| ------------------------------------------------------------- | ---- | ------------------------------------- |
| `desktop-app-vue/src/main/remote/handlers/mobile-skill-whitelist.js` | ⏳ 新 | exposeRemoteSkills 校验 + 拒绝日志（Phase D2） |
| `desktop-app-vue/src/main/remote/handlers/mobile-approval-channel.js`| ⏳ 新| 高风险 skill 的桌面发起 / 手机响应路由（Phase D3） |
| `desktop-app-vue/src/main/p2p/MobileSignClient.js` (及 IPC)    | ✅ 新 | M5 `6d482d066` 桌面端反向签名客户端，配 Android `SignAsService` |
...
| **M4** | 2 天    | L3 RemoteSkillRegistry + 桌面白名单 + ApprovalUI              | ⚠️ 部分（D1 RemoteSkillRegistry `df61914ff` + D2 Android ApprovalUI `eb7489bc4`；剩桌面 whitelist + approval-channel） | 55 单测 + 桌面白名单严格生效 + 审批 E2E |
...
|        | **共 11 天**（v0.1 是 10 天，M3 +1 天） |                                       | **v1.0 已落 ≈ 7 天有效工**（含 M4 D2 Android ApprovalUI `eb7489bc4`）；**v1.0 剩 ≈ 5 天**（M3 剩 D-voice/D-cam/D-push + D-share/D-loc Manifest 收尾 / M4 剩桌面 whitelist+approval-channel / M6 性能验收 / M7 发版）；**QRPairing 真落地 ~2.5d 推 v1.1**，与 §8.5 桌面 Mobile Bridge 面板合并 | |
...
- D1: ⚠️ 部分 RemoteSkillRegistry — `df61914ff`（23-skill seed，file-level granularity）；剩 method-level 元数据补全 + 桌面 mobile-skill-whitelist.js + 单测（30）
- D2: ⚠️ 部分 Android ApprovalUI 已落 — `eb7489bc4`（`AndroidApprovalGate` 104 行 + `ApprovalDialogHost` 263 行 + Hilt DI 23 行 + 单测 184 行 + MainActivity +22 行）；剩桌面 `mobile-approval-channel.js` router + E2E（25 + E2E）
