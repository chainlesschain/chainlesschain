...
> **版本**: v0.12 (§10 B.1 PR1 landed + Unified KeyStore 准入条件重评，2026-05-15) | **状态**: 🎉 ADR 8/8 accepted · M1/M2/M5 ✅ · **M3 5/5 code 全落（VoiceMode + CameraOCR + LocationTagger + ShareReceiver + PushNotifier，+3,861 行 / 99 单测，真机待用户出场）** · **M4 D1 ✅ method-level / D2 ✅ desktop-side full + Android RPC 接收器 + **ApprovalUI 4 类 category** + **ProgressViewer***（真机 E2E 仍待） · M6 性能预算文档就位待真机回填 · M7 GA flip ✅ tag `v1.0.0` + GitHub Release published | **关联**: Android **v1.0.0 GA** | **桌面对标**: v5.0.3.48
>
...
- **B.1（P1）web-shell private key signing UI** 🟢 PR1 ✅ landed (2026-05-15 [`B1_WebShell_Multisig_Sign_spike.md`](B1_WebShell_Multisig_Sign_spike.md))：**准入条件重评** — Unified KeyStore infra 已 ready（`unified-key-manager.js` BIP-32 派生 + `unified_keys` SQLite + 9 配套 ukey modules + `ukey:sign` IPC，2026-05-15 audit），原 "v1.2 还没收口" framing 不成立。**PR1 ✅**：`MultisigSigner` middleware（4 sources: hex / path / ukey / unified）+ web-shell `multisig.sign` topic（INVALID_KEY / UKEY_NOT_WIRED / UNIFIED_NOT_IMPLEMENTED 等错码软化成 `{accepted:false, reason}`）+ 31 unit tests。**PR2 ⏳**：core-multisig `signWithExternal` API + ukeySigner 真 wiring + `SignProposalModal.vue`（payload hash 短码 per A.2 §2.4.c）。**PR3 ⏳**：`unified_keys` 加 `did` 列 + `findKeyForDid` + CLI `--keystore` flag
