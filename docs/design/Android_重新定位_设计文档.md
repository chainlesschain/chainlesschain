...
> **版本**: v0.15 (§10 C.1 PR2 landed — phone-side wear→phone voice listener 链路收口，2026-05-15) | **状态**: 🎉 ADR 8/8 accepted · M1/M2/M5 ✅ · **M3 5/5 code 全落（VoiceMode + CameraOCR + LocationTagger + ShareReceiver + PushNotifier，+3,861 行 / 99 单测，真机待用户出场）** · **M4 D1 ✅ method-level / D2 ✅ desktop-side full + Android RPC 接收器 + **ApprovalUI 4 类 category** + **ProgressViewer***（真机 E2E 仍待） · M6 性能预算文档就位待真机回填 · M7 GA flip ✅ tag `v1.0.0` + GitHub Release published | **关联**: Android **v1.0.0 GA** | **桌面对标**: v5.0.3.48
>
...
- **C.1（P1）watch face → VoiceMode shortcut** 🟢 PR1 ✅ + PR2 ✅ landed (2026-05-15 [`C1_WatchFace_VoiceMode_spike.md`](C1_WatchFace_VoiceMode_spike.md))：**准入条件重评** — 原 framing "Auto Phase 1 voice intent 抽出 generic" 不准确 (`cc.voice.start` 仓库内 0 匹配；Auto VoiceMode 用 `androidx.car.app` Screen API 不是 Intent；phone VoiceModeScreen 完全孤儿，无 NavGraph 注册)。**真实 scope = 从零建立** phone-side intent + NavGraph 路由 + wear→phone Data Layer 桥 (3 PR)。**PR1 ✅**：`VoiceLaunchActions.ACTION_START_VOICE_MODE` Android Intent constant + `VoiceTriggerSource` 4-enum + `Screen.VoiceMode` route + NavGraph composable + MainActivity 处理 + manifest `<intent-filter>` + 15 Robolectric unit tests。**PR2 ✅**：`CcPhoneVoiceListener` WearableListenerService 监听 **精确路径** `/cc/voice/start` → `VoiceForwardWire` JSON 解析 → `startVoiceActivity` 走 VoiceLaunchActions.buildIntent → `startActivity` + 11 Robolectric tests。安全锁：payload `trigger_source` 不可信，源固定 WEAR_FORWARD 防 wear-side forge。phone-side 链路收口。**PR3 ⏳**：wear 端 `WearVoiceSender` + tile/complication "Voice" entry point (A.2 §3.1 大按钮+vibration)
