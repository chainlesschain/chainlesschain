...
> **版本**: v0.18 (§10 A.1 PR2 landed — cc pair token 预生成 issuer/manager，2026-05-15) | **状态**: 🎉 ADR 8/8 accepted · M1/M2/M5 ✅ · **M3 5/5 code 全落（VoiceMode + CameraOCR + LocationTagger + ShareReceiver + PushNotifier，+3,861 行 / 99 单测，真机待用户出场）** · **M4 D1 ✅ method-level / D2 ✅ desktop-side full + Android RPC 接收器 + **ApprovalUI 4 类 category** + **ProgressViewer***（真机 E2E 仍待） · M6 性能预算文档就位待真机回填 · M7 GA flip ✅ tag `v1.0.0` + GitHub Release published | **关联**: Android **v1.0.0 GA** | **桌面对标**: v5.0.3.48
>
...
- **A.1（P1）桌面端 Linux native 配对** 🟢 PR1 ✅ + PR2 ✅ landed (2026-05-15 [`A1_Linux_Native_Pairing_spike.md`](A1_Linux_Native_Pairing_spike.md) v0.2)：**准入条件重评** — 原 framing "Linux 需补 mDNS systemd 单元 + Wayland 屏幕权限" 大部分不准确（`@libp2p/mdns` + `bonjour-service` 是纯 JS 不依赖 avahi-daemon；配对 UI 不抓屏）。**真缺口** = preflight 诊断 + token 预生成 + docs。**PR1 ✅**：`cc pair preflight` 5 项检查（interfaces / multicast bind / port 5353 / distro / firewall hint）+ 22 tests。**PR2 ✅**：`cc pair token generate/list/show/revoke` subcommand 群 + `lan-pairing-tokens.js` lib + 持久化 `~/.chainlesschain/pairing-tokens.json` + one-active-token-per-DID 不变量 + 兼容现 `device-pairing-handler.js` QR shape + 35 tests (26 unit + 9 E2E)。**PR3 ⏳**：systemd unit template + Linux setup docs。**Full headless signaling listener** (实际跑 WS + libp2p) 改为 PR3+ 评估，工作量大且 GA 反馈优先级敏感；PR1+PR2 已覆盖 CI/SSH 主场景 80%。GA 反馈决定 PR3+ 优先级
