...
> **版本**: v0.2 (调研收口稿, 2026-05-11) | **状态**: 📋 RFC 评审中 | **关联**: Android v0.37.0 → v1.0.0 | **桌面对标**: v5.0.3.47
>
...
>
> v0.1 → v0.2 变更：基于 2026-05-11 二轮代码核实修正多处与实际不符的事实声明（mobile-app/ 不存在 / ukey 半成品不存在 / cowork+workflow 已是真代码非 stub / REMOTE 实际 23 commands 而非 8 / 资源类型实际 6 而非 5 等）。原 §8.1 §8.2 副线、ADR-5、M1 stub 处置工时全部更新为对应真状态。
...
| 模块                  | 职责                                        | 后端                                | 现状                             | 单测目标 |
| --------------------- | ------------------------------------------- | ----------------------------------- | -------------------------------- | -------- |
| `StrongBoxKeyManager` | Ed25519 密钥生成 / 签名，私钥不出 TEE       | Android Keystore + StrongBox tier   | ⏳ 新建（当前 AndroidDIDKeyStore 是 Keystore 但无 StrongBox 显式 tier） | 25 |
| `DIDWallet`           | DID 文档构建 / 多身份切换 / 助记词备份导入  | StrongBoxKeyManager + EncryptedFile | ⏳ 部分（DIDManager 已有单 DID；多 DID 切换 + 助记词备份新建）          | 30 |
| `BiometricGate`       | 解锁前置门，敏感操作二次确认                | androidx.biometric                  | ⏳ 部分（USE_BIOMETRIC 权限 + Settings flag 已有；统一 Gate 新建）       | 12 |
| `QRPairing`           | 扫码生成 P2P 配对码 / 扫码登录授权          | ZXing + DevicePairingHandler        | ⚠️ 桌面侧 device-pairing-handler.js 已有；Android 侧扫码 UI 状态未深入核实 | 18 |
| `SignAsService`       | 暴露 `sign.request` REMOTE 给桌面调用       | RemoteCommandClient 反向通道        | ⏳ 新建                                                                  | 14 |
...
| `LocationTagger` | GPS + 时间戳附加到笔记 / DID 凭证        | ⏳ 全新建（无 ACCESS_FINE_LOCATION 权限，无任何 Location 代码） | Android → Desktop (sync.push) |
| `PushNotifier`   | FCM + 本地 channel，4 类通知             | ⏳ 全新建（无 google-services.json，无 FCM SDK）           | Desktop → Android (推送)       |
| `ShareReceiver`  | Intent.ACTION_SEND 接收，落 Inbox        | ⏳ 全新建（Manifest 无 ACTION_SEND intent-filter）         | local → 队列 → sync.push       |

### 5.4 L3 REMOTE 层模块

| 模块                  | 职责                                                    | 现状                                                       |
| --------------------- | ------------------------------------------------------- | ---------------------------------------------------------- |
| `RemoteSkillRegistry` | 维护可远程调用的 skill 清单 + 元数据，开机从桌面拉取    | ⏳ 新建（当前 23 commands + 18 ui 子模块 hardcoded，无注册）|
...
| 文件                                                          | 状态 | 职责                                  |
| ------------------------------------------------------------- | ---- | ------------------------------------- |
| `desktop-app-vue/src/main/remote/handlers/mobile-skill-whitelist.js` | ⏳ 新 | exposeRemoteSkills 校验 + 拒绝日志    |
| `desktop-app-vue/src/main/remote/handlers/mobile-approval-channel.js`| ⏳ 新| 高风险 skill 的桌面发起 / 手机响应路由 |
| `desktop-app-vue/src/main/remote/command-router.js`            | ✅ 改 | 加 sign.request 反向通道 + whitelist 前置 |
| `desktop-app-vue/src/main/remote/remote-gateway.js`            | ✅ 改 | 接 mobileBridge 配置                    |
| `desktop-app-vue/src/main/config/unified-config-manager.js`   | ✅ 改 | 加 mobileBridge.* 配置字段            |
...
| 里程碑 | 工期    | 目标                                                         | 验收                                  |
| ------ | ------- | ------------------------------------------------------------ | ------------------------------------- |
| **M1** | 0.5 天  | 文档评审 + REMOTE 23 commands 现状梳理 + ADR-5 能力对照表落地 | RFC 通过 + 23 commands 注册元数据 PR 合并（不再有 v0.1 stub 处置工时） |
| **M2** | 3 天    | L1 钥匙层：StrongBox tier + DIDWallet 多身份+助记词 + BiometricGate + QRPairing 收敛 | 67 单测 + E2E 首启动落 DID            |
| **M3** | 3 天    | L2 五件：Voice Mode + CameraOCR + LocationTagger + PushNotifier + ShareReceiver（v0.1 漏列后三件，工期 +1 天） | 80+ 单测 + 5 件 E2E + Manifest 权限/intent-filter 改造 |
...
|        | **共 11 天**（v0.1 是 10 天，M3 +1 天） |                                       |                                       |

### M2 拆分（3 天）

- D1: StrongBoxKeyManager（与 AndroidDIDKeyStore 整合，加 tier 判定）+ 单测（25）
- D2: DIDWallet（多身份 + 助记词 + 备份导入）+ 单测（30）
- D3: BiometricGate（统一现有零散 Settings flag）+ QRPairing 收敛 + E2E 首启动

### M3 拆分（3 天）

- D1: VoiceMode 串联（已有 ASR + TTS 端点）+ CameraOCR 流水线
- D2: LocationTagger（含 Manifest 权限 + Foreground Service）+ ShareReceiver（含 intent-filter）
- D3: PushNotifier（FCM 集成 + 本地 channel 兜底）+ 5 件 E2E

### M4 拆分（2 天）

- D1: RemoteSkillRegistry + 23 commands 元数据补全 + 桌面 mobile-skill-whitelist.js + 单测（30）
