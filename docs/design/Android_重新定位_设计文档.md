...
**v1.3+ 影响**：无代码改动。已有移动端配置（如 BiometricPrompt 开关）若散落在其他存储位置，可在 v1.3+ 顺手迁到 `user_settings.mobile.*`，opportunistic 不必预防性。
...
- v1.3+ 行动：预留 `SkillMetadata.signature: String? = null` forward-compat 字段 + verifier no-op stub（~0.3d）；真验签 deferred 到 marketplace M0 上线后

**v1.3+ 影响**：行动项 AI-3 + AI-5 见 [ADR Review v2.0](Android_ADR_重评估_v2.0.md) §4。
...
| **P1 主体** | v1.3 主 scope，GA 反馈引导内部优先级 | A.1、A.2、B.1、B.5、C.1 |
| **P2 候选** | 容量风险大 / 依赖未明 / 基础设施投资 / 可推下个 milestone | B.3、B.4、C.2、C.3 |
...
- **B.4 + B.5 容量互斥**：D 列两项各可独立成 milestone，v1.3 scope 框可能仅容其一
...
- **A.2（P1）跨手机/手表/桌面三端 UI consistency 设计文档**：v1.2 让三端同时存在，但各 surface 风格独立（Android Compose Material3 / Wear Compose-for-Wear / Desktop V6 Claude-Desktop shell + Vue3 Ant Design）。需文档明确 *必须一致*（语义颜色 / 高风险红 / DID 短显示规则 / m-of-n 进度展示） vs *必须不同*（手表大按钮 / 桌面侧栏 / 车载语音 only）。GA 用户反馈决定哪些"必须一致"的边界
- **A.3（P0 前置）v2.0 重新评估 ADR**：8 个 ADR（ADR-1 ~ ADR-8）实施一年后回看，**v1.3 任何 ADR 相关技术决策开工前必须先做**。详 [Android_ADR_重评估_v2.0.md](Android_ADR_重评估_v2.0.md)（2026-05-12 v1.0 收稿）：5 keep / 2 amend (ADR-7 / ADR-8) / 1 revise (ADR-2，blocks B.3)：
...
- **B.1（P1，外部前置）web-shell private key signing UI**：v1.2 `/multisig` 只能列 + 取消 + 执行已达阈提案，*不能本地签*（私钥在 desktop main 进程 / U-Key 硬件，渲染层拿不到）。需先接通 Unified KeyStore（v1.2 还没收口），再加 SignProposalModal
- **B.2（P0 前置，纯技术）削 cc subprocess 冷启**：v1.2 web-shell `/multisig` 走 `ws.execute("cc multisig …")` 子进程，asar:true 后冷启 6-10s（见 memory `desktop_release_b4_surgery_lessons.md`）。改 in-process WS handlers 直接调 `@chainlesschain/core-multisig` JS API（v0.1.0 已 publish `912617cc2`，可独立做不依赖反馈）
...
- **B.4（P2，spike）air-gapped QR signing**：U-Key 离线机签字。手机/桌面 export 提案为 QR → 离线机扫描签字 → 输出 QR → 重扫入链。比较硬，列在这里要先开 spike 估准工，不必 v1.3 做完。与 B.5 容量互斥
- **B.5（P1）跨链桥 outbound 接入**：v1.2 多签只签内部 marketplace consume；接入 EVM/Cosmos outbound 后 m-of-n 直接成跨链 gateway governance 层（marketing 价值 > air-gapped，优先级建议更高）。**与 B.4 容量互斥**，v1.3 框入此项则 B.4 留 spike
- **B.6（P0 前置，纯技术）PQC 严格模式**：MTC v0.11 已支持 Ed25519+SLH-DSA hybrid（memory `mtc_landing_v0_11.md`）。严格模式 = 拒收纯 Ed25519，强制每个 partial signature 必带 SLH-DSA 段（受 NIST 监管类客户欢迎，代价签名包 +几 KB）。hybrid 已落，严格模式纯技术活，可独立做
...
- v1.3 与 **v2.0 ADR 重评估** 互相约束：若 ADR-2 决定砍 TEE 中间层，v1.3 m-of-n DID rotate 的 attestation 格式可能需重设计（**A.3 → B.3 依赖链**）
- 跨链桥 outbound + air-gapped 两项足够长，每项都可能独立长成 milestone；v1.3 scope 框可能仅容其一（**B.4/B.5 互斥**）
- **P0 前置 3 项（A.3 / B.2 / B.6）不依赖 GA 反馈，可在 v1.2 GA 上架审核期并行启动**，不占 v1.3 主 scope 工期
- v1.3 milestone **暂不立即开 issue** — 等 v1.2（[#20](https://github.com/chainlesschain/chainlesschain/issues/20)）close 时再依本节起 [#21](https://github.com/chainlesschain/chainlesschain/issues/21)
