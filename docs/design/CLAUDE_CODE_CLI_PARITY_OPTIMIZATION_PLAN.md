...
| 4     | Agent Team      | **🟡 安全扩容基础，未闭合** | lease+DAG + `cc team` 真执行 + v5 authority 恢复 + fenced lease + 10k task/64 worker + 两阶段 worktree cleanup                                                                                                                                              | IDE 人工接管、分布式队列、长期 soak、通用 side-effect adjudication、依赖分支基线传递                                                                                                                                                                          |
...
**结论**：Phase 6 完全闭合；Phase 4 已完成安全扩容基础但仍未闭合。1、2、3、5、7 的
Windows-doable 主体已落地并测试；剩余项既包括其它 OS 内核、各语言 toolchain、真机三端、
活模型和 Docker 等环境门，也包括 Phase 4 的 IDE 人工接管、分布式队列、长期 soak、通用
side-effect adjudication 与依赖分支基线传递。
...
>
> **3.4 已落地（远程 registry/manifest source + 私有仓认证 + 离线 seed cache，2026-07-05）**：`lib/plugin-runtime/remote-source.js` —— `cc plugin add` 现接受**远程 registry/manifest URL**（`https?://….json`）。两种形状：多插件索引（`{plugins:[{name,source,ref,version,sha256,description}]}`，按 `--name` 选）或单插件 manifest（顶层 `source`）。**远程层是纯 INDIRECTION**：把 name → git source 串（`source#ref`）交给既有 `installFromSource` clone，**不碰同步 install 核心、不引 tar 解包器**。**离线 seed cache**：拉到的 registry JSON 按 URL sha256 内容寻址缓存（`<userData>/plugin-registry-cache/`），网络/HTTP 失败自动回退缓存（`fromCache` 标记 + 可见 warning），离线仍可 browse/装曾见过的 registry。**私有仓认证**：Bearer token 从 `--token` / `CC_PLUGIN_REGISTRY_TOKEN` env / `config.plugins.registryTokens[host]` 三级解析。entry `sha256` 顺带成安装完整性校验（复用 `--sha256` 签名路径）。新 `cc plugin browse [query] --registry <url>` 发现命令（列/过滤远程插件）。18 单测（注入 fetch+fs：形状校验/单插件包装/按名选/离线回退/HTTP 错/auth 头/token 三级/内容寻址）+ **真机 E2E**（本地 HTTP registry → `browse` 列出 → `add toml-tools --registry <url> --scope local` 拉 git → 装 → 插件 skill 现于 `cc skill list [plugin]`；停服 → `browse` 回退缓存；auth server 无 token→401 干净报错、`--token`→成功）。**局限**：仅 git-indirection artifact（tarball 直载 defer，git 覆盖主场景）；registry URL 用户显式指定故不设 SSRF 私网封禁（私有内网 registry 是正当用例）。\***\*3.z 已落地（签名锁 manifest 路径穿越守卫，2026-07-06，安全 bugfix）**：`plugin-runtime/signature.js` `verifyInstalledSignature`（gate 载入期 `requireSignedPlugins`）原用 `path.join(plugin.root, lock.manifest)` 后直接 re-hash 比对——但 `lock.manifest` 来自**可写**插件目录里的 `.plugin-lock.json`（未签名），攻击者可令 `manifest: "../../secret"` + 设 `sha256` 匹配那个越界文件 → **re-hash 越界文件命中 → 伪造 signed:true**（node 真机复现 pre-fix signed=true）。修 = 加边界校验（`path.relative(resolve(root), resolve(manifestFile))`，`..`/绝对/空 即拒 `signed:false`——1.w sandbox-fs-policy 同 pattern），插件 manifest 恒在自身 root 内故合法嵌套子目录 manifest 仍过。2 单测（越界拒 + 嵌套子目录不误拒）+ 既有 129 plugin-runtime 全绿。**（注：签名锁本身未签名=已文档化的完整性锚而非供应链封印，靠不可变版本目录兜底；此守卫堵的是越界读这一具体洞。）** **3.y 已落地（inline hooks/monitors 组件被静默丢弃，2026-07-06，bugfix）**：manifest 解析器支持组件在 `plugin.json` 里**内联**声明（返 `{inline:true, file:null, ...}` 无 `absPath`，因归一化只留 count/names）；`mcp.js` 采集器正确处理 inline（重读 `manifestPath` 取实际 server 体），但 `hooks.js`/`monitors.js` 采集器要求 `!absPath → continue`，故**内联声明的 hooks/monitors 被静默丢弃永不触发/spawn**（fail-closed 功能缺口非安全洞，且与 mcp.js 不对称）。修 = 两采集器补 inline 分支重读 `manifestPath` 取 `raw.hooks`/`raw.monitors`（镜像 mcp.js）。2 单测（inline hooks 触发 / inline monitors spawn，无 fix 则跳过=空）+ 既有 131 plugin-runtime 绿。**待完成**：tarball artifact 直载（niche）。**Phase 3 全部工作项 + 4 条验收全落地**（8/8 组件 + 安装生命周期 add/upgrade/uninstall/rollback + \*\*本地/git/GitHub/远程 registry 四类 source\*\* + 两道 fail-closed 加载 gate org-policy/requireSignedPlugins + trust 模型 + 热加载 + monitor 每轮注入 + 私有仓认证 + 离线缓存）。
...
> 状态：**🟡 安全扩容基础已落地，整体未闭合（更新于 2026-07-29）** —— 4.1–4.4 的早期能力仍在，但 P2-16 复审后不再以“全部闭合”表述。当前增加 10k task / 64 worker、fenced lease、严格 v5 resume authority、append-only governance journal、预算 reservation、run-scoped worktree OID 与两阶段 cleanup。仍缺 IDE 人工接管、跨进程分布式队列、长期 soak、通用 side-effect adjudication 与依赖分支基线传递。
>
...
>
> **4.4 基础能力已落地并在 P2-16 加固（更新于 2026-07-29）**：`TeamBudget` 继续提供 maxTasks/maxTokens/maxUsd/maxWallMs 四维团队预算；恢复只允许收紧 cap，并保留已消耗 active wall time而不计进程停机时间。mailbox 使用有界队列/backpressure，TeamRunner 提供 teammate 生命周期和显式非幂等重试 opt-in；崩溃后只有 dry-run 或显式 `retrySafe: true` 的任务会自动重领，其他真实任务停止并要求裁决。启用 token/USD cap 时，usage 缺失或远端模型无法定价会 fail closed。`--agent --worktree` 的真实并行执行强制 worktree，恢复状态升级为严格 v5 authority；v2–v4 不直接恢复。worktree 记录 task commit OID、preview/merge 状态，并以 prepare→persist→remove→persist 清理。`--state` 是未签名可信控制面，真实执行必须放在 Agent 可写 repo 外；journal seq+digest 是一致性锚点而非认证。该阶段是安全扩容基础，不等于生产级大规模 Agent Teams 完成。
>
...
> 更新判读（2026-07-29）：Sandbox 与 Remote Control 的既有结论保留；Agent Team 不再标记“全绿”。P2-16 已完成 10k task/64 worker、安全预算、fenced lease、严格 v5 authority、append-only journal 和两阶段 worktree cleanup 的基础加固，但仍缺 IDE 人工接管、分布式队列、长期 soak、通用 side-effect adjudication 与依赖分支基线传递。因此当前未全绿项不止环境阻塞，Agent Team 仍是 🟡。
