...
见正文「1. 背景与目标」。Plan C 用 signaling-forward（信令转发）实现 Android 远程操控桌面，含 LAN → 中继 fallback，已落地 v5.0.3.50。

### 2. 核心特性

signaling-forward 命令路径；LAN → 中继 fallback；命令 req/res 协议；三段位最底层。

### 3. 系统架构

见正文「2. 数据流与组件」（Android 新增 / 修改组件 + Desktop 新增 / 修改组件）。

### 4. 系统定位

Android 远程操控的**信令转发底座**（Plan C，A+B 在其上加 WebRTC）。

### 5. 核心功能

见正文「3. 协议」：命令请求（Android→Desktop）/ 响应（Desktop→Android）/ LAN→中继 fallback。

### 6. 技术架构

signaling 转发；org.json；relay 双发；handlePairAckFromRelay。

### 7. 系统特点

落地 v5.0.3.50；后续 Plan A+B（WebRTC）在此之上升级。

### 8. 应用场景

Android 远程操控桌面命令（剪贴板 / 文件 / 截屏 / 系统信息等 REMOTE 命令）。

### 9. 竞品对比

见 `Android_Remote_Operate_Plan_AB.md`（WebRTC P2P 升级）。

### 10. 配置参考

signaling 中继地址见 `Android_Remote_Operate_Plan_AB.md` §9 部署位置。

### 11. 性能指标

LAN 直连优先，失败回退中继（见正文 §3.3）。

### 12. 测试覆盖

见正文 §7 相关 commits（含 scan-test assertEquals + web-panel JSON parser）。

### 13. 安全考虑

命令路径基于配对信任；destructive action 需 approval（见 `mobile-approval-channel.js`）。

### 14. 故障排除

连接 DISCONNECTED / pair ack 丢失 → 见 commits `8073edc62`（relay 双发 + handlePairAckFromRelay 修复）。

### 15. 关键文件

Android RelayClient；Desktop signaling handler；web-panel JSON parser。
...
见正文头部关联：`Android_W3_Pairing_E2E.md`、`Android_REMOTE_commands_inventory.md`、`Android_重新定位_设计文档.md`。
