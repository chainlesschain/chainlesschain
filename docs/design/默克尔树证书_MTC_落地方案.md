...
- [x] **P3.3 federation 服务发现（commit `aa13e07a9`，2026-05-02）**：`core-mtc/lib/federation.js` 定义 `mtc-federation-announce/v1` schema，`createMemberAnnounce` 自签 + `verifyMemberAnnounce` 校验签名/TTL/JWK 一致性，`FederationAnnounceCache` TTL-evicting 名册。CLI `cc mtc federation discover <fed-id> --drop-zone <dir> [--member-id <m>] [--ttl <s>] [--once]` 通过共享文件系统（NFS / Syncthing / SMB / USB）实现真跨进程发现：每成员定期写自签 announce 到 `<drop-zone>/federation-announces/<fed-id>/`，其他节点扫描 + 验证 + 入 cache，TTL/3 间隔 re-announce。Listen-only 模式（不带 --member-id）让 verifier 节点纯订阅。+15 core-mtc 单测 + 7 跨进程集成测试（双节点共享 drop-zone 互发现、SLH-DSA 异构、tamper rejection）。
  - **真 libp2p gossipsub auto-discovery 留作 Phase 3.4**：announce schema + verify + cache 都 transport-agnostic，把 filesystem drop-zone 替换成 gossipsub topic `mtc-federation/v1/<fed-id>` 是纯交付层改动。当前 filesystem 模式已覆盖 LAN / 共享 fs / 离线场景，生产用途够用。
