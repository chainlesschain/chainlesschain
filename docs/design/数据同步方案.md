...
| WS topic | 主进程 handler | 用途 |
|---|---|---|
| `sync.status` | sync-status-handlers.js | 直接 raw SQL 查 `cli_sync_state` / `cli_sync_conflicts` 计数 |
| `sync.push` / `sync.pull` | 同上 | 调 sync-manager.js (ESM) pushResources/pullResources |
| `sync.conflicts` / `sync.resolve` | 同上 | 列出 + 解决冲突（local/remote 两策略） |
| `sync.webdav.{test,run,config-get,config-set,config-clear}` | sync-webdav-handlers.js | WebDAV 凭证 + 一轮同步 |
| `git.config-{get,set,clear}` | git-config-handlers.js | Git 仓库配置 |
