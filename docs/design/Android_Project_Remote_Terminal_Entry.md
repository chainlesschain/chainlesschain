...
桌面有 `cc` CLI（172 commands），Android 端没有，也不该有 —— `cc` 依赖 Node.js / Electron / Ollama / Qdrant / Postgres，移动端跑不动。但用户在外面想做 `git rebase` / `npm install` / `cc skill` / `cc workflow run` 类工作时，没有出口。
