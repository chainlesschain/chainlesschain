...
> 本文为 ADR 重评估文档。为对齐项目文档标准结构，下列章节以 `见正文` 指引或简述方式补齐若干视角，不重复正文细节。

### 1. 概述

见正文「0. 目的与范围」。Android ADR 重评估 v2.0（#21 v1.3+ A.3，blocks B.3 DID rotate）：对 `Android_重新定位_设计文档` 8 ADR 全 audit，5 keep / 2 amend / 1 revise，产出 3 action items。

### 2. 核心特性

8 ADR 逐项重评；keep/amend/revise 分类；下游影响分析；action items 入 v1.3 队列。

### 3. 系统架构

见 `Android_重新定位_设计文档` §4 决策（ADR-1~ADR-8）。

### 4. 系统定位

Android 三层定位 8 ADR 的**重评估 / triage 文档**（#21 P0 前置）。

### 5. 核心功能

见正文 1–5：总览 / 逐 ADR 详评 / 下游影响 / action items / 待触发项。

### 6. 技术架构

ADR triage 分层（设计文档 v0.8 §10）；与 #21 任务队列联动。

### 7. 系统特点

P0 前置（blocks B.3 DID rotate）；部分 ADR 待 GA 反馈 / 外部触发（见正文 5）。
...
—（ADR 文档）；keep/amend/revise 决策见正文 2。

### 10. 配置参考

ADR-2 决策依赖 Play Console user agent / API level 分布（见正文 5）。
...
—（设计决策文档）；action items 落 v1.3 实现期验证。

### 13. 安全考虑

ADR-8 signed manifest（marketplace M0 触发后 ~1 周完成签名链路，见正文 5）。

### 14. 故障排除

—；下游影响与 action items 见正文 3/4。

### 15. 关键文件

`Android_重新定位_设计文档.md`（被评估的 8 ADR 源）。
...
`Android_重新定位_设计文档.md` §4 ADR、#21 任务队列。
