...
本批实现已重放到 `github/main` 的 `a1860ae5fd`；下表提交号均为 rebase 后的新提交号。审计快照仍保留文首固定 SHA，用于说明最初差距判断的代码基线。

| 工作包                                                                              | 当前结论                                                                                                                                              | 已提交与本地验证                                                                                                          | 尚未完成/发布门禁                                                                       |
| ----------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| P1-1 MCP resource subscription 重连恢复                                             | 该残余项本地完成；desired subscription 可跨重连恢复，失败后再次连接及 `disconnectAll` 清理均有回归                                                    | `9a5e7ba6d4`；rebase 后聚焦 fake transport 19/19                                                                          | P1-1 的进度/空闲超时、disabled server、结构化错误和 discovery cache 仍是独立未完成项    |
...
| P0-1 Remote Control 默认姿态与 IDE 兼容                                             | 本地完成；默认 loopback，LAN 需显式 opt-in，无 relay 不静默回退 LAN；补迁移告警、失败清理、地址边界、旧 state 提示及 VS Code/JetBrains fail-closed UI | `e7fa206afe`；rebase 后 Remote 核心 105/105、headless/stream 132/132、VS Code 120/120；JetBrains 本地复验受缺 JDK 21 阻塞 | 仍须在精确提交 SHA 上通过 CLI/IDE 的 GitHub Actions 矩阵；生产 relay 不属于本地完成范围 |
| P0-2 插件 canonical identity/managed policy                                         | 本地实现与对抗回归完成；已闭合 authority/ref/path/registry 间接源、零 I/O gate、redirect 与错误脱敏边界                                               | `cf50236b9c`；rebase 后精确门禁集 165/165，制品读回 63 个通过、1 个跳过；独立终审无阻断                                   | 仍须在同一精确提交上通过 CLI CI、CLI Strict Sandbox 与供应链 Actions                    |
