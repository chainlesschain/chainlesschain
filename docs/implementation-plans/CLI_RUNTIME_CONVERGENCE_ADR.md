...
- ✅ Phase 6 已完成: 历史 lib 实体已退化为兼容 re-export shim,canonical 实现归入 `runtime/`、`gateways/ws/`、`harness/`

**2026-07-03 推进小结(Phase 7 完成):** Phase 3–6b 的反向迁移保持完成；`doctor --json` / `status --json` 稳定 schema (`chainlesschain.doctor.v1` / `chainlesschain.status.v1`) 已落地。Phase 7 已具备 deterministic mock provider、agent loop / session resume / envelope bridge / MCP / plugin / approval / shell policy / worktree parity 测试，并新增文件化 golden transcript 与跨平台规范化器。golden 合同会剥离事件 ID、时间、序号和遥测旁路字段，统一 Windows/POSIX 工作区路径，避免非行为差异造成快照漂移。
...
### 1. 概述
见正文头部。本文：ADR: CLI Runtime 收口边界冻结。
