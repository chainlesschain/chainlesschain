...
> **📦 CLI install**: `npm i -g chainlesschain@0.163.5` (current fully gated build; aliases `cc` / `clc` / `clchain`).
...
>
> ```bash
> npm i -g chainlesschain@0.163.5 --registry https://registry.npmjs.org
> ```
>
...
## 2026-08-12 Current mainline — **v5.0.3.135 / CLI 0.163.5 / PDH 0.4.57 / Open VSX 0.37.49 / JetBrains 0.4.85**

> **Release status:** `chainlesschain@0.163.5` is the current npm `latest` and production recommendation. Immutable tag `v-npm-0-163-5` resolves to exact commit [`095087c1e8`](https://github.com/chainlesschain/chainlesschain/commit/095087c1e859a8451ce01ed58c59af3fede756fd); that SHA passed [CLI CI](https://github.com/chainlesschain/chainlesschain/actions/runs/31509337185), [CLI Strict Sandbox](https://github.com/chainlesschain/chainlesschain/actions/runs/31509336854), the [dedicated npm release](https://github.com/chainlesschain/chainlesschain/actions/runs/31509336832), and [independent public readback](https://github.com/chainlesschain/chainlesschain/actions/runs/31514940240). npm now reports `latest=0.163.5`; the public tarball SHA-1 is `5dc6677dee6d1d73b708e6a50e3808007a314894`.
>
> **CLI 0.163.5:** the unified scheduler runtime now owns Routine manual/cron/once, Agenda wakeup/cron, and Cowork Cron execution. Its versioned SQLite store, definition snapshots, expected-revision CAS, logical-occurrence deduplication, owner/fence claims, lease heartbeat, bounded retry, dead-letter history, and terminal-evidence recovery prevent duplicate execution between cooperating drivers. Only verifiable terminal evidence may be settled after a crash; start-only or post-side-effect persistence failures become outcome-unknown, so this release does not claim global exactly-once behavior. Micro-compaction checkpoints are also durable across REPL/agent recovery.
>
> **Current source candidate:** this branch's [`15a641fa85`](https://github.com/chainlesschain/chainlesschain/commit/15a641fa85) additionally routes Automation cron `run-scheduled` through the same kernel, binding flow snapshots and authority while recovering completed deterministic execution IDs. It is not in `github/main` or `v-npm-0-163-5`, so it is not part of the npm installation contract. Agenda monitor, Loop, Routine GitHub, a standalone scheduler daemon, shared permission/budget resolution, and complete IANA timezone/DST/missed-run behavior remain future work.
>
> **Native boundary:** the closed npm `0.163.5` release does not imply a signed Desktop/native release. Unsigned six-target validation remains fixed to `signed=false` and `releaseEligible=false`; Linux signing, Windows Authenticode, macOS signing/notarization, updater keys, and public signed fresh-install/upgrade/rollback readback remain open.
>
...
>
> **IDE mainline:** Open VSX `0.37.49` and JetBrains Marketplace `0.4.85` are public; both immutable tags resolve to [`2c76d288a2`](https://github.com/chainlesschain/chainlesschain/commit/2c76d288a2). These maintenance certifications intentionally preserve user behavior while re-running the 11-cell native ARM64 host matrix. On 2026-08-12, the Open VSX API reported 23,624 downloads and the JetBrains API reported `0.4.85` as approved and listed. Microsoft VS Code Marketplace and JetBrains author signing remain open.
>
> **Release-chain closure:** `0.163.5` has a closed chain of tag identity, three-OS CLI CI/Strict matrices, immutable artifacts, Trusted Publishing, signed provenance, registry and independent public readback. Registry presence alone is still never treated as release authority. Numeric product releases only consume an existing CLI with matching tag, exact-SHA, and registry evidence; they do not hold CLI publish authority. PDH remains `0.4.57` (92 collection contracts / 18 source categories), and Agent SDK remains `0.1.7`.
>
...
![CLI](https://img.shields.io/badge/cli-0.163.5-blue.svg)
