...
> **📦 CLI install**: `npm i -g chainlesschain@0.163.3` (current fully gated build; aliases `cc` / `clc` / `clchain`).
...
## 2026-08-10 Current mainline — **v5.0.3.135 / CLI 0.163.3 / PDH 0.4.57 / Open VSX 0.37.48 / JetBrains 0.4.84**

> **Release status:** `chainlesschain@0.163.3` is the current npm `latest` and production recommendation; immutable tag `v-npm-0-163-3` resolves to exact commit [`17fcf6aa79`](https://github.com/chainlesschain/chainlesschain/commit/17fcf6aa7917dd0fcc83b3ab5204c196bbb81758). That SHA passed [CLI CI](https://github.com/chainlesschain/chainlesschain/actions/runs/31329476135), [CLI Strict Sandbox](https://github.com/chainlesschain/chainlesschain/actions/runs/31329476020), and the [three-OS two-hour reliability/malicious-MCP gate](https://github.com/chainlesschain/chainlesschain/actions/runs/31329539092). The [dedicated npm release](https://github.com/chainlesschain/chainlesschain/actions/runs/31335579227) completed its exact-SHA gate, immutable tarball/CycloneDX SBOM, Trusted Publishing, SLSA provenance, and npmmirror synchronization; an [independent public readback](https://github.com/chainlesschain/chainlesschain/actions/runs/31336362525) proved the registry tarball byte-identical to the workflow artifact, with public tarball SHA-256 `68396c418029fc314403c9882c92fb9bef5ec6e9a61bb186333209884d410a92`.
>
> **CLI 0.163.3:** non-`stream-json` background agents launched from a clean Git checkout now create an isolated worktree at committed `HEAD` by default. `--worktree` requests the same safe path, only `--no-worktree` shares the current checkout, and dirty repositories are rejected. Generation/token fences, atomic state, durable turn intents, and grammar-safe detached argv stop stale workers from reviving, deleting a live worktree, or reparsing prompt text as privileged options. MCP source policy/cwd authority, sealed per-file Linux plugin snapshots, and re-attested fail-closed Windows adapter artifacts are also included.
>
> **Post-release source:** security increment `d2fcbddc99` forces every exact-package stdio MCP capsule to add the host-owned `code-snapshot`, `filesystem`, `network`, and `process-tree` boundaries. Source configuration may tighten this floor but cannot disable it. The commit is newer than `v-npm-0-163-3` and has not earned its own exact-SHA three-OS release authority, so it is not part of the `0.163.3` installation contract.
>
> **Native boundary:** the closed npm `0.163.3` release does not imply a signed Desktop/native release. Unsigned six-target validation remains fixed to `signed=false` and `releaseEligible=false`; Linux signing, Windows Authenticode, macOS signing/notarization, updater keys, and public signed fresh-install/upgrade/rollback readback remain open.
>
...
>
> **IDE mainline:** Open VSX `0.37.48` and JetBrains Marketplace `0.4.84` are public; both immutable tags resolve to exact commit [`5c9506c73c`](https://github.com/chainlesschain/chainlesschain/commit/5c9506c73c7692edc48e0b528355035c7e018fc6). The [ARM64 real-host gate](https://github.com/chainlesschain/chainlesschain/actions/runs/31325856489) aggregates exactly 11 cells: VS Code stable/minimum on Linux, Windows, and macOS ARM64; JetBrains 2024.2/2025.2 on Linux/macOS plus exact Windows ARM64 2026.2.0.1. The [VS Code](https://github.com/chainlesschain/chainlesschain/actions/runs/31326880152) and [JetBrains](https://github.com/chainlesschain/chainlesschain/actions/runs/31327569046) publication/readback gates succeeded. Microsoft VS Code Marketplace remains unpublished.
>
> **Release-chain closure:** `0.163.3` has a closed chain of tag identity, three-OS matrices, reliability/malicious-MCP evidence, immutable tarball/SBOM, Trusted Publishing, signed provenance, registry and npmmirror readback. `0.162.199` failed before upload and remains immutable; its fix shipped in `0.162.200`. Registry presence alone is still never treated as release authority. Numeric product releases only consume an existing CLI with matching tag, exact-SHA, and registry evidence; they do not hold CLI publish authority. PDH remains `0.4.57` (92 collection contracts / 18 source categories), and Agent SDK remains `0.1.7`.
>
