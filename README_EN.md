...
> **📦 CLI install**: `npm i -g chainlesschain@0.163.0` (current fully gated build; aliases `cc` / `clc` / `clchain`).
...
>
> ```bash
> npm i -g chainlesschain@0.163.0 --registry https://registry.npmjs.org
> ```
>
...
## 2026-08-08 Current mainline — **v5.0.3.135 / CLI 0.163.0 / PDH 0.4.57 / Open VSX 0.37.45 / JetBrains 0.4.81**

> **Release status:** `chainlesschain@0.163.0` is the current npm `latest` and production recommendation, published from exact commit [`aed0a3ae53`](https://github.com/chainlesschain/chainlesschain/commit/aed0a3ae5327917ce0490a5decbddd777f66f33b). The same SHA passed [CLI CI](https://github.com/chainlesschain/chainlesschain/actions/runs/31205224902), [CLI Strict Sandbox](https://github.com/chainlesschain/chainlesschain/actions/runs/31205231874), and the [dedicated npm release](https://github.com/chainlesschain/chainlesschain/actions/runs/31209345410) across Ubuntu, Windows, and macOS, including the exact-SHA gate, immutable tarball/CycloneDX SBOM, Trusted Publishing, SLSA provenance, registry readback, and npmmirror synchronization. The tree now carries package metadata `0.163.1` as a post-release source candidate; the immutable `v-npm-0-163-0` SHA remains the production contract.
>
> **CLI 0.163.0:** carries the complete `0.162.200` canonical-session authority, pinned npm-backed MCP execution capsules, native update recovery, and portable reliability gates, then repairs macOS `node-pty` helper permissions. It prepared matching-host validation for all six native targets, but its release evidence covers only the Ubuntu, Windows, and macOS npm matrices; the unsigned six-target binary execution was completed later on the `0.163.1` source candidate. It starts the second command-lifecycle observation cycle while retaining all 25 compatibility aliases.
>
> **0.163.1 source candidate (not published):** implementation baseline [`e3f56b11e2`](https://github.com/chainlesschain/chainlesschain/commit/e3f56b11e27ae1bd5d19ad8638434843c244aa68) includes durable-session-witness race hardening, bounded long-session compaction, atomic Windows MCP launch, native signing/notarization/updater credential preflight, and reproducible six-target standalone validation. That SHA passed [CLI CI](https://github.com/chainlesschain/chainlesschain/actions/runs/31240892299), [CLI Strict Sandbox](https://github.com/chainlesschain/chainlesschain/actions/runs/31240892177), the [unsigned six-target native gate](https://github.com/chainlesschain/chainlesschain/actions/runs/31240927257), and the [formal two-hour reliability gate on all three operating systems](https://github.com/chainlesschain/chainlesschain/actions/runs/31240943985). Each reliability artifact records 8/8 scenarios, 1,000 turns, at least 7,200 seconds of continuous duplex operation, and passing RSS, FD/handle, and descendant-process checks. The [release workflow](https://github.com/chainlesschain/chainlesschain/actions/runs/31241030666) was still only a dry run, with the exact-SHA, artifact, and publish jobs skipped; the native evidence is also fixed to `signed=false` and `releaseEligible=false`. Neither `v-npm-0-163-1` nor registry version `0.163.1` exists, npm `latest` remains `0.163.0`, and the production install contract is unchanged.
>
...
>
> **IDE mainline:** Open VSX `0.37.45` is public, and `ide-vscode-v0.37.45` resolves to exact commit [`aed0a3ae53`](https://github.com/chainlesschain/chainlesschain/commit/aed0a3ae5327917ce0490a5decbddd777f66f33b). The [VS Code release gate](https://github.com/chainlesschain/chainlesschain/actions/runs/31207738786) passed immutable VSIX verification, stable/minimum × Windows/macOS/Linux real hosts, isolated multi-root and multi-window identities, publication, and exact Open VSX readback. JetBrains Marketplace `0.4.81` remains public from [`b5177f13c9`](https://github.com/chainlesschain/chainlesschain/commit/b5177f13c950fcc74be8a2a4aa573f5c422a1b13), with its [2024.2/2025.2 three-OS release gate](https://github.com/chainlesschain/chainlesschain/actions/runs/31090452195) complete. Microsoft VS Code Marketplace remains unpublished.
>
> **Release-chain closure:** `0.163.0` has a closed chain of tag identity, three-OS matrices, immutable tarball/SBOM, Trusted Publishing, signed provenance, registry and npmmirror readback. `0.162.199` failed before upload and remains immutable; its fix shipped in `0.162.200`. Registry presence alone is still never treated as release authority. Numeric product releases only consume an existing CLI with matching tag, exact-SHA, and registry evidence; they do not hold CLI publish authority. PDH remains `0.4.57` (92 collection contracts / 18 source categories), and Agent SDK remains `0.1.7`.
>
...
![CLI](https://img.shields.io/badge/cli-0.163.0-blue.svg)
