...
> **📦 CLI install**: `npm i -g chainlesschain@0.166.63` (current npm `latest`; aliases `cc` / `clc` / `clchain`).
...
>
> ```bash
> npm i -g chainlesschain@0.166.63 --registry https://registry.npmjs.org
> ```
>
...
## 2026-09-19 current release — CLI 0.166.63 / Open VSX 0.37.108 / JetBrains 0.4.129
...
npm latest is `0.166.63`; immutable tag `v-npm-0-166-63` resolves to exact commit `f98797e8b5`. Its [CLI CI](https://github.com/chainlesschain/chainlesschain/actions/runs/35339309099) and [CLI Strict Sandbox](https://github.com/chainlesschain/chainlesschain/actions/runs/35339308634) passed every configured Linux, Windows, and macOS job. The [npm OIDC publication workflow](https://github.com/chainlesschain/chainlesschain/actions/runs/35339308709) uploaded `chainlesschain@0.166.63` with signed provenance, then concluded failure because the public registry did not expose the version within its 30-attempt readback window; that workflow conclusion remains recorded as a failure. On 2026-09-19 the official registry independently reported `latest=0.166.63` and exposed the tarball integrity metadata. Agent Protocol `0.1.11`, Context Memory Kernel `0.1.4`, Agent SDK TS/Python `0.2.11/0.2.9`, and Session Core `0.3.12` retain independent release boundaries.
...
The CLI artifact source boundary is `f98797e8b5`; the JetBrains artifact boundary is `b7978f4915`, and the VS Code artifact boundary is `e39edecb7f`. [Open VSX 0.37.108](https://open-vsx.org/extension/chainlesschain/chainlesschain-ide) and [JetBrains Marketplace 0.4.129](https://plugins.jetbrains.com/plugin/32208-chainlesschain-ide-bridge) are public and recommend CLI `0.166.63`; the [VS Code release gate](https://github.com/chainlesschain/chainlesschain/actions/runs/35398555660) and [JetBrains release gate](https://github.com/chainlesschain/chainlesschain/actions/runs/35374270179) succeeded. Microsoft VS Code Marketplace and Desktop native installers remain independent release surfaces and do not inherit npm or IDE-marketplace evidence. Production authorities, remote durability, a workspace recovery set, atomic restore, and the real Electron DID/RBAC PM E2E remain outstanding. See [module 112 design](docs/design/modules/112-governed-skill-evolution-design.md), the [governed evolution user guide](docs-site/docs/chainlesschain/governed-skill-evolution.md), and the [upgrade guide](docs-site/docs/chainlesschain/agent-platform-release.md).
