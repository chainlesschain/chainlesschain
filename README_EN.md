...
> **📦 CLI install**: `npm i -g chainlesschain@0.166.56` (current npm `latest`; aliases `cc` / `clc` / `clchain`).
...
## 2026-09-15 current release — CLI 0.166.56 / Open VSX 0.37.103 / JetBrains 0.4.124
...
npm latest is `0.166.56`; immutable tag `v-npm-0-166-56` resolves to exact commit `d55de4810e`. Its [CLI CI](https://github.com/chainlesschain/chainlesschain/actions/runs/34940431736) and [CLI Strict Sandbox](https://github.com/chainlesschain/chainlesschain/actions/runs/34940431550) passed every configured Linux, Windows, and macOS job, and the [npm OIDC exact-SHA publication/readback gate](https://github.com/chainlesschain/chainlesschain/actions/runs/34940431525) succeeded. The official registry reported `latest=0.166.56` on 2026-09-15. Agent Protocol `0.1.10`, Context Memory Kernel `0.1.3`, Agent SDK TS/Python `0.2.10/0.2.8`, and Session Core `0.3.12` retain independent release boundaries.

Building on `0.166.48`'s balanced plugin evaluation, deferred questions, background-session index, and governed semantic recall, this release ships request-accurate context windows and token/cache projections without letting child-agent or semantic-compaction usage overwrite primary-model accounting. Evolution now provides `cc evolution deployment init-test/replace-test`: a local Ed25519 TEST profile still follows normal signature verification and atomic profile writes, can rotate safely to a formal descriptor/trust root, and canonicalizes macOS `/var`/`/private/var` physical paths. TEST identity grants no review or publication authority; static model profiles retain `runtimeVerified:false`, and automatic active Skill promotion remains `HOLD`.

This documentation tracks `main@ac0e61b3cd`. [Open VSX 0.37.103](https://open-vsx.org/extension/chainlesschain/chainlesschain-ide) and [JetBrains Marketplace 0.4.124](https://plugins.jetbrains.com/plugin/32208-chainlesschain-ide-bridge) are public and recommend CLI `0.166.56`; the same IDE commit passed the [VS Code release gate](https://github.com/chainlesschain/chainlesschain/actions/runs/34948682154) and [JetBrains release gate](https://github.com/chainlesschain/chainlesschain/actions/runs/34948681939). Both Skill Evolution configuration surfaces expose one-click TEST creation/refresh and formal-certificate replacement. Microsoft VS Code Marketplace and Desktop native installers remain independent release surfaces and do not inherit npm or IDE-marketplace evidence. See the [current runtime design](docs/design/cli-runtime-current.md), [governed Skill evolution design](docs/design/modules/112-governed-skill-evolution-design.md), and [upgrade guide](docs-site/docs/chainlesschain/agent-platform-release.md).
