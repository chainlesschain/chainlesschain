...
> **📦 CLI install**: `npm i -g chainlesschain@0.166.4` (current fully gated build; aliases `cc` / `clc` / `clchain`).
...
## 2026-08-26 Current mainline — **v5.0.3.135 / Agent Platform CLI 0.166.4 / Agent SDK 0.2.3 / Agent Protocol 0.1.4 / PDH 0.4.59**

> **Release status:** `chainlesschain@0.166.4` is the current npm `latest` and production recommendation. Immutable tag `v-npm-0-166-4` resolves to exact commit [`6b1619926c`](https://github.com/chainlesschain/chainlesschain/commit/6b1619926c5aadc4586e17994b607169b2ae58ae); that SHA passed [CLI CI](https://github.com/chainlesschain/chainlesschain/actions/runs/32954164746), [CLI Strict Sandbox](https://github.com/chainlesschain/chainlesschain/actions/runs/32954183220), and the [dedicated npm/SDK release](https://github.com/chainlesschain/chainlesschain/actions/runs/32959864584). npm reports `latest=0.166.4`.
>
> **Agent Platform 0.166.4:** on top of `0.166.3` TeamMailbox v3, custody handoff, `SessionMessageFabric`, and structured approvals, this release adds a canonical Schema-generated inventory and envelope validation for **37 Agent stream event types**. TypeScript, Python, Kotlin, Swift, and the vendored VS Code SDK share one source and the same valid/invalid fixtures; transports continue preserving unknown future events for forward compatibility.
>
> **SDK / Protocol:** npm `@chainlesschain/agent-sdk@0.2.3`, PyPI `chainlesschain-agent-sdk==0.2.3`, and npm `@chainlesschain/agent-protocol@0.1.4` are public. Alongside canonical approval bindings, the release exports `CC_AGENT_STREAM_EVENT_TYPES`, typed envelopes, and strict validators. Package versions evolve independently from wire protocol v1.
>
...
>
> **IDE release and 30k-download milestone:** Open VSX `0.37.69` and JetBrains Marketplace `0.4.99` are public, and ChainlessChain IDE Bridge has passed **30,000 downloads** on Open VSX. Both IDEs consume Schema-generated Agent event types; VS Code retains exact one-operation/current-turn/current-session approvals and bounded content-free message/handoff counters. JetBrains `0.4.100` is on main and routes production chat events through the generated enum, while the public install baseline remains Marketplace `0.4.99`. Microsoft VS Code Marketplace remains unpublished.
>
> **Release and source boundary:** `6b1619926c` publishes the canonical stream-event inventory in the CLI, TypeScript SDK, and Open VSX build; Protocol/Python SDK publication is anchored at `e7a059d3ed`. The JetBrains generated-event routing at current `main@33603c631e` is a later source increment. Desktop/Cowork/Scheduler cutover, production relay, signed native delivery, and long-running validation remain separate evidence questions.
>
...
>
> **Release-chain closure:** `0.166.4` has a closed chain of tag identity, three-OS CLI CI/Strict matrices, immutable artifacts, Trusted Publishing, signed provenance, and independent registry readback. TypeScript/Python Agent SDK `0.2.3` and Agent Protocol `0.1.4` also completed OIDC/Trusted Publishing and public readback. PDH remains `0.4.59` (optional native SQLite with a `sql.js` fallback).
>
...
![CLI](https://img.shields.io/badge/cli-0.166.3-blue.svg)
