...
> **📦 CLI install**: `npm i -g chainlesschain@0.163.2` (current fully gated build; aliases `cc` / `clc` / `clchain`).
...
## 2026-08-09 Current mainline — **v5.0.3.135 / CLI 0.163.2 / PDH 0.4.57 / Open VSX 0.37.47 / JetBrains 0.4.83**

> **Release status:** `chainlesschain@0.163.2` is the current npm `latest` and production recommendation; immutable tag `v-npm-0-163-2` resolves to exact commit [`2d6f19aea2`](https://github.com/chainlesschain/chainlesschain/commit/2d6f19aea243ed4f054b585d4bc709d4209ff80d). That SHA passed [CLI CI](https://github.com/chainlesschain/chainlesschain/actions/runs/31277578939), [CLI Strict Sandbox](https://github.com/chainlesschain/chainlesschain/actions/runs/31277578889), and the [three-OS two-hour reliability/malicious-MCP gate](https://github.com/chainlesschain/chainlesschain/actions/runs/31271803404). The [dedicated npm release](https://github.com/chainlesschain/chainlesschain/actions/runs/31277578900) completed its exact-SHA gate, immutable tarball/CycloneDX SBOM, Trusted Publishing, SLSA provenance, and npmmirror synchronization; an [independent public readback](https://github.com/chainlesschain/chainlesschain/actions/runs/31278310621) proved the registry tarball byte-identical to the workflow artifact, with SHA-256 `394a755d52fca98e7527228859a78e1c57eae69abfdcaf77cc56e9b665b13fe1`.
>
> **CLI 0.163.2:** stdio MCP configuration adds explicit `--runtime-kind` identities. Linux pins fixed npm/Node runtime, entry, and arguments through descriptor-bound launch; Windows restricted-token/AppContainer launch authority becomes one-use and Broker-private; macOS returns a typed fail-closed result when public APIs cannot prove an atomic final-image binding. Malicious-host evidence v4 separates Linux pathname replacement, the Windows strict gate, and explicit macOS fail-closed proof. Arbitrary native/shared-library closure, distributed revocation, the complete malicious-Skill matrix, and signed native publication remain outside this release; all 25 compatibility aliases remain.
>
> **Post-release mainline:** latest commit [`9fa5162e66`](https://github.com/chainlesschain/chainlesschain/commit/9fa5162e668fa9b457b0d70d54a0806773c363ab) propagates `sandboxPolicy.requiredBoundaries` (`filesystem` / `network`) and trusted `cwd` from local/project/user/managed, Skill, and Cowork MCP sources into the Process Broker. It also prevents an invalid higher-precedence definition from silently falling through to a weaker same-name source. This is newer than the current npm tag and becomes a production contract only after a later release passes its own exact-SHA three-OS gates.
>
> **Native boundary:** the closed npm `0.163.2` release does not imply a signed Desktop/native release. Unsigned six-target validation remains fixed to `signed=false` and `releaseEligible=false`; Linux signing, Windows Authenticode, macOS signing/notarization, updater keys, and public signed fresh-install/upgrade/rollback readback remain open.
>
...
>
> **IDE mainline:** Open VSX `0.37.47` and JetBrains Marketplace `0.4.83` are public; both immutable tags resolve to exact commit [`9db081c5a9`](https://github.com/chainlesschain/chainlesschain/commit/9db081c5a9b24d1c51952e86513b0520620feadd). The [ARM64 real-host gate](https://github.com/chainlesschain/chainlesschain/actions/runs/31269850865) aggregates exactly 11 cells: VS Code stable/minimum on Linux, Windows, and macOS ARM64; JetBrains 2024.2/2025.2 on Linux/macOS plus exact Windows ARM64 2026.2.0.1. Stop/force-stop, restart recovery, rewind, Sessions Workbench, and artifact journeys run on real hosts. The [VS Code](https://github.com/chainlesschain/chainlesschain/actions/runs/31271800174) and [JetBrains](https://github.com/chainlesschain/chainlesschain/actions/runs/31271800206) release gates succeeded. Microsoft VS Code Marketplace remains unpublished.
>
> **Release-chain closure:** `0.163.2` has a closed chain of tag identity, three-OS matrices, reliability/malicious-MCP evidence, immutable tarball/SBOM, Trusted Publishing, signed provenance, registry and npmmirror readback. `0.162.199` failed before upload and remains immutable; its fix shipped in `0.162.200`. Registry presence alone is still never treated as release authority. Numeric product releases only consume an existing CLI with matching tag, exact-SHA, and registry evidence; they do not hold CLI publish authority. PDH remains `0.4.57` (92 collection contracts / 18 source categories), and Agent SDK remains `0.1.7`.
>
