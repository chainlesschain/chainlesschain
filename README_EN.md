...
> **📦 CLI install**: `npm i -g chainlesschain@0.162.198` (current fully gated build; aliases `cc` / `clc` / `clchain`).
...
## 2026-08-07 Current mainline — **v5.0.3.135 / CLI 0.162.198 / PDH 0.4.57 / Open VSX 0.37.44 / JetBrains 0.4.81**

> **Release status:** `chainlesschain@0.162.198` is the current npm `latest` and production recommendation, published from exact commit [`3c0f62fa17`](https://github.com/chainlesschain/chainlesschain/commit/3c0f62fa17242cfa3123ab502a9bf5d1cbed8481). The same SHA passed [CLI CI](https://github.com/chainlesschain/chainlesschain/actions/runs/31078499968), [CLI Strict Sandbox](https://github.com/chainlesschain/chainlesschain/actions/runs/31078499270), and the [dedicated npm release](https://github.com/chainlesschain/chainlesschain/actions/runs/31081337370) across Ubuntu, Windows, and macOS, including the exact-SHA gate, immutable tarball/SBOM handoff, and npm provenance. An [independent public readback](https://github.com/chainlesschain/chainlesschain/actions/runs/31082366544) reverified the registry bytes and publishing identity. The tree still carries package metadata `0.162.198`, but HEAD includes post-release reliability work; the immutable tag SHA remains the production contract.
>
> **CLI 0.162.198:** on top of `0.162.197` typed configuration, MCP recovery authority, canonical session/budget, and checkpoint saga, this release adds interactive output backpressure, CLI-owned Sessions Workbench/rewind routes, and portable three-OS release fixtures. It is also the first public baseline for the command-lifecycle observation window.
>
> **Source HEAD hardening (not republished yet):** session host leases and anti-rollback witnesses, persistence-failure classification and bounded cleanup, approval for untrusted MCP effects, MCP executable trust generations and pinned npm dependency closures, POSIX/Windows installer and OTA generation recovery, and expanded cross-platform fault/soak measurement are now on main. They become a production install contract only after a future version passes its own exact-SHA release matrix.
>
...
>
> **IDE mainline:** Open VSX `0.37.44` and JetBrains Marketplace `0.4.81` are public, and both tags resolve to exact commit [`b5177f13c9`](https://github.com/chainlesschain/chainlesschain/commit/b5177f13c950fcc74be8a2a4aa573f5c422a1b13). The [VS Code release gate](https://github.com/chainlesschain/chainlesschain/actions/runs/31090450115) passed immutable VSIX verification, real Windows/macOS/Linux hosts, publication, and Open VSX readback. The [JetBrains release gate](https://github.com/chainlesschain/chainlesschain/actions/runs/31090452195) passed the 2024.2/2025.2 three-OS host matrix, upload, and Marketplace readback. The public builds now drive all five canonical session kinds through Dispatch → `needs_input` → Reply → done, artifact/PR readback, and recovery in a second IDE process. Microsoft VS Code Marketplace remains unpublished.
>
> **Release-chain closure:** `0.162.198` now has a closed chain of tag identity, three-OS matrices, immutable tarball/SBOM, Trusted Publishing, signed provenance, registry and npmmirror readback. Registry presence alone is still never treated as release authority. Numeric product releases only consume an existing CLI with matching tag, exact-SHA, and registry evidence; they do not hold CLI publish authority. PDH remains `0.4.57` (92 collection contracts / 18 source categories), and Agent SDK remains `0.1.7`.
>
