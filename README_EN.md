...
> **📦 CLI install**: `npm i -g chainlesschain@0.166.5` (current fully gated build; aliases `cc` / `clc` / `clchain`).
...
>
> ```bash
> npm i -g chainlesschain@0.166.5 --registry https://registry.npmjs.org
> ```
>
...
## 2026-08-27 Current mainline — **v5.0.3.135 / CLI 0.166.5 production recommendation · npm latest 0.166.6 / Agent SDK 0.2.4 / Agent Protocol 0.1.5 / PDH 0.4.59**

> **Release status:** `chainlesschain@0.166.5` remains the fully gated production recommendation. Immutable tag `v-npm-0-166-5` resolves to exact commit [`2f5b0f263a`](https://github.com/chainlesschain/chainlesschain/commit/2f5b0f263a142fd31daca1396456a8735c2a7ee6); that SHA passed [CLI CI](https://github.com/chainlesschain/chainlesschain/actions/runs/33006394588), [CLI Strict Sandbox](https://github.com/chainlesschain/chainlesschain/actions/runs/33006393969), [Agent Protocol](https://github.com/chainlesschain/chainlesschain/actions/runs/33006393605), [Python SDK](https://github.com/chainlesschain/chainlesschain/actions/runs/33006393608), and the [App Server soak](https://github.com/chainlesschain/chainlesschain/actions/runs/33006394148). npm now reports `latest=0.166.6`; tag `v-npm-0-166-6` points to `7f18511fbc`, but CLI CI was cancelled and Strict Sandbox failed at that exact commit, so it cannot inherit the `0.166.5` release authorization.
>
> **Agent Platform 0.166.5:** the 37 canonical Agent stream events now have payload-level discriminated unions in addition to discriminator/envelope validation. TypeScript, Python, Kotlin, Swift, Desktop, and VS Code share generated sources, valid/invalid fixtures, and cross-client causal conformance. Desktop and VS Code expose a default-off, fixed-method App Server pilot; the App Server WebSocket is public only as an experimental, token-protected, bounded transport with TLS required for remote binding.
>
...
>
> **Security and capacity boundaries:** Normal Agent startup remains Docker-optional while explicit strict/workspace-write sandbox policies stay fail-closed. App Server request/output/payload/socket/cleanup paths are bounded. CLI `0.166.6` adds legacy WS, MCP, browser-control, P2P, permission, media, and signing backlog admission plus global/per-agent Agent IPC bounds. It is visible in npm, but its exact-commit gate is incomplete, so production remains pinned to `0.166.5`.
>
> **IDE release and 30k-download milestone:** Open VSX `0.37.70` and JetBrains Marketplace `0.4.100` are public, and ChainlessChain IDE Bridge has passed **30,000 downloads** on Open VSX. Both IDEs consume Schema-generated Agent event types; VS Code retains exact-scoped approvals, bounded content-free message/handoff counters, and the App Server pilot. JetBrains `0.4.101` has been uploaded and is pending Marketplace review, so it is not the public install baseline. Microsoft VS Code Marketplace remains unpublished.
>
> **Release and source boundary:** `2f5b0f263a` is the fully gated `0.166.5` release commit; `7f18511fbc` is the public but incompletely gated `0.166.6` tag commit. Observed later mainline `42d516c0dd` contains release-test and IDE-metadata maintenance only and is not a new CLI release identity. Desktop/Cowork/Scheduler authoritative Graph cutover, production relay, signed native delivery, and real-provider journeys remain separate evidence questions. A GraphRun only unifies run identity, authority, budget, and revision; the Task Graph, dynamic Agent Tree, and Artifact/Trace evidence projection must not be collapsed into one graph.
>
...
>
> **Release-chain closure:** `0.166.5` has a closed chain of tag identity, three-OS CLI CI/Strict matrices, immutable artifacts, Trusted Publishing, signed provenance, and independent registry readback. `0.166.6` currently has tag/npm visibility only and must not be described as fully gated. TypeScript/Python Agent SDK `0.2.4` and Agent Protocol `0.1.5` also completed public readback. The `0.166.5` exact commit's 1,800.21-second App Server soak processed 2,427,887 requests with zero unexpected errors, zero drain leftovers, and 0.762% RSS growth. PDH remains `0.4.59` (optional native SQLite with a `sql.js` fallback).
>
