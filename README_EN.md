...
> **📦 CLI install**: `npm i -g chainlesschain@0.166.59` (current npm `latest`; aliases `cc` / `clc` / `clchain`).
...
## 2026-09-16 current release — CLI 0.166.59 / Open VSX 0.37.105 / JetBrains 0.4.126
...
npm latest is `0.166.59`; immutable tag `v-npm-0-166-59` resolves to exact commit `a148ec7a57`. Its [CLI CI](https://github.com/chainlesschain/chainlesschain/actions/runs/35057646611) and [CLI Strict Sandbox](https://github.com/chainlesschain/chainlesschain/actions/runs/35057646344) passed every configured Linux, Windows, and macOS job, and the [npm OIDC publication/public-readback gate](https://github.com/chainlesschain/chainlesschain/actions/runs/35057646385) succeeded. The official registry reported `latest=0.166.59` on 2026-09-16. Agent Protocol `0.1.11`, Context Memory Kernel `0.1.4`, Agent SDK TS/Python `0.2.11/0.2.9`, and Session Core `0.3.12` retain independent release boundaries.

This release adds durable IDE task notes and fresh-session handoff. Context/Memory Kernel checkpoints retain bounded goals, file evidence, failures, changes, plans, and compaction boundaries; the CLI projects verified state to `.chainlesschain/sessions/<session-id>/WORKLOG.md`. VS Code and JetBrains can save at a completed tool boundary and continue in an independent chat without copying the full transcript or inheriting temporary permissions. Invalid history fails visibly before model execution. The additive `TaskCheckpoint` contract is also published through Agent Protocol and the TypeScript/Python SDKs. Existing request-level context accounting, Evolution TEST deployment, and automatic active Skill promotion `HOLD` boundaries remain unchanged.

This documentation tracks `main@a148ec7a57`. [Open VSX 0.37.105](https://open-vsx.org/extension/chainlesschain/chainlesschain-ide) and [JetBrains Marketplace 0.4.126](https://plugins.jetbrains.com/plugin/32208-chainlesschain-ide-bridge) are public and recommend CLI `0.166.59`; the same exact commit passed the [VS Code release gate](https://github.com/chainlesschain/chainlesschain/actions/runs/35095569167) and [JetBrains release gate](https://github.com/chainlesschain/chainlesschain/actions/runs/35097287263). Microsoft VS Code Marketplace and Desktop native installers remain independent release surfaces and do not inherit npm or IDE-marketplace evidence. See the [task-worklog design](docs/design/ide-task-worklog-handoff.md), [task-worklog user guide](docs/features/ide-task-worklog-user-guide.md), and [upgrade guide](docs-site/docs/chainlesschain/agent-platform-release.md).
