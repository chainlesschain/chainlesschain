...
- **Phase 14.1 mobile native entry (in development, not yet shipped)**: Android `PersonalDataHubCommands.kt` 22-method typed wrapper (21 data + `runSkill` analysis) + 9 UI files (Ask / Adapters / Audit screens + HealthCard + AcceptNonLocalDialog + 3-tab container) + 22 unit tests. iOS Phase 14.2 v0.1 scaffold landed (8/22 method wrappers — UI-path subset; remaining 14 await UI extension at ~15 LOC each). Pre-release static audit caught 17 Android camelCase / kebab-case method-name mismatches (would 404 on first call); sed-aligned production + unit tests + desktop whitelist test. Phase 14.3 streaming sync progress + Phase 14.4 real-device E2E not yet started.
