...
> **📦 CLI install**: `npm i -g chainlesschain@0.166.6` (current fully gated build; aliases `cc` / `clc` / `clchain`).
...
>
> ```bash
> npm i -g chainlesschain@0.166.6 --registry https://registry.npmjs.org
> ```
>
...
## 2026-08-27 Current mainline — **v5.0.3.135 / CLI 0.166.6 production recommendation and npm latest / Agent SDK 0.2.4 / Agent Protocol 0.1.5 / PDH 0.4.59**

> **Release status:** `chainlesschain@0.166.6` is the fully gated production recommendation and npm `latest`. Immutable tag `v-npm-0-166-6` resolves to exact commit [`f2a249bf3d`](https://github.com/chainlesschain/chainlesschain/commit/f2a249bf3daf77af32ab84cfe5d567485f08b3e7); that SHA passed [CLI CI](https://github.com/chainlesschain/chainlesschain/actions/runs/33030279940), [CLI Strict Sandbox](https://github.com/chainlesschain/chainlesschain/actions/runs/33030279716), and [independent npm artifact/provenance readback](https://github.com/chainlesschain/chainlesschain/actions/runs/33033884686). The earlier cancelled/failed `7f18511fbc` candidate remains historical release evidence and no longer identifies the tag.
>
> **Agent Platform 0.166.6:** carries forward `0.166.5`'s 37 canonical payload unions, four-language generated sources, causal conformance, fixed-capability Desktop/VS Code App Server pilot, and experimental WebSocket. It additionally bounds Agent IPC registration/startup, interactions, requests, stdio frames/queues, and deadlines globally and per agent.
>
...
>
> **Security and capacity boundaries:** Normal Agent startup remains Docker-optional while explicit strict/workspace-write sandbox policies stay fail-closed. App Server request/output/payload/socket/cleanup paths are bounded. CLI `0.166.6` adds legacy WS, MCP, browser-control, P2P, permission, media, and signing backlog admission plus global/per-agent Agent IPC bounds, with its exact-commit release chain now closed. Later mainline also caps the Desktop Cowork AgentPool wait queue at 128 by default and returns `OVERLOADED` with a 100ms retry hint when full.
>
> **IDE release and 30k-download milestone:** Open VSX `0.37.71` and JetBrains Marketplace `0.4.100` are public, and ChainlessChain IDE Bridge has passed **30,000 downloads** on Open VSX. VS Code `0.37.71` adds Marketplace Overview/install-link coherence and bounded Agent IPC guidance on top of `0.37.70`'s Schema-generated payload contract. The JetBrains source/upload identity is now `0.4.102` and remains pending Marketplace review, so it is not the public install baseline. Microsoft VS Code Marketplace remains unpublished. Pair with fully gated CLI `0.166.6`.
>
> **Release and source boundary:** `f2a249bf3d` is the fully gated `0.166.6` release commit; `42d516c0dd` is the VS Code `0.37.71` / JetBrains `0.4.102` IDE release identity; `c64c4bcac6` bounds the Desktop Cowork AgentPool wait queue, and current mainline `a8484915fa` moves its regressions into the Vitest unit gate. None of the later commits creates a new CLI release identity. Desktop/Cowork/Scheduler authoritative Graph cutover, production relay, signed native delivery, and real-provider journeys remain separate evidence questions. A GraphRun only unifies run identity, authority, budget, and revision; the Task Graph, dynamic Agent Tree, and Artifact/Trace evidence projection must not be collapsed into one graph.
>
