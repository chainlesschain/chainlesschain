...
## 2026-09-23 current release — CLI 0.166.71 / Open VSX 0.37.113 / JetBrains 0.4.133 public

> Release boundary: npm `chainlesschain@0.166.71` and Open VSX `0.37.113` have been publicly read back. JetBrains Marketplace currently lists `0.4.133`; the CLI `0.166.71` pairing `0.4.134` passed its release workflow and was uploaded, but is not publicly listed yet. Microsoft VS Code Marketplace remains unpublished because `VSCE_PAT` is not configured, and Desktop native installers remain independently qualified artifacts.

npm latest is `0.166.71`; immutable tag `v-npm-0-166-71` resolves to exact commit `fc7d6e102a`. Its [CLI CI](https://github.com/chainlesschain/chainlesschain/actions/runs/35805928010) and [CLI Strict Sandbox](https://github.com/chainlesschain/chainlesschain/actions/runs/35805932793) passed the configured Linux, Windows, and macOS jobs. The [npm OIDC publication](https://github.com/chainlesschain/chainlesschain/actions/runs/35810795671) and [independent public readback](https://github.com/chainlesschain/chainlesschain/actions/runs/35811559611) succeeded. The paired IDE source boundary is `94141e6fef`: [Open VSX `0.37.113`](https://open-vsx.org/extension/chainlesschain/chainlesschain-ide) is public, while the [JetBrains `0.4.134` release workflow](https://github.com/chainlesschain/chainlesschain/actions/runs/35820067327) uploaded successfully and the public API still lists `0.4.133`.

CLI `0.166.71` adds `--decision-provider typesafe|laya|system-one` to the default-off typed Skill-decision pilot, allowing local Laya or a compatible `/v1/systemone` service. It still supports only durable, single-prompt, headless Agent runs: `shadow` records bounded observations without changing routing, while `suggest` attaches a suggestion without executing a Skill. IDEs store no decision-model credential and receive no routing or execution authority. Live TypeSafe/Laya integration and quality, latency, and cost evaluation remain unfinished. See [module 114 design](docs/design/modules/114-jev-decision-layer-design.md), the [Skill decision user guide](docs-site/docs/chainlesschain/jev-decision-layer.md), and the [release and upgrade guide](docs-site/docs/chainlesschain/agent-platform-release.md).
...
npm latest is `0.166.68`; immutable tag `v-npm-0-166-68` resolves to exact commit `815fdbc0c4`. Its [CLI CI](https://github.com/chainlesschain/chainlesschain/actions/runs/35507258259) and [CLI Strict Sandbox](https://github.com/chainlesschain/chainlesschain/actions/runs/35507258127) passed every configured Linux, Windows, and macOS job. The [npm OIDC publication and public readback](https://github.com/chainlesschain/chainlesschain/actions/runs/35509552030) succeeded with signed provenance. Before CLI publication, the workflow byte-audited all 13 child-package tarballs. Session Core `0.3.13`, Context/Memory Kernel `0.1.5`, and Personal Data Hub `0.4.62` contained source changes and were published first; every reused child package had to match its Git tree, immutable tag, provenance, and public bytes.

This release puts browser observation, navigation, tab creation, history traversal, keyboard actions, and downloads behind explicit action authorities. Download bytes stream into durable filesystem quarantine governed by cross-process locks, authenticated retain/revoke decisions, crash recovery, expiry sweeps, and auditable disposal. PM exploration runners, reviewers, and graders also run as bounded child processes whose evidence, recovery snapshots, egress decisions, and benchmark receipts bind to the governed execution context.

The release workflow now builds deterministic generated output before auditing reused packages. A changed package without a version bump, public-tarball drift, or missing trusted provenance stops the release before the CLI. A release tag may reuse complete exact-SHA CLI CI and Strict Sandbox results instead of rerunning the full suite; local-only, partial, timed-out, or older-commit checks never satisfy that gate.

The CLI artifact source boundary is `815fdbc0c4`; the VS Code and JetBrains artifact boundary is `5860f1e4a4`. [Open VSX 0.37.110](https://open-vsx.org/extension/chainlesschain/chainlesschain-ide) and [JetBrains Marketplace 0.4.131](https://plugins.jetbrains.com/plugin/32208-chainlesschain-ide-bridge) are public and recommend CLI `0.166.68`; the [exact-commit IDE gate](https://github.com/chainlesschain/chainlesschain/actions/runs/35510430698), [Open VSX publication](https://github.com/chainlesschain/chainlesschain/actions/runs/35511532130), and [JetBrains publication](https://github.com/chainlesschain/chainlesschain/actions/runs/35512228012) succeeded. Mainline `45557d27dc` then fixed failed-job rerun aggregation so evidence from earlier successful attempts is retained; the resulting [11-cell ARM64 host matrix and aggregate](https://github.com/chainlesschain/chainlesschain/actions/runs/35514094545) passed. Microsoft Marketplace remains unpublished because `VSCE_PAT` is not configured. Desktop native installers remain separately qualified artifacts. See the [release-boundary design](docs/design/modules/110-agent-platform-release-boundaries.md), [module 112 design](docs/design/modules/112-governed-skill-evolution-design.md), the [governed evolution user guide](docs-site/docs/chainlesschain/governed-skill-evolution.md), and the [upgrade guide](docs-site/docs/chainlesschain/agent-platform-release.md).

The documented source baseline `8e7c45e32e` further hardens governed Desktop IPC and local secret storage. Volcengine and Secure Storage operations bind the main window, main frame, DID tenant, and fixed purpose before side effects; function calls can only consume an opaque capability supplied by a signed deployment. Encrypted configuration now uses flush/rename/directory-sync atomic commits with authenticated recovery, serialized by cross-process owner records, dead-owner reclamation, and a recovery fence. Backups retain 10 entries by default and reject symlinks, directory escapes, oversized files, and restore paths outside the server inventory. Core LLM IPC success results and alert, model-budget, and retention database rows are projected into bounded plain data, filtering provider or agent internals, extra columns, accessors, proxies, non-finite values, and oversized collections. These are post-release source changes and are not part of the npm, IDE, or Desktop native public artifacts above.
...
> npm latest is `0.166.43`, tagged `v-npm-0-166-43` at [`7528bfb81d`](https://github.com/chainlesschain/chainlesschain/commit/7528bfb81ddb45427b5eb5d1281498cf65e8ab0c). The same commit passed the complete [CLI CI](https://github.com/chainlesschain/chainlesschain/actions/runs/34476406749), [Strict Sandbox](https://github.com/chainlesschain/chainlesschain/actions/runs/34476405180), and [IDE Extensions](https://github.com/chainlesschain/chainlesschain/actions/runs/34476405124) gates, followed by [npm Trusted Publishing and public readback](https://github.com/chainlesschain/chainlesschain/actions/runs/34479754264).
>
> Administrators can now verify, persist, and toggle governed-evolution deployments with `cc evolution deployment status/configure/enable/disable`. Configuration verifies the Ed25519 descriptor, trust root, and module digest before atomically writing an owner-only profile. Environment overrides remain available for centrally managed installations and take precedence. Configuration loads deployment authority; automatic active promotion always remains `HOLD`.
>
> VS Code, JetBrains, and `cc ui` use the same CLI-owned configuration surface. IDEs neither store governance private keys nor receive review or release authority. Releases `0.166.39`–`0.166.42` also converge repeated log, GitHub, and pull-request investigations, preserve compacted evidence, execute explicit PR-close requests, and harden Windows secure-pipe, capacity, and lock-contention validation.
>
> [Open VSX 0.37.93](https://open-vsx.org/extension/chainlesschain/chainlesschain-ide) and [JetBrains Marketplace 0.4.120](https://plugins.jetbrains.com/plugin/32208-chainlesschain-ide-bridge) are public and recommend CLI `0.166.43`. Agent SDK `0.2.9`, Agent Protocol `0.1.9`, Session Core `0.3.12`, Core DB `0.1.5`, Context Memory Kernel `0.1.1`, and PDH `0.4.60` retain independent release identities; the extension is still not published to Microsoft Marketplace.
>
> **Source and deployment boundary:** local and GitHub `main@7528bfb81d` match the published CLI/IDE source, and the npm, Open VSX, and JetBrains artifacts have each been read back publicly. Configuration does not create production keys, identities, graders, KMS/PKI, or a witness; the target deployment must provide those authorities and qualify Desktop native installers independently. See the [governed Skill-evolution guide](docs-site/docs/chainlesschain/governed-skill-evolution.md), [current CLI runtime](docs/design/cli-runtime-current.md), [module 112 design](docs/design/modules/112-governed-skill-evolution-design.md), and [module 113 design](docs/design/modules/113-governed-desktop-model-ingress-design.md).
...
> **Release status at the time:** `chainlesschain@0.166.15` was the fully gated recommendation and npm `latest` for this historical snapshot. The evidence below describes that snapshot only; use `0.166.43` above for the current install.
>
...
>
> **SDK / Protocol:** npm `@chainlesschain/agent-sdk@0.2.7`, PyPI `chainlesschain-agent-sdk==0.2.7`, and npm `@chainlesschain/agent-protocol@0.1.7` are public. Session Core `0.3.8` includes fail-closed approval and persistent-adapter fixes. The generated 37-payload union, strict validators, bounded Context/Memory and App Server messages, cross-language causal conformance, and lossless unknown-event transport remain intact.
>
> **Security and Windows runtime:** a drifting fixed renderer/main capability manifest now fails the Desktop build. CLI defaults to network-denied `workspace-write`; shell execution is refused when approval, policy loading, or persistence is unavailable. High-risk process creation requires a durable redacted admission record. Ordinary Agent sessions do not probe Docker, and Windows resolves bare commands through `PATHEXT` so Docker Desktop's POSIX shim cannot shadow `docker.exe`.
>
> **IDE release:** Open VSX `0.37.77` and JetBrains Marketplace `0.4.107` are public and recommend CLI `0.166.15`. Both use exact release commit `22db04f559`; the [VS Code release gate](https://github.com/chainlesschain/chainlesschain/actions/runs/33393387965) and [JetBrains release gate](https://github.com/chainlesschain/chainlesschain/actions/runs/33393394812) passed. Microsoft VS Code Marketplace still does not expose the extension.
>
> **Release and source boundary:** current GitHub `main` is [`458b342f5f`](https://github.com/chainlesschain/chainlesschain/commit/458b342f5f11f2ee82c0e6a91ee485d4309485fb), later than the `0.166.15` release SHA. Post-release source adds transient audit-blob retries, per-Agent HOME/config/cache isolation for formal Windows evaluation, stable CI cleanup, and a final Windows latency-ratio ceiling of `1.65`, while Linux, macOS, and the aggregate remain at `1.5`. In [formal run 33411796790](https://github.com/chainlesschain/chainlesschain/actions/runs/33411796790), pinned to `db53dc2da4`, Linux, macOS, and all three platforms' functional/security metrics passed, but Windows measured `1.6379980224` against the then-current `1.6` ceiling, so the run, workflow aggregate, and OIDC attestation did not succeed; the offline weighted three-platform aggregate was `0.6008293973 < 1.5`. After final threshold commit `917d18b055`, the release owner explicitly accepted the residual risk of no rerun and no final-SHA aggregate/OIDC evidence and closed P2-3. That decision does not rewrite the failed run as successful, is not part of the `0.166.15` artifact, and is not a general release-gate precedent.
>
> **Frozen local source-only snapshot for this cycle:** the anchor is `233e1bdc`; `b8490faa` is the specific evidence-projector commit, `d073bdf3` is the specific tamper-evident `EvolutionLedger` commit (signed append-only entries, independent witness, receipt/verify, and fail-closed handling of missing or inconsistent evidence), and `233e1bdc` then adds mutation-transition subject binding. The original six governance test files reran in that worktree with 6/6 files and 126/126 tests passing in 28.84 seconds, but that supplies no production import, instantiation, or unified wiring; the standalone ledger suite remains at 34 passed and one timeout out of 35. The snapshot is not in GitHub `main@458b342f5f`, `0.166.15`, or formal qualification and must not be described as automatically rewriting or upgrading an active Skill.
>
> **Release-chain closure:** `0.166.15/0.37.77/0.4.107@22db04f559` has a closed chain of immutable tags, three-OS CLI CI and Strict Sandbox, immutable artifacts, Trusted Publishing, signed provenance, IDE host gates, and public-channel readback. Record & Replay's three-OS journey and Desktop Signed Skill qualification remain attached to their exact `0.166.14@ee88125256` evidence chain; `0.166.15` carries the product code forward without rewriting that older evidence identity. Current production reruns of Desktop Signed Skill qualification accept only the current head of protected `main`; tags and ordinary branches fail closed. This tighter trigger boundary likewise does not rewrite the existing successful evidence at `ee88125256`. PDH remains `0.4.59` with optional native SQLite and a `sql.js` fallback.
>
> See the [Agent Platform 0.166.15 guide](docs-site/docs/chainlesschain/agent-platform-release.md), [Agent SDK guide](docs-site/docs/chainlesschain/agent-sdk.md), [GraphRun observation and evaluation guide](docs-site/docs/chainlesschain/cli-team-graph.md), [current CLI Runtime](docs-site/docs/chainlesschain/cli-runtime-current.md), [Graph Kernel design](docs/design/modules/105_Graph_Kernel设计.md), [Agent Platform release-boundary design](docs/design/modules/110-agent-platform-release-boundaries.md), and the [changelog](CHANGELOG.md).
...
## 2026-07-23 Release — **VS Code extension 0.37.31: coordinated compatibility release for CLI 0.162.177**

> VS Code `0.37.30` → **`0.37.31`** ([Open VSX](https://open-vsx.org/extension/chainlesschain/chainlesschain-ide)). The bridge protocol and minimum CLI floor are unchanged; packaged compatibility fixtures now track `0.162.177`. The release passed VSIX metadata checks and real Extension Host install/activation/command/bridge-port smoke tests in VS Code Stable `1.130.0` and the minimum supported `1.85.2`. A Microsoft VS Marketplace token is not configured; stock VS Code users can install the `.vsix` downloaded from Open VSX.

## 2026-07-16 Release — **cc CLI 0.162.168: incremental gap-analysis wiring, batch 2 (real background-agent "waiting for approval" state / hook trace-parent lineage / LSP multi-root + restart backoff ON by default / strict hook merge in true parallel / monorepo subagent worktree + subtree instructions / run_code install audit)**

> CLI-only release (0.162.167 → **0.162.168**, npm `latest`, provenance). Background agents stuck on human approval now write `phase:"waiting_permission"` + a live `pendingApprovals` count into shared state (the dashboard "Needs input" group finally has real data) + the unified install audit extends to `run_code`'s pip auto-install (recognizes `python -m pip install` incl. sudo wrapping; one `install-commands.jsonl` covers every "download and run third-party code" entry point) + hook event lineage (`event_id`/`trace_id`/`parent_id` on all four payload construction sites; child-agent hook events correlate to the parent trace across depth) + strictest-merge decision merging (opt-in `CC_HOOK_STRICT_MERGE=1`, fixing "an earlier `ask` masks a later `block`") with PreToolUse hooks in true parallel + LSP restart backoff ON by default (1s→2s→4s→8s, `CC_LSP_RESTART_BACKOFF_MS=0` to disable) and multi-root `code_intelligence` (files under an `--add-dir` root get their own project's language server) + lazy subtree instruction injection ON by default (first tool touch of a subtree attaches its `cc.md`/`CLAUDE.md`/`AGENTS.md`; `CC_SUBTREE_INSTRUCTIONS=0` to disable) + subagent worktree sparse-checkout passthrough / agent-file `background:true` honored / headless `--add-dir` seeds MCP roots. Command count **175 unchanged**.

## 2026-07-16 Release — **cc CLI 0.162.167: incremental gap-analysis closeout (P0 remote-script execution detection / 4 lifecycle event hooks / doctor orphan subprocesses / 4 misc wirings)**

> CLI-only release (0.162.166 → **0.162.167**, npm `latest`, provenance). A unified "code acquisition" classifier groups 20+ package-manager install commands with download-and-execute remote scripts (`curl … | sh` etc.), raising the risk floor at shell approval + audit trail (`remoteExec` flag; `cat file | sh` and download-only don't false-positive) + real producers for 4 lifecycle hooks (`CwdChanged` / `WorktreeCreate`·`WorktreeRemove` / `InstructionsLoaded` — carries the authoritative instruction-file list, never contents; observe-only, byte-identical with no hooks registered) + `cc doctor` orphan agent-subprocess detection (process start-time anchored against PID reuse, emits `taskkill`/`kill` fix commands) + async-hook persistent re-wake queue with crash recovery / MCP roots/list_changed broadcast / LSP readiness install-gap check / Session Mirror at-rest encryption·deletion·retention·key rotation. Command count **175 unchanged**.

## 2026-07-15 Release — **cc CLI 0.162.166: P1-9 "Capability Manifest & redacted diagnostics bundle" finale (protocol-doc CI byte-diff / offline protocol replay / governance coverage metrics)**

> CLI-only release (0.162.165 → **0.162.166**, npm `latest`, provenance). The protocol capability doc is generated from the single source `capability-manifest.js` with an in-test byte-diff assertion (adding a wire field without regenerating turns tests red — drift can't merge green) + offline protocol replay & forward-compat audit (replay a recorded stream-json session against a negotiation context, fail-closed on any frame carrying a gated-OFF wire field) + governance coverage metrics (high-risk call Ledger/Trace coverage + Plugin/MCP/Skill/Hook provenance traceability; `ok` only at double-100%, offline tools can enforce in CI). Command count **175 unchanged**.

## 2026-07-13 Release — **cc CLI 0.162.165: lean project memory (`--no-project-memory` off switch / `CC_PROJECT_MEMORY=lean` entry-file-only / IDE chat "lean context" ON by default)**

> CLI-only release (0.162.164 → **0.162.165**, npm `latest`, provenance). In docs-heavy repos `cc agent` re-sent the full project memory every turn (~12k tokens/turn measured) — now three tiers: `full` (default, unchanged) / `lean` (keeps the first entry file of `cc.md` > `CLAUDE.md` > `AGENTS.md`; drops `CLAUDE.local.md`, `.claude/rules/*`, `.chainlesschain/rules.md`; ~12k → ~5k measured) / `off` (`--no-project-memory` skips the whole tier). The IDE side ships it via an environment variable (older `cc` safely falls back to full). Command count **175 unchanged**.

## 2026-07-13 Release — **cc CLI 0.162.164: IDE incremental gap-analysis, full batch wired (credential redaction on 3 export surfaces / /rewind branch-from-here / cc session pr-status / diff line-comment anchoring / worktree cleanup safety gate / crash-recovery ledger / redacted diagnostics bundle)**

> CLI-only release (0.162.163 → **0.162.164**, npm `latest`, provenance). `cc session export` / OTLP / `cc doctor --export-bundle` share one recall-first secret scanner (anything leaving the machine fails toward "no leak") + a two-phase side-effect ledger for crash recovery (`--resume` reconciles; started-unsettled non-idempotent ops inject a Recovery notice instead of blind re-run) + `/rewind <n> --branch` forks an independent session from a chosen turn (the parent is never truncated) + review findings anchor to the code they reference (file+baseHash+line text; re-anchor or mark outdated after edits) + `cc session pr-status` (PR/CI status bar + OFF-by-default auto-merge eligibility with exhaustive unmet-item listing) + fail-closed worktree-reaper safety gate (uncommitted/unpushed/PR-linked → keep) + `cc doctor` "Execution context" section + unattended action gate + cross-device operation fingerprint full-tuple verification. Command count **175 unchanged**.

## 2026-07-13 Release — **cc CLI 0.162.163: incremental gap-analysis last-mile runtime wiring (subagent contract full-axis enforcement / cross-agent authority / hook event log + replay / plugin consent lifecycle / /goal loop / --json-schema / OTel)**

> CLI-only release (0.162.162 → **0.162.163**, npm `latest`, provenance). The pure cores announced in 0.162.162 land as real runtime wiring: spawn actually consumes `resolveSubagentContract` (tighten-only) + local headless approval chains carry and verify the authority envelope + session completion-condition engine wired + Turn↔Checkpoint binding persisted with coverage-aware `/rewind` + side-effect-ledger crash-recovery core + persistent Scheduler jitter + `cc plugin consent` re-consent + `cc hook replay`/`events-log` + REPL `/goal` loop + `--json-schema` structured output + unified OTel ids. Subcommands and flags only — **top-level command count 175 unchanged**. Full local test pyramid green (unit+integration 25,008 + e2e 628).

## 2026-07-12 Release — **cc CLI 0.162.162: incremental gap-analysis wrap-up (Subagent contract / Turn-Checkpoint binding / Plugin capability schema / Hooks event bus / JSON Schema / multi-agent Review + LSP diagnostics / Doctor+docs+OTel) + IDE gap P0**

> CLI-only release (0.162.161 → **0.162.162**, npm `latest`, provenance). Against `docs/research/cli/CLAUDE_CODE_CLI_INCREMENTAL_GAP_ANALYSIS_2026-07-12.md` (vs Claude Code v2.1.207), **every P0/P1/P2 section is now closed out**: P1 Subagent contract (tighten-only inheritance/override — widened permissions clamp to the parent, capabilities intersect, memory never granted if the parent denied) + Turn↔Checkpoint binding (FULL/PARTIAL/NONE coverage, restore plan never over-promises) + Plugin capability declaration & options schema (DENY by default + widening requires re-consent + sensitive options rejected from project config); P2 unified Hooks event bus (envelope + `event_id` + strictest-wins decisions + decision-event replay must be explicitly sandboxed) + JSON Schema structured output (Draft 2020-12 subset + `schema_digest` + `structured_result`) + multi-agent Review aggregation core (cross-dimension dedupe + verifier reproduction + structured path/line/category/severity/failure_scenario/evidence) and LSP auto-diagnostics scheduling (debounce/throttle + token cap dropping least-severe first) + Doctor Runtime Checkup (agenda expiry / slow-or-broken hooks / dead plugins·LSP / verbose instruction files) + CLI reference doc-drift detection (built from the 175-command manifest + 26 tools, diffed both ways) + unified OTel ids (nine canonical ids + content redacted `[redacted]` by default + bounded cardinality, every span carries workflow.run_id+session.id) + IDE gap P0 (remote-approval operation fingerprint + session lifecycle state machine). Command count **175 unchanged**. Full local test pyramid green before release (unit 23,182 / integration 1,044 / e2e 628).

## 2026-07-12 Release — **cc CLI 0.162.161: incremental gap-analysis (background state machine / cross-agent authority / credential proxy / completion conditions / monorepo excludes / persistent Scheduler) + bg-\* WS relay protocol hardening**

> CLI-only release (0.162.160 → **0.162.161**, npm `latest`, provenance). Cross-event `trace_id` (run-scoped on every stream-json line, charset ≤128) + background-agent Idle/Needs-input state machine (dashboard grouping) + cross-agent authority envelope (`origin→authority`, only user/permission_tool/authenticated approve-scoped remote may approve; approval binding is constant-time fail-closed against impersonation) + credential proxy (subprocess env masks long-lived credentials with the `cc-cred-redacted:<NAME>` sentinel; audit never records the restored plaintext; approved-host fail-closed injection) + session-level completion-condition engine (exit-zero/file-exists/contains/regex/model/bare + budget + resumable snapshot) + monorepo `instructionExcludes` (glob-exclude legacy/vendor/generated, @import also skipped so credential files can't be pulled into the prompt) + unified persistent Scheduler planner (FNV-1a deterministic jitter against thundering-herd + adaptive wakeup + expiry retirement) + bg-\* WS relay seq-gap replay / outbound backpressure / cross-language fixture contract. Command count **175 unchanged**.

## 2026-07-11 Release — **cc CLI 0.162.160: runtime safety & determinism, 8 batches (sandbox strict mode + dependency/credential safety + deterministic Headless + Subagent contract + MCP lifecycle + Hooks hardening)**

> CLI-only release (0.162.159 → **0.162.160**, npm `latest`, provenance). ⚠️ **Three behavior changes**: `run_code` pip dependencies are no longer auto-installed by default (now opt-in + allowlist + audit JSONL); agent scripts default to the OS temp dir (`persist:true` to reach the project); headless/agent exit codes are refined (max-turns→3 / budget→4 / model error→5 / config error→6). Also `--ephemeral` read-only replay + init events carry protocol/persistence/fingerprint fields + `cc agent --capabilities` machine-readable capability list + `llm.apiKeyHelper` (external command → OS keychain, fail-closed warn-once, never silently substituted) + sandbox `failIfUnavailable` enforced at startup (refuses to start if unavailable, no silent downgrade) + Subagent `disallowedTools`/`maxTurns`/`isolation:worktree` (and fixed a real bug where the subagent tool allowlist was never passed to the LLM) + `cc session rename/prune` · `cc daemon rm` · `cc mcp trust-project` + MCP `list_changed` merged re-fetch + `.mcp.json` fingerprint-change fail-closed re-trust + Hook payload `schema_version` + consecutive-failure circuit breaker. Command count **175 unchanged** (subcommands only).
...
- CLI index: [docs/cli/CLI_COMMANDS_REFERENCE.md](./docs/cli/CLI_COMMANDS_REFERENCE.md) → System Management
...
**Docs restructure**: `docs/cli/CLI_COMMANDS_REFERENCE.md` trimmed from 54.8k → 4.4k (thin index). Full command listings moved to six sub-files under `docs/cli/` (core-phases / managed-agents / blockchain-enterprise / observability / platform / video), with all command comments translated to Chinese (~371 entries). New docs-site pages `cli-federation.md` / `cli-perception.md` added to the VitePress sidebar.
