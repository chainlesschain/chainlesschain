...
> **📦 CLI install**: `npm i -g chainlesschain@0.166.3` (current fully gated build; aliases `cc` / `clc` / `clchain`).
...
>
> **Agent Platform 0.166.3:** on top of TeamMailbox v3, this release adds idle-followup turns, canonical message/handoff projections, the custody-handoff state machine, and the state-backed `SessionMessageFabric`; it also ships schema-derived structured approvals, exact turn/session grants, and durable authority-event recovery. Private child tools still revalidate holder/task/attempt/lease/fence authority on every call; credentials never enter prompts and are not inheritable.
>
> **SDK / Protocol:** npm `@chainlesschain/agent-sdk@0.2.2`, PyPI `chainlesschain-agent-sdk==0.2.2`, and npm `@chainlesschain/agent-protocol@0.1.2` are public. The SDK now exposes canonical ApprovalDecision/binding/requested-permissions callbacks while retaining N-1 boolean wire compatibility and an independent release lifecycle from the CLI.
>
...
>
> **Release-chain closure:** `0.166.3` has a closed chain of tag identity, three-OS CLI CI/Strict matrices, immutable artifacts, Trusted Publishing, signed provenance, and independent registry readback. TypeScript/Python Agent SDK `0.2.2` and Agent Protocol `0.1.2` also completed OIDC publication and public readback. PDH remains `0.4.59` (optional native SQLite with a `sql.js` fallback).
>
