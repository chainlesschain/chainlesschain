...
>
> **Source HEAD 0.163.1 candidate (not published):** current HEAD [`a29eb4203d`](https://github.com/chainlesschain/chainlesschain/commit/a29eb4203d333756fc258b493b0c43af4fc36759) additionally hardens live session-tail races: transcript deletion or restoration after the path check is reclassified through the durable session witness and fails closed instead of leaking a platform-dependent raw `ENOENT`. The candidate also contains bounded long-session compaction, atomic Windows MCP launch, real cross-platform reliability probes, and native signing/notarization/updater credential preflight. [CLI CI](https://github.com/chainlesschain/chainlesschain/actions/runs/31219886076) and [CLI Strict Sandbox](https://github.com/chainlesschain/chainlesschain/actions/runs/31219890201) have passed on that SHA, but the [release workflow](https://github.com/chainlesschain/chainlesschain/actions/runs/31221549835) ran in dry-run mode with the exact-SHA, artifact, and publish jobs skipped. Neither `v-npm-0-163-1` nor registry version `0.163.1` exists, so the production install contract remains `0.163.0`.
>
