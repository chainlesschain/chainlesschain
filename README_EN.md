...
> VS Code `0.37.15` → **`0.37.16`** (Open VSX) / JetBrains `0.4.59` → **`0.4.60`** (JetBrains Marketplace). A 30+ fix plugin audit batch: **Windows cmd.exe argv hardening** (user text with `&`/`|`/quotes is no longer mangled or executed as a second command; cross-spawn algorithm, round-trip-proven against a real cmd.exe) + **JB EDT freeze/deadlock class fixed** (`/stop`/`/compact`/approval replies/tab-close no longer do blocking pipe I/O on the UI thread) + **ghost-text process-tree kill** (a dismissed completion no longer leaves an orphaned `cc complete` burning a full LLM call and holding the SQLite lock) + **blocked background agents visible** (`waiting_permission`/`pendingApprovals` with cc ≥ 0.162.168 — "waiting for you" badge, sorted first, JB Resume enabled) + **budget/retry stream events** (API reconnects and budget stops render as info lines with reasons; fixture-pinned cross-IDE parity) + deep-link containment / plugin Trust-Untrust scope / JB session-index concurrency / pipe CJK byte-carry. Verified: VS 914 tests green + vsix parse-level package check; JB JUnit+smokeTest 1213/0 + verifyPlugin Compatible.
