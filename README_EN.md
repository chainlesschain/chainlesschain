...
> **📦 CLI install**: `npm i -g chainlesschain@0.166.38` (current npm `latest`; aliases `cc` / `clc` / `clchain`).
...
## 2026-09-09 current release — CLI 0.166.38 / VS Code 0.37.92 / JetBrains 0.4.119: governed model ingress and reliable configuration

> npm latest is `0.166.38`, tagged `v-npm-0-166-38` at [`de8ec4e5c8`](https://github.com/chainlesschain/chainlesschain/commit/de8ec4e5c8234087d1fb86a062371b7000931790). The same commit passed the complete [CLI CI](https://github.com/chainlesschain/chainlesschain/actions/runs/34290643410), [Strict Sandbox](https://github.com/chainlesschain/chainlesschain/actions/runs/34290648161), and [IDE Extensions](https://github.com/chainlesschain/chainlesschain/actions/runs/34300419533) gates, followed by [npm publishing and public-install checks](https://github.com/chainlesschain/chainlesschain/actions/runs/34295661756).
>
> Direct CLI streams, intent requests, WebSocket chat, and Hub analysis, skill, and entity-resolution model calls now share host-provided Evolution governance. Incomplete provider streams, model-identity drift, and governance failures fail closed instead of being hidden by optional commentary or fallbacks. Evolution file witnesses use immutable 256-record segments and invalidate verified caches when the trust epoch changes.
>
> VS Code and JetBrains atomically save model connection forms and read back redacted settings before confirming success. Connection tests give reasoning models a bounded 1,024-token output allowance and distinguish reasoning exhaustion from empty responses. `0.166.38` also publishes Agent Protocol `0.1.9` and Context Memory Kernel `0.1.1`; the TypeScript Agent SDK is `0.2.9`, while the Python SDK remains `0.2.8`.
>
> [Open VSX 0.37.92](https://open-vsx.org/extension/chainlesschain/chainlesschain-ide) and [JetBrains Marketplace 0.4.119](https://plugins.jetbrains.com/plugin/32208-chainlesschain-ide-bridge) are public and recommend CLI `0.166.38`. Session Core `0.3.12`, Core DB `0.1.5`, and PDH `0.4.60` retain independent release identities; the extension is still not published to Microsoft Marketplace.
>
> **Source and deployment boundary:** local `main@a238e6c245` embeds CLI `0.166.38` and adds post-release wiring that routes Desktop Personal Data Hub resolver/Skill IPC and the embedded Web Shell through opaque governance authority held only by the main process. It also verifies that CLI-owned background workers and the Desktop Coding Agent `cc serve` bridge re-enter the canonical loader, while legacy ImageGen content entry points fail closed before cache, provider selection, or fallback. The verified GitHub `main@1895749692` and Gitee `main@3806866d80` both include release commit `de8ec4e5c8`, but not yet those Desktop follow-ups. CLI/Hub governance wiring is included in the public npm artifact, but Evolution Workbench still needs a trusted descriptor, trust root, identity, approvals, PKI/KMS, witness, and grader from the target deployment. Desktop native installers remain independently qualified artifacts. See the [upgrade guide](docs-site/docs/chainlesschain/agent-platform-release.md), [current CLI runtime](docs/design/cli-runtime-current.md), and [module 113 design](docs/design/modules/113-governed-desktop-model-ingress-design.md).
