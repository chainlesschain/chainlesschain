...
> **📦 CLI install**: `npm i -g chainlesschain@0.166.65` (current npm `latest`; aliases `cc` / `clc` / `clchain`).
...
## 2026-09-19 current release — CLI 0.166.65 / Open VSX 0.37.109 / JetBrains 0.4.130
...
npm latest is `0.166.65`; immutable tag `v-npm-0-166-65` resolves to exact commit `17509017a2`. Its [CLI CI](https://github.com/chainlesschain/chainlesschain/actions/runs/35438409239) and [CLI Strict Sandbox](https://github.com/chainlesschain/chainlesschain/actions/runs/35438410004) passed every configured Linux, Windows, and macOS job. The [npm OIDC publication and public readback](https://github.com/chainlesschain/chainlesschain/actions/runs/35440962350) succeeded with signed provenance, public `latest=0.166.65`, tarball integrity, and a clean installation. Agent Protocol `0.1.11`, Context Memory Kernel `0.1.4`, Agent SDK TS/Python `0.2.11/0.2.9`, and Session Core `0.3.12` retain independent release boundaries.

This release fixes an IDE long-task stall reported as `Repeated file reads or identical large command outputs`: recovery may read an explicit small range of a known target, and runtime-generated pause results no longer feed the repeated-read or identical-output counters that caused the pause. Genuine repeated reads, unrelated remote discovery, policy rejections, and six real no-progress attempts still fail closed, so approval, sandbox, and execution boundaries are unchanged.

This release completes RSIAgent batches 9–23 on top of the existing Broad/Deep recovery contract: host-enforced token/tool/wall-clock budgets and four-role Ed25519 receipts; real Volcengine usage/cost settlement with durable readback; an independent PM business grader; a Desktop fixed-`SELECT` outcome port bound to the signed database path; and SQLite pre/post/failure seals, a tainted-host gate, durable transition commits, authenticated chain-head recovery, and transition-bound SQLite recovery snapshots. Desktop still exposes only read-only readiness. `readyForExecution`, `runtimeVerified`, `authenticated`, and `qualifiesForPromotion` all remain `false`, so this does not enable Explorer or automatic promotion.

The CLI artifact source boundary is `17509017a2`; the VS Code and JetBrains artifact boundary is `063d491dab`. [Open VSX 0.37.109](https://open-vsx.org/extension/chainlesschain/chainlesschain-ide) and [JetBrains Marketplace 0.4.130](https://plugins.jetbrains.com/plugin/32208-chainlesschain-ide-bridge) are public and recommend CLI `0.166.65`; the [exact-commit IDE gate](https://github.com/chainlesschain/chainlesschain/actions/runs/35444203114), [ARM64 gate](https://github.com/chainlesschain/chainlesschain/actions/runs/35444203032), [Open VSX publication](https://github.com/chainlesschain/chainlesschain/actions/runs/35445857141), and [JetBrains publication](https://github.com/chainlesschain/chainlesschain/actions/runs/35447194795) succeeded. The Microsoft VS Code Marketplace backfill stopped before upload because its publishing credential was not configured, so no Microsoft listing is claimed. Desktop native installers remain separately qualified artifacts. Production authorities, remote durability, a workspace recovery set, atomic restore, and the real Electron DID/RBAC PM E2E remain outstanding. See [module 112 design](docs/design/modules/112-governed-skill-evolution-design.md), the [governed evolution user guide](docs-site/docs/chainlesschain/governed-skill-evolution.md), and the [upgrade guide](docs-site/docs/chainlesschain/agent-platform-release.md).
