...
> **📦 CLI install**: `npm i -g chainlesschain@0.163.1` (current fully gated build; aliases `cc` / `clc` / `clchain`).
...
>
> ```bash
> npm i -g chainlesschain@0.163.1 --registry https://registry.npmjs.org
> ```
>
...
## 2026-08-08 Current mainline — **v5.0.3.135 / CLI 0.163.1 / PDH 0.4.57 / Open VSX 0.37.45 / JetBrains 0.4.81**

> **Release status:** `chainlesschain@0.163.1` is the current npm `latest` and production recommendation; immutable tag `v-npm-0-163-1` resolves to exact commit [`e3f56b11e2`](https://github.com/chainlesschain/chainlesschain/commit/e3f56b11e27ae1bd5d19ad8638434843c244aa68). That SHA passed [CLI CI](https://github.com/chainlesschain/chainlesschain/actions/runs/31240892299) and [CLI Strict Sandbox](https://github.com/chainlesschain/chainlesschain/actions/runs/31240892177) across Ubuntu, Windows, and macOS. The [dedicated npm release](https://github.com/chainlesschain/chainlesschain/actions/runs/31246063305) completed its exact-SHA gate, immutable tarball/CycloneDX SBOM, Trusted Publishing, SLSA provenance, registry readback, and npmmirror synchronization. The public registry tarball is byte-identical to the workflow artifact, with SHA-256 `d9e09e25c6086e0777e97a670105649e3a7e5fb1c2816e1834557da32157cbee`.
>
> **CLI 0.163.1:** carries forward the `0.163.0` canonical-session authority, pinned npm-backed MCP execution capsules, and native update recovery, then adds durable-session-witness race hardening, bounded long-session compaction, atomic Windows MCP launch, a reproducible Web Panel build, and Node 22 standalone bases. The same exact SHA passed the [unsigned six-target native gate](https://github.com/chainlesschain/chainlesschain/actions/runs/31240927257) and the [formal two-hour reliability gate on all three operating systems](https://github.com/chainlesschain/chainlesschain/actions/runs/31240943985). Each reliability artifact records 8/8 scenarios, 1,000 turns, at least 7,200 seconds of continuous duplex operation, and passing RSS, FD/handle, and descendant-process checks. The second command-lifecycle observation cycle continues while all 25 compatibility aliases remain available.
>
> **Native boundary:** the closed npm `0.163.1` release does not imply a signed Desktop/native release. The six-target validation records are fixed to `signed=false` and `releaseEligible=false`; the repository still lacks Windows Authenticode, macOS signing/notarization, and updater keys. Signed public native assets, public fresh-install/upgrade/rollback readback, and IDE ARM64 therefore remain open.
>
...
![CLI](https://img.shields.io/badge/cli-0.163.1-blue.svg)
