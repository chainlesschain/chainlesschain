...
> **📦 CLI install**: `npm i -g chainlesschain@0.166.62` (current npm `latest`; aliases `cc` / `clc` / `clchain`).
...
>
> ```bash
> npm i -g chainlesschain@0.166.62 --registry https://registry.npmjs.org
> ```
>
...
## 2026-09-18 current release — CLI 0.166.62 / Open VSX 0.37.107 / JetBrains 0.4.128
...
npm latest is `0.166.62`; immutable tag `v-npm-0-166-62` resolves to exact commit `77572e7355`. Its [CLI CI](https://github.com/chainlesschain/chainlesschain/actions/runs/35267622585) and [CLI Strict Sandbox](https://github.com/chainlesschain/chainlesschain/actions/runs/35267621821) passed every configured Linux, Windows, and macOS job, and the [npm OIDC exact-SHA publication/public-readback gate](https://github.com/chainlesschain/chainlesschain/actions/runs/35274352115) succeeded. The official registry reported `latest=0.166.62` on 2026-09-18. Agent Protocol `0.1.11`, Context Memory Kernel `0.1.4`, Agent SDK TS/Python `0.2.11/0.2.9`, and Session Core `0.3.12` retain independent release boundaries.

This release builds on durable IDE task notes and fresh-session handoff with governed PM Broad/Deep exploration contracts, an independent read-only grader, quiescent recovery snapshots, Evolution Ledger/Artifact-authority anchoring, and exact-SHA three-OS recovery evidence. Desktop receives only a branded read-only readiness projection; `readyForExecution` remains fixed at `false`, so this does not enable Explorer or automatic promotion. The runtime also normalizes Windows recovery temp paths, preserves live state locks, bounds transient ACL retries, verifies unchanged reused-package provenance during npm publication, and bounds scheduler-soak leases. Synthetic EROFS/ENOSPC/authority fault injection is not evidence of real disks, remote services, or physical power-loss qualification.

The CLI artifact source boundary is `77572e7355`; the IDE artifact source boundary is `6edebc8b25`. [Open VSX 0.37.107](https://open-vsx.org/extension/chainlesschain/chainlesschain-ide) and [JetBrains Marketplace 0.4.128](https://plugins.jetbrains.com/plugin/32208-chainlesschain-ide-bridge) are public and recommend CLI `0.166.62`; the [VS Code release gate](https://github.com/chainlesschain/chainlesschain/actions/runs/35291245830) and [JetBrains release gate](https://github.com/chainlesschain/chainlesschain/actions/runs/35291246219) passed at the IDE source commit. Microsoft VS Code Marketplace and Desktop native installers remain independent release surfaces and do not inherit npm or IDE-marketplace evidence. See [module 112 design](docs/design/modules/112-governed-skill-evolution-design.md), the [governed evolution user guide](docs-site/docs/chainlesschain/governed-skill-evolution.md), and the [upgrade guide](docs-site/docs/chainlesschain/agent-platform-release.md).
