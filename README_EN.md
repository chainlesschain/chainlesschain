...
> **📦 CLI install**: `npm i -g chainlesschain@0.163.6` (current fully gated build; aliases `cc` / `clc` / `clchain`).
...
## 2026-08-13 Current mainline — **v5.0.3.135 / CLI 0.163.6 / PDH 0.4.57 / Open VSX 0.37.51 / JetBrains 0.4.87**

> **Release status:** `chainlesschain@0.163.6` is the current npm `latest` and production recommendation. Immutable tag `v-npm-0-163-6` resolves to exact commit [`85c3577c88`](https://github.com/chainlesschain/chainlesschain/commit/85c3577c887003fea98d0a159603cd359506f09b); that SHA passed [CLI CI](https://github.com/chainlesschain/chainlesschain/actions/runs/31595865423), [CLI Strict Sandbox](https://github.com/chainlesschain/chainlesschain/actions/runs/31595865206), the [dedicated npm release](https://github.com/chainlesschain/chainlesschain/actions/runs/31595865181), and [independent public readback](https://github.com/chainlesschain/chainlesschain/actions/runs/31597577056). npm now reports `latest=0.163.6`; the public tarball SHA-1 is `18bb9d807a4a35a36cf9049dcc15f67eb47bbfa6`.
>
> **CLI 0.163.6:** the unified scheduler runtime now covers Agenda monitors, Loop iterations, Automation cron flows, Routine GitHub polling, channel automation events, and the standalone daemon. Agenda and Cowork share canonical IANA timezone, DST, and collapsed missed-run semantics. Agenda, Routine, Cowork, Automation, and Loop also share revision-bound permission/budget authority; missing, stale, disabled, exhausted, or inconsistent policy state fails closed, while retries reuse the original reservation instead of charging twice.
>
...
>
> **Scheduler source increment:** mainline commit `c7d49beaa1` closes durable human adjudication for `*_OUTCOME_UNKNOWN` dead letters. After every scheduler host is stopped, dispatched work is drained, and the external result is verified, an operator records a CAS-bound decision using the evidence digest, attempt, and fence. `confirmed_applied` settles without replay; `confirmed_not_applied` authorizes one bounded execution. This has not entered a newer npm release, so `0.163.6` users must not assume the command is published. Complete mixed-version migration/rollback, scheduler disk-fault closure, and the three-OS long-soak matrix remain open.
>
> **Native boundary:** the closed npm `0.163.6` release does not imply a signed Desktop/native release. Unsigned six-target validation remains fixed to `signed=false` and `releaseEligible=false`; Linux signing, Windows Authenticode, macOS signing/notarization, updater keys, and public signed fresh-install/upgrade/rollback readback remain open.
>
