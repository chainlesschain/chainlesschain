...
> **📦 CLI install**: `npm i -g chainlesschain@0.166.7` (current fully gated build; aliases `cc` / `clc` / `clchain`).
...
>
> ```bash
> npm i -g chainlesschain@0.166.7 --registry https://registry.npmjs.org
> ```
>
...
## 2026-08-28 Current mainline — **v5.0.3.135 / CLI 0.166.7 production recommendation and npm latest / Agent SDK 0.2.4 / Agent Protocol 0.1.5 / PDH 0.4.59**

> **Release status:** `chainlesschain@0.166.7` is the fully gated production recommendation and npm `latest`. Immutable tag `v-npm-0-166-7` resolves to exact commit [`19834a1845`](https://github.com/chainlesschain/chainlesschain/commit/19834a18457c1b763b5a7a2cfada713340e7c273). That SHA passed [CLI CI](https://github.com/chainlesschain/chainlesschain/actions/runs/33154181913), [CLI Strict Sandbox](https://github.com/chainlesschain/chainlesschain/actions/runs/33154181656), and [immutable npm artifact/provenance public readback](https://github.com/chainlesschain/chainlesschain/actions/runs/33154181649).
>
> **Agent Platform 0.166.7:** carries forward `0.166.6`'s 37 canonical payload unions, cross-language causal conformance, fixed-capability App Server pilot, experimental WebSocket, and bounded Agent IPC. This release routes CLI graph, Team, distributed-team, Cowork, Scheduler, and App Server entries through the Graph Kernel cutover ledger. Durable authority now binds writer, store, checked-out source evidence, leases/fences, receipts, takeover, migration, and crash/resume. Retired runtimes keep only explicit read-only history; unclassified or mutating legacy routes fail closed with canonical replacement targets.
>
> **SDK / Protocol:** npm `@chainlesschain/agent-sdk@0.2.4`, PyPI `chainlesschain-agent-sdk==0.2.4`, and npm `@chainlesschain/agent-protocol@0.1.5` are public. They export per-event payload unions alongside the event inventory, typed envelopes, and strict validators while transports preserve unknown future events losslessly. Package versions evolve independently from wire protocol v1.
>
> **Desktop Graph and Skill-security source increments:** locally committed source `3e4d70eb52` through `2286267dea` adds a Graph Run Debugger to AI Chat, Workflow Monitor, and Agent Dashboard (topology, timeline, budget, trace, causality, and historical replay). External Cowork handlers now require Ed25519 signatures, pre-execution digest rereads, a one-shot isolated worker, and host capability brokers. Every bundled handler is bound to a generated capability catalog; fixed-domain, dynamic-public, loopback-model, network-diagnostic, environment-value, and shell-process surfaces use separate bounded brokers, and secrets, argv, and output never enter audit records. These are Desktop source capabilities, not bytes in the npm `0.166.7` package.
>
> **IDE release:** Open VSX `0.37.72` and JetBrains Marketplace `0.4.103` are public and read back; Open VSX downloads now exceed **31,000**. Both re-certify CLI `0.166.7` Graph authority, fenced recovery, and legacy containment while leaving projections read-only and CLI-authoritative. Their tags point to the same `19834a1845` release identity, and the [IDE release gate](https://github.com/chainlesschain/chainlesschain/actions/runs/33154181439) succeeded. Microsoft VS Code Marketplace remains unpublished.
>
> **Release and source boundary:** `19834a1845` is the exact CLI `0.166.7`, VS Code `0.37.72`, and JetBrains `0.4.103` release identity. Local Desktop/Skill commit `2286267dea` and fetched upstream collaboration, federation, gossip, and mesh retained-state maintenance at `5edef7544b` are later source states and do not inherit that release authorization. A GraphRun only unifies run identity, authority, budget, and revision; the Task Graph, dynamic Agent Tree, and Artifact/Trace evidence projection must not be collapsed into one graph.
>
...
>
> **Release-chain closure:** `0.166.7@19834a1845` has a closed chain of tag identity, Linux/Windows/macOS CLI CI and Strict Sandbox matrices, immutable artifacts, Trusted Publishing, signed provenance, and independent registry readback. TypeScript/Python Agent SDK `0.2.4` and Agent Protocol `0.1.5` remain public with no package-byte changes in this CLI-only release. The preceding `0.166.5` exact commit's 1,800.21-second App Server soak processed 2,427,887 requests with zero unexpected errors, zero drain leftovers, and 0.762% RSS growth. PDH remains `0.4.59` (optional native SQLite with a `sql.js` fallback).
>
> See the [Agent SDK guide](docs-site/docs/chainlesschain/agent-sdk.md), [CC App Server](docs-site/docs/chainlesschain/cli-app-server.md), [Agent Kernel](docs-site/docs/chainlesschain/cli-agent-kernel.md), [GraphRun observability and evaluation](docs-site/docs/chainlesschain/cli-team-graph.md), [Desktop Graph and Skill security guide](docs-site/docs/chainlesschain/desktop-graph-skill-security.md), [current CLI Runtime guide](docs-site/docs/chainlesschain/cli-runtime-current.md), [Graph Kernel design](docs/design/modules/105_Graph_Kernel设计.md), [Desktop Cowork Skill execution-security design](docs/design/modules/109_Desktop_Cowork_Skill_Execution_Security.md), and [changelog](CHANGELOG.md).
...
![CLI](https://img.shields.io/badge/cli-0.166.5-blue.svg)
