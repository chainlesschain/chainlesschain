...
> **📦 CLI install**: `npm i -g chainlesschain@0.166.9` (current fully gated build and npm `latest`; aliases `cc` / `clc` / `clchain`).
...
>
> ```bash
> npm i -g chainlesschain@0.166.9 --registry https://registry.npmjs.org
> ```
>
...
## 2026-08-29 Current mainline — **v5.0.3.135 / CLI 0.166.9 production recommendation and npm latest / Agent SDK 0.2.5 (TS) and 0.2.6 (Python) / Agent Protocol 0.1.6 / PDH 0.4.59**

> **Release status:** `chainlesschain@0.166.9` is the fully gated production recommendation and npm `latest`. Immutable tag `v-npm-0-166-9` resolves to exact commit [`222396f6a8`](https://github.com/chainlesschain/chainlesschain/commit/222396f6a8429d4b862292a2572067a5cacb1003). That SHA passed the [three-OS CLI CI](https://github.com/chainlesschain/chainlesschain/actions/runs/33228796205), [CLI Strict Sandbox](https://github.com/chainlesschain/chainlesschain/actions/runs/33232869268), and [npm Trusted Publishing, provenance, and public readback](https://github.com/chainlesschain/chainlesschain/actions/runs/33232869286).
>
> **Agent Platform 0.166.9:** fully carries forward `0.166.8` durable Graph history, HumanTask quorum, migration/retirement evidence, Team scheduling fairness, temporal message custody, and multi-surface single-winner approval CAS. Its real Playwright UI replay driver admits only the reviewed `observe/click/type/select/assert` vocabulary and blocks filesystem, HTTP(S), and WebSocket access. Reports retain domain-separated digests and bounded structural metadata, never selectors, typed values, page text, URLs, or screenshot content.
>
> **Compatibility and fail-closed fallback:** the optional Codex App Server adapter admits only explicitly listed upstream patch versions. Prereleases and unverified future patches fall back to stable `codex exec --json` before turn admission. A dedicated three-OS matrix generates each upstream schema and verifies `initialize → initialized → thread/list` plus the independent JSONL fallback without making a model request.
>
> **SDK / Protocol:** npm `@chainlesschain/agent-sdk@0.2.5`, PyPI `chainlesschain-agent-sdk==0.2.6`, and npm `@chainlesschain/agent-protocol@0.1.6` are public. Python `0.2.5` was withheld because its install metadata named a CLI that was not yet public; `0.2.6` keeps the same runtime API and fixes that metadata. The generated 37-payload union, strict validators, cross-language causal conformance, and lossless unknown-event transport remain intact.
>
> **Graph, approvals, and Team:** retirement now requires N-1 definition backups, rollback digests, replay validation, replacement reachability, historical-read proof, and zero-success legacy-writer observations. HumanTask waits release Agent capacity and bind revision/attempt/operation, quorum, separation of duties, and cancel/decision CAS. Android, iOS, Web Panel, Desktop, VS Code, and JetBrains use single-winner settlement; dependency/scope aging and a fairness SLO prevent long-lived Team starvation.
>
> **Desktop signing and Skill qualification:** the same source identity adds macOS entitlements/notarization hooks, signed-install and launch probes, and an exact-SHA Desktop Skill qualification producer. External handlers still require Ed25519 signatures, digest rereads, and one-shot isolated workers; bundled-handler network, filesystem, process, environment, and credential access stays behind bounded brokers. This is Desktop/native source and qualification evidence, not npm CLI package bytes or proof of public native fresh-install/upgrade/rollback distribution.
>
> **IDE release:** Open VSX `0.37.73` is public and read back with more than **32,000** downloads. JetBrains source and immutable tag are `0.4.104`, while the public Marketplace still serves `0.4.103`; user installation guidance therefore remains `0.4.103`. The [IDE release gate](https://github.com/chainlesschain/chainlesschain/actions/runs/33230594120) succeeded. Microsoft VS Code Marketplace remains unpublished.
>
> **Release and source boundary:** `222396f6a8` is the shared Git identity for CLI `0.166.9`, VS Code `0.37.73`, JetBrains `0.4.104` source/tag, and TypeScript SDK `0.2.5`, but every public artifact is still governed by its own registry readback. Later GitHub `main` work on real-collaboration quality and IDE ARM64 Robot retry does not inherit `0.166.9` release authorization. GraphRun unifies run identity, authority, budget, and revision only; Task Graph, dynamic Agent Tree, and Artifact/Trace evidence remain distinct projections.
>
> **Release-chain closure:** `0.166.9@222396f6a8` has a closed chain of tag identity, Linux/Windows/macOS CLI CI and Strict Sandbox matrices, immutable artifacts, Trusted Publishing, signed provenance, and independent registry readback. The real UI replay, Codex App Server compatibility, and signed Desktop Skill qualification matrices also passed at that exact SHA. The preceding `0.166.5` 1,800.21-second App Server soak remains predecessor-only evidence. PDH remains `0.4.59` with optional native SQLite and a `sql.js` fallback.
>
> See the [Agent Platform 0.166.9 guide](docs-site/docs/chainlesschain/agent-platform-release.md), [Agent SDK guide](docs-site/docs/chainlesschain/agent-sdk.md), [Graph Kernel operations](docs-site/docs/chainlesschain/cli-graph-kernel.md), [Desktop Graph debugging and Skill security](docs-site/docs/chainlesschain/desktop-graph-skill-security.md), [current CLI Runtime](docs-site/docs/chainlesschain/cli-runtime-current.md), [Graph Kernel design](docs/design/modules/105_Graph_Kernel设计.md), [Agent Platform release-boundary design](docs/design/modules/110-agent-platform-release-boundaries.md), and the [changelog](CHANGELOG.md).
