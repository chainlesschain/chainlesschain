...
console.log("消息阈值:", config.threshold);
console.log("后台运行:", config.isRunning);

// 更新配置
sessionManager.updateAutoSummaryConfig({
  enabled: true,
  threshold: 5, // 5条消息后自动生成摘要
  interval: 300000, // 后台检查间隔（5分钟）
...
// 获取没有摘要的会话
const pending = await sessionManager.getSessionsWithoutSummary({
  limit: 50,
  minMessages: 5,
});

// 触发批量摘要生成
const result = await sessionManager.triggerBulkSummaryGeneration({
  overwrite: false,
  limit: 100,
});
console.log("已加入队列:", result.queued);

// 获取自动摘要统计
const stats = await sessionManager.getAutoSummaryStats();
console.log("总会话数:", stats.totalSessions);
console.log("有摘要:", stats.withSummary);
console.log("自动生成:", stats.autoGenerated);
...
| `enableAutoSummary`       | true   | 启用自动摘要生成 (v0.22.0)         |
| `autoSummaryThreshold`    | 5      | 触发自动摘要的消息数阈值 (v0.22.0) |
| `autoSummaryInterval`     | 300000 | 后台摘要检查间隔（毫秒）(v0.22.0)  |
| `enableBackgroundSummary` | true   | 启用后台摘要生成 (v0.22.0)         |
...
| `session:get-auto-summary-config`    | 获取自动摘要配置 (v0.22.0) |
| `session:update-auto-summary-config` | 更新自动摘要配置 (v0.22.0) |
| `session:start-background-summary`   | 启动后台摘要生成 (v0.22.0) |
| `session:stop-background-summary`    | 停止后台摘要生成 (v0.22.0) |
| `session:get-without-summary`        | 获取无摘要会话 (v0.22.0)   |
| `session:trigger-bulk-summary`       | 触发批量摘要 (v0.22.0)     |
| `session:get-auto-summary-stats`     | 获取自动摘要统计 (v0.22.0) |
...
- **自动摘要测试**: `desktop-app-vue/scripts/test-auto-summary.js`
...
| `executing`  | file*\*, html*\_, css\__, js*\*, git*_, code\_\_ | 写入和修改 |
| `validating` | file*reader, test*\_, validate\_\_, check\_\*    | 只读和测试 |
