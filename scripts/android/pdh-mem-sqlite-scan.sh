...
#         （pid 须是已登录、且已打开过目标 DB 的“热”进程；冷启动进程内存里没有明文页）
...
awk '/rw-p/{print $1}' "/proc/$PID/maps" | while read range; do
  ...
  size=$((end - start))
  [ "$size" -lt 8192 ] && continue
  [ "$size" -gt 268435456 ] && continue   # 跳过 >256MB（dalvik 大空间，非 DB 页缓存）
  sblk=$((start / 4096)); nblk=$((size / 4096))
  dd if="/proc/$PID/mem" bs=4096 skip="$sblk" count="$nblk" 2>/dev/null > "$OUT/_r.bin" || continue
  grep -abo "SQLite format 3" "$OUT/_r.bin" 2>/dev/null | cut -d: -f1 | while read off; do
    ...
    nm="cc_$(echo "$range" | cut -d- -f1)_$off"
