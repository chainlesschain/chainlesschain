#!/usr/bin/env bash
# Atomic swap re-deploy (docs + www) for VS Code extension 0.22.0 changelog.
# docs.chainlesschain.com: VitePress build with the [Unreleased] "Added — IDE
# Bridge 0.22.0" changelog entry. www.chainlesschain.com: Astro build with the
# ide page's new feature cards (会话 tabs / App Preview / inline comments /
# openMultiDiff). design site unchanged this round, skipped.
# Per-site: extract to .new, verify html count + index.html, swap with a
# timestamped .bak rollback point, fix owner.
set -u
STAMP="20260615-ide022"
WROOT=/www/wwwroot
...
deploy_one docs.chainlesschain.com /tmp/deploy-docs-ide022.tar.gz 500
deploy_one www.chainlesschain.com  /tmp/deploy-www-ide022.tar.gz  18
echo "=== DONE ==="
