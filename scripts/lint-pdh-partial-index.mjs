 ...
 *
 * Failure mode: `CREATE UNIQUE INDEX IF NOT EXISTS ... ON
 * <t>(source_adapter, [source_scope,] source_original_id)`
 ...
 * `(source_adapter, source_original_id)` or
 * `(source_adapter, source_scope, source_original_id)` *immediately after*
 * `CREATE UNIQUE INDEX ... ON ...`, the WHERE clause
 * `WHERE source_original_id IS NOT NULL` must appear within 300 chars of the
 * tuple. This tolerates the SQL split across multiple template-literal
 * concatenations (`...) ` + `WHERE ...`).
 *
...
const TUPLE_RE =
  /CREATE\s+UNIQUE\s+INDEX[\s\S]{1,200}?\(\s*source_adapter\s*,\s*(?:source_scope\s*,\s*)?source_original_id\s*\)/gi;
...
if (isMain) {
  ...
  if (total > 0) {
    ...
    console.error(
      "[source_scope,] source_original_id)` without the required partial WHERE",
    );
    console.error(
