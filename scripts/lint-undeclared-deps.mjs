...
/** Collect external import specifiers from a source file. */
...
let rootPkg;
try {
  rootPkg = JSON.parse(readFileSync(join(REPO_ROOT, "package.json"), "utf-8"));
} catch (err) {
...
if (staleBaseline.length > 0) {
  ...
  for (const s of staleBaseline) console.log(`    • ${s.package} → ${s.base}`);
  console.log("");
}
...
console.log("(or `optionalDependencies` if it's a soft/fallback driver), then");
console.log("bump the package version + propagate the dep-range to any sibling");
console.log("workspace package that pins it (trap #28). A devDependency is NOT");
...
console.log("to ALLOWED_AMBIENT in scripts/lint-undeclared-deps.mjs instead.");
console.log("");
