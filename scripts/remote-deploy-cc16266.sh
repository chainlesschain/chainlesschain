#!/usr/bin/env bash
# Atomic swap deploy for cc CLI 0.162.66 docs refresh — docs + www.
# (design unchanged since v5.0.3.111 deploy → skipped.) Uploaded to /tmp and run
# via ssh. Per-site: extract to .new, verify html count + index.html, swap with a
# timestamped .bak rollback point, fix ownership, drop the tarball.
set -u
...
deploy_one docs.chainlesschain.com /tmp/deploy-docs-cc16266.tar.gz 500
deploy_one www.chainlesschain.com  /tmp/deploy-www-cc16266.tar.gz  18
echo "=== DONE ==="
