...
const HANDLERS_TO_TEST = {
  ...
  "Sync IPC": ["sync:start", "sync:stop", "sync:get-status", "sync:get-config"],
  "Notification IPC": [
  ...
  for (const [category, handlers] of Object.entries(HANDLERS_TO_TEST)) {
    ...
    handlers.forEach((handler) => {
      ...
      if (isRegistered) {
        ...
        console.log(
          `  ✅ ${handler} (${listenerCount} listener${listenerCount > 1 ? "s" : ""})`,
        );
        registeredHandlers++;
    ...
    const categorySuccess =
      results[category].registered === results[category].total;
    const percentage = (
      (results[category].registered / results[category].total) *
      100
    ).toFixed(1);
    ...
    console.log(
      `  结果: ${results[category].registered}/${results[category].total} (${percentage}%) ${categorySuccess ? "✅" : "❌"}\n`,
    );
...
function printAllRegisteredHandlers() {
  ...
  handlers.forEach(({ channel, listenerCount }) => {
    console.log(
      `  ✅ ${channel} (${listenerCount} listener${listenerCount > 1 ? "s" : ""})`,
    );
  });
