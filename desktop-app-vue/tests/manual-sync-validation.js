...
assert(localRecord.sync_status === 'synced', 'toLocal默认设置sync_status为synced');
assert(localRecord.user_id === 'user-456', '字段名转换（camelCase -> snake_case）');
...
assert(preservedRecord.sync_status === 'pending', 'preserveLocalStatus保留本地sync_status');
assert(preservedRecord.synced_at === 1703596700000, 'preserveLocalStatus保留本地synced_at');
...
const updateRecord = mapper.toLocalForUpdate(backendRecord, 'projects', existingRecord);
...
(async () => {
  ...
  assert(result === 'success after retry' && callCount === 3, '失败任务自动重试');
})();
...
(async () => {
  ...
  const tasks = Array.from({ length: 10 }, () => syncQueue.enqueue(createTask()));
...
(async () => {
  ...
  const createTask = (id, delay = 50) => async () => {
  ...
  // 前2个是先开始的低优先级任务
  assert(executionOrder[0] === 'low1' && executionOrder[1] === 'low2', '先启动的任务先执行');

  // 后续任务应该按优先级排序：high1, high2, mid1, low3
  const queuedTasks = executionOrder.slice(2);
  assert(queuedTasks.indexOf('high1') < queuedTasks.indexOf('mid1'), '高优先级任务在中优先级之前');
  assert(queuedTasks.indexOf('high2') < queuedTasks.indexOf('mid1'), '高优先级任务在中优先级之前');
  assert(queuedTasks.indexOf('mid1') < queuedTasks.indexOf('low3'), '中优先级任务在低优先级之前');
})();
