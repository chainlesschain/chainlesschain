...
const createMockSkillManager = () => ({
  ...
  getSkillTools: vi.fn().mockImplementation(async (skillId) => {
    const toolMappings = {
      ...
      'skill-3': [
        { tool_id: 'tool-4', name: 'csv_reader' },
      ],
    };
...
describe('SkillRecommender', () => {
  ...
  describe('getPopularSkills()', () => {
    ...
    it('should sort by usage and success rate', async () => {
      ...
      if (popular.length > 1) {
        const score1 = popular[0].usage_count * (popular[0].success_count / popular[0].usage_count);
        const score2 = popular[1].usage_count * (popular[1].success_count / popular[1].usage_count);
  ...
  describe('searchSkills()', () => {
    ...
    it('should include disabled skills when enabledOnly is false', async () => {
      const results = await recommender.searchSkills('disabled', { enabledOnly: false });
