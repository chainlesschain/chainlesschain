 ...
 * 图形提取器测试
 *
 * 测试 src/main/graph-extractor.js
 * 目标覆盖率: 80%
 *
 * 测试范围:
 * - Wiki链接提取 [[title]]
...
import GraphExtractor from '../../src/main/graph-extractor.js';
...
describe('GraphExtractor - 知识图谱关系提取器', () => {
  ...
  beforeEach(() => {
    ...
    testNotes = [
      ...
      },
      {
        id: 'note-002',
        title: 'Vue框架',
        content: 'Vue.js是渐进式框架。查看[官方文档](https://vuejs.org)了解更多。',
        created_at: Date.now(),
        updated_at: Date.now()
      },
      {
      ...
      },
      {
        id: 'note-004',
        title: '前端工程化',
        content: '包含webpack、vite等工具。代码示例：\n```js\nimport { defineConfig } from "vite"\nimport vue from "@vitejs/plugin-vue"\n```',
        created_at: Date.now(),
        updated_at: Date.now()
      }
    ];
  ...
  describe('extractMarkdownLinks - Markdown链接提取', () => {
    ...
    it('should extract multiple markdown links', () => {
      const content = '参考[Vue文档](https://vuejs.org)和[React文档](https://reactjs.org)';
      const links = extractor.extractMarkdownLinks(content);
  ...
  describe('findPotentialLinks - 潜在链接发现', () => {
    ...
    it('should calculate confidence based on title length', () => {
      ...
      const jsSuggestion = suggestions.find(s => s.title === 'JavaScript基础');
      if (jsSuggestion) {
  ...
  describe('extractSemanticRelations - 语义关系提取', () => {
    ...
    it('should extract semantic relations using LLM', async () => {
      ...
      // Mock LLM响应
      mockLLMManager.chat.mockResolvedValue(JSON.stringify([
    ...
    it('should limit note titles to 50 in prompt', async () => {
      // 添加超过50个笔记
      for (let i = 0; i < 60; i++) {
        mockDb.notesByTitle.set(`Note${i}`, { id: `note-${i}`, title: `Note${i}` });
      }

      mockLLMManager.chat.mockResolvedValue('[]');

      await extractor.extractSemanticRelations('note-001', 'Test', mockLLMManager);
