...
describe('BuiltinSkills', () => {
  ...
  describe('技能字段验证', () => {
    it('each skill should have required fields', () => {
      builtinSkills.forEach((skill, index) => {
        expect(skill, `Skill at index ${index} should have id`).toHaveProperty('id');
        expect(skill, `Skill at index ${index} should have name`).toHaveProperty('name');
        expect(skill, `Skill at index ${index} should have display_name`).toHaveProperty('display_name');
        expect(skill, `Skill at index ${index} should have description`).toHaveProperty('description');
        expect(skill, `Skill at index ${index} should have category`).toHaveProperty('category');
      });
    });
  ...
  describe('工具关联', () => {
    ...
    it('at least some skills should have tools', () => {
      const skillsWithTools = builtinSkills.filter(s => s.tools && s.tools.length > 0);
  ...
  describe('唯一性验证', () => {
    it('should check for duplicate ids', () => {
      ...
      if (hasDuplicates) {
        console.log(`Warning: Found ${ids.length - uniqueIds.size} duplicate skill IDs`);
      }
    ...
    it('should check for duplicate names', () => {
      ...
      if (hasDuplicates) {
        console.log(`Warning: Found ${names.length - uniqueNames.size} duplicate skill names`);
      }
