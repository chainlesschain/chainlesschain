...
describe('P2PEnhancedManager - Voice/Video Integration', () => {
  ...
  describe('语音/视频通话集成', () => {
    ...
    test('应该能够接受通话', async () => {
      ...
      expect(enhancedManager.voiceVideoManager.acceptCall).toHaveBeenCalledWith(callId);
    });
    ...
    test('应该能够结束通话', async () => {
      ...
      expect(enhancedManager.voiceVideoManager.endCall).toHaveBeenCalledWith(callId);
    });
    ...
    test('应该能够切换静音', () => {
      ...
      expect(enhancedManager.voiceVideoManager.toggleMute).toHaveBeenCalledWith(callId);
    });
    ...
    test('应该能够切换视频', () => {
      ...
      expect(enhancedManager.voiceVideoManager.toggleVideo).toHaveBeenCalledWith(callId);
    });
    ...
    test('应该能够获取通话信息', () => {
      ...
      expect(enhancedManager.voiceVideoManager.getCallInfo).toHaveBeenCalledWith(callId);
    });
    ...
    test('应该能够获取活动通话列表', () => {
      ...
      enhancedManager.voiceVideoManager.getActiveCalls.mockReturnValue(mockCalls);
  ...
  describe('统计信息', () => {
    test('应该包含语音/视频统计信息', () => {
      ...
      enhancedManager.voiceVideoManager.getStats.mockReturnValue(mockVoiceVideoStats);
  ...
  describe('错误处理', () => {
    ...
    test('应该在未初始化时拒绝接受通话', async () => {
      ...
      await expect(
        uninitializedManager.acceptCall('call-123')
      ).rejects.toThrow('增强管理器未初始化');
    });
