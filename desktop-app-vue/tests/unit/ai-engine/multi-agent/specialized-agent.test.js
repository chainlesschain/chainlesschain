...
describe("SpecializedAgent", () => {
  ...
  describe("canHandle", () => {
    ...
    it("should return 0.5 for partial capability match (task contains capability)", () => {
      const task = {
        type: "code-generation-advanced",
        input: "advanced generation",
      };
      const score = agent.canHandle(task);
  ...
  describe("execute", () => {
    it("should throw error if not implemented by subclass", async () => {
      ...
      await expect(agent.execute({ type: "test" })).rejects.toThrow(
        "子类必须实现 execute 方法",
      );
    });
  ...
  describe("receiveMessage", () => {
    ...
    // (logger.info assertion 删除 — source require() bypasses vi.mock;
    // message receive behavior covered by adjacent assertion tests)
  ...
  describe("callTool", () => {
    ...
    it("should call tool with params", async () => {
      ...
      const result = await agent.callTool("readFile", {
        path: "/test/file.txt",
      });
    ...
    it("should throw error if FunctionCaller not set", async () => {
      await expect(
        agent.callTool("readFile", { path: "/test" }),
      ).rejects.toThrow("FunctionCaller 未设置");
    });
  });
