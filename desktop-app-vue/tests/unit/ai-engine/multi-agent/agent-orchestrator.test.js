...
describe("AgentOrchestrator", () => {
  ...
  beforeEach(async () => {
    ...
    // Import module
    const module =
      await import("../../../../src/main/ai-engine/multi-agent/agent-orchestrator.js");
  ...
  describe("Agent Registration", () => {
    describe("registerAgent", () => {
      ...
      it("should throw error if agent has no agentId", () => {
        ...
        expect(() => orchestrator.registerAgent(invalidAgent)).toThrow(
          "Agent must have an agentId",
        );
      });
  ...
  describe("Task Dispatching", () => {
    ...
    describe("dispatch", () => {
      ...
      it("should record execution history", async () => {
        ...
        expect(orchestrator.executionHistory[0].task.type).toBe(
          "code-generation",
        );
      ...
      it("should handle timeout", async () => {
        ...
        mockAgent1.execute.mockImplementation((receivedTask) => {
          receivedSignal = receivedTask.signal;
        ...
        await expect(shortTimeout.dispatch(task)).rejects.toThrow("执行超时");
        expect(receivedSignal.aborted).toBe(true);
        expect(physicallySettled).toBe(true);
        expect(shortTimeout.activeExecutions.size).toBe(0);
      });
  ...
  describe("Parallel Execution", () => {
    ...
    describe("executeParallel", () => {
      ...
      it("serializes shared writes and permits disjoint write scopes", async () => {
        let activeCount = 0;
        let maxActive = 0;
        mockAgent1.execute.mockImplementation(async () => {
          activeCount++;
        ...
        await orchestrator.executeParallel(tasks, { maxConcurrency: 3 });
      ...
      it("should stop on error if stopOnError is true", async () => {
        ...
        await expect(
          orchestrator.executeParallel(tasks, { stopOnError: true }),
        ).rejects.toThrow("Task failed");
      });

      it("does not dispatch new tasks after stopOnError", async () => {
        const tasks = Array.from({ length: 4 }, (_, index) => ({
          type: "code-generation",
          input: `task-${index}`,
        }));
        mockAgent1.execute.mockRejectedValueOnce(new Error("first failed"));
    ...
    describe("executeChain", () => {
      ...
      it("should pass previous result to next task", async () => {
        ...
        expect(secondCallArgs.context.previousResult).toEqual({
          data: "result1",
        });
      });
      ...
      it("should stop on error", async () => {
        ...
        await expect(orchestrator.executeChain(tasks)).rejects.toThrow(
          "Step 1 failed",
        );
  ...
  describe("Agent Communication", () => {
    ...
    describe("sendMessage", () => {
      it("should send message to target agent", async () => {
        ...
        expect(mockAgent2.receiveMessage).toHaveBeenCalledWith(message, {
          from: "agent1",
        });
      });
      ...
      it("should throw error if target agent does not exist", async () => {
        await expect(
          orchestrator.sendMessage("agent1", "nonexistent", {}),
      ...
      it("should fail closed when the message queue is full", async () => {
        ...
        await expect(
          orchestrator.sendMessage("agent1", "agent2", {}),
        ).rejects.toMatchObject({ code: "AGENT_MESSAGE_QUEUE_FULL" });
        expect(orchestrator.messageQueue).toHaveLength(1200);
      });
    ...
    describe("broadcast", () => {
      it("should send message to all agents except sender", async () => {
        ...
        expect(mockAgent3.receiveMessage).toHaveBeenCalledWith(message, {
          from: "agent1",
        });
      });
  ...
  describe("Statistics and Debugging", () => {
    ...
    describe("getStats", () => {
      ...
      it("should calculate success rate correctly", async () => {
        ...
        mockAgent1.execute.mockRejectedValue(new Error("Failed"));
        await expect(
          orchestrator.dispatch({ type: "code-generation", input: "test" }),
        ).rejects.toThrow();
    ...
    describe("exportDebugInfo", () => {
      ...
      it("should include agent capabilities", () => {
        ...
        expect(debugInfo.agents[0].capabilities).toEqual([
          "code-generation",
          "code-review",
        ]);
