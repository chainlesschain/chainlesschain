...
describe("pr-reviewer isSafeRef (command injection guard)", () => {
  ...
  it("does NOT execute a shell command when the target is unsafe", async () => {
    ...
    const context = {
      cwd: process.cwd(),
      ...createTestProcessContext("pr-reviewer", spy),
    };
    const review = await handler.execute({ input: "review 1;rm" }, context);
    expect(review.success).toBe(false);
    const summary = await handler.execute(
      { input: "summary main;evil" },
      context,
    );
  ...
  it("does execute for a safe target", async () => {
    ...
    const res = await handler.execute({ input: "summary main" }, context);
