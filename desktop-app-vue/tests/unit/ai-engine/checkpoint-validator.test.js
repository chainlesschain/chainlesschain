...
describe("CheckpointValidator", () => {
  ...
  beforeEach(async () => {
    ...
    // Dynamic import
    const module =
      await import("../../../src/main/ai-engine/checkpoint-validator.js");
  ...
  describe("构造函数", () => {
    ...
    it("应该初始化预期输出配置", () => {
      expect(validator.expectedOutputs["html_generator"]).toEqual([
        "html",
        "title",
      ]);
      expect(validator.expectedOutputs["css_generator"]).toEqual(["css"]);
      expect(validator.expectedOutputs["file_writer"]).toEqual([
        "path",
        "success",
      ]);
    });
  ...
  describe("checkNextStepDependencies", () => {
    ...
    it("应该通过提供了所有依赖的结果", () => {
      const plan = {
        subtasks: [
          ...
          {
            tool: "file_writer",
            params: { content: "{{html}}", path: "/output.html" },
          },
        ],
      };
    ...
    it("result 为 null 时不应抛错，应标记依赖缺失", () => {
      // Regression: `input in result` threw a TypeError when result was null
      // (a null step result is otherwise handled by the other validators).
      const plan = {
        subtasks: [
          { tool: "html_generator" },
          {
      ...
      expect(() =>
        validator.checkNextStepDependencies(0, null, plan),
      ).not.toThrow();
      const check = validator.checkNextStepDependencies(0, null, plan);
      expect(check.passed).toBe(false);
      expect(check.missingDependencies.length).toBeGreaterThan(0);
    });
  });
  ...
  describe("saveValidationHistory", () => {
    ...
    it("应该处理数据库保存失败", async () => {
      ...
      // 不应该抛出错误
      await expect(
        validator.saveValidationHistory(summary),
    ...
    it("应该处理没有数据库的情况", async () => {
      ...
      await expect(
        validatorNoDb.saveValidationHistory(summary),
      ).resolves.toBeUndefined();
    });
  });
  ...
  describe("validateCheckpoint - 集成测试", () => {
    ...
    it("应该跳过LLM检查当skipLLMCheck=true", async () => {
      ...
      expect(summary.validations.every((v) => v.type !== "llm_quality")).toBe(
        true,
      );
    });
    ...
    it("应该统计失败的校验数量", async () => {
      const plan = {
        subtasks: [
          {
            tool: "html_generator",
            expected_outputs: ["html", "title", "meta"],
          },
        ],
      };
