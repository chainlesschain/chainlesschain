...
describe("DataCollector", () => {
  ...
  beforeEach(async () => {
    ...
    // Dynamic import
    const module =
      await import("../../../src/main/ai-engine/data-collector.js");
  ...
  describe("updateUserProfile", () => {
    ...
    it("应该写入合法的 0（成功率/时长），而非保留旧值", async () => {
      // Regression: `updates.successRate || existing.success_rate` treated a
      // valid 0 as missing and kept the stale value.
      mockPrepareStmt.get.mockReturnValue({
        user_id: "user123",
        success_rate: 0.8,
        avg_task_duration: 5000,
      ...
      // run(taskIncrement, successRate, avgTaskDuration, mostUsedTools, userId)
      const args = mockPrepareStmt.run.mock.calls.at(-1);
      expect(args[1]).toBe(0); // successRate written as 0, not 0.8
      expect(args[2]).toBe(0); // avgTaskDuration written as 0, not 5000
    });

    it("应该为新用户创建画像", async () => {
      mockPrepareStmt.get.mockReturnValue(null);
      const createProfileSpy = vi
        .spyOn(collector, "createUserProfile")
        .mockResolvedValue();
