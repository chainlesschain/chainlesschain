...
const createMockFunctionCaller = () => ({
  ...
  callFunction: vi
    .fn()
    .mockResolvedValue({ success: true, result: "mock result" }),
  isInitialized: true,
});
...
describe("ToolManager", () => {
  ...
  describe("unregisterTool()", () => {
    ...
    it("should throw error if tool does not exist", async () => {
      ...
      await expect(toolManager.unregisterTool("nonexistent")).rejects.toThrow(
        "工具不存在",
      );
    });
    ...
    it("should unregister from functionCaller", async () => {
      ...
      expect(mockFunctionCaller.unregisterTool).toHaveBeenCalledWith(
        "delete_me",
      );
    });
  });
  ...
  describe("getAllTools()", () => {
    ...
    it("should support limit and offset", async () => {
      mockDb.all.mockResolvedValueOnce([{ id: "tool-2", name: "tool2" }]);
  ...
  describe("getToolsByCategory()", () => {
    it("should get tools by category", async () => {
      mockDb.all.mockResolvedValueOnce([{ id: "tool-1", category: "testing" }]);
  ...
  describe("getToolDoc()", () => {
    beforeEach(() => {
      ...
      mockDocGenerator.readToolDoc = vi
        .fn()
        .mockResolvedValue("# Tool Documentation");
    });
    ...
    it("should throw error if tool does not exist", async () => {
      ...
      await expect(toolManager.getToolDoc("nonexistent")).rejects.toThrow(
        "工具不存在",
      );
    });
  });
  ...
  describe("regenerateDoc()", () => {
    ...
    it("should throw error if tool does not exist", async () => {
      ...
      await expect(toolManager.regenerateDoc("nonexistent")).rejects.toThrow(
        "工具不存在",
      );
    });
  });
