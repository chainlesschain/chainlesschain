...
describe("APWebFinger", () => {
  ...
  describe("resolve()", () => {
    it("should throw on invalid address (no @)", async () => {
      webfinger = new APWebFinger(mockApBridge);
      await expect(webfinger.resolve("invalid")).rejects.toThrow(
        "Invalid WebFinger address format",
      );
    });

    it("should throw on null address", async () => {
      webfinger = new APWebFinger(mockApBridge);
      await expect(webfinger.resolve(null)).rejects.toThrow(
        "Invalid WebFinger address format",
      );
    });
    ...
    it("should throw on empty address", async () => {
      webfinger = new APWebFinger(mockApBridge);
      await expect(webfinger.resolve("")).rejects.toThrow(
        "Invalid WebFinger address format",
      );
    });
    ...
    it("should return cached result within TTL", async () => {
      ...
      webfinger._cache.set("alice@test.com", {
        data: cachedData,
        timestamp: Date.now(),
      });
    ...
    it("should not use expired cache", async () => {
      ...
      webfinger._cache.set("old@test.com", {
        data: cachedData,
  ...
  describe("buildLocalResponse()", () => {
    it("should return proper JRD response", () => {
      ...
      const profileLink = result.links.find(
  ...
  describe("extractActorUrl()", () => {
    ...
    it("should return null for response without links property", () => {
      webfinger = new APWebFinger(mockApBridge);
      expect(
        webfinger.extractActorUrl({ subject: "acct:test@example.com" }),
      ).toBeNull();
    });
