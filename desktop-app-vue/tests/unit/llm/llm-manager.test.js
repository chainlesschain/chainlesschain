...
const mockOllamaClient = {
  ...
  chat: vi.fn(async () => ({ message: { role: 'assistant', content: 'Ollama chat response' } })),
...
const mockOpenAIClient = {
  checkStatus: vi.fn(async () => ({ available: true, models: ['gpt-3.5-turbo'] })),
  chat: vi.fn(async () => ({ message: { role: 'assistant', content: 'OpenAI response' } })),
...
const mockDeepSeekClient = {
  checkStatus: vi.fn(async () => ({ available: true, models: ['deepseek-chat'] })),
  chat: vi.fn(async () => ({ message: { role: 'assistant', content: 'DeepSeek response' } })),
  embeddings: vi.fn(async () => ({ embedding: Array(1536).fill(0.1) }))
};
...
const mockAnthropicClient = {
  checkStatus: vi.fn(async () => ({ available: true, models: ['claude-3-opus-20240229'] })),
  chat: vi.fn(async () => ({ message: { role: 'assistant', content: 'Claude response' } })),
  chatStream: vi.fn(async () => {}),
...
const mockVolcengineToolsClient = {
  chat: vi.fn(async () => ({ message: { role: 'assistant', content: 'Volcengine tools response' } }))
};
...
describe('LLMManager', () => {
  ...
  describe('构造函数', () => {
    ...
    it('应该启用tokenTracker（如果提供）', () => {
      ...
      expect(mockTokenTracker.on).toHaveBeenCalledWith('budget-alert', expect.any(Function));
    });
  ...
  describe.skip('initialize', () => {
    // TODO: Skipped - Client mock doesn't intercept CommonJS require()
    ...
    it('应该发出initialized事件', async () => {
      ...
      expect(handler).toHaveBeenCalledWith({ available: true, models: ['llama2'] });
    });

    it('应该在checkStatus失败时发出unavailable事件', async () => {
      mockOllamaClient.checkStatus.mockResolvedValueOnce({ available: false, error: 'Service down' });
      llmManager = new LLMManager({ provider: 'ollama' });
      const handler = vi.fn();
    ...
    it('应该在checkStatus抛出错误时仍标记为初始化', async () => {
      mockOllamaClient.checkStatus.mockRejectedValueOnce(new Error('Network error'));
      llmManager = new LLMManager({ provider: 'ollama' });
  ...
  describe('createClient', () => {
    ...
    it('应该为不支持的provider抛出错误', async () => {
      await expect(llmManager.createClient('unsupported')).rejects.toThrow('不支持的提供商');
    });
  });

  describe.skip('switchProvider', () => {
    // TODO: Skipped - Depends on initialize() which has CommonJS mock issues
    ...
    it('应该切换到新provider', async () => {
      const result = await llmManager.switchProvider('openai', { apiKey: 'test-key' });
    ...
    it('应该合并新配置', async () => {
      await llmManager.switchProvider('openai', { apiKey: 'new-key', model: 'gpt-4' });
    ...
    it('应该在切换失败时抛出错误', async () => {
      await expect(llmManager.switchProvider('invalid-provider')).rejects.toThrow();
    });
  });

  describe.skip('checkStatus', () => {
    // TODO: Skipped - Depends on client.checkStatus() which causes real HTTP requests
    ...
    it('应该在checkStatus失败时返回错误', async () => {
      llmManager = new LLMManager({ provider: 'ollama' });
      await llmManager.initialize();
      mockOllamaClient.checkStatus.mockRejectedValueOnce(new Error('Connection error'));
  ...
  describe.skip('query', () => {
    // TODO: Skipped - Depends on client methods which cause real HTTP requests
  ...
  describe.skip('embeddings', () => {
    // TODO: Skipped - Depends on client.embeddings() which causes real HTTP requests
    ...
    it('应该在未初始化时抛出错误', async () => {
      ...
      await expect(llmManager.embeddings('test')).rejects.toThrow('LLM服务未初始化');
    });
    ...
    it('应该在客户端不支持embeddings时抛出错误', async () => {
      ...
      await expect(llmManager.embeddings('test')).rejects.toThrow('当前LLM不支持嵌入向量生成');
    });
  });

  describe.skip('listModels', () => {
    // TODO: Skipped - Depends on client.listModels() which causes real HTTP requests
  ...
  describe.skip('Manus优化', () => {
    ...
    it('应该调用setToolMask', () => {
      ...
      expect(mockManusOptimizations.setToolMask).toHaveBeenCalledWith(['tool1', 'tool2']);
    });

    it('应该调用setToolsByPrefix', () => {
      llmManager.setToolsByPrefix('file:', true);

      expect(mockManusOptimizations.setToolsByPrefix).toHaveBeenCalledWith('file:', true);
    });

    it('应该调用validateToolCall', () => {
      const result = llmManager.validateToolCall('tool1');

      expect(mockManusOptimizations.validateToolCall).toHaveBeenCalledWith('tool1');
    ...
    it('应该调用transitionToPhase', () => {
      ...
      expect(mockManusOptimizations.transitionToPhase).toHaveBeenCalledWith('execution');
      expect(result).toBe(true);
    });
