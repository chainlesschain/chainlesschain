...
vi.mock('fluent-ffmpeg', () => {
  const createMockCommand = () => {
    ...
    cmd.run.mockImplementation(() => {
      setTimeout(() => {
        const endHandler = cmd.on.mock.calls.find(call => call[0] === 'end')?.[1];
...
describe('视频引擎测试', () => {
  ...
  beforeEach(async () => {
    ...
    // Import VideoEngine after mocks
  ...
  describe('基础功能', () => {
    ...
    it('should have supported subtitle formats', () => {
      expect(videoEngine.supportedSubtitleFormats).toEqual(['srt', 'ass', 'vtt']);
    });
  ...
  describe('compressVideo', () => {
    it('should compress video with preset', async () => {
      ...
      expect(cmd.outputOptions).toHaveBeenCalledWith(['-crf 23', '-preset medium']);
  ...
  describe('generateSubtitlesWithAI', () => {
    ...
    it('should call progress callback at different stages', async () => {
      ...
      expect(onProgress).toHaveBeenCalledWith(
        expect.objectContaining({ percent: 10, message: expect.stringContaining('提取音频') })
      );
      expect(onProgress).toHaveBeenCalledWith(
  ...
  describe('handleProjectTask', () => {
    it('should route to correct handler', async () => {
      const tasks = [
        { taskType: 'convert', inputPath: '/in.avi', outputPath: '/out.mp4', options: {} },
        { taskType: 'trim', inputPath: '/in.mp4', outputPath: '/out.mp4', options: { startTime: '00:00:00', duration: '00:00:10' } },
        { taskType: 'extractAudio', inputPath: '/in.mp4', outputPath: '/audio.mp3', options: {} },
        { taskType: 'generateThumbnail', inputPath: '/in.mp4', outputPath: '/thumb.jpg', options: {} },
        { taskType: 'compress', inputPath: '/in.mp4', outputPath: '/out.mp4', options: { preset: '720p' } },
      ];
