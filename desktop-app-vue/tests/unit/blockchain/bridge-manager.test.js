...
describe("BridgeManager - loadBridgeContracts", () => {
  ...
  describe("从数据库加载桥接合约", () => {
    it("应该加载所有类型为bridge的合约", async () => {
      ...
      expect(bridgeManager.bridgeContracts.get(1)).toBe(
        "0x1234567890abcdef1234567890abcdef12345678",
      );
      expect(bridgeManager.bridgeContracts.get(137)).toBe(
    ...
    it("应该加载名称包含bridge的合约", async () => {
      ...
      expect(bridgeManager.bridgeContracts.get(56)).toBe(
  ...
  describe("多链支持", () => {
    it("应该支持同时加载多条链的桥接合约", async () => {
      ...
      expect(bridgeManager.bridgeContracts.get(42161)).toBe(
        "0xARBITRUM_BRIDGE",
      );
    });
  });
  ...
  describe("集成场景", () => {
    // BridgeManager.initialize() delegates table setup to its securityManager /
    // relayer collaborators, which reach for `database.db.exec(...)` and the
    // adapter's getSupportedChains(). Stub those collaborators here so the test
    // exercises BridgeManager's own orchestration (table init via database.run,
    // contract loading via database.all, idempotent re-init) in isolation.
    beforeEach(() => {
      bridgeManager.securityManager.initialize = vitest
        .fn()
        .mockResolvedValue();
      bridgeManager.relayer.initialize = vitest.fn().mockResolvedValue();
    });
