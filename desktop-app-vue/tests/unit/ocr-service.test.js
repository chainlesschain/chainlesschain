...
// Hoist mock worker so it can be accessed from the mock
const mockWorker = {
...
// Mock tesseract.js BEFORE any imports - must be at module level
vi.mock("tesseract.js", () => ({
  createWorker: vi.fn(() => {
    // Don't call logger callback - it causes DataCloneError in worker
    // The callback would be called by the real worker, not our mock
...
describe("OCRService - OCR文字识别服务", () => {
  ...
  describe("Configuration and Initialization - 配置和初始化", () => {
    ...
    it("should handle initialization errors", async () => {
      mockWorker.loadLanguage.mockRejectedValue(
        new Error("Language load failed"),
      );
  ...
  describe("Image Recognition - 图片识别", () => {
    ...
    it("should handle recognition errors", async () => {
      ...
      await expect(ocrService.recognize("/path/to/image.png")).rejects.toThrow(
        "Recognition failed",
      );
      expect(errorSpy).toHaveBeenCalled();
    });
