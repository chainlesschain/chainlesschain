...
describe('TextSplitter', () => {
  ...
  describe.skip('_splitTextWithSeparator', () => {
    // TODO: Skipped to avoid OOM - private method tests
    beforeEach(() => {
      splitter = new RecursiveCharacterTextSplitter();
    });
  ...
  describe.skip('错误处理和边界情况', () => {
    ...
    it('应该处理超长文本', () => {
      ...
      expect(chunks.length).toBeGreaterThan(1);
      chunks.forEach(chunk => {
        expect(chunk.content.length).toBeLessThanOrEqual(splitter.config.chunkSize);
      });
    });
...
Content for section 2.`;
  ...
  describe('CodeTextSplitter.getSeparatorsForLanguage', () => {
    it('应该返回JavaScript分隔符', () => {
      const separators = CodeTextSplitter.getSeparatorsForLanguage('javascript');
...
def greet(name):
  ...
  describe.skip('性能和优化', () => {
    ...
    it('应该正确计算自定义长度函数', () => {
      ...
      const wordCountFn = (text) => text.split(/\s+/).filter(w => w.length > 0).length;
