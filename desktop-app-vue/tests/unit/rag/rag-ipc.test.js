...
describe("RAG IPC 处理器注册", () => {
  ...
  describe("rag:rebuild-index handler", () => {
    ...
    it("should throw error when ragManager is not initialized", async () => {
      ...
      await expect(
        mockIpcMainNoMgr.invoke("rag:rebuild-index"),
      ).rejects.toThrow("RAG服务未初始化");
    });
  });
  ...
  describe("rag:update-config handler", () => {
    ...
    it("should throw error when ragManager is not initialized", async () => {
      ...
      await expect(
        mockIpcMainNoMgr.invoke("rag:update-config", {}),
      ).rejects.toThrow("RAG服务未初始化");
    });
  });
  ...
  describe("rag:set-reranking-enabled handler", () => {
    ...
    it("should throw error when ragManager is not initialized", async () => {
      ...
      await expect(
        mockIpcMainNoMgr.invoke("rag:set-reranking-enabled", true),
      ).rejects.toThrow("RAG服务未初始化");
    });
  });
