...
describe("Reranker", () => {
  ...
  describe("rerankWithLLM", () => {
    ...
    it("应该按rerankScore降序排序", async () => {
      ...
      for (let i = 0; i < result.length - 1; i++) {
        expect(result[i].rerankScore).toBeGreaterThanOrEqual(
          result[i + 1].rerankScore,
        );
      }
    });
  });
  ...
  describe("rerankWithKeywordMatch", () => {
    ...
    it("应该按分数降序排序", () => {
      const result = reranker.rerankWithKeywordMatch(
        "test",
        sampleDocuments,
        3,
      );
    ...
    it("空查询不产生 NaN 分数（兜底为 0）", () => {
      // 空串/纯标点/纯空白都会 tokenize 成 []，旧实现 matchScore/(0*3)=NaN
      // 会污染每个文档的 score 并让排序行为未定义。
      for (const q of ["", "   ", "？！。"]) {
        const result = reranker.rerankWithKeywordMatch(q, sampleDocuments, 3);
        expect(result.length).toBe(3);
        result.forEach((doc) => {
          expect(Number.isFinite(doc.score)).toBe(true);
          expect(doc.score).toBe(0);
          expect(Number.isFinite(doc.rerankScore)).toBe(true);
        });
      }
    });
  });
