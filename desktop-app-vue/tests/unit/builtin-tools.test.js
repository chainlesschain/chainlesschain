...
describe('BuiltinTools', () => {
  ...
  describe('工具字段验证', () => {
    it('each tool should have required fields', () => {
      builtinTools.forEach((tool, index) => {
        ...
        expect(tool, `Tool at index ${index} should have display_name`).toHaveProperty('display_name');
        expect(tool, `Tool at index ${index} should have description`).toHaveProperty('description');
        expect(tool, `Tool at index ${index} should have category`).toHaveProperty('category');
        expect(tool, `Tool at index ${index} should have tool_type`).toHaveProperty('tool_type');
      });
    });
  ...
  describe('示例和文档', () => {
    ...
    it('at least some tools should have examples', () => {
      const toolsWithExamples = builtinTools.filter(t => t.examples && t.examples.length > 0);
  ...
  describe('权限配置', () => {
    ...
    it('at least some tools should have permissions', () => {
      const toolsWithPerms = builtinTools.filter(t => t.required_permissions && t.required_permissions.length > 0);
  ...
  describe('特定工具验证', () => {
    ...
    it('should include network tools', () => {
      const networkSpeedTester = builtinTools.find(t => t.id === 'tool_network_speed_tester');
      expect(networkSpeedTester).toBeDefined();
      expect(networkSpeedTester.category).toBe('network');
    });

    it('should include diagnostic tool', () => {
      const diagnostic = builtinTools.find(t => t.id === 'tool_network_diagnostic_tool');
  ...
  describe('唯一性验证', () => {
    it('should check for duplicate ids', () => {
      ...
      if (hasDuplicates) {
        ...
        console.log(`Warning: Found ${ids.length - uniqueIds.size} duplicate tool IDs:`, [...new Set(duplicates)]);
      }
    ...
    it('should check for duplicate names', () => {
      ...
      if (hasDuplicates) {
        ...
        console.log(`Warning: Found ${names.length - uniqueNames.size} duplicate tool names:`, [...new Set(duplicates)]);
      }
    ...
    it('tools with high risk should have permissions', () => {
      ...
      highRiskTools.forEach((tool) => {
        ...
        if (!tool.required_permissions || tool.required_permissions.length === 0) {
          console.log(`Warning: High-risk tool ${tool.id} (risk=${tool.risk_level}) has no permissions`);
        }
      });
    ...
    it('should have reasonable parameter schemas', () => {
      builtinTools.forEach((tool) => {
        if (tool.parameters_schema && tool.parameters_schema.properties) {
          const paramCount = Object.keys(tool.parameters_schema.properties).length;

          // Most tools should have between 0 and 20 parameters
          if (paramCount > 20) {
            console.log(`Warning: Tool ${tool.id} has ${paramCount} parameters (unusually high)`);
          }
        }
      });
    ...
    it('should have consistent naming conventions', () => {
      builtinTools.forEach((tool) => {
        ...
        if (tool.id !== expectedId && !tool.id.startsWith('tool_')) {
          console.log(`Warning: Tool ${tool.id} doesn't follow naming convention (name=${tool.name})`);
        }
      });
