...
describe('Database 模块', () => {
  ...
  describe('事务处理', () => {
    it('应该支持事务开始', async () => {
      ...
      expect(result.success).toBe(true);
      expect(mockElectronAPI.db.execute).toHaveBeenCalledWith('BEGIN TRANSACTION');
    });
  ...
  describe('错误处理', () => {
    ...
    it('应该处理约束违规错误', async () => {
      ...
      await expect(
        mockElectronAPI.db.run('INSERT INTO notes (id) VALUES (?)', ['duplicate-id'])
    ...
    it('应该处理外键约束错误', async () => {
      ...
      await expect(
        mockElectronAPI.db.run('INSERT INTO project_files (project_id) VALUES (?)', ['non-existent'])
