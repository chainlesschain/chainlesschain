...
describe("SqlSecurity - SQL注入防御", () => {
  describe("validateOrder", () => {
    ...
    test("应该拒绝无效的排序方向", () => {
      expect(() => SqlSecurity.validateOrder("INVALID")).toThrow(
        "非法的排序方向",
      );
      expect(() =>
        SqlSecurity.validateOrder("ASC; DROP TABLE users"),
      ).toThrow();
  ...
  describe("validateTableName", () => {
    ...
    test("应该允许有效的表名", () => {
      expect(SqlSecurity.validateTableName("users", allowedTables)).toBe(
        "users",
      );
      expect(SqlSecurity.validateTableName("projects", allowedTables)).toBe(
        "projects",
      );
      expect(SqlSecurity.validateTableName("notes", allowedTables)).toBe(
        "notes",
      );
    });
    ...
    test("应该允许包含下划线的表名", () => {
      ...
      expect(SqlSecurity.validateTableName("project_files", tables)).toBe(
        "project_files",
      );
    });
    ...
    test("应该拒绝不在白名单的表名", () => {
      expect(() =>
        SqlSecurity.validateTableName("malicious", allowedTables),
      ).toThrow("不允许访问的表");
      expect(() =>
        SqlSecurity.validateTableName("admin_table", allowedTables),
      ).toThrow();
    });

    test("应该拒绝包含非法字符的表名", () => {
      expect(() =>
        SqlSecurity.validateTableName("users; DROP TABLE users", allowedTables),
      ).toThrow("非法的表名");
      expect(() =>
        SqlSecurity.validateTableName("users-2", allowedTables),
      ).toThrow("非法的表名");
      expect(() =>
        SqlSecurity.validateTableName("users.admin", allowedTables),
      ).toThrow("非法的表名");
      expect(() =>
        SqlSecurity.validateTableName("users OR 1=1", allowedTables),
      ).toThrow("非法的表名");
    });

    test("应该拒绝空值和无效类型", () => {
      expect(() => SqlSecurity.validateTableName("", allowedTables)).toThrow(); // 空字符串会被正则拒绝
      expect(() => SqlSecurity.validateTableName(null, allowedTables)).toThrow(
        "表名无效",
      );
      expect(() =>
        SqlSecurity.validateTableName(undefined, allowedTables),
      ).toThrow("表名无效");
      expect(() => SqlSecurity.validateTableName(123, allowedTables)).toThrow(
        "表名无效",
      );
    });
  });
  ...
  describe("validateColumnName", () => {
    ...
    test("应该验证列名白名单", () => {
      ...
      expect(() => SqlSecurity.validateColumnName("password", allowed)).toThrow(
        "不允许的列",
      );
    });
    ...
    test("应该拒绝非法列名", () => {
      expect(() =>
        SqlSecurity.validateColumnName("name; DROP TABLE users"),
      ).toThrow("非法的列名");
      expect(() => SqlSecurity.validateColumnName("name-2")).toThrow(
        "非法的列名",
      );
      expect(() => SqlSecurity.validateColumnName("name.field")).toThrow(
        "非法的列名",
      );
    });
  ...
  describe("validateLimit", () => {
    ...
    test("应该拒绝无效的LIMIT值", () => {
      ...
      expect(() => SqlSecurity.validateLimit("abc")).toThrow(
        "LIMIT必须是非负整数",
      );
      expect(() => SqlSecurity.validateLimit(null)).toThrow(
        "LIMIT必须是非负整数",
      );
    });
  });
  ...
  describe("validateOffset", () => {
    ...
    test("应该拒绝无效的OFFSET值", () => {
      expect(() => SqlSecurity.validateOffset(-1)).toThrow(
        "OFFSET必须是非负整数",
      );
      expect(() => SqlSecurity.validateOffset("abc")).toThrow(
        "OFFSET必须是非负整数",
      );
    });
  });
  ...
  describe("containsSqlInjectionPattern", () => {
    test("应该检测经典SQL注入模式", () => {
      expect(
        SqlSecurity.containsSqlInjectionPattern("'; DROP TABLE users; --"),
      ).toBe(true);
    ...
    test("应该检测UNION注入", () => {
      expect(
        SqlSecurity.containsSqlInjectionPattern(
    ...
    test("应该检测注释模式", () => {
      ...
      expect(
        SqlSecurity.containsSqlInjectionPattern("value /* comment */"),
      ).toBe(true);
    });

    test("应该检测存储过程调用", () => {
      expect(
        SqlSecurity.containsSqlInjectionPattern("'; EXEC sp_executesql"),
      ).toBe(true);
      expect(
        SqlSecurity.containsSqlInjectionPattern("'; EXECUTE xp_cmdshell"),
      ).toBe(true);
    });

    test("应该允许正常的输入", () => {
      expect(SqlSecurity.containsSqlInjectionPattern("John Doe")).toBe(false);
      expect(SqlSecurity.containsSqlInjectionPattern("user@example.com")).toBe(
        false,
      );
      expect(SqlSecurity.containsSqlInjectionPattern("Project #123")).toBe(
        false,
      );
    });
  ...
  describe("validateSearchKeyword", () => {
    test("应该允许正常的搜索词", () => {
      ...
      expect(SqlSecurity.validateSearchKeyword("hello world")).toBe(
        "hello world",
      );
      expect(SqlSecurity.validateSearchKeyword("user@example.com")).toBe(
        "user@example.com",
      );
    });

    test("应该拒绝包含SQL注入模式的关键词", () => {
      expect(() => SqlSecurity.validateSearchKeyword("' OR 1=1 --")).toThrow(
        "搜索关键词包含非法字符",
      );
      expect(() =>
        SqlSecurity.validateSearchKeyword("'; DROP TABLE users"),
      ).toThrow("搜索关键词包含非法字符");
    });
  ...
  describe("buildSafeWhereClause", () => {
    ...
    test("某个IN为空数组时仍保留其他条件", () => {
      const { whereClause, params } = SqlSecurity.buildSafeWhereClause(
        { status: [], name: "John" },
        allowedFields,
      );

      expect(whereClause).toBe("WHERE 1 = 0 AND name = ?");
      expect(params).toEqual(["John"]);
    });
  });
  ...
  describe("真实SQL注入攻击场景", () => {
    test("应该阻止经典的OR 1=1注入", () => {
      ...
      expect(SqlSecurity.containsSqlInjectionPattern(maliciousInput)).toBe(
        true,
      );
  ...
  describe("边界条件测试", () => {
    ...
    test("应该处理特殊Unicode字符", () => {
      ...
      expect(SqlSecurity.validateSearchKeyword(unicodeInput)).toBe(
        unicodeInput,
      );
    });
