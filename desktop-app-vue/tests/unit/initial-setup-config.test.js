/**
 * 单元测试：初始设置配置管理器
 * 测试文件：src/main/initial-setup-config.js
 */
...
describe('InitialSetupConfig - 配置管理器单元测试', () => {
  ...
  describe('save() 和 load()', () => {
    ...
    it('应正确加载配置文件', () => {
      ...
      expect(newInstance.get('enterprise.serverUrl')).toBe('https://enterprise.test.com');
...
describe('InitialSetupConfig - 配置应用到系统', () => {
  ...
  it('应正确应用个人版配置', async () => {
    ...
    await configInstance.applyToSystem(mockAppConfig, mockLlmConfig, mockDatabase);

    expect(mockAppConfig.set).toHaveBeenCalledWith('app.edition', 'personal');
    expect(mockAppConfig.setDatabasePath).toHaveBeenCalledWith('C:\\test\\db.sqlite');
    expect(mockDatabase.setSetting).toHaveBeenCalledWith('project.rootPath', 'C:\\test\\projects');
  });
  ...
  it('应正确应用企业版配置', async () => {
    ...
    expect(mockDatabase.setSetting).toHaveBeenCalledWith('enterprise.serverUrl', 'https://ent.test.com');
    expect(mockDatabase.setSetting).toHaveBeenCalledWith('enterprise.tenantId', 'tenant-123');
    expect(mockDatabase.setSetting).toHaveBeenCalledWith('enterprise.apiKey', 'ent-api-key');
  });
  ...
  it('应正确应用LLM配置', async () => {
    ...
    expect(mockLlmConfig.set).toHaveBeenCalledWith('volcengine.apiKey', 'volcengine-key-123');
    expect(mockLlmConfig.set).toHaveBeenCalledWith('volcengine.baseURL', 'https://ark.cn-beijing.volces.com/api/v3');
    expect(mockLlmConfig.set).toHaveBeenCalledWith('volcengine.model', 'doubao-pro-4k');
