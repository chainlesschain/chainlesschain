...
const createMockSkillManager = () => ({
  ...
  getSkillTools: vi.fn().mockResolvedValue([
    { id: 'tool-1', name: 'test_tool', role: 'primary' },
  ]),
...
const createMockSkillRecommender = () => ({
  recommendSkills: vi.fn().mockResolvedValue([
    { id: 'skill-1', score: 0.95 },
  ]),
  getPopularSkills: vi.fn().mockResolvedValue([
    { id: 'skill-1', usage_count: 100 },
  ]),
  getRelatedSkills: vi.fn().mockResolvedValue([
    { id: 'skill-2', similarity: 0.85 },
  ]),
  searchSkills: vi.fn().mockResolvedValue([
    { id: 'skill-1', relevance: 0.92 },
  ]),
});
...
describe('SkillToolIPC', () => {
  ...
  describe('Skill IPC Handlers', () => {
    ...
    it('should handle skill:update-config', async () => {
      ...
      const result = await mockIpcMain.invoke('skill:update-config', 'skill-1', config);

      expect(result.success).toBe(true);
      expect(mockSkillMgr.updateSkill).toHaveBeenCalledWith('skill-1', { config });
    });
  ...
  describe('Tool IPC Handlers', () => {
    ...
    it('should handle tool:update-schema', async () => {
      ...
      const result = await mockIpcMain.invoke('tool:update-schema', 'tool-1', schema);

      expect(result.success).toBe(true);
      expect(mockToolMgr.updateTool).toHaveBeenCalledWith('tool-1', { parameters_schema: schema });
    });
  ...
  describe('Recommendation IPC Handlers', () => {
    ...
    it('should handle skill:get-related', async () => {
      const result = await mockIpcMain.invoke('skill:get-related', 'skill-1', 5);

      expect(result.success).toBe(true);
      expect(result.data).toBeDefined();
      expect(global.skillRecommender.getRelatedSkills).toHaveBeenCalledWith('skill-1', 5);
    });
  ...
  describe('Config IPC Handlers', () => {
    ...
    it('should handle config:export-skills', async () => {
      const result = await mockIpcMain.invoke('config:export-skills', ['skill-1']);
    ...
    it('should handle config:export-tools', async () => {
      const result = await mockIpcMain.invoke('config:export-tools', ['tool-1']);
    ...
    it('should handle config:export-to-file', async () => {
      const data = { skills: [] };
      const result = await mockIpcMain.invoke('config:export-to-file', data, '/path/to/file.json', 'json');

      expect(result.success).toBe(true);
      expect(global.configManager.exportToFile).toHaveBeenCalledWith(data, '/path/to/file.json', 'json');
    });

    it('should handle config:import-from-file', async () => {
      const result = await mockIpcMain.invoke('config:import-from-file', '/path/to/file.json');
    ...
    it('should handle config:create-template', async () => {
      const result = await mockIpcMain.invoke('config:create-template', 'skill');
  ...
  describe('Error Handling', () => {
    ...
    it('should handle async errors in tool handlers', async () => {
      mockToolMgr.getAllTools.mockRejectedValueOnce(new Error('Database error'));
    ...
    it('should handle errors in analytics handlers', async () => {
      mockSkillMgr.getEnabledSkills.mockRejectedValueOnce(new Error('Query failed'));
