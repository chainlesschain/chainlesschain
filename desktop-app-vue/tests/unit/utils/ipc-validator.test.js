...
describe("IPC Validator", () => {
  describe("基础 Schema", () => {
    ...
    describe("safePathSchema", () => {
      ...
      it("应该拒绝小写盘符 / 正斜杠盘符 / UNC 路径（此前会绕过）", () => {
        // 小写盘符
        expect(() => safePathSchema.parse("c:\\Windows\\System32")).toThrow();
        // 正斜杠盘符
        expect(() => safePathSchema.parse("C:/Windows/System32")).toThrow();
        expect(() => safePathSchema.parse("d:/secret.txt")).toThrow();
        // Windows UNC（反斜杠）
        expect(() => safePathSchema.parse("\\\\attacker\\share\\x")).toThrow();
        // 单个反斜杠开头
        expect(() => safePathSchema.parse("\\etc\\passwd")).toThrow();
        // 正斜杠 UNC（由 startsWith('/') 覆盖）
        expect(() => safePathSchema.parse("//attacker/share/x")).toThrow();
      });
  ...
  describe("Git 相关 Schema", () => {
    describe("gitBranchSchema", () => {
      it("应该接受有效的分支名", () => {
        ...
        expect(() =>
          gitBranchSchema.parse("feature/new-feature"),
        ).not.toThrow();
