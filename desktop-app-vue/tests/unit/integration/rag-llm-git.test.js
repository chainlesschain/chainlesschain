...
describe("RAG检索模块", () => {
  ...
  describe("文档索引", () => {
    ...
    it("应该能够批量索引文档", async () => {
      ...
      const result = await mockElectronAPI.llm.query("batch-index", {
        documents,
      });
...
describe("LLM服务模块", () => {
  ...
  describe("基本查询", () => {
    ...
    it("应该支持流式响应", async () => {
      ...
      mockElectronAPI.llm.stream.mockImplementation(
  ...
  describe("错误处理", () => {
    it("应该处理网络错误", async () => {
      mockElectronAPI.llm.query.mockRejectedValue(new Error("Network error"));

      await expect(mockElectronAPI.llm.query("测试")).rejects.toThrow(
        "Network error",
      );
    });
