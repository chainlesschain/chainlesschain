...
describe("FileManager", () => {
  ...
  describe("uploadFile", () => {
    ...
    it("should check organization permission before upload", async () => {
      const newFile = { id: "file_new123", file_name: "test.txt" };
      mockPrepare.get.mockReturnValueOnce(null).mockReturnValueOnce(newFile);
      fs.existsSync.mockReturnValue(true);
    ...
    it("should throw error if no permission", async () => {
      ...
      await expect(fileManager.uploadFile(fileData, "user123")).rejects.toThrow(
        "无权限上传文件到此组织",
      );
    });
    ...
    it("should insert file record with correct data", async () => {
      ...
      const insertCall = mockPrepare.run.mock.calls.find(
        (call) => call.length > 5 && call[1] === "proj1",
      );
  ...
  describe("getFile", () => {
    ...
    it("should throw error if file not found", () => {
      ...
      expect(() => fileManager.getFile("nonexistent")).toThrow("文件不存在");
    });
  });
  ...
  describe("lockFile", () => {
    ...
    it("should set lock expiry time", async () => {
      ...
      const updateCall = mockPrepare.run.mock.calls.find(
        (call) => call.length > 2,
      );
  ...
  describe("Private Methods", () => {
    ...
    describe("_saveFileToDisk", () => {
      it("should return original path", async () => {
        const savedPath = await fileManager._saveFileToDisk(
          "/original/path.txt",
          "content",
        );
      ...
      it("should handle different paths", async () => {
        const path1 = await fileManager._saveFileToDisk(
          "/path/one.txt",
          "content1",
        );
        const path2 = await fileManager._saveFileToDisk(
          "/path/two.txt",
          "content2",
        );
    ...
    describe("_logFileAccess", () => {
      ...
      it("does not throw when the access-log insert fails (CHECK violation)", async () => {
        mockPrepare.run.mockImplementationOnce(() => {
          throw new Error("CHECK constraint failed: file_access_logs");
        });

        // Audit logging is best-effort: a failed insert must not abort the
        // caller. Previously uploadFile returned {success:false} on every
        // upload because 'upload' was not in the file_access_logs CHECK set.
        await expect(
          fileManager._logFileAccess("file123", "user123", "upload"),
