...
const mockBrowserEngine = {
  start: vi
    .fn()
    .mockResolvedValue({ success: true, cdpPort: 18800, pid: 12345 }),
...
/**
 * Re-apply mock implementations (needed because vitest config has mockReset: true)
 */
function setupMockImplementations() {
  mockBrowserEngine.start.mockResolvedValue({
    success: true,
    cdpPort: 18800,
    pid: 12345,
  });
  mockBrowserEngine.stop.mockResolvedValue({ success: true, uptime: 10000 });
  mockBrowserEngine.createContext.mockResolvedValue({
    success: true,
    profileName: "default",
    exists: false,
  });
  mockBrowserEngine.openTab.mockResolvedValue({
    success: true,
    targetId: "tab-1",
    url: "https://example.com",
    title: "Example",
  });
  mockBrowserEngine.closeTab.mockResolvedValue({
    success: true,
    targetId: "tab-1",
  });
  mockBrowserEngine.focusTab.mockResolvedValue({
    success: true,
    targetId: "tab-1",
  });
  mockBrowserEngine.listTabs.mockResolvedValue([
    {
      targetId: "tab-1",
      url: "https://example.com",
      title: "Example",
  ...
  mockBrowserEngine.navigate.mockResolvedValue({
    success: true,
    url: "https://example.com",
    title: "Example",
  });
  mockBrowserEngine.screenshot.mockResolvedValue(
    Buffer.from("fake-screenshot"),
  );
  mockBrowserEngine.getStatus.mockReturnValue({
    isRunning: true,
    uptime: 5000,
    cdpPort: 18800,
    contextsCount: 1,
  ...
  mockBrowserEngine.saveSession.mockResolvedValue({
    success: true,
    stateFile: "/path/to/state.json",
    cookiesCount: 5,
  });
  mockBrowserEngine.restoreSession.mockResolvedValue({
    success: true,
    profileName: "default",
    cookiesCount: 5,
  });
  mockBrowserEngine.cleanup.mockResolvedValue(undefined);
  mockCreateIPCErrorHandler.mockImplementation(() => (handler) => handler);
  mockGetBrowserEngine.mockImplementation(() => mockBrowserEngine);
}
...
describe("Browser IPC", () => {
  ...
  describe("browser:createContext", () => {
    ...
    it("应该成功创建上下文", async () => {
      ...
      const result = await handler({}, "test-profile", {
  ...
  describe("browser:screenshot", () => {
    ...
    it("应该成功截图", async () => {
      ...
      expect(mockBrowserEngine.screenshot).toHaveBeenCalledWith("tab-1", {
        type: "png",
      });
  ...
  describe("browser:saveSession", () => {
    ...
    it("应该成功保存会话", async () => {
      ...
      expect(mockBrowserEngine.saveSession).toHaveBeenCalledWith(
        "default",
        null,
      );
      expect(result.success).toBe(true);
  ...
  describe("browser:restoreSession", () => {
    ...
    it("应该成功恢复会话", async () => {
      ...
      expect(mockBrowserEngine.restoreSession).toHaveBeenCalledWith(
        "default",
        null,
      );
      expect(result.success).toBe(true);
