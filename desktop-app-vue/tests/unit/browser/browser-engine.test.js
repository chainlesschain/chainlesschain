 ...
 * BrowserEngine 单元测试
 *
 * Uses the `_setChromiumForTesting` seam exported by browser-engine.js
 * (see memory vi_mock_cjs_interop_systemic): vi.mock('playwright-core')
 * cannot intercept the CommonJS require() reliably across Vitest's
 * CJS/ESM bridge, so the source exposes a setter for tests to swap the
 * chromium binding directly.
 */

import { describe, it, expect, beforeEach, afterEach, vi } from "vitest";
import {
  BrowserEngine,
  _setChromiumForTesting,
} from "../../../src/main/browser/browser-engine.js";

// Project-module mocks (vi.mock works fine for ../../src/... paths)
vi.mock("../../../src/main/browser/snapshot-engine", () => {
  ...
  class MockSnapshotEngine extends EventEmitter {
    ...
    getStats() {
      return { count: this.snapshots.size };
    }
    clearSnapshot() {
...
// Fake chromium factory — fresh page per newPage() call so tests that open
// multiple tabs get distinct page instances (source mutates page._targetId).
// Each page's context() returns the parent fake context, which matters for
// BrowserEngine.listTabs() (identity-compares page.context() against entries
// in this.contexts to derive profileName).
function makeFakePage(parentContext) {
  return {
    ...
    on: vi.fn(),
    context: vi.fn().mockReturnValue(parentContext),
  };
}
function makeFakeContext() {
  ...
  ctx.newPage = vi
    .fn()
    .mockImplementation(() => Promise.resolve(makeFakePage(ctx)));
  return ctx;
}
function makeFakeBrowser() {
  return {
    newContext: vi
      .fn()
      .mockImplementation(() => Promise.resolve(makeFakeContext())),
    close: vi.fn().mockResolvedValue(undefined),
...
function makeFakeChromium() {
  return {
    launch: vi
      .fn()
      .mockImplementation(() => Promise.resolve(makeFakeBrowser())),
  };
}
...
describe("BrowserEngine", () => {
  ...
  beforeEach(() => {
    _setChromiumForTesting(makeFakeChromium());
  ...
  afterEach(async () => {
    ...
    _setChromiumForTesting(null); // restore real chromium binding
  });
  ...
  describe("stop()", () => {
    ...
    it("应该成功停止浏览器", async () => {
      ...
      expect(result.success).toBe(true);
      // uptime can legitimately be 0 if start→stop fits within one Date.now()
      // tick on Windows (timer resolution ~15ms); assert non-negative instead
    ...
    it("如果浏览器未运行，应该抛出错误", async () => {
      ...
      await expect(browserEngine.stop()).rejects.toThrow(
  ...
  describe("openTab()", () => {
    ...
    it("应该成功打开新标签页", async () => {
      const result = await browserEngine.openTab(
        "default",
        "https://example.com",
      );
    ...
    it("应该为每个标签页生成唯一 ID", async () => {
      const result1 = await browserEngine.openTab(
        "default",
        "https://example.com",
      );
      const result2 = await browserEngine.openTab(
        "default",
        "https://example.org",
      );
