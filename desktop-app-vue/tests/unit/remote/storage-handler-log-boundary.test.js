...
const HANDLER_PATH = resolve(
  process.cwd(),
  "src/main/remote/handlers/storage-handler.js",
);

describe("remote storage handler log boundary", () => {
  it("routes storage logs through the browser redactor", () => {
  ...
  it("keeps storage log messages static", () => {
    const source = readFileSync(HANDLER_PATH, "utf8");
    const logCalls = [
