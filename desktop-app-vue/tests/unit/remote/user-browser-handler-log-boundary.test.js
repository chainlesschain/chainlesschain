...
const HANDLER_PATH = resolve(
  process.cwd(),
  "src/main/remote/handlers/user-browser-handler.js",
);

describe("remote user browser handler log boundary", () => {
  it("routes user browser logs through the browser redactor", () => {
  ...
  it("keeps user browser log messages static", () => {
    const source = readFileSync(HANDLER_PATH, "utf8");
    const logCalls = [
