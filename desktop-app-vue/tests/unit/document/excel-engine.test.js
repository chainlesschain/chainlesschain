...
describe("ExcelEngine", () => {
  ...
  describe("readExcel", () => {
    ...
    it("should throw error for unsupported file format", async () => {
      ...
      await expect(excelEngine.readExcel(txtPath)).rejects.toThrow(
        "不支持的文件格式",
      );
    });
  ...
  describe("writeCSV", () => {
    ...
    it("should handle missing rows data", async () => {
      ...
      await expect(
        excelEngine.writeCSV(outputPath, invalidData),
  ...
  describe("jsonToExcel", () => {
    ...
    it("should handle empty array", async () => {
      ...
      await expect(excelEngine.jsonToExcel([], outputPath)).rejects.toThrow(
        "JSON数据必须是非空数组",
      );
    });
  ...
  describe("getSheetStats", () => {
    ...
    it("should detect formulas correctly", () => {
      const sheetWithFormula = {
        name: "Sheet",
        rows: [{ values: [{ value: 1, formula: "=SUM(A1:A10)" }] }],
        columns: [],
      };
      ...
      expect(excelEngine.getSheetStats(sheetWithFormula).hasFormulas).toBe(
        true,
      );
      expect(excelEngine.getSheetStats(sheetWithoutFormula).hasFormulas).toBe(
        false,
      );
    });
  });
