...
describe("U-Key IPC 处理器", () => {
  ...
  beforeEach(async () => {
    ...
    mockUnifiedKeyManager = {
      deriveKey: vi
        .fn()
        .mockResolvedValue({ keyId: "k-1", publicKey: "pub-1" }),
  ...
  describe("设备检测功能 (ukey:detect)", () => {
    ...
    it("should handle detect errors gracefully", async () => {
      mockUkeyManager.detect.mockRejectedValueOnce(
  ...
  describe("总体验证", () => {
    ...
    it("should have all required handler channels", () => {
      ...
      // Phase 46: Threshold + Biometric
      expect(handlers["threshold-security:setup-keys"]).toBeDefined();
      expect(handlers["threshold-security:sign"]).toBeDefined();
      expect(handlers["threshold-security:bind-biometric"]).toBeDefined();
      expect(handlers["threshold-security:verify-biometric"]).toBeDefined();
      // Phase 47: BLE
      expect(handlers["ble-ukey:scan-devices"]).toBeDefined();
      expect(handlers["ble-ukey:pair-device"]).toBeDefined();
      expect(handlers["ble-ukey:connect"]).toBeDefined();
      expect(handlers["ble-ukey:disconnect"]).toBeDefined();
    });
