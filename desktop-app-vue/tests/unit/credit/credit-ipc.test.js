...
// Mock logger with different path patterns
...
describe("Credit IPC Handlers", () => {
  ...
  describe("credit:get-score-history", () => {
    ...
    it("should return score history with default limit", async () => {
      ...
      expect(mockCreditManager.getScoreHistory).toHaveBeenCalledWith(
        userDid,
        undefined,
      );
  ...
  describe("credit:get-leaderboard", () => {
    ...
    it("should verify leaderboard ordering", async () => {
      ...
      expect(result[0].credit_score).toBeGreaterThanOrEqual(
        result[1].credit_score,
      );
      expect(result[1].credit_score).toBeGreaterThanOrEqual(
        result[2].credit_score,
      );
    });
  });
  ...
  describe("Integration Scenarios", () => {
    ...
    it("should handle complete user credit workflow", async () => {
      ...
      // 1. Get user credit
      const credit = await mockIpcMain.invoke(
        "credit:get-user-credit",
        userDid,
      );
      ...
      // 2. Update score
      const updatedScore = await mockIpcMain.invoke(
        "credit:update-score",
        userDid,
      );
      ...
      // 3. Get score history
      const history = await mockIpcMain.invoke(
        "credit:get-score-history",
        userDid,
        10,
      );
      ...
      // 4. Get credit level
      const level = await mockIpcMain.invoke(
        "credit:get-credit-level",
        credit.credit_score,
      );
    ...
    it("should handle leaderboard and statistics together", async () => {
      const leaderboard = await mockIpcMain.invoke(
        "credit:get-leaderboard",
        10,
      );
