...
const { AIEngineManager, getAIEngineManager } = await import('../../src/main/ai-engine/ai-engine-manager.js');
...
describe('AIEngineManager', () => {
  ...
  beforeEach(() => {
    ...
    mockPlan.mockResolvedValue({
      steps: [
        { tool: 'html_generator', params: { title: 'Test Page' }, name: '生成HTML' }
      ]
    });
  ...
  describe('processUserInput - 完整工作流', () => {
    ...
    it('should handle execution errors gracefully', async () => {
      ...
      manager.functionCaller.call = vi.fn().mockRejectedValueOnce(new Error('Tool execution failed'));
    ...
    it('should continue execution even if one step fails', async () => {
      ...
      manager.functionCaller.call = vi.fn()
    ...
    it('should calculate correct overall success status', async () => {
      manager.functionCaller.call = vi.fn().mockResolvedValue({ success: true });

      const successResult = await manager.processUserInput('test');
      expect(successResult.success).toBe(true);

      manager.functionCaller.call = vi.fn().mockResolvedValue({ success: false });
  ...
  describe('步骤更新回调', () => {
    ...
    it('should mark step as failed when execution fails', async () => {
      manager.functionCaller.call = vi.fn().mockRejectedValueOnce(new Error('Failed'));
  ...
  describe('执行历史管理', () => {
    ...
    it('should limit total history to 100 entries', async () => {
      ...
      for (let i = 0; i < 105; i++) {
        manager.executionHistory.push({ id: `exec_${i}`, userInput: `test ${i}` });
      }
  ...
  describe('工具管理', () => {
    ...
    it('should unregister tool', () => {
      ...
      expect(manager.functionCaller.unregisterTool).toHaveBeenCalledWith('custom_tool');
    });
  ...
  describe('错误处理', () => {
    it('should throw error if intent classification fails', async () => {
      ...
      manager.intentClassifier.classify = vi.fn().mockRejectedValueOnce(new Error('Classification failed'));

      await expect(
        manager.processUserInput('test')
      ).rejects.toThrow('AI引擎处理失败');
    });

    it('should throw error if task planning fails', async () => {
      manager.taskPlanner.plan = vi.fn().mockRejectedValueOnce(new Error('Planning failed'));
    ...
    it('should handle errors during step execution gracefully', async () => {
      manager.functionCaller.call = vi.fn().mockRejectedValueOnce(new Error('Execution error'));
    ...
    it('should log errors to console', async () => {
      ...
      // Override the classify method to reject
      manager.intentClassifier.classify = vi.fn().mockRejectedValueOnce(new Error('Test error'));
  ...
  describe('上下文处理', () => {
    it('should pass context through entire pipeline', async () => {
      ...
      const mockClassifyFn = vi.fn().mockResolvedValue({ intent: 'CREATE_FILE', confidence: 0.9, entities: {} });
      const mockPlanFn = vi.fn().mockResolvedValue({ steps: [{ tool: 'test', params: {}, name: 'Test' }] });
      const mockCallFn = vi.fn().mockResolvedValue({ success: true });
  ...
  describe('单例模式', () => {
    ...
    it('should maintain state across getInstance calls', async () => {
      ...
      instance1.intentClassifier.classify = vi.fn().mockResolvedValue({ intent: 'CREATE_FILE', confidence: 0.9, entities: {} });
      instance1.taskPlanner.plan = vi.fn().mockResolvedValue({ steps: [{ tool: 'test', params: {}, name: 'Test' }] });
      instance1.functionCaller.call = vi.fn().mockResolvedValue({ success: true });
  ...
  describe('边缘情况', () => {
    ...
    it('should include all execution metadata', async () => {
      const result = await manager.processUserInput('test', { projectId: '123' });
