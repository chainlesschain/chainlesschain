...
describe('SyncQueue - 并发同步队列测试', () => {
  ...
  describe('并发控制', () => {
    ...
    it('应该在任务完成后继续处理队列', async () => {
      ...
      // 创建5个任务
      const tasks = Array.from({ length: 5 }, (_, i) => syncQueue.enqueue(createTask(i)));
  ...
  describe('优先级队列', () => {
    it('应该按照优先级顺序执行任务', async () => {
      ...
      expect(executionOrder.slice(3)).toEqual(['high1', 'high2', 'mid1', 'mid2']);
    });
  });
  ...
  describe('性能测试', () => {
    it('并发执行应该快于串行执行', async () => {
      ...
      const tasks = Array.from({ length: 6 }, () => concurrentQueue.enqueue(createTask()));
      await Promise.all(tasks);
      ...
      console.log(`性能提升: ${((serialDuration / concurrentDuration) * 100).toFixed(1)}%`);
    });
  });
});
...
describe('DBSyncManager - 并发同步集成测试', () => {
  ...
  beforeEach(() => {
    ...
    mockHttpClient = {
      ...
      downloadIncremental: vi.fn(async () => ({ newRecords: [], updatedRecords: [], deletedIds: [] }))
    };
  ...
  describe('并发性能验证', () => {
    it('并发同步应该比串行同步更快', async () => {
      ...
      mockHttpClient.uploadBatch = vi.fn(async () => {
        await new Promise(resolve => setTimeout(resolve, tableSyncDuration / 2));
        return { successCount: 1, conflictCount: 0 };
      });
      ...
      console.log(`性能提升: ${((tableCount * tableSyncDuration / duration) * 100).toFixed(1)}%`);
    });
  });
});
