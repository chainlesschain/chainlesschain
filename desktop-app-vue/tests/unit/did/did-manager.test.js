...
// Inert vi.mock — vi.mock 不拦截 source require (B1 finding)；source 走 _setNaclForTesting seam。
// Object.assign 合并 fn-with-static 避免 no-dupe-keys。
vi.mock("tweetnacl", () => ({
  default: {
    sign: {
      keyPair: Object.assign(
        vi.fn(() => mockSignKeyPair),
        {
          fromSeed: vi.fn(() => ({
  ...
  default: {
    createHash: vi.fn(() => ({
      ...
      digest: vi.fn(
        () =>
          new Uint8Array(
            Buffer.from("0123456789abcdef0123456789abcdef0123456789ab", "hex"),
          ),
      ),
    })),
  },
...
// Fake nacl/bip39 injected via seams (RFC T1; vi.mock 不拦截 source require).
// 见 `docs/design/desktop_vi_mock_cjs_migration_rfc.md`.
function makeFakeNacl() {
  const signKeyPair = () => ({
    publicKey: new Uint8Array(32).fill(1),
    secretKey: new Uint8Array(64).fill(2),
  });
  signKeyPair.fromSeed = (seed) => ({
    publicKey: new Uint8Array(32).fill(10),
    secretKey: new Uint8Array(64).fill(11),
  });

  const signDetached = (msg, sk) => new Uint8Array(64).fill(5);
  signDetached.verify = (msg, sig, pk) => true;

  const boxKeyPair = () => ({
    publicKey: new Uint8Array(32).fill(3),
    secretKey: new Uint8Array(32).fill(4),
  });
  boxKeyPair.fromSecretKey = (sk) => ({
    publicKey: new Uint8Array(32).fill(12),
    secretKey: new Uint8Array(32).fill(13),
  });

  return {
    sign: { keyPair: signKeyPair, detached: signDetached },
    box: { keyPair: boxKeyPair },
  };
}

function makeFakeBip39() {
  return {
    generateMnemonic: (strength) => mockMnemonic,
    validateMnemonic: (m) => m === mockMnemonic,
    mnemonicToSeedSync: (m) => new Uint8Array(64).fill(15),
  };
}
...
describe("DIDManager", () => {
  ...
  beforeEach(async () => {
    ...
    // Inject fake nacl + bip39 via seams (overrides real require()).
    DIDManager._setNaclForTesting(makeFakeNacl());
    DIDManager._setBip39ForTesting(makeFakeBip39());
  ...
  afterEach(() => {
    if (DIDManager) {
      DIDManager._setNaclForTesting(null);
      DIDManager._setBip39ForTesting(null);
    }
    didManager = null;
  });
  ...
  describe("构造函数", () => {
    ...
    it("应该支持自定义配置", () => {
      ...
      const customManager = new DIDManager(
        mockDb,
        mockP2PManager,
        customConfig,
      );
  ...
  describe("初始化", () => {
    ...
    it("初始化失败时应该抛出错误", async () => {
      didManager.cache.initialize = vi
        .fn()
        .mockRejectedValue(new Error("Cache init failed"));

      await expect(didManager.initialize()).rejects.toThrow(
        "Cache init failed",
      );
    });
  });
  ...
  describe("DID 创建", () => {
    ...
    it("应该生成 X25519 加密密钥对", async () => {
      ...
      expect(identity.didDocument.verificationMethod[0].type).toBe(
        "Ed25519VerificationKey2020",
      );
      expect(identity.didDocument.verificationMethod[1].type).toBe(
    ...
    it("创建失败时应该抛出错误", async () => {
      ...
      await expect(didManager.createIdentity({})).rejects.toThrow(
        "Database error",
      );
    });
  });
  ...
  describe("createDIDDocument", () => {
    ...
    it("应该包含签名验证方法", () => {
      ...
      const document = didManager.createDIDDocument(did, keys);
      const signMethod = document.verificationMethod.find(
    ...
    it("应该包含加密密钥协商方法", () => {
      ...
      const document = didManager.createDIDDocument(did, keys);
      const encryptMethod = document.verificationMethod.find(
  ...
  describe("signDIDDocument 和 verifyDIDDocument", () => {
    it("应该正确签名 DID 文档", () => {
      ...
      expect(signedDocument.proof.verificationMethod).toBe(
        "did:chainlesschain:test123#sign-key-1",
      );
  ...
  describe("身份管理", () => {
    ...
    it("身份不存在时应该返回 null", () => {
      ...
      const identity = didManager.getIdentityByDID(
        "did:chainlesschain:nonexistent",
      );
    ...
    it("不应该删除默认身份", async () => {
      ...
      await expect(didManager.deleteIdentity(testDID)).rejects.toThrow(
        "不能删除默认身份",
      );
    });
  });
  ...
  describe("DHT 发布和解析", () => {
    ...
    it("P2P 未初始化时应该抛出错误", async () => {
      ...
      await expect(
        didManager.publishToDHT("did:chainlesschain:test"),
      ).rejects.toThrow("P2P 节点未初始化");
    });
    ...
    it("应该从 DHT 解析 DID", async () => {
      ...
      const mockDIDData = {
        ...
        didDocument: {
          ...
          proof: {
            ...
            // valid base64 required — real tweetnacl-util.decodeBase64 throws on invalid input
            proofValue:
              "BQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQU=",
          },
          verificationMethod: [
      ...
      mockP2PManager.dhtGet = vi
        .fn()
        .mockResolvedValue(Buffer.from(JSON.stringify(mockDIDData)));
      didManager.cache.get = vi.fn().mockResolvedValue(null); // Cache miss
      didManager.cache.set = vi.fn().mockResolvedValue(undefined);
    ...
    it("DID 格式无效时应该抛出错误", async () => {
      await expect(didManager.resolveFromDHT("invalid-did")).rejects.toThrow(
        "无效的 DID 格式",
      );
    });
  });
  ...
  describe("助记词功能", () => {
    ...
    it("使用助记词创建身份", async () => {
      ...
      const identity = await didManager.createIdentityFromMnemonic(
        profile,
        mockMnemonic,
      );
