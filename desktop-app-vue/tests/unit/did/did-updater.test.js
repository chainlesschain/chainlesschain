...
describe("DIDUpdater", () => {
  ...
  beforeEach(async () => {
    ...
    mockDidManager = {
      ...
      // getVersionHistory normalizes rows via the manager; mirror the real
      // helper (plain better-sqlite3 rows pass through; sql.js {columns,values}
      // are converted).
  ...
  describe("构造函数", () => {
    ...
    it("应该支持自定义配置", () => {
      ...
      const customUpdater = new DIDUpdater(
        mockDidManager,
        mockP2PManager,
        customConfig,
      );
  ...
  describe("版本历史", () => {
    ...
    it("应该获取版本历史", () => {
      const did = "did:chainlesschain:test123";
      // better-sqlite3 .all() returns plain row objects (keyed by column).
      const rows = [
        { version: 2, changes: "Updated profile", updated_at: Date.now() },
        {
          version: 1,
      ...
      mockDb.prepare().all = vi.fn().mockReturnValue(rows);
  ...
  describe("republish", () => {
    ...
    it("发布失败时应该抛出错误", async () => {
      mockDidManager.publishToDHT = vi
        .fn()
        .mockRejectedValue(new Error("Publish failed"));

      await expect(didUpdater.republish("did:test")).rejects.toThrow(
        "Publish failed",
      );
    });
  });
  ...
  describe("incrementVersion", () => {
    ...
    it("DID 不存在时应该抛出错误", async () => {
      ...
      await expect(
        didUpdater.incrementVersion("did:nonexistent"),
      ).rejects.toThrow("不存在");
    });
  });
  ...
  describe("边界情况", () => {
    ...
    it("应该处理数据库错误", async () => {
      ...
      // 不应该抛出错误
      await expect(
        didUpdater.saveVersionHistory("did:test", {}),
    ...
    it("更新检测失败时应该触发 update-error 事件", async () => {
      ...
      mockDidManager.resolveFromDHT = vi
        .fn()
