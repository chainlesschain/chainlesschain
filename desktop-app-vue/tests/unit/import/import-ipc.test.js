...
describe("ImportIPC", () => {
  ...
  beforeEach(async () => {
    ...
    // Create mock ipcGuard that always allows registration
    mockIpcGuard = {
      isModuleRegistered: vi.fn(() => false),
      markModuleRegistered: vi.fn(),
      isChannelRegistered: vi.fn(() => false),
    ...
    mockFileImporter = {
      ...
      getSupportedFormats: vi
        .fn()
        .mockReturnValue([".md", ".markdown", ".pdf", ".doc", ".docx", ".txt"]),
  ...
  describe("registerImportIPC()", () => {
    ...
    it("should register format check handlers", () => {
      ...
      expect(
        mockIpcMain.getHandler("import:get-supported-formats"),
      ).toBeDefined();
  ...
  describe("import:select-files", () => {
    ...
    it("should throw error when fileImporter is not initialized", async () => {
      ...
      await expect(
        mockIpcMainNoImporter.invoke("import:select-files"),
  ...
  describe("import:import-file", () => {
    ...
    it("should import a single file successfully", async () => {
      ...
      expect(mockFileImporter.importFile).toHaveBeenCalledWith(
        filePath,
        options,
      );
    ...
    it("should register event listeners for import progress", async () => {
      ...
      expect(mockFileImporter.on).toHaveBeenCalledWith(
        "import-start",
        expect.any(Function),
      );
      expect(mockFileImporter.on).toHaveBeenCalledWith(
    ...
    it("should add imported item to RAG index", async () => {
      ...
      expect(mockDatabase.getKnowledgeItemById).toHaveBeenCalledWith(
        "note-123",
      );
  ...
  describe("import:import-files", () => {
    ...
    it("should import multiple files successfully", async () => {
      ...
      expect(mockFileImporter.importFiles).toHaveBeenCalledWith(
        filePaths,
        options,
      );
  ...
  describe("import:get-supported-formats", () => {
    ...
    it("should throw error when fileImporter is not initialized", async () => {
      ...
      await expect(
        mockIpcMainNoImporter.invoke("import:get-supported-formats"),
      ).rejects.toThrow("文件导入器未初始化");
    });
    ...
    it("should propagate getSupportedFormats errors", async () => {
      ...
      await expect(
        mockIpcMain.invoke("import:get-supported-formats"),
      ).rejects.toThrow("Failed to get formats");
    });
  });
  ...
  describe("import:check-file", () => {
    ...
    it("should propagate check errors", async () => {
      ...
      await expect(
        mockIpcMain.invoke("import:check-file", "/path/to/test.md"),
  ...
  describe("Error Handling", () => {
    ...
    it("should handle async errors in import:import-file", async () => {
      mockFileImporter.importFile.mockRejectedValueOnce(
        new Error("File not found"),
      );

      await expect(
        mockIpcMain.invoke("import:import-file", "/nonexistent.md", {}),
      ).rejects.toThrow("File not found");
    });

    it("should handle async errors in import:import-files", async () => {
      mockFileImporter.importFiles.mockRejectedValueOnce(
        new Error("Permission denied"),
      );

      await expect(
        mockIpcMain.invoke("import:import-files", ["/protected.md"], {}),
      ).rejects.toThrow("Permission denied");
    });

    it("should handle RAG index errors gracefully", async () => {
      mockRagManager.addToIndex.mockRejectedValueOnce(
    ...
    it("should handle mainWindow being null", async () => {
      ...
      await mockIpcMainNoWindow.invoke(
        "import:import-file",
        "/path/to/test.md",
        {},
      );
  ...
  describe("Event Handling", () => {
    ...
    it("should handle multiple simultaneous imports", async () => {
      const promise1 = mockIpcMain.invoke(
        "import:import-file",
        "/path/to/file1.md",
        {},
      );
      const promise2 = mockIpcMain.invoke(
        "import:import-file",
        "/path/to/file2.md",
        {},
      );
