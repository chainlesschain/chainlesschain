...
describe("AIPromptsPage", () => {
  ...
  describe("组件挂载", () => {
    ...
    it("应该渲染提示词面板组件", () => {
      wrapper = createWrapper();
      expect(
        wrapper.findComponent({ name: "SuggestedPromptsPanel" }).exists(),
      ).toBe(true);
    });
  });
  ...
  describe("发送提示词", () => {
    it("应该能发送提示词并创建对话", async () => {
      ...
      expect(window.electronAPI.conversation.create).toHaveBeenCalledWith({
        title: "Help me write a function to so...",
      });
    ...
    it("应该截断长标题", async () => {
      ...
      const longPrompt =
        "This is a very long prompt text that should be truncated to 30 characters";

      await wrapper.vm.handleSend(longPrompt);

      const createCall =
        window.electronAPI.conversation.create.mock.calls[0][0];
