...
describe('MarketPage.vue', () => {
  ...
  describe('Sort Functionality', () => {
    ...
    it('应该按价格从高到低排序', async () => {
      ...
      const filtered = wrapper.vm.filteredProjects;
      for (let i = 0; i < filtered.length - 1; i++) {
        expect(filtered[i].price).toBeGreaterThanOrEqual(
          filtered[i + 1].price
        );
      }
    });
  ...
  describe('Refresh Functionality', () => {
    ...
    it('应该处理刷新失败', async () => {
      ...
      wrapper.vm.loadMarketProjects = vi.fn().mockRejectedValue(
        new Error('刷新失败')
      );
  ...
  describe('Buy Project', () => {
    ...
    it('应该扣除余额', async () => {
      ...
      wrapper.vm.selectedProject = { ...wrapper.vm.marketProjects[0], price: 100 };
  ...
  describe('Sell Project', () => {
    ...
    it('应该处理上架失败', async () => {
      ...
      expect(logger.error).toHaveBeenCalled();
      expect(message.error).toHaveBeenCalledWith(expect.stringContaining('上架失败'));
    });
  });
