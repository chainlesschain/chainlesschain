...
vi.mock('@ant-design/icons-vue', () => ({
  ...
  AppstoreOutlined: { name: 'AppstoreOutlined', template: '<span>Appstore</span>' },
...
describe('FeedList', () => {
  ...
  describe('组件挂载和初始化', () => {
    ...
    it('应该能加载订阅源', async () => {
      ...
      expect(window.electron.ipcRenderer.invoke).toHaveBeenCalledWith('rss:get-feeds');
  ...
  describe('添加订阅源', () => {
    ...
    it('处理添加订阅失败', async () => {
      const { message } = require('ant-design-vue');
      window.electron.ipcRenderer.invoke.mockRejectedValue(new Error('添加失败'));

      wrapper.vm.feedForm.url = 'https://example.com/feed.xml';
      await wrapper.vm.handleAddFeed();

      expect(message.error).toHaveBeenCalledWith(expect.stringContaining('添加失败'));
    });
  });
  ...
  describe('编辑订阅源', () => {
    ...
    it('处理更新失败', async () => {
      const { message } = require('ant-design-vue');
      window.electron.ipcRenderer.invoke.mockRejectedValue(new Error('更新失败'));
      ...
      expect(message.error).toHaveBeenCalledWith(expect.stringContaining('更新失败'));
    });
  });
  ...
  describe('刷新订阅源', () => {
    ...
    it('处理刷新失败', async () => {
      const { message } = require('ant-design-vue');
      window.electron.ipcRenderer.invoke.mockRejectedValue(new Error('刷新失败'));

      await wrapper.vm.refreshFeed('feed-1');

      expect(message.error).toHaveBeenCalledWith(expect.stringContaining('刷新失败'));
    });
    ...
    it('处理批量刷新失败', async () => {
      const { message } = require('ant-design-vue');
      window.electron.ipcRenderer.invoke.mockRejectedValue(new Error('批量刷新失败'));
  ...
  describe('删除订阅源', () => {
    ...
    it('处理删除失败', async () => {
      const { message } = require('ant-design-vue');
      window.electron.ipcRenderer.invoke.mockRejectedValue(new Error('删除失败'));

      await wrapper.vm.deleteFeed('feed-1');

      expect(message.error).toHaveBeenCalledWith(expect.stringContaining('删除失败'));
    });
  });
  ...
  describe('发现订阅源', () => {
    ...
    it('处理发现订阅失败', async () => {
      const { message } = require('ant-design-vue');
      window.electron.ipcRenderer.invoke.mockRejectedValue(new Error('发现失败'));

      wrapper.vm.discoverUrl = 'https://example.com';
      await wrapper.vm.handleDiscoverFeeds();

      expect(message.error).toHaveBeenCalledWith(expect.stringContaining('发现失败'));
    });
  });
