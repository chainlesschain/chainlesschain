...
vi.mock('@ant-design/icons-vue', () => ({
  FileTextOutlined: { name: 'FileTextOutlined', template: '<span>FileText</span>' },
...
vi.mock('../components/common/VirtualGrid.vue', () => ({
  default: {
    name: 'VirtualGrid',
    template: '<div class="mock-virtual-grid"><slot :item="item" v-for="item in items" /></div>',
...
describe('KnowledgeListPage', () => {
  ...
  describe('排序功能', () => {
    ...
    it('标题排序应该按字母顺序', () => {
      ...
      for (let i = 0; i < items.length - 1; i++) {
        expect(items[i].title.localeCompare(items[i + 1].title)).toBeLessThanOrEqual(0);
      }
    });
  });
  ...
  describe('搜索和排序组合', () => {
    ...
    it('搜索后应该保持排序顺序', () => {
      ...
      if (items.length > 1) {
        for (let i = 0; i < items.length - 1; i++) {
          expect(items[i].title.localeCompare(items[i + 1].title)).toBeLessThanOrEqual(
            0
          );
        }
      }
    ...
    it('排序后应该保持搜索过滤', () => {
      ...
      expect(items.every((item) => item.title.toLowerCase().includes('vue'))).toBe(
        true
      );
    });
  ...
  describe('响应式状态', () => {
    ...
    it('排序方式应该是响应式的', async () => {
      wrapper.vm.sortBy = 'time';
      await nextTick();
      const timeOrder = wrapper.vm.filteredKnowledgeItems.map((item) => item.id);

      wrapper.vm.sortBy = 'title';
      await nextTick();
      const titleOrder = wrapper.vm.filteredKnowledgeItems.map((item) => item.id);
