...
import OrganizationRolesPage from '@/pages/OrganizationRolesPage.vue';
...
describe('OrganizationRolesPage.vue', () => {
  ...
  describe('Edit Role', () => {
    ...
    it('应该能打开编辑角色对话框', () => {
      ...
      expect(wrapper.vm.roleForm.permissions).toEqual(['project.manage', 'member.view']);
    });
    ...
    it('应该处理更新角色失败', async () => {
      const { message } = require('ant-design-vue');
      window.electron.ipcRenderer.invoke.mockRejectedValue(new Error('Update failed'));
      wrapper.vm.isEditMode = true;
      wrapper.vm.roleFormRef = {
        validate: vi.fn().mockResolvedValue(),
        resetFields: vi.fn(),
  ...
  describe('Form Validation Rules', () => {
    ...
    it('应该有权限必选规则', () => {
      ...
      expect(permRules.some(r => r.required && r.type === 'array')).toBe(true);
    });
  });
