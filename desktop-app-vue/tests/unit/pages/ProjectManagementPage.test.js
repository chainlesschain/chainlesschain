...
describe('ProjectManagementPage.vue', () => {
  ...
  const createWrapper = () => {
    ...
    return mount(ProjectManagementPage, {
      global: {
        stubs: {
          'a-page-header': { template: '<div><slot /><slot name="extra" /></div>' },
          'a-button': { template: '<button><slot /></button>' },
          'a-row': { template: '<div><slot /></div>' },
          'a-col': { template: '<div><slot /></div>' },
          'a-card': { template: '<div><slot /><slot name="title" /><slot name="extra" /></div>' },
  ...
  describe('Filter Functionality', () => {
    ...
    it('应该能按状态筛选', () => {
      ...
      expect(mockProjectStore.setFilter).toHaveBeenCalledWith('status', 'active');
      expect(mockProjectStore.setPagination).toHaveBeenCalledWith(1);
    });
  ...
  describe('Table Functionality', () => {
    ...
    it('应该处理排序', () => {
      ...
      expect(mockProjectStore.setSort).toHaveBeenCalledWith('created_at', 'asc');
    });
    ...
    it('应该处理降序排序', () => {
      ...
      expect(mockProjectStore.setSort).toHaveBeenCalledWith('total_size', 'desc');
    });
  ...
  describe('Create Project', () => {
    ...
    it('应该处理创建失败', async () => {
      ...
      expect(message.error).toHaveBeenCalledWith(expect.stringContaining('创建失败'));
    });
  });
  ...
  describe('Batch Delete', () => {
    ...
    it('应该处理批量删除失败', async () => {
      ...
      expect(logger.error).toHaveBeenCalled();
      expect(message.error).toHaveBeenCalledWith(expect.stringContaining('批量删除失败'));
    });
  });
  ...
  describe('Computed Properties', () => {
    it('应该返回分页数据', () => {
      ...
      expect(wrapper.vm.paginatedData).toEqual(mockProjectStore.paginatedProjects);
    });
  ...
  describe('Edge Cases', () => {
    it('应该处理无用户情况', async () => {
      ...
      expect(mockProjectStore.fetchProjects).toHaveBeenCalledWith('default-user');
    });
