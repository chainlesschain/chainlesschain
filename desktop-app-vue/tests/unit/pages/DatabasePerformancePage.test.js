...
describe("DatabasePerformancePage", () => {
    ...
    return mount(
      {
        ...
        setup() {
          ...
          // Use deep copies to isolate each test from shared state mutations
          const stats = ref(JSON.parse(JSON.stringify(mockStats)));
          const slowQueries = ref(JSON.parse(JSON.stringify(mockSlowQueries)));
          const indexSuggestions = ref(
            JSON.parse(JSON.stringify(mockIndexSuggestions)),
          );
          ...
          const applyIndexSuggestion = async (suggestion) => {
            ...
            try {
              ...
              message.success(
                `索引 ${suggestion.table}.${suggestion.column} 已应用`,
              );
  ...
  describe("慢查询日志", () => {
    ...
    it("应该能格式化时间", () => {
      wrapper = createWrapper();
      const formatted = wrapper.vm.formatTime("2026-01-26T10:30:00.000Z");
  ...
  describe("索引优化建议", () => {
    ...
    it("应该能应用单个索引建议", async () => {
      ...
      expect(message.success).toHaveBeenCalledWith("索引 notes.content 已应用");
    });
    ...
    it("应该能处理批量应用失败", async () => {
      ...
      expect(message.error).toHaveBeenCalledWith("批量应用索引失败: 批量失败");
    });
  ...
  describe("缓存统计", () => {
    ...
    it("应该显示命中率", async () => {
      // Set up mocks for the 3 onMounted calls
      window.electronAPI.invoke
        .mockResolvedValueOnce(mockStats)
        .mockResolvedValueOnce(mockSlowQueries)
        .mockResolvedValueOnce(mockIndexSuggestions);
  ...
  describe("错误处理", () => {
    ...
    it("应该能处理慢查询加载失败", async () => {
      ...
      expect(message.error).toHaveBeenCalledWith("加载慢查询失败: 查询错误");
    });
    ...
    it("应该能处理索引建议加载失败", async () => {
      ...
      expect(message.error).toHaveBeenCalledWith("加载索引建议失败: 建议错误");
    });
  ...
  describe("数据刷新", () => {
    it("应该能同时刷新所有数据", async () => {
      wrapper = createWrapper();
      // Wait for onMounted to complete and clear previous mock calls
      await wrapper.vm.$nextTick();
      await new Promise((resolve) => setTimeout(resolve, 0));
