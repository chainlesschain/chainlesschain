...
const mockAntdComponents = {
  ...
  "a-form-item": {
    name: "AFormItem",
    template:
      '<div class="a-form-item" :class="{ error: validateStatus === \'error\' }"><label v-if="label">{{ label }}</label><slot /></div>',
...
describe("OrganizationKnowledgePage", () => {
  ...
  beforeEach(async () => {
    ...
    // Use mockImplementation with channel-based routing so all 3 sequential
    // awaits in loadOrgKnowledge() resolve properly regardless of timing
    mockIpcRenderer.invoke.mockImplementation((channel) => {
      if (channel === "org:get-knowledge") {
        return Promise.resolve([...mockKnowledgeItems]);
      }
      if (channel === "org:get-categories") {
        return Promise.resolve([...mockCategories]);
      }
      if (channel === "org:get-tags") {
        return Promise.resolve([...mockTags]);
      }
      if (channel === "org:share-knowledge") {
        return Promise.resolve({ success: true });
      }
      if (channel === "org:unshare-knowledge") {
        return Promise.resolve({ success: true });
      }
      if (channel === "org:update-knowledge-visibility") {
        return Promise.resolve({ success: true });
      }
      if (channel === "org:update-knowledge-permissions") {
        return Promise.resolve({ success: true });
      }
    ...
    OrganizationKnowledgePage = {
      ...
      methods: {
        ...
        async handleChangeVisibility(item) {
          ...
          const newVisibility =
            item.visibility === "org-wide" ? "team" : "org-wide";
  ...
  describe("1. Component Mounting and Initialization", () => {
    ...
    it("should display shared knowledge items", async () => {
      ...
      expect(wrapper.vm.knowledgeList[0].title).toBe(
        "Project Architecture Guide",
      );
      expect(wrapper.vm.knowledgeList[1].title).toBe("API Documentation.pdf");
      expect(wrapper.vm.knowledgeList[2].title).toBe(
  ...
  describe("3. Knowledge List Management", () => {
    ...
    it("should show knowledge type icons (note/file/link)", async () => {
      ...
      // Mock a-list stub does not render slot items, so check vm data instead
      expect(wrapper.vm.filteredKnowledge.length).toBeGreaterThan(0);

      const noteItem = wrapper.vm.filteredKnowledge.find(
        (k) => k.type === "note",
      );
      const fileItem = wrapper.vm.filteredKnowledge.find(
        (k) => k.type === "file",
      );
      const linkItem = wrapper.vm.filteredKnowledge.find(
        (k) => k.type === "link",
      );
  ...
  describe("4. Search and Filtering", () => {
    it("should filter by knowledge title", async () => {
      ...
      expect(wrapper.vm.filteredKnowledge[0].title).toBe(
        "Project Architecture Guide",
      );
    });
  ...
  describe("5. Visibility Control", () => {
    ...
    it("should show visibility icon (public/private)", async () => {
      ...
      const teamItem = wrapper.vm.filteredKnowledge.find(
        (k) => k.visibility === "team",
      );
    ...
    it("should update visibility via IPC", async () => {
      ...
      // Use an item owned by the current user
      const ownedItem = wrapper.vm.filteredKnowledge.find(
        (k) => k.sharedBy === "did:key:user123",
      );

      await wrapper.vm.handleChangeVisibility(ownedItem);
      await flushPromises();
      await nextTick();
      ...
      expect(mockIpcRenderer.invoke).toHaveBeenCalledWith(
        "org:update-knowledge-visibility",
        ownedItem.id,
        ownedItem.visibility === "org-wide" ? "team" : "org-wide",
        "did:key:user123",
      );
    });
    ...
    it("should validate owner permission for visibility change", async () => {
      ...
      const visibilityCalls = mockIpcRenderer.invoke.mock.calls.filter(
        (call) => call[0] === "org:update-knowledge-visibility",
      );
  ...
  describe("6. Permission Management", () => {
    it("should set view-only permission", async () => {
      ...
      // Use an item owned by the current user (has view permission)
      const ownedItem = wrapper.vm.filteredKnowledge.find(
        (k) => k.sharedBy === "did:key:user123" && k.permissions === "view",
      );
    ...
    it("should prevent non-owner from changing permissions", async () => {
      ...
      const permissionCalls = mockIpcRenderer.invoke.mock.calls.filter(
        (call) => call[0] === "org:update-knowledge-permissions",
      );
  ...
  describe("8. Error Handling", () => {
    it("should handle load knowledge failure", async () => {
      ...
      mockIpcRenderer.invoke.mockRejectedValue(
        new Error("Failed to load knowledge"),
      );
    ...
    it("should handle unshare failure", async () => {
      ...
      mockIpcRenderer.invoke.mockImplementation((channel) => {
        if (channel === "org:unshare-knowledge") {
          return Promise.reject(new Error("Unshare failed"));
        }
        return Promise.resolve(null);
      });
