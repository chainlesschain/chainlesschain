    ...
    return mount(
      {
        ...
        setup() {
          ...
          const friendGroups = computed(() => {
            const groups = new Set(allFriends.value.map((f) => f.group).filter(Boolean));
          ...
          const handleMenuAction = (key, friend) => {
            if (key === 'edit') {
              ...
              Modal.confirm({
                ...
                onOk: async () => {
                  try {
                    await window.electronAPI.invoke('friends:delete', friend.id);
                    allFriends.value = allFriends.value.filter((f) => f.id !== friend.id);
          ...
          const handleAddFriend = async () => {
            ...
            try {
              await window.electronAPI.invoke('friends:add', addFriendForm.value);
