...
describe('OrganizationActivityLogPage.vue', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    window.electron.ipcRenderer.invoke.mockImplementation((channel, ...args) => {
  ...
  describe('Component Mounting and Initialization', () => {
    ...
    it('应该处理加载活动日志异常', async () => {
      window.electron.ipcRenderer.invoke.mockRejectedValue(new Error('Network error'));
      const { message } = require('ant-design-vue');
      wrapper = createWrapper();
  ...
  describe('Filtering', () => {
    ...
    it('应该能按操作者筛选', async () => {
      ...
      expect(filtered.every(a => a.actor_did === 'did:chainlesschain:user1')).toBe(true);
    });
  ...
  describe('Export', () => {
    ...
    it('应该处理导出异常', async () => {
      window.electron.ipcRenderer.invoke.mockRejectedValue(new Error('Export error'));
      const { message } = require('ant-design-vue');
  ...
  describe('Helper Functions', () => {
    ...
    it('应该返回未知操作的原值', () => {
      expect(wrapper.vm.getActionLabel('unknown_action')).toBe('unknown_action');
    });
  ...
  describe('Table Columns', () => {
    ...
    it('应该有时间排序功能', () => {
      const timestampColumn = wrapper.vm.columns.find(c => c.key === 'timestamp');
