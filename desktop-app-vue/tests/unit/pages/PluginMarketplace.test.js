...
// Helper to flush all pending promises (microtasks from onMounted loadPlugins)
...
describe("PluginMarketplace", () => {
          ...
          const loadPlugins = async () => {
            ...
            try {
              ...
              if (result.success) {
                ...
                plugins.value = mockPlugins.map((p) => ({ ...p }));
              }
  ...
  describe("组件挂载", () => {
    ...
    it("应该在挂载时加载插件列表", async () => {
      window.electronAPI.pluginMarketplace.list.mockResolvedValue({
        success: true,
        data: { plugins: mockPlugins.map((p) => ({ ...p })) },
      });
      wrapper = createWrapper();
  ...
  describe("搜索功能", () => {
    it("应该能按名称搜索插件", async () => {
      ...
      wrapper.vm.plugins = mockPlugins.map((p) => ({ ...p }));
  ...
  describe("插件详情", () => {
    it("应该能显示插件详情", async () => {
      ...
      expect(wrapper.vm.selectedPlugin).toStrictEqual(plugin);
  ...
  describe("权限描述", () => {
    it("应该返回正确的权限描述", () => {
      ...
      expect(wrapper.vm.getPermissionDescription("database:read")).toBe(
        "读取数据库",
      );
      expect(wrapper.vm.getPermissionDescription("database:write")).toBe(
        "写入数据库",
      );
      expect(wrapper.vm.getPermissionDescription("llm:query")).toBe(
        "调用AI模型",
      );
      expect(wrapper.vm.getPermissionDescription("network:request")).toBe(
        "访问网络",
      );
      expect(wrapper.vm.getPermissionDescription("ui:register")).toBe(
        "注册界面组件",
      );
    });
