...
describe("PathSecurity - 路径安全验证", () => {
  ...
  describe("validateFileAccess", () => {
    test("应该验证存在的文件", async () => {
      ...
      const result = await PathSecurity.validateFileAccess(
        filePath,
        testProject,
      );
  ...
  describe("containsDangerousChars", () => {
    test("应该检测父目录遍历", () => {
      ...
      expect(PathSecurity.containsDangerousChars("subdir/../file.txt")).toBe(
        true,
      );
    });
    ...
    test("应该检测系统目录路径", () => {
      ...
      expect(PathSecurity.containsDangerousChars("C:\\Windows\\System32")).toBe(
        true,
      );
      expect(
        PathSecurity.containsDangerousChars("C:\\Program Files\\app"),
      ).toBe(true);
    });
    ...
    test("应该允许正常的文件路径", () => {
      ...
      expect(PathSecurity.containsDangerousChars("my-project_v2.txt")).toBe(
        false,
      );
    });
  });
  ...
  describe("validateFileExtension", () => {
    test("应该允许白名单中的扩展名", () => {
      ...
      expect(PathSecurity.validateFileExtension("test.txt", allowed)).toBe(
        true,
      );
      expect(PathSecurity.validateFileExtension("README.md", allowed)).toBe(
        true,
      );
      expect(PathSecurity.validateFileExtension("config.json", allowed)).toBe(
        true,
      );
    });
    ...
    test("应该阻止白名单外的扩展名", () => {
      const allowed = ["txt", "md", "json"];
      expect(PathSecurity.validateFileExtension("script.exe", allowed)).toBe(
        false,
      );
      expect(PathSecurity.validateFileExtension("malware.bat", allowed)).toBe(
        false,
      );
    });
  ...
  describe("sanitizeFilename", () => {
    test("应该移除路径分隔符", () => {
      expect(PathSecurity.sanitizeFilename("path/to/file.txt")).toBe(
        "path_to_file.txt",
      );
      expect(PathSecurity.sanitizeFilename("path\\to\\file.txt")).toBe(
        "path_to_file.txt",
      );
    });

    test("应该移除危险字符", () => {
      expect(PathSecurity.sanitizeFilename("file<name>.txt")).toBe(
        "filename.txt",
      );
      expect(PathSecurity.sanitizeFilename("file|name.txt")).toBe(
        "filename.txt",
      );
      expect(PathSecurity.sanitizeFilename("file\0name.txt")).toBe(
        "filename.txt",
      );
    });
    ...
    test("应该保留正常的文件名", () => {
      expect(PathSecurity.sanitizeFilename("normal-file_v2.txt")).toBe(
        "normal-file_v2.txt",
      );
      expect(PathSecurity.sanitizeFilename("中文文件名.txt")).toBe(
        "中文文件名.txt",
      );
    });
  });
  ...
  describe("isSensitiveSystemPath", () => {
    ...
    test("阻止 home 下的凭据/密钥目录 (展开后的绝对路径)", () => {
      expect(
        PathSecurity.isSensitiveSystemPath(path.join(home, ".ssh", "id_rsa")),
      ).toBe(true);
      expect(
        PathSecurity.isSensitiveSystemPath(
          path.join(home, ".aws", "credentials"),
        ),
      ).toBe(true);
      expect(
        PathSecurity.isSensitiveSystemPath(path.join(home, ".gnupg")),
      ).toBe(true);
      expect(
        PathSecurity.isSensitiveSystemPath(
      ...
      expect(
        PathSecurity.isSensitiveSystemPath(
          path.join(home, ".config", "gcloud", "x"),
        ),
      ).toBe(true);
    });

    test("阻止 ~ 形式 (与展开后一致)", () => {
      expect(PathSecurity.isSensitiveSystemPath("~/.ssh/id_rsa")).toBe(true);
      expect(PathSecurity.isSensitiveSystemPath("~/.aws/credentials")).toBe(
        true,
      );
    });

    test("阻止 home 下的敏感单文件", () => {
      expect(
        PathSecurity.isSensitiveSystemPath(path.join(home, ".netrc")),
      ).toBe(true);
      expect(
        PathSecurity.isSensitiveSystemPath(path.join(home, ".git-credentials")),
      ).toBe(true);
    });
    ...
    test("阻止系统目录 (按平台)", () => {
      if (isWin) {
        expect(
          PathSecurity.isSensitiveSystemPath("C:\\Windows\\System32\\config"),
        ).toBe(true);
        expect(
          PathSecurity.isSensitiveSystemPath("C:\\Program Files\\app\\x.exe"),
        ).toBe(true);
        expect(
          PathSecurity.isSensitiveSystemPath("c:\\windows\\system32"),
        ).toBe(true); // 大小写不敏感
      } else {
        expect(PathSecurity.isSensitiveSystemPath("/etc/passwd")).toBe(true);
        expect(PathSecurity.isSensitiveSystemPath("/root/.bashrc")).toBe(true);
        expect(PathSecurity.isSensitiveSystemPath("/proc/self/environ")).toBe(
          true,
        );
      }
    });
    ...
    test("允许普通用户文件 (不受影响)", () => {
      expect(
        PathSecurity.isSensitiveSystemPath(
          path.join(os.tmpdir(), "cc-export.txt"),
        ),
      ).toBe(false);
      expect(
        PathSecurity.isSensitiveSystemPath(
      ...
      expect(
        PathSecurity.isSensitiveSystemPath(
          path.join(home, "Downloads", "photo.png"),
        ),
      ).toBe(false);
    });

    test("边界: 相似前缀的兄弟目录不误伤 (.ssh-notes ≠ .ssh)", () => {
      expect(
        PathSecurity.isSensitiveSystemPath(
          path.join(home, ".ssh-notes", "x.txt"),
        ),
      ).toBe(false);
    });

    test("空/非字符串输入返回 false", () => {
      expect(PathSecurity.isSensitiveSystemPath("")).toBe(false);
      expect(PathSecurity.isSensitiveSystemPath(null)).toBe(false);
      expect(PathSecurity.isSensitiveSystemPath(undefined)).toBe(false);
      expect(PathSecurity.isSensitiveSystemPath(123)).toBe(false);
    });
  });
});
