...
describe("WhisperAPIRecognizer", () => {
  ...
  describe("recognize()", () => {
    ...
    it.skip("should throw error if file exceeds 25MB", async () => {
      ...
      await expect(recognizer.recognize("/large.wav")).rejects.toThrow(
        "超过 25MB 限制",
      );
    });
  });
...
describe("WhisperLocalRecognizer", () => {
  ...
  describe("构造函数", () => {
    it("should create instance with config", () => {
      expect(recognizer.serverUrl).toBe("http://localhost:8002");
    ...
    it("should use default values", () => {
      ...
      expect(defaultRecognizer.serverUrl).toBe("http://localhost:8002");
      expect(defaultRecognizer.modelSize).toBe("base");
      expect(defaultRecognizer.device).toBe("auto");
    });
  });
  ...
  describe("recognize()", () => {
    ...
    it.skip("should call Whisper server when file exists", async () => {
      // SKIP: CommonJS require() 限制导致 fs/axios mock 无法生效
      // 源代码使用 require('fs') 和 require('axios')，vitest 的 vi.mock() 主要支持 ES 模块
      mockFs.promises.access.mockResolvedValue(undefined);
      mockFs.promises.stat.mockResolvedValue({ size: 1024 * 100 });
  ...
  describe("isAvailable()", () => {
    ...
    it.skip("should return false when server returns non-200 status", async () => {
      // SKIP: CommonJS require() 限制导致 axios mock 无法生效
  ...
  describe("createEngine()", () => {
    ...
    it("should use default engine for unknown type", () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining("未知引擎类型"),
      );
