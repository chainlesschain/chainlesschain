...
describe("compliance-ipc", () => {
  ...
  beforeEach(async () => {
    ...
    mockSoc2Compliance = {
      collectAuditLogEvidence: vi
        .fn()
        .mockResolvedValue({ id: "ev1", title: "Audit" }),
      collectAccessControlEvidence: vi
        .fn()
        .mockResolvedValue({ id: "ev2", title: "Access" }),
      collectConfigurationEvidence: vi
        .fn()
        .mockResolvedValue({ id: "ev3", title: "Config" }),
      generateReport: vi
        .fn()
        .mockResolvedValue({ title: "Report", complianceScore: 80 }),
      verifyEvidence: vi
        .fn()
        .mockResolvedValue({ success: true, status: "verified" }),
    ...
    mockDataClassifier = {
      classify: vi
        .fn()
        .mockResolvedValue({ category: "pii", severity: "high" }),
    ...
    mockClassificationPolicy = {
      autoTag: vi
        .fn()
        .mockResolvedValue({ contentId: "c1", level: "confidential" }),
  ...
  it("should call soc2Compliance.collectAuditLogEvidence for collect-audit-evidence", async () => {
    ...
    expect(mockSoc2Compliance.collectAuditLogEvidence).toHaveBeenCalledWith({
      periodStart: 100,
    });
  });
  ...
  it("should return error when soc2Compliance is null for collect-audit-evidence", async () => {
    ...
    const handler = localIpc.handle.mock.calls.find(
      (c) => c[0] === "compliance:collect-audit-evidence",
    )[1];
  ...
  it("should call dataClassifier.classify for classify-content", async () => {
    const result = await handlers["compliance:classify-content"](
      {},
      { content: "test@example.com", options: {} },
    );
    expect(result.success).toBe(true);
    expect(result.result.category).toBe("pii");
    expect(mockDataClassifier.classify).toHaveBeenCalledWith(
      "test@example.com",
      {},
    );
  });
  ...
  it("should return error when dataClassifier is null", async () => {
    ...
    const handler = localIpc.handle.mock.calls.find(
      (c) => c[0] === "compliance:classify-content",
    )[1];
  ...
  it("should call classificationPolicy.checkAccess for check-access", async () => {
    const result = await handlers["compliance:check-access"](
      {},
      { requiredLevel: "confidential", userClearance: "top_secret" },
    );
    expect(result.success).toBe(true);
    expect(result.allowed).toBe(true);
    expect(mockClassificationPolicy.checkAccess).toHaveBeenCalledWith(
      "confidential",
      "top_secret",
    );
  });
  ...
  it("should call dataClassifier.batchScan for batch-scan", async () => {
    const result = await handlers["compliance:batch-scan"](
      {},
      { items: [{ id: "1", content: "test" }] },
    );
    expect(result.success).toBe(true);
    expect(mockDataClassifier.batchScan).toHaveBeenCalledWith([
      { id: "1", content: "test" },
    ]);
  });
  ...
  it("should call classificationPolicy.autoTag for auto-tag", async () => {
    ...
    const result = await handlers["compliance:auto-tag"](
      {},
      { contentId: "c1", classificationResult: classResult },
    );
    expect(result.success).toBe(true);
  ...
  it("should return error when classificationPolicy is null for check-access", async () => {
    ...
    const handler = localIpc.handle.mock.calls.find(
      (c) => c[0] === "compliance:check-access",
    )[1];
    const result = await handler(
      {},
      { requiredLevel: "top_secret", userClearance: "public" },
    );
    expect(result.success).toBe(false);
    expect(result.error).toContain("not initialized");
  });
