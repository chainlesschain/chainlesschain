...
const createMockProcessor = () => {
  ...
  processor.segmentAudio = vi.fn().mockResolvedValue(['/audio1.wav', '/audio2.wav']);
  processor.convertToWhisperFormat = vi.fn().mockResolvedValue({ outputPath: '/converted.wav' });
  processor.cleanupTempFiles = vi.fn().mockResolvedValue(true);
  processor.denoiseAudio = vi.fn().mockResolvedValue({ success: true, outputPath: '/denoised.wav' });
