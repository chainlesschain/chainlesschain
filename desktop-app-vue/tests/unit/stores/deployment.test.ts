...
import {
  useDeploymentStore,
  resetDeploymentEventListenersBoundForTest,
} from "../../../src/renderer/stores/deployment";
...
describe("Deployment Store", () => {
  ...
  beforeEach(() => {
    ...
    // Reset the module-level bind-once guard so each test re-binds its mock.
    resetDeploymentEventListenersBoundForTest();
    mockInvoke = vi.fn();
  ...
  describe("createPipeline()", () => {
    it("should create pipeline and refresh list", async () => {
      mockInvoke.mockImplementation((channel: string) => {
        if (channel === "dev-pipeline:create")
          return { success: true, data: { id: "p1" } };
        if (channel === "dev-pipeline:get-all")
          return { success: true, data: [{ id: "p1" }] };
        return { success: true };
      });
      ...
      expect(mockInvoke).toHaveBeenCalledWith("dev-pipeline:create", {
        template: "feature",
      });
  ...
  describe("startPipeline()", () => {
    it("should start and update status", async () => {
      mockInvoke.mockImplementation((channel: string) => {
        ...
        if (channel === "dev-pipeline:get-status")
          return { success: true, data: { id: "p1", status: "running" } };
        return { success: true };
      });
  ...
  describe("configure()", () => {
    it("should update config", async () => {
      ...
      expect(result.success).toBe(true);
      expect(mockInvoke).toHaveBeenCalledWith("dev-pipeline:configure", {
        parallelLimit: 5,
      });
    });
  });
  ...
  describe("initEventListeners()", () => {
    it("should register event listeners", () => {
      ...
      expect(mockOn).toHaveBeenCalledWith(
        "dev-pipeline:stage-updated",
        expect.any(Function),
      );
      expect(mockOn).toHaveBeenCalledWith(
        "dev-pipeline:gate-pending",
        expect.any(Function),
      );
    });
    ...
    it("should update stage on stage-updated event", () => {
      ...
      const mockOn = vi.fn((event, handler) => {
        if (event === "dev-pipeline:stage-updated")
          stageUpdatedHandler = handler;
      });
      (window as any).electronAPI = { on: mockOn };
