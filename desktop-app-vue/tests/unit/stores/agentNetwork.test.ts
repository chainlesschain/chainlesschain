...
import {
  useAgentNetworkStore,
  resetEventListenersBoundForTest,
} from "../../../src/renderer/stores/agentNetwork";
...
describe("AgentNetwork Store", () => {
  ...
  beforeEach(() => {
    ...
    // The store binds IPC listeners once via a module-level guard; reset it so
    // each test re-binds on its fresh electronAPI mock.
    resetEventListenersBoundForTest();
    mockInvoke = vi.fn();
  ...
  describe("agent discovery", () => {
    ...
    it("registerAgent should register to federation", async () => {
      ...
      expect(result.success).toBe(true);
      expect(mockInvoke).toHaveBeenCalledWith("fed-registry:register", {
        skills: ["test"],
      });
    });
    ...
    it("querySkills should query by skill", async () => {
      ...
      expect(mockInvoke).toHaveBeenCalledWith("fed-registry:query-skills", {
        skill: "code-review",
      });
    });
  ...
  describe("cross-org tasks", () => {
    it("routeTask should route and refresh log", async () => {
      mockInvoke.mockImplementation((channel) => {
        if (channel === "cross-org:route-task")
          return { success: true, data: { id: "t1" } };
        if (channel === "cross-org:get-log")
          return { success: true, data: [{ id: "t1" }] };
        return { success: true };
      });
      ...
      expect(mockInvoke).toHaveBeenCalledWith("cross-org:route-task", {
        type: "test",
        targetAgent: "a1",
      });
    });
  ...
  describe("reputation system", () => {
    ...
    it("getRanking should load ranking", async () => {
      const ranking = [
        { did: "d1", score: 0.95, rank: 1 },
        { did: "d2", score: 0.88, rank: 2 },
      ];
  ...
  describe("initEventListeners()", () => {
    ...
    it("should add new agent on agent-discovered event", () => {
      ...
      const mockOn = vi.fn((event, handler) => {
        if (event === "agent-network:agent-discovered")
    ...
    it("should not duplicate agent on agent-discovered event", () => {
      ...
      const store = useAgentNetworkStore();
      store.discoveredAgents = [
        { did: "did:cc:existing", name: "Existing" } as any,
      ];
      store.initEventListeners();
    ...
    it("should update task on task-updated event", () => {
      ...
      const mockOn = vi.fn((event, handler) => {
        if (event === "agent-network:task-updated")
