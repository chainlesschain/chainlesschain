...
// Shared mock for IPC — hoisted so vi.mock factories (which are themselves hoisted)
// can reference it. Lets the store module and the tests share state: the store's
// createRetryableIPC(...) returns this same object, and tests call
// mockIpcRenderer.invoke.mockResolvedValue(...) to drive behavior.
...
vi.mock("@renderer/utils/ipc", () => ({
  createRetryableIPC: vi.fn(() => mockIpcRenderer),
...
describe("Social Store", () => {
  ...
  describe("好友管理", () => {
    ...
    it("加载好友失败时应该处理错误", async () => {
      ...
      // Renderer logger wraps messages with timestamp/level prefix, so match on substring.
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining("加载好友列表失败"),
      );
      expect(store.friends).toEqual([]);
  ...
  describe("在线状态监听器", () => {
    ...
    it("应该处理好友上线事件", () => {
      ...
      const friend = store.friends.find(
        (f) => f.friend_did === "did:example:123",
      );
  ...
  describe("Getters", () => {
    ...
    it("应该正确计算在线好友列表", () => {
      ...
      expect(
        onlineFriends.every(
          (f) => store.onlineStatus.get(f.friend_did) === "online",
        ),
      ).toBe(true);
    });
  ...
  describe("错误处理", () => {
    it("加载通知失败时应该静默处理中断错误", async () => {
      ...
      // logger.info → console.info (not console.log)
      const consoleSpy = vi.spyOn(console, "info").mockImplementation(() => {});

      await store.loadNotifications();

      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining("通知加载被中断"),
      );
      expect(store.notifications).toEqual([]);
      expect(store.unreadNotifications).toBe(0);
    ...
    it("加载通知失败时应该处理未注册处理器错误", async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining("IPC处理器未注册"),
      );
      expect(store.notifications).toEqual([]);
