...
describe('SCIMServer', () => {
  ...
  describe('listGroups() — corrupt-row resilience', () => {
    it('tolerates a bad members JSON row (one bad row must not fail the whole list)', async () => {
      const mockRows = [
        { id: 'g1', display_name: 'Good', members: '["u1","u2"]', created_at: 1700000000000 },
        { id: 'g2', display_name: 'Corrupt', members: '{not json', created_at: 1700000000001 },
      ];
      mockDb.db.prepare = vi.fn((sql) => {
        if (sql.includes('COUNT')) return { get: vi.fn(() => ({ count: 2 })) };
        return { all: vi.fn(() => mockRows), get: vi.fn(() => null), run: vi.fn() };
      });
