...
describe("SCIMServer", () => {
  ...
  describe("createUser()", () => {
    ...
    it("should handle emails array", async () => {
      const userData = {
        userName: "bob",
        emails: [{ value: "bob@test.com", primary: true }],
      };
      const result = await server.createUser(userData);
  ...
  describe("listUsers()", () => {
    ...
    it("should support userName eq filter", async () => {
      ...
      const hasFilter = prepareCalls.some((c) =>
        c[0].includes("user_name = ?"),
      );
      expect(hasFilter).toBe(true);
    });
  });

  describe("listGroups() — corrupt-row resilience", () => {
    it("tolerates a bad members JSON row (one bad row must not fail the whole list)", async () => {
      const mockRows = [
        {
          id: "g1",
          display_name: "Good",
      ...
      // Before the safeParse fix the corrupt row threw and the whole list failed.
      const result = await server.listGroups({ startIndex: 1, count: 10 });
      expect(result.Resources).toHaveLength(2);
      expect(result.Resources[0].members).toEqual(["u1", "u2"]);
      expect(result.Resources[1].members).toEqual([]); // corrupt → safe fallback
    });
  });
  ...
  describe("updateUser()", () => {
    it("should update DB and return updated user", async () => {
      ...
      const result = await server.updateUser("u1", {
  ...
  describe("createGroup()", () => {
    it("should create group and return SCIM Group response", async () => {
      const result = await server.createGroup({
  ...
  describe("exported constants", () => {
    it("SCIM_SCHEMAS should have USER, GROUP, LIST_RESPONSE, PATCH_OP, ERROR", () => {
      expect(SCIM_SCHEMAS.USER).toBe(
        "urn:ietf:params:scim:schemas:core:2.0:User",
      );
      expect(SCIM_SCHEMAS.GROUP).toBe(
        "urn:ietf:params:scim:schemas:core:2.0:Group",
      );
      expect(SCIM_SCHEMAS.LIST_RESPONSE).toBe(
        "urn:ietf:params:scim:api:messages:2.0:ListResponse",
      );
      expect(SCIM_SCHEMAS.PATCH_OP).toBe(
        "urn:ietf:params:scim:api:messages:2.0:PatchOp",
      );
      expect(SCIM_SCHEMAS.ERROR).toBe(
