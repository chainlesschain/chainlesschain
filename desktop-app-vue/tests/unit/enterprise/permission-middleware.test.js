...
describe("PermissionMiddleware Unit Tests", () => {
  ...
  beforeEach(async () => {
    ...
    testDbPath = path.join(
      __dirname,
      `../temp/test-permission-middleware-${uniqueSuffix}.db`,
    );
  ...
  describe("Permission Checking Middleware", () => {
    ...
    beforeEach(async () => {
      ...
      db.prepare(
        `DELETE FROM organization_members WHERE org_id = 'org_123'`,
      ).run();
      db.prepare(
        `DELETE FROM organization_info WHERE org_id = 'org_123'`,
      ).run();
      db.prepare(
        `DELETE FROM permission_audit_log WHERE org_id = 'org_123'`,
      ).run();
    ...
    it("should require single permission (denied)", async () => {
      ...
      await expect(checkMiddleware(mockEvent, mockArgs)).rejects.toThrow(
        "Permission denied: org.delete",
      );
    });

    it("should extract context from args (orgId, userDID)", () => {
      const context1 = middleware.extractContext({
        orgId: "org_123",
      ...
      const context2 = middleware.extractContext({
        org_id: "org_456",
      ...
      const context3 = middleware.extractContext({
        orgId: "org_789",
        did: "did:test:user3",
      });
    ...
    it("should check permission with cache hit", async () => {
      ...
      const result1 = await middleware.checkPermission(
        org.org_id,
        ownerDID,
        "org.view",
      );
      ...
      const result2 = await middleware.checkPermission(
        org.org_id,
        ownerDID,
        "org.view",
      );
  ...
  describe("Multiple Permissions (AND logic)", () => {
    ...
    beforeEach(async () => {
      // Clean up any existing test data
      db.prepare(
        `DELETE FROM organization_members WHERE org_id = 'org_456'`,
      ).run();
      db.prepare(
        `DELETE FROM organization_info WHERE org_id = 'org_456'`,
      ).run();
  ...
  describe("Any Permission (OR logic)", () => {
    ...
    beforeEach(async () => {
      // Clean up any existing test data
      db.prepare(
        `DELETE FROM organization_members WHERE org_id = 'org_789'`,
      ).run();
      db.prepare(
        `DELETE FROM organization_info WHERE org_id = 'org_789'`,
      ).run();
    ...
    it("should require any permission (first granted)", async () => {
      const checkMiddleware = middleware.requireAnyPermission([
        "org.view",
        "org.delete",
      ]);
  ...
  describe("Role-Based Checks", () => {
    ...
    beforeEach(async () => {
      // Clean up any existing test data
      db.prepare(
        `DELETE FROM organization_members WHERE org_id = 'org_role'`,
      ).run();
      db.prepare(
        `DELETE FROM organization_info WHERE org_id = 'org_role'`,
      ).run();
    ...
    it("should require member role", async () => {
      const checkMiddleware = middleware.requireRole([
        "member",
        "admin",
        "owner",
      ]);
    ...
    it("should handle multiple role requirements", async () => {
      ...
      await expect(checkMiddleware(mockEvent3, mockArgs3)).rejects.toThrow(
        "Role required",
      );
    });
  });
  ...
  describe("Permission Cache", () => {
    ...
    beforeEach(async () => {
      // Clean up any existing test data
      db.prepare(
        `DELETE FROM organization_members WHERE org_id = 'org_cache'`,
      ).run();
      db.prepare(
        `DELETE FROM organization_info WHERE org_id = 'org_cache'`,
      ).run();
    ...
    it("should skip cache when skipCache option is true", async () => {
      ...
      await middleware.checkPermission(org.org_id, ownerDID, "org.settings", {
        skipCache: true,
      });
    ...
    it("should clear cache on invalidation", () => {
      ...
      middleware.permissionCache.set(`${org.org_id}:${ownerDID}:org.view`, {
        value: true,
        expiresAt: Date.now() + 60000,
      });
      middleware.permissionCache.set(`${org.org_id}:${ownerDID}:org.edit`, {
        value: true,
        expiresAt: Date.now() + 60000,
      });
      middleware.permissionCache.set(`${org.org_id}:${memberDID}:org.view`, {
        value: true,
        expiresAt: Date.now() + 60000,
      });
      ...
      expect(
        middleware.permissionCache.has(`${org.org_id}:${ownerDID}:org.view`),
      ).toBe(false);
      expect(
        middleware.permissionCache.has(`${org.org_id}:${ownerDID}:org.edit`),
      ).toBe(false);
      expect(
        middleware.permissionCache.has(`${org.org_id}:${memberDID}:org.view`),
      ).toBe(true);
  ...
  describe("Rate Limiting", () => {
    ...
    beforeEach(async () => {
      // Clean up any existing test data
      db.prepare(
        `DELETE FROM organization_members WHERE org_id = 'org_rate'`,
      ).run();
      db.prepare(
        `DELETE FROM organization_info WHERE org_id = 'org_rate'`,
      ).run();
    ...
    it("should allow operations within rate limit", async () => {
      const limiter = middleware.rateLimit("test_operation", {
        max: 5,
        window: 60000,
      });
    ...
    it("should block operations exceeding rate limit", async () => {
      const limiter = middleware.rateLimit("block_test", {
        max: 3,
        window: 60000,
      });
    ...
    it("should apply rate limit per user", async () => {
      const limiter = middleware.rateLimit("per_user_test", {
        max: 2,
        window: 60000,
      });
      ...
      await expect(
        limiter(mockEvent, { orgId: org.org_id, userDID: ownerDID }),
      ).rejects.toThrow("Rate limit exceeded");

      // Member should still be allowed (different user)
      const result = await limiter(mockEvent, {
        orgId: org.org_id,
        userDID: memberDID,
      });
      expect(result).toBe(true);
    ...
    it("should reset rate limit window after expiration", async () => {
      const limiter = middleware.rateLimit("reset_test", {
        max: 2,
        window: 100,
      }); // 100ms window
      ...
      await expect(limiter(mockEvent, mockArgs)).rejects.toThrow(
        "Rate limit exceeded",
      );
  ...
  describe("Audit Logging", () => {
    ...
    beforeEach(async () => {
      // Clean up any existing test data
      db.prepare(
        `DELETE FROM permission_audit_log WHERE org_id = 'org_audit'`,
      ).run();
      db.prepare(
        `DELETE FROM organization_members WHERE org_id = 'org_audit'`,
      ).run();
      db.prepare(
        `DELETE FROM organization_info WHERE org_id = 'org_audit'`,
      ).run();
    ...
    it("should log permission grant", async () => {
      await middleware.logPermissionGrant(org.org_id, ownerDID, "org.view", {
        test: "context",
      });
    ...
    it("should log permission denial", async () => {
      ...
      await middleware.logPermissionDenial(
        org.org_id,
        memberDID,
        "org.delete",
    ...
    it("should include operation details in audit", async () => {
      ...
      await middleware.logPermissionGrant(
        org.org_id,
        ownerDID,
        "member.edit",
        operationContext,
  ...
  describe("Error Handling", () => {
    ...
    beforeEach(async () => {
      // Clean up any existing test data
      db.prepare(
        `DELETE FROM organization_members WHERE org_id = 'org_error'`,
      ).run();
      db.prepare(
        `DELETE FROM organization_info WHERE org_id = 'org_error'`,
      ).run();
    ...
    it("should handle permission manager errors", async () => {
      ...
      const failingMiddleware = new PermissionMiddleware(
        db,
        failingPermissionManager,
      );

      // Should handle the error gracefully
      try {
        await failingMiddleware.checkPermission(
          org.org_id,
          ownerDID,
          "org.view",
        );
  ...
  describe("Ownership Checks", () => {
    ...
    beforeEach(async () => {
      // Clean up any existing test data
      db.prepare(
        `DELETE FROM org_knowledge_folders WHERE org_id = 'org_own'`,
      ).run();
      db.prepare(
        `DELETE FROM organization_members WHERE org_id = 'org_own'`,
      ).run();
      db.prepare(
        `DELETE FROM organization_info WHERE org_id = 'org_own'`,
      ).run();
    ...
    it("should check resource ownership (folder)", async () => {
      const isOwner = await middleware.checkOwnership(
        org.org_id,
        ownerDID,
        "folder",
        "folder_123",
      ...
      const isNotOwner = await middleware.checkOwnership(
        org.org_id,
        memberDID,
        "folder",
        "folder_123",
  ...
  describe("Audit Log Retrieval", () => {
    ...
    beforeEach(async () => {
      // Clean up any existing test data
      db.prepare(
        `DELETE FROM permission_audit_log WHERE org_id = 'org_log'`,
      ).run();
      db.prepare(
        `DELETE FROM organization_members WHERE org_id = 'org_log'`,
      ).run();
      db.prepare(
        `DELETE FROM organization_info WHERE org_id = 'org_log'`,
      ).run();
      ...
      await middleware.logPermissionGrant(org.org_id, ownerDID, "org.edit", {
        action: "edit",
      });
      await middleware.logPermissionDenial(
        org.org_id,
    ...
    it("should filter audit logs by result", async () => {
      const grantedLogs = await middleware.getAuditLog(org.org_id, {
        result: "granted",
      });
      const deniedLogs = await middleware.getAuditLog(org.org_id, {
        result: "denied",
      });
