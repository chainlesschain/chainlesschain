...
describe("OrganizationManager Unit Tests", () => {
  ...
  describe("Organization Lifecycle", () => {
    ...
    it("should reject organization creation without current identity", async () => {
      ...
      const tempDb = new DatabaseManager(testPath, {
      ...
      const tempOrgManager = new OrganizationManager(
        tempDb,
        tempDIDManager,
        p2pManager,
      );
      ...
      await expect(tempOrgManager.createOrganization(orgData)).rejects.toThrow(
        "未找到当前用户身份",
      );
  ...
  describe("Organization Queries", () => {
    ...
    it("should get organization by DID", async () => {
      const result = db
        .prepare("SELECT * FROM organization_info WHERE org_did = ?")
        .get(org1.org_did);
    ...
    it("should search organizations by name", async () => {
      // 这个功能可能需要在未来添加
      const orgs = db
        .prepare("SELECT * FROM organization_info WHERE name LIKE ?")
        .all("%Query Org%");
  ...
  describe("Permission Checks", () => {
    ...
    it("should allow owner to manage organization", async () => {
      const canManage = await orgManager.checkPermission(
        org.org_id,
        ownerDID,
        "org.manage",
      );
    ...
    it("should prevent member from removing other members", async () => {
      const canRemove = await orgManager.checkPermission(
        org.org_id,
        memberDID,
        "member.remove",
      );
    ...
    it("should prevent viewer from deleting organization", async () => {
      const canDelete = await orgManager.checkPermission(
        org.org_id,
        viewerDID,
        "org.delete",
      );
