...
vi.mock("qrcode", () => ({
  default: {
    toDataURL: vi.fn(
      async (data, options) => `data:image/png;base64,mockQRCode_${data}`,
    ),
    toBuffer: vi.fn(async (data, options) =>
      Buffer.from(`mockQRCodeBuffer_${data}`),
    ),
    toString: vi.fn(
      async (data, options) => `<svg>mockQRCodeSVG_${data}</svg>`,
    ),
  },
}));
...
describe("DIDInvitationManager Unit Tests", () => {
  ...
  beforeEach(async () => {
    ...
    } = require("../../../src/main/organization/did-invitation-manager");
    DIDInvitationManager = InvitationManagerClass;
    ...
    // 初始化邀请管理器
    invitationManager = new DIDInvitationManager(
      db,
      didManager,
      p2pManager,
      orgManager,
  ...
  describe("Direct DID Invitation", () => {
    ...
    it("should send invitation with expiration time", async () => {
      ...
      expect(invitation.expiresAt - invitation.createdAt).toBeCloseTo(
        expiresIn,
        -2,
      );
    });
    ...
    it("should get invitation by ID", async () => {
      ...
      const retrieved = invitationManager.getInvitation(
  ...
  describe("Invitation Acceptance", () => {
    ...
    it("should accept invitation and join organization", async () => {
      ...
      // 验证成员已添加到组织
      const isMember = await orgManager.isMember(
        org.org_id,
        inviteeIdentity.did,
      );
    ...
    it("should reject invitation", async () => {
      ...
      await invitationManager.rejectInvitation(
        invitation.invitationId,
        "Not interested",
      );
  ...
  describe("Invitation Status Management", () => {
    ...
    it("should mark invitation as expired", async () => {
      ...
      // 验证邀请已创建但是过期的
      const beforeCleanup = invitationManager.getInvitation(
        expiredInv.invitationId,
      );
  ...
  describe("Invite Link/Code", () => {
    ...
    it("should generate QR code for invitation", async () => {
      ...
      const qrCode = await invitationManager.generateInvitationQRCode(
    ...
    it("should accept invitation via link token", async () => {
      ...
      const result = await invitationManager.acceptInvitationLink(
        link.invitationToken,
      );
    ...
    it("should accept invitation via code", async () => {
      ...
      // 验证令牌
      const linkInfo = await invitationManager.validateInvitationToken(
        link.invitationToken,
      );
    ...
    beforeEach(async () => {
      ...
      await orgManager.addMember(org.org_id, {
        ...
        permissions: JSON.stringify(
          orgManager.getDefaultPermissionsByRole("admin"),
        ),
      });
      ...
      await orgManager.addMember(org.org_id, {
        ...
        role: "member",
        permissions: JSON.stringify(
          orgManager.getDefaultPermissionsByRole("member"),
        ),
      });
      ...
      await orgManager.addMember(org.org_id, {
        ...
        role: "viewer",
        permissions: JSON.stringify(
          orgManager.getDefaultPermissionsByRole("viewer"),
        ),
      });
  ...
  describe("QR Code Generation", () => {
    ...
    it("should generate QR code in different formats", async () => {
      ...
      const pngQR = await invitationManager.generateInvitationQRCode(
        link.invitationToken,
        {
          format: "png",
        },
      ...
      // SVG格式
      const svgQR = await invitationManager.generateInvitationQRCode(
        link.invitationToken,
        {
          format: "svg",
        },
