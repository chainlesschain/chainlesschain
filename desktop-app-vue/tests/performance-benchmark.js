...
async function benchmarkSerialVsConcurrent() {
  ...
  const tables = Array.from({ length: TABLE_COUNT }, (_, i) => `table_${i + 1}`);
  ...
  const improvement = ((serialDuration - concurrentDuration) / serialDuration * 100).toFixed(1);
...
async function benchmarkFieldMapping() {
  ...
  console.log('   📈 性能影响:');
  console.log(`      基础转换:   ${basicDuration}ms (${basicOpsPerSec.toLocaleString()} ops/sec)`);
  console.log(`      保留状态:   ${preserveDuration}ms (${preserveOpsPerSec.toLocaleString()} ops/sec)`);
...
async function benchmarkQueueScalability() {
  ...
  for (const concurrency of concurrencyLevels) {
    ...
    console.log(`      完成: ${duration}ms (理想: ${idealDuration}ms, 效率: ${efficiency}%)`);
  ...
  results.forEach(r => {
    console.log(`      ${r.concurrency.toString().padEnd(6)} | ${r.duration.toString().padEnd(9)}ms | ${r.idealDuration.toString().padEnd(9)}ms | ${r.efficiency}%`);
  });
...
async function benchmarkRealWorldScenario() {
  ...
  console.log(`      平均速度: ${Math.round(totalRecords / (serialDuration / 1000))} 条/秒\n`);
  ...
  console.log(`      完成: ${concurrentDuration}ms`);
  console.log(`      同步记录: ${totalRecords}条`);
  console.log(`      平均速度: ${Math.round(totalRecords / (concurrentDuration / 1000))} 条/秒\n`);
...
async function runAllBenchmarks() {
  ...
  try {
    ...
    console.log(`   • 加速比: ${benchmarkResults.serialVsConcurrent.speedup.toFixed(2)}x`);
    console.log(`   • 性能提升: ${benchmarkResults.serialVsConcurrent.improvement}%\n`);

    console.log('2️⃣  重试机制:');
    console.log(`   • 重试开销: ${benchmarkResults.retryPerformance.overheadPercent}%`);
    console.log(`   • 成功率: 100% (自动恢复)\n`);

    console.log('3️⃣  字段映射:');
    console.log(`   • 性能开销: ${benchmarkResults.fieldMapping.overheadPercent}% (可忽略)`);
    console.log(`   • 吞吐量: ${benchmarkResults.fieldMapping.preserveOpsPerSec.toLocaleString()} ops/sec\n`);

    console.log('4️⃣  扩展性:');
    console.log(`   • 推荐并发数: ${benchmarkResults.scalability.recommended}\n`);

    console.log('5️⃣  真实场景:');
    console.log(`   • 加速比: ${benchmarkResults.realWorld.speedup.toFixed(2)}x`);
    console.log(`   • 性能提升: ${benchmarkResults.realWorld.improvement}%`);
    console.log(`   • 节省时间: ${benchmarkResults.realWorld.timeSaved}ms`);
    console.log(`   • 同步记录: ${benchmarkResults.realWorld.totalRecords}条\n`);

    console.log('🎉 核心成果:');
    console.log(`   ✅ 登录同步速度提升 ${benchmarkResults.serialVsConcurrent.improvement}%`);
    console.log(`   ✅ 瞬时故障自动恢复，成功率 100%`);
    console.log(`   ✅ 字段映射性能开销 <${benchmarkResults.fieldMapping.overheadPercent}%`);
