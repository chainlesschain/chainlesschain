...
test.describe('DID邀请完整流程', () => {
  ...
  test('管理员发送DID邀请', async ({ page }) => {
    ...
    // 等待组织创建成功
    await expect(page.locator('.ant-message-success')).toContainText('组织创建成功');
    ...
    // 输入Bob的DID
    await page.fill('[placeholder="did:chainlesschain:..."]', 'did:chainlesschain:bob');

    // 选择角色
    await page.selectOption('select[name="role"]', 'member');

    // 添加消息
    await page.fill('[data-testid="invitation-message"]', '欢迎加入Alice Team！');

    // 提交邀请
    await page.click('button:has-text("创建邀请")');

    // 验证成功消息
    await expect(page.locator('.ant-message-success')).toContainText('DID邀请已发送');

    // 验证显示了邀请信息
    await expect(page.locator('.generated-invitation')).toBeVisible();
    await expect(page.locator('.generated-invitation')).toContainText('did:chainlesschain:bob');
  });
  ...
  test('用户接受DID邀请', async ({ browser }) => {
    ...
    await alicePage.fill('[data-testid="did-input"]', 'did:chainlesschain:alice');
    ...
    await alicePage.fill('[placeholder="did:chainlesschain:..."]', 'did:chainlesschain:bob');
    ...
    // 4. Bob应该看到邀请通知
    await expect(bobPage.locator('[data-testid="invitation-bell"] .ant-badge-count')).toContainText('1');
    ...
    await expect(bobPage.locator('.invitation-card')).toContainText('Alice Team');
    ...
    await expect(bobPage.locator('[data-testid="invitation-bell"] .ant-badge-count')).not.toBeVisible();
    ...
    // 验证切换成功
    await expect(bobPage.locator('.current-identity')).toContainText('Alice Team');
  ...
  test('多个邀请管理', async ({ browser }) => {
    ...
    await charliePage.fill('[data-testid="did-input"]', 'did:chainlesschain:charlie');
    ...
    await charliePage.fill('[placeholder="did:chainlesschain:..."]', 'did:chainlesschain:bob');
    ...
    // 4. Bob应该看到2个邀请
    await expect(bobPage.locator('[data-testid="invitation-bell"] .ant-badge-count')).toContainText('2');
    ...
    await bobPage.locator('.invitation-card:has-text("Alice Team") button:has-text("接受邀请")').click();
    ...
    await bobPage.locator('.invitation-card:has-text("Charlie Corp") button:has-text("拒绝")').click();
