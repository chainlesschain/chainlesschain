...
test.describe('PQC Ecosystem Page', () => {
  let app: any;
  let window: any;
  ...
  test('should navigate to pqc-ecosystem page', async () => {
    await window.evaluate(() => {
      window.location.hash = '#/pqc-ecosystem?e2e=true';
    });
    await window.waitForSelector('body', { timeout: 10000 });
    await window.waitForTimeout(2000);
    const url = await window.evaluate(() => window.location.hash);
    expect(url).toContain('/pqc-ecosystem');
  });
  ...
  test('should display main UI elements', async () => {
    ...
    const hasUI = await window.evaluate(() => {
      ...
      return bodyText.includes('PQC') ||
             bodyText.includes('Migration') ||
             bodyText.length > 0;
    });
