...
test.describe('Anti-Censorship Page', () => {
  let app: any;
  let window: any;
  ...
  test('should navigate to anti-censorship page', async () => {
    await window.evaluate(() => {
      window.location.hash = '#/anti-censorship?e2e=true';
    });
    await window.waitForSelector('body', { timeout: 10000 });
    await window.waitForTimeout(2000);
    const url = await window.evaluate(() => window.location.hash);
    expect(url).toContain('/anti-censorship');
  });
  ...
  test('should display main UI elements', async () => {
    ...
    const hasUI = await window.evaluate(() => {
      ...
      return bodyText.includes('Tor') ||
             bodyText.includes('Censorship') ||
             bodyText.length > 0;
    });
