...
test.describe('PQC Migration Page', () => {
  let app: any;
  let window: any;
  ...
  test('should navigate to PQC migration page', async () => {
    await window.evaluate(() => {
      window.location.hash = '#/pqc-migration?e2e=true';
    });
    await window.waitForSelector('body', { timeout: 10000 });
    await window.waitForTimeout(2000);
    const url = await window.evaluate(() => window.location.hash);
    expect(url).toContain('/pqc-migration');
  });
  ...
  test('should display main UI elements', async () => {
    ...
    const hasUI = await window.evaluate(() => {
      ...
      return bodyText.includes('Post-Quantum') ||
             bodyText.includes('PQC') ||
             bodyText.includes('ML-KEM') ||
