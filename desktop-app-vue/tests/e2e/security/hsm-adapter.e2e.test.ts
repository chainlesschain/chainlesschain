...
test.describe('HSM Adapter Page', () => {
  let app: any;
  let window: any;
  ...
  test('should navigate to hsm-adapter page', async () => {
    await window.evaluate(() => {
      window.location.hash = '#/hsm-adapter?e2e=true';
    });
    await window.waitForSelector('body', { timeout: 10000 });
    await window.waitForTimeout(2000);
    const url = await window.evaluate(() => window.location.hash);
    expect(url).toContain('/hsm-adapter');
  });
  ...
  test('should display main UI elements', async () => {
    ...
    const hasUI = await window.evaluate(() => {
      ...
      return bodyText.includes('HSM') ||
             bodyText.includes('Adapter') ||
             bodyText.length > 0;
    });
