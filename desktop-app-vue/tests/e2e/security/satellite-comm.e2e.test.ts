...
test.describe('Satellite Communication Page', () => {
  let app: any;
  let window: any;
  ...
  test('should navigate to satellite-comm page', async () => {
    await window.evaluate(() => {
      window.location.hash = '#/satellite-comm?e2e=true';
    });
    await window.waitForSelector('body', { timeout: 10000 });
    await window.waitForTimeout(2000);
    const url = await window.evaluate(() => window.location.hash);
    expect(url).toContain('/satellite-comm');
  });
  ...
  test('should display main UI elements', async () => {
    ...
    const hasUI = await window.evaluate(() => {
      ...
      return bodyText.includes('Satellite') ||
