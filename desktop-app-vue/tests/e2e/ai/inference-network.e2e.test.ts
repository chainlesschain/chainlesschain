...
test.describe('Inference Network Page', () => {
  let app: any;
  let window: any;
  ...
  test('should navigate to inference-network page', async () => {
    await window.evaluate(() => {
      window.location.hash = '#/inference-network?e2e=true';
    });
    await window.waitForSelector('body', { timeout: 10000 });
    await window.waitForTimeout(2000);
    const url = await window.evaluate(() => window.location.hash);
    expect(url).toContain('/inference-network');
  });
  ...
  test('should display main UI elements', async () => {
    ...
    const hasUI = await window.evaluate(() => {
      ...
      return bodyText.includes('Inference') ||
