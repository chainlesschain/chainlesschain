...
test.describe('EvoMap Governance Page', () => {
  let app: any;
  let window: any;
  ...
  test('should navigate to evomap-governance page', async () => {
    await window.evaluate(() => {
      window.location.hash = '#/evomap-governance?e2e=true';
    });
    await window.waitForSelector('body', { timeout: 10000 });
    await window.waitForTimeout(2000);
    const url = await window.evaluate(() => window.location.hash);
    expect(url).toContain('/evomap-governance');
  });
  ...
  test('should display main UI elements', async () => {
    ...
    const hasUI = await window.evaluate(() => {
      ...
      return bodyText.includes('Governance') ||
             bodyText.includes('DAO') ||
             bodyText.length > 0;
    });
