...
test.describe('Federation Hardening Page', () => {
  let app: any;
  let window: any;
  ...
  test('should navigate to federation-hardening page', async () => {
    await window.evaluate(() => {
      window.location.hash = '#/federation-hardening?e2e=true';
    });
    await window.waitForSelector('body', { timeout: 10000 });
    await window.waitForTimeout(2000);
    const url = await window.evaluate(() => window.location.hash);
    expect(url).toContain('/federation-hardening');
  });
  ...
  test('should display main UI elements', async () => {
    ...
    const hasUI = await window.evaluate(() => {
      ...
      return bodyText.includes('Federation') ||
