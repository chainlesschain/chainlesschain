  ...
  test.describe("流式对话控制测试", () => {
    test("应该能够启动流式对话并接收数据", async () => {
      ...
      try {
        ...
        // 监听流式输出事件（实际事件读取自 window 全局，下方设置）
        ...
        console.log("发起流式对话...");
        const chatResult = await callIPC(
          window,
    ...
    test("应该能够在流式输出过程中暂停和恢复", async () => {
      ...
      try {
          ...
          if (ipcRenderer) {
            ...
            ipcRenderer.on(
              ...
              (_event: any, data: any) => {
                (window as any).streamEvents =
                  (window as any).streamEvents || [];
                (window as any).streamEvents.push({
                  type: "chunk",
                  data,
                  timestamp: Date.now(),
                });
        ...
        console.log(
          `✅ 流式对话完成，共接收 ${finalEvents.events.length} 个chunks`,
        );
      } finally {
        await closeElectronApp(app);
      }
    ...
    test("应该能够取消流式输出", async () => {
      ...
      try {
        ...
        // 注意：取消功能也需要IPC接口
        console.log(
          "⚠️  注意：取消功能需要添加 conversation:stream-cancel IPC接口",
        );
    ...
    test("应该能够获取流式输出的统计信息", async () => {
      ...
      try {
        ...
        if (stats) {
          ...
          console.log(
            `  平均chunk时间: ${stats.averageChunkTime?.toFixed(2)}ms`,
          );
...
test.describe("流式输出控制改进建议", () => {
  test("建议添加流式控制IPC接口", async () => {
    ...
    console.log(
      "   - 返回: { success: boolean, stats?: StreamStats, error?: string }\n",
    );
    console.log("========================================\n");
  });
});
