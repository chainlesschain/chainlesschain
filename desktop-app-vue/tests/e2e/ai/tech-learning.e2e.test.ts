...
test.describe('Tech Learning Page', () => {
  let app: any;
  let window: any;
  ...
  test('should navigate to tech-learning page', async () => {
    await window.evaluate(() => {
      window.location.hash = '#/tech-learning?e2e=true';
    });
    await window.waitForSelector('body', { timeout: 10000 });
    await window.waitForTimeout(2000);
    const url = await window.evaluate(() => window.location.hash);
    expect(url).toContain('/tech-learning');
  });
