...
test.describe('EvoMap Federation Page', () => {
  let app: any;
  let window: any;
  ...
  test('should navigate to evomap-federation page', async () => {
    await window.evaluate(() => {
      window.location.hash = '#/evomap-federation?e2e=true';
    });
    await window.waitForSelector('body', { timeout: 10000 });
    await window.waitForTimeout(2000);
    const url = await window.evaluate(() => window.location.hash);
    expect(url).toContain('/evomap-federation');
  });
  ...
  test('should display main UI elements', async () => {
    ...
    const hasUI = await window.evaluate(() => {
      ...
      return bodyText.includes('EvoMap') ||
             bodyText.includes('Federation') ||
             bodyText.length > 0;
    });
