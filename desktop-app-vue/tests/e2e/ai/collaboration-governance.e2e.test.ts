...
test.describe('Collaboration Governance Page', () => {
  let app: any;
  let window: any;
  ...
  test('should navigate to collaboration-governance page', async () => {
    await window.evaluate(() => {
      window.location.hash = '#/collaboration-governance?e2e=true';
    });
    await window.waitForSelector('body', { timeout: 10000 });
    await window.waitForTimeout(2000);
    const url = await window.evaluate(() => window.location.hash);
    expect(url).toContain('/collaboration-governance');
  });
  ...
  test('should display main UI elements', async () => {
    ...
    const hasUI = await window.evaluate(() => {
      ...
      return bodyText.includes('Collaboration') ||
             bodyText.includes('Governance') ||
             bodyText.includes('Autonomy') ||
             bodyText.length > 0;
    });
