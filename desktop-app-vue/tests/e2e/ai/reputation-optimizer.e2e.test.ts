...
test.describe('Reputation Optimizer Page', () => {
  let app: any;
  let window: any;
  ...
  test('should navigate to reputation-optimizer page', async () => {
    await window.evaluate(() => {
      window.location.hash = '#/reputation-optimizer?e2e=true';
    });
    await window.waitForSelector('body', { timeout: 10000 });
    await window.waitForTimeout(2000);
    const url = await window.evaluate(() => window.location.hash);
    expect(url).toContain('/reputation-optimizer');
  });
  ...
  test('should display main UI elements', async () => {
    ...
    const hasUI = await window.evaluate(() => {
      ...
      return bodyText.includes('Reputation') ||
             bodyText.includes('Optimizer') ||
