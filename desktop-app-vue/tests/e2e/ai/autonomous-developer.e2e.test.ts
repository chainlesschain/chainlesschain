...
test.describe('Autonomous Developer Page', () => {
  let app: any;
  let window: any;
  ...
  test('should navigate to autonomous-developer page', async () => {
    await window.evaluate(() => {
      window.location.hash = '#/autonomous-developer?e2e=true';
    });
    await window.waitForSelector('body', { timeout: 10000 });
    await window.waitForTimeout(2000);
    const url = await window.evaluate(() => window.location.hash);
    expect(url).toContain('/autonomous-developer');
  });
  ...
  test('should display main UI elements', async () => {
    ...
    const hasUI = await window.evaluate(() => {
      ...
      return bodyText.includes('Autonomous') ||
