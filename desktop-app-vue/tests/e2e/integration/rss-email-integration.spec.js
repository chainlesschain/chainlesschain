...
const testUserDataDir = path.join(
  os.tmpdir(),
  "chainlesschain-rss-email-e2e-" + Date.now(),
);
...
test.describe("RSS 和邮件集成 E2E 测试", () => {
  ...
  test.describe("RSS 订阅功能", () => {
    test("应该能够添加 RSS 订阅源", async () => {
      ...
      // 查找添加订阅按钮
      const addButton = await window
        .locator('button:has-text("添加订阅")')
        .first();
    ...
    test("应该能够刷新 RSS 订阅并验证缓存", async () => {
      // 查找刷新按钮
      const refreshButton = await window
        .locator('button:has-text("刷新")')
        .first();
    ...
    test("应该能够标记文章为已读/未读", async () => {
      // 查找标记按钮
      const markButton = await window
        .locator('button:has-text("标记")')
        .first();
    ...
    test("应该能够收藏文章", async () => {
      // 查找收藏按钮
      const starButton = await window
        .locator('button[aria-label*="收藏"]')
        .first();
      if (await starButton.isVisible()) {
        ...
        // 验证收藏状态
        const starredIndicator = await window
          .locator(".starred-indicator")
          .first();
  ...
  test.describe("邮件集成功能", () => {
    test("应该能够添加邮件账户", async () => {
      ...
      // 查找添加账户按钮
      const addAccountButton = await window
        .locator('button:has-text("添加账户")')
        .first();
      if (await addAccountButton.isVisible()) {
        ...
        const passwordInput = await window
    ...
    test("应该能够撰写新邮件", async () => {
      // 查找撰写按钮
      const composeButton = await window
        .locator('button:has-text("撰写")')
        .first();
      if (await composeButton.isVisible()) {
        ...
        // 填写邮件信息
        const toInput = await window
          .locator('input[placeholder*="收件人"]')
          .first();
        await toInput.fill("recipient@example.com");

        const subjectInput = await window
          .locator('input[placeholder*="主题"]')
          .first();
        await subjectInput.fill("测试邮件");

        const contentInput = await window
          .locator('textarea[placeholder*="内容"]')
          .first();
  ...
  test.describe("性能优化验证", () => {
    ...
    test("应该验证批量获取的并发控制", async () => {
      ...
      // 测试批量刷新
      const refreshAllButton = await window
        .locator('button:has-text("全部刷新")')
        .first();
  ...
  test.describe("数据清理功能", () => {
    test("应该能够手动触发数据清理", async () => {
      ...
      if (await settingsButton.isVisible()) {
        ...
        // 查找数据清理按钮
        const cleanupButton = await window
          .locator('button:has-text("清理数据")')
          .first();
    ...
    test("应该显示数据统计信息", async () => {
      ...
      if (await statsSection.isVisible()) {
        ...
        expect(
          (await rssCount.isVisible()) || (await emailCount.isVisible()),
        ).toBeTruthy();
      }
    });
  });
