...
const testUserDataDir = path.join(
  os.tmpdir(),
  "chainlesschain-e2e-test-" + Date.now(),
);
...
test.describe("全局设置向导 E2E 测试", () => {
  ...
  test("应完成6步向导流程", async () => {
    ...
    // 步骤2: 项目路径
    await window.waitForSelector('input[placeholder*="项目文件"]', {
      timeout: 5000,
    });
    ...
    // 步骤3: 数据库路径
    await window.waitForSelector('input[placeholder*="数据库"]', {
      timeout: 5000,
    });
...
test.describe("企业版服务器连接测试 E2E", () => {
  ...
  test.beforeAll(async () => {
    ...
    const tempDir = path.join(
      os.tmpdir(),
      "chainlesschain-ent-test-" + Date.now(),
    );
  ...
  test("应显示企业版连接测试按钮", async () => {
    ...
    // 等待企业版配置表单出现
    await window.waitForSelector('input[placeholder*="服务器地址"]', {
      timeout: 5000,
    });

    // 验证测试按钮存在
    await expect(
      window.locator('button:has-text("测试服务器连接")'),
    ).toBeVisible();
  });
...
test.describe("配置导入导出 E2E", () => {
  ...
  const tempDir = path.join(
    os.tmpdir(),
    "chainlesschain-import-test-" + Date.now(),
  );
...
test.describe("路径选择器 E2E", () => {
  ...
  test.beforeAll(async () => {
    const tempDir = path.join(
      os.tmpdir(),
      "chainlesschain-path-test-" + Date.now(),
    );
    if (!fs.existsSync(tempDir)) {
      fs.mkdirSync(tempDir, { recursive: true });
    }
