...
test.describe('Decentralized Storage Page', () => {
  let app: any;
  let window: any;
  ...
  test('should navigate to decentralized-storage page', async () => {
    await window.evaluate(() => {
      window.location.hash = '#/decentralized-storage?e2e=true';
    });
    await window.waitForSelector('body', { timeout: 10000 });
    await window.waitForTimeout(2000);
    const url = await window.evaluate(() => window.location.hash);
    expect(url).toContain('/decentralized-storage');
  });
  ...
  test('should display main UI elements', async () => {
    ...
    const hasUI = await window.evaluate(() => {
      ...
      return bodyText.includes('Storage') ||
             bodyText.includes('Filecoin') ||
             bodyText.length > 0;
    });
