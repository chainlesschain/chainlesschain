...
test.describe('AI Social Enhancement Page', () => {
  let app: any;
  let window: any;
  ...
  test('should navigate to ai-social-enhancement page', async () => {
    await window.evaluate(() => {
      window.location.hash = '#/ai-social-enhancement?e2e=true';
    });
    await window.waitForSelector('body', { timeout: 10000 });
    await window.waitForTimeout(2000);
    const url = await window.evaluate(() => window.location.hash);
    expect(url).toContain('/ai-social-enhancement');
  });
  ...
  test('should display main UI elements', async () => {
    ...
    const hasUI = await window.evaluate(() => {
      ...
      return bodyText.includes('Social') ||
