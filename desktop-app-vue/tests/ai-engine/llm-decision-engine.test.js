...
const {
  LLMDecisionEngine,
  DecisionStrategy,
} = require("../../src/main/ai-engine/llm-decision-engine.js");
...
describe("LLMDecisionEngine", () => {
  ...
  describe("初始化", () => {
    ...
    it("应该使用默认配置", () => {
      engine = new LLMDecisionEngine();
      assert.strictEqual(
        engine.config.contextLengthThreshold,
        10000,
        "默认上下文阈值应为10000",
      );
      assert.strictEqual(
        engine.config.subtaskCountThreshold,
        3,
        "默认子任务阈值应为3",
      );
    });

    it("应该保留显式 llmTemperature=0（确定性输出，不被默认 0.3 覆盖）", () => {
      // 旧实现 `0 || 0.3` 会把确定性温度悄悄改成 0.3
      engine = new LLMDecisionEngine({ llmTemperature: 0 });
      assert.strictEqual(
        engine.config.llmTemperature,
        0,
        "显式 0 温度应被保留",
      );
    });

    it("llmTemperature 缺省时仍回退到 0.3", () => {
      engine = new LLMDecisionEngine();
      assert.strictEqual(engine.config.llmTemperature, 0.3, "缺省温度应为 0.3");
    });
  });
  ...
  describe("基础规则决策", () => {
    ...
    it("应该对简单任务推荐单代理", async () => {
      const task = {
        ...
        subtasks: [{ title: "Step 1", tool: "bash" }],
    ...
    it("应该检测可并行任务", async () => {
      ...
      assert.strictEqual(
        decision.strategy,
        DecisionStrategy.PARALLEL_EXECUTION,
        "策略应为并行执行",
      );
    });
    ...
    it("应该检测需要专业化的任务", async () => {
      ...
      assert.strictEqual(
        decision.strategy,
        DecisionStrategy.SPECIALIZED_AGENTS,
        "策略应为专业化代理",
      );
    });
    ...
    it("应该检测上下文过长的任务", async () => {
      ...
      assert.strictEqual(
        decision.strategy,
        DecisionStrategy.DIVIDE_CONTEXT,
        "策略应为分割上下文",
      );
    });
  });
  ...
  describe("历史学习", () => {
    ...
    it("应该基于历史数据调整决策", async () => {
      ...
      // 历史数据应该影响决策
      assert.ok(
        decision.reason.includes("历史") || decision.useMultiAgent,
        "应该考虑历史数据",
      );
    });
    ...
    it("应该在历史数据显示单代理更稳定时调整", async () => {
      // 历史数据显示单代理成功率更高
      mockDatabase.all = async () => [
        { use_multi_agent: 1, avg_time: 5000, avg_success: 0.8, count: 10 },
    ...
    it("应该在历史数据不足时不调整", async () => {
      // 历史数据样本太少
      mockDatabase.all = async () => [
        { use_multi_agent: 1, avg_time: 5000, avg_success: 0.9, count: 2 },
  ...
  describe("决策缓存", () => {
    ...
    it("应该缓存相同任务的决策", async () => {
      ...
      assert.strictEqual(
        decision1.useMultiAgent,
        decision2.useMultiAgent,
        "决策应该一致",
      );
      assert.strictEqual(
        decision1.strategy,
  ...
  describe("禁用模式", () => {
    ...
    it("禁用时应该总是返回单代理", async () => {
      ...
      const decision = await engine.shouldUseMultiAgent(task, {
        length: 20000,
      });
  ...
  describe("执行结果记录", () => {
    ...
    it("应该记录任务执行结果", async () => {
      ...
      mockDatabase.run = async (query, params) => {
        recordCalled = true;
        assert.ok(
          query.includes("task_execution_history"),
          "应该插入执行历史表",
        );
