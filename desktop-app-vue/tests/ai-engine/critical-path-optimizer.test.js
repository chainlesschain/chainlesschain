...
describe('CriticalPathOptimizer', () => {
  ...
  describe('简单线性依赖链', () => {
    ...
    it('应该识别线性关键路径', () => {
      ...
      assert.ok(parseFloat(stats.avgCriticalPathLength) > 0, '应该找到关键路径');
    });
  });
  ...
  describe('并行任务', () => {
    ...
    it('应该正确处理并行任务', () => {
      const tasks = [
        ...
        { id: 'task4', duration: 1000, dependencies: ['task1', 'task2', 'task3'] },
      ];
  ...
  describe('复杂依赖图', () => {
    ...
    it('应该识别钻石依赖的关键路径', () => {
      ...
      // right应该有更高的优先级
      assert.ok(rightIndex <= leftIndex, 'right应该在left之前或同时（关键路径上）');
    });
  });
  ...
  describe('拓扑排序', () => {
    ...
    it('应该正确执行拓扑排序', () => {
      ...
      // 优化后的顺序应该满足依赖关系
      const task1Index = optimized.findIndex(t => t.id === 'task1');
      const task2Index = optimized.findIndex(t => t.id === 'task2');
      const task3Index = optimized.findIndex(t => t.id === 'task3');

      assert.ok(task1Index < task2Index, 'task1应该在task2之前');
      assert.ok(task2Index < task3Index, 'task2应该在task3之前');
    });
  });
  ...
  describe('实际工作流场景', () => {
    ...
    it('应该优化典型的软件构建流程', () => {
      ...
      const stats = optimizer.getStats();
      assert.ok(parseFloat(stats.avgCriticalPathLength) >= 3, '关键路径长度应该>=3');
    });
  });
});
