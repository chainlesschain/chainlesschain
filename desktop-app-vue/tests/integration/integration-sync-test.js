...
async function testBackendConnection() {
  ...
  try {
    ...
    logTest(
      "连接后端服务",
      "passed",
      `服务器时间: ${new Date(serverTime.timestamp).toISOString()}`,
    );

    const offset = Date.now() - serverTime.timestamp;
    logTest(
      "时间偏移检查",
      offset < 5000 ? "passed" : "failed",
      `偏移: ${offset}ms`,
    );
...
async function testLocalOperations(database) {
  ...
  try {
    ...
    if (deleted) {
      const softDeleted = database.db
        .prepare("SELECT deleted, sync_status FROM projects WHERE id = ?")
        .get(projectId);
    ...
    if (restored) {
      const restoredRecord = database.db
        .prepare("SELECT deleted, sync_status FROM projects WHERE id = ?")
        .get(projectId);
  ...
  try {
    ...
    if (syncManager.syncQueue) {
      logTest(
        "并发队列初始化",
        "passed",
        `最大并发: ${syncManager.syncQueue.maxConcurrency}`,
      );
    } else {
    ...
    if (syncManager.retryPolicy) {
      logTest(
        "重试策略初始化",
        "passed",
        `最大重试: ${syncManager.retryPolicy.maxRetries}`,
      );
    } else {
...
async function testSyncScenarios(syncManager, projectId) {
  ...
  try {
    ...
    const uploadResult = await syncManager
      .uploadLocalChanges("projects")
      .catch((err) => {
    ...
    const downloadResult = await syncManager
      .downloadRemoteChanges("projects")
      .catch((err) => {
    ...
    if (downloadResult.conflicts) {
      logTest(
        "下载远程变更",
        "passed",
        `冲突${downloadResult.conflicts.length}个`,
      );
    } else {
    ...
    const incrementalResult = await syncManager
      .syncIncremental()
      .catch((err) => {
...
async function runIntegrationTests() {
  ...
  // 测试4: 同步管理器
  const syncManager = backendAvailable
    ? await testSyncManagerInit(database)
    : null;
  ...
  if (total > 0) {
    console.log(
      `   📈 通过率: ${((results.passed.length / total) * 100).toFixed(1)}%`,
    );
  }
  ...
  if (backendAvailable) {
    ...
    console.log(
      "   启动命令: cd backend/project-service && mvn spring-boot:run\n",
    );
  }
