...
async function testSpeechManagerIntegration() {
  ...
  try {
    ...
    if (
      mainIndexContent.includes("// 初始化语音管理器") &&
  ...
  try {
    const fs = require("fs");
    const ipcRegistryPath = path.join(
      __dirname,
    ...
    if (
      ipcRegistryContent.includes("app.initializeSpeechManager") &&
  ...
  try {
    ...
    if (
      preloadContent.includes("speech:") &&
  ...
  console.log(
    `成功率: ${((passedTests / (passedTests + failedTests)) * 100).toFixed(1)}%`,
  );
  console.log("=".repeat(60));
  console.log();
