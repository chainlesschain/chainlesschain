...
const StatsCleaner = require('../../src/main/skill-tool-system/stats-cleaner');
...
describe('技能工具系统集成测试', () => {
  ...
  describe('技能-工具关联流程', () => {
    ...
    it('应该支持一个技能关联多个工具', async () => {
      ...
      for (let i = 1; i <= 3; i++) {
        ...
        await skillManager.addToolToSkill(skill.id, tool.id, i === 1 ? 'primary' : 'secondary');
      }
  ...
  describe('统计数据清理', () => {
    it('应该清理过期的使用日志', async () => {
      ...
      const remaining = db.prepare('SELECT COUNT(*) as count FROM skill_tool_usage_logs').get();
  ...
  describe('插件扩展集成', () => {
    it('应该能够通过插件注册技能和工具', async () => {
      ...
      // 验证
      const skill = await skillManager.getSkillById(`${pluginId}_plugin_skill_1`);
      expect(skill).toBeDefined();
