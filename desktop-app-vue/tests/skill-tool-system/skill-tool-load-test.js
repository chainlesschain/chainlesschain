...
async function testSkillToolSystem() {
  ...
  try {
    ...
    newTools.forEach((toolName) => {
      ...
      if (found) {
        console.log(
          `   ✅ ${found.display_name} (${toolName}) - ${found.category}`,
        );
      } else {
    ...
    try {
      ...
      console.log(
        `   ✅ FunctionCaller 创建成功，注册了 ${caller.tools.size} 个工具`,
      );
    ...
    builtinTools.forEach((tool) => {
      toolCategoryCount[tool.category] =
        (toolCategoryCount[tool.category] || 0) + 1;
    });
