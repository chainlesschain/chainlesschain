...
describe('RemoteDesktopHandler', () => {
  ...
  describe('stopSession', () => {
    it('应该成功停止远程桌面会话', async () => {
      ...
      // 停止会话
      const result = await handler.stopSession({ sessionId }, { did: 'did:key:test123' });
    ...
    it('应该拒绝不存在的会话', async () => {
      await expect(
        handler.stopSession({ sessionId: 'invalid-session' }, { did: 'did:key:test123' })
  ...
  describe('getFrame', () => {
    it('应该成功捕获屏幕帧', async () => {
      ...
      // 获取屏幕帧
      const result = await handler.getFrame({ sessionId }, { did: 'did:key:test123' });
    ...
    it('应该限制帧率', async () => {
      // 创建会话（30 FPS = 33ms 间隔）
      const startResult = await handler.startSession({ maxFps: 30 }, { did: 'did:key:test123' });
      const sessionId = startResult.sessionId;
    ...
    it('应该拒绝不存在的会话', async () => {
      await expect(
        handler.getFrame({ sessionId: 'invalid-session' }, { did: 'did:key:test123' })
      ).rejects.toThrow(/Session not found/);
    });
  });
