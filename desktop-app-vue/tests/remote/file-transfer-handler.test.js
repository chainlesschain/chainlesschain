...
describe('FileTransferHandler', () => {
  ...
  describe('requestUpload', () => {
    ...
    it('应该拒绝超过大小限制的文件', async () => {
      ...
      await expect(handler.requestUpload(params, context)).rejects.toThrow(/exceeds maximum allowed size/);
    });
    ...
    it('应该拒绝无效的文件名', async () => {
      ...
      await expect(handler.requestUpload(params, context)).rejects.toThrow(/fileName.*required/);
    });
  });
  ...
  describe('uploadChunk', () => {
    ...
    it('应该拒绝不存在的传输', async () => {
      ...
      await expect(handler.uploadChunk(params, context)).rejects.toThrow(/Transfer not found/);
    });
  ...
  describe('requestDownload', () => {
    ...
    it('应该拒绝目录外的文件路径', async () => {
      ...
      await expect(handler.requestDownload(params, context)).rejects.toThrow(/Access denied.*outside allowed/);
    });
  });
  ...
  describe('cleanupExpiredTransfers', () => {
    it('应该清理过期的传输任务', async () => {
      ...
      // 清理过期传输（超过24小时）
      const cleaned = await handler.cleanupExpiredTransfers(24 * 60 * 60 * 1000);
