...
const AICommandHandlerEnhanced = require('../../src/main/remote/handlers/ai-handler-enhanced');
...
describe('AICommandHandlerEnhanced', () => {
  ...
  beforeEach(() => {
    ...
    mockDatabase = {
      ...
      getConversationMessages: vi.fn(() => [
        { role: 'user', content: 'Previous message', created_at: Date.now() - 1000 },
        { role: 'assistant', content: 'Previous response', created_at: Date.now() - 500 }
      ]),
  ...
  describe('chat', () => {
    ...
    it('should use existing conversation if conversationId provided', async () => {
      ...
      expect(mockDatabase.getConversation).toHaveBeenCalledWith('conv-existing');
    ...
    it('should include conversation history in messages', async () => {
      ...
      expect(mockDatabase.getConversationMessages).toHaveBeenCalledWith('conv-with-history', 10);
      expect(mockLLMManager.chat).toHaveBeenCalled();
    ...
    it('should reject message that is too long', async () => {
      ...
      await expect(handler.chat(params, mockContext)).rejects.toThrow('Message too long');
    });

    it('should reject missing message parameter', async () => {
      const params = {};

      await expect(handler.chat(params, mockContext)).rejects.toThrow('Parameter "message" is required');
    });
    ...
    it('should handle LLM service unavailable', async () => {
      ...
      await expect(handler.chat(params, mockContext)).rejects.toThrow('LLM service not available');
    });
  ...
  describe('getConversations', () => {
    ...
    it('should handle database unavailable', async () => {
      ...
      await expect(handler.getConversations({}, mockContext)).rejects.toThrow('Database not available');
    });

    it('should calculate hasMore correctly', async () => {
      const result1 = await handler.getConversations({ limit: 5, offset: 0 }, mockContext);
      expect(result1.hasMore).toBe(true); // 0 + 5 < 10

      const result2 = await handler.getConversations({ limit: 5, offset: 5 }, mockContext);
  ...
  describe('ragSearch', () => {
    ...
    it('should reject missing query parameter', async () => {
      ...
      await expect(handler.ragSearch(params, mockContext)).rejects.toThrow('Parameter "query" is required');
    });
    ...
    it('should handle RAG service unavailable', async () => {
      ...
      await expect(handler.ragSearch(params, mockContext)).rejects.toThrow('RAG service not available');
    });
    ...
    it('should support custom hybrid search and reranker options', async () => {
      ...
      expect(mockRAGManager.search).toHaveBeenCalledWith('test query', expect.objectContaining({
        useHybridSearch: false,
        useReranker: false
      }));
    });
  ...
  describe('controlAgent', () => {
    ...
    it('should reject invalid action', async () => {
      ...
      await expect(handler.controlAgent(params, mockContext)).rejects.toThrow('Parameter "action" must be one of');
    });
    ...
    it('should reject missing agentId for non-list actions', async () => {
      ...
      await expect(handler.controlAgent(params, mockContext)).rejects.toThrow('Parameter "agentId" is required');
    });
  ...
  describe('getModels', () => {
    ...
    it('should handle Ollama models fetch error gracefully', async () => {
      mockLLMManager.client.listModels.mockRejectedValue(new Error('Ollama not available'));
