...
class RulesValidator {
  ...
  async checkSQLInjection() {
    ...
    for (const file of jsFiles) {
      ...
      lines.forEach((line, index) => {
        ...
        if (line.includes('db.exec(') || line.includes('database.exec(')) {
          ...
          if (!line.trim().startsWith('//') && !line.trim().startsWith('*')) {
            this.errors.push({
              ...
              line: lineNumber,
              message: '使用 db.exec() 可能导致 SQL 注入，请改用 db.prepare() 参数化查询',
              code: line.trim()
            });
            issueCount++;
          }
        ...
        const hasStringConcat = line.includes("'") && (line.includes('+') || line.includes('${'));

        if (hasSQLKeyword && (hasTemplateLiteral || hasStringConcat)) {
          // 排除安全的占位符拼接（如 placeholders = ids.map(() => '?').join(',')）
          const isSafePlaceholder = line.includes("'?'") || line.includes('"?"');
          const isComment = line.trim().startsWith('//') || line.trim().startsWith('*');

          if (!isSafePlaceholder && !isComment && !line.includes('db.prepare')) {
            this.warnings.push({
              type: 'SQL_INJECTION',
              severity: 'MEDIUM',
              file: path.relative(this.rootDir, file),
  ...
  async checkP2PEncryption() {
    ...
    for (const file of jsFiles) {
      ...
      lines.forEach((line, index) => {
        ...
        if (line.includes('.pubsub.publish(') || line.includes('.publish(')) {
          ...
          if (!hasEncryption && !line.trim().startsWith('//')) {
            this.errors.push({
              ...
              file: path.relative(this.rootDir, file),
              line: lineNumber,
              message: 'P2P 消息发布前未检测到加密操作，必须使用 Signal Protocol 加密',
              code: line.trim()
            });
            issueCount++;
          }
        ...
        if ((line.includes('toString(\'base64\')') || line.includes('.btoa(')) &&
            (line.includes('encrypt') || line.includes('secure') || line.includes('protect'))) {
          this.errors.push({
            type: 'WEAK_ENCRYPTION',
            severity: 'HIGH',
            file: path.relative(this.rootDir, file),
            line: lineNumber,
            message: 'Base64 是编码而非加密，请使用 Signal Protocol 或 node-forge',
            code: line.trim()
          });
          issueCount++;
        }
  ...
  async checkSensitiveDataLeak() {
    ...
    const sensitivePatterns = [
      { pattern: /console\.(log|info|debug|warn)\([^)]*\b(pin|PIN)\b/i, message: 'PIN 码不应记录到日志' },
      { pattern: /console\.(log|info|debug|warn)\([^)]*\b(password|pwd|passwd)\b/i, message: '密码不应记录到日志' },
      { pattern: /console\.(log|info|debug|warn)\([^)]*\b(key|secret|token)\b.*[:=]/i, message: '密钥/令牌不应记录到日志' },
      { pattern: /const\s+(pin|PIN)\s*=\s*['"`]\d+['"`]/, message: 'PIN 码不应硬编码（除非在模拟模式配置中）' },
      { pattern: /pragma.*key.*=.*['"`][^'"`]+['"`]/, message: '数据库加密密钥不应硬编码' },
    ];
      ...
      lines.forEach((line, index) => {
        ...
        sensitivePatterns.forEach(({ pattern, message }) => {
          if (pattern.test(line)) {
            ...
            const isSimulationConfig = fileName.includes('simulation') || fileName.includes('mock');
            const isDefaultPIN = line.includes('123456') && line.includes('simulation');
    ...
    try {
      ...
      if (error.stdout) {
        try {
          ...
          const total = vulnerabilities.critical + vulnerabilities.high +
