...
Readability.prototype = {
  ...
  /**
   * Reduce a NodeList to a single value using the provided function.
   *
   * For convenience, the current object context is bound to the provided
   * reducer function.
   *
   * @param  NodeList nodeList     The NodeList.
   * @param  Function fn           The reducer function(accumulator, currentNode).
   * @param  any      initialValue The initial accumulator value.
   * @return any
   */
  _reduceNodeList: function (nodeList, fn, initialValue) {
    return Array.prototype.reduce.call(nodeList, fn.bind(this), initialValue);
  },
   ...
   * Uses getElementsByTagName('*') for flat traversal instead of recursion.
   *
   * @param Element
   * @return void
   **/
  _cleanStyles: function (e) {
    ...
    // Collect root + all descendants via flat traversal
    var allElements = e.getElementsByTagName("*");

    // Process root element first, then all descendants
    for (var j = -1; j < allElements.length; j++) {
      var cur = j === -1 ? e : allElements[j];

      if (cur.tagName.toLowerCase() === "svg") continue;

      // Remove `style` and deprecated presentational attributes
      for (var i = 0; i < this.PRESENTATIONAL_ATTRIBUTES.length; i++) {
        cur.removeAttribute(this.PRESENTATIONAL_ATTRIBUTES[i]);
      }

      if (this.DEPRECATED_SIZE_ATTRIBUTE_ELEMS.indexOf(cur.tagName) !== -1) {
  ...
  _getLinkDensity: function (element) {
    ...
    var linkLength = this._reduceNodeList(
      ...
      function (sum, linkNode) {
        var href = linkNode.getAttribute("href");
        var coefficient = href && this.REGEXPS.hashUrl.test(href) ? 0.3 : 1;
        return sum + this._getInnerText(linkNode).length * coefficient;
      },
      0,
    );
  ...
  _cleanConditionally: function (e, tag) {
    ...
    this._removeNodes(this._getAllNodesWithTag(e, [tag]), function (node) {
      ...
      // Use the node's original readability contentScore if it was previously scored
      var contentScore =
        (node.readability && node.readability.contentScore) || 0;
