...
function registerFileSharingIPC(app) {
  ...
  ipcMain.handle("file:upload", async (event, { fileData }) => {
    try {
      ...
      const file = await app.fileManager.uploadFile(
        fileData,
        currentIdentity.did,
      );
  ...
  ipcMain.handle("file:delete", async (event, { fileId }) => {
    try {
      ...
      const result = await app.fileManager.deleteFile(
        fileId,
        currentIdentity.did,
      );
  ...
  ipcMain.handle("file:lock", async (event, { fileId, expiresIn }) => {
    try {
      ...
      const result = await app.fileManager.lockFile(
        fileId,
        currentIdentity.did,
        expiresIn,
      );
  ...
  ipcMain.handle("file:unlock", async (event, { fileId }) => {
    try {
      ...
      const result = await app.fileManager.unlockFile(
        fileId,
        currentIdentity.did,
      );
  ...
  ipcMain.handle(
    ...
    async (event, { fileId, version1, version2 }) => {
      try {
        if (!app.versionManager) {
          return { success: false, error: "版本管理器未初始化" };
        }

        const comparison = app.versionManager.compareVersions(
          fileId,
          version1,
          version2,
        );
