...
class BackupManager {
  ...
  async createBackup(options = {}) {
    try {
      ...
      // 要备份的文件和目录
      const itemsToBackup = [
        { path: path.join(this.userDataPath, 'chainlesschain.db'), name: 'database/chainlesschain.db' },
        { path: path.join(this.userDataPath, 'config.json'), name: 'config.json' },
        { path: path.join(this.userDataPath, 'window-states.json'), name: 'window-states.json' },
        { path: path.join(this.userDataPath, 'settings'), name: 'settings', isDir: true },
        { path: path.join(this.userDataPath, 'plugins'), name: 'plugins', isDir: true },
      ];
  ...
  createZipArchive(outputPath, items) {
    return new Promise((resolve, reject) => {
      ...
      output.on('close', () => {
        console.log(`[BackupManager] Archive created: ${archive.pointer()} bytes`);
        resolve();
      });
  ...
  async restoreBackup(backupPath, options = {}) {
    try {
      ...
      // 创建当前数据的备份（以防恢复失败）
      const safetyBackup = await this.createBackup({ name: 'safety-backup-before-restore' });
  ...
  async autoBackup() {
    try {
      const result = await this.createBackup({ name: `auto-backup-${Date.now()}` });
