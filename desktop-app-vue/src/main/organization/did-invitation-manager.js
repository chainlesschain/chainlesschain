...
class DIDInvitationManager {
  ...
  async createDIDInvitation(params) {
    ...
    try {
      ...
      const hasPermission = await this.orgManager.checkPermission(
        orgId,
        currentDID,
        "member.invite",
      );
  ...
  async handleIncomingInvitation(invitation, senderPeerId) {
    logger.info(
      `[DIDInvitationManager] 处理收到的邀请: ${invitation.invitationId}`,
    );
    ...
    try {
      ...
      try {
        const inviter = this.didManager?.getIdentityByDID?.(
          invitation.inviterDID,
        );
        inviterName =
          inviter?.nickname || inviter?.name || invitation.inviterDID.slice(-8);
      } catch (e) {
        // 忽略
      }
  ...
  getInvitation(invitationId) {
    try {
      ...
      return {
        ...invitation,
        metadata: invitation.metadata_json
          ? JSON.parse(invitation.metadata_json)
          : {},
      };
    } catch (error) {
  ...
  getInvitationStats(orgId = null) {
    try {
      ...
      const received = this.db
        .prepare(receivedQuery)
        .get(...receivedParams).count;
  ...
  async validateInvitationToken(token) {
    ...
    try {
      ...
      if (link.status !== "active") {
        throw new Error(
          `邀请链接已${link.status === "revoked" ? "撤销" : "失效"}`,
        );
      }
      ...
      // 6. 获取邀请人信息
      const inviterIdentity = await this.didManager.getIdentityByDID(
        link.inviter_did,
      );
  ...
  async acceptInvitationLink(token, options = {}) {
    ...
    try {
      ...
      // 3. 检查是否已经是成员
      const isMember = await this.orgManager.isMember(
        linkInfo.orgId,
        currentDID,
      );
      if (isMember) {
  ...
  async revokeInvitationLink(linkId) {
    ...
    try {
      ...
      const isAdmin = await this.orgManager.checkPermission(
        link.org_id,
        currentDID,
        "member.manage",
      );
  ...
  async generateInvitationQRCode(linkId, options = {}) {
    ...
    try {
      ...
      switch (format) {
        case "png":
          qrCode = await QRCode.toBuffer(invitationUrl, {
            ...qrOptions,
            type: "png",
          });
          break;
        case "svg":
          qrCode = await QRCode.toString(invitationUrl, {
            ...qrOptions,
            type: "svg",
          });
          break;
  ...
  async generateBatchInvitationQRCodes(orgId, options = {}) {
    ...
    try {
      // 1. 获取邀请链接列表
      const links = this.getInvitationLinks(orgId, {
      ...
      const qrCodes = await Promise.all(
        links.map(async (link) => {
          try {
            ...
            logger.error(
              `[DIDInvitationManager] 生成QR码失败 (${link.link_id}):`,
              error,
            );
            return null;
          }
      ...
      logger.info(
        `[DIDInvitationManager] ✓ 批量生成完成: ${validQRCodes.length}/${links.length}`,
      );
