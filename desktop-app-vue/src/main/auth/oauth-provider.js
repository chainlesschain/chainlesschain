...
const MAX_REDIRECTS = 5; // cap redirect-following to avoid infinite recursion
...
class OAuthProvider {
  ...
  constructor({ config } = {}) {
    ...
    if (!config.authorizationEndpoint) {
      throw new Error(
        "[OAuthProvider] config.authorizationEndpoint is required",
      );
    }
  ...
  getAuthorizationUrl(state, codeChallenge, options = {}) {
    ...
    if (
      ...
      typeof options.additionalParams === "object"
    ) {
    ...
    logger.debug(
      "[OAuthProvider] Authorization URL generated for state:",
      state,
    );
  ...
  validateIdToken(idToken, options = {}) {
    ...
    if (this.issuer && payload.iss) {
      ...
      if (normalizedIssuer !== normalizedPayloadIss) {
        validationErrors.push(
          `Issuer mismatch: expected ${this.issuer}, got ${payload.iss}`,
        );
      }
    }
    ...
    if (payload.aud) {
      ...
      if (!audiences.includes(this.clientId)) {
        validationErrors.push(
          `Audience mismatch: token audience ${JSON.stringify(payload.aud)} does not include client ${this.clientId}`,
        );
      }
      ...
      if (audiences.length > 1) {
        if (payload.azp && payload.azp !== this.clientId) {
          validationErrors.push(
            `Authorized party mismatch: expected ${this.clientId}, got ${payload.azp}`,
          );
        }
      }
    }
    ...
    if (payload.exp) {
      ...
      if (payload.exp + clockSkew < now) {
        validationErrors.push(
          `Token has expired: exp=${payload.exp}, now=${now}`,
        );
      }
    } else {
    ...
    if (payload.iat) {
      ...
      if (payload.iat - clockSkew > now) {
        validationErrors.push(
          `Token issued in the future: iat=${payload.iat}, now=${now}`,
        );
      }
    }
    ...
    if (options.nonce && payload.nonce !== options.nonce) {
      validationErrors.push(
        `Nonce mismatch: expected ${options.nonce}, got ${payload.nonce}`,
      );
    }
    ...
    if (validationErrors.length > 0) {
      logger.warn(
        "[OAuthProvider] ID token validation warnings:",
        validationErrors,
      );
      logger.warn(
        "[OAuthProvider] Note: Cryptographic signature verification requires a dedicated JWT library",
      );
    } else {
  ...
  async revokeToken(token, tokenTypeHint) {
    ...
    if (!this.revocationEndpoint) {
      logger.debug(
        "[OAuthProvider] No revocation endpoint configured, skipping revocation",
      );
    ...
    try {
      ...
      if (response.error) {
        logger.warn(
          "[OAuthProvider] Token revocation returned error:",
          response.error,
        );
        return {
          success: false,
  ...
  _httpGet(url, headers = {}, depth = 0) {
    return new Promise((resolve, reject) => {
      try {
        ...
        const req = protocol.request(options, (res) => {
          this._handleResponse(res, resolve, reject, depth);
        });
   ...
   */
  _handleResponse(res, resolve, reject, depth = 0) {
    const chunks = [];
    let totalSize = 0;

    // Handle redirects (3xx)
    if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
      // Cap redirect depth — without it an IdP returning a self/loop redirect
      // would recurse until the stack overflows (DoS/crash on every outbound
      // token/userinfo/revocation call).
      ...
      logger.debug(
        `[OAuthProvider] Following redirect to: ${res.headers.location}`,
      );
      // For simplicity, we only follow GET redirects
      this._httpGet(res.headers.location, {}, depth + 1)
        .then(resolve)
        .catch(reject);
      return;
    }
    ...
    res.on("end", () => {
      ...
      if (res.statusCode >= 400) {
        ...
        try {
          ...
          errorBody = {
            error: "server_error",
            error_description: rawBody.substring(0, 500),
          };
        }
        ...
        if (res.statusCode === 401) {
          resolve({
            error: "unauthorized",
            error_description:
              errorBody.error_description || "Invalid or expired token",
          });
        } else {
          resolve({
      ...
      try {
        ...
        try {
          ...
          logger.error(
            "[OAuthProvider] Failed to parse response:",
