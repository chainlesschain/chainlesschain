...
function exampleChatSelection() {
  ...
  logger.info(
    "高质量方案:",
    highQualityChat.name,
    "-",
    highQualityChat.description,
  );
...
function realWorldScenarios() {
  ...
  logger.info(
    `成本: 输入¥${docModel.pricing.input}/百万tokens + 图片¥${docModel.pricing.imagePrice}/张`,
  );
  ...
  logger.info(
    `价格: ¥${marketingImage.pricing.standard}/张（标准） - ¥${marketingImage.pricing.high}/张（高清）`,
  );
  ...
  logger.info(
    `价格: ¥${shortVideo.pricing.perSecond}/秒（5秒视频约¥${shortVideo.pricing.perSecond * 5}）`,
  );
  ...
  logger.info(
    `示例: 100万tokens（约50万字）成本 = ¥${ragEmbedding.pricing.input}`,
  );
