...
describe("OllamaClient", () => {
  ...
  describe("constructor", () => {
    it("should initialize with default config", () => {
      ...
      expect(defaultClient.timeout).toBe(300000); // 5 minutes
  ...
  describe("generateStream", () => {
      ...
      const mockStream = {
        on: vi.fn((event, callback) => {
          if (event === "data") {
            ...
            setTimeout(() => {
              callback(
                Buffer.from(JSON.stringify({ response: "Hello" }) + "\n"),
              );
              callback(
                Buffer.from(JSON.stringify({ response: ", World!" }) + "\n"),
              );
              callback(
                Buffer.from(
                  JSON.stringify({
    ...
    it("should handle stream errors", async () => {
      ...
      await expect(client.generateStream("Hello", vi.fn())).rejects.toThrow(
        "Stream error",
      );
    });
        ...
        on: vi.fn((event, callback) => {
          if (event === "data") {
            setTimeout(() => {
              callback(
                Buffer.from(
                  JSON.stringify({ message: { content: "Test" } }) + "\n",
                ),
              );
            }, 5);
          }
  ...
  describe("showModel", () => {
    it("should get model info successfully", async () => {
      ...
      expect(client.client.post).toHaveBeenCalledWith("/api/show", {
        name: "llama2",
      });
    });
