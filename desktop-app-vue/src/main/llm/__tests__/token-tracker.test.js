...
describe("TokenTracker", () => {
  ...
  describe("calculateCost", () => {
    ...
    it("should price current Claude 4 Opus at $5/$25, not the generic default", () => {
      // Regression: claude-opus-4-8 used to be absent from PRICING_DATA and fell
      // to the generic { input: 0.5, output: 1.5 } default → ~10x under-count.
      const result = tracker.calculateCost(
        "anthropic",
        "claude-opus-4-8",
        1000000, // 1M input
      ...
      // $5 + $25 = $30 (generic default would have given $0.5 + $1.5 = $2)
      expect(result.costUsd).toBeCloseTo(30, 2);
      expect(result.pricing.input).toBe(5.0);
      expect(result.pricing.output).toBe(25.0);
    });
    ...
    it("should return time-bucketed data", async () => {
      mockDb.prepare.mockReturnValue({
        all: vi.fn().mockReturnValue([
          {
            time_bucket: 1704067200000,
            calls: 50,
            tokens: 10000,
            cost_usd: 1.5,
          },
          {
            time_bucket: 1704153600000,
            calls: 75,
            tokens: 15000,
            cost_usd: 2.0,
          },
