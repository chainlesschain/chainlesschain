...
describe("ContextEngineering", () => {
  ...
  describe("_cleanSystemPrompt", () => {
    ...
    it("should remove session IDs", () => {
      const cleaned = contextEngine._cleanSystemPrompt("session_id: abc123xyz");
  ...
  describe("error tracking", () => {
    ...
    it("should resolve errors", () => {
      ...
      expect(contextEngine.errorHistory[0].resolution).toBe(
        "Fixed by retrying",
      );
    });
  ...
  describe("Skill-Embedded MCP filter (activeMcpServers)", () => {
    const tools = [
      { name: "read_file", source: "function-caller", tags: [] },
      {
        name: "mcp_weather_today",
        source: "mcp",
        serverName: "weather",
        tags: ["mcp", "weather", "server:weather"],
      },
      {
        name: "mcp_github_issues",
        source: "mcp",
        serverName: "github",
        tags: ["mcp", "github", "server:github"],
      },
      { name: "mcp_legacy_no_server", source: "mcp", tags: ["mcp"] },
    ];

    it("passes through all tools when activeMcpServers is null", () => {
      const r = contextEngine.buildOptimizedPrompt({
        systemPrompt: "sys",
        tools,
      });
      const toolMsg = r.messages.find((m) =>
        m.content?.startsWith("## Available Tools"),
      );
      expect(toolMsg.content).toContain("mcp_weather_today");
      expect(toolMsg.content).toContain("mcp_github_issues");
      expect(toolMsg.content).toContain("read_file");
    });

    it("keeps non-MCP tools and only allowed MCP servers", () => {
      const r = contextEngine.buildOptimizedPrompt({
        systemPrompt: "sys",
        tools,
      ...
      // tool with no resolvable serverName is treated as global → kept
      expect(toolMsg.content).toContain("mcp_legacy_no_server");
    });

    it("accepts an array whitelist (not just Set)", () => {
      const r = contextEngine.buildOptimizedPrompt({
        systemPrompt: "sys",
        tools,
      ...
      expect(toolMsg.content).toContain("mcp_github_issues");
      expect(toolMsg.content).not.toContain("mcp_weather_today");
    });

    it("hides all skill-embedded MCPs when whitelist is empty", () => {
      const r = contextEngine.buildOptimizedPrompt({
        systemPrompt: "sys",
        tools,
