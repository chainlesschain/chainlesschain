...
describe('LLMSelector', () => {
  ...
  describe('selectBestLLM', () => {
    it('should select best LLM based on strategy', () => {
      const best = selector.selectBestLLM({ taskType: 'chat', strategy: 'balanced' });

      expect(['volcengine', 'ollama', 'deepseek', 'openai', 'dashscope', 'zhipu']).toContain(best);
    });
    ...
    it('should return first priority when no provider is available', () => {
      mockDatabase.getSetting = vi.fn((key) => {
        if (key === 'llm.priority') return ['volcengine', 'ollama'];
        if (key === 'llm.autoSelect') return true;
        return null;
      });
  ...
  describe('getFallbackList', () => {
    ...
    it('should only include configured providers', () => {
      mockDatabase.getSetting = vi.fn((key) => {
        if (key === 'llm.priority') return ['volcengine', 'ollama', 'openai'];
        if (key === 'llm.volcengineApiKey') return 'key';
        if (key === 'llm.ollamaHost') return 'http://localhost:11434';
        // openai 未配置
        return null;
      });
  ...
  describe('selectFallback', () => {
    ...
    it('should return null when all providers tried', () => {
      mockDatabase.getSetting = vi.fn((key) => {
        if (key === 'llm.priority') return ['volcengine', 'ollama'];
        if (key === 'llm.autoFallback') return true;
        if (key === 'llm.ollamaHost') return 'http://localhost:11434';
        return null;
      });
