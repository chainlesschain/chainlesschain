...
class LLaVAClient extends EventEmitter {
  ...
  async imageToBase64(imagePath) {
    try {
      ...
      if (stats.size > this.maxImageSize) {
        throw new Error(`图片大小超过限制 (${(this.maxImageSize / 1024 / 1024).toFixed(1)}MB)`);
      }

      // 检查格式
      const ext = path.extname(imagePath).toLowerCase();
      if (!SUPPORTED_FORMATS.includes(ext)) {
        throw new Error(`不支持的图片格式: ${ext}. 支持: ${SUPPORTED_FORMATS.join(', ')}`);
      }
   ...
   */
  async analyzeImage({ imagePath, imageBase64, prompt = '请详细描述这张图片的内容' }, options = {}) {
    try {
   ...
   */
  async analyzeImageStream({ imagePath, imageBase64, prompt = '请详细描述这张图片的内容' }, onChunk, options = {}) {
    try {
  ...
  async visualQA({ imagePath, imageBase64, question }, options = {}) {
    ...
    return this.analyzeImage({ imagePath, imageBase64, prompt }, {
      ...options,
   ...
   */
  async performOCR({ imagePath, imageBase64, language = '中文和英文' }, options = {}) {
...
请直接输出识别到的文字内容：`;
   ...
   */
  async describeImage({ imagePath, imageBase64, style = 'detailed' }, options = {}) {
    const prompts = {
...
请用专业术语描述。`,
    ...
    return this.analyzeImage({ imagePath, imageBase64, prompt: prompts[style] || prompts.detailed }, options);
  }
  ...
  updateConfig(config) {
    ...
    if (config.maxImageSize) this.maxImageSize = config.maxImageSize;
    if (config.baseURL) {
