...
class NotificationManager extends EventEmitter {
  ...
  async loadConfig() {
    try {
      ...
      const setting = db
        .prepare("SELECT value FROM system_settings WHERE key = 'notification_config'")
        .get();
  ...
  async getUnreadNotifications() {
    try {
      const db = this.database.getDatabase();
      const notifications = db
        .prepare('SELECT * FROM notifications WHERE is_read = 0 ORDER BY created_at DESC')
        .all();
  ...
  async getNotificationsByType(type) {
    try {
      const db = this.database.getDatabase();
      const notifications = db
        .prepare('SELECT * FROM notifications WHERE type = ? ORDER BY created_at DESC')
        .all(type);
