...
class CreditScoreManager extends EventEmitter {
  constructor(db) {
    ...
    // 评分权重配置
    this.scoreWeights = {
      completionRate: 0.3, // 交易完成率权重 30%
      tradeVolume: 0.2, // 交易金额权重 20%
      positiveRate: 0.25, // 好评率权重 25%
      responseSpeed: 0.1, // 响应速度权重 10%
      disputeRate: 0.1, // 纠纷率权重 10%
  ...
  async calculateCreditScore(userDid) {
    ...
    const completionRate =
      credit.total_transactions > 0
        ...
        : 0;
    score += (completionRate * 300 * this.scoreWeights.completionRate) / 0.3;

    // 2. 交易金额 (20%) - 对数增长，最高 200 分
    const volumeScore = Math.min(200, Math.log10(credit.total_volume + 1) * 50);
    score += (volumeScore * this.scoreWeights.tradeVolume) / 0.2;

    // 3. 好评率 (25%)
    const totalReviews = credit.positive_reviews + credit.negative_reviews;
    const positiveRate =
      totalReviews > 0 ? credit.positive_reviews / totalReviews : 0.5; // 默认中等
    score += (positiveRate * 250 * this.scoreWeights.positiveRate) / 0.25;

    // 4. 响应速度 (10%) - 响应时间越短越好
    const responseScore =
      credit.avg_response_time > 0
        ? Math.max(0, 100 - credit.avg_response_time / 3600000) // 1小时内满分
        : 50; // 默认中等
    score += (responseScore * this.scoreWeights.responseSpeed) / 0.1;
    ...
    const disputeRate =
      credit.total_transactions > 0
        ...
        : 0;
    score += ((1 - disputeRate) * 100 * this.scoreWeights.disputeRate) / 0.1;
