...
class ReviewManager extends EventEmitter {
  ...
  async createReview(options) {
    ...
    // 验证被评价人
    if (
      revieweeDid !== transaction.buyer_did &&
      revieweeDid !== transaction.seller_did
    ) {
    ...
    // 更新信用评分
    if (rating >= 3) {
      await this.creditScoreManager.onPositiveReview(
        revieweeDid,
        reviewId,
        rating,
      );
    } else {
      await this.creditScoreManager.onNegativeReview(
        revieweeDid,
        reviewId,
        rating,
      );
  ...
  getPendingReviews() {
    ...
    return rows.map((row) => ({
      ...
      // 确定待评价的对象
      revieweeDid:
        row.buyer_did === this.currentUserDid ? row.seller_did : row.buyer_did,
    }));
  }
