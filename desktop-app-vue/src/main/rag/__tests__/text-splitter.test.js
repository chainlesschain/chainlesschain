...
describe("RecursiveCharacterTextSplitter", () => {
  ...
  describe("splitText", () => {
    ...
    it("should split text into chunks", () => {
      const text =
        "This is a test. This is another test. And this is the third test.";
  ...
  describe("getChunkStats", () => {
    ...
    it("returns zeroed stats (not Infinity) for whitespace-only input", () => {
      // splitText returns [] for blank input, so Math.min(...[])=Infinity,
      // Math.max(...[])=-Infinity and text.length/0=Infinity rendered "Infinity".
      const stats = splitter.getChunkStats("   \n\n  ");
      expect(stats.totalChunks).toBe(0);
      expect(stats.minChunkSize).toBe(0);
      expect(stats.maxChunkSize).toBe(0);
      expect(stats.avgChunkSize).toBe(0);
      expect(stats.compressionRatio).toBe("0.00");
      expect(Number.isFinite(stats.minChunkSize)).toBe(true);
      expect(Number.isFinite(stats.maxChunkSize)).toBe(true);
    });
  });
