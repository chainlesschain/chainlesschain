...
describe("QueryRewriter", () => {
  ...
  describe("multiQueryRewrite", () => {
    it("should generate multiple query variants", async () => {
      mockLLMManager.query.mockResolvedValue(
        '["what is RAG?", "explain RAG system", "RAG basics"]',
      );
    ...
    it("should respect maxVariants option", async () => {
      mockLLMManager.query.mockResolvedValue('["v1", "v2", "v3", "v4", "v5"]');

      const result = await rewriter.multiQueryRewrite("test", {
        maxVariants: 2,
      });
  ...
  describe("stepBackRewrite", () => {
    it("should generate abstract query", async () => {
      ...
      const result =
        await rewriter.stepBackRewrite("如何在Python中读取CSV文件?");
  ...
  describe("decomposeQuery", () => {
    it("should decompose complex query", async () => {
      ...
      const result =
        await rewriter.decomposeQuery("RAG系统的实现原理和优化方法");
  ...
  describe("parseQueryVariants", () => {
    ...
    it("extracts separate, marker-stripped variants from a numbered list", () => {
      // 旧实现的 filter 会把所有编号行过滤掉 → 落到「整段响应当作 1 个变体」分支，
      // 返回一个含全部 3 行的垃圾变体，而非 3 个干净变体。
      const response =
        "1. First query variant here\n2. Second query variant here\n3. Third query variant here";
      const variants = rewriter.parseQueryVariants(response, 3);
      expect(variants.length).toBe(3);
      expect(variants[0]).toBe("First query variant here"); // 编号已剥离
      expect(variants[1]).toBe("Second query variant here");
    });

    it("extracts separate, marker-stripped variants from a bullet list", () => {
      const response =
        "- First query variant here\n- Second query variant here\n- Third query variant here";
      const variants = rewriter.parseQueryVariants(response, 3);
      expect(variants.length).toBe(3);
      expect(variants[0]).toBe("First query variant here"); // 项目符号已剥离
    });
  ...
  describe("unknown method", () => {
    it("should return original query for unknown method", async () => {
      const result = await rewriter.rewriteQuery("test", {
