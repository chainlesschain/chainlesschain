...
describe("BM25Search", () => {
  ...
  describe("calculateBM25Score", () => {
    ...
    it("should return zero for non-matching terms", () => {
      bm25 = new BM25Search({ language: "en" });
      bm25.indexDocuments([{ id: "doc1", content: "apple banana cherry" }]);
  ...
  describe("getTermFrequency", () => {
    it("should count term occurrences", () => {
      bm25 = new BM25Search({ language: "en" });
      bm25.indexDocuments([{ id: "doc1", content: "apple apple banana" }]);
    ...
    it("should return zero for missing terms", () => {
      bm25 = new BM25Search({ language: "en" });
      bm25.indexDocuments([{ id: "doc1", content: "apple banana" }]);
  ...
  describe("addDocument", () => {
    ...
    it("guards avgDocLength against an all-empty-token corpus (no NaN scoring)", () => {
      // A fresh index whose only document tokenizes to nothing (whitespace)
      // would make avgDocLength 0; BM25 scoring then divides by it → NaN. The
      // || 1 guard (matching indexDocuments) keeps avgDocLength at 1.
      const empty = new BM25Search({ language: "en" });
      empty.addDocument({ id: "blank", content: "   " }); // tokenizes to []
      expect(empty.avgDocLength).toBe(1);
