...
class AdvancedFeaturesIPC {
  ...
  setupHandlers() {
    ...
    // ===== 自适应阈值 =====
    ipcMain.handle('advanced-features:threshold-monitor', async (event, days = 7) => {
    ...
    ipcMain.handle('advanced-features:threshold-auto', async (event, interval = 60) => {
      return this.executeScript('adaptive-threshold.js', ['auto', `--interval=${interval}`]);
    });

    ipcMain.handle('advanced-features:threshold-history', async (event, limit = 20) => {
      return this.getThresholdHistory(limit);
    });

    // ===== 在线学习 =====
    ipcMain.handle('advanced-features:learning-train', async (event, days = 30) => {
      return this.executeScript('online-learning.js', ['train', `--days=${days}`]);
    });
    ...
    ipcMain.handle('advanced-features:optimizer-bottleneck', async (event, days = 7) => {
      return this.executeScript('advanced-optimizer.js', ['bottleneck', `--days=${days}`]);
    });
    ...
    // ===== 日志 =====
    ipcMain.handle('advanced-features:get-logs', async (event, options = {}) => {
