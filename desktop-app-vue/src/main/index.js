...
class ChainlessChainApp {
  constructor() {
    ...
    this.mcpSecurity = null;
    this.mcpConfigLoader = null;
      ...
      if (testModeConfig.mockLLM) {
        ...
        console.log(`[Main] LLM管理器配置:`, {
          ...
          apiKey: managerConfig.apiKey
            ? `${managerConfig.apiKey.substring(0, 8)}...`
            : "(未设置)",
        });
    ...
    try {
      ...
      if (Notification.isSupported()) {
        const notification = new Notification({
          ...
          body,
          urgency,
          icon: this.mainWindow?.webContents?.getURL() ? undefined : undefined, // 可选：添加应用图标
          silent: alert.level !== "critical", // 危险级别发出声音
        });

        notification.on("click", () => {
          // 点击通知时打开主窗口并跳转到 Token 使用页面
          if (this.mainWindow) {
  ...
  async initializeMobileBridge() {
    ...
    try {
      ...
      this.pcStatusHandler = new PCStatusHandler(
        this.p2pManager,
        this.mobileBridge,
      );
  ...
  async initializeP2PEnhancedManager() {
    ...
    try {
      ...
      this.callHistoryIPC = new CallHistoryIPC(
      ...
      this.connectionHealthManager = new P2PConnectionHealthManager(
        this.p2pManager,
        {
  ...
  buildLLMManagerConfig(provider) {
    ...
    switch (provider) {
      case "ollama":
        config.ollamaURL =
          this.database.getSetting("llm.ollamaHost") ||
          "http://localhost:11434";
        config.model =
          this.database.getSetting("llm.ollamaModel") || "qwen2:7b";
        break;

      case "openai":
        config.apiKey = this.database.getSetting("llm.openaiApiKey") || "";
        config.baseURL =
          this.database.getSetting("llm.openaiBaseUrl") ||
          "https://api.openai.com/v1";
        config.model =
          this.database.getSetting("llm.openaiModel") || "gpt-3.5-turbo";
        break;
      ...
      case "volcengine":
        ...
        config.model =
          this.database.getSetting("llm.volcengineModel") ||
      ...
      case "deepseek":
        ...
        config.model =
          this.database.getSetting("llm.deepseekModel") || "deepseek-chat";
        break;

      case "dashscope":
        config.apiKey = this.database.getSetting("llm.dashscopeApiKey") || "";
        config.model =
          this.database.getSetting("llm.dashscopeModel") || "qwen-turbo";
        break;
  ...
  setupPerformanceIPC() {
    ...
    // 跟踪操作性能
    ipcMain.handle(
      "performance:track-operation",
      async (event, operationInfo) => {
        try {
  ...
  setupMobileBridgeIPC() {
    ...
    ipcMain.handle("mobile:pair-with-code", async (event, pairingCode) => {
      try {
        ...
        const result = await this.devicePairingHandler.pairWithCode(
          pairingCode,
          null,
          null,
        );
  ...
  async scanAndRegisterProjectFiles(projectId, projectPath) {
    ...
    try {
      ...
      for (const file of files) {
        ...
        // 检查文件是否已在数据库中
        const existingFile = this.database.db
          .prepare(
      ...
      if (registeredCount > 0) {
        try {
          // 统计该项目的总文件数
          const totalFiles = this.database.db
            .prepare(
