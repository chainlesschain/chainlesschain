...
class TemplateSynchronizer {
  ...
  async syncTemplate(filePath, templateName) {
    try {
      ...
      `,
        ...
        .run(newSkills, newTools, newEngine, Date.now(), existing.id);
  ...
  async printReport() {
    ...
    logger.info(
      `   - 已配置技能: ${dbStats.with_skills} (${((dbStats.with_skills / dbStats.total) * 100).toFixed(1)}%)`,
    );
    logger.info(
      `   - 已配置工具: ${dbStats.with_tools} (${((dbStats.with_tools / dbStats.total) * 100).toFixed(1)}%)`,
    );
    logger.info(
      `   - 已配置执行引擎: ${dbStats.with_engine} (${((dbStats.with_engine / dbStats.total) * 100).toFixed(1)}%)`,
    );
    logger.info("=".repeat(70));
  }
