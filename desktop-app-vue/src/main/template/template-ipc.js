...
function registerTemplateIPC({ templateManager }) {
  ...
  ipcMain.handle(
    "template:recommend",
    async (_event, userInput, projectType, userId, options = {}) => {
      try {
        if (!templateManager) {
          throw new Error("模板管理器未初始化");
        }
        logger.info("[Template] 推荐模板:", { userInput, projectType, userId });
        const templates = await templateManager.recommendTemplates(
          userInput,
          projectType,
          userId,
          options,
  ...
  ipcMain.handle("template:duplicate", async (_event, templateId, newName) => {
    try {
      ...
      const originalTemplate =
        await templateManager.getTemplateById(templateId);
  ...
  ipcMain.handle(
    ...
    async (_event, templateId, userVariables) => {
      try {
        ...
        const safeVariables =
          userVariables && typeof userVariables === "object"
            ? userVariables
            : {};
        ...
        if (
          ...
          logger.warn(
            `[Template] 模板 ${templateId} 的 prompt_template 为空，尝试重新初始化模板`,
          );
          ...
          if (
            ...
            throw new Error(
              `模板 ${templateId} (${template.display_name}) 的 prompt_template 字段为空，请检查模板文件是否正确`,
            );
          }
        }
        ...
        return {
          success: false,
          error: errorMessage || "渲染模板时发生未知错误",
        };
      }
    },
  );
  ...
  ipcMain.handle("template:render", async (_event, params) => {
    try {
      ...
      const rendered = templateManager.renderTemplateString(
        templateContent,
        variables,
      );
  ...
  ipcMain.handle(
    ...
    async (_event, templateId, userId, projectId, variablesUsed) => {
      try {
        if (!templateManager) {
          throw new Error("模板管理器未初始化");
        }
        await templateManager.recordTemplateUsage(
          templateId,
          userId,
          projectId,
          variablesUsed,
  ...
  ipcMain.handle(
    "template:rate",
    async (_event, templateId, userId, rating, review) => {
      try {
        if (!templateManager) {
          throw new Error("模板管理器未初始化");
        }
  ...
  ipcMain.handle(
    ...
    async (_event, variableDefinitions) => {
      try {
        if (!templateManager) {
          throw new Error("模板管理器未初始化");
        }
        ...
        const defaults =
          templateManager.getDefaultVariables(variableDefinitions);
