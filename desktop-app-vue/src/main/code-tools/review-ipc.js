...
function registerReviewIPC(context) {
  ...
  ipcMain.handle(
    ...
    async (_event, targetId, targetType, filters) => {
      try {
        if (!reviewManager) {
          return [];
        }
        return await reviewManager.getReviewsByTarget(
          targetId,
          targetType,
          filters,
        );
  ...
  ipcMain.handle(
    "review:report",
    async (_event, reviewId, reason, description) => {
      try {
        if (!reviewManager) {
          throw new Error("评价管理器未初始化");
        }
  ...
  ipcMain.handle(
    ...
    async (_event, targetId, targetType) => {
      try {
        if (!reviewManager) {
          return null;
        }
