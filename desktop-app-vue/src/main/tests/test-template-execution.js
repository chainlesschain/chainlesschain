    ...
    for (const test of tests) {
      ...
      try {
        ...
        if (result.success !== false) {
          logger.info(`   ✓ 执行成功`);
          if (result.filePath) {
            logger.info(`     输出文件: ${result.filePath}`);
          }
          if (result.formula) {
            logger.info(`     生成公式: ${result.formula}`);
          }
        } else {
          logger.info(`   ⚠️  执行完成但有警告`);
          if (result.error) {
            logger.info(`     警告: ${result.error}`);
          }
        }
    ...
    try {
      ...
      if (issues.length > 0) {
        ...
        issues.slice(0, 5).forEach((issue, index) => {
          logger.info(`   ${index + 1}. ${issue.template}: ${issue.issue}`);
        });
        if (issues.length > 5) {
          logger.info(`   ... 还有 ${issues.length - 5} 个问题`);
        }
      } else {
        logger.info(`\n   ✓ 所有模板都已配置技能和工具`);
      }
