...
class FileSyncManager extends EventEmitter {
  ...
  async syncFromFilesystem(projectId, relativePath) {
    ...
    try {
      ...
      // 4. 检查同步状态
      const syncState = this.database.db
        .prepare("SELECT * FROM file_sync_state WHERE file_id = ?")
        .get(file.id);

      // 5. 检测冲突（数据库和文件系统都被修改）
      if (
        syncState &&
        syncState.db_hash !== syncState.fs_hash &&
        file.content_hash !== syncState.db_hash
      ) {
  ...
  async watchProject(projectId, rootPath) {
    ...
    watcher.on("change", async (fsPath) => {
      try {
        ...
        if (!file) {
          logger.info(
            `[FileSyncManager] 数据库中没有该文件记录: ${relativePath}`,
          );
          return;
        }
    ...
    watcher.on("unlink", async (fsPath) => {
      try {
        ...
        // 从数据库删除
        this.database.db.run("DELETE FROM project_files WHERE id = ?", [
          file.id,
        ]);
        this.database.db.run("DELETE FROM file_sync_state WHERE file_id = ?", [
          file.id,
        ]);
        this.database.saveToFile();
