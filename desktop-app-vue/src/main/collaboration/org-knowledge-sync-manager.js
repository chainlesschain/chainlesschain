...
class OrgKnowledgeSyncManager extends EventEmitter {
  ...
  async shareKnowledge(orgId, knowledgeId, options = {}) {
    try {
      ...
      const orgKnowledge = {
        ...
        permissions: JSON.stringify(
          options.permissions || { view: ["member"], edit: ["admin", "owner"] },
        ),
      ...
      stmt.run(
        orgKnowledge.id,
        orgKnowledge.knowledge_id,
        orgKnowledge.org_id,
        orgKnowledge.folder_id,
        orgKnowledge.permissions,
        orgKnowledge.is_public,
        orgKnowledge.created_by,
        orgKnowledge.last_edited_by,
        orgKnowledge.created_at,
        orgKnowledge.updated_at,
      );
      ...
      // Log activity
      await this._logActivity(orgId, knowledgeId, "share", {
  ...
  async getOrganizationKnowledge(orgId, options = {}) {
    try {
      ...
      logger.error(
        "[OrgKnowledgeSync] Error getting organization knowledge:",
        error,
      );
      return [];
    }
  ...
  async _handleKnowledgeCreate(orgId, payload, from) {
    try {
      ...
      if (existing) {
        logger.info(
          `[OrgKnowledgeSync] Knowledge ${knowledge.id} already exists`,
        );
        return;
      }
      ...
      kStmt.run(
        ...
        knowledge.type,
        knowledge.content,
        knowledge.content_path,
        knowledge.embedding_path,
        knowledge.created_at,
        knowledge.updated_at,
        knowledge.git_commit_hash,
        knowledge.device_id,
        "synced",
      );
      ...
      logger.error(
        "[OrgKnowledgeSync] Error handling knowledge create:",
        error,
      );
    }
  }
  ...
  async _handleKnowledgeUpdate(orgId, payload, from) {
    try {
      ...
      const local = db
        .prepare("SELECT updated_at FROM knowledge_items WHERE id = ?")
        .get(knowledgeId);

      if (local && local.updated_at >= timestamp) {
        logger.info(
          `[OrgKnowledgeSync] Local version is newer, skipping update`,
        );
        return;
      }
      ...
      logger.error(
        "[OrgKnowledgeSync] Error handling knowledge update:",
        error,
      );
    }
  }
  ...
  async _handleKnowledgeDelete(orgId, payload, from) {
    try {
      ...
      logger.error(
        "[OrgKnowledgeSync] Error handling knowledge delete:",
        error,
      );
    }
  }
  ...
  async _handleFolderCreate(orgId, payload, from) {
    try {
      ...
      // Check if already exists
      const existing = db
        .prepare("SELECT id FROM org_knowledge_folders WHERE id = ?")
        .get(payload.id);
