...
class RealtimeCollabManager extends EventEmitter {
        ...
        users: cursors.map((c) => ({
          ...
          cursor:
            c.cursor_line !== null
              ...
              : null,
          selection:
            c.selection_start_line !== null
              ? {
                  start: {
                    line: c.selection_start_line,
                    column: c.selection_start_column,
                  },
                  end: {
                    line: c.selection_end_line,
                    column: c.selection_end_column,
                  },
                }
              : null,
  ...
  async acquireLock(
    ...
    try {
      ...
      logger.info(
        `[RealtimeCollab] Lock ${lockId} acquired by ${userDid} on document ${docId}`,
      );
  ...
  async requestConflictResolution(docId, conflictData) {
    try {
      ...
      `,
        ...
        conflictData.localUserDid,
        conflictData.remoteUserDid,
        now,
      );

      // Update conflict stats
      await this._updateCollabStats(docId, conflictData.orgId, {
        conflictCount: 1,
      });
      ...
      logger.info(
        `[RealtimeCollab] Conflict ${conflictId} detected in document ${docId}`,
      );
      ...
      logger.error(
        "[RealtimeCollab] Error requesting conflict resolution:",
        error,
      );
      throw error;
    }
  ...
  async resolveConflict(conflictId, resolverDid, resolution) {
    try {
      ...
      logger.info(
        `[RealtimeCollab] Conflict ${conflictId} resolved by ${resolverDid}`,
      );
  ...
  async addInlineComment(docId, comment) {
    try {
      ...
      logger.info(
        `[RealtimeCollab] Comment ${commentId} added to document ${docId}`,
      );
  ...
  async resolveComment(commentId, resolverDid) {
    try {
      ...
      logger.info(
        `[RealtimeCollab] Comment ${commentId} resolved by ${resolverDid}`,
      );
  ...
  async restoreVersion(docId, versionId, userDid) {
    try {
      ...
      logger.info(
        `[RealtimeCollab] Version ${versionId} restored by ${userDid}`,
      );
