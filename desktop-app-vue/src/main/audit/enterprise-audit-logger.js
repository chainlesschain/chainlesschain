...
class EnterpriseAuditLogger extends EventEmitter {
  constructor({
    ...
    if (this.db) {
      this._initTable().catch((err) => {
        logger.error(
          "[EnterpriseAuditLogger] Failed to initialize table:",
          err,
        );
      });
    }
  ...
  async log(eventType, operation, details = {}) {
    ...
    try {
      ...
      if (this.alertOnHighRisk && RiskWeight[entry.riskLevel] >= 3) {
        ...
        logger.warn("[EnterpriseAuditLogger] High risk operation", {
          eventType,
          operation,
      ...
      return {
        success: true,
        data: { id: entry.id, riskLevel: entry.riskLevel },
      };
    } catch (error) {
  ...
  assessRisk(eventType, operation, details = {}) {
    ...
    if (CRITICAL_OPERATIONS.some((c) => op.includes(c))) {
      return RiskLevel.CRITICAL;
    }
    if ((HIGH_RISK_OPERATIONS[eventType] || []).some((h) => op.includes(h))) {
  ...
  sanitizeData(data, depth = 0) {
    ...
    if (Array.isArray(data)) {
      return data.map((i) => this.sanitizeData(i, depth + 1));
    }
    ...
    for (const [key, value] of Object.entries(data)) {
      ...
      if (SENSITIVE_FIELDS.some((f) => kl === f.toLowerCase())) {
        out[key] = "***REDACTED***";
        continue;
      }
      if (
        ...
        value.length > 5000 &&
        (kl.includes("image") ||
          kl.includes("screenshot") ||
          kl.includes("base64"))
      ) {
        out[key] = `[BINARY_DATA: ${value.length} bytes]`;
        continue;
      }
    ...
    try {
      ...
      if (this.db && this._tableInitialized) {
        ...
        const countRow = await this.db.get(
          `SELECT COUNT(*) as total FROM enterprise_audit_log ${where}`,
          params,
        );
      ...
      if (filters.operation) {
        const ol = filters.operation.toLowerCase();
        results = results.filter((e) => e.operation.toLowerCase().includes(ol));
      }
      if (filters.actor) {
      ...
      return {
        success: true,
        data: {
          logs: results.slice(offset, offset + pageSize),
          pagination: {
            page,
            pageSize,
            total,
  ...
  async getLogDetail(id) {
    ...
    try {
      if (this.db && this._tableInitialized) {
        ...
        return row
          ? { success: true, data: this._row(row) }
      ...
      if (this.db && this._tableInitialized) {
        const totalRow = await this.db.get(
          `SELECT COUNT(*) as total, SUM(CASE WHEN success=1 THEN 1 ELSE 0 END) as sc,
           SUM(CASE WHEN success=0 THEN 1 ELSE 0 END) as fc FROM enterprise_audit_log WHERE timestamp>=? AND timestamp<=?`,
          tp,
        );
        const byET = await this.db.all(
          "SELECT event_type,COUNT(*) as count FROM enterprise_audit_log WHERE timestamp>=? AND timestamp<=? GROUP BY event_type ORDER BY count DESC",
          tp,
        );
        const byRL = await this.db.all(
          "SELECT risk_level,COUNT(*) as count FROM enterprise_audit_log WHERE timestamp>=? AND timestamp<=? GROUP BY risk_level ORDER BY count DESC",
          tp,
        );
        const actors = await this.db.all(
          "SELECT actor,COUNT(*) as count FROM enterprise_audit_log WHERE timestamp>=? AND timestamp<=? GROUP BY actor ORDER BY count DESC LIMIT 10",
          tp,
        );
        const hrRow = await this.db.get(
          "SELECT COUNT(*) as count FROM enterprise_audit_log WHERE timestamp>=? AND timestamp<=? AND risk_level IN ('high','critical')",
          tp,
        );
        ...
        const trend = await this.db.all(
          `SELECT strftime('${fmt}',timestamp) as period,COUNT(*) as count FROM enterprise_audit_log WHERE timestamp>=? AND timestamp<=? GROUP BY period ORDER BY period ASC`,
          tp,
        );
        ...
        return {
          ...
          data: {
            ...
            summary: {
              ...
              successCount: totalRow?.sc || 0,
              failureCount: totalRow?.fc || 0,
              highRiskCount: hrRow?.count || 0,
            },
            byEventType: (byET || []).reduce((a, r) => {
              a[r.event_type] = r.count;
              return a;
            }, {}),
            byRiskLevel: (byRL || []).reduce((a, r) => {
              a[r.risk_level] = r.count;
              return a;
            }, {}),
            topActors: actors || [],
            trend: trend || [],
    ...
    try {
      if (!this.db || !this._tableInitialized) {
        ...
        return {
          success: true,
          data: {
            deletedCount: before - this.memoryBuffer.length,
            source: "memory",
          },
        };
      }
      ...
      const cutoff = new Date(Date.now() - retDays * 86400000).toISOString();
      if (keepHR) {
        const r1 = await this.db.run(
          "DELETE FROM enterprise_audit_log WHERE timestamp < ? AND risk_level NOT IN ('high','critical')",
          [cutoff],
        );
        deleted += (r1 && r1.changes) || 0;
        const hrCutoff = new Date(Date.now() - hrDays * 86400000).toISOString();
        const r2 = await this.db.run(
          "DELETE FROM enterprise_audit_log WHERE timestamp < ? AND risk_level IN ('high','critical')",
          [hrCutoff],
        );
      ...
      // `cur` is counted AFTER the time-based deletes, so it already excludes
      // them. Track only the max-records trim here; computing
      // `cur - deleted` would subtract the time-based deletes a second time.
      let remainingCount = cur;
      if (cur > maxRec) {
        const r = await this.db.run(
          `DELETE FROM enterprise_audit_log WHERE id IN (SELECT id FROM enterprise_audit_log ORDER BY created_at ASC LIMIT ?)`,
          [cur - maxRec],
        );
        const trimmed = (r && r.changes) || 0;
        deleted += trimmed;
        remainingCount = cur - trimmed;
      }

      logger.info("[EnterpriseAuditLogger] Retention applied", {
        retDays,
        deleted,
      });
      return {
  ...
  _registerHookListeners() {
    ...
    this.hookSystem.on("hookExecuted", (d) => {
      this.log(map[d.event] || "system", `hook:${d.event}`, {
        ...
        logger.error("[EnterpriseAuditLogger] Hook log failed:", err),
      );
    });
  ...
  async logBrowser(op, d = {}) {
    return this.log(EventType.BROWSER, op, d);
  }
  async logPermission(op, d = {}) {
    return this.log(EventType.PERMISSION, op, d);
  }
  async logFile(op, d = {}) {
    return this.log(EventType.FILE, op, d);
  }
  async logDb(op, d = {}) {
    return this.log(EventType.DB, op, d);
  }
  async logApi(op, d = {}) {
    return this.log(EventType.API, op, d);
  }
  async logCowork(op, d = {}) {
    return this.log(EventType.COWORK, op, d);
  }
  async logAuth(op, d = {}) {
    return this.log(EventType.AUTH, op, d);
  }
  async logSystem(op, d = {}) {
    return this.log(EventType.SYSTEM, op, d);
  }
