...
function registerComplianceIPC({
  soc2Compliance,
  dataClassifier,
  ...
  ipcMain.handle(
    ...
    async (_event, options) => {
      try {
        ...
        return {
          success: true,
          evidence: await soc2Compliance.collectAuditLogEvidence(options),
        };
      } catch (error) {
  ...
  ipcMain.handle("compliance:collect-access-evidence", async () => {
    try {
      ...
      return {
        success: true,
        evidence: await soc2Compliance.collectAccessControlEvidence(),
      };
    } catch (error) {
  ...
  ipcMain.handle("compliance:collect-config-evidence", async () => {
    try {
      ...
      return {
        success: true,
        evidence: await soc2Compliance.collectConfigurationEvidence(),
      };
    } catch (error) {
  ...
  ipcMain.handle(
    "compliance:verify-evidence",
    async (_event, { evidenceId, verifiedBy }) => {
      try {
        if (!soc2Compliance) {
          throw new Error("SOC2 Compliance not initialized");
        }
  ...
  ipcMain.handle(
    ...
    async (_event, { content, options }) => {
      try {
        if (!dataClassifier) {
          throw new Error("Data Classifier not initialized");
        }
        return {
          success: true,
          result: await dataClassifier.classify(content, options),
        };
      } catch (error) {
  ...
  ipcMain.handle(
    "compliance:auto-tag",
    async (_event, { contentId, classificationResult }) => {
      try {
        ...
        return {
          success: true,
          tag: await classificationPolicy.autoTag(
            contentId,
  ...
  ipcMain.handle(
    "compliance:check-access",
    async (_event, { requiredLevel, userClearance }) => {
      try {
        if (!classificationPolicy) {
          throw new Error("Classification Policy not initialized");
        }
        const allowed = classificationPolicy.checkAccess(
          requiredLevel,
          userClearance,
        );
  ...
  logger.info(
    `[Compliance IPC] ✓ Registered ${CHANNELS.length} compliance IPC handlers`,
  );
