...
class PythonBridge {
  ...
  findPythonExecutable() {
    ...
    const possiblePaths = [
      ...
      'C:/Users/' + require('os').userInfo().username + '/AppData/Local/Programs/Python/Python312/python.exe',
    ...
    for (const pythonCmd of possiblePaths) {
      try {
        ...
        console.log(`[PythonBridge] 找到Python: ${pythonCmd}, 版本: ${result.trim()}`);
        return pythonCmd;
      } catch (e) {
        // 继续尝试下一个
      }
    ...
    return new Promise((resolve, reject) => {
      ...
      python.on('close', (code) => {
        ...
        if (code === 0) {
          ...
          // 进程失败
          reject(new Error(`Python工具执行失败 (退出码: ${code})\n${stderr || stdout}`));
        }
      });
  ...
  async checkEnvironment() {
    try {
      const result = await this.callTool('check_environment', {}, { timeout: 10000 });
      return result;
    } catch (error) {
