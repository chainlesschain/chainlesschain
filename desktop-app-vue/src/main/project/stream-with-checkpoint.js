...
async function createProjectWithCheckpoint(options) {
  ...
  let retryCount = checkpoint.retry_count || 0;
  ...
  try {
    ...
    streamControl = await httpClient.createProjectStream(createData, {
      ...
      onProgress: async (data) => {
        try {
          ...
          logger.info(`[StreamCheckpoint] 流式进度: ${data.stage} - ${data.message}`);
      ...
      onContent: async (data) => {
        try {
          ...
          // 定期保存检查点（每收到 10KB 内容）
          if (accumulatedData.contentByStage[data.stage].length % 10240 < data.content.length) {
            checkpointManager.updateCheckpoint(checkpoint.id, {
              accumulatedData
            });
          }
    ...
    if (retryCount < maxRetries && isRetryableError(error)) {
      logger.info(`[StreamCheckpoint] 自动重试 (${retryCount + 1}/${maxRetries})...`);
