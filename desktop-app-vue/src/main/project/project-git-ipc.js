...
function registerProjectGitIPC({
  ...
  ipcMain.handle(
    "project:git-init",
    async (_event, repoPath, remoteUrl = null) => {
      try {
  ...
  ipcMain.handle(
    "project:git-commit",
    async (_event, projectId, repoPath, message, autoGenerate = false) => {
      try {
        // 解析路径（将 /data/projects/xxx 转换为绝对路径）
        const projectConfig = getProjectConfig();
        const resolvedPath = projectConfig.resolveProjectPath(repoPath);
        ...
        const result = await GitAPI.commit(
          resolvedPath,
          message,
          author,
          autoGenerate,
  ...
  ipcMain.handle(
    "project:git-push",
    async (_event, repoPath, remote = "origin", branch = null) => {
      try {
        // 解析路径（将 /data/projects/xxx 转换为绝对路径）
        const projectConfig = getProjectConfig();
        const resolvedPath = projectConfig.resolveProjectPath(repoPath);
  ...
  ipcMain.handle(
    "project:git-pull",
    async (_event, projectId, repoPath, remote = "origin", branch = null) => {
      try {
        // 解析路径（将 /data/projects/xxx 转换为绝对路径）
        const projectConfig = getProjectConfig();
        const resolvedPath = projectConfig.resolveProjectPath(repoPath);
  ...
  ipcMain.handle(
    "project:git-log",
    async (_event, repoPath, page = 1, pageSize = 20) => {
      try {
        const projectConfig = getProjectConfig();
        const resolvedPath = projectConfig.resolveProjectPath(repoPath);
  ...
  ipcMain.handle(
    "project:git-diff",
    async (_event, repoPath, commit1 = null, commit2 = null) => {
      try {
        const projectConfig = getProjectConfig();
        const resolvedPath = projectConfig.resolveProjectPath(repoPath);
  ...
  ipcMain.handle(
    ...
    async (_event, repoPath, branchName, fromBranch = null) => {
      try {
        const projectConfig = getProjectConfig();
        const resolvedPath = projectConfig.resolveProjectPath(repoPath);
  ...
  ipcMain.handle(
    ...
    async (_event, repoPath, branchName) => {
      try {
        const projectConfig = getProjectConfig();
        const resolvedPath = projectConfig.resolveProjectPath(repoPath);
  ...
  ipcMain.handle(
    "project:git-merge",
    async (_event, repoPath, sourceBranch, targetBranch = null) => {
      try {
        const projectConfig = getProjectConfig();
        const resolvedPath = projectConfig.resolveProjectPath(repoPath);
  ...
  ipcMain.handle(
    ...
    async (_event, repoPath, filePath = null, strategy = null) => {
      try {
        const projectConfig = getProjectConfig();
        const resolvedPath = projectConfig.resolveProjectPath(repoPath);
        return await GitAPI.resolveConflicts(
          resolvedPath,
          filePath,
          false,
          strategy,
   ...
   */
  ipcMain.handle(
    "project:git-generate-commit-message",
