...
function registerFileIPC({
  ...
  const handleReadContent = async (_event, filePath) => {
    const content = await handleReadContentLegacy(_event, filePath);
    return {
      success: true,
      content,
    };
  ...
  // Backward compatibility: support both channel names
  ipcMain.handle("file:read-content", handleReadContentLegacy);
  try {
    ipcMain.handle("file:readContent", handleReadContent);
  } catch (error) {
    const message = String(error?.message || error);
    if (message.includes("Attempted to register a second handler")) {
      logger.warn(
        "[Main] file:readContent handler already registered, skipping",
      );
    } else {
      throw error;
    }
  ...
  ipcMain.handle(
    "file:copyItem",
    async (_event, { projectId, sourcePath, targetPath }) => {
      try {
        const fs = require("fs").promises;
  ...
  ipcMain.handle(
    "file:renameItem",
    async (_event, { projectId, oldPath, newName }) => {
      try {
        const fs = require("fs").promises;
  ...
  ipcMain.handle(
    "file:createFolder",
    async (_event, { projectId, folderPath }) => {
      try {
        const fs = require("fs").promises;
   ...
   */
  ipcMain.handle(
    "file:copyToSystemClipboard",
    async (
      _event,
      { projectId, filePath, fullPath, isDirectory, fileName },
    ) => {
      try {
        const fs = require("fs").promises;
        ...
        clipboard.writeBuffer(
          "FileNameW",
          Buffer.from(resolvedPath + "\0", "ucs2"),
        );
   ...
   */
  ipcMain.handle(
    "file:importFromSystemClipboard",
    async (_event, { projectId, targetPath, clipboardData }) => {
      try {
        const fs = require("fs").promises;

        logger.info("[Main] 从系统剪贴板导入文件:", {
          targetPath,
          clipboardData,
        });
  ...
  ipcMain.handle("file:openWith", async (_event, { projectId, filePath }) => {
    try {
      ...
      spawn(programPath, [resolvedPath], {
        detached: true,
        stdio: "ignore",
      }).unref();
   ...
   */
  ipcMain.handle(
    "file:openWithProgram",
    async (_event, { filePath, programPath }) => {
      try {
