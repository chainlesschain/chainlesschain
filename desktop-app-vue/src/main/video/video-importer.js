...
class VideoImporter extends EventEmitter {
  ...
  async importVideoBatch(filePaths, options = {}) {
    ...
    for (let i = 0; i < filePaths.length; i += concurrency) {
      ...
      const progress = Math.round(
        ((i + batch.length) / filePaths.length) * 100,
      );
  ...
  async copyFileToStorage(sourceFilePath) {
    const fileName = this.config.generateFileName(
  ...
  async generateThumbnail(filePath, taskId) {
    const thumbnailDir = this.config.getStoragePath(
      "thumbnails",
      this.userDataPath,
    );
    ...
    const result = await this.videoEngine.generateThumbnail(
      filePath,
      thumbnailPath,
      {
  ...
  async extractKeyframes(video) {
    const keyframeDir = this.config.getStoragePath(
      "keyframes",
      this.userDataPath,
    );
    ...
    for (let i = 0; i < count; i++) {
      ...
      try {
        await this.videoEngine.generateThumbnail(
          video.file_path,
          keyframePath,
          {
