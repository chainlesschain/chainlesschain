...
function registerWalletIPC({ walletManager, externalWalletConnector }) {
  ...
  ipcMain.handle(
    ...
    async (_event, { mnemonic, password, chainId = 1 }) => {
      try {
        ...
        return await walletManager.importFromMnemonic(
          mnemonic,
          password,
          chainId,
        );
  ...
  ipcMain.handle(
    ...
    async (_event, { privateKey, password, chainId = 1 }) => {
      try {
        ...
        return await walletManager.importFromPrivateKey(
          privateKey,
          password,
          chainId,
        );
  ...
  ipcMain.handle(
    ...
    async (_event, { walletId, transaction, useUKey = false }) => {
      try {
        if (!walletManager) {
          throw new Error("钱包管理器未初始化");
        }
  ...
  ipcMain.handle(
    "wallet:sign-message",
    async (_event, { walletId, message, useUKey = false }) => {
      try {
        if (!walletManager) {
          throw new Error("钱包管理器未初始化");
        }
  ...
  ipcMain.handle(
    "wallet:get-balance",
    async (_event, { address, chainId, tokenAddress = null }) => {
      try {
        if (!walletManager) {
          throw new Error("钱包管理器未初始化");
        }
  ...
  ipcMain.handle(
    ...
    async (_event, { walletId, password }) => {
      try {
        if (!walletManager) {
          throw new Error("钱包管理器未初始化");
        }
  ...
  ipcMain.handle(
    ...
    async (_event, { address, provider, chainId }) => {
      try {
        if (!externalWalletConnector) {
          throw new Error("外部钱包连接器未初始化");
        }

        await externalWalletConnector._saveExternalWallet({
          address,
          provider,
          chainId,
        });
