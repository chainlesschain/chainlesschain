...
class BlockchainIntegrationIPC {
  ...
  registerHandlers() {
    ...
    // 创建链上Token
    ipcMain.handle('blockchain-integration:create-token', async (event, localAssetId, options) => {
      try {
    ...
    // 创建链上NFT
    ipcMain.handle('blockchain-integration:create-nft', async (event, localAssetId, options) => {
      try {
    ...
    // 转账链上资产
    ipcMain.handle('blockchain-integration:transfer-asset', async (event, localAssetId, options) => {
      try {
    ...
    // 同步资产余额
    ipcMain.handle('blockchain-integration:sync-balance', async (event, localAssetId, ownerAddress) => {
      try {
    ...
    // 获取资产映射
    ipcMain.handle('blockchain-integration:get-asset-mapping', async (event, localAssetId) => {
      try {
    ...
    // 创建链上托管
    ipcMain.handle('blockchain-integration:create-escrow', async (event, localEscrowId, options) => {
      try {
    ...
    // 同步托管状态
    ipcMain.handle('blockchain-integration:sync-escrow', async (event, localEscrowId) => {
      try {
    ...
    // 获取托管映射
    ipcMain.handle('blockchain-integration:get-escrow-mapping', async (event, localEscrowId) => {
      try {
    ...
    // 监控交易
    ipcMain.handle('blockchain-integration:monitor-transaction', async (event, txHash, confirmations) => {
      try {
    ...
    // 获取交易映射
    ipcMain.handle('blockchain-integration:get-transaction-mapping', async (event, localTxId) => {
      try {
    ...
    // 获取待确认交易
    ipcMain.handle('blockchain-integration:get-pending-transactions', async () => {
      try {
    ...
    // 启动自动同步
    ipcMain.handle('blockchain-integration:start-auto-sync', async (event, interval) => {
      try {
