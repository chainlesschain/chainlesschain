 ...
 */
function registerBlockchainIPC({
  blockchainAdapter,
  transactionMonitor,
  database,
  mainWindow,
  ...
  ipcMain.handle(
    ...
    async (_event, { address, chainId, limit = 100, offset = 0 }) => {
      try {
        if (!transactionMonitor) {
          throw new Error("交易监控器未初始化");
        }

        return await transactionMonitor.getTxHistory({
          address,
          chainId,
          limit,
          offset,
  ...
  ipcMain.handle("blockchain:deploy-token", async (_event, options) => {
    try {
      ...
      const { walletId, name, symbol, decimals, initialSupply, chainId } =
        options;
  ...
  ipcMain.handle("blockchain:mint-nft", async (_event, options) => {
    try {
      ...
      return await blockchainAdapter.mintNFT(
        walletId,
        contractAddress,
        to,
        metadataURI,
  ...
  ipcMain.handle("blockchain:transfer-token", async (_event, options) => {
    try {
      ...
      return await blockchainAdapter.transferToken(
        walletId,
        tokenAddress,
        to,
        amount,
  ...
  ipcMain.handle(
    ...
    async (_event, { transaction, chainId }) => {
      try {
        if (!blockchainAdapter) {
          throw new Error("区块链适配器未初始化");
        }
  ...
  ipcMain.handle(
    ...
    async (_event, { blockNumber, chainId }) => {
      try {
        if (!blockchainAdapter) {
          throw new Error("区块链适配器未初始化");
        }
  ...
  ipcMain.handle(
    ...
    async (_event, { contractAddress, eventName, abi, chainId }) => {
      try {
        if (!blockchainAdapter) {
          throw new Error("区块链适配器未初始化");
        }

        await blockchainAdapter.listenToEvents(
          contractAddress,
          eventName,
          abi,
          chainId,
