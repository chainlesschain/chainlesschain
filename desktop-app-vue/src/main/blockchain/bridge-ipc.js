...
function registerBridgeIPC(bridgeManager) {
  ...
  ipcMain.handle(
    ...
    async (_event, { chainId, contractAddress }) => {
      try {
        if (!bridgeManager) {
          throw new Error("跨链桥管理器未初始化");
        }
  ...
  ipcMain.handle(
    "bridge:get-balance",
    async (_event, { address, tokenAddress, chainId }) => {
      try {
        if (!bridgeManager) {
          throw new Error("跨链桥管理器未初始化");
        }

        return await bridgeManager.getAssetBalance(
          address,
          tokenAddress,
          chainId,
        );
  ...
  // 批量查询余额
  ipcMain.handle(
    "bridge:get-batch-balances",
    async (_event, { address, assets }) => {
      try {
        if (!bridgeManager) {
          throw new Error("跨链桥管理器未初始化");
        }
  ...
  // 查询锁定余额
  ipcMain.handle(
    "bridge:get-locked-balance",
    async (_event, { tokenAddress, chainId }) => {
      try {
        if (!bridgeManager) {
          throw new Error("跨链桥管理器未初始化");
        }
