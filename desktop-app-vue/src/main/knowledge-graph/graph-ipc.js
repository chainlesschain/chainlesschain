...
function registerGraphIPC(context) {
  ...
  ipcMain.handle(
    "graph:process-note",
    async (_event, noteId, content, tags) => {
      try {
        if (!graphExtractor) {
  ...
  ipcMain.handle(
    "graph:add-relation",
    async (_event, sourceId, targetId, type, weight, metadata) => {
      try {
        if (!database) {
  ...
  ipcMain.handle(
    ...
    async (_event, noteId, content) => {
      try {
        ...
        return await graphExtractor.extractSemanticRelations(
          noteId,
          content,
          llmManager,
        );
    ...
    async (_event, nodes, edges, type) => {
      try {
        ...
        switch (type) {
          case "degree":
            return Array.from(
              analytics.calculateDegreeCentrality(nodes, edges).entries(),
            );
          case "closeness":
            return Array.from(
              analytics.calculateClosenessCentrality(nodes, edges).entries(),
            );
          case "betweenness":
            return Array.from(
              analytics.calculateBetweennessCentrality(nodes, edges).entries(),
            );
          case "pagerank":
            return Array.from(
              analytics.calculatePageRank(nodes, edges).entries(),
            );
          default:
  ...
  ipcMain.handle("graph:extract-entities", async (_event, text, useLLM) => {
    try {
      ...
      if (useLLM && llmManager) {
        ...
        return {
          entities: entityExtraction.extractEntities(text),
          relations: [],
        };
      }
    } catch (error) {
