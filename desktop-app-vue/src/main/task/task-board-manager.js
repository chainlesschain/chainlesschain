...
/**
 * Tolerant JSON column parse — a single automation rule with a corrupt
 * trigger_conditions string must not throw out of the rules loop and skip every
 * later rule. The `x ? JSON.parse(x) : d` form it replaces only guarded NULL.
 */
function safeParse(raw, fallback) {
  ...
  try {
    ...
    logger.warn(
      `[TaskBoardManager] Bad JSON column, using fallback: ${err.message}`,
    );
    return fallback;
  }
}
...
class TaskBoardManager extends EventEmitter {
  ...
  async createBoard(boardData) {
    try {
      ...
      logger.info(
        `[TaskBoard] Created board ${boardId} for org ${boardData.orgId}`,
      );
  ...
  async createSprint(boardId, sprintData) {
    try {
      ...
      logger.info(
        `[TaskBoard] Created sprint ${sprintId} for board ${boardId}`,
      );
      ...
      const avgVelocity =
        recentSprints.length > 0
          ? (recentSprints.reduce(
              (sum, s) => sum + (s.velocity_completed || 0),
              0,
            ) +
  ...
  async moveToSprint(taskIds, sprintId) {
    try {
      ...
      logger.info(
        `[TaskBoard] Moved ${taskIds.length} tasks to sprint ${sprintId}`,
      );
  ...
  async executeWorkflowRules(boardId, triggerEvent, task, context = {}) {
    try {
      ...
      for (const rule of rules) {
        const conditions = safeParse(rule.trigger_conditions, {});
  ...
  async getBoardAnalytics(boardId, options = {}) {
    try {
      ...
      const dateFrom =
        options.dateFrom || Date.now() - 30 * 24 * 60 * 60 * 1000; // 30 days
