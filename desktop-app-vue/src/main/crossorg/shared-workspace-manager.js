...
class SharedWorkspaceManager extends EventEmitter {
  ...
  async inviteOrg(workspaceId, orgData, invitedByDid) {
    try {
      ...
      logger.info(
        `[SharedWorkspace] Invited org ${orgData.orgId} to workspace ${workspaceId}`,
      );
  ...
  async removeOrg(workspaceId, orgId, _removedByDid) {
    try {
      ...
      logger.info(
        `[SharedWorkspace] Removed org ${orgId} from workspace ${workspaceId}`,
      );
  ...
  async addMember(workspaceId, memberData, addedByDid) {
    try {
      ...
      logger.info(
        `[SharedWorkspace] Added member ${memberData.memberDid} to workspace ${workspaceId}`,
      );
  ...
  async removeMember(workspaceId, memberDid, _removedByDid) {
    try {
      ...
      logger.info(
        `[SharedWorkspace] Removed member ${memberDid} from workspace ${workspaceId}`,
      );
  ...
  async updateMemberRole(
    ...
    try {
      ...
      db.prepare(
        ...
        WHERE workspace_id = ? AND member_did = ?
      `,
      ).run(role, JSON.stringify(permissions), now, workspaceId, memberDid);

      logger.info(
        `[SharedWorkspace] Updated member ${memberDid} role to ${role}`,
      );
    ...
    try {
      ...
      return {
        ...
        workspaces: workspaces.map((ws) => ({
          ...
          status: ws.status,
          settings: safeParse(ws.settings, null),
          orgRole: ws.org_role,
    ...
    try {
      ...
      return {
        ...
        members: members.map((m) => ({
          ...
          role: m.role,
          permissions: safeParse(m.permissions, []),
