...
class MCPBenchmark {
  ...
  async setup() {
    ...
    fs.writeFileSync(
      testFilePath,
      "This is a test file for MCP benchmarking.\n".repeat(100),
    );
    ...
    for (let i = 0; i < TEST_CONFIG.iterations; i++) {
      ...
      if ((i + 1) % 10 === 0) {
        process.stdout.write(
          `  Progress: ${i + 1}/${TEST_CONFIG.iterations}\r`,
        );
      }
    }
  ...
  calculateOverhead() {
    ...
    if (
      this.results.directCall.length === 0 ||
      this.results.mcpCall.length === 0
    ) {
    ...
    for (
      let i = 0;
      i < Math.min(this.results.directCall.length, this.results.mcpCall.length);
      i++
    ) {
    ...
    const avgDirect =
      this.results.directCall.reduce((a, b) => a + b, 0) /
      this.results.directCall.length;
    const avgMCP =
      this.results.mcpCall.reduce((a, b) => a + b, 0) /
      this.results.mcpCall.length;
    ...
    logger.info(
      `    Avg: ${avgOverhead.toFixed(2)}ms (${overheadPercent.toFixed(1)}%)`,
    );
  ...
  generateReport() {
    ...
    const criteria = [
      ...
      },
      {
        ...
        acceptable: "< 5%",
        actual:
          ((summary.toolCalls.failed / summary.toolCalls.total) * 100).toFixed(
            1,
          ) + "%",
