...
class EmailIPCHandler {
  ...
  registerHandlers() {
    ...
    ipcMain.handle('email:update-account', async (event, accountId, updates) => {
    ...
    // 邮件管理
    ipcMain.handle('email:fetch-emails', async (event, accountId, options = {}) => {
    ...
    ipcMain.handle('email:mark-as-starred', async (event, emailId, starred = true) => {
    ...
    ipcMain.handle('email:send-email', async (event, accountId, mailOptions) => {
    ...
    ipcMain.handle('email:download-attachment', async (event, attachmentId, savePath) => {
  ...
  async removeAccount(accountId) {
    try {
      ...
      // 删除账户
      const stmt = this.database.db.prepare('DELETE FROM email_accounts WHERE id = ?');
      stmt.run([accountId]);
  ...
  async getAccounts() {
    try {
      const stmt = this.database.db.prepare('SELECT * FROM email_accounts ORDER BY email');
  ...
  async getAccount(accountId) {
    try {
      const stmt = this.database.db.prepare('SELECT * FROM email_accounts WHERE id = ?');
  ...
  async syncMailboxes(accountId) {
    try {
      const accountStmt = this.database.db.prepare('SELECT * FROM email_accounts WHERE id = ?');
  ...
  async getEmail(emailId) {
    try {
      const stmt = this.database.db.prepare('SELECT * FROM emails WHERE id = ?');
  ...
  async markAsRead(emailId) {
    try {
      const stmt = this.database.db.prepare('UPDATE emails SET is_read = 1 WHERE id = ?');
      stmt.run([emailId]);
  ...
  async markAsStarred(emailId, starred = true) {
    try {
      const stmt = this.database.db.prepare('UPDATE emails SET is_starred = ? WHERE id = ?');
  ...
  async archiveEmail(emailId) {
    try {
      const stmt = this.database.db.prepare('UPDATE emails SET is_archived = 1 WHERE id = ?');
      stmt.run([emailId]);
  ...
  async saveToKnowledge(emailId) {
    try {
      const emailStmt = this.database.db.prepare('SELECT * FROM emails WHERE id = ?');
  ...
  async getAttachments(emailId) {
    try {
      const stmt = this.database.db.prepare('SELECT * FROM email_attachments WHERE email_id = ?');
  ...
  async downloadAttachment(attachmentId, savePath) {
    try {
      const stmt = this.database.db.prepare('SELECT * FROM email_attachments WHERE id = ?');
  ...
  async getLabels() {
    try {
      const stmt = this.database.db.prepare('SELECT * FROM email_labels ORDER BY name');
