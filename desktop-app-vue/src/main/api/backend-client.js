...
let _pythonClient = axios.create({
...
class ProjectFileAPI {
  ...
  static async getFiles(
    ...
    try {
      const response = await _javaClient.get(
  ...
  static async createFile(projectId, fileData) {
    try {
      const response = await _javaClient.post(
        `/api/projects/${projectId}/files`,
        fileData,
      );
      return response.data;
  ...
  static async updateFile(projectId, fileId, fileData) {
    try {
      const response = await _javaClient.put(
        `/api/projects/${projectId}/files/${fileId}`,
        fileData,
      );
      return response.data;
  ...
  static async deleteFile(projectId, fileId) {
    try {
      const response = await _javaClient.delete(
        `/api/projects/${projectId}/files/${fileId}`,
      );
      return response.data;
    } catch (error) {
...
class GitAPI {
  ...
  static async init(repoPath, remoteUrl = null, branchName = "main") {
    try {
      const response = await _pythonClient.post("/api/git/init", {
        repo_path: repoPath,
  ...
  static async status(repoPath) {
    try {
      const response = await _pythonClient.get("/api/git/status", {
  ...
  static async commit(
    ...
    message = null,
    files = null,
    autoGenerateMessage = false,
  ) {
    try {
      const response = await _pythonClient.post("/api/git/commit", {
        repo_path: repoPath,
        message,
        files,
  ...
  static async push(repoPath, remote = "origin", branch = null) {
    try {
      const response = await _pythonClient.post("/api/git/push", {
        repo_path: repoPath,
        remote,
        branch,
      });
  ...
  static async pull(repoPath, remote = "origin", branch = null) {
    try {
      const response = await _pythonClient.post("/api/git/pull", {
        repo_path: repoPath,
        remote,
        branch,
      });
  ...
  static async log(repoPath, limit = 20) {
    try {
      const response = await _pythonClient.get("/api/git/log", {
  ...
  static async diff(repoPath, commit1 = null, commit2 = null) {
    try {
      const response = await _pythonClient.get("/api/git/diff", {
  ...
  static async branches(repoPath) {
    try {
      const response = await _pythonClient.get("/api/git/branches", {
        params: { repo_path: repoPath },
      });
      return response.data;
    } catch (error) {
  ...
  static async createBranch(repoPath, branchName, fromBranch = null) {
    try {
      const response = await _pythonClient.post("/api/git/branch/create", {
        repo_path: repoPath,
        branch_name: branchName,
  ...
  static async checkoutBranch(repoPath, branchName) {
    try {
      const response = await _pythonClient.post("/api/git/branch/checkout", {
        repo_path: repoPath,
        branch_name: branchName,
      });
      return response.data;
  ...
  static async merge(repoPath, sourceBranch, targetBranch = null) {
    try {
      const response = await _pythonClient.post("/api/git/merge", {
        repo_path: repoPath,
  ...
  static async resolveConflicts(
    ...
    try {
      const response = await _pythonClient.post("/api/git/resolve-conflicts", {
        repo_path: repoPath,
        file_path: filePath,
...
class RAGAPI {
  ...
  static async getIndexStats(projectId) {
    try {
      const response = await _pythonClient.get("/api/rag/index/stats", {
  ...
  static async deleteProjectIndex(projectId) {
    try {
      const response = await _pythonClient.delete(
        `/api/rag/index/project/${projectId}`,
      );
      return response.data;
    } catch (error) {
...
class CodeAPI {
  ...
  static async generate(
    ...
    try {
      const response = await _pythonClient.post("/api/code/generate", {
        description,
        language,
        style,
  ...
  static async review(code, language, focusAreas = null) {
    try {
      const response = await _pythonClient.post("/api/code/review", {
        code,
        language,
  ...
  static async refactor(
    ...
    try {
      const response = await _pythonClient.post("/api/code/refactor", {
        code,
        language,
  ...
  static async explain(code, language) {
    try {
      const response = await _pythonClient.post("/api/code/explain", {
        code,
        language,
      });
      return response.data;
  ...
  static async fixBug(code, language, bugDescription = null) {
    try {
      const response = await _pythonClient.post("/api/code/fix-bug", {
        code,
        language,
  ...
  static async generateTests(code, language) {
    try {
      const response = await _pythonClient.post("/api/code/generate-tests", {
        code,
        language,
      });
      return response.data;
  ...
  static async optimize(code, language) {
    try {
      const response = await _pythonClient.post("/api/code/optimize", {
        code,
        language,
      });
      return response.data;
...
module.exports = {
  ...
  RAGAPI,
  CodeAPI,
  // 导出客户端实例供高级用法（getters so test-injected fakes are visible to external readers）
  get javaClient() {
    return _javaClient;
  },
  get pythonClient() {
  ...
  // Test seam — see RFC docs/design/desktop_vi_mock_cjs_migration_rfc.md T1
