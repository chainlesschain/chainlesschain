...
class ShadowDOMScanner {
  ...
  async scan(targetId, options = {}) {
    ...
    try {
      const result = await page.evaluate(({ rootSel, maxDepth, includeHidden }) => {
        const elements = [];
        let refCounter = 0;
        ...
        function scanElement(element, depth, shadowPath = []) {
          ...
          if (element.id) {
            ...
            selectorPath.push(`${element.tagName.toLowerCase()}:nth-child(${index + 1})`);
          }
          ...
          if (element.shadowRoot) {
            const newShadowPath = [...shadowPath, buildSelector(element) + '::shadow'];
        ...
        function getAccessibleName(el) {
          return el.getAttribute('aria-label') ||
                 ...
                 el.getAttribute('alt') ||
                 (el.tagName.toLowerCase() === 'img' ? '' : el.textContent?.trim().substring(0, 100)) ||
                 '';
        }
        ...
        function getAttributes(el) {
          const result = {};
          const keep = ['id', 'class', 'name', 'type', 'href', 'src', 'value',
                       'placeholder', 'data-testid', 'data-id'];
        ...
        return {
          ...
          maxDepthReached: elements.some(e => (e.shadowPath?.length || 0) >= maxDepth)
        };
