...
function registerRecordingIPC() {
  ...
  ipcMain.handle(
    ...
    withErrorHandler(async (event, targetId, options = {}) => {
      ...
      logger.info("[RecordingIPC] Recording started", {
        targetId,
  ...
  ipcMain.handle(
    ...
    withErrorHandler(async (event, recording, targetId, options = {}) => {
      const p = getPlayer();
  ...
  ipcMain.handle(
    ...
    withErrorHandler(async (event, playbackId) => {
      const p = getPlayer();
  ...
  ipcMain.handle(
    ...
    withErrorHandler(async (event, recording) => {
      const store = getStorage();
   ...
   */
  ipcMain.handle(
    "browser:baseline:save",
    withErrorHandler(async (event, baseline) => {
      const store = getStorage();
  ...
  logger.info(
    "[RecordingIPC] All Recording IPC handlers registered (15 handlers)",
  );
}
