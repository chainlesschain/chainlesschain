...
describe("SmartElementDetector", () => {
  ...
  describe("detect", () => {
    ...
    it("should throw when engine not set", async () => {
      ...
      await expect(d.detect("tab1", { text: "Submit" })).rejects.toThrow(
        "Browser engine not set",
      );
    });
  ...
  describe("waitFor", () => {
    it("should wait for element to appear", async () => {
      ...
      const result = await detector.waitFor(
        "tab1",
        { text: "Submit" },
        {
          timeout: 2000,
  ...
  describe("_detectExact by-id (CSS.escape ReferenceError regression)", () => {
    it("resolves the by-id branch via an attribute selector without throwing", async () => {
      // Before the fix this called CSS.escape (a browser-only global absent in
      // the Node main process) → ReferenceError, swallowed by detect()'s
      // try/catch, so the entire exact-ID path silently never matched.
      let usedSelector = null;
      mockPage.$ = vi.fn(async (sel) => {
        usedSelector = sel;
        return {
      ...
      mockPage.evaluate = vi
        .fn()
        .mockResolvedValue({ tagName: "div", text: "x" });

      const res = await detector._detectExact("tab1", { text: "my-id" });

      expect(usedSelector).toBe('[id="my-id"]');
      expect(res).not.toBeNull();
      expect(res.strategy).toBe(DetectionStrategy.EXACT);
    });

    it("escapes quotes in the id value", async () => {
      let usedSelector = null;
      mockPage.$ = vi.fn(async (sel) => {
        usedSelector = sel;
        return null;
      });
      await detector._detectExact("tab1", { text: 'a"b' });
      expect(usedSelector).toBe('[id="a\\"b"]');
    });
  });
});
