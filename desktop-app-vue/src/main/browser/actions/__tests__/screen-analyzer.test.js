...
describe('ScreenAnalyzer', () => {
  ...
  beforeEach(() => {
    ...
    mockPage = {
      evaluate: vi.fn().mockResolvedValue({
        ...
        regions: [
          { type: 'header', bounds: { x: 0, y: 0, width: 1920, height: 60 }, element: { tagName: 'header' } },
          { type: 'navigation', bounds: { x: 0, y: 60, width: 200, height: 1020 }, element: { tagName: 'nav' } },
          { type: 'content', bounds: { x: 200, y: 60, width: 1720, height: 1020 }, element: { tagName: 'main' } }
        ],
  ...
  describe('analyze', () => {
    ...
    it('should support different analysis modes', async () => {
      ...
      await analyzer.analyze('tab1', { mode: AnalysisMode.DETAILED, forceRefresh: true });
  ...
  describe('captureRegion', () => {
    ...
    it('should fail when engine not set', async () => {
      const a = new ScreenAnalyzer();
      const result = await a.captureRegion('tab1', { x: 0, y: 0, width: 100, height: 100 });
    ...
    it('should fail when page not found', async () => {
      mockEngine.getPage.mockReturnValue(null);
      const result = await analyzer.captureRegion('tab1', { x: 0, y: 0, width: 100, height: 100 });
  ...
  describe('compare', () => {
    it('should compare two analyses', () => {
      const before = {
        ...
        title: 'Page 1',
        regions: [
          { type: 'header', bounds: { x: 0, y: 0, width: 100, height: 50 }, element: {} }
        ]
      };
      ...
      const after = {
        ...
        regions: [
          { type: 'header', bounds: { x: 0, y: 0, width: 100, height: 50 }, element: {} },
          { type: 'footer', bounds: { x: 0, y: 900, width: 100, height: 50 }, element: {} }
        ]
      };
    ...
    it('should detect removed regions', () => {
      const before = {
        ...
        title: 'Page',
        regions: [
          { type: 'header', bounds: { x: 0, y: 0, width: 100, height: 50 }, element: { id: 'header' } },
          { type: 'sidebar', bounds: { x: 0, y: 50, width: 50, height: 100 }, element: { id: 'sidebar' } }
        ]
      };
