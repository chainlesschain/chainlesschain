...
class ScreenRecorder extends EventEmitter {
  constructor(browserEngine = null, config = {}) {
    ...
    this.config = {
      ...
      outputDir:
        config.outputDir ||
        path.join(process.cwd(), ".chainlesschain", "recordings"),
  ...
  async startRecording(targetId = null, options = {}) {
    ...
    // 设置最大录制时长。存句柄并在 stopRecording 清除——否则手动停止录制 A 后，A 的
    // maxDuration 定时器仍会触发；若此时录制 B 正在进行（singleton，state===RECORDING），
    // 守卫通过 → 误把 B 提前停掉（跨录制干扰）。
  ...
  async stopRecording() {
    ...
    // 清除 maxDuration 定时器，避免它在下一次录制期间误触发（跨录制干扰）。
  ...
  async _captureFrame() {
    ...
    try {
      ...
      if (this.currentRecording.targetId && this.browserEngine) {
        // 浏览器页面截图
        buffer = await this.browserEngine.screenshot(
          this.currentRecording.targetId,
          {
            type: "jpeg",
  ...
  getStatus() {
    const elapsed =
      this.state === RecordingState.RECORDING && this.startTime
