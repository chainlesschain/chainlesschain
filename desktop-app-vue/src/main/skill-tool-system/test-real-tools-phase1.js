...
async function testPhase1RealTools() {
  ...
  try {
    ...
    if (scanResult.success) {
      ...
      if (scanResult.codes && scanResult.codes.length > 0) {
        ...
        logger.info(`   → 位置: x=${scanResult.codes[0].position.x}, y=${scanResult.codes[0].position.y}\n`);
      }
      passedTests++;
  ...
  results.forEach((result, index) => {
    ...
    if (result.data) {logger.info(`   数据: ${result.data}`);}
    if (result.ratio) {logger.info(`   压缩率: ${result.ratio}`);}
  });
