...
async function testBatch12Tools() {
  ...
  const batch12Tools = allTools.filter(tool => {
    const batch12ToolNames = [
      'file_compressor', 'file_decompressor', 'image_editor', 'image_filter',
      'video_cutter', 'video_merger', 'pdf_converter', 'office_converter',
      'qrcode_generator_advanced', 'qrcode_scanner', 'screenshot_tool', 'screen_recorder',
      'calendar_manager', 'reminder_scheduler', 'note_editor', 'note_searcher',
      'password_generator_advanced', 'password_vault', 'network_speed_tester', 'network_diagnostic_tool'
    ];
    return batch12ToolNames.includes(tool.name);
  });

  console.log(`✅ 已注册的第十二批工具数量: ${batch12Tools.length}/20\n`);
