...
async function testPhase2RealTools() {
  console.log('========================================');
  console.log('Phase 2 真实功能测试 - 图片处理');
  console.log('========================================\n');
  ...
  try {
    ...
    console.log(`   → 路径: ${testImagePath}`);
    console.log(`   → 尺寸: 800x600`);
    console.log(`   → 大小: ${stats.size} 字节\n`);
  } catch (error) {
    console.log(`   ❌ 创建测试图片失败: ${error.message}\n`);
    process.exit(1);
  }

  // ==================== 测试1: 图片缩放 ====================
  console.log('📝 测试1: 图片缩放\n');
  try {
    ...
    if (result.success) {
      try {
        ...
        console.log(`   → 原始尺寸: ${result.original_dimensions.width}x${result.original_dimensions.height}`);
        console.log(`   → 输出尺寸: ${metadata.width}x${metadata.height}`);
        console.log(`   → 文件大小: ${stats.size} 字节`);
        console.log(`   → 应用操作: ${result.operations_applied.join(', ')}\n`);
  ...
  try {
    ...
    if (result.success) {
      try {
        ...
        console.log('   ✅ 图片裁剪成功!');
        console.log(`   → 输出路径: ${result.output_path}`);
        console.log(`   → 裁剪区域: 100,100 → 400x300`);
        console.log(`   → 输出尺寸: ${metadata.width}x${metadata.height}`);
        console.log(`   → 文件大小: ${stats.size} 字节`);
        console.log(`   → 应用操作: ${result.operations_applied.join(', ')}\n`);
  ...
  try {
    ...
    if (result.success) {
      try {
        ...
        console.log(`   → 输出路径: ${result.output_path}`);
        console.log(`   → 质量: 60%`);
        console.log(`   → 原始大小: ${result.original_dimensions.size} 字节`);
        console.log(`   → 输出大小: ${stats.size} 字节`);
        console.log(`   → 应用操作: ${result.operations_applied.join(', ')}\n`);
      ...
      console.log(`   ❌ 质量调整失败: ${result.error}\n`);
      failedTests++;
  ...
  try {
    ...
    if (result.success) {
      try {
        ...
        console.log('   ✅ 灰度滤镜成功!');
        console.log(`   → 输出路径: ${result.output_path}`);
        console.log(`   → 应用滤镜: ${result.filters_applied.join(', ')}`);
        console.log(`   → 滤镜数量: ${result.filter_count}`);
        console.log(`   → 文件大小: ${stats.size} 字节\n`);
      ...
      console.log(`   ❌ 灰度滤镜失败: ${result.error}\n`);
      failedTests++;
  ...
  try {
    ...
    if (result.success) {
      ...
      console.log(`   ❌ 模糊滤镜失败: ${result.error}\n`);
      failedTests++;
  ...
  try {
    ...
    if (result.success) {
      ...
      console.log(`   ❌ 锐化滤镜失败: ${result.error}\n`);
      failedTests++;
  ...
  try {
    ...
    if (result.success) {
      ...
      console.log(`   ❌ 亮度调整失败: ${result.error}\n`);
      failedTests++;
  ...
  // ==================== 测试10: 组合滤镜 ====================
  console.log('📝 测试10: 组合滤镜（灰度+锐化+归一化）\n');
  try {
    const result = await functionCaller.call('image_filter', {
      input_path: testImagePath,
    ...
    if (result.success) {
      ...
      console.log(`   ❌ 组合滤镜失败: ${result.error}\n`);
      failedTests++;
