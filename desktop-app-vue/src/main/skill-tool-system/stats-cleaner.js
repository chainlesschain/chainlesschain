...
class StatsCleaner {
  ...
  async cleanupUsageLogs() {
    ...
    cutoffDate.setDate(
      cutoffDate.getDate() - this.config.usageLogsRetentionDays,
    );
  ...
  async cleanupExecutionLogs() {
    const cutoffDate = new Date();
    cutoffDate.setDate(
      cutoffDate.getDate() - this.config.executionLogsRetentionDays,
    );
    const cutoffTimestamp = cutoffDate.getTime();
  ...
  async cleanupOldStats() {
    const cutoffDate = new Date();
    cutoffDate.setDate(
      cutoffDate.getDate() - this.config.dailyStatsRetentionDays,
    );
    ...
    logger.info(
      `[StatsCleaner] 清理了 ${skillStatsResult.changes} 条旧技能统计, ${toolStatsResult.changes} 条旧工具统计`,
    );
  }
  ...
  async getCleanupStats() {
    // 统计各表的记录数
    const usageLogsCount = await this.db.get(
      "SELECT COUNT(*) as count FROM skill_tool_usage_logs",
    );
    const executionLogsCount = await this.db.get(
      "SELECT COUNT(*) as count FROM tool_executions",
    );
    const skillStatsCount = await this.db.get(
      "SELECT COUNT(*) as count FROM skill_stats",
    );
    const toolStatsCount = await this.db.get(
      "SELECT COUNT(*) as count FROM tool_stats",
    );
