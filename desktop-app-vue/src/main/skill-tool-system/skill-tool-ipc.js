...
function registerSkillToolIPC({
  ...
  const getAllSkillsHandler = async (event, options = {}) => {
    try {
      ...
      if (result && result.success) {
        logger.info(
          `[IPC] skill:get-all 成功，技能数量: ${result.skills?.length || 0}`,
        );
        return {
          success: true,
          data: result.skills || [],
      ...
      if (Array.isArray(result)) {
        logger.info(
          `[IPC] skill:get-all 成功(数组格式)，技能数量: ${result.length}`,
        );
  ...
  ipcMain.handle(
    "skill:get-stats",
    async (event, skillId, dateRange = null) => {
      try {
  ...
  ipcMain.handle(
    "skill:add-tool",
    async (event, skillId, toolId, role = "primary") => {
      try {
    ...
    try {
      ...
      for (const skill of skills) {
        skillCategories[skill.category] =
          (skillCategories[skill.category] || 0) + 1;
      }

      for (const tool of tools) {
        toolCategories[tool.category] =
          (toolCategories[tool.category] || 0) + 1;
      }
  ...
  ipcMain.handle("skill:recommend", async (event, userInput, options = {}) => {
    try {
      ...
      const recommendations = await global.skillRecommender.recommendSkills(
        userInput,
        options,
      );
  ...
  ipcMain.handle("skill:get-related", async (event, skillId, limit = 5) => {
    try {
      ...
      const related = await global.skillRecommender.getRelatedSkills(
        skillId,
        limit,
      );
  ...
  ipcMain.handle("skill:search", async (event, query, options = {}) => {
    try {
      ...
      const results = await global.skillRecommender.searchSkills(
        query,
        options,
      );
  ...
  ipcMain.handle(
    ...
    async (event, data, filePath, format = "json") => {
      try {
        if (!global.configManager) {
          return { success: false, error: "配置管理器未初始化" };
        }
  ...
  ipcMain.handle(
    ...
    async (event, filePath, options = {}) => {
      try {
        ...
        const result = await global.configManager.importFromFile(
          filePath,
          options,
        );
        return { success: true, data: result };
  ...
  ipcMain.handle(
    ...
    async (event, templateType = "skill") => {
      try {
        if (!global.configManager) {
          return { success: false, error: "配置管理器未初始化" };
        }
