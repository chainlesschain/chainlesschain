...
async function testPhase8RealTools() {
  ...
  results.forEach((result, index) => {
    ...
    if (result.avg_time) {logger.info(`   平均延迟: ${result.avg_time}ms`);}
    if (result.packet_loss !== undefined) {logger.info(`   丢包率: ${result.packet_loss}%`);}
    if (result.ip_count) {logger.info(`   解析IP数: ${result.ip_count}`);}
    if (result.port_status) {logger.info(`   端口状态: ${result.port_status}`);}
    if (result.config_path) {logger.info(`   配置文件: ${result.config_path}`);}
