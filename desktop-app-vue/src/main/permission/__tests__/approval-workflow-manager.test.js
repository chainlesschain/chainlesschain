...
describe("ApprovalWorkflowManager", () => {
  ...
  describe("updateWorkflow", () => {
    ...
    it("should throw on database error", async () => {
      ...
      await expect(
        manager.updateWorkflow("wf1", { name: "New" }),
  ...
  describe("deleteWorkflow", () => {
    ...
    it("should throw on database error", async () => {
      ...
      await expect(manager.deleteWorkflow("wf1")).rejects.toThrow(
        "Delete failed",
      );
    });
  });
  ...
  describe("approveRequest", () => {
    it("should call _processDecision with approve", async () => {
      const spy = vi
        .spyOn(manager, "_processDecision")
  ...
  describe("_processDecision", () => {
    it("should return error when request not found", async () => {
      ...
      const result = await manager._processDecision(
        "req1",
        "did:approver",
        "approve",
        null,
  ...
  describe("getPendingApprovals", () => {
    ...
    it("skips a request with corrupt approvers JSON without breaking the whole list", async () => {
      // Regression: an unguarded JSON.parse(r.approvers) inside the filter used
      // to throw on one corrupt row and (via the outer catch) fail the entire
      // pending-approvals list. The corrupt row is now skipped (fail-closed) and
      // the valid one still returned.
      const allMock = vi.fn().mockReturnValue([
        { id: "bad", approvers: "{not-valid", current_step: 0 },
        {
          id: "good",
          workflow_id: "wf1",
          workflow_name: "Workflow 1",
      ...
      expect(result.success).toBe(true);
      expect(result.requests).toHaveLength(1);
      expect(result.requests[0].id).toBe("good");
    });

    it("tolerates a corrupt request_data on an approvable request", async () => {
      const allMock = vi.fn().mockReturnValue([
        {
          id: "good",
          workflow_id: "wf1",
      ...
      expect(result.success).toBe(true);
      expect(result.requests).toHaveLength(1);
      expect(result.requests[0].requestData).toBeNull();
    });
  });
  ...
  describe("_isAuthorizedApprover", () => {
    ...
    it("should return false for non-matching string approver", () => {
      expect(manager._isAuthorizedApprover("did:user", "did:other")).toBe(
        false,
      );
    });
    ...
    it("should return true when approver is in array", () => {
      expect(
        manager._isAuthorizedApprover("did:user", ["did:user", "did:other"]),
      ).toBe(true);
    });

    it("should return false when approver not in array", () => {
      expect(
        manager._isAuthorizedApprover("did:user", ["did:admin", "did:other"]),
      ).toBe(false);
    });
    ...
    it("should return false for other types", () => {
      expect(manager._isAuthorizedApprover("did:user", { role: "admin" })).toBe(
        false,
      );
    });
  });
