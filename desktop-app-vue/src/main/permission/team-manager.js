...
/**
 * Tolerant JSON column parse — a single team with a corrupt settings string must
 * not throw out of the .map and drop the whole team list. The `x ? JSON.parse(x)
...
function safeParse(raw, fallback) {
  ...
  try {
    ...
    logger.warn(
      `[TeamManager] Bad JSON column, using fallback: ${err.message}`,
    );
    return fallback;
  }
}
    ...
    try {
      ...
      return {
        ...
        teams: teams.map((t) => ({
          ...
          avatar: t.avatar,
          settings: safeParse(t.settings, null),
