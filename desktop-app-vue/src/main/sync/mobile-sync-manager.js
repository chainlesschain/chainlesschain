...
class MobileSyncManager extends EventEmitter {
  ...
  getStats() {
    return {
      ...
      onlineDevicesCount: Array.from(this.mobileDevices.values()).filter(d => d.status === 'online').length
    };
  }
   ...
   */
  getMobileDevices() {
    return Array.from(this.mobileDevices.entries()).map(([deviceId, device]) => ({
      deviceId,
      ...device
    }));
  }
