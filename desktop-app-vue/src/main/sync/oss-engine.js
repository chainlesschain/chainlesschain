/**
 * S3 / OSS 同步业务编排 (Phase 3c follow-up — Phase 3c.3 OSS provider)
 *
 * 与 webdav-engine 在控制流上完全一致：ensureCursor → drain tombstones →
 * push 增量 → updateCursor。唯一差异：
 *   - providerId 默认 "oss"
 *   - log 前缀 [oss-engine]，方便日志区分
 *   - 错误码 412 + 501 都视作 conflict（501 = S3-compat 不支持 IfMatch；
 *     v1 仍 advance cursor 防止每次 sync 重新冲突；用户在 Settings 强制
 *     重推按钮可手动 reset map）
 *
 ...
 *
 * 与 WebDAV engine 重叠度高 → 后续可抽 `external-sync-engine.js` 共享；
 * v1 保持各自独立，让 provider-specific 错误码扩展（如 OSS 501）不影响
 * WebDAV codebase。
 */
 ...
 * @param {Object} deps.client                 — OSSClient instance
 ...
 * @param {string} [deps.providerId="oss"]
 ...
 */
async function runOSSSync(deps) {
  const {
    dbManager,
    client,
    store,
  ...
  // 进度条总数（push pending + KNOWLEDGE_ITEM tombstones）
  totalPending =
  ...
  for (const t of tombstones) {
    ...
    try {
      ...
      logger.error(`[oss-engine] deleteFile 抛异常: ${err?.message}`);
  ...
  pushLoop: while (true) {
    ...
    for (const item of batch) {
      ...
      try {
        ...
        logger.error(`[oss-engine] putFile 抛异常: ${err?.message}`);
        lastError = err?.message || String(err);
        break pushLoop;
      }
      ...
      if (res.ok) {
        ...
        // 12-Factor 推则进 (D7 决策) — 同 WebDAV 412 行为
        skipped++;
        store.updateCursor(
          dbManager,
          providerId,
