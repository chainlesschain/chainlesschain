...
class HealthCheckService {
      ...
      return {
        ...
        autoFix: async () => {
          try {
            ...
            // 验证服务是否启动
            await axios.get(`${process.env.OLLAMA_HOST || 'http://localhost:11434'}/api/tags`);
      ...
      return {
        ...
        autoFix: async () => {
          try {
            ...
            await axios.get(`${process.env.QDRANT_HOST || 'http://localhost:6333'}/collections`);
  ...
  async checkProjectService() {
    try {
      const serviceUrl = process.env.PROJECT_SERVICE_URL || 'http://localhost:9090';
  ...
  async checkUKey() {
    try {
      ...
      return {
        healthy: true,
        message: detected ? 'U-Key detected' : 'U-Key not detected (simulation mode available)',
        details: {
          detected,
