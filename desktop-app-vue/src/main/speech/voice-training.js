...
class VoiceTraining extends EventEmitter {
  constructor(config = {}) {
    ...
    this.config = {
      profilePath:
        config.profilePath ||
        path.join(process.cwd(), "data", "voice-profiles"),
    ...
    try {
      ...
      if (exists) {
        ...
        if (this.userProfile.customVocabulary) {
          this.customVocabulary = new Map(
            Object.entries(this.userProfile.customVocabulary),
          );
        }
        ...
        if (this.userProfile.commandUsageStats) {
          this.commandUsageStats = new Map(
            Object.entries(this.userProfile.commandUsageStats),
          );
        }
        ...
        if (this.userProfile.accentPatterns) {
          this.accentPatterns = new Map(
            Object.entries(this.userProfile.accentPatterns),
          );
        }
  ...
  async saveUserProfile() {
    try {
      ...
      // 转换 Map 为对象
      this.userProfile.customVocabulary = Object.fromEntries(
        this.customVocabulary,
      );
      this.userProfile.commandUsageStats = Object.fromEntries(
        this.commandUsageStats,
      );
  ...
  pruneVocabulary() {
    // 按词频排序
    const sorted = Array.from(this.customVocabulary.entries()).sort(
      (a, b) => b[1] - a[1],
    );
    ...
    logger.info(
      `[VoiceTraining] 词汇表已修剪至 ${this.customVocabulary.size} 个词`,
    );
  }
  ...
  calculateCommandScore(stats) {
    ...
    const recencyScore = Math.exp(
      -(Date.now() - stats.lastUsed) / (7 * 24 * 60 * 60 * 1000),
    ); // 7天衰减
  ...
  async getStats() {
    return {
      totalTranscriptions:
        this.userProfile?.learningStats?.totalTranscriptions || 0,
      averageConfidence:
        this.userProfile?.learningStats?.averageConfidence || 0,
  ...
  async exportProfile() {
    try {
      ...
      const exportPath = path.join(
        this.config.profilePath,
        `${this.currentUserId}_export_${Date.now()}.json`,
      );
      await fs.writeFile(
        exportPath,
