...
class PermissionGate {
  ...
  async verify(auth, method) {
    ...
    try {
      // 1. 基础验证
      if (!auth || !auth.did || !auth.signature || !auth.timestamp || !auth.nonce) {
        logger.warn('[PermissionGate] 验证失败: 认证信息不完整');
        await this.logAudit(auth?.did, method, 0, false, 'Incomplete auth info');
        return false;
      }
      ...
      if (timeDiff > this.options.timestampWindow) {
        logger.warn(`[PermissionGate] 验证失败: 时间戳过期 (差值: ${timeDiff}ms)`);
      ...
      if (deviceLevel < requiredLevel) {
        logger.warn(`[PermissionGate] 验证失败: 权限不足 (需要: ${requiredLevel}, 当前: ${deviceLevel})`);
        await this.logAudit(auth.did, method, requiredLevel, false, `Permission denied (${deviceLevel} < ${requiredLevel})`);
        return false;
      }

      // 8. 频率限制检查
      if (this.options.enableRateLimit) {
        const rateCheck = await this.checkRateLimit(auth.did, method, requiredLevel);
        if (!rateCheck.allowed) {
          logger.warn(`[PermissionGate] 验证失败: 频率限制 (${rateCheck.current}/${rateCheck.limit})`);
          await this.logAudit(auth.did, method, requiredLevel, false, 'Rate limit exceeded');
          return false;
        }
      }

      // 9. Level 4 命令需要 U-Key 验证
      if (requiredLevel === PERMISSION_LEVELS.ROOT && this.options.requireUKeyForLevel4 && this.ukeyManager) {
        const isUKeyValid = await this.verifyUKey();
        if (!isUKeyValid) {
          logger.warn('[PermissionGate] 验证失败: U-Key 验证失败');
          await this.logAudit(auth.did, method, requiredLevel, false, 'U-Key verification failed');
          return false;
        }
      }
      ...
      logger.info(`[PermissionGate] ✅ 验证成功: ${method} by ${auth.did} (${duration}ms)`);
      await this.logAudit(auth.did, method, requiredLevel, true, 'Success');

      return true;
    } catch (error) {
      logger.error('[PermissionGate] 验证异常:', error);
      await this.logAudit(auth?.did, method, 0, false, `Error: ${error.message}`);
      return false;
    }
  }
  ...
  getCommandPermissionLevel(method) {
    ...
    // 4. 默认权限级别
    logger.debug(`[PermissionGate] 命令 ${method} 未配置权限，使用默认级别: ${PERMISSION_LEVELS.NORMAL}`);
  ...
  async getDevicePermissionLevel(did) {
    ...
    try {
      const row = this.database
        .prepare('SELECT permission_level, expires_at FROM device_permissions WHERE did = ?')
        .get(did);
    ...
    // 3. 默认权限级别（新设备）
    logger.info(`[PermissionGate] 新设备使用默认权限: ${did} -> Level ${PERMISSION_LEVELS.PUBLIC}`);
    return PERMISSION_LEVELS.PUBLIC;
  }
  ...
  async setDevicePermissionLevel(did, level, options = {}) {
    try {
      ...
      // 记录审计日志
      await this.logAudit(did, 'device.setPermission', level, true, `Level changed to ${level}`);
  ...
  async checkRateLimit(did, method, permissionLevel) {
    ...
    // 获取限制值
    const limit = permissionLevel >= PERMISSION_LEVELS.ADMIN
    ...
    // 清理过期请求
    rateInfo.requests = rateInfo.requests.filter(timestamp => now - timestamp < windowMs);
  ...
  async loadDevicePermissions() {
    try {
      const rows = this.database
        .prepare('SELECT did, permission_level FROM device_permissions WHERE expires_at IS NULL OR expires_at > ?')
        .all(Date.now());
    ...
    try {
      this.database
        .prepare(`
          ...
          VALUES (?, ?, ?, ?, ?, ?)
        `)
        .run(did || 'unknown', method, permissionLevel, granted ? 1 : 0, reason, Date.now());
    } catch (error) {
  ...
  cleanup() {
    ...
    for (const [did, rateInfo] of this.rateLimitCache.entries()) {
      if (now - rateInfo.lastCleanup > 300000) { // 5 分钟无请求
    ...
    if (expiredNonces > 0 || cleanedRateLimits > 0) {
      logger.debug(`[PermissionGate] 清理: ${expiredNonces} nonces, ${cleanedRateLimits} rate limits`);
    }
  }
