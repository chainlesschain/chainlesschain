...
class CommandLogger extends EventEmitter {
  ...
  log(logEntry) {
    ...
    try {
      ...
      if (!requestId || !deviceDid || !namespace || !action) {
        throw new Error(
          "Missing required fields: requestId, deviceDid, namespace, action",
        );
      }
      ...
      logger.debug(
        `[CommandLogger] 日志已记录: ${namespace}.${action} (ID: ${info.lastInsertRowid})`,
      );
  ...
  query(options = {}) {
    ...
    try {
      ...
      // 获取总数
      let countQuery =
        "SELECT COUNT(*) as total FROM remote_command_logs WHERE 1=1";
      const countParams = [];
  ...
  cleanup() {
    try {
      ...
      const { total } = this.database
        .prepare("SELECT COUNT(*) as total FROM remote_command_logs")
        .get();
  ...
  clearAll() {
    try {
      const result = this.database
        .prepare("DELETE FROM remote_command_logs")
        .run();
   ...
   */
  getStats(options = {}) {
    const { deviceDid = null, startTime = null, endTime = null } = options;
    ...
    try {
      ...
      const { total } = this.database
        .prepare(
          `SELECT COUNT(*) as total FROM remote_command_logs ${whereClause}`,
        )
        .get(...params);
      ...
      const statusStats = this.database
        .prepare(
          `SELECT status, COUNT(*) as count FROM remote_command_logs ${whereClause} GROUP BY status`,
        )
        .all(...params);
      ...
      const levelStats = this.database
        .prepare(
          `SELECT level, COUNT(*) as count FROM remote_command_logs ${whereClause} GROUP BY level`,
        )
        .all(...params);
      ...
      const namespaceStats = this.database
        .prepare(
          `SELECT command_namespace, COUNT(*) as count FROM remote_command_logs ${whereClause} GROUP BY command_namespace`,
        )
        .all(...params);
      ...
      const { avgDuration } = this.database
        .prepare(
          `SELECT AVG(duration) as avgDuration FROM remote_command_logs ${whereClause} AND duration IS NOT NULL`,
        )
        .get(...params);
      ...
      return {
        ...
        byNamespace: namespaceStats.reduce(
