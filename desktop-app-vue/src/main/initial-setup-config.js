...
class InitialSetupConfig {
  ...
  async applyToSystem(appConfig, llmConfig, database) {
    ...
    if (this.config.paths?.projectRoot) {
      await database.setSetting('project.rootPath', this.config.paths.projectRoot);
    }
    ...
    if (this.config.edition === 'enterprise' && this.config.enterprise) {
      await database.setSetting('enterprise.serverUrl', this.config.enterprise.serverUrl);
      await database.setSetting('enterprise.tenantId', this.config.enterprise.tenantId);
      // API Key 应加密存储
      if (this.config.enterprise.apiKey) {
        await database.setSetting('enterprise.apiKey', this.config.enterprise.apiKey);
      }
    }
