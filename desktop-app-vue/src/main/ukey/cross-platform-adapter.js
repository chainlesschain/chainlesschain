...
class CrossPlatformAdapter extends EventEmitter {
  constructor(config = {}) {
    ...
    this.driverPreference = config.driverPreference || this.getDefaultDriverPreference();
  }
  ...
  getDefaultDriverPreference() {
    switch (this.platform) {
      case Platforms.WINDOWS:
        return ['xinjinke', 'feitian', 'watchdata', 'huada', 'tdr', 'pkcs11', 'simulated'];
  ...
  checkWindowsDriverDLL(driverType) {
    ...
    const searchPaths = [
      ...
      path.join(process.env.ProgramFiles || 'C:\\Program Files', driverType, dllName),
      path.join(process.env['ProgramFiles(x86)'] || 'C:\\Program Files (x86)', driverType, dllName),
  ...
  getPlatformInfo() {
    return {
      ...
      isHardwareSupported: this.platform === Platforms.WINDOWS || this.availableDrivers.includes('pkcs11')
    };
  }
