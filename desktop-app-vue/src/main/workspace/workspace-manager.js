...
class WorkspaceManager {
  ...
  async getWorkspace(workspaceId) {
    try {
      const workspace = this.db
        .prepare("SELECT * FROM organization_workspaces WHERE id = ?")
        .get(workspaceId);
  ...
  async permanentDeleteWorkspace(workspaceId, deleterDID) {
    try {
      ...
      // 5. 删除工作区任务
      this.db
        .prepare("DELETE FROM workspace_tasks WHERE workspace_id = ?")
        .run(workspaceId);

      // 6. 删除工作区
      this.db
        .prepare("DELETE FROM organization_workspaces WHERE id = ?")
        .run(workspaceId);
