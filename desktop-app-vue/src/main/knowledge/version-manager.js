...
class KnowledgeVersionManager {
  ...
  getVersionHistory(knowledgeId, limit = 50) {
    try {
      ...
      return versions.map(v => ({
        ...v,
        content_snapshot: v.content_snapshot ? JSON.parse(v.content_snapshot) : null,
  ...
  getVersion(versionId) {
    try {
      ...
      return {
        ...version,
        content_snapshot: version.content_snapshot ? JSON.parse(version.content_snapshot) : null,
  ...
  async restoreVersion(knowledgeId, versionId, restoredBy) {
    try {
      ...
      // 获取当前知识状态
      const currentKnowledge = this.db.prepare(`
        SELECT * FROM knowledge_items WHERE id = ?
      `).get(knowledgeId);
  ...
  pruneOldVersions(knowledgeId, keepCount = 50) {
    try {
      // 获取版本总数
      const totalVersions = this.db.prepare(`
