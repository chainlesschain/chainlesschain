...
class MLToolMatcher {
  ...
  async recommendTools(task, userId) {
    try {
      // 1. 提取特征
      const features = await this.featureExtractor.extractFeatures(task, userId);
      ...
      this.stats.avgConfidence =
        (this.stats.avgConfidence * (this.stats.totalRecommendations - 1) +
          (recommendations[0]?.confidence || 0)) / this.stats.totalRecommendations;
  ...
  generateExplanation(recommendation, features) {
    ...
    if (recommendation.breakdown.historicalSuccess > 0.7) {
      reasons.push(`历史成功率${(recommendation.breakdown.historicalSuccess * 100).toFixed(0)}%`);
    }
  ...
  getStats() {
    return {
      ...
      acceptanceRate: this.stats.totalRecommendations > 0
        ? (this.stats.acceptedRecommendations / this.stats.totalRecommendations * 100).toFixed(2) + '%'
        : '0%'
    };
  }
