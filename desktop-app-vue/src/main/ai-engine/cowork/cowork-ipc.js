...
function registerCoworkIPC(dependencies = {}) {
   ...
   * 创建团队
   */
  ipcMain.handle('cowork:create-team', async (event, { teamName, config = {} }) => {
    try {
   ...
   * 请求加入团队
   */
  ipcMain.handle('cowork:request-join', async (event, { teamId, agentId, agentInfo = {} }) => {
    try {
      logger.info(`[Cowork IPC] 代理 ${agentId} 请求加入团队 ${teamId}`);
      const result = await teammateTool.requestJoin(teamId, agentId, agentInfo);
   ...
   * 分配任务
   */
  ipcMain.handle('cowork:assign-task', async (event, { teamId, agentId, task }) => {
    try {
   ...
   * 广播消息
   */
  ipcMain.handle('cowork:broadcast-message', async (event, { teamId, fromAgent, message }) => {
    try {
      const result = await teammateTool.broadcastMessage(teamId, fromAgent, message);
      return { success: true, result };
    } catch (error) {
   ...
   * 发送消息
   */
  ipcMain.handle('cowork:send-message', async (event, { fromAgent, toAgent, message }) => {
    try {
      const result = await teammateTool.sendMessage(fromAgent, toAgent, message);
      return { success: true, result };
    } catch (error) {
   ...
   * 投票决策
   */
  ipcMain.handle('cowork:vote-on-decision', async (event, { teamId, decision, votes }) => {
    try {
      logger.info(`[Cowork IPC] 投票决策: 团队 ${teamId}`);
      const result = await teammateTool.voteOnDecision(teamId, decision, votes);
      return { success: true, result };
    } catch (error) {
   ...
   * 终止代理
   */
  ipcMain.handle('cowork:terminate-agent', async (event, { agentId, reason = '' }) => {
    try {
   ...
   * 合并结果
   */
  ipcMain.handle('cowork:merge-results', async (event, { teamId, results, strategy = {} }) => {
    try {
      logger.info(`[Cowork IPC] 合并结果: 团队 ${teamId}, 策略: ${strategy.type || 'aggregate'}`);
      const mergedResult = await teammateTool.mergeResults(teamId, results, strategy);
   ...
   * 创建检查点
   */
  ipcMain.handle('cowork:create-checkpoint', async (event, { teamId, metadata = {} }) => {
    try {
      const checkpoint = await teammateTool.createCheckpoint(teamId, metadata);
   ...
   * 更新团队配置
   */
  ipcMain.handle('cowork:update-team-config', async (event, { teamId, config }) => {
    try {
   ...
   * 请求文件访问权限
   */
  ipcMain.handle('cowork:request-file-access', async (event, { teamId, folderPath, permissions = ['read'], options = {} }) => {
    try {
      logger.info(`[Cowork IPC] 请求文件访问: 团队 ${teamId}, 路径 ${folderPath}`);
      ...
      if (mainWindow && !options.autoApprove) {
        return new Promise((resolve) => {
          ...
          ipcMain.once(responseChannel, async (responseEvent, { approved }) => {
            if (approved) {
              await fileSandbox.grantAccess(teamId, folderPath, permissions, options);
              resolve({ success: true, granted: true });
            } else {
              resolve({ success: true, granted: false, reason: 'user_denied' });
            }
          });
      ...
      // 自动批准模式
      const granted = await fileSandbox.requestAccess(teamId, folderPath, permissions, options);
   ...
   * 授予文件访问权限
   */
  ipcMain.handle('cowork:grant-file-access', async (event, { teamId, folderPath, permissions, options = {} }) => {
    try {
      await fileSandbox.grantAccess(teamId, folderPath, permissions, options);
      return { success: true };
    } catch (error) {
   ...
   * 撤销文件访问权限
   */
  ipcMain.handle('cowork:revoke-file-access', async (event, { teamId, folderPath }) => {
    try {
      logger.info(`[Cowork IPC] 撤销文件访问: 团队 ${teamId}, 路径 ${folderPath}`);
   ...
   * 验证文件访问
   */
  ipcMain.handle('cowork:validate-file-access', async (event, { teamId, filePath, permission = 'read' }) => {
    try {
      const validation = await fileSandbox.validateAccess(teamId, filePath, permission);
   ...
   * 读取文件
   */
  ipcMain.handle('cowork:read-file', async (event, { teamId, agentId, filePath, options = {} }) => {
    try {
      const content = await fileSandbox.readFile(teamId, agentId, filePath, options);
   ...
   * 写入文件
   */
  ipcMain.handle('cowork:write-file', async (event, { teamId, agentId, filePath, content, options = {} }) => {
    try {
      await fileSandbox.writeFile(teamId, agentId, filePath, content, options);
      return { success: true };
    } catch (error) {
   ...
   * 删除文件
   */
  ipcMain.handle('cowork:delete-file', async (event, { teamId, agentId, filePath }) => {
    try {
   ...
   * 列出目录
   */
  ipcMain.handle('cowork:list-directory', async (event, { teamId, agentId, dirPath }) => {
    try {
   ...
   * 获取审计日志
   */
  ipcMain.handle('cowork:get-audit-log', async (event, { filters = {}, limit = 100 }) => {
    try {
   ...
   * 取消任务
   */
  ipcMain.handle('cowork:cancel-task', async (event, { taskId, reason = '' }) => {
    try {
   ...
   * 从检查点恢复
   */
  ipcMain.handle('cowork:restore-from-checkpoint', async (event, { checkpointId }) => {
    try {
   ...
   * 执行技能
   */
  ipcMain.handle('cowork:execute-skill', async (event, { skillId, task, context = {} }) => {
    try {
   ...
   * 自动执行任务（选择最佳技能）
   */
  ipcMain.handle('cowork:auto-execute-task', async (event, { task, context = {} }) => {
    try {
   ...
   * 查找适合任务的技能
   */
  ipcMain.handle('cowork:find-skills-for-task', async (event, { task, options = {} }) => {
    try {
  ...
  ipcMain.handle('cowork:get-analytics', async (event, data) => {
    try {
      ...
      const activeAgents = database.prepare(activeAgentsQuery).get(...activeAgentsParams).count;
      ...
      const totalAgents = database.prepare(totalAgentsQuery).get(...totalAgentsParams).count;
      ...
      const avgExecutionTime = completedTaskTimes.length > 0
      ...
      const previousTasks = database.prepare(`
  ...
  logger.info('[Cowork IPC] ✓ 所有 IPC 处理器已注册（45 个处理器）');
