  ...
  async execute(task, context = {}, skill) {
    ...
    try {
      switch (parsed.action) {
        case "create": return handleCreate(parsed, context);
        case "list": return handleList(context);
        case "remove": return handleRemove(parsed.target, context);
        case "status": return handleStatus(context);
        case "prune": return handlePrune(context);
        default:
...
function parseInput(input) {
  if (!input || typeof input !== "string") return { action: "list" };
...
function git(cmd, cwd) {
  return execSync(`git ${cmd}`, { cwd: cwd || process.cwd(), encoding: "utf8", timeout: 30000 }).trim();
}

function handleCreate(parsed, context) {
  const branch = parsed.target;
  if (!branch) return { success: false, error: "No branch name provided." };

  const cwd = context.cwd || process.cwd();
  const worktreePath = parsed.customPath || path.join(cwd, "..", `worktree-${branch.replace(/\//g, "-")}`);
...
function handleRemove(target, context) {
  if (!target) return { success: false, error: "No worktree path provided." };
  const cwd = context.cwd || process.cwd();
  git(`worktree remove "${target}"`, cwd);
  return { success: true, action: "remove", path: target, message: `Worktree "${target}" removed.` };
}
...
function handleStatus(context) {
  ...
  const statuses = (list.worktrees || []).map((wt) => {
    try {
      ...
      return { ...wt, changes: status ? status.split("\n").length : 0, clean: !status };
    } catch {
...
function handlePrune(context) {
  ...
  return { success: true, action: "prune", output, message: "Stale worktrees pruned." };
}
