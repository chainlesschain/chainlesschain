...
const https = createBundledSkillHttpsClient("news-monitor");
...
const BUILTIN_SOURCES = {
  ...
  reddit: {
    name: "Reddit",
    baseUrl: "https://www.reddit.com/r/{sub}/hot.json?limit=25",
    type: "api",
  },
  github: {
    name: "GitHub Trending",
    url: "https://api.github.com/search/repositories?q=created:>{date}&sort=stars&per_page=20",
    type: "api",
  },
};
  ...
  async execute(task, context = {}, skill) {
    ...
    try {
      switch (parsed.action) {
        case "watch":
          return handleWatch(parsed.keywords, parsed.options);
        case "digest":
          return handleDigest(parsed.options);
        case "trends":
          return handleTrends(parsed.options);
        case "source":
          return handleSource(parsed.subAction, parsed.target);
        case "fetch":
          return await handleFetch(parsed.keywords);
        default:
...
function parseInput(input) {
  if (!input || typeof input !== "string") {
    return { action: "digest", keywords: [], options: {} };
  }
...
function handleWatch(keywords, options) {
  if (!keywords.length) {
    return {
      ...
      message: `Active watches: ${state.watches.length}`,
    };
  }
...
function handleSource(subAction, target) {
  switch (subAction) {
    case "add":
      if (!target) {
        return { success: false, error: "Provide a URL to add." };
      }
    ...
    case "remove":
      ...
      return {
        ...
        subAction: "remove",
        url: target,
        message: `Removed source: ${target}`,
      };

    case "list":
    default: {
      const all = [
        ...Object.entries(BUILTIN_SOURCES).map(([k, v]) => ({
          name: v.name,
          key: k,
          builtin: true,
        })),
        ...state.sources.map((s) => ({ url: s.url, builtin: false })),
      ];
      return {
        success: true,
        action: "source",
        subAction: "list",
        sources: all,
        message: `${all.length} source(s) available.`,
      };
    }
  }
}
...
function fetchJSON(url) {
  return new Promise((resolve, reject) => {
    https
      .get(url, { headers: { "User-Agent": "ChainlessChain/1.0" } }, (res) => {
        let data = "";
