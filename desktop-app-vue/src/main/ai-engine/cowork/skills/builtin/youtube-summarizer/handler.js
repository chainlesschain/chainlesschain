  ...
  async execute(task, context = {}, skill) {
    ...
    try {
      switch (parsed.action) {
        case "summarize":
          return await handleSummarize(parsed.videoId, parsed.options);
        case "transcript":
          return await handleTranscript(parsed.videoId, parsed.options);
        case "chapters":
          return await handleChapters(parsed.videoId, parsed.options);
        default:
          return await handleSummarize(parsed.videoId, parsed.options);
      }
    } catch (error) {
...
function parseInput(input) {
  if (!input || typeof input !== "string") {
    return { action: "summarize", videoId: null, options: {} };
  }
  ...
  return {
    action,
    videoId,
    options: { lang: langMatch ? langMatch[1] : "en" },
  };
}
...
async function handleTranscript(videoId, options) {
  ...
  if (!transcript.length) {
    return {
      success: false,
      error: `No transcript found for video ${videoId}. Captions may be disabled.`,
    };
  }
...
async function handleSummarize(videoId, options) {
  ...
  if (!transcript.length) {
    return {
      success: false,
      error: `No transcript found for video ${videoId}.`,
    };
  }
  ...
  for (const entry of transcript) {
    if (entry.start - current.start >= segmentDuration && current.text) {
      ...
      current = {
        start: entry.start,
        text: "",
        startFormatted: formatTime(entry.start),
      };
    }
...
function fetchText(url) {
  return new Promise((resolve, reject) => {
    ...
    https
      .get(url, { headers: { "User-Agent": "Mozilla/5.0" } }, handler)
      .on("error", reject);
  });
}
