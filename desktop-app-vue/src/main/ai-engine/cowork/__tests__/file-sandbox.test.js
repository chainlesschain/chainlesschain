...
describe("FileSandbox", () => {
  ...
  describe("grantAccess", () => {
    test("应该成功授予访问权限", async () => {
      await sandbox.grantAccess("team-1", testDir, [
        Permission.READ,
        Permission.WRITE,
      ]);
  ...
  describe("revokeAccess", () => {
    test("应该成功撤销访问权限", async () => {
      ...
      expect(sandbox.hasPermission("team-1", testDir, Permission.READ)).toBe(
        true,
      );
  ...
  describe("hasPermission", () => {
    ...
    test("应该检查权限", () => {
      ...
      expect(sandbox.hasPermission("team-1", testDir, Permission.WRITE)).toBe(
        true,
      );
      expect(sandbox.hasPermission("team-1", testDir, Permission.EXECUTE)).toBe(
        false,
      );
    });
    ...
    test("应该检查子路径权限", () => {
      ...
      expect(sandbox.hasPermission("team-1", subPath, Permission.READ)).toBe(
        true,
      );
    });

    test("应该拒绝未授权的团队", () => {
      expect(sandbox.hasPermission("team-2", testDir, Permission.READ)).toBe(
        false,
      );
    });
  });
  ...
  describe("checkPathSafety", () => {
    test("应该检测路径遍历攻击", () => {
      // 使用无法被 path.normalize 完全解析的路径
      // 注意：path.join 会规范化路径，所以直接使用字符串构建
      const dangerousPath =
        testDir +
        path.sep +
        ".." +
      ...
      // 如果路径被完全解析（不再包含 ..），则检查是否超出了授权范围
      // 如果仍包含 ..，则应该返回 path_traversal
      // 在 Windows 上，深层的 .. 可能被解析掉
      if (path.normalize(dangerousPath).includes("..")) {
        expect(result.safe).toBe(false);
        expect(result.reason).toBe("path_traversal");
      } else {
        // 路径被规范化后不含 ..，但应该检查其他安全性
        // 此时这个测试验证的是 path.normalize 的行为，而非沙箱检测
  ...
  describe("validateAccess", () => {
    ...
    test("应该验证合法访问", async () => {
      ...
      const result = await sandbox.validateAccess(
        "team-1",
        filePath,
        Permission.READ,
      );
    ...
    test("应该拒绝不安全的路径", async () => {
      ...
      // 路径遍历或权限不足都是有效的拒绝原因
      expect(["path_traversal", "insufficient_permission"]).toContain(
        result.reason,
      );
    });
  ...
  describe("readFile", () => {
    ...
    test("应该记录审计日志", async () => {
      ...
      const result = sandbox.getAuditLog({
        teamId: "team-1",
        operation: "read",
      });
  ...
  describe("getAuditLog", () => {
    ...
    test("应该按团队过滤", () => {
      const result = sandbox.getAuditLog({ teamId: "team-1" });
      const logs = result.logs || result;
    ...
    test("应该按代理过滤", () => {
      const result = sandbox.getAuditLog({ agentId: "agent-1" });
      const logs = result.logs || result;
    ...
    test("应该按操作类型过滤", () => {
      const result = sandbox.getAuditLog({ operation: "read" });
      const logs = result.logs || result;
  ...
  describe("isDangerousOperation", () => {
    ...
    test("应该允许安全操作", () => {
      ...
      expect(sandbox.isDangerousOperation("copy file1.txt file2.txt")).toBe(
        false,
      );
    });
  });
