...
`;
        ...
        case "ml":
          await fs.mkdir(path.join(projectPath, "data", "raw"), {
            recursive: true,
          });
          await fs.mkdir(path.join(projectPath, "data", "processed"), {
            recursive: true,
          });
          await fs.mkdir(path.join(projectPath, "notebooks"), {
            recursive: true,
          });
        ...
        default: // package or script
          await fs.mkdir(path.join(projectPath, projectName), {
            recursive: true,
          });
...
`;
            await fs.writeFile(
              path.join(projectPath, projectName, "__init__.py"),
              initPy,
              "utf-8",
            );
...
`;
    ...
    try {
      ...
      if (autoDetect) {
        ...
        try {
          ...
          logger.warn(
            "[Requirements Generator] 自动检测失败，使用手动指定的包",
          );
        }
      }
  ...
  getGitignoreTemplate(template) {
    const templates = {
      ...
      VisualStudioCode: ["# VSCode", ".vscode/", "*.code-workspace"],
      JetBrains: ["# JetBrains", ".idea/", "*.iml"],
      macOS: ["# macOS", ".DS_Store", ".AppleDouble", ".LSOverride"],
      Windows: ["# Windows", "Thumbs.db", "ehthumbs.db", "Desktop.ini"],
    };
  ...
  register(functionCaller) {
    functionCaller.registerTool(
      ...
      this.tool_npm_project_setup.bind(this),
    );
    functionCaller.registerTool(
      "tool_package_json_builder",
      this.tool_package_json_builder.bind(this),
    );
    functionCaller.registerTool(
      "tool_python_project_setup",
      this.tool_python_project_setup.bind(this),
    );
    functionCaller.registerTool(
      "tool_requirements_generator",
      this.tool_requirements_generator.bind(this),
    );
    functionCaller.registerTool(
      "tool_dockerfile_generator",
      this.tool_dockerfile_generator.bind(this),
    );
    functionCaller.registerTool(
      "tool_gitignore_generator",
      this.tool_gitignore_generator.bind(this),
    );
