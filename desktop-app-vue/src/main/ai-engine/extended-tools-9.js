  ...
  static registerAll(functionCaller) {
    ...
    functionCaller.registerTool("irrigation_controller", async (params) => {
      ...
      if (action === "optimize") {
        ...
        return {
          ...
          water_usage: {
            daily: optimal_schedule.reduce(
              (sum, s) => sum + (s.duration * s.flow_rate) / 60,
              0,
            ),
            unit: "liters",
          },
        ...
        trajectory.push({
          ...
          position: {
            ...
            z:
              ...
              Math.sin((M_current * Math.PI) / 180),
          },
        });
      }
    ...
    functionCaller.registerTool("flight_planner", async (params) => {
      ...
      const distance = waypoints.reduce((sum, wp, i) => {
        ...
        return (
          sum +
          Math.sqrt((wp.lat - prev.lat) ** 2 + (wp.lon - prev.lon) ** 2) * 111
        ); // 近似km
      }, 0);
    ...
    functionCaller.registerTool("power_dispatcher", async (params) => {
      ...
      const total_emissions = dispatch_plan.reduce(
        (sum, d) => sum + d.emissions,
        0,
      );
