  ...
  static registerAll(functionCaller) {
    ...
    functionCaller.registerTool(
      ...
      },
      {
        ...
        description: '加密解密',
        parameters: { action: { type: 'string' }, data: { type: 'string' }, algorithm: { type: 'string' }, key: { type: 'string' } }
      }
    );
    ...
    functionCaller.registerTool(
      ...
      async (params) => {
        try {
          const { action, data, privateKey, publicKey, signature, algorithm = 'RSA-SHA256' } = params;
      ...
      async (params) => {
        try {
          ...
          if (type === 'symmetric') {
            ...
            return {
              success: true,
              key: format === 'hex' ? key.toString('hex') : key.toString('base64')
            };
          } else if (type === 'rsa') {
          ...
          for (const type of analysis) {
            switch (type) {
              ...
              case 'anomaly': {
                ...
                results.anomalies = data.filter(d =>
      ...
      async (params) => {
        try {
          ...
          for (let i = window; i < data.length; i++) {
            const prev = data.slice(i - window, i).reduce((a, b) => a + b, 0) / window;
            const curr = data[i];
    ...
    functionCaller.registerTool(
      ...
      async (params) => {
        try {
          const { directory, recursive = true, filter, debounce = 500 } = params;
    ...
    functionCaller.registerTool(
      ...
      async (params) => {
        try {
          const { action, cronExpression, taskId, task, timezone = 'UTC' } = params;
    ...
    functionCaller.registerTool(
      ...
      },
      {
        ...
        description: '条形码生成',
        parameters: { data: { type: 'string' }, format: { type: 'string' }, outputPath: { type: 'string' } }
      }
    );
    ...
    functionCaller.registerTool(
      ...
      async (params) => {
        try {
          const { action, address, latitude, longitude, provider = 'google' } = params;
    ...
    functionCaller.registerTool(
      ...
      async (params) => {
        try {
          const { point1, point2, algorithm = 'haversine', unit = 'km' } = params;
    ...
    functionCaller.registerTool(
      ...
      },
      {
        ...
        description: '坐标转换',
        parameters: { latitude: { type: 'number' }, longitude: { type: 'number' }, from: { type: 'string' }, to: { type: 'string' } }
      }
    );
    ...
    functionCaller.registerTool(
      ...
      },
      {
        name: 'video_editor',
        description: '视频编辑',
        parameters: { action: { type: 'string' }, inputPath: { type: 'string' }, outputPath: { type: 'string' } }
      }
    );
    ...
    functionCaller.registerTool(
      ...
      },
      {
        ...
        description: '视频转码',
        parameters: { inputPath: { type: 'string' }, outputPath: { type: 'string' } }
      }
    );
    ...
    functionCaller.registerTool(
      ...
      },
      {
        ...
        description: '视频截图',
        parameters: { videoPath: { type: 'string' }, outputPath: { type: 'string' } }
      }
    );
