...
class FeatureExtractor {
  constructor(config = {}) {
    ...
    this.categoryKeywords = {
      development: ['代码', '函数', '类', '变量', 'code', 'function', 'class', 'variable', 'develop'],
      data: ['数据', '分析', '图表', '统计', 'data', 'analysis', 'chart', 'statistics'],
      design: ['设计', 'UI', 'UX', '界面', '样式', 'design', 'style', 'layout'],
      writing: ['文档', '写作', '文章', 'document', 'write', 'article', 'markdown'],
  ...
  extractTextFeatures(text) {
    ...
    // TF-IDF特征
    const tfidf = this.config.enableTFIDF
      ? this.calculateTFIDF(tokens)
      : null;
  ...
  categoryToNumber(category) {
    const categories = ['development', 'data', 'design', 'writing', 'testing', 'deployment', 'general'];
  ...
  fileTypeToNumber(fileType) {
    const hash = fileType.split('').reduce((sum, c) => sum + c.charCodeAt(0), 0);
    return (hash % 100) / 100;
  }
  ...
  experienceToNumber(experience) {
    const map = { novice: 0.2, beginner: 0.4, intermediate: 0.6, advanced: 0.8, expert: 1.0 };
