      ...
      <div v-if="linkDetail" class="link-detail">
        ...
        <a-descriptions title="基本信息" bordered :column="2">
          ...
          <a-descriptions-item label="使用情况">
            {{ linkDetail.used_count }} / {{ linkDetail.max_uses }} (剩余
            {{ linkDetail.remainingUses }})
          </a-descriptions-item>
          ...
          <a-descriptions-item label="过期时间">
            {{
              linkDetail.expires_at
                ? formatDate(linkDetail.expires_at)
                : "永不过期"
            }}
          </a-descriptions-item>
          <a-descriptions-item label="最后使用">
            {{
              linkDetail.last_used_at
                ? formatDate(linkDetail.last_used_at)
                : "未使用"
            }}
          </a-descriptions-item>
          <a-descriptions-item
        ...
        <a-card title="使用记录" size="small" style="margin-top: 16px">
          ...
          <a-empty
            v-if="
              !linkDetail.usageRecords || linkDetail.usageRecords.length === 0
            "
            description="暂无使用记录"
          />
        </a-card>
        ...
        <a-card
          v-if="
            linkDetail.metadata && Object.keys(linkDetail.metadata).length > 0
          "
          title="元数据"
          size="small"
