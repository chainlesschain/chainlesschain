      ...
      <a-tab-pane key="export" tab="导出配置">
        <a-space direction="vertical" style="width: 100%" :size="16">
          ...
          <a-space>
            ...
            <a-button :disabled="!exportPreview" @click="handleCopyToClipboard">
              <CopyOutlined /> 复制到剪贴板
            </a-button>
          </a-space>
        </a-space>
      ...
      <a-tab-pane key="import" tab="导入配置">
        <a-space direction="vertical" style="width: 100%" :size="16">
          ...
          <a-card v-if="importMethod === 'file'" title="选择文件" size="small">
            ...
            >
              <a-button> <UploadOutlined /> 选择配置文件 </a-button>
            </a-upload>
          </a-card>
          ...
          <a-card v-if="importResult" title="导入结果" size="small">
            ...
            >
              <a-collapse-panel header="查看跳过的项">
                <a-list
                  :data-source="[
                    ...importResult.skipped.skills,
                    ...importResult.skipped.tools,
                  ]"
                  size="small"
                >
...
const loadAvailableItems = async () => {
  try {
    // 加载技能
    const skillsResult = await window.electron.ipcRenderer.invoke(
      "skill:get-all",
      {},
    );
    ...
    // 加载工具
    const toolsResult = await window.electron.ipcRenderer.invoke(
      "tool:get-all",
      {},
    );
  ...
  try {
    ...
    if (exportType.value === "skills") {
      ...
      result = await window.electron.ipcRenderer.invoke(
        ...
        {
          includeBuiltin: exportOptions.value.includeBuiltin,
...
const handleImport = async () => {
  ...
  try {
    ...
    if (result.success) {
      ...
      message.success(
        `导入完成: ${result.data.imported.skills} 个技能, ${result.data.imported.tools} 个工具`,
      );
