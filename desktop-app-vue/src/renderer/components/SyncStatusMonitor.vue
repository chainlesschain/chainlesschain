...
const syncStatusText = computed(() => {
  if (stats.value.conflicts > 0) {
    return `${stats.value.conflicts} 冲突`;
  }
  if (stats.value.pending > 0) {
    return `${stats.value.pending} 待同步`;
  }
  return "已同步";
});
...
async function handleSyncNow() {
  ...
  try {
    ...
    if (result.success) {
      message.success(
        `同步完成: 应用 ${result.applied} 项, 推送 ${result.pushed} 项`,
      );
