    ...
    >
      ...
      <div v-if="currentStep === 0">
        <a-row :gutter="[16, 16]">
          ...
          >
            ...
            >
              ...
              <a-tag :color="getCategoryColor(template.category)" style="margin-top: 8px">
        ...
        <div style="text-align: right; margin-top: 16px">
          <a-button type="primary" :disabled="!selectedTemplate" @click="nextStep">
            下一步
          </a-button>
        </div>
      </div>
      ...
      <div v-if="currentStep === 1 && selectedTemplate">
        ...
        <a-form layout="vertical">
          ...
          <template v-if="selectedTemplate.id === 'simple_trade'">
            <a-form-item label="买家 DID" required>
              <a-input v-model:value="formData.buyerDid" placeholder="输入买家 DID" />
            </a-form-item>
            <a-form-item label="卖家 DID" required>
              <a-input v-model:value="formData.sellerDid" placeholder="输入卖家 DID" />
            </a-form-item>
            <a-form-item label="资产 ID" required>
              <a-input v-model:value="formData.assetId" placeholder="输入资产 ID" />
            </a-form-item>
            <a-form-item label="资产名称" required>
              <a-input v-model:value="formData.assetName" placeholder="输入资产名称" />
            </a-form-item>
            <a-form-item label="数量" required>
              <a-input-number v-model:value="formData.quantity" :min="1" style="width: 100%" />
            </a-form-item>
            <a-form-item label="支付资产 ID" required>
              <a-input v-model:value="formData.paymentAssetId" placeholder="输入支付资产 ID" />
            </a-form-item>
            <a-form-item label="支付金额" required>
              <a-input-number v-model:value="formData.paymentAmount" :min="0" style="width: 100%" />
            </a-form-item>
            <a-form-item label="交付天数">
              <a-input-number v-model:value="formData.deliveryDays" :min="1" :max="365" style="width: 100%" />
            </a-form-item>
          </template>

          <!-- 订阅付费合约 -->
          <template v-if="selectedTemplate.id === 'subscription'">
            <a-form-item label="订阅者 DID" required>
              <a-input v-model:value="formData.subscriberDid" placeholder="输入订阅者 DID" />
            </a-form-item>
            <a-form-item label="创作者 DID" required>
              <a-input v-model:value="formData.creatorDid" placeholder="输入创作者 DID" />
            </a-form-item>
            <a-form-item label="服务名称" required>
              <a-input v-model:value="formData.serviceName" placeholder="输入服务名称" />
            </a-form-item>
            <a-form-item label="支付资产 ID" required>
              <a-input v-model:value="formData.paymentAssetId" placeholder="输入支付资产 ID" />
            </a-form-item>
            <a-form-item label="月费" required>
              <a-input-number v-model:value="formData.monthlyPrice" :min="0" style="width: 100%" />
            </a-form-item>
            <a-form-item label="订阅月数">
              <a-input-number v-model:value="formData.durationMonths" :min="1" :max="12" style="width: 100%" />
            </a-form-item>
          ...
          <template v-if="selectedTemplate.id === 'bounty'">
            <a-form-item label="发布者 DID" required>
              <a-input v-model:value="formData.publisherDid" placeholder="输入发布者 DID" />
            </a-form-item>
            <a-form-item label="任务标题" required>
              <a-input v-model:value="formData.taskTitle" placeholder="输入任务标题" />
            </a-form-item>
            <a-form-item label="任务描述" required>
              <a-textarea v-model:value="formData.taskDescription" :rows="4" placeholder="详细描述任务要求..." />
            </a-form-item>
            <a-form-item label="支付资产 ID" required>
              <a-input v-model:value="formData.paymentAssetId" placeholder="输入支付资产 ID" />
            </a-form-item>
            <a-form-item label="赏金金额" required>
              <a-input-number v-model:value="formData.rewardAmount" :min="0" style="width: 100%" />
            </a-form-item>
            <a-form-item label="截止天数">
              <a-input-number v-model:value="formData.deadlineDays" :min="1" :max="365" style="width: 100%" />
            </a-form-item>
            <a-form-item label="所需批准数">
              <a-input-number v-model:value="formData.requiredApprovals" :min="1" :max="10" style="width: 100%" />
            </a-form-item>
          </template>

          <!-- 技能交换合约 -->
          <template v-if="selectedTemplate.id === 'skill_exchange'">
            <a-form-item label="甲方 DID" required>
              <a-input v-model:value="formData.party1Did" placeholder="输入甲方 DID" />
            </a-form-item>
            <a-form-item label="甲方技能" required>
              <a-input v-model:value="formData.party1Skill" placeholder="输入甲方提供的技能" />
            </a-form-item>
            <a-form-item label="甲方工时" required>
              <a-input-number v-model:value="formData.party1Hours" :min="1" style="width: 100%" />
            </a-form-item>
            <a-form-item label="乙方 DID" required>
              <a-input v-model:value="formData.party2Did" placeholder="输入乙方 DID" />
            </a-form-item>
            <a-form-item label="乙方技能" required>
              <a-input v-model:value="formData.party2Skill" placeholder="输入乙方提供的技能" />
            </a-form-item>
            <a-form-item label="乙方工时" required>
              <a-input-number v-model:value="formData.party2Hours" :min="1" style="width: 100%" />
            </a-form-item>
            <a-form-item label="有效天数">
              <a-input-number v-model:value="formData.durationDays" :min="1" :max="365" style="width: 100%" />
            </a-form-item>
          </template>

          <!-- 多重签名托管 -->
          <template v-if="selectedTemplate.id === 'multisig_escrow'">
            <a-form-item label="参与方 DIDs（每行一个）" required>
              <a-textarea v-model:value="participantsText" :rows="4" placeholder="输入参与方 DID，每行一个" />
            </a-form-item>
            <a-form-item label="资产 ID" required>
              <a-input v-model:value="formData.assetId" placeholder="输入资产 ID" />
            </a-form-item>
            ...
            <a-form-item label="所需签名数" required>
              <a-input-number v-model:value="formData.requiredSignatures" :min="2" style="width: 100%" />
            </a-form-item>
          ...
          <template v-if="selectedTemplate.id === 'timelock_escrow'">
            <a-form-item label="发送者 DID" required>
              <a-input v-model:value="formData.senderDid" placeholder="输入发送者 DID" />
            </a-form-item>
            <a-form-item label="接收者 DID" required>
              <a-input v-model:value="formData.recipientDid" placeholder="输入接收者 DID" />
            </a-form-item>
            <a-form-item label="资产 ID" required>
              <a-input v-model:value="formData.assetId" placeholder="输入资产 ID" />
            </a-form-item>
            ...
            <a-form-item label="解锁日期" required>
              <a-date-picker
      ...
      <div v-if="currentStep === 2 && selectedTemplate">
        ...
        <a-descriptions :column="2" bordered>
          <a-descriptions-item label="合约模板" :span="2">
        ...
        <div style="text-align: right; margin-top: 16px">
          <a-space>
            <a-button @click="prevStep">上一步</a-button>
            <a-button type="primary" @click="handleCreate">创建合约</a-button>
          </a-space>
        </div>
      </div>
    </a-modal>
