      ...
      >
        ...
        <a-descriptions-item v-if="asset.metadata?.custom" label="自定义数据">
          <pre class="custom-data">{{
            JSON.stringify(asset.metadata.custom, null, 2)
          }}</pre>
        </a-descriptions-item>
      </a-descriptions>
...
const nftAttributes = computed(() => {
  if (
    ...
    !props.asset.metadata.attributes
  ) {
    return [];
  }
  return props.asset.metadata.attributes.filter(
...
const canMint = computed(() => {
  if (!props.asset || !props.currentUserDid) {
    return false;
  }
  return (
    props.asset.creator_did === props.currentUserDid &&
    props.asset.asset_type === "token"
  );
});
