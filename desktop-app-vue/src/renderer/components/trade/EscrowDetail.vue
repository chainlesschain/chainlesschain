<template>
  ...
  >
    <div v-if="escrow" class="escrow-detail-content">
      ...
      <a-card size="small" style="margin-bottom: 16px">
        ...
        <div class="status-section">
          ...
          <div class="status-info">
            <div class="status-title">{{ getStatusTitle(escrow.status) }}</div>
            <div class="status-description">{{ getStatusDescription(escrow.status) }}</div>
          </div>
        </div>
      </a-card>
      ...
      <a-card size="small" title="基本信息" style="margin-bottom: 16px">
        <a-descriptions :column="2" bordered size="small">
          ...
          <a-descriptions-item label="交易 ID" :span="2">
            <a-typography-text copyable>{{ escrow.transaction_id }}</a-typography-text>
          </a-descriptions-item>

          <a-descriptions-item v-if="escrow.order_id" label="订单 ID" :span="2">
            <a-typography-text copyable>{{ escrow.order_id }}</a-typography-text>
          </a-descriptions-item>
          ...
          <a-descriptions-item v-if="escrow.updated_at" label="更新时间" :span="2">
      ...
      <a-card size="small" title="交易双方" style="margin-bottom: 16px">
        <a-descriptions :column="1" bordered size="small">
          <a-descriptions-item label="买家">
            <a-space>
              <user-outlined />
              <a-typography-text copyable :ellipsis="{ tooltip: escrow.buyer_did }">
                {{ formatDid(escrow.buyer_did) }}
              </a-typography-text>
              <a-tag v-if="isCurrentUser(escrow.buyer_did)" color="blue">我</a-tag>
            </a-space>
          </a-descriptions-item>
          ...
          <a-descriptions-item label="卖家">
            <a-space>
              <shop-outlined />
              <a-typography-text copyable :ellipsis="{ tooltip: escrow.seller_did }">
                {{ formatDid(escrow.seller_did) }}
              </a-typography-text>
              <a-tag v-if="isCurrentUser(escrow.seller_did)" color="green">我</a-tag>
            </a-space>
          </a-descriptions-item>
        </a-descriptions>
      </a-card>
      ...
      <!-- 托管条件 -->
      <a-card v-if="escrow.conditions" size="small" title="托管条件" style="margin-bottom: 16px">
