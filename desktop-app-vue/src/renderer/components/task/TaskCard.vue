<template>
  ...
  >
    ...
    <!-- 任务描述 -->
    <div v-if="task.description && showDescription" class="task-description">
    ...
    <div class="task-card-footer">
      <div class="footer-left">
        ...
        <!-- 截止日期 -->
        <a-tooltip v-if="task.due_date" :title="getDueDateTooltip()">
          <span
            class="due-date"
        ...
        <!-- 评论数量 -->
        <span v-if="task.comment_count > 0" class="comment-count">
          <message-outlined />
          {{ task.comment_count }}
        </span>

        <!-- 附件数量 -->
        <span v-if="task.attachment_count > 0" class="attachment-count">
          <paperclip-outlined />
      ...
      <div class="footer-right">
        <!-- 分配的用户头像 -->
        <a-tooltip v-if="task.assigned_to" :title="getAssignedUserName()">
          <a-avatar
            :size="24"
    ...
    <!-- 进度条（如果有预估工时） -->
    <div v-if="task.estimate_hours && task.actual_hours" class="task-progress">
      <a-progress
        :percent="
          Math.min(
            Math.round((task.actual_hours / task.estimate_hours) * 100),
            100,
          )
        "
        :size="'small'"
        :show-info="false"
        :status="
          task.actual_hours > task.estimate_hours ? 'exception' : 'normal'
        "
      />
...
const isSoon = computed(() => {
  ...
  return (
    props.task.due_date < Date.now() + threeDays &&
    props.task.due_date > Date.now()
  );
});
...
function getAssignedUserName() {
  ...
  const member = members.find(
    (m) =>
      m.member_did === props.task.assigned_to ||
      m.did === props.task.assigned_to,
  );
  if (member) {
    return (
      member.display_name ||
