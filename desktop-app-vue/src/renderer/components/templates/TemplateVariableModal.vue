...
const formRules = computed(() => {
  ...
  variablesSchema.value.forEach((variable) => {
    ...
    if (variable.type === "string" && variable.pattern) {
      let patternRe = null;
      try {
        patternRe = new RegExp(variable.pattern);
      } catch {
        // Invalid regex in the template definition would otherwise throw
        // inside this computed and crash the whole modal render. Skip the
        // pattern rule instead — the field just gets no format validation.
        patternRe = null;
      }
      if (patternRe) {
        rules[variable.name] = [
