<template>
  <div class="template-editor">
          ...
          <a-row :gutter="16">
            <a-col :span="12">
              <a-form-item label="模板名称" name="name" required>
                <a-input v-model:value="formData.name" placeholder="请输入模板名称（英文标识）" />
              </a-form-item>
            </a-col>
            <a-col :span="12">
              <a-form-item label="显示名称" name="display_name" required>
                <a-input v-model:value="formData.display_name" placeholder="请输入显示名称（中文）" />
              </a-form-item>
            </a-col>
          </a-row>
          ...
          <a-row :gutter="16">
            <a-col :span="12">
              <a-form-item label="子分类" name="subcategory">
                <a-input v-model:value="formData.subcategory" placeholder="请输入子分类（可选）" />
              </a-form-item>
            </a-col>
            <a-col :span="12">
              <a-form-item label="图标" name="icon">
                <a-input v-model:value="formData.icon" placeholder="请输入图标名称（可选）" />
              </a-form-item>
            </a-col>
          </a-row>
    ...
    <a-modal
      ...
      width="700px"
    >
      <div class="helper-content" v-pre>
        <h4>基本变量</h4>
...
{{/each}}</pre>
        ...
        <h4>内置 Helpers</h4>
        <ul>
          <li><code>{{formatDate date 'yyyy-MM-dd'}}</code> - 格式化日期</li>
          <li><code>{{uppercase text}}</code> - 转大写</li>
          <li><code>{{lowercase text}}</code> - 转小写</li>
          <li><code>{{capitalize text}}</code> - 首字母大写</li>
          <li><code>{{default value 'defaultValue'}}</code> - 默认值</li>
          <li><code>{{add a b}}</code> - 加法</li>
          <li><code>{{subtract a b}}</code> - 减法</li>
        </ul>
      </div>
    </a-modal>
  </div>
...
const helpText = computed(() => '支持 Handlebars 语法，例如: {{title}}, {{#if condition}}...{{/if}}, {{#each items}}...{{/each}}')
...
function loadTemplate(template) {
  ...
  fileStructureJson.value = JSON.stringify(template.file_structure || {}, null, 2)
  defaultFilesJson.value = JSON.stringify(template.default_files || [], null, 2)
}
