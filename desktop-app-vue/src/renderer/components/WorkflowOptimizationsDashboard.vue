      ...
      <a-row :gutter="16" class="summary-row">
        <a-col :span="6">
          <a-statistic
            ...
            :value="enabledCount"
            :suffix="`/ ${totalCount}`"
            :valueStyle="{ color: enabledCount === totalCount ? '#3f8600' : '#faad14' }"
          >
            <template #prefix>
              ...
              <a-card title="代理池" size="small">
                <a-descriptions :column="1" size="small">
                  <a-descriptions-item label="复用率">
                    <a-tag :color="getReuseRateColor(stats.agentPool?.reuseRate)">
...
const performanceGains = ref([
  { metric: '任务成功率', baseline: '40%', optimized: '70%', improvement: '+75%' },
  { metric: '任务规划速度', baseline: '2-3秒', optimized: '1秒', improvement: '-60%' },
  { metric: 'LLM成本', baseline: '100%', optimized: '30%', improvement: '-70%' },
  { metric: '多代理利用率', baseline: '70%', optimized: '90%', improvement: '+20%' },
  { metric: '代理获取速度', baseline: '100ms', optimized: '10ms', improvement: '-90%' },
  { metric: '任务执行时间', baseline: '100%', optimized: '75%', improvement: '-25%' },
  { metric: '质量问题发现', baseline: '30分钟', optimized: '<1秒', improvement: '1800x' },
]);
...
const totalCount = computed(() => {
  return phase1Optimizations.value.length +
...
const healthColor = computed(() => {
  const percentage = (enabledCount.value / totalCount.value * 100);
  if (percentage === 100) return '#3f8600';
...
async function toggleOptimization(key, enabled) {
  try {
    // 调用IPC保存配置
    const result = await window.electron.invoke('workflow-optimizations:toggle', { key, enabled });
  ...
  try {
    ...
    if (result.success) {
      ...
      stats.value = {
        ...
        decisionEngine: { multiAgentRate: '0%', llmCallRate: '0%', avgDecisionTime: '0ms' },
        agentPool: { reuseRate: '0%', available: 0, busy: 0 },
        criticalPath: { totalAnalyses: 0, avgCriticalPathLength: '0', avgSlack: '0ms' },
      };
...
async function exportStats() {
  try {
    // 调用IPC导出配置
    const result = await window.electron.invoke('workflow-optimizations:export-config');
...
async function generateReport() {
  try {
    // 调用IPC生成报告
    const result = await window.electron.invoke('workflow-optimizations:get-report');
...
async function loadOptimizationStatus() {
  try {
    const result = await window.electron.invoke('workflow-optimizations:get-status');
