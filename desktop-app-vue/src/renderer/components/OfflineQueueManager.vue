<template>
  <div class="offline-queue-manager">
    <a-card title="离线消息队列" :loading="loading">
      <template #extra>
        <a-space>
          <a-badge :count="queueStats.totalMessages" :number-style="{ backgroundColor: '#1890ff' }">
            <a-button @click="loadQueue">
              <template #icon><ReloadOutlined /></template>
              刷新
            </a-button>
          </a-badge>
          <a-button @click="showRetryAllModal = true" :disabled="queueStats.totalMessages === 0">
            <template #icon><SyncOutlined /></template>
            重试全部
          </a-button>
          <a-button danger @click="showClearAllModal = true" :disabled="queueStats.totalMessages === 0">
      ...
      <a-row :gutter="16" style="margin-bottom: 24px">
        <a-col :span="6">
          <a-statistic title="待发送消息" :value="queueStats.pendingMessages" :value-style="{ color: '#1890ff' }">
            <template #prefix><ClockCircleOutlined /></template>
          </a-statistic>
        </a-col>
        <a-col :span="6">
          <a-statistic title="发送中" :value="queueStats.sendingMessages" :value-style="{ color: '#faad14' }">
            <template #prefix><LoadingOutlined /></template>
          </a-statistic>
        </a-col>
        <a-col :span="6">
          <a-statistic title="失败消息" :value="queueStats.failedMessages" :value-style="{ color: '#ff4d4f' }">
      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'targetPeerId'">
            <a-typography-text copyable>{{ shortenPeerId(record.targetPeerId) }}</a-typography-text>
          </template>
          ...
          <template v-else-if="column.key === 'retryCount'">
            <a-badge :count="record.retryCount" :number-style="{ backgroundColor: record.retryCount >= 3 ? '#ff4d4f' : '#faad14' }" />
          </template>
    ...
    >
      <a-descriptions v-if="selectedMessage" bordered :column="1">
        <a-descriptions-item label="消息ID">
          <a-typography-text copyable>{{ selectedMessage.id }}</a-typography-text>
        </a-descriptions-item>
        <a-descriptions-item label="目标节点">
          <a-typography-text copyable>{{ selectedMessage.targetPeerId }}</a-typography-text>
        </a-descriptions-item>
        <a-descriptions-item label="设备ID">
          <a-typography-text copyable>{{ selectedMessage.deviceId || '未指定' }}</a-typography-text>
        </a-descriptions-item>
        ...
        <a-descriptions-item label="最后重试时间">
          {{ selectedMessage.lastRetryAt ? new Date(selectedMessage.lastRetryAt).toLocaleString('zh-CN') : '未重试' }}
        </a-descriptions-item>
        <a-descriptions-item label="错误信息">
          <a-typography-text type="danger">{{ selectedMessage.error || '无' }}</a-typography-text>
        </a-descriptions-item>
        <a-descriptions-item label="消息内容">
          <a-typography-paragraph :copyable="{ text: selectedMessage.content }">
            <pre style="white-space: pre-wrap; word-break: break-all">{{ selectedMessage.content }}</pre>
    ...
    >
      ...
      <p>共 {{ queueStats.pendingMessages + queueStats.failedMessages }} 条消息将被重试。</p>
    </a-modal>
...
const queueStats = computed(() => {
  const pending = queueMessages.value.filter((m) => m.status === 'pending').length;
  const sending = queueMessages.value.filter((m) => m.status === 'sending').length;
  const failed = queueMessages.value.filter((m) => m.status === 'failed').length;
