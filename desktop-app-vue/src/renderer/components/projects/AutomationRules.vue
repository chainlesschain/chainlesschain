...
const editRule = (rule) => {
  ...
  ruleForm.triggerConfig = safeJsonParse(rule.trigger_config, {});
  ruleForm.actionType = rule.action_type;
  ruleForm.actionConfig = safeJsonParse(rule.action_config, {});
  modalVisible.value = true;
};
...
const handleSaveRule = async () => {
  ...
  try {
    // 处理脚本参数
    if (
      ruleForm.actionType === "run_script" &&
      ruleForm.actionConfig.argsText
    ) {
