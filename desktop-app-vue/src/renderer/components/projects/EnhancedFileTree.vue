      ...
      >
        <template #title="{ title, isLeaf, dataRef }">
          ...
          >
            ...
            <a-tag
              v-if="
                gitStatus && dataRef.filePath && gitStatus[dataRef.filePath]
              "
...
const treeData = computed(() => {
  logger.info(
    "[EnhancedFileTree] ========== treeData computed 执行 ==========",
  );
  ...
  logger.info(
    "[EnhancedFileTree] ========== treeData computed 结束 ==========",
  );
  ...
  try {
    ...
    try {
      systemClipboardResult =
        await window.electronAPI.file.pasteFromSystemClipboard();
...
const handleOpenDefault = async () => {
  if (!contextNode.value || !contextNode.value.isLeaf) {
    return;
  }
