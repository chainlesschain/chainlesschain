          ...
          <a-col :xs="24" :sm="12" :md="8" :lg="5" :xl="5">
            <a-card size="small" class="stat-card">
              <a-statistic
                ...
                :value-style="{
                  color: getSuccessRateColor(overview.successRate),
                  fontSize: '28px',
                }"
              >
                <template #prefix>
              ...
              >
                <template #bodyCell="{ column, record }">
                  ...
                  <template v-if="column.key === 'successRate'">
                    <span
                      :style="{
                        color: getSuccessRateColor(record.successRate + '%'),
                      }"
                    >
                      {{ record.successRate }}%
                    </span>
                      ...
                      <template #description>
                        <a-space>
                          ...
                          <span style="color: #8c8c8c">{{
                            item.timeSinceLastUse
                          }}</span>
                          <span style="color: #8c8c8c">
                            调用 {{ item.usage_count }} 次 | 成功率
                            {{ getToolSuccessRate(item) }}%
                          </span>
                        </a-space>
                      </template>
                    </a-list-item-meta>
...
const saveSavedFilters = () => {
  try {
    localStorage.setItem(
      SAVED_FILTERS_KEY,
      JSON.stringify(savedFiltersList.value),
    );
  } catch (error) {
...
const loadDashboardData = async () => {
  ...
  try {
    ...
    const result =
      await window.electronAPI.tool.getAdditionalV3Dashboard(filters);
...
const exportAllCharts = async () => {
  try {
    ...
    message.success({
      content: `成功导出 ${successCount} 个图表`,
      key: "exportAllCharts",
    });
  } catch (error) {
    logger.error("[Export] 导出所有图表失败:", error);
    message.error({
      content: "导出所有图表失败: " + error.message,
      key: "exportAllCharts",
    });
  }
};
