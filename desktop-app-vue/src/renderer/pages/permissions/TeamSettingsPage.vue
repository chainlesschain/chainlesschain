<template>
  <div class="team-settings-page">
    <a-page-header title="团队设置" sub-title="管理组织内的团队结构" @back="goBack">
      <template #extra>
              ...
              <template #bodyCell="{ column, record }">
                ...
                <template v-else-if="column.key === 'action'">
                  <a-space>
                    ...
                    <a-popconfirm title="移除此成员？" @confirm="removeMember(record.memberDid)">
    ...
    >
      <a-form :model="newTeam" layout="vertical">
        ...
        <a-form-item label="描述">
          <a-textarea v-model:value="newTeam.description" placeholder="团队描述" />
        </a-form-item>
        <a-form-item label="父团队">
          <a-select v-model:value="newTeam.parentTeamId" allow-clear placeholder="选择父团队（可选）">
    ...
    >
      <a-form :model="newMember" layout="vertical">
        <a-form-item label="成员DID" required>
          <a-input v-model:value="newMember.memberDid" placeholder="输入成员DID" />
        </a-form-item>
        <a-form-item label="成员名称" required>
          <a-input v-model:value="newMember.memberName" placeholder="输入成员名称" />
        </a-form-item>
...
const memberColumns = [
  ...
  { title: '角色', key: 'role' },
  { title: '加入时间', dataIndex: 'joinedAt', key: 'joinedAt', customRender: ({ text }) => formatTime(text) },
  { title: '操作', key: 'action' },
];
...
const setAsLead = async (member) => {
  try {
    await teamStore.setLead(currentTeam.value.id, member.memberDid, member.memberName);
