<template>
  <div class="permission-groups-tab">
      ...
      >
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'assignmentCount'">
            <a-badge :count="record.assignments?.length || 0" :number-style="{ backgroundColor: '#52c41a' }" />
          </template>
          ...
          <template v-else-if="column.key === 'actions'">
            <a-space>
              ...
              <a-button type="link" size="small" @click="handleViewGroup(record)">
                查看详情
              </a-button>
              <a-popconfirm
                title="确定要删除此权限组吗?"
    ...
    >
      <a-form :model="groupForm" :label-col="{ span: 6 }" :wrapper-col="{ span: 18 }">
        <a-form-item label="权限组名称" required>
          <a-input v-model:value="groupForm.groupName" placeholder="输入权限组名称" />
        </a-form-item>
    ...
    >
      <a-form :model="assignForm" :label-col="{ span: 6 }" :wrapper-col="{ span: 18 }">
        <a-form-item label="角色名称" required>
          <a-input v-model:value="assignForm.roleName" placeholder="输入角色名称" />
        </a-form-item>
      </a-form>
    </a-modal>
  </div>
