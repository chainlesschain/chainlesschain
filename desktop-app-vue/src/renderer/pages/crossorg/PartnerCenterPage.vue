                  ...
                  <template #actions>
                    <template v-if="item.status === 'pending'">
                      <a-button
                        type="primary"
                        size="small"
                        @click="acceptPartnership(item.id)"
                      >
                        接受
                      </a-button>
                      <a-button
                        danger
                        size="small"
                        @click="rejectPartnership(item.id)"
                      >
                        拒绝
                      </a-button>
                    </template>
                      ...
                      <a-dropdown>
                        ...
                        <template #overlay>
                          <a-menu>
                            ...
                            <a-menu-item
                              danger
                              @click="terminatePartnership(item.id)"
                            >
                              终止合作
                            </a-menu-item>
                          </a-menu>
        ...
        <a-card title="合作统计">
          <a-row :gutter="[16, 16]">
            ...
            <a-col :span="12">
              <a-statistic
                title="共享资源"
                :value="stats.sharedResourceCount"
              />
            </a-col>
            <a-col :span="12">
...
const stats = computed(() => ({
  ...
  sharedResourceCount:
    crossOrgStore.outgoingShares.length + crossOrgStore.incomingShares.length,
...
const acceptPartnership = async (partnershipId) => {
  try {
    await crossOrgStore.acceptPartnership(
      partnershipId,
...
const rejectPartnership = async (partnershipId) => {
  try {
    await crossOrgStore.rejectPartnership(
      partnershipId,
      authStore.currentUser?.did,
    );
    message.success("已拒绝");
...
const terminatePartnership = async (partnershipId) => {
  try {
    await crossOrgStore.terminatePartnership(
      partnershipId,
      authStore.currentUser?.did,
    );
