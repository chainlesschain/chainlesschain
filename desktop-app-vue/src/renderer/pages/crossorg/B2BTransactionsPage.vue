<template>
  <div class="b2b-transactions-page">
      ...
      >
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'direction'">
            <a-tag
              :color="record.senderOrgId === currentOrgId ? 'blue' : 'green'"
            >
            ...
            <a-space>
              ...
              >
                <a-button
                  type="primary"
                  size="small"
                  @click="acceptTransaction(record.id)"
                >
                  接受
                </a-button>
                <a-button
                  danger
                  size="small"
                  @click="rejectTransaction(record.id)"
                >
                  拒绝
                </a-button>
              </template>
    ...
    >
      <a-form :model="initiateForm" layout="vertical">
        ...
        <a-form-item label="数据类型">
          <a-input
            v-model:value="initiateForm.dataType"
            placeholder="例如: customer_data, reports"
          />
        </a-form-item>
        <a-form-item label="数据大小 (字节)">
          <a-input-number
            v-model:value="initiateForm.dataSize"
            :min="0"
            style="width: 100%"
          />
        </a-form-item>
        <a-form-item label="备注">
          <a-textarea
            v-model:value="initiateForm.notes"
            placeholder="交易说明..."
          />
        </a-form-item>
      </a-form>
    ...
    <a-modal
      ...
      :title="`交易详情 - ${selectedTransaction?.id}`"
      :footer="null"
      width="600px"
    >
      <template v-if="selectedTransaction">
        <a-descriptions :column="2" bordered>
          ...
          <a-descriptions-item label="接收方">
            {{ selectedTransaction.receiverOrgId }}
          </a-descriptions-item>
          <a-descriptions-item label="交易类型">
            {{ selectedTransaction.transactionType }}
          </a-descriptions-item>
          <a-descriptions-item label="数据类型">
            {{ selectedTransaction.dataType }}
          </a-descriptions-item>
          <a-descriptions-item label="数据大小">
            {{ formatSize(selectedTransaction.dataSize) }}
          </a-descriptions-item>
          ...
          <a-descriptions-item label="完成时间">
            {{
              selectedTransaction.completedAt
                ? formatTime(selectedTransaction.completedAt)
                : "-"
            }}
          </a-descriptions-item>
        </a-descriptions>
...
const filteredTransactions = computed(() => {
  ...
  if (transactionFilter.value === "incoming") {
    return transactions.value.filter(
      (t) => t.receiverOrgId === currentOrgId.value,
    );
  }
  if (transactionFilter.value === "outgoing") {
    return transactions.value.filter(
      (t) => t.senderOrgId === currentOrgId.value,
    );
  }
  if (transactionFilter.value === "pending") {
...
const stats = computed(() => ({
  outgoingCount: transactions.value.filter(
    (t) => t.senderOrgId === currentOrgId.value,
  ).length,
  incomingCount: transactions.value.filter(
    (t) => t.receiverOrgId === currentOrgId.value,
  ).length,
  pendingCount: transactions.value.filter((t) => t.status === "pending").length,
  completedCount: transactions.value.filter((t) => t.status === "completed")
    .length,
}));
...
const acceptTransaction = async (transactionId) => {
  try {
    await crossOrgStore.acceptTransaction(
      transactionId,
...
const rejectTransaction = async (transactionId) => {
  try {
    await crossOrgStore.rejectTransaction(
      transactionId,
      authStore.currentUser?.did,
    );
    message.success("已拒绝");
