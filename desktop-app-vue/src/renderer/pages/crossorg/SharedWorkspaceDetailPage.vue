<template>
  <div class="shared-workspace-detail-page">
                ...
                <template #bodyCell="{ column, record }">
                  ...
                  <template v-else-if="column.key === 'action'">
                    <a-button
                      ...
                      danger
                      size="small"
                      @click="removeMember(record.memberDid)"
                    >
                      移除
                    </a-button>
                  </template>
            ...
            <template v-else-if="column.key === 'action'">
              <a-space>
                ...
                <a-button
                  type="link"
                  danger
                  @click="unshareResource(record.id)"
                >
                  取消共享
                </a-button>
              </a-space>
    ...
    <!-- 共享资源对话框 -->
    <a-modal
      v-model:open="showShareResource"
      title="共享资源"
...
const handleInviteOrg = async () => {
  ...
  inviting.value = true;
  try {
    await crossOrgStore.inviteOrgToWorkspace(
      workspaceId.value,
      inviteOrgForm.value,
...
onMounted(async () => {
  ...
  try {
    ...
    await crossOrgStore.loadSharedResources(authStore.currentOrg?.id, {
