    ...
    <a-spin :spinning="loading">
      <a-row :gutter="16">
        <a-col v-for="workspace in workspaces" :key="workspace.id" :span="8">
          ...
          >
            <template #cover>
              <div class="workspace-cover" :style="{ backgroundColor: getWorkspaceColor(workspace.workspaceType) }">
            ...
            <div class="workspace-meta">
              <a-space>
                ...
                <a-tag :color="workspace.orgRole === 'admin' ? 'gold' : 'default'">
                  {{ workspace.orgRole }}
                </a-tag>
              </a-space>
    ...
    >
      <a-form :model="newWorkspace" layout="vertical">
        ...
        <a-form-item label="描述">
          <a-textarea v-model:value="newWorkspace.description" placeholder="工作空间描述" />
        </a-form-item>
...
import { PlusOutlined, TeamOutlined, UserOutlined } from '@ant-design/icons-vue';
...
const handleCreate = async () => {
  ...
  try {
    ...
    if (result.success) {
      ...
      newWorkspace.value = { name: '', description: '', workspaceType: 'project', visibility: 'private' };
