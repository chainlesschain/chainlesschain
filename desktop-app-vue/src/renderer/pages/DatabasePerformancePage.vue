        ...
        <a-col :span="6">
          <a-card>
            <a-statistic
              ...
              suffix="ms"
              :precision="2"
              :value-style="{ color: stats.avgQueryTime > 50 ? '#cf1322' : '#3f8600' }"
            />
          </a-card>
        </a-col>
        <a-col :span="6">
          <a-card>
            <a-statistic
              ...
              :value-style="{ color: stats.slowQueries > 0 ? '#cf1322' : '#3f8600' }"
            />
          </a-card>
        </a-col>
        <a-col :span="6">
          <a-card>
            <a-statistic
              ...
              suffix="%"
              :precision="2"
              :value-style="{ color: cacheHitRate > 80 ? '#3f8600' : '#cf1322' }"
            />
          </a-card>
        </a-col>
      </a-row>
...
const refreshStats = async () => {
  ...
  try {
    ...
    // 获取慢查询
    const slowQueriesResult = await window.electronAPI.dbPerformance.getSlowQueries(20);
    if (slowQueriesResult.success) {
      slowQueries.value = slowQueriesResult.data;
    }

    // 获取索引建议
    const suggestionsResult = await window.electronAPI.dbPerformance.getIndexSuggestions();
...
const applyIndexSuggestion = async (suggestion) => {
  ...
  try {
    const result = await window.electronAPI.dbPerformance.applyIndexSuggestion(suggestion);
    if (result.success) {
...
const applyAllIndexSuggestions = async () => {
  ...
  try {
    const result = await window.electronAPI.dbPerformance.applyAllIndexSuggestions();
    if (result.success) {
      ...
      if (failCount === 0) {
        ...
        message.warning(`成功创建 ${successCount} 个索引，失败 ${failCount} 个`);
      }
