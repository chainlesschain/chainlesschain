<template>
  <div class="ble-devices-page">
    <a-page-header
      title="BLE U-Key Devices"
      sub-title="Bluetooth Low Energy device management"
    >
      <template #extra>
        <a-space>
          <a-button
    ...
    <a-tabs v-model:active-key="activeTab">
      ...
      <a-tab-pane
        key="discovered"
        tab="Discovered Devices"
      >
        <a-spin :spinning="store.scanning">
          <a-empty
            v-if="store.devices.length === 0"
            description="No BLE devices found. Click Scan to search."
          />
          <a-list
            v-else
            ...
            <template #renderItem="{ item }">
              <a-list-item>
                <a-list-item-meta
                  ...
                  :description="`RSSI: ${item.rssi} dBm | ~${item.distance?.toFixed(1)}m`"
                >
                  <template #avatar>
      ...
      >
        ...
        <a-list
          v-else
          :data-source="store.pairedDevices"
          item-layout="horizontal"
        >
          <template #renderItem="{ item }">
            <a-list-item>
              <a-list-item-meta
                ...
                :description="`Last seen: ${formatDate(item.lastSeen)}`"
              />
              <template #actions>
                <a-button
                  size="small"
    ...
    >
      ...
      >
        <a-descriptions-item label="Device">
          {{ store.connectedDevice.name || store.connectedDevice.id }}
        </a-descriptions-item>
        <a-descriptions-item label="Signal">
          {{ store.connectedDevice.rssi }} dBm
        </a-descriptions-item>
      </a-descriptions>
    </a-card>
