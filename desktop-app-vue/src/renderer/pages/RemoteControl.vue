        ...
        <a-form layout="vertical">
          <a-row :gutter="16">
            <a-col :span="8">
              <a-form-item label="目标设备">
                <a-select
                  v-model:value="testCommand.peerId"
                  placeholder="选择设备"
                >
                  <a-select-option
            ...
            <a-col :span="8">
              <a-form-item label="命令类型">
                <a-select
                  v-model:value="testCommand.method"
                  placeholder="选择命令"
                >
                  <a-select-option value="system.getStatus">
                    系统状态
                  </a-select-option>
          ...
          <a-form-item v-if="testCommand.result" label="执行结果">
            <a-alert
              ...
              :message="
                testCommand.result.success ? '命令执行成功' : '命令执行失败'
              "
            >
              <template #description>
                <pre>{{
                  JSON.stringify(testCommand.result.data, null, 2)
                }}</pre>
              </template>
            </a-alert>
          </a-form-item>
    ...
    >
      <a-form layout="vertical">
        <a-form-item label="设备 DID">
          <a-input
            :value="permissionModal.device ? permissionModal.device.did : ''"
            disabled
          />
        </a-form-item>
        ...
        <a-form-item label="设备名称">
          <a-input
            v-model:value="permissionModal.deviceName"
            placeholder="为设备起个名字"
          />
        </a-form-item>
        ...
        <a-form-item label="过期时间（小时）">
          <a-input-number
            v-model:value="permissionModal.expiresInHours"
            :min="1"
            :max="168"
          />
  ...
  try {
    ...
    if (result.success) {
      ...
      for (const device of connectedDevices.value) {
        const permResult = await ipcRenderer.invoke(
...
async function savePermission() {
  try {
    const expiresIn =
      permissionModal.expiresInHours > 0
