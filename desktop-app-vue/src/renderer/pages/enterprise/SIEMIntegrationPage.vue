<template>
  <div class="siem-integration-page">
    <a-page-header title="SIEM Integration" sub-title="Log export to Splunk HEC, Elasticsearch, Azure Sentinel">
      <template #extra>
        <a-space>
          <a-button @click="handleExportAll" :loading="store.loading">Export Now</a-button>
          <a-button type="primary" @click="showAddModal = true">Add Target</a-button>
        </a-space>
      </template>
    </a-page-header>
    ...
    <a-row :gutter="16" style="margin-bottom: 16px">
      <a-col :span="8">
        <a-statistic title="Active Targets" :value="store.activeTargets.length" />
      </a-col>
      <a-col :span="8">
        <a-statistic title="Total Exported" :value="store.totalExported" />
      </a-col>
      <a-col :span="8">
        <a-statistic title="Formats" value="JSON / CEF / LEEF" />
      </a-col>
    </a-row>

    <a-tabs v-model:activeKey="activeTab">
      <!-- Targets Tab -->
      <a-tab-pane key="targets" tab="Export Targets">
        <a-empty v-if="store.targets.length === 0" description="No SIEM targets configured" />
        <a-table v-else :columns="targetColumns" :data-source="store.targets" :loading="store.loading" row-key="id" size="small">
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'type'">
              <a-tag :color="typeColor(record.target_type)">{{ record.target_type }}</a-tag>
            </template>
            ...
            <template v-if="column.key === 'actions'">
              <a-button size="small" type="link" @click="handleExport(record.id)">Export</a-button>
            </template>
          </template>
        </a-table>
      </a-tab-pane>
      ...
      <!-- Stats Tab -->
      <a-tab-pane key="stats" tab="Export Stats">
        <a-card size="small">
          <a-empty v-if="!store.stats" description="No export stats" />
          <a-list v-else :data-source="store.stats.targets" item-layout="horizontal">
            <template #renderItem="{ item }">
              <a-list-item>
                <a-list-item-meta :title="item.type" :description="`${item.exported_count} logs exported`" />
                <template #actions>
    ...
    <!-- Add Target Modal -->
    <a-modal v-model:open="showAddModal" title="Add SIEM Target" @ok="handleAddTarget" :confirm-loading="store.loading">
      <a-form layout="vertical">
        <a-form-item label="Target Type">
          <a-select v-model:value="addForm.type">
            <a-select-option value="splunk_hec">Splunk HEC</a-select-option>
            <a-select-option value="elasticsearch">Elasticsearch</a-select-option>
            <a-select-option value="azure_sentinel">Azure Sentinel</a-select-option>
          </a-select>
        </a-form-item>
        <a-form-item label="URL">
          <a-input v-model:value="addForm.url" placeholder="https://splunk.example.com:8088/services/collector" />
        </a-form-item>
        <a-form-item label="Format">
          <a-select v-model:value="addForm.format">
            <a-select-option value="json">JSON</a-select-option>
            <a-select-option value="cef">CEF</a-select-option>
            <a-select-option value="leef">LEEF</a-select-option>
          </a-select>
        </a-form-item>
      </a-form>
    </a-modal>
