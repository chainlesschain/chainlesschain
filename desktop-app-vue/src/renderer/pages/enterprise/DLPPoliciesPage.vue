<template>
  <div class="dlp-policies-page">
    <a-page-header title="Data Loss Prevention" sub-title="Content detection policies and incident management">
      <template #extra>
        <a-button type="primary" @click="showCreateModal = true">Create Policy</a-button>
      </template>
    </a-page-header>

    <a-row :gutter="16" style="margin-bottom: 16px">
      <a-col :span="8">
        <a-statistic title="Active Policies" :value="store.activePolicies.length" />
      </a-col>
      <a-col :span="8">
        <a-statistic title="Unresolved Incidents" :value="store.unresolvedIncidents.length" />
      </a-col>
      <a-col :span="8">
        <a-statistic title="Total Scanned" :value="store.stats?.scanned || 0" />
      </a-col>
    </a-row>
    ...
    <a-tabs v-model:activeKey="activeTab">
      ...
      <a-tab-pane key="policies" tab="Policies">
        <a-table :columns="policyColumns" :data-source="store.policies" :loading="store.loading" row-key="id" size="small">
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'enabled'">
              <a-switch :checked="record.enabled" size="small" @change="(v: boolean) => handleToggle(record.id, v)" />
            </template>
            ...
            <template v-if="column.key === 'actions'">
              <a-popconfirm title="Delete this policy?" @confirm="handleDelete(record.id)">
      ...
      <a-tab-pane key="incidents" tab="Incidents">
        <a-space style="margin-bottom: 16px">
          <a-button @click="handleFetchIncidents" :loading="store.loading">Refresh</a-button>
        </a-space>
        <a-table :columns="incidentColumns" :data-source="store.incidents" :loading="store.loading" row-key="id" size="small">
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'severity'">
              <a-tag :color="severityColor(record.severity)">{{ record.severity }}</a-tag>
            </template>
            <template v-if="column.key === 'resolved'">
              <a-tag v-if="record.resolved_at" color="green">Resolved</a-tag>
              <a-button v-else size="small" type="link" @click="handleResolve(record.id)">Resolve</a-button>
            </template>
    ...
    <!-- Create Policy Modal -->
    <a-modal v-model:open="showCreateModal" title="Create DLP Policy" @ok="handleCreate" :confirm-loading="store.loading" width="600px">
      <a-form layout="vertical">
        <a-form-item label="Name">
          <a-input v-model:value="createForm.name" placeholder="Policy name" />
        </a-form-item>
        <a-form-item label="Description">
          <a-textarea v-model:value="createForm.description" :rows="2" />
        </a-form-item>
        <a-form-item label="Channels">
          <a-checkbox-group v-model:value="createForm.channels" :options="channelOptions" />
        </a-form-item>
        <a-form-item label="Patterns (one regex per line)">
          <a-textarea v-model:value="createForm.patternsText" :rows="3" placeholder="\b\d{3}-\d{2}-\d{4}\b" />
        </a-form-item>
        <a-form-item label="Keywords (comma separated)">
          <a-input v-model:value="createForm.keywordsText" placeholder="confidential, secret, classified" />
        </a-form-item>
        <a-row :gutter="16">
          <a-col :span="12">
            <a-form-item label="Action">
              <a-select v-model:value="createForm.action">
                ...
                <a-select-option value="quarantine">Quarantine</a-select-option>
              </a-select>
            </a-form-item>
          </a-col>
          <a-col :span="12">
