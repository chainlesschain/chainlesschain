<template>
  <div class="ent-widget">
    <div class="widget-header">
      <BankOutlined class="widget-icon" />
      <span class="widget-title">企业仪表盘</span>
    </div>
...
const actions: EnterpriseAction[] = [
  { id: "refresh", label: "刷新组织数据", kind: "刷新", args: "refresh" },
  { id: "members", label: "查看成员", kind: "查看", args: "members" },
  { id: "audit", label: "查看审计日志", kind: "查看", args: "audit" },
];

function pick(action: EnterpriseAction): void {
  dispatchSlash("builtin:openEnterpriseDashboardPanel", {
    trigger: "/enterprise",
    args: action.args,
  });
}
...
.widget-icon {
  color: var(--cc-shell-accent, #1890ff);
  font-size: 16px;
}
