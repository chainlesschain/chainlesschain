<template>
  <div class="bridge-widget">
    <div class="widget-header">
      <SwapOutlined class="widget-icon" />
      <span class="widget-title">跨链桥</span>
    </div>
...
const actions: BridgeAction[] = [
  { id: "transfer", label: "发起跨链转移", kind: "转移", args: "transfer" },
  { id: "history", label: "查看转移历史", kind: "查看", args: "history" },
  { id: "assets", label: "查看跨链资产", kind: "查看", args: "assets" },
];

function pick(action: BridgeAction): void {
  dispatchSlash("builtin:openCrossChainBridgePanel", {
    trigger: "/bridge",
    args: action.args,
  });
}
