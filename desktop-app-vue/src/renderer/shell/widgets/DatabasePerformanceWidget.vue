<template>
  <div class="dbperf-widget">
    <div class="widget-header">
      <DatabaseOutlined class="widget-icon" />
      <span class="widget-title">数据库性能</span>
    </div>
...
const actions: DbPerfAction[] = [
  { id: "cache", label: "缓存详情", kind: "查看", args: "cache" },
  { id: "slow", label: "慢查询", kind: "查看", args: "slow" },
  { id: "suggestions", label: "索引建议", kind: "优化", args: "suggestions" },
];

function pick(action: DbPerfAction): void {
  dispatchSlash("builtin:openDbPerformancePanel", {
    trigger: "/db-performance",
    args: action.args,
  });
}
