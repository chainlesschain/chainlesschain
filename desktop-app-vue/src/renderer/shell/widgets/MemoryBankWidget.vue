<template>
  <div class="memory-bank-widget">
    <div class="widget-header">
      <DatabaseOutlined class="widget-icon" />
      <span class="widget-title">记忆库</span>
    </div>
...
const actions: MemoryBankAction[] = [
  { id: "patterns", label: "学习模式", kind: "查看", args: "patterns" },
  { id: "preferences", label: "用户偏好", kind: "查看", args: "preferences" },
  { id: "storage", label: "存储管理", kind: "管理", args: "storage" },
];

function pick(action: MemoryBankAction): void {
  dispatchSlash("builtin:openMemoryBankPanel", {
    trigger: "/memory-bank",
    args: action.args,
  });
}
