    ...
    <a-row :gutter="16" class="stats-section">
      <a-col :span="6">
        <a-statistic
          ...
          :loading="loading"
        >
          <template #prefix><TagsOutlined style="color: #1890ff" /></template>
        </a-statistic>
      </a-col>
      <a-col :span="6">
        <a-statistic
...
const renameForm = ref({ oldTag: "", newTag: "" });
...
const columns = [
  { title: "标签名称", dataIndex: "name", key: "name", width: "50%" },
  {
    title: "使用次数",
    dataIndex: "count",
    key: "count",
  ...
  { title: "操作", key: "actions", width: "25%", align: "center" },
];
...
const handleRename = (tag) => {
  renameForm.value = { oldTag: tag.name, newTag: tag.name };
