...
describe("file-utils", () => {
  describe("sanitizePath", () => {
    ...
    it('应该拒绝包含".."的路径', () => {
      ...
      expect(() => sanitizePath(basePath, relativePath)).toThrow(
        '路径不能包含 ".." 字符',
      );
    });
  ...
  describe("formatFileSize", () => {
    ...
    it('超过 TB 的大小应回落到 TB 而非输出 "undefined"', () => {
      // 1 PB = 1024^5：单位数组止于 TB，需将索引钳制到 TB 而不是 sizes[5]=undefined
      const result = formatFileSize(1125899906842624); // 1024^5
      expect(result).toBe("1024.00 TB");
      expect(result).not.toContain("undefined");
    });

    it('不足 1 字节的正数应显示为 B 而非 "undefined"', () => {
      // 0<bytes<1 时 log 为负 → 索引为 -1，需钳制到 0（B）
      const result = formatFileSize(0.5);
      expect(result).toBe("0.50 B");
      expect(result).not.toContain("undefined");
    });
