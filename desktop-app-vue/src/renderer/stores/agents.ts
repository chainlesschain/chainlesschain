...
export const useAgentsStore = defineStore("agents", {
  ...
  getters: {
    ...
    overallSuccessRate(): number {
      ...
      const successCount = this.taskHistory.filter(
        (t) => t.success === true,
      ).length;
  ...
  actions: {
    ...
    async fetchTemplates(): Promise<void> {
      ...
      try {
        ...
        if (result.success) {
          ...
          agentsLogger.info(
            `加载代理模板成功: ${this.templates.length} 个模板`,
          );
        } else {
    ...
    async createTemplate(
      templateData: Partial<AgentTemplate>,
    ...
    async fetchInstances(): Promise<void> {
      ...
      try {
        ...
        if (result.success) {
          ...
          agentsLogger.info(
            `加载代理实例成功: ${this.instances.length} 个实例`,
          );
        } else {
    ...
    async assignTask(
      ...
      try {
        ...
        if (result.success) {
          ...
          // 更新实例状态为 running
          const instIndex = this.instances.findIndex(
            (i) => i.id === instanceId,
          );
          if (instIndex !== -1) {
          ...
          agentsLogger.info(
            `分配任务成功: ${instanceId} -> ${taskDescription}`,
          );
          return task;
        } else {
    ...
    async fetchPerformance(): Promise<void> {
      ...
      try {
        ...
        if (result.success) {
          ...
          agentsLogger.info(
            `加载性能数据成功: ${this.performance.length} 条记录`,
          );
        } else {
