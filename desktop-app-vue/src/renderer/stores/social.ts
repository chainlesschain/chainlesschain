...
export const useSocialStore = defineStore("social", {
  ...
  actions: {
            ...
            if (friend.onlineStatus) {
              ...
              status =
                typeof friend.onlineStatus === "object"
                  ? (friend.onlineStatus as FriendOnlineStatus).status ||
                    "offline"
     ...
     * 删除好友
     */
    async removeFriend(friendDid: string): Promise<void> {
      try {
        await ipcRenderer.invoke("friend:remove", friendDid);
        await this.loadFriends();
      } catch (error) {
        logger.error("删除好友失败:", error as any);
        throw error;
      }
    },

    /**
     * 更新好友信息（任意子集：nickname / groupName / notes）。
     * nickname/groupName 走 friend:* 专用通道；notes 走 contact:update
     * （好友的备注信息存储在 contacts 表上）。
     */
    async updateFriendInfo(
      friendDid: string,
      updates: { nickname?: string; groupName?: string; notes?: string },
    ): Promise<void> {
      try {
        ...
        if (updates.notes !== undefined) {
          tasks.push(
            ipcRenderer.invoke("contact:update", friendDid, {
              notes: updates.notes,
            }),
          );
        }
        ...
        logger.error("更新好友信息失败:", error as any);
        throw error;
      }
    },
    ...
    initOnlineStatusListeners(): void {
      ...
      ipcRenderer.on(
        "friend:online",
        (_event: any, data: { friendDid: string }) => {
          ...
          if (friend) {
            if (
              !friend.onlineStatus ||
              typeof friend.onlineStatus === "string"
            ) {
