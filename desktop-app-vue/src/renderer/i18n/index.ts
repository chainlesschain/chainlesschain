...
export function t(key: string): string {
  ...
  let value: MessageValue | undefined = messages[state.currentLocale];
...
export function setLocale(locale: string): void {
  if (Object.values(LOCALES).includes(locale as LocaleKey)) {
    state.currentLocale = locale as LocaleKey;
    try {
      localStorage.setItem('skill-tool-locale', locale);
    } catch (error) {
      logger.warn('[i18n] 保存语言设置失败:', (error as Error).message);
    }
  }
}
...
export function initLocale(): void {
  try {
    ...
    if (saved && Object.values(LOCALES).includes(saved as LocaleKey)) {
      state.currentLocale = saved as LocaleKey;
    }
  } catch (error) {
    logger.warn('[i18n] 读取语言设置失败:', (error as Error).message);
  }
}
