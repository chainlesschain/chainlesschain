...
> ⚠️ **当前默认 `enabled=false`**。`cc audit mtc emit` 在 disabled 状态下会**拒绝写入**（除非 `--force`）。产线启用前必须等等保三级最终性窗口口径 + T/ZGCMCA 023—2025 条款摘要法务出函。脚手架本身已为 60s 严判 / 3600s 宽松两条路径预留，出函后单一 flag 即可上线，无代码改动。
