...
见正文「1. 简介」。桌面版 V6 是一款**以对话为主入口、以去中心化能力为动词、以硬件安全为环境**的个人办公客户端：AI 对话、个人知识、DID、P2P、钱包、签名整合到同一界面，不离开对话完成一天工作。

### 2. 核心特性

见正文「1.3 核心理念」与「2. 相比旧版的主要变化」。要点：对话优先、环境级安全（U-Key / DID 常驻状态栏）、去中心化是动词、万物皆插件、企业 Profile 定制。

### 3. 系统架构

见正文「3. 新界面速览」三区壳（左导航 / 中对话流 / 右 Artifact）+ 常驻状态栏：

```
┌── 左导航 ──┬──── 中对话流（主角）────┬── 右 Artifact ──┐
│ 新会话/今日 │  消息列表 + / @ 📎 🔒 输入   │  可签名/路由/加密 │
│ Spaces/联系人│                          │  的去中心化对象   │
└────────────┴────────────────────────┴────────────────┘
 状态栏：U-Key ● · DID · P2P 节点数 · LLM Provider · 成本 · 租户
```

### 4. 系统定位

**日常办公主力端**。与 CLI（脚本 / CI）、Web 面板（远程轻量）分工明确，三端共享同一内核（skills / stores / RAG / DID），UI 各走各路（见正文 1.4）。

### 5. 核心功能

| 功能 | 入口 | 说明 |
|---|---|---|
| 会话 / Spaces | 左导航 | 带独立 RAG + 提示词 + 联系人圈 + 权限的身份上下文 |
| Artifacts | 右栏 | 可签名 / 可路由 / 可加密的去中心化对象 |
| Slash `/` | 输入框 | skills + CLI 常用子集动作面板 |
| Mention `@` | 输入框 | 引用联系人 / 笔记 / 交易 / 合约 |
| 命令面板 `Ctrl+K` | 全局 | 搜会话 / 联系人 / 笔记 / skills / 设置 / 插件 |

### 6. 技术架构

Electron + Vue3 + Pinia 三区壳；插件平台贯穿 UI / 能力 / 企业三类扩展点（见正文 7.2）。能力类插件（`ai.*` / `data.*` / `net.*` / `auth.*` / `compliance.*`）三端共用；UI 类插件仅在支持端生效。

### 7. 系统特点

- 97 旧页 → 3 区壳 + 按需展开（见正文「2.」对比表）
- DID / 钱包 / P2P 从独立页内联到对话 + Artifact + 状态栏
- 安全状态常驻（Ambient Security）而非独立页面
- 企业能力通过 `.ccprofile` Profile 叠加，默认面向个人用户

### 8. 应用场景

见正文「5. 常见使用场景」：把笔记整理成周报签名发老板、查账并转账、用 ZKP 证明年龄、多智能体并行研究——全程不离开对话。

### 9. 竞品对比

| 维度 | 桌面版 V6 | ChatGPT / Claude 客户端 | 企业管理后台 |
|---|---|---|---|
| 私有数据 + 硬件密钥 | ✅ | ❌ | ⚠️ |
| 去中心化对象 Artifact | ✅ 可签名 / 路由 / 加密 | ❌ 纯文本 | ❌ |
| 插件平台（UI + 能力 + 企业） | ✅ 同契约 | ⚠️ 有限 | ⚠️ |
| 企业 Profile 一键切换 | ✅ `.ccprofile` | ❌ | ⚠️ |

### 10. 配置参考

企业 Profile（`.ccprofile`）含品牌 / 插件清单 / 策略 / 预置 Space / 企业 Skill 包 / 签名；强制度 `overlay`（默认）/ `exclusive` / `locked`（见正文 6.1–6.3）。插件放入 `~/.chainlesschain/plugins/`，`plugin.json` 声明 `permissions` + `contributes`（见正文 7.1）。

### 11. 性能指标

UI 重构目标是**降低查找成本**而非吞吐：97 页 → 3 区壳，功能查找从「翻菜单」变为 `Ctrl+K` / `/` / `@` 即时唤起。冷启动与 IPC 往返指标见 [系统架构](/guide/architecture) 附录「11. 性能指标」。

### 12. 测试覆盖

V6 Shell 骨架与 Pinia stores 由桌面单测覆盖（store 专项测试见 [系统架构](/guide/architecture) 附录「12. 测试覆盖」）；UI 重构以 V5→V6 页面 port baseline 渐进迁移，每页带回归基线。

### 13. 安全考虑

- 环境级安全：U-Key / DID / 加密状态常驻状态栏，不跳独立页
- 默认所有数据 SQLCipher 本地加密；IPFS / 云同步需显式选择
- 无 U-Key 时自动降级软件密钥，需硬件签名的操作显式标风险等级；企业版可强制禁用降级
- 插件敏感能力（U-Key 等）需硬件确认（见正文 7.3）

### 14. 故障排除

见正文「8. 常见问题」：旧页面去向、无 U-Key 怎么用、公有 Marketplace 插件、桌面与 Web 面板分工、个人用户是否需懂 Profile / DID、数据是否在本地。

### 15. 关键文件

| 路径 | 说明 |
|---|---|
| `~/.chainlesschain/plugins/` | 用户插件目录 |
| `plugin.json` | 插件清单（id / permissions / contributes） |
| `.ccprofile` | 企业可分发定制包 |
| `docs/design/桌面版UI重构_设计文档.md` | 设计源文档（§4.2 扩展点接口） |

V6 Shell 渲染层文件树见 [系统架构](/guide/architecture) 附录「15. 关键文件」。

### 16. 使用示例

```jsonc
// 最小可用插件 plugin.json（放入 ~/.chainlesschain/plugins/ 重启生效）
{
  "id": "com.acme.my-plugin",
  "name": "My Plugin",
  "version": "0.1.0",
  "permissions": ["llm:query", "storage:write"],
  "contributes": { "slash": [{ "trigger": "hello", "handler": "./hello.js" }] }
}
```

### 17. 相关文档

- [桌面版 V6 Chat-First Shell](/guide/desktop-v6-shell)
- [系统架构](/guide/architecture)
- [快速开始](/guide/getting-started)
- 设计源：`docs/design/桌面版UI重构_设计文档.md`
