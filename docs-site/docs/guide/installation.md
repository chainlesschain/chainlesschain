...
2. **3–30 秒**：后台加载 146 个 Skill、初始化 P2P 节点、连接本地数据库
3. **30–60 秒**：主窗口出现
...
C:\Program Files\ChainlessChain\（散文件，asar:false）
   ▼
首次启动：splash → 加载 146 Skill → 初始化 P2P → 连接本地 DB → 主窗口
```

### 4. 系统定位

桌面客户端的**安装与生命周期层**——把 Electron 主进程、本地 SQLCipher 数据、146 内置 Skill 一次性落到用户机器，之后常驻托盘后台运行。
