...
> **版本: v1.0 (规划稿, 2026-05-10) | 状态: 📋 RFC 评审中 | Android v0.32.0 → v1.0.0 | 桌面对标: v5.0.3.46**
>
> ChainlessChain Android 客户端定位为 **DID 钱包 + 移动场景捕获 + REMOTE 遥控器**，不重复实现桌面 139 skills，专注桌面无法替代的手机独占价值。本文档同时面向用户与开发者。

## 概述

ChainlessChain 桌面端是"重资产个人 AI 主机"——139 内置 skills、Cowork 多智能体、RAG 检索、MCP、Marketplace、联邦治理、Workflow、ZKP，是一个完整的 AI 工作站。Android 端在过去版本里追求 skill 数量对标桌面，结果留下 8 个长期 doc-only stub 与模糊的产品定位。
...
| 独占能力   | MCP、Computer Use、139 skills、Cowork  | StrongBox、Voice Mode、摄像头、推送、生物识别 |
...
- **L2 捕获层**：摄像头 OCR、Voice Mode 连续语音、位置感知、推送通知——手机独占场景
- **L3 REMOTE 层**：所有桌面重计算 skill（RAG / Cowork / Marketplace / Workflow）通过 JSON-RPC 调用桌面，手机做触发与展示，不做 native port

## 核心特性

### L1 钥匙层（v1.0 必交付）

- 🔐 **StrongBox 硬件签名** — Android Keystore TEE/StrongBox 后端，私钥不出芯片
- 🆔 **DID 钱包** — Ed25519 + 助记词备份 + 多 DID 切换
- 👆 **生物识别解锁** — BiometricPrompt + 失败回退 PIN
- 📱 **QR 配对 / 授权** — 扫码加好友 / 扫码登录桌面 / 扫码授权 marketplace 操作
- ✍️ **签名即服务** — 桌面可发起 `sign.request` 让手机 StrongBox 签名（跨平台硬件 USB Key）
- 🛡️ **三阶段降级** — StrongBox → TEE → 软件 Ed25519，UI 标识当前签名等级

### L2 捕获层（v1.0 必交付）

- 📸 **拍照笔记 + OCR** — 摄像头一拍 → REMOTE LLM OCR → 自动入桌面 KB
- 🎤 **Voice Mode** — 长按说话 / 双击启动连续对话；ASR → REMOTE LLM → TTS
- 📍 **位置时间打卡** — GPS + 时间戳标记笔记 / 任务（桌面无 GPS）
- 🔔 **推送通知** — 朋友消息 / agent 跑完 / marketplace 提醒 / 同步异常 / 待签名请求
- 🧾 **快速捕获** — 锁屏 widget / 共享菜单接收文本 / 图片 / 链接

### L3 REMOTE 层（v1.0 必交付）

- 🌐 **统一 REMOTE skill 表** — 所有桌面 skill 通过 `RemoteCommandClient` 调用
- 📡 **JSON-RPC over WebRTC** — 复用 Phase 3d v1.3 已落地的 transport
...
| (NOTIFICATION) | 计划中   | v1.1                         |

## 配置参考

### Android 端（`mobile-app/app/src/main/assets/cc-mobile.json`）
...
```jsonc
{
  "mobileBridge": {
    "enabled": true,
    "exposeRemoteSkills": [                // 白名单：仅这些 skill 可被手机调用
      "rag.search", "rag.embed",
      "cowork.*",
      "marketplace.browse", "marketplace.purchase",
      "workflow.run", "workflow.status",
      "ocr.process", "chat.stream"
    ],
...
L3 REMOTE 层：
✅ RemoteSkillRegistryTest          - 15 用例（拉取/缓存/版本协商）
...
1. **白名单（防越权）** — 桌面 `exposeRemoteSkills` 严格白名单，未列出的 skill 即便手机请求也拒绝
...
补充：
4. **防重放** — RPC envelope 含 nonce + timestamp，桌面 5 分钟窗口内不重收
5. **传输加密** — Signal Protocol over WebRTC DataChannel（继承 Phase 3d）
...
4. **Root 检测** — 检测 root / debuggable 时降级（StrongBox 必检失败警告，但不强制阻断）
...
### Q: 8 个 doc-only stub 的功能现在去哪了？

**A**: v1.0 处置后：
- trading / marketplace / cowork / BI → 转 REMOTE，桌面侧白名单开启即可使用
- workflow / 多模态 → 归并到对应 cli-* 文档，REMOTE 调用
- agent federation / 自治 agent runner → 删除（不在 mobile 范围）
...
> 源码位置以 v1.0 实施完成后为准。⏳ 标识尚未创建。

### Android 端（`mobile-app/`）

```
mobile-app/app/src/main/java/com/chainlesschain/android/
├── wallet/                                     # ⏳ L1 钥匙层
│   ├── StrongBoxKeyManager.kt
│   ├── DIDWallet.kt
│   ├── BiometricGate.kt
│   ├── QRPairing.kt
│   └── SignAsService.kt
...
├── sync/mobile-bridge-sync.js                  # ✅ Phase 3d
├── remote/command-handler.js                   # ✅ 现有，v1.0 加 sign.request
├── remote/mobile-skill-whitelist.js            # ⏳ 新增（exposeRemoteSkills）
├── remote/mobile-approval-channel.js           # ⏳ 新增（审批回路）
└── config/unified-config-manager.js            # ✅ 现有，加 mobileBridge.*
```

### 配置与文档

```
mobile-app/app/src/main/assets/cc-mobile.json   # ⏳ Android 端配置
.chainlesschain/config.json                     # ✅ 加 mobileBridge.* 字段
docs/design/Android_重新定位_设计文档.md         # ✅ 设计文档源
docs-site/docs/chainlesschain/mobile-positioning.md  # ✅ 本文档（用户文档源）
```

### 测试

```
mobile-app/app/src/test/.../wallet/             # ⏳ L1 单测（99 用例）
mobile-app/app/src/test/.../capture/            # ⏳ L2 单测（42 用例）
mobile-app/app/src/test/.../remote/             # ⏳ L3 单测（55 用例）
mobile-app/app/src/androidTest/.../e2e/         # ⏳ E2E（9 场景）
...
> **变更记录**
> - 2026-05-10 v1.0 (规划稿)：初稿，三层定位 + 配置 + 性能 + 测试 + 安全 + FAQ
