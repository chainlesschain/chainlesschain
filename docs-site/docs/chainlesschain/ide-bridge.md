...
> **版本：Design Module 98（机制篇，更新 2026-08-27）| 公开版：VS Code `0.37.71` Open VSX / JetBrains `0.4.100` Marketplace | Git 源码：`0.37.71` / `0.4.102`**
>
> Open VSX `0.37.71` 已公开、累计下载已突破 **3 万**；JetBrains Marketplace `0.4.100` 已审核、listed 并公开。两端消费 Agent Protocol `0.1.5` 生成的 stream event payload union 并通过跨端 causal conformance；VS Code 继续提供 CLI `0.166.6` 的 exact turn/session 结构化审批、无正文 canonical message/handoff 健康投影，以及默认关闭的固定能力 App Server pilot。`0.37.71` 另补齐市场 Overview/直接 VSIX 链接一致性与有界 Agent IPC 配对说明；JetBrains `0.4.102` 已上传待审。微软 VS Code Marketplace 仍未发布。
>
> 公开市场版本与 CLI `0.166.6@f2a249bf3d` 的发布证据分别回读；后续源码不能继承该发布授权。Agent Platform 的消息、handoff 与授权 authority 仍留在 CLI-owned bridge，IDE 只提交宿主已审阅的决定并消费无内容健康投影。
>
...
>
> 📖 本页讲**桥接机制**（发现协议 / lockfile / MCP 传输 / 安全模型）。插件的**日常使用**（Chat 面板全功能、Plan 审阅、Diff 评审、各面板、快捷键、完整设置面）见 [IDE 插件使用指南](/chainlesschain/ide-plugin)。
...
编辑器 server 以保留名 **`ide`** 接入，工具自动进入 agent 循环。最初的核心 5 工具如下；后续已扩展到 **9+**（新增 `getActiveFile` / `openMultiDiff` / `getTerminalOutput` / `getPreviewState`，完整工具表见 [IDE 插件使用指南](/chainlesschain/ide-plugin)「系统架构」节）：
...
**会话选择器**(0.10.0):`/sessions` 弹原生 QuickPick 列出已存会话(`cc session list`),
选中即切换续接。
...
**面板斜杠命令**(扩展 0.9.0+):输入框支持 `/new`(新会话)、`/plan`(进出 Plan 模式)、
`/approve` / `/reject`(裁决当前 plan 或审批卡)、`/stop`(中断回合)、`/help`,与按钮等价、键盘流不离手。
面板的 LLM 可经 `chainlesschain.chat.provider` / `chainlesschain.chat.model` 设置单独指定
(留空 = 跟随 CLI 自身配置)。

### 9. 结构化授权、payload union 与事件投影（0.37.71 / 0.4.100）
...
cc --version          # 生产推荐 0.166.6；该桥接路径最低 0.162.39
...
- _（官方 VS Code）_ 不要使用 Open VSX 页面的通用 **Install** 链接（它会跳到未上架的 Microsoft Marketplace）。请直接下载 [0.37.71 VSIX](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.71/file/chainlesschain.chainlesschain-ide-0.37.71.vsix)，再运行 **Extensions: Install from VSIX...**。
...
- **已过审上架 JetBrains Marketplace**（`com.chainlesschain.ide`，当前 v0.4.100）：IDE 的 _Settings → Plugins → Marketplace_ 搜 **ChainlessChain IDE** 一键装（2024.2+），或访问 [plugin 32208](https://plugins.jetbrains.com/plugin/32208-chainlesschain-ide-bridge)。源码/上传身份 `0.4.102` 已过六宿主发布门，仍待市场审核。
...
(或 Chat 面板标题栏的 **⚙** 按钮);JetBrains 系用 **Tools → ChainlessChain: Configure LLM(配置大模型)** —— 选提供商
...
- VS Code：命令面板 _ChainlessChain IDE: Show Bridge Status_ / _Restart Bridge_；输出面板 "ChainlessChain IDE" 频道。
- JetBrains：Tools 菜单 _ChainlessChain IDE: Show Bridge Status_。
...
- [`cc agent` 托管智能体](/chainlesschain/cli-agent) — agent CLI 与 `## IDE Bridge` / `## PDH 桥接` 章节
