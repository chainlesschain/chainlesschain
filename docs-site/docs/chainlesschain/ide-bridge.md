...
> **版本：Design Module 98（机制篇，更新 2026-09-04）| 公开版：VS Code `0.37.81` Open VSX / JetBrains `0.4.110` Marketplace**
>
> Open VSX `0.37.81` 已公开、累计下载已突破 **3.6 万**；JetBrains Marketplace `0.4.110` 已完成公共回读。两端推荐 CLI `0.166.21`，并加入受治理 Evolution Workbench 与证据排序 Skill Retrieval。IDE 投影继续由 CLI/部署宿主裁决，默认关闭的固定能力 App Server pilot 不提供任意请求逃生口。微软 VS Code Marketplace 仍未发布。
>
> CLI `0.166.21@1ff70b7856`、Open VSX `0.37.81` 与 JetBrains Marketplace `0.4.110` 保持独立制品身份。后续源码不能继承已发布授权。Agent Platform 的消息、handoff、Skill mutation 与 knowledge merge authority 仍留在 CLI-owned bridge/部署宿主，IDE 只提交宿主已审阅的决定并消费有界投影。
>
...
### 9. 结构化授权、payload union 与事件投影（0.37.81 / 0.4.110）
...
cc --version          # 生产推荐 0.166.21；该桥接路径最低 0.162.39
...
- _（官方 VS Code）_ 不要使用 Open VSX 页面的通用 **Install** 链接（它会跳到未上架的 Microsoft Marketplace）。请直接下载 [0.37.81 VSIX](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.81/file/chainlesschain.chainlesschain-ide-0.37.81.vsix)，再运行 **Extensions: Install from VSIX...**。
...
- **已过审上架 JetBrains Marketplace**（`com.chainlesschain.ide`，当前 v0.4.110）：IDE 的 _Settings → Plugins → Marketplace_ 搜 **ChainlessChain IDE** 一键装（2024.2+），或访问 [plugin 32208](https://plugins.jetbrains.com/plugin/32208-chainlesschain-ide-bridge)。
