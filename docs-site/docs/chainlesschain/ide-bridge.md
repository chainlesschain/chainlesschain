...
> **版本：Design Module 98（机制篇，更新 2026-09-03）| 公开版：VS Code `0.37.80` Open VSX / JetBrains `0.4.108` Marketplace**
>
> Open VSX `0.37.80` 已公开、累计下载已突破 **3.5 万**；JetBrains Marketplace `0.4.108` 已完成公共回读。VS Code 推荐 CLI `0.166.20`；JetBrains 源码/tag `0.4.109` 已配对 `0.166.20`，但等待 Marketplace 公共可见性。IDE 投影继续只读，默认关闭的固定能力 App Server pilot 不提供任意请求逃生口。微软 VS Code Marketplace 仍未发布。
>
> CLI `0.166.20@75a3339714`、Open VSX `0.37.80`、JetBrains Marketplace `0.4.108` 与源码 `main@c681e2b91d` 保持独立制品身份。后续源码不能继承已发布授权。Agent Platform 的消息、handoff 与授权 authority 仍留在 CLI-owned bridge，IDE 只提交宿主已审阅的决定并消费无内容健康投影。
>
...
### 9. 结构化授权、payload union 与事件投影（0.37.80 / 0.4.108）
...
cc --version          # 生产推荐 0.166.20；该桥接路径最低 0.162.39
...
- _（官方 VS Code）_ 不要使用 Open VSX 页面的通用 **Install** 链接（它会跳到未上架的 Microsoft Marketplace）。请直接下载 [0.37.80 VSIX](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.80/file/chainlesschain.chainlesschain-ide-0.37.80.vsix)，再运行 **Extensions: Install from VSIX...**。
...
- **已过审上架 JetBrains Marketplace**（`com.chainlesschain.ide`，当前 v0.4.108）：IDE 的 _Settings → Plugins → Marketplace_ 搜 **ChainlessChain IDE** 一键装（2024.2+），或访问 [plugin 32208](https://plugins.jetbrains.com/plugin/32208-chainlesschain-ide-bridge)。
