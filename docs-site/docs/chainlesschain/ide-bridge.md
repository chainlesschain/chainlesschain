...
> **版本：Design Module 98（机制篇，更新 2026-08-31）| 公开版：VS Code `0.37.76` Open VSX / JetBrains `0.4.106` Marketplace**
>
> Open VSX `0.37.76` 已公开、累计下载已突破 **3.3 万**；JetBrains Marketplace `0.4.106` 已审核、listed 并公开。两端消费 Agent Protocol `0.1.7` 生成的 stream event payload union，并推荐 CLI `0.166.14`。IDE 投影继续只读，默认关闭的固定能力 App Server pilot 不提供任意请求逃生口。微软 VS Code Marketplace 仍未发布。
>
> CLI `0.166.14@ee88125256` 与 IDE `0.37.76/0.4.106@0f0b9f7c8c` 分别完成自己的发布与公网回读；两者不是同一制品 SHA。后续源码不能继承这些发布授权。Agent Platform 的消息、handoff 与授权 authority 仍留在 CLI-owned bridge，IDE 只提交宿主已审阅的决定并消费无内容健康投影。
>
...
### 9. 结构化授权、payload union 与事件投影（0.37.76 / 0.4.106）
...
cc --version          # 生产推荐 0.166.14；该桥接路径最低 0.162.39
...
- _（官方 VS Code）_ 不要使用 Open VSX 页面的通用 **Install** 链接（它会跳到未上架的 Microsoft Marketplace）。请直接下载 [0.37.76 VSIX](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.76/file/chainlesschain.chainlesschain-ide-0.37.76.vsix)，再运行 **Extensions: Install from VSIX...**。
...
- **已过审上架 JetBrains Marketplace**（`com.chainlesschain.ide`，当前 v0.4.106）：IDE 的 _Settings → Plugins → Marketplace_ 搜 **ChainlessChain IDE** 一键装（2024.2+），或访问 [plugin 32208](https://plugins.jetbrains.com/plugin/32208-chainlesschain-ide-bridge)。
