...
> 适用版本：Agent Platform CLI `0.166.70`；更新：2026-09-22
>
> 适用对象：使用学习合成、Evolution Workbench、证据排序 Skill Retrieval、Desktop Skill Creator、Skill Sync 或加密知识同步的用户与管理员

> 发布状态：npm `chainlesschain@0.166.70` 是当前 `latest`，对应标签 `v-npm-0-166-70` 与提交 `75778a75e1`。安装或升级 CLI：`npm i -g chainlesschain@0.166.70`。Open VSX `0.37.112` 已公开；JetBrains `0.4.133` 已上传待 Marketplace 审核，当前公开版为 `0.4.132`。
...
> `0.166.68` 历史版将浏览器动作与下载纳入显式 authority，把下载字节先放入可恢复、可撤销、可审计销毁的耐久隔离区；PM runner/reviewer/grader 的结果绑定有界子进程与执行上下文。`0.166.70` 另加入默认关闭的 Jev 决策试点；它只建议、不执行 Skill，也不改变 candidate、review、promotion 或 active mutation 的 authority 边界。
...
`cc --version` 应输出 `0.166.70`。公开版本的正确行为是：
