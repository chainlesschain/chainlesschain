...
- Typed artifacts：共享 Skill/Prompt/Hook/Knowledge schema、类型隔离 policy/authority、Prompt/Hook candidate gate、Hook 高风险硬门与依赖 stale 级联；Skill Sync 成功结果已强制进入共享 Skill gate，旧 Skill-only store 只作为 tenant-bound 持久后端，knowledge 统一 adapter 尚待收口。
