...
```bash
# 1. Scoped Memory：按项目/会话粒度写入
chainlesschain memory scope set --project chainlesschain --session sess_123
chainlesschain memory put "首选测试框架=vitest" --scope project
chainlesschain memory list --scope session

# 2. Session Approval Policy：对敏感命令启用审批
chainlesschain session policy set --mode ask-on-write --tools "shell,file-write"
chainlesschain session policy show
chainlesschain session approval list           # 查看待审批队列
chainlesschain session approval approve <id>

# 3. Beta Flags：按需打开对标能力
chainlesschain config beta enable scoped-memory
chainlesschain config beta enable approval-policy
chainlesschain config beta list

# 4. 典型联动流程
chainlesschain chat "帮我重构 src/cache.js" \
  --scope project --policy ask-on-write
# → 命中 file-write 时自动进入审批；approve 后继续执行
```
