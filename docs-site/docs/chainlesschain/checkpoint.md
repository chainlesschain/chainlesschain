...
> **当前稳定版: 0.163.2 | npm latest 与生产推荐一致 | 双引擎 (git-plumbing + copy fallback) | 持久 Restore Saga + 保守 Recovery CLI**
>
...
| `recovery` 子命令不存在    | 使用的是早于 `0.162.197` 的 CLI                         | 升级并固定 `chainlesschain@0.163.2`                    |
