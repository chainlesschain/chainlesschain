...
> 适用版本：生产推荐与 npm `latest` 均为 `chainlesschain@0.166.14`（精确发布 SHA `ee88125256`）｜性质：CLI 内置执行内核，不是独立 daemon 或独立安装包｜适用对象：CLI 用户、SDK/App Server 集成方与运维人员
...
- Agent Kernel 首次随 `chainlesschain@0.166.0` 发布，当前公开稳定基线为 `0.166.14`；它不是独立 npm 包或公网服务。
