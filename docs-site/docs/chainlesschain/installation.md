...
> **2026-08-08 发布提示**：生产环境请固定 `chainlesschain@0.163.1`。npm `latest`、源码包元数据与生产推荐版已经对齐；该版本在精确 tag SHA 上完整通过 Ubuntu / Windows / macOS 权威门禁，并完成不可变制品、SBOM、provenance 与 npm registry/npmmirror 回读。
...
- ⚡ **CLI 安装**: `npm install -g chainlesschain@0.163.1` 一行命令（支持 `cc` / `clc` / `clchain` 短命令别名）
...
```
                   安装部署架构
┌─────────────────────────────────────────┐
│  npm install -g chainlesschain@0.163.1  │  CLI 工具
│         │                               │
│         ▼                               │
