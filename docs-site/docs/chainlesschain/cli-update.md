...
- 🔹 **仅检查模式**: 只检查是否有更新，不下载
- 🔹 **启动被动提醒**（v0.162.41+）: 任意 `cc` 命令启动时同步读一次缓存（`~/.chainlesschain/update-check.json`），有新版则向 stderr 打一行灰字提示（仅 TTY，不污染管道/JSON 输出）；缓存超过 24h 由 detached 子进程后台刷新供下次启动用——热路径零网络。`CC_UPDATE_NOTICE=0` 关闭。
