...
> 状态：P2-16 已完成并随 CLI `0.162.189` 首次公开；当前生产推荐版为 `0.165.6`（2026-08-22）。`0.163.8` 新增受治理的 file/hunk merge review，`0.164.0` 加入作用域执行与上下文权限，`0.165.1` 加固远程成员权威与 canonical 执行边界，`0.165.2–0.165.6` 又把 child-call receipt、会话预算、远端结果审阅/应用、workflow recovery、跨会话消息与 MCP 生命周期恢复纳入耐久 authority；实现候选
