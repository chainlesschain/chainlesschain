...
> **状态（2026-08-20）：92 个采集契约 / 18 类已注册；PDH 0.4.59 与 CLI 0.165.4 均已成为 npm `latest`。0.4.59 将 `better-sqlite3-multiple-ciphers` 改为 optional dependency；无 native prebuild、Python 或编译器时首次安装可跳过 native addon 并使用 CLI 内置 `sql.js` WASM fallback。92 是能力清单，不代表每个来源都已在当前设备或账号上可用；页面会按真实输入与宿主能力显示“可采集 / 需配置 / 不可用”。**
>
...
> 注：改动 `packages/personal-data-hub/lib/**` 发版时必同步 bump pdh 包 version + npm publish + Android `USR_VERSION`（否则真机走 fast-path 跳解压用旧代码）。当前公网版本为 PDH 0.4.59 / CLI 0.165.4；Android runtime bundle 仍按独立发布步骤与应用版本同步。
