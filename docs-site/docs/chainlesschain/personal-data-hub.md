...
> **状态（2026-08-31）：92 个采集契约 / 18 类已注册；PDH `0.4.59` 与 CLI `0.166.14` 分别为当前 npm `latest`。PDH 0.4.59 将 `better-sqlite3-multiple-ciphers` 改为 optional dependency；无 native prebuild、Python 或编译器时首次安装可跳过 native addon 并使用 CLI 内置 `sql.js` WASM fallback。92 是能力清单，不代表每个来源都已在当前设备或账号上可用；页面会按真实输入与宿主能力显示“可采集 / 需配置 / 不可用”。**
>
...
> 注：改动 `packages/personal-data-hub/lib/**` 发版时必同步 bump pdh 包 version + npm publish + Android `USR_VERSION`（否则真机走 fast-path 跳解压用旧代码）。当前公网版本为 PDH 0.4.59 / CLI 0.166.14；Android runtime bundle 仍按独立发布步骤与应用版本同步。
...
> 截至 2026-07-23：路线图 adapter 已从 placeholder/scaffold 升级为可执行的 snapshot、SQLite、ADB 或 cookie 采集路径；“代码路径已接通”不等于“当前线上端点已用真账号验收”。v1 sign-off 仍需在用户本人设备和账号上验证 AIChat 私有接口字段、头条/快手等动态签名命中率、WhatsApp 真机备份 UX，以及微信等 root 路径。
...
| **Social × 6** (Bilibili / Weibo / Douyin / Xiaohongshu / Toutiao / Kuaishou) | ✅ × 6 采集路径已接通；真账号按平台验收                                         | Android 本机 SQLite (dual-mode snapshot + sqlite) 或桌面 cookie web API：B 站 history / 微博 timeline (`c087c36eb`) / 抖音 watch (`20f9b2188`) / 小红书 收藏 (`20f9b2188`) / 头条 read_history (`e1155b1d7`) / 快手 photo_history (`e1155b1d7`)        | Mobile Extraction Layer pull / Android in-app collector / sjqz parser 移植                                                                                                       | snapshot/sqlite/ADB 结构回归已覆盖；动态签名和新版加密库仍以真机 E2E 为发布闸门                                                                                                                                          |
| **Messaging × 3** (QQ / Telegram / WhatsApp)                                  | ✅ 三家 Android 本地采集                                                        | QQ Android **普通 SQLite + msgData XOR(IMEI) 加密**；Telegram `cache4.db`；WhatsApp 可由桌面/CLI ADB 从公共备份目录自动拉取（无需 root），或导入明文 `msgstore.db` / 用户自有 key 的 crypt14 / crypt15，并兼容新旧消息表、媒体、位置、名片、引用与通话 | QQ XOR 解密与 dual-mode adapter；WhatsApp ADB 路径白名单 + 流式 AES-GCM 认证解密 + zlib + SQLite 校验，拉取副本与临时明文自动清理                                                | WhatsApp 32 单测 + 上游真实 crypt14 / crypt15 / msgstore fixture 交叉验证；QQ Kotlin 27 unit + JS snapshot 13 + longtail 6                                                                                               |
...
| 10.2 (+)                   | **AIChatHistoryAdapter 9th: 豆包 Doubao** v0.2             | cookie/snapshot 采集、cursor 分页与 `has_more` 已接通；私有 endpoint 字段仍待 Phase 10.4 真实 HAR fixture pin + 真账号 smoke                                                                                                                                                                                                                                                                                                                                                                                                                                                | 结构/错误路径回归已覆盖；v1 sign-off 待账号                                                                                                                                                                                                                                                                                          |
...
| 13.8 (+)                   | **Social Toutiao (今日头条)** v0.2.1 dual-mode             | snapshot + Android SQLite：profile / read_history / collection_article / search_history；桌面/CLI 与 Android ADB cookie/sign/root collector 已接线，真机端点命中率按 E2E runbook 验收                                                                                                                                                                                                                                                                                                                                                                                       | snapshot/sqlite/ADB 回归已覆盖                                                                                                                                                                                                                                                                                                       |
| 13.9 (+)                   | **Social Kuaishou (快手)** v0.2.1 dual-mode                | snapshot + Android SQLite：profile / photo_history / user_collect / search_record；桌面/CLI 与 Android ADB cookie/sign/root collector 已接线，`__NS_sig3` 真机命中率按 E2E runbook 验收                                                                                                                                                                                                                                                                                                                                                                                     | snapshot/sqlite/ADB 回归已覆盖                                                                                                                                                                                                                                                                                                       |
...
> **状态**：采集卡片和执行链已覆盖 Bilibili / 微博 / 抖音 / 小红书 / 今日头条 / 快手，并支持 cookie/snapshot 与可用时的 root SQLite 路径；当前环境没有 ADB 设备，动态签名和账号数据仍须按 E2E runbook 在真机验收。
...
1. **本机数据** (system-data-android) ✅ — ContentResolver 通讯录 + PackageManager 已装应用
2. **Bilibili** (social-bilibili) ✅ — 观看历史 / 收藏 / 动态 / 关注
3. **微博** (social-weibo) ✅ — cookie API + snapshot/root SQLite 路径
4. **抖音** (social-douyin) ✅ — cookie/SignBridge + snapshot/root SQLite 路径；线上签名命中率待真机验收
5. **小红书** (social-xiaohongshu) ✅ — cookie/SignBridge + snapshot/root SQLite 路径；`X-S`/`b1` 版本兼容待真机验收
6. **今日头条** (social-toutiao) ✅ — cookie/SignBridge + snapshot/root SQLite 路径；`_signature` 待真机验收
7. **快手** (social-kuaishou) ✅ — cookie/SignBridge + snapshot/root SQLite 路径；`__NS_sig3` 待真机验收
...
- 微博：WebView `SUB` cookie、UID 恢复和 `/api/container/getIndex` 已接线；真账号确认风控返回码。
- 抖音：SignBridge / 本地 DB 双路已接线；确认当前 App/Web 版本的 msToken、X-Bogus 命中率。
- 小红书：SignBridge / 本地 DB 双路已接线；确认当前 `X-S`、`b1` 组合。
- 今日头条 / 快手：分别确认 `_signature` / `__NS_sig3`，失败时可退回用户本人 rooted-device SQLite 路径。
- Bilibili：如官方收紧 wbi-\* endpoint，按 runbook 更新 WBI fixture。
...
> 复现坑（写权限须 App 可写目录 / 必须等 libwcdb2 加载后再 attach / 点开会话才有查询 / DB*MATCH 用 `/encrypted*.\*\.db$/`/ export 走 onLeave 防重入）+ checklist 见`docs/internal/pdh-db-decryption-runbook.md` §3.6.1。
...
- **覆盖矩阵**：
  ...
  - `__tests__/whatsapp-adapter.test.js` + `adapters/messaging-whatsapp.test.js` + `adapters/whatsapp-backup-decryptor.test.js` + `adapters/whatsapp-adb-extension.test.js` — Phase 13.7b WhatsApp ADB public-backup pull + crypt14/crypt15 authenticated streaming decrypt + modern/legacy sqlite parse
...
- **需外部条件覆盖**：真账号 IMAP / 真 Alipay ZIP / 9 厂商 AIChat cookie / 社交平台动态签名 / WhatsApp 真机备份 + 用户 key / rooted 微信 DB 端到端 — 走 `Personal_Data_Hub_E2E_Runbook.md` 手工跑
...
| `adapters/messaging-{qq,telegram,whatsapp}/index.js`                                   | Phase 13 三 messaging adapter（含 WhatsApp crypt14 / crypt15 本地直解）     |
...
```js
// 1) 测试授权（不持久化）
const probe = await window.electron.invoke(
...
- [代码路径 → v1 Fixture Pin Protocol](https://design.chainlesschain.com/Personal_Data_Hub_Fixture_Pin_Protocol.html) — 通用方法论：HAR/DB capture → field-rename-map → fixture-driven test → 真账号 smoke → v1 sign-off gate；当前覆盖 Doubao (Phase 10.4) + Toutiao/Kuaishou (Phase 13.10) 等待验收路径
