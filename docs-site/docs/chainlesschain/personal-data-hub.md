...
> **版本: v5.0.3.75 (Phase 0–13 + 14.1/14.2/14.3 全部落地 + Phase 10.3.6 CLI + Phase 12.6.7 WeChat bootstrap, 2026-05-21) | 状态: ✅ 可用 — 19 个 Adapter ✅ + 3 个 v0.1 scaffold (豆包 Doubao + 今日头条 Toutiao + 快手 Kuaishou) — 总计 22 (Email + Alipay + SystemData + WeChat v0.5 + 4 Travel + 3 Shopping + 6 Social + 3 Messaging + 9 AIChat 厂商) + Mobile Extraction Layer (Android ADB / iOS iTunes backup) + EntityResolver 跨源人物归并 + 5 内置 Analysis Skill (Spending / Relations / Footprint / Interests / Timeline) | **25 IPC 通道 + 25 WS 主题** (Phase 10.3 wizard 4 通道双端 wire 完成) + Phase 14.1 Android + Phase 14.2 iOS 全 `PersonalDataHubCommands` 22 method typed wrapper | **63 测试文件 / 1188 测试** (含 6 集成 + 3 E2E 场景 + 16 新 scaffold + 22 Android PDH 单测 + 102 Phase 10.3 wizard 单测 + 14 Phase 12.6.7 bootstrap + iOS 1491 LOC Hub tests) | 设计文档: 13-Phase 路线图 + 8 个 Adapter 专题 + EntityResolver + sjqz 借鉴比对 + Phase 14 移动端原生入口 + Phase 10.3 AIChat WebView 鉴权向导 + **Phase 12.6.7 WeChat bootstrap 编排层**
>
...
| 当前合计 | — | — | **63 文件 / 1173 测试**（含 Phase 10.2 集成 + E2E 6 + 3 场景；doubao scaffold + toutiao/kuaishou scaffold + analysis-skills backfill + WeChat Phase 12.6 §18.1-6 KeyProvider + Frida agent + env-probe + Setup + 12.6.7 bootstrap + 2026-05-21 Phase 10.3.1 cookie-capture-spec 21 + 10.3.2 wizard-controller 31 + 10.3.3 factory+IPC 20 + 10.3.4 cli wizard+WS 17 + 10.3.5 health-checker 13 + 10.3.6 CLI 15 + 10.3 E2E integration 4 + 12.6.7 bootstrap 14 单测）|

> 全部路线图 phase 已实施完成。剩余 follow-up：Phase 10.3+ (9 厂商 AIChat WebView UI 鉴权向导 + 真账号 smoke) — **设计稿 + cookie-capture-spec + wizard-controller + 桌面 IPC + cli WS + HealthChecker + `cc hub aichat` CLI + 117 单测全 land**；Vue 组件 wiring (web-panel `AIChatWizard.vue`) 已 land (070af86be)，剩真账号 smoke (blocked on accounts)；Phase 12.6+ (WeChat 8.0+ Android frida-dep libwcdb hook 路径) **§18.1-7 已全 land** (含 12.6.7 bootstrap)，剩 12.6.8-10 (`register-wechat` IPC + `cc hub wechat` CLI + Vue UI 卡片) + Phase 12.9 (rooted device 真机 E2E)。
