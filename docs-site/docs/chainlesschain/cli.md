...
> **版本: v0.37.10 | 状态: ✅ 生产就绪 | 1009 测试通过 | 37 命令 | 纯 JS 无原生依赖**
...
- 🤖 **37 个命令**: AI 对话、笔记管理、DID 身份、加密解密、项目初始化、多智能体协作等
...
- 🧪 **1009 测试通过**: 118 核心包测试 + 891 CLI 测试，55 个测试文件
...
- `packages/cli/src/commands/` — 所有命令实现（37 个命令）
- `packages/cli/src/lib/` — 共享库（42 个模块）
