...
> **版本: v0.37.6 | 90内置技能 | Agent Skills开放标准 | 统一工具注册表**

Skills 系统提供 90 个内置技能，使用 Markdown 定义技能(SKILL.md)，支持四层加载、Agent Skills 开放标准(13扩展字段)、门控检查和自定义命令。v0.37.6 新增 10 个系统+安全+设计+分析技能（backup-manager、query-enhancer、memory-insights、data-exporter、crypto-toolkit、network-diagnostics、password-generator、text-transformer、color-picker、performance-profiler），Handler 覆盖率 90/90 (100%)。
...
marketplace/   # 插件市场安装的技能 (v0.34.0新增)
    ↓
bundled/       # 内置技能（90个，100% Handler覆盖，最低优先级）
```
...
**50个内置技能 (100% Handler覆盖) + Agent Skills标准 + 统一工具注册表** 🛠️
