...
> **版本: v0.36.1 | 40内置技能 | Agent Skills开放标准 | 统一工具注册表**

Skills 系统提供 40 个内置技能，使用 Markdown 定义技能(SKILL.md)，支持四层加载、Agent Skills 开放标准(13扩展字段)、门控检查和自定义命令。v0.36.1 新增 10 个高级技能（architect-mode、bugbot、commit-splitter、diff-previewer、fault-localizer、impact-analyzer、research-agent、rules-engine、screenshot-to-code、task-decomposer），Handler 覆盖率 40/40 (100%)。
...
marketplace/   # 插件市场安装的技能 (v0.34.0新增)
    ↓
bundled/       # 内置技能（40个，100% Handler覆盖，最低优先级）
```
...
**40个内置技能 (100% Handler覆盖) + Agent Skills标准 + 统一工具注册表** 🛠️
