...
在 `~/.chainlesschain/config.json` 中配置代币激励参数：
...
| 配置项                               | 类型   | 默认值  | 说明                        |
| ------------------------------------ | ------ | ------- | --------------------------- |
| `tokenIncentive.currency`            | string | `"CCT"` | 代币货币单位标识            |
| `rewardMultipliers.skill`            | number | `1.0`   | 技能贡献的奖励倍数          |
| `rewardMultipliers.gene`             | number | `1.2`   | Gene 贡献的奖励倍数（最高） |
| `reputationPricing.maxDiscount`      | number | `0.5`   | 最大信誉折扣（50%）         |
| `reputationPricing.discountPerPoint` | number | `0.1`   | 每点信誉对应折扣比例        |
| `ledger.maxTransactionHistory`       | number | `10000` | 本地账本最大保留交易数      |
...
| ------------------------ | ------ | ----- | ---- |
| 余额查询                 | <20ms  | ~10ms | ✅   |
| 贡献提交与奖励计算       | <100ms | ~65ms | ✅   |
| 信誉定价查询             | <30ms  | ~18ms | ✅   |
| 交易历史分页查询（20条） | <50ms  | ~28ms | ✅   |
| 奖励汇总统计             | <80ms  | ~45ms | ✅   |
| 贡献排行榜生成           | <150ms | ~90ms | ✅   |
...
// 2. 提交一项技能贡献
const contribution = await window.electron.ipcRenderer.invoke(
  "token:submit-contribution",
  {
    type: "skill",
    contributorDid: "did:example:alice",
...
console.log(
  `总奖励: ${summary.totalRewards} CCT, 奖励次数: ${summary.rewardCount}`,
);
```
...
console.log(
  `原价: ${pricing.pricing.basePrice}, 折扣: ${(pricing.pricing.reputationDiscount * 100).toFixed(0)}%, 最终: ${pricing.pricing.finalPrice} CCT`,
);

// 查看交易历史（按奖励类型过滤）
const txns = await window.electron.ipcRenderer.invoke(
...
txns.forEach((tx) =>
  console.log(`[${tx.type}] +${tx.amount} CCT - ${tx.description}`),
);
```
