...
```javascript
// 1. 创建凭证 (注册)
const credential = await window.electronAPI.invoke(
  "ukey:fido2-make-credential",
  {
    rpId: "example.com",
    userHandle: "user-123",
...
it("asserts after credential registration", async () => {
  ...
  const assertion = await fido2Driver.getAssertion({
    rpId: "test.local",
    challenge,
  });
...
it("parses APDU error status words correctly", async () => {
  ...
  await expect(usbTransport.sendApdu([0x00, 0xa4, 0x04, 0x00])).rejects.toThrow(
    "6A82",
  );
});
```
