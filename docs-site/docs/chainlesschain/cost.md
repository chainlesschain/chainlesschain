...
> **版本: Claude-Code cost 平价 | 状态: ✅ 生产可用 | 纯报表视图（不采集新数据）| 价格可配置覆盖**
>
...
Cost — global
  ...
  anthropic  claude-opus-4-8           $0.0980  in=12000 out=3400
...
覆盖 `mergePricing`（覆盖叠加）、`lookupRate`（命中/未命中）、`estimateCost`、`priceRollup`（free / matched / unpriced 分类与总额排除）。`cc cost` 命令在其上做展示层封装。
...
| `packages/cli/src/lib/llm-pricing.js` | `PRICE_TABLE` / `FREE_PROVIDERS` / `mergePricing` / `lookupRate` / `estimateCost` / `priceRollup` |
| `packages/cli/src/lib/session-usage.js` | `sessionUsage` / `allSessionsUsage`（token 聚合） |
| `packages/cli/__tests__/unit/llm-pricing.test.js` | 价格表与 rollup 单元测试 |
