...
// 计算影响力
const influence = await window.electronAPI.invoke(
  "social:calculate-influence",
  {
...
// 发现社区
const communities = await window.electronAPI.invoke(
  "social:detect-communities",
  {
    minSize: 3,
  },
);
...
// 3. 发现我的社区
const myCommunity = await window.electronAPI.invoke(
  "social:detect-communities",
  {
    minSize: 3,
  },
...
```javascript
// 识别社区中的关键意见领袖（KOL）
const influencers = await window.electronAPI.invoke(
  "social:calculate-influence",
  {
    did: "did:chainless:abc123",
  },
