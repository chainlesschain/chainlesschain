...
> **文档快照：2026-08-28 | Yjs CRDT | P2P 实时同步 | 有界文档/awareness/federation/gossip/mesh 生命周期 | 崩溃恢复**
...
2026-08-28 的远端主线 `5edef7544b` 及其前序维护进一步约束了 Yjs、组织知识同步、联邦模型 transport、社交 gossip 与 mesh retained state：文档、peer、awareness、离线编辑、replay、subscriber、IPC payload、stream、消息/peer 历史、传播队列与 mesh 留存记录都有默认上限与 TTL；进程重启从持久更新恢复，过期 generation、late callback 和销毁后的 transport 不能重新写回。达到容量上限时系统返回明确的过载/边界错误，不会无限增长内存队列。
...
- 🧯 **有界生命周期**: 活跃文档、协作者、awareness、离线编辑、回放字节与订阅者均有容量和 TTL
- ♻️ **可验证恢复**: 崩溃后从耐久 Yjs update 恢复；旧连接、旧 generation 与 late event 被 fencing
- 📡 **有界传播**: gossip 去重、peer 活跃集、消息留存、重试/超时与销毁清理都有明确容量和生命周期
