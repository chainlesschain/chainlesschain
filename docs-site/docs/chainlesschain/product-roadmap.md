# 产品演进路线图

> **当前版本**: v1.0.0 企业版 | **Cowork**: v2.1.0 | **SIMKey**: v0.39.0 | **最后更新**: 2026-02-26

本文档描述 ChainlessChain 产品的整体演进规划，涵盖 AI 智能体、安全硬件、去中心化社交、企业级平台四条主线。

> 各模块详细路线图：[Cowork 路线图](/chainlesschain/cowork-roadmap) | [更新日志](/changelog)

---

## 当前状态总览

### 平台矩阵

| 平台                          | 版本    | 技术栈                | 技能数                           | 状态     |
| ----------------------------- | ------- | --------------------- | -------------------------------- | -------- |
| Desktop (Windows/macOS/Linux) | v0.35.0 | Electron + Vue3       | 95                               | 生产就绪 |
| Android                       | v0.37.0 | Jetpack Compose       | 28 (5 LOCAL + 8 REMOTE + 15 doc) | 生产就绪 |
| iOS                           | v0.35.0 | SwiftUI               | —                                | 生产就绪 |
| Web 文档站                    | v0.39.0 | VitePress             | —                                | 已上线   |
| 后端服务                      | v1.0.0  | Spring Boot + FastAPI | —                                | 生产就绪 |

### 核心数据

| 指标       | 数值     |
| ---------- | -------- |
| 总代码行数 | 310,000+ |
| Vue 组件   | 370+     |
| IPC 处理器 | 380+     |
| 内置技能   | 95       |
| 数据库表   | 21+      |
| 测试用例   | 2,500+   |

---

## 演进主线

```
                    ChainlessChain 产品演进
                    ========================

  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐
  │  AI 智能体  │  │  安全硬件   │  │ 去中心化社交│  │  企业平台   │
  │  主线       │  │  主线       │  │  主线       │  │  主线       │
  └──────┬──────┘  └──────┬──────┘  └──────┬──────┘  └──────┬──────┘
         │                │                │                │
  v2.1.0 已完成    v0.39.0 已完成   v1.0.0 已完成    v1.0.0 已完成
  自进化+知识图谱  TEE+ZKP+卫星    社区+直播+协作    审计+SSO+CRDT
         │                │                │                │
         ▼                ▼                ▼                ▼
  ┌──────────────────────────────────────────────────────────────┐
  │              v1.1.0 — 2026 Q1-Q2 融合升级                   │
  │  Cowork v3.0-v3.3 | SIMKey v0.40 | 社交 AI | 企业合规      │
  └──────────────────────────────────────────────────────────────┘
         │
         ▼
  ┌──────────────────────────────────────────────────────────────┐
  │              v2.0.0 — 2026 H2 去中心化代理网络               │
  │  Cowork v4.0 | Agent DID | 联邦发现 | 跨组织协作             │
  └──────────────────────────────────────────────────────────────┘
```

---

## 第一阶段：v1.1.0 — 全栈智能化（2026 Q1-Q2）

### 主线 A：AI 智能体演进（Cowork v3.0-v3.3）

#### A1. 全自动开发流水线（v3.0）

**目标**: 从需求描述到生产部署全程 AI 代理协作，人工干预率 < 20%

```
需求 → 解析 → 架构 → 编码 → 测试 → 审查 → 部署 → 监控
  │      │       │      │      │      │      │      │
  NL   Parser  Orch.  Skills  VLoop  Debate Deploy Monitor
```

| 模块                  | 文件                     | 职责                                            |
| --------------------- | ------------------------ | ----------------------------------------------- |
| Pipeline Orchestrator | `pipeline-ipc.js`        | DAG 流水线编排，5 种步骤类型，10 个预置模板     |
| Requirement Parser    | `requirement-parser.js`  | NL→Spec JSON，用户故事/验收标准提取             |
| Deploy Agent          | `deploy-agent.js`        | 多环境部署（dev/staging/prod），蓝绿/金丝雀策略 |
| Post-Deploy Monitor   | `post-deploy-monitor.js` | 健康监控，KPI 基线偏差检测                      |
| Rollback Manager      | `rollback-manager.js`    | Git Revert/Docker/Config 多策略自动回滚         |

**KPI 目标**:

| 指标             | 目标值   |
| ---------------- | -------- |
| 人工干预率       | < 20%    |
| 需求→部署时间    | < 2 小时 |
| 流水线成功率     | > 85%    |
| 自动回滚响应时间 | < 30 秒  |

**新增**: 15 IPC 处理器，3 张数据库表

---

#### A2. 自然语言编程（v3.1）

**目标**: 用自然语言描述需求，代理团队自动实现符合项目编码约定的代码，需求→代码转化率 > 80%

| 模块                   | 文件                        | 职责                                            |
| ---------------------- | --------------------------- | ----------------------------------------------- |
| Spec Translator        | `spec-translator.js`        | NL→结构化 Spec 9 步翻译，9 种意图分类，LLM 增强 |
| Project Style Analyzer | `project-style-analyzer.js` | 编码约定提取（命名/缩进/注释），自动规则生成    |

**核心流程**:

```
用户自然语言 → 意图识别 → 上下文补全 → Spec 生成
                                         ↓
                              项目风格分析 ← CKG + Instinct
                                         ↓
                              代码生成（符合本地约定）
```

**新增**: 10 IPC 处理器，2 张数据库表

---

#### A3. 多模态协作（v3.2）

**目标**: 集成语音、视觉、文档等多模态输入输出，支持 5+ 模态

| 模块               | 文件                    | 职责                                       |
| ------------------ | ----------------------- | ------------------------------------------ |
| Modality Fusion    | `modality-fusion.js`    | 音频/图像/文档/屏幕/文本统一融合引擎       |
| Document Parser    | `document-parser.js`    | PDF/Word/Excel 解析，表格/图片提取，OCR    |
| Screen Recorder    | `screen-recorder.js`    | Electron desktopCapturer 截屏/录制         |
| Multimodal Context | `multimodal-context.js` | 多模态会话上下文，Token 预算控制           |
| Multimodal Output  | `multimodal-output.js`  | 富媒体输出（Markdown/HTML/ECharts/幻灯片） |

**新增**: 12 IPC 处理器，2 张数据库表

---

#### A4. 自主运维（v3.3）

**目标**: 代理自动监控、诊断、修复生产环境问题，MTTR < 5 分钟

| 模块                 | 文件                      | 职责                                         |
| -------------------- | ------------------------- | -------------------------------------------- |
| Alert Manager        | `alert-manager.js`        | 多通道告警（Webhook/Email/IM），P0-P3 升级链 |
| Auto Remediator      | `auto-remediator.js`      | Playbook 驱动自动修复，8 种动作类型          |
| Postmortem Generator | `postmortem-generator.js` | LLM 事故报告生成，根因分析，改进建议         |

**KPI 目标**:

| 指标            | 目标值   |
| --------------- | -------- |
| MTTR            | < 5 分钟 |
| 自动修复成功率  | > 80%    |
| 误报率          | < 5%     |
| Playbook 覆盖率 | > 70%    |

**新增**: 15 IPC 处理器，3 张数据库表

---

### 主线 B：安全硬件演进

#### B1. SIMKey v0.40 — 跨平台统一安全层

| 方向           | 内容                                          | 优先级 |
| -------------- | --------------------------------------------- | ------ |
| 统一密钥管理   | U盾 + SIMKey + TEE 三端密钥统一派生与轮换     | P0     |
| FIDO2 WebAuthn | SIMKey 作为 FIDO2 认证器，支持 Web 无密码登录 | P0     |
| 硬件安全聚合   | 多设备硬件安全模块聚合签名（2-of-3 阈值签名） | P1     |
| 生物特征绑定   | TEE 内生物特征模板与 SIMKey 密钥绑定          | P1     |
| 量子迁移路线   | ML-KEM/ML-DSA 全面替换 RSA/ECDSA 的迁移计划   | P2     |

#### B2. U盾 v2.0 — 跨平台扩展

| 方向             | 内容                                | 优先级 |
| ---------------- | ----------------------------------- | ------ |
| macOS/Linux 驱动 | 基于 libusb/WebUSB 的跨平台 U盾驱动 | P0     |
| 蓝牙 U盾         | BLE 协议支持，移动端无线连接        | P1     |
| U盾固件 OTA      | 安全固件远程升级通道                | P2     |

---

### 主线 C：去中心化社交演进

#### C1. 社交智能化

| 方向            | 内容                                               | 优先级 |
| --------------- | -------------------------------------------------- | ------ |
| AI 社交助手增强 | 上下文感知回复建议、话题深度分析、社交关系图谱     | P0     |
| 智能内容推荐    | 基于本地知识库和兴趣模型的去中心化推荐（无服务器） | P1     |
| AI 社区治理     | 自动提案分析、投票影响预测、治理参数优化建议       | P2     |

#### C2. 跨平台互通

| 方向             | 内容                                          | 优先级 |
| ---------------- | --------------------------------------------- | ------ |
| ActivityPub 完善 | Mastodon/Misskey 双向互通，评论/转发/点赞同步 | P0     |
| Nostr 桥接       | Nostr 事件格式兼容，中继发现                  | P1     |
| Matrix 集成      | Matrix 协议桥接，支持加密群聊互通             | P2     |

---

### 主线 D：企业平台演进

#### D1. 合规与治理

| 方向         | 内容                                            | 优先级 |
| ------------ | ----------------------------------------------- | ------ |
| SOC 2 合规包 | 自动化 SOC 2 Type II 证据收集与报告生成         | P0     |
| 数据分类分级 | 自动识别敏感数据（PII/PHI/PCI），标签与策略关联 | P0     |
| DLP 防泄漏   | 基于内容检测的数据防泄漏策略（审计日志集成）    | P1     |

#### D2. 企业集成

| 方向               | 内容                                             | 优先级 |
| ------------------ | ------------------------------------------------ | ------ |
| SCIM 用户同步      | SCIM 2.0 协议，与 Azure AD/Okta 自动同步用户和组 | P0     |
| SIEM 对接          | Splunk/ELK/Sentinel 审计日志实时推送             | P1     |
| Terraform Provider | 基础设施即代码管理 ChainlessChain 配置           | P2     |

---

### v1.1.0 汇总

| 维度            | 增量                    |
| --------------- | ----------------------- |
| 新增 IPC 处理器 | +72（总计 238+）        |
| 新增代码行      | +17,120（总计 ~45,120） |
| 新增数据库表    | +13（总计 21+）         |
| 新增模块        | 28 个                   |

---

## 第二阶段：v2.0.0 — 去中心化代理网络（2026 H2）

### Cowork v4.0 — 全球代理网络

**目标**: 基于 DID 的代理身份认证和跨组织协作，支持 100+ 节点

```
┌──────────────────────────────────────────────────────────┐
│                   去中心化代理网络                        │
│                                                          │
│   组织 A              KadDHT              组织 B         │
│  ┌────────┐     ┌──────────────┐     ┌────────┐         │
│  │Agent-α │────→│ 联邦代理注册 │←────│Agent-β │         │
│  │DID:cc:a│     │  技能发现    │     │DID:cc:b│         │
│  └────────┘     │  信誉评分    │     └────────┘         │
│       │         └──────────────┘          │              │
│       │              ↕                    │              │
│       └──── 跨组织任务委派 + VC 认证 ─────┘              │
│                                                          │
│   组织 C                                                 │
│  ┌────────┐                                              │
│  │Agent-γ │  ← 延迟感知路由，SLA 预算，全程审计          │
│  │DID:cc:c│                                              │
│  └────────┘                                              │
└──────────────────────────────────────────────────────────┘
```

| 模块                | 文件                          | 职责                                                     |
| ------------------- | ----------------------------- | -------------------------------------------------------- |
| Agent DID           | `agent-did.js`                | W3C DID 规范，`did:cc:agent-{uuid}` 格式，Ed25519 密钥对 |
| Credential Manager  | `agent-credential-manager.js` | W3C VC 签发/验证/吊销，3 种凭证类型                      |
| Agent Authenticator | `agent-authenticator.js`      | Challenge-Response / 凭证证明 / 双向 TLS                 |
| Federated Registry  | `federated-agent-registry.js` | KadDHT 去中心化发现，跨组织技能查询                      |
| Cross-Org Router    | `cross-org-task-router.js`    | 跨组织任务委派，SLA 预算，审计日志                       |
| Reputation System   | `agent-reputation.js`         | 多维信誉评分（0.0-1.0），时间衰减，4 级声望              |

**KPI 目标**:

| 指标             | 目标值  |
| ---------------- | ------- |
| 网络节点数       | ≥ 100   |
| 跨组织任务成功率 | > 90%   |
| DID 认证延迟     | < 500ms |
| 技能发现延迟     | < 2 秒  |
| 信誉更新一致性   | > 99%   |

**新增**: 20 IPC 处理器，3 张数据库表

---

## 第三阶段：长期愿景（2027+）

### 方向 1：全自主 AI 开发者

```
愿景: 用户描述业务目标 → AI 自主完成从架构到运维的全生命周期
  - 自主学习新技术栈
  - 自主发现和组合社区代理
  - 自主监控和优化生产系统
  - 人工仅参与业务决策审批
```

### 方向 2：去中心化 AI 市场

```
愿景: 基于代理信誉的去中心化 AI 服务市场
  - Agent 技能即服务（Skill-as-a-Service）
  - 代币激励机制（贡献技能/算力/数据）
  - 去中心化模型推理网络
  - 隐私计算（联邦学习 + TEE）
```

### 方向 3：硬件安全生态

```
愿景: 形成完整的硬件信任链生态
  - U盾 + SIMKey + TEE 三位一体信任根
  - 后量子密码全面迁移
  - 硬件安全模块开放标准
  - 卫星通信离线安全通道
```

### 方向 4：全球去中心化社交

```
愿景: 跨协议、跨平台、跨国界的去中心化社交网络
  - ActivityPub / Nostr / Matrix 多协议融合
  - 多语言 AI 实时翻译
  - 去中心化内容存储与分发（IPFS + Filecoin）
  - 抗审查通信（Tor + 卫星）
```

---

## 实施时间线

```
2026 Q1  ┃ v1.1.0-alpha ┃ v3.0 流水线框架 + v3.3 异常检测
         ┃              ┃ SIMKey FIDO2 原型
         ┃              ┃
2026 Q2  ┃ v1.1.0       ┃ v3.1 NL编程 + v3.2 多模态
         ┃              ┃ U盾跨平台驱动
         ┃              ┃ ActivityPub 双向互通
         ┃              ┃ SOC 2 合规包
         ┃              ┃
2026 Q3  ┃ v2.0.0-alpha ┃ v4.0 Agent DID + 联邦发现
         ┃              ┃ 跨组织任务路由原型
         ┃              ┃
2026 Q4  ┃ v2.0.0       ┃ v4.0 完整去中心化代理网络
         ┃              ┃ 信誉系统 + 生产加固
         ┃              ┃
2027+    ┃ v3.0.0       ┃ 全自主 AI + 去中心化市场
```

---

## 版本对比总表

| 版本   | 主题             | 核心技术                                  | 状态        |
| ------ | ---------------- | ----------------------------------------- | ----------- |
| v1.0.0 | 企业版发布       | 95技能, P2P社交, CRDT协作, 硬件安全       | ✅ 已发布   |
| v1.1.0 | 全栈智能化       | 全自动流水线, NL编程, 多模态, 自主运维    | 🔄 开发中   |
| v2.0.0 | 去中心化代理网络 | Agent DID, 联邦发现, 跨组织协作, 信誉系统 | 📋 规划中   |
| v3.0.0 | 全自主 AI 生态   | Skill-as-a-Service, 代币激励, 联邦学习    | 🔮 远期愿景 |

---

> 本文档持续更新。各模块详细规划请参阅：
>
> - [Cowork 路线图（v3.0-v4.0 详细设计）](/chainlesschain/cowork-roadmap)
> - [SIMKey 企业版](/chainlesschain/simkey-enterprise)
> - [EvoMap GEP 协议](/chainlesschain/evomap)
> - [更新日志](/changelog)
