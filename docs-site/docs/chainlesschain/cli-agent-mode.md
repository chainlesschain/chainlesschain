...
| 命令                                | 说明                                                                                                                                                     |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/context`                          | 按角色（system / user / assistant / tool）显示实时上下文窗口占用 + MCP tool schema 占用（per-server token/占比、tool search 状态与优化建议，v0.162.155） |
...
大规模 MCP 面（多 server、几十上百工具）下，工具 schema 超过窗口 10%（可配）时自动启用 **Tool Search**：schema 延迟装载、模型经内部 `tool_search` 工具按需检索，prompt cache 保持稳定；`CC_TOOL_SEARCH=1|0|auto` / settings `mcp.toolSearch` 控制。详见 [MCP 服务器管理](./cli-mcp#mcp-tool-search--大规模工具面的上下文治理v0162155)。
...
| `/hooks`                         | ✅      | `/hooks`（查看已加载的 settings.json hooks；observe-only DB hooks 用 `cc hook`）— _2026-06-28 补_    |
| `/todos`                         | ✅      | `/todos`（查看本会话 TODO 列表）— _2026-06-28 补_                                                    |
| `/add-dir`                       | ✅      | `/add-dir [dir]`（会话内加额外工作根；之前仅 `--add-dir` 旗标）— _2026-06-28 补_                     |
| `/init`                          | ✅      | `/init [--force]`（会话内盘点生成 cc.md；复用 `cc init` 同款离线 census）— _2026-06-28 补_           |
| `/agents`                        | ✅      | `/agents [show｜new <name>]`（会话内管理 `.chainlesschain/agents/*.md` 子代理定义）— _2026-06-28 补_ |
| `/status`                        | ✅      | `/status`（版本 / 模型 / 会话 / cwd / 工作根 / IDE·MCP·hooks 计数快照）— _2026-06-28 补_             |
| `/pr-comments`                   | ✅      | `/pr-comments [<n>]`（经 `gh` 拉 PR 评审/评论/行内评论，交 agent 处理）— _2026-06-28 补_             |
| `/release-notes`                 | ✅      | `/release-notes`（当前版本 + 更新日志链接 + 升级方式）— _2026-06-28 补_                              |
