...
在 `cc agent` 交互会话里，输入 `/` 触发 **TAB 补全**会列出全部内置斜杠命令。当前共 **41 个内置斜杠命令**，按类别整理如下（输入 `/help` 看会话内即时帮助）：
...
下表把 **Claude Code 的主要内置斜杠命令**逐条对到 cc 的现状。结论：cc 的斜杠命令**并不少**（41 个内置 + 自定义宏 + MCP prompts），在自主 / 多智能体 / provider 切换 / 跨会话搜索等维度**超出** Claude Code；真正的差距是少数几个，已整理成下面的「待补清单」逐个完善。
...
| `/hooks` | ❌ | cc 有 `cc hook` CLI + settings.json hooks，**无 `/hooks` REPL 管理命令** → 可补 |
| `/todos` | ❌ | cc 有 `todo_write` 工具，**无 `/todos` REPL 视图** → 可补 |
...
1. **`/hooks`**（❌）— REPL 内查看 / 管理 settings.json hooks（底层 `cc hook` 已具备）。
2. **`/todos`**（❌）— REPL 内查看当前会话 TODO 列表（`todo_write` 工具数据已在）。
3. **`/add-dir`**（🔶）— 会话中途加额外工作根（`--add-dir` 逻辑已有，包装成 REPL 命令）。
4. **`/init`**（🔶）— 会话内触发项目盘点（`cc init` 已有）。
5. **`/agents`**（🔶）— 交互式子代理配置 / 创建（当前 `/sub-agents` 仅查看）。
6. **`/pr-comments`**（❌）— 拉取并注入 GitHub PR 评论（新 feature）。
7. **`/status` / `/release-notes`**（🔶/❌，低优先）— 版本 / 环境摘要、发版说明。
