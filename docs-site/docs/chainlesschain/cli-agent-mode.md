...
在 `cc agent` 交互会话里，输入 `/` 触发 **TAB 补全**会列出全部内置斜杠命令。当前共 **46 个内置斜杠命令**，按类别整理如下（输入 `/help` 看会话内即时帮助）：
...
下表把 **Claude Code 的主要内置斜杠命令**逐条对到 cc 的现状。结论：cc 的斜杠命令**并不少**（46 个内置 + 自定义宏 + MCP prompts），在自主 / 多智能体 / provider 切换 / 跨会话搜索等维度**超出** Claude Code；真正的差距是少数几个，已整理成下面的「待补清单」逐个完善。
