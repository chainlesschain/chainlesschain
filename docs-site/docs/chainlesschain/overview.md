...
> **当前版本（2026-08-09）：v5.0.3.135 | CLI 0.163.2（npm latest / 源码包元数据 / 生产推荐一致）| PDH 0.4.57（92 个采集契约 / 18 类来源）| Agent SDK 0.1.7 | Open VSX 0.37.47 | JetBrains Marketplace 0.4.83 | Android versionCode 503135 | iOS build 135**
>
> **当前能力与发布边界**：CLI `0.163.2` 已公开显式 MCP runtime identity、Linux descriptor-bound npm/Node capsule、Windows 一次性 Broker authority 与 macOS typed fail-closed boundary，并承接 canonical session、checkpoint recovery、有界长会话和可复现 Web Panel 基线。生产安装建议固定 `chainlesschain@0.163.2`；同一精确 SHA 的三平台 CLI/Strict、两小时可靠性/恶意 MCP 门与 npm 发布/公网回读均成功。最新主线 `9fa5162e66` 的 filesystem/network MCP source policy 晚于 tag，尚非生产契约；unsigned native 证据也不等于签名 Desktop/native 发行。
>
