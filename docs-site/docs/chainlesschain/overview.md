...
> **当前版本（2026-08-27）：v5.0.3.135 | Agent Platform CLI 0.166.6（npm latest / 生产推荐）| Agent SDK 0.2.4 | Agent Protocol 0.1.5 | PDH 0.4.59（92 个采集契约 / 18 类来源）| Open VSX 0.37.71 | JetBrains Marketplace 0.4.100 | Android versionCode 503135 | iOS build 135**
>
> **当前能力与发布边界**：生产推荐与 npm `latest` CLI `0.166.6` 延续真实 Team child 消息、custody handoff、`SessionMessageFabric`、结构化审批与 Agent/Graph Kernel，37 类 Agent stream event 已从单一 Schema 生成 payload union 并通过跨端 causal conformance；Desktop/VS Code 固定能力 App Server pilot、强鉴权/TLS/有界背压的实验 WebSocket、1,800 秒 overload/RSS soak，以及 Agent IPC/legacy backlog 有界化均已闭环。精确提交 `f2a249bf3d` 的 CLI CI、Strict Sandbox 与独立 npm 制品/provenance 回读成功。Open VSX `0.37.71` 与 JetBrains `0.4.100` 已公开；JetBrains `0.4.102` 已上传待审。
>
