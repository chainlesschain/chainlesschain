...
> **当前版本: v5.0.3.40 进化版 | 141 桌面技能 + 28 Android 技能 | CLI v0.161.3 / 112 命令 / 14,800+ 测试 | V6 Chat-First 桌面壳（P0–P9c 全量落地 + LanguageSwitcher）| CLI V2 规范层 220+ 治理表面（iter16–iter28 · ~5,984 V2 测试）| cc pack 多平台打包（macOS / Windows / Linux + 项目模式 Phase 2a/2b/3a/3b + OTA check-update / SHA-256 / 自替换）| MTC v0.5→v0.11（联邦 M-of-N 多签 + libp2p gossipsub 自发现 + Ed25519/SLH-DSA 异构 + 跨联邦信任 + 多跳 + gas-aware + SLA 监控 + Op Log 行级徽标 + Q-ENG-2 后端桥；476+ 测试）+ in-process 状态查询（`mtc.audit-status` / `mtc.bridge-status` / `mtc.bridge-sla` 绕过 asar 冷启动）| Web Panel i18n M1–M3 + Express@5 ASAR hoist 修复 + B4 post-pack ASAR surgery（Win 安装 20m → ~5m）| **社区/频道跨机同步真正打通**（Phase A 修 7 个底层 bug + Phase B v1 MTC 联邦双轨 + B4 Ed25519 DID 签名 + 自动 MTC peer 桥接，22 个 p2p+social+mtc 文件 891 测试全绿）| Web Shell Phase 3c.7（截图识别 + 通知设置 + 托盘 5 个 quick-action 路由收口）| v5.0.3.40 MTC 视图 in-process 提速 + CI 三发解锁（macOS isSymlink / rules-validator __tests__ 排除 / Win cold-start 60s）**
