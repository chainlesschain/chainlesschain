...
> **当前版本（2026-08-28）：v5.0.3.135 | Agent Platform CLI 0.166.7（npm latest / 生产推荐）| Agent SDK 0.2.4 | Agent Protocol 0.1.5 | PDH 0.4.59（92 个采集契约 / 18 类来源）| Open VSX 0.37.72 | JetBrains Marketplace 0.4.103 | Android versionCode 503135 | iOS build 135**
>
> **当前能力与发布边界**：生产推荐与 npm `latest` CLI `0.166.7` 将 CLI graph、Team、distributed-team、Cowork、Scheduler 与 App Server 入口统一接入 Graph Kernel cutover ledger，并以耐久 writer/store/source/lease/fence evidence 约束恢复和接管；legacy mutation 失败闭合。精确提交 `19834a1845` 的三平台 CLI CI、Strict Sandbox、不可变制品、Trusted Publishing 与独立 npm provenance 回读成功；Open VSX `0.37.72` 与 JetBrains `0.4.103` 也已公开。发布后的 Desktop 源码另增加只读 Graph Run Debugger、签名外部 Skill 隔离、内置 Handler 能力目录与网络/模型/诊断 Broker；远端主线继续约束 Yjs 协作生命周期。这些后续源码状态不属于 `0.166.7` 制品。
>
