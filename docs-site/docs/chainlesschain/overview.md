...
> **当前版本: v5.0.2.34 进化版 | 139桌面技能 + 28 Android技能 | 102 Phase | AI Agent 2.0 + Web3深化 + 企业平台 + 自进化AI + CLI 109命令/7600+测试 + Agent Context Engineering + Autonomous Agent + EvoMap + 10 LLM Providers + 3 Proxy Relays + Task Model Selector + CLI-Anything Integration + WebSocket Server + Sub-Agent Isolation v2 + Phase 7-8 企业CLI + Persona系统 + Web UI管理面板 + AI Orchestration编排层 + Hashline哈希锚定编辑 + Skill-Embedded MCP + Category Routing类别路由 + Canonical Tool Descriptor工具描述规范 + Coding Agent Envelope Protocol + Sub-Runtime Pool子运行时池 + CLI V2 治理浪潮（iter16–iter28，136 Gov 表面）**

## 概述

ChainlessChain 是一个完全去中心化的个人 AI 管理系统，整合知识库管理、去中心化社交和交易辅助三大核心功能。系统通过 U-Key/SIMKey 硬件加密和本地 AI 模型提供军事级隐私保护，支持 139 个桌面内置技能、28 个 Android 技能、109 个 CLI 命令以及 14+ 云端 LLM 和本地 Ollama 模型集成。
...
- 🤖 **AI 智能引擎**: 16 个专用引擎 + 139 内置技能 + 14+ 云 LLM + Ollama 本地模型 + Cowork 多智能体 + AI Orchestration编排
...
┌───────────────────▼─────────────────────────┐
│              AI 智能层                        │
│  Cowork多Agent │ Skills(139) │ Plan Mode    │
...
| Vue组件数   | 564+个                                  |
| IPC处理器   | 735+ (+178 v5.0)                        |
| 测试用例    | 7,600+ (桌面+CLI+Android)               |
| AI专用引擎  | 16个                                    |
| 内置技能    | 139个                                   |
| 统一工具    | 60+ FunctionCaller + 8 MCP + 139 Skills |
| Pinia Store | 99个                                    |
| 数据库表    | 95+张                                   |
| CLI命令     | 109个                                   |
| CLI版本     | v0.156.6（CLI V2 治理浪潮 iter16–iter28：136 Gov 表面 / ~5,984 V2 测试 / 1,965 回归测试零失败） |
| 钩子事件    | 32个 (4种类型)                          |
| 开发阶段    | Phase 1-102 完成                        |
...
- 🧩 **技能系统**: 139个内置技能，Agent Skills开放标准，四层加载架构
...
可扩展的技能框架，139个内置技能覆盖开发、文档、安全、媒体等全场景。
...
- ✅ Cowork协作演化技能 (debate-review, ab-compare, orchestrate, verification-loop, stream-processor, 共139个内置技能)
...
- [AI技能系统](/chainlesschain/skills) - 139个内置技能
...
| 测试文件                                                                           | 覆盖范围                                        |
| ---------------------------------------------------------------------------------- | ----------------------------------------------- |
| ✅ `desktop-app-vue/tests/unit/ai-engine/skill-handlers.test.js`                   | 139 技能 Handler 端到端（200+ tests）           |
...
| ✅ `packages/cli/__tests__/` (全量)                                                | CLI 109 命令，9 技能包，管理 Agent（7600+ tests，含 iter16–iter23 V2 治理浪潮 ~2,800 V2 tests）|
...
- [AI 技能系统](/chainlesschain/skills) - 139 个内置技能完整列表
