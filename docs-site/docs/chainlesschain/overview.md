...
> **当前版本（2026-08-20）：v5.0.3.135 | CLI 0.165.4（npm latest / 主线包元数据 / 生产推荐一致）| PDH 0.4.59（92 个采集契约 / 18 类来源）| Agent SDK 0.1.7 | Open VSX 0.37.58 | JetBrains Marketplace 0.4.93 | Android versionCode 503135 | iOS build 135**
>
> **当前能力与发布边界**：CLI `0.165.4` 已公开耐久 workflow receipt/checkpoint/input、REPL/headless/WS 预算、远端结果审阅应用、Artifact access/deletion/cleanup settlement、governed plugin transaction 与无 native toolchain 首次安装，并承接 `0.165.1` 的动态准入、远程权威和 canonical 执行边界。生产安装建议固定 `chainlesschain@0.165.4`；精确 SHA `86abb5f65e` 的三平台 CLI/Strict、npm 发布与公网回读均成功。Open VSX `0.37.58` / JetBrains `0.4.93` 已公开；`HEAD@f2fbf331da46` 的 MCP native-addon policy、IDE/桌面/iOS 恢复能力与 P1-5 Marketplace 十二格供应链矩阵仍是发布后源码增量，P1-5 exact-head aggregate 尚待 Actions 回读。正式 Scheduler campaign、签名 native 与跨宿主 revoke 仍不能按稳定发行宣称。
>
