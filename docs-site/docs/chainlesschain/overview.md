...
> **当前版本: v5.0.3.45 进化版 | 141 桌面技能 + 28 Android 技能 | CLI v0.161.7 / 112 命令 / 14,800+ 测试 | **意图理解默认 opt-in（Chat 项目模式不再卡"理解中…"）+ cc ui QuickAsk llm.chat 路由对齐桌面 web-shell** | V6 Chat-First 桌面壳（P0–P9c 全量落地 + LanguageSwitcher + chat-panel-v5 Phase E 反向对齐）| CLI V2 规范层 220+ 治理表面（iter16–iter28 · ~5,984 V2 测试）| cc pack 多平台打包（macOS / Windows / Linux + 项目模式 Phase 2a/2b/3a/3b + OTA check-update / SHA-256 / 自替换）| MTC v0.5→v0.11（联邦 M-of-N 多签 + libp2p gossipsub 自发现 + Ed25519/SLH-DSA 异构 + 跨联邦信任 + 多跳 + gas-aware + SLA 监控 + Op Log 行级徽标 + Q-ENG-2 后端桥；476+ 测试）+ in-process 状态查询（`mtc.audit-status` / `mtc.bridge-status` / `mtc.bridge-sla` 绕过 asar 冷启动）+ **landmark.publisher_signature 验证（v5.0.3.43）**（LandmarkCache opt-in + real-verifier callers 启用 + M-of-N strip-all-sigs 对称化 — 单成员 sig 篡改不再炸 publisher_signature）| Web Panel i18n M1–M3 + Express@5 ASAR hoist 修复 + B4 post-pack ASAR surgery（Win 安装 20m → ~5m）| **社区/频道跨机同步 + B4 audit-grade 闭环**（Phase A + B v1 MTC 联邦双轨 + B4 Ed25519 DID 签名 + 自动 MTC peer 桥接 + B4-merkle envelope finality + B4-cross / cross-trust / archive / ui / mofn / crossfed / webshell / webpanel / mofn-sign v2 / cred-persist / auto-archive，§2.2.10 → §2.2.24 共 15 节，私钥 / 密码均不过线）| **chat-panel-v5 三壳对齐**（V5 / V6 / web-shell 聊天能力严格对等：流式 + 历史会话切换 + 上下文记忆引用 + 工具调用面板）| Web Shell Phase 3c.7（截图识别 + 通知设置 + 托盘 5 个 quick-action 路由收口）| **安全硬化级联（v5.0.3.43）**（HIGH 44→0 / MOD 4→0 / LOW 45→0：drop hdkey + werift + speedtest-net + jspdf · override serialize-javascript / tar / semver / undici / make-fetch-happen / tmp / ip-address / dompurify · channel-manager DDL 加固 · wrtc-compat ip.isPublic 补丁 CVE-2024-29415）| v5.0.3.40 MTC 视图 in-process 提速 + CI 三发解锁（macOS isSymlink / rules-validator __tests__ 排除 / Win cold-start 60s）**
