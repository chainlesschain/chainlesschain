...
const tasks = await window.electron.invoke(
  "coding-agent:list-background-tasks",
);
const task = await window.electron.invoke("coding-agent:get-background-task", {
  taskId,
});
const history = await window.electron.invoke(
...
内部门控由 `ws-session-manager.isReviewBlocking(sessionId)` 实现，agent handler 在执行 turn 前会优先检查。
