...
> **版本：公开 CLI `0.166.48`；请求级窗口增强已进入 `0.166.49` 源码候选 · 状态：✅ 只读报表 | 对已存无头会话按角色桶估算 token；候选版优先使用最近主模型请求记录的实际窗口 |**
>
...
> 上下文、压缩和持久记忆的关系，以及模块 108 已完成设计但尚未完成跨端切换的边界，见[上下文与记忆](./context-memory.md)。
...
- 公开版从**每模型的上下文窗口表**取窗口大小（不认识的模型走 provider 默认，再不行硬回退 32768）。`0.166.49` 源码候选会优先读取最近一个无 attribution 的主模型 `token_usage.contextWindow`；显式 `--model` / `--provider` 与记录不一致时仍重新解析，避免把旧窗口套到新模型。
- 显示每桶占 total 的份额、total 占窗口的百分比（绿 ≤70% / 黄 >70% / 红 >90%）、剩余余量，溢出时红字提示「需要压缩」。

它**不产生新数据**——只读会话已写下的事件。消息角色分桶仍使用启发式 token 估算；窗口分母在候选版可来自请求时持久化的实际配置，因此“估算的消息占用”和“请求实际容量”会明确分层。
...
- 🎯 **主请求窗口优先（0.166.49 源码候选）**：只消费无 `attribution` 且 `source` 为空或为 `model` 的最后一条用量，子 Agent 与语义压缩调用不会覆盖主窗口；持久值必须是 `1024..16777216` 的安全整数。
...
从 `0.166.49` 源码候选开始，解析还会纳入请求的 `baseUrl` 与 `contextMemoryModelWindowTokens`。下表是没有可信请求记录或用户显式切换模型/provider 时的回退依据，不再是唯一窗口来源。
...
| `__tests__/unit/context-recorded-window.test.js` | 2    | 最近主请求窗口优先 · attributed 子调用排除 · 显式模型覆盖时不用旧窗口                                                                            |
| `__tests__/unit/context-window-budget.test.js`   | 4    | endpoint/显式覆盖参数化解析 · 大窗口按 token 压力触发 · 小窗口保留消息数保护                                                                     |
