...
> **数据同步 | 状态: ✅ 生产就绪 | isomorphic-git | 去中心化版本控制 | 增量同步**

ChainlessChain使用Git作为核心同步机制，提供版本控制和跨设备数据同步。

## 概述

Git 同步模块基于 isomorphic-git（纯 JS 实现）提供去中心化的跨设备数据同步能力，支持完整版本历史、增量传输和智能冲突解决。系统支持 GitHub/GitLab/Gitee 等多平台托管，内置自动提交检测和 AI 辅助的 3-way merge 冲突解决，同时兼容 git-crypt 透明加密。

## 核心特性

- 🔄 **去中心化同步**: 无需中心服务器，设备间直连同步
- 📜 **完整版本历史**: 每次修改自动记录，支持任意版本回滚
- 🤝 **智能冲突解决**: 成熟的 3-way merge 算法 + AI 辅助
- 📦 **增量传输**: 仅传输变化的数据，节省带宽
- 🔐 **加密友好**: 支持 git-crypt 透明加密
- 🌐 **多平台托管**: GitHub、GitLab、Gitee 等免费私有仓库
- 🤖 **自动提交**: 智能检测变更并自动 commit
...
- [x] **Git Smart Protocol over WebRTC** — 在 WebRTC DataChannel 上实现 Git Smart HTTP 传输协议，复用 isomorphic-git 的 `http` 插件接口
- [x] **设备发现与配对** — 基于现有 libp2p 发现机制，自动发现局域网/互联网上的 ChainlessChain 设备
- [x] **双向引用协商** — 实现 `git-upload-pack` / `git-receive-pack` 的引用广告和对象协商，仅传输缺失对象
- [x] **离线队列与重连** — 复用 `P2PSyncEngine` 的离线队列机制（5秒重试间隔，最多5次指数退避），网络恢复后自动续传
- [x] **多设备拓扑同步** — 支持星型（一主多从）和网状（任意设备互同步）两种拓扑，通过向量时钟保证最终一致性
- [x] **带宽自适应** — 根据 WebRTC DataChannel 的可用带宽动态调整 packfile 分片大小（16KB-256KB）

#### 安全机制

- [x] **DID 身份验证** — 每次 P2P 连接使用 DID 签名握手，防止中间人攻击
- [x] **端到端加密** — Git 对象通过 DTLS（WebRTC 内置）加密传输，敏感文件额外使用 git-crypt
- [x] **设备授权白名单** — 只有通过 U盾/SIMKey 确认的设备才能参与 P2P 同步
...
- [x] **Thin Pack 传输** — 生成 thin packfile，仅包含接收方缺失的 delta 对象，减少冗余传输
- [x] **对象复用协商** — 推送前先交换 `have`/`want` 引用列表，精确计算最小传输集
- [x] **分层 Delta 链** — 对大文件（>1MB）构建多层 delta 链，每层压缩率 40-60%
- [x] **流式 Packfile** — packfile 边生成边传输，减少内存峰值占用（目标 <100MB）

#### 文件级差分

- [x] **rsync 算法集成** — 对大型二进制文件（数据库备份 `.db.enc`、多媒体文件）使用 rolling checksum + MD5 差分算法
- [x] **块级去重** — 4KB 固定块 + 内容定义分块（CDC）混合策略，跨文件去重
- [x] **压缩管线** — Delta → zstd 压缩 → 传输 → 解压 → 应用，端到端压缩率 >80%

#### 数据库差分同步

与现有 `DbSyncManager`（973 行）深度集成：

- [x] **行级变更追踪** — 基于 SQLite trigger 记录行级 INSERT/UPDATE/DELETE，替代全表时间戳扫描
- [x] **CRDT 数据结构** — 对高频冲突字段（计数器、集合）使用 CRDT，实现无冲突自动合并
- [x] **变更日志压缩** — 同一行的多次变更合并为最终状态，减少同步开销
...
- [x] **非重叠变更** — 不同区域的修改自动合并（已有 Git 3-way merge 基础）
- [x] **追加合并** — 双方都在文件末尾追加内容，自动按时间戳排列
- [x] **配置文件合并** — JSON/YAML 配置文件按 key 进行字段级合并，自动处理新增字段
- [x] **格式差异忽略** — 忽略空白、换行、缩进等格式差异，只关注实质内容变更

**Level 2: AST 合并（半自动，<2秒）**

- [x] **代码 AST 分析** — 对 JavaScript/TypeScript/Python 代码解析 AST，在语法树级别合并
- [x] **函数级合并** — 不同函数的修改自动合并，同一函数的修改提示用户
- [x] **import 语句合并** — 自动合并双方新增的 import 语句，去重排序
- [x] **Markdown 结构合并** — 按标题层级分段合并，同一段落的修改提示用户

**Level 3: LLM 语义合并（智能，<10秒）**

- [x] **意图理解** — 使用 Ollama 本地 LLM 分析双方修改的意图和目的
- [x] **语义合并建议** — LLM 生成合并建议，保留双方修改的核心语义
- [x] **笔记内容合并** — 对知识库笔记的内容冲突，AI 理解上下文后智能合并
- [x] **冲突解释** — 用自然语言向用户解释冲突原因和推荐的合并方案

#### 学习与优化

- [x] **用户选择学习** — 记录用户的冲突解决偏好（选择本地/远程/手动编辑的模式），训练个性化策略
- [x] **冲突模式库** — 积累常见冲突模式和最佳解决方案，优先匹配历史经验
- [x] **Permanent Memory 集成** — 将冲突解决经验写入 MEMORY.md，跨会话复用
...
- [x] **统一配置界面** — 选择平台后自动填充 API 端点、认证方式等参数
- [x] **OAuth 一键授权** — 集成现有 `OAuthProvider`（PKCE 支持），浏览器内完成授权流程
- [x] **SSH 密钥管理** — 自动生成 Ed25519 密钥对，一键添加到托管平台
- [x] **连接测试** — 自动验证认证、权限、仓库可达性
- [x] **多仓库镜像** — 一键配置同时推送到多个平台（数据冗余备份）
- [x] **平台迁移向导** — 从一个平台迁移到另一个平台，保留完整历史

#### 国内优化

- [x] **Gitee/Coding 加速** — 国内平台自动检测，优先使用国内 CDN 节点
- [x] **代理配置** — 内置 HTTP/SOCKS5 代理支持，解决 GitHub 访问慢的问题
- [x] **镜像仓库** — 国外主仓库 + 国内镜像仓库，自动双向同步
...
- [x] **Yjs CRDT 集成** — 基于 Yjs 库实现文档的 CRDT 表示，支持并发编辑无冲突自动合并
- [x] **WebRTC Provider** — 通过现有 `webrtc-data-channel.js` 传输 Yjs 更新消息，P2P 直连无需服务器
- [x] **操作转换** — 实时将用户输入转换为 CRDT 操作（insert、delete、format），毫秒级同步
- [x] **光标感知** — 显示其他协作者的实时光标位置和选区，带用户颜色标识

**Markdown 协作**

- [x] **结构化编辑** — Markdown 文档映射为 Yjs 结构化数据（标题、段落、列表、代码块），支持块级并发编辑
- [x] **富文本同步** — 加粗、斜体、链接等格式标记的实时同步
- [x] **图片与附件** — 协作中插入的图片通过 P2P 传输，自动添加到 Git 仓库
- [x] **评论与标注** — 类似 Google Docs 的行内评论功能，评论数据通过 CRDT 同步

**会话管理**

- [x] **协作房间** — 基于 libp2p Topic 创建文档协作房间，支持邀请链接
- [x] **权限控制** — 集成 `PermissionEngine` RBAC，支持只读/编辑/管理三级权限
- [x] **在线状态** — 显示协作者在线/离线/编辑中状态
- [x] **离线编辑** — 断网后继续编辑，重新上线时 CRDT 自动合并离线期间的变更

**Git 集成**

- [x] **自动提交** — 协作编辑的变更每隔 N 分钟自动提交到本地 Git（复用 `git-auto-commit.js`）
- [x] **版本快照** — 重要编辑节点自动创建 Git tag，方便回溯
- [x] **协作历史** — 记录每个变更的作者信息，Git blame 可追溯到具体协作者
- [x] **冲突预防** — CRDT 保证实时编辑无冲突，Git 层面的冲突通过 AI 智能合并处理
...
- `desktop-app-vue/src/main/git/` — isomorphic-git 集成
- `packages/cli/src/commands/git.js` — CLI Git 命令
- `packages/cli/src/lib/git-integration.js` — Git 集成库

## 相关文档

- [知识库管理](./knowledge-base) — 知识库数据管理
- [配置说明](./configuration) — 同步配置项
- [安装部署](./installation) — Git 环境检查
