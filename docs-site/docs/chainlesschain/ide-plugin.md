...
> **当前推荐组合（2026-08-31）：CLI `0.166.14` + VS Code 扩展 `0.37.76`（Open VSX）+ JetBrains 插件 `0.4.106`（Marketplace）。两端保持只读投影与 CLI authority，并推荐已完成三平台门禁的 CLI。CLI 发布 SHA 为 `ee88125256`；双 IDE 标签使用独立发布 SHA `0f0b9f7c8c`，不能混成同一个制品身份。**
>
...
>
> **发布提示**：Open VSX `0.37.76` 已公开回读且累计下载突破 **3.3 万**；JetBrains `0.4.106` 已由 Marketplace API 回读为 approved/listed。npm `latest` CLI `0.166.14` 已完成精确 SHA 的三平台完整门禁、发布与独立 registry/provenance 回读。微软 VS Code Marketplace 与 JetBrains 作者签名仍未完成。
...
```bash
npm i -g chainlesschain@0.166.14 # 需要 Node ≥ 22.12.0；当前完整门禁基线
...
- **已发布到 [Open VSX Registry](https://open-vsx.org/extension/chainlesschain/chainlesschain-ide)**（扩展 ID `chainlesschain.chainlesschain-ide`，需 VS Code ≥ 1.85）。在使用 Open VSX 的编辑器里，扩展面板搜 **ChainlessChain IDE** 一键安装。
  > 官方 VS Code Marketplace（marketplace.visualstudio.com）**暂未上架**。官方版 VS Code 不查询 Open VSX，不要点 Open VSX 的通用 **Install** 链接；请直接下载 [0.37.76 VSIX](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.76/file/chainlesschain.chainlesschain-ide-0.37.76.vsix)，再运行 **Extensions: Install from VSIX...**。也可从源码打包：
  ```bash
...
当前 VS Code `0.37.76` 已从 Open VSX 公开回读，JetBrains `0.4.106` 已由 Marketplace API 回读为 approved/listed。两端消费 Protocol `0.1.7` 的生成 payload union，并推荐 CLI `0.166.14`；IDE 继续只提交宿主已审阅决定、消费无正文投影，不重建 CLI writer。消息正文/摘要、attempt/agent 身份、artifact id 和 authority digest 不进入 Webview。微软 VS Code Marketplace 仍不能扩写为已经发行。
...
- **结构化授权（VS Code 0.37.76）**：单次批准仍是快速路径；展开后可选当前回合、当前会话、拒绝或取消。回合/会话 grant 只采用 CLI 请求中的 exact capability/scope/binding，Webview 不能扩权。
...
- **CLI-owned Sessions Workbench（公开版 VS Code 0.37.76 / JetBrains 0.4.106）**：会话列表只消费 CLI 生成的 immutable projection revision；resume、attach、delivery 与 remote-control 动作必须由该 revision 明确声明，过期按钮失败闭合。公开版显示跨会话消息的 delivered/refused/full/expired 结果，并包含分组、多选批量移动与 Focus View。
- **生成事件与无正文协作投影（VS Code 0.37.76 / JetBrains 0.4.106）**：双端使用 Schema 生成的 known-event 类型并保留未知未来事件；VS Code 额外显示 canonical message/follow-up 与 custody handoff 计数。malformed、oversize 或 duplicate 可选投影失败闭合，发送、接收、ACK 与 handoff authority 始终留在 CLI-owned agent bridge。
