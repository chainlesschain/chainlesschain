...
> **当前公开组合（2026-09-20）：CLI `0.166.68` + VS Code 扩展 `0.37.110`（Open VSX）+ JetBrains 插件 `0.4.131`（Marketplace）。CLI 制品来自 `815fdbc0c4`，两个 IDE 制品来自 `5860f1e4a4`，均已完成公开回读；npm tarball、VSIX 与 JetBrains ZIP 仍是独立制品身份。**
>
...
>
> **发布提示**：Open VSX `0.37.110` 与 JetBrains Marketplace `0.4.131` 已公开并推荐 npm `latest` CLI `0.166.68@815fdbc0c4`。浏览器动作现由显式 authority 管理，下载先进入耐久隔离区；13 个子包完成字节审计并先发布有变化的包。IDE 精确提交门和最新 11 单元 ARM64 真机矩阵均成功。微软 VS Code Marketplace 因未配置 `VSCE_PAT` 仍未公开。详见[发布与升级指南](/chainlesschain/agent-platform-release)。
...
VS Code / VSCodium 扩展 `0.37.110` 支持 `chainlesschain.chat.maxTurns`。默认 `0` 跟随 CLI 交互默认；设为正整数可限制每条消息的模型轮次。设置改变后在下一个空闲回合重启聊天宿主并保留会话历史，不会打断正在执行的任务或待处理审批。它是 VS Code 设置，JetBrains `0.4.131` 没有同名设置。CLI `0.166.68` 同时承接读取游标恢复、重复输出抑制、长时间探索后的聚焦恢复、浏览器动作治理、可靠 Stop 与受治理演进部署配置。

当 IDE 显示 `Repeated file reads or identical large command outputs` 时，`0.166.68` 继承恢复暂停与真实观察分离的行为，不再将控制结果计入产生暂停的重复读取/输出计数；已知目标仍可用显式小范围继续读取。真正重复整段输出、无关远端枚举或连续六次真实无进展仍会停止，并明确保留“任务未完成”状态。
...
```bash
npm i -g chainlesschain@0.166.68 # 需要 Node ≥ 22.12.0；当前 npm latest
...
- **已发布到 [Open VSX Registry](https://open-vsx.org/extension/chainlesschain/chainlesschain-ide)**（扩展 ID `chainlesschain.chainlesschain-ide`，需 VS Code ≥ 1.85）。在使用 Open VSX 的编辑器里，扩展面板搜 **ChainlessChain IDE** 一键安装。
  > 官方 VS Code Marketplace（marketplace.visualstudio.com）**暂未上架**。官方版 VS Code 不查询 Open VSX，不要点 Open VSX 的通用 **Install** 链接；请直接下载 [0.37.110 VSIX](https://open-vsx.org/api/chainlesschain/chainlesschain-ide/0.37.110/file/chainlesschain.chainlesschain-ide-0.37.110.vsix)，再运行 **Extensions: Install from VSIX...**。也可从源码打包：
  ```bash
...
当前 VS Code `0.37.110` 已从 Open VSX 公开回读并推荐 CLI `0.166.68`；JetBrains `0.4.131` 已完成 Marketplace 公共回读并推荐同一 CLI。IDE 继续只提交宿主已审阅决定、消费有界投影，不重建 CLI writer；Workbench 的批准/拒绝/回滚、PM readiness 与 Skill Retrieval 的结果必须由 CLI/部署宿主验证。微软 VS Code Marketplace 仍不能扩写为已经发行。
...
- **任务记录与新会话接力（VS Code 0.37.110 / JetBrains 0.4.131）**：运行事件自动写入 Context/Memory Kernel 检查点并投影为有界 `WORKLOG.md`。VS Code `↗` / **Continue in New Conversation with Task Notes** 与 JetBrains **Continue in new chat** 会先保存，再把已校验历史交给独立新会话；不复制完整聊天或临时权限。完整说明见[任务记录用户文档](/chainlesschain/ide-task-worklog)。
- **PM readiness 与恢复链（VS Code 0.37.110 / JetBrains 0.4.131）**：只读展示 CLI-owned 的预算、签名 authority、SQLite seal、transition recovery 与 snapshot-store 配置状态。插件不接收 runner、grader、数据库字节、Ledger 或 durability authority；`readyForExecution:false` 与 automatic promotion `HOLD` 仍是公开版本的预期结果。
...
| 接力按钮不可用或提示 CLI 过旧          | 升级到 CLI `0.166.68` 和配套插件；发送一条消息完成 init 后重试                                    |
