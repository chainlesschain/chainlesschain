...
- 🔘 **系统级来电界面（CallStyle）**：锁屏 / 熄屏 / 通知栏渲染醒目的「接听 / 拒绝」按钮（即便全屏 intent 被 Android 14+/MIUI 降级，按钮仍在），不依赖手动授予「后台弹出」权限也能接听。
- 📵 **未接来电通知**：来电未接通（对端放弃 / 超时，非本端拒接）→ 持久「未接语音 / 视频通话」通知，点击进入。
- 🌐 **断网重连宽限**：通话中网络瞬断（ICE DISCONNECTED）进入 20s 重连宽限，等 WebRTC 自动恢复；超时未恢复才结束 —— 不再卡死在「通话中」无声音。
...
- 合计 **44 个 JVM 测试全绿**（状态机 21 / 信令 7 / 启动器集成 8 / 握手 e2e 2 / 通话记录 6，非 flaky 多轮复跑）。**真机验证（amethyst↔chopin）**：发起 / 来电响铃 / 接听 / **双向语音通话**（已实测可正常通话）/ 通话记录 / 来电铃声均已通过；视频画面 + 锁屏全屏来电仍在真机验收中。
