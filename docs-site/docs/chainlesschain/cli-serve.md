...
> 适用版本：生产推荐与 npm `latest` 均为 `chainlesschain@0.166.20`（精确发布 SHA `75a3339714`）。`chainlesschain serve` 默认启动既有 WebSocket Gateway；`--app-server` 默认启动 stdio JSON-RPC，显式加 `--app-server-websocket` 才启动独立的实验 App Server WebSocket。两个 WS 协议面不可混用，App Server 详见[独立指南](./cli-app-server.md)。
