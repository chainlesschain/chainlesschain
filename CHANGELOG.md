...
## [in-progress] - 2026-05-21 — iOS PIN-unlock crash hotfix (round 2) + Android quarantine 7 → 0 + Plan A real-device closure

> 5 commits in flight after v5.0.3.74 GitHub Release. **Untagged, no .ipa shipped.** Direction note only — folds into the next tag once .ips definitively pins the iOS root cause. `productVersion` already bumped to v5.0.3.75 (release.yml CI will publish a GitHub Release); release-sizes unchanged.
