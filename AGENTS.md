...
- Root is a JavaScript/TypeScript monorepo. Primary app lives in `desktop-app` (Electron + React + TS) with `src/main` for the Electron process, `src/renderer` for UI, `src/shared` for cross-process types, and `src/preload` for the bridge.  
...
- Install deps: `npm install` (root) plus `npm install` in `desktop-app` if not using workspaces.  
- Run desktop in dev: `npm run dev:desktop` (root) or `npm run dev` inside `desktop-app` to start main + renderer watchers.  
...
- Build desktop: `npm run build:desktop`; package artifacts: `npm run package --workspace=desktop-app`.  
...
- Lint with ESLint + TS/React rules (`npm run lint` or `npm run lint --workspace=desktop-app`). Fix warnings before PR.  
