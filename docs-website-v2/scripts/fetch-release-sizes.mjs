...
function inferProductVersion(assets = []) {
  for (const asset of assets) {
    const name = asset?.name || "";
    const match = name.match(
      /(?:ChainlessChain-(?:Setup-|Portable-)?|chainlesschain-desktop-vue[_-])(\d+\.\d+\.\d+)(?:-alpha[.-](\d+))?/i,
    );
    if (match) {
...
async function fetchPublishedCliVersion() {
  try {
    ...
    console.warn(
      `[fetch-release-sizes] npm CLI version fetch failed: ${err.message}`,
    );
    return null;
  }
}
...
async function fetchByProductVersion() {
  ...
  if (res.status === 404) {
    console.warn(
      `[fetch-release-sizes] productVersion ${tag} not published yet (404); falling back to list`,
    );
    return null;
  }
  if (!res.ok) throw new Error(`HTTP ${res.status} ${res.statusText}`);
  ...
  if (rel.draft || !(rel.assets || []).some((a) => classify(a.name))) {
    console.warn(
      `[fetch-release-sizes] ${tag} has no desktop assets yet; falling back to list`,
    );
    return null;
  }
  console.log(
    `[fetch-release-sizes] picked ${rel.tag_name} (by productVersion tag)`,
  );
  return rel;
}
...
async function main() {
  ...
  try {
    ...
    if (!data) {
      ...
      if (!data)
        throw new Error(
          `no release with desktop assets in first ${list.length}`,
        );
      console.log(
        `[fetch-release-sizes] picked ${data.tag_name} (list sweep; skipped internal-binaries-* + drafts)`,
      );
    }
  } catch (err) {
    ...
    if (cached && cached.tag && !cached.tag.startsWith("internal-binaries-")) {
      console.warn(
        `[fetch-release-sizes] keeping cache (tag=${cached.tag}, fetchedAt=${cached.fetchedAt})`,
      );
      return;
    }
    console.warn(
      `[fetch-release-sizes] no usable cache; writing empty placeholder`,
    );
    await fs.mkdir(path.dirname(OUT), { recursive: true });
    await fs.writeFile(
      OUT,
      JSON.stringify(
        { tag: null, fetchedAt: null, cliVersion: null, sizes: {} },
        null,
        2,
      ) + "\n",
    );
  ...
  const out = {
    tag: data.tag_name,
    productVersion: inferProductVersion(data.assets) || data.tag_name,
  ...
  console.log(
    `[fetch-release-sizes] wrote ${Object.keys(sizes).length} sizes for ${data.tag_name} (cli ${cliVersion || "n/a"})`,
  );
